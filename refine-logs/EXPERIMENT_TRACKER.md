...
| P5-COMMON-PROTOCOL | Final common admission/measurement/drain protocol | P5.5 BLOCKED / QUEUE-SEMANTICS QC CHECKER CORRECTION PREREGISTERED | 0/90 canonical online; 2/2 first-run attempts quarantined; 9/9 tapes; 90/90 references | Formula-faithful NSESche plus method-neutral FCFS admission, capacity-proportional active cohort, explicit phases, common runtime, complete accounting, and all twelve protocol gates | First row completed twice but the legacy QC branch rejected the frozen reviewer-v4 queue-semantics label; retain both attempts and correct only the version-aware checker before the same run's final attempt; see `P5_ONLINE_QUEUE_SEMANTICS_QC_CORRECTION_PREREGISTRATION.md` |
