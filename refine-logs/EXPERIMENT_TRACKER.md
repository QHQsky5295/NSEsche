...
| P5-COMMON-PROTOCOL | Final common admission/measurement/drain protocol | P5.2 BLOCKED / PRE-RESULT ADAPTER CORRECTION FROZEN / NO ONLINE SAMPLING | 0/90 online; 0/9 tapes; 0/90 references; 3/3 old pre-launch attempts failed | Formula-faithful NSESche plus method-neutral FCFS admission, capacity-proportional active cohort, explicit phases, common runtime, complete accounting, and all twelve protocol gates | The first key produced no tape or summary because the adapter still allowed only reviewer-v3; all old attempts are quarantined and exhausted; only the exact v3/v4 allowlist correction and complete refreeze are authorized; see `P5_ADAPTER_VERSION_ALLOWLIST_CORRECTION_PREREGISTRATION.md` |
