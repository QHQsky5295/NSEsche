...
| P5-COMMON-PROTOCOL | Final common admission/measurement/drain protocol | P5.5 RESUME CONTROL AUDITED / EXPLICIT SAME-RUN ATTEMPT 3 AUTHORIZED AFTER AUDIT COMMIT | 0/90 canonical online; 2/2 first-run attempts quarantined; attempt 3 absent; 9/9 tapes; 90/90 references | Formula-faithful NSESche plus method-neutral FCFS admission, capacity-proportional active cohort, explicit phases, common runtime, complete accounting, and all twelve protocol gates | Fail-closed one-run resume passes 291/291 protocol and 223/223 analysis tests and dry-validates both immutable attempts without ledger mutation; next is only the explicit first-row attempt 3; see `P5_ONLINE_CORRECTED_QC_RESUME_AUDIT.md` |
