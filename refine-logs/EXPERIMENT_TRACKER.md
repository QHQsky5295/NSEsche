...
| P5-COMMON-PROTOCOL | Final common admission/measurement/drain protocol | CORRECTED ZERO-RESULT PROTOCOL FROZEN / P5.2 INPUT CAPTURE AUTHORIZED / NO ONLINE SAMPLING | 0/90 online; 0/9 corrected tapes; 0/90 references; 3/3 old pre-launch attempts retained | Formula-faithful NSESche plus method-neutral FCFS admission, capacity-proportional active cohort, explicit phases, common runtime, complete accounting, and all twelve protocol gates | Source `3de688c` accepts exact v3/v4 adapter protocols; 285/285 protocol tests and the corrected zero-result manifest pass; next is nine fixed-order input captures only; see `P5_ADAPTER_VERSION_ALLOWLIST_CORRECTION_AUDIT.md` |
