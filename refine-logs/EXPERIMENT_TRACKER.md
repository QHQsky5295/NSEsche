...
| P5-COMMON-PROTOCOL | Final common admission/measurement/drain protocol | P5.5 QC CORRECTION AUDITED / SAME FIRST RUN ATTEMPT 3 AUTHORIZED AFTER AUDIT COMMIT | 0/90 canonical online; 2/2 first-run attempts quarantined; 9/9 tapes; 90/90 references | Formula-faithful NSESche plus method-neutral FCFS admission, capacity-proportional active cohort, explicit phases, common runtime, complete accounting, and all twelve protocol gates | Version-aware queue-semantics QC correction passes 289/289 protocol and 223/223 analysis tests without changing frozen inputs; next is only the same first row's final attempt; see `P5_ONLINE_QUEUE_SEMANTICS_QC_CORRECTION_AUDIT.md` |
