...
| M1-DYNAMIC | Fresh-bank dynamic-contention guard | SCREEN COMPLETE / TERMINAL FAILURE | 90/90 screen; 0/1200 forbidden qualification | Dynamic guard wins global screen, then six-cell dual-first qualification | Three fixed high-load rows had zero completions and undefined QPR; frozen screen could not rank; family rejected; post-screen evidence localizes the bottleneck to serviceability/admission rather than Nash feasibility or placement ranking alone; see `M1_DYNAMIC_CONTENTION_GUARD_RESULT_AUDIT.md` and `M1_DYNAMIC_CONTENTION_TERMINAL_DIAGNOSIS.md`; no M2 run authorized |
