...
| P5-COMMON-PROTOCOL | Final common admission/measurement/drain protocol | P5.5 FIRST CANONICAL VALIDATED / REMAINING 89 ONLINE RUNS AUTHORIZED AFTER AUDIT COMMIT | 1/90 canonical online; 2 first-run attempts quarantined; 9/9 tapes; 90/90 references | Formula-faithful NSESche plus method-neutral FCFS admission, capacity-proportional active cohort, explicit phases, common runtime, complete accounting, and all twelve protocol gates | Exact first row canonicalized on its remaining attempt 3 and passes independent identity/QC/inventory validation; continue the remaining fixed-order rows without the resume option; see `P5_FIRST_ONLINE_CANONICAL_INTEGRATION_AUDIT.md` |
...
P5 pre-result adapter-version failure and correction freeze (2026-09-05):
after audit commit `24985f6`, the first input-only key exhausted the generic
three-attempt budget before simulator launch. Every attempt exited 2 with the
same stale Python-adapter requirement for `reviewer-v3`; P5 correctly uses
`reviewer-v4`. No tape, catalog, summary, canonical capture, online method,
rank, or metric exists, and the remaining eight keys were not launched. The
old attempt tree and four-event ledger remain immutable. The only authorized
change is to accept exactly reviewer-v3 or reviewer-v4 in the adapter's
workload-profile preflight while preserving every other identity check. A new
source commit, release, zero-result manifest, and audit are required before a
new source-bound capture pass; this is not a fourth old-manifest attempt. See
`P5_ADAPTER_VERSION_ALLOWLIST_CORRECTION_PREREGISTRATION.md`.

P5 adapter correction and corrected zero-result refreeze (2026-09-05):
preregistered commit `4e70fc3` and implementation commit `3de688c` change only
the adapter protocol allowlist from reviewer-v3 to the exact v3/v4 set. New
v3/v4/unknown-version tests pass, and the full protocol suite passes 285/285.
The old 16-file, 59,089-byte failure tree remains exhausted with ledger hash
`842493b1...de0d` and tip `bd905e58...e1b5`; it contains no tape or summary.
The corrected 5,013,504-byte release has SHA-256 `0d802d6f...61676`. Its new
2,217,365-byte zero-result manifest has file/object hashes
`068639a8...e3036`/`259aa0b4...e9bb`, exactly 90 unique runs/specs, nine tape
keys, 90 reference keys, and no bound input or result. After the correction
audit commit, only the nine fixed-order input captures in the new workspace
are authorized. See `P5_ADAPTER_VERSION_ALLOWLIST_CORRECTION_AUDIT.md`.

P5 second pre-result integration failure and correction freeze (2026-09-05):
the first key in the `3de688c` instance exhausted three attempts after adapter
verification and simulator launch but before reset, each on
`KeyError: 'dag_type'`. No tape, catalog, summary, canonical capture, result,
rank, or metric exists; the other eight keys did not launch. Source inspection
finds that P5 replacement of the base `simulation` object omitted the complete
adapter-consumed set: `dag_type=mix`, `cold_start=high`, and `fn_type=cpu`, all
already frozen in the base protocol. The sole authorized change is to restore
those exact fields and add complete reset-payload/mutation tests before a new
source/release/manifest/audit freeze. Both earlier attempt trees remain
exhausted. See
`P5_SIMULATION_FIELD_COMPLETENESS_CORRECTION_PREREGISTRATION.md`.

P5 complete-fields correction and third zero-result freeze (2026-09-05):
commit `11e682f` restores only the inherited `dag_type=mix`,
`cold_start=high`, and `fn_type=cpu` identities. Directed 19/19 and full
protocol 286/286 tests pass, including complete reset-payload construction and
three independent mutation failures. Both earlier three-attempt workspaces
remain exhausted and input-free. The 5,013,504-byte release has SHA-256
`128f895f...43a8d`. The 2,224,993-byte manifest has file/object hashes
`64c39bd9...1562c`/`bd08b507...67412`, 90 unique runs/specs, nine tape keys,
90 reference keys, exact `mix/high/cpu`, and no bound input or result. After
the audit commit, only its nine fixed-order input captures are authorized. See
`P5_SIMULATION_FIELD_COMPLETENESS_CORRECTION_AUDIT.md`.

P5 third pre-result integration failure and correction freeze (2026-09-05):
the first key in the `11e682f` instance exhausted three attempts at Rust reset
validation because the generic input-capture clone combined
`workload.mode=capture` with reviewer-v4 admission enabled. The retained
19-file, 66,013-byte tree has no tape, catalog, summary, canonical capture,
result, rank, or metric; the other eight keys did not launch. Its ledger hash
is `3e45c5ab...79fcf` with tip `174bcb94...d1b3`. The sole authorized change
normalizes only reviewer-v4 input-capture clones to the established
reviewer-v3/admission-disabled capture contract and records both source and
capture protocol identities. Formal P5 manifest/reference/online runs remain
...
P5 offline-reference closure (2026-09-05): all 90 method-state references
canonicalized on attempt 1 in manifest order with no retry or quarantine.
Independent streaming inspection revalidated all 181,568 state-pair rows and
matched every table, state-pair sequence, assignment sequence, build spec,
tape, config, process, summary, receipt, and catalog identity. The 1,350-file
canonical tree contains 3,788,381,881 bytes with inventory hash
`1bb69fea...123c`. The 90-event ledger has hash/tip
`d26d7bdd...efa0`/`926d7fa8...8360`. The 2,770,804-byte ready manifest has
file/object hashes `7f9720e9...3d19`/`0a02d480...e3ee`, 90 unique run specs,
90 unique reference hashes, and tapes/model/references = true/true/true. No
online result, selection, or analysis exists. After the reference audit
commit, only the result-free 90-run selection/analyzer freeze is authorized.
See `P5_COMMON_PLATFORM_OFFLINE_REFERENCE_AUDIT.md`.

P5 analyzer and online-selection freeze (2026-09-05): source commit `7f4bfcb`
adds only a result-blind selection builder/validator around the pre-existing
twelve-condition gate. Directed tests pass 13/13 and the full analysis suite
passes 223/223. The 37,022-byte analyzer has SHA-256
`de087cdf...d1f66`. The 50,808-byte selection has file/document hashes
`e70dd418...0d8f`/`b352ab18...25ed`, exactly 90 manifest-order rows, 90
unique run specs/references, nine tape hashes, and no result/metric/rank
field. The online parent did not exist at freeze. The P5.5 wording is
corrected pre-result to load-major, seed-major, method-ordinal, matching the
manifest, all completed input stages, and the paper-order requirement. After
the audit commit, exactly one complete 90-run online batch is authorized; the
predeclared duplicate follows its canonical observation. See
`P5_COMMON_PLATFORM_ANALYZER_SELECTION_AUDIT.md`.

P5 first-online-row technical stop (2026-09-05): the low/P5P01/greedy row
completed twice, but both attempts were quarantined with the same sole issue,
`queue_semantics_mismatch`. The generic QC path still required
`unbounded_wait_by_design`, whereas the frozen reviewer-v4 P5 contract emits
`external_fcfs_bounded_active_dag_plus_node_task_queue`. No canonical result
exists. Both attempts and the seven-event ledger are retained. A narrowly
versioned QC correction is preregistered; no manifest, binary, seed, method,
algorithm, admission, timing, result threshold, or selection change is
authorized. The same row's attempt 3 remains blocked until a correction audit
is committed. See
`P5_ONLINE_QUEUE_SEMANTICS_QC_CORRECTION_PREREGISTRATION.md`.

P5 queue-semantics QC correction audit (2026-09-05): implementation commit
`dfdf87e` changes only the version-aware expected label and directed tests.
The protocol suite passes 289/289, the analysis suite 223/223, and both
retained summaries revalidate with zero issues while preserving hashes
`bd2a0c30...c828` and `d9eeb1c6...ad0a`. The ready manifest, selection, and
binary retain hashes `7f9720e9...3d19`, `e70dd418...0d8f`, and
`945e0dec...62d8`. After the audit commit, only the same first row's remaining
attempt 3 is authorized. See
`P5_ONLINE_QUEUE_SEMANTICS_QC_CORRECTION_AUDIT.md`.

P5 corrected-QC resume stop (2026-09-05): the post-audit ordinary invocation
consumed no attempt because the runner correctly re-detected the two stored
identical failure reports. It appended only ledger events 8--10; canonical
count remains zero and attempt 3 is absent. A one-run, explicit, fail-closed
resume control is preregistered. It must revalidate both immutable attempts
under the current QC source and bind the correction audit before appending an
authorization event and launching attempt 3. No result value participates.
See `P5_ONLINE_CORRECTED_QC_RESUME_PREREGISTRATION.md`.

P5 corrected-QC resume-control audit (2026-09-05): implementation commit
`3fb43c5` leaves ordinary blocking intact and adds an exact one-run explicit
path. Directed recovery tests pass, the protocol suite passes 291/291, and the
analysis suite passes 223/223. The real dry validation binds attempts 1--2,
current QC, and correction-audit evidence without using metrics; evidence hash
is `9839add4...95e8`. The real ledger remains ten events and attempt 3 is
absent. After the audit commit, only the exact first row may use the explicit
resume option once. See `P5_ONLINE_CORRECTED_QC_RESUME_AUDIT.md`.

P5 first online canonical integration (2026-09-05): the exact first row used
only attempt 3 and became canonical after the explicit audit-bound resume.
Independent validation passes with zero QC issues, exact run/tape/reference
identity, 16 files, and inventory hash `30995bb4...725`. The ledger has 15
