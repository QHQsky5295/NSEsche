...
| P5-COMMON-PROTOCOL | Final common admission/measurement/drain protocol | P5.5 BLOCKED / EXPLICIT CORRECTED-QC RESUME CONTROL PREREGISTERED | 0/90 canonical online; 2/2 first-run attempts quarantined; attempt 3 absent; 9/9 tapes; 90/90 references | Formula-faithful NSESche plus method-neutral FCFS admission, capacity-proportional active cohort, explicit phases, common runtime, complete accounting, and all twelve protocol gates | Ordinary resume consumed no attempt because the retained failure signature remains locked; add only an audited one-run recovery control that revalidates both immutable attempts before attempt 3; see `P5_ONLINE_CORRECTED_QC_RESUME_PREREGISTRATION.md` |
