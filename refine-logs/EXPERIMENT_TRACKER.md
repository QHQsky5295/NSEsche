...
| P5-COMMON-PROTOCOL | Final common admission/measurement/drain protocol | P5.2 BLOCKED / SECOND PRE-RESULT CORRECTION FROZEN / NO ONLINE SAMPLING | 0/90 online; 0/9 tapes; 0/90 references; 6 exhausted technical attempts across two source instances | Formula-faithful NSESche plus method-neutral FCFS admission, capacity-proportional active cohort, explicit phases, common runtime, complete accounting, and all twelve protocol gates | The v4 adapter instance reached `_full_config` and exposed omitted inherited `dag_type/cold_start/fn_type` fields before reset; only their exact restoration, tests, and complete refreeze are authorized; see `P5_SIMULATION_FIELD_COMPLETENESS_CORRECTION_PREREGISTRATION.md` |
