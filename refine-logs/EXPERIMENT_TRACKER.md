...
| G0-EQUATIONS | Paper Eqs. (1)--(20), code and log alignment | AUDIT COMPLETE / STRICT-CANDIDATE REFREEZE REQUIRED | 0 | Formal candidate uses strict Eq. (15), reference/gap bases and Eq. (14) proxy are disclosed | Eqs. (1)--(14), (16)--(20) map to shared code; 5%/15% guarded variants relax Eq. (15) and are ineligible for the claimed PNE proof; corrected candidate set is `ready_order`, `ready_finish_tie`, `formula`; see `G0_PAPER_CODE_EQUATION_ALIGNMENT.md` |
| G0-FEEDBACK | Eq. (16)--(20) control-path observability | IMPLEMENTED / CORRECTED-RUNTIME REPLAY REQUIRED | 0 | Control gap, gamma, price multiplier and outer assignment trace revalidate exactly from real logs | commits `cafb7c5`/`6e5643e` separate control/report gaps and enforce strict Eq. (15), price-basis, beta-domain and Eqs. (16)/(19)/(20) checks before canonical; Rust 31/31, protocol 167/167 and analysis 48/48 pass; no new seed captured; see `G0_OUTER_FEEDBACK_OBSERVABILITY_AUDIT.md` |
| G1-TECH | Corrected-runtime D44 technical gate | AUTHORIZED / PROTOCOL FREEZE IN PROGRESS | 0 new seeds | Matching reference, runtime identity, pairing, `stream_contract_ready=true`, and analyzer pass | D44 tape is technical-only; `G1_CORRECTED_RUNTIME_PREREGISTRATION.md` |
| G1-SCREEN | Strict-Eq.15 corrected-runtime candidate screen | AUTHORIZED / WAITING FOR TECHNICAL GATE | 0/90 | Complete D61--D65 `3 candidates x 6 cells x 5 seeds` global-maximin selection | Candidates are `ready_order`, `ready_finish_tie`, `formula`; no D61 tape exists before the technical gate |
