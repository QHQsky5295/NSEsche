...
| P5-COMMON-PROTOCOL | Final common admission/measurement/drain protocol | P5.4 RESULT-BLIND SELECTION FROZEN / 90 ONLINE RUNS AUTHORIZED AFTER AUDIT COMMIT | 0/90 online; 9/9 tapes; 90/90 references; 9 exhausted technical attempts retained in three old instances | Formula-faithful NSESche plus method-neutral FCFS admission, capacity-proportional active cohort, explicit phases, common runtime, complete accounting, and all twelve protocol gates | Exact load-major 90-row selection binds ready manifest and analyzer hash with no result fields; next is one complete fixed-order online batch; see `P5_COMMON_PLATFORM_ANALYZER_SELECTION_AUDIT.md` |
