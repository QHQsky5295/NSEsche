...
ROOT = Path("tmp/nse_e3_load_band_substitution_cap_training_20260828_v99")
PLAN = Path(
    "scripts/reviewer_experiments/protocol/"
    "nse_e3_load_band_substitution_cap_training_plan_v99.json"
)
PLAN_SHA256 = "5f1dfe65916182fd97c09445ffdde7bb6895d45a1cd43bf056cb783e2c68c3f1"
V98_RESULT_RECEIPT = Path(
    "scripts/reviewer_experiments/protocol/"
    "nse_e3_terminal_load_band_training_result_v98.json"
)
V98_RESULT_RECEIPT_SHA256 = (
    "deb571ec551011a0185e7d21401a198c7174b3144700dafac54847066cc6c19b"
)
V98_SOURCE_RESULT = Path(
    "tmp/nse_e3_terminal_load_band_training_20260828_v98/training-result-v98.json"
)
V98_SOURCE_RESULT_SHA256 = (
    "a7cecf7be25245a490409af7ac04f456f54d0caef6af623ae72a521c4c13b51d"
)
FORMAL_RESULT = Path(
...
BINARY_PATH = Path("tmp/nse_v99_build_03a90de/release/serverless_sim.exe")
BINARY_SHA256 = "bdcafbb8424af9ae73c463c4a1ec836a230fa8bbefa7ae76946d8821bd1f4517"
BINARY_SOURCE_COMMIT = "03a90de78b780e55f67be84604c2cbdfc3b30886"
...
SOURCE_INITIAL_SEEDS = [f"E{index}" for index in range(866, 876)]
SOURCE_CI_SEEDS = [f"E{index}" for index in range(876, 886)]
TRAINING_SEED_LIST = ["E866", "E867", "E868"]
TRAINING_SEEDS = set(TRAINING_SEED_LIST)
CONFIRMATION_SEEDS = [f"E{index}" for index in range(886, 906)]
PREVIOUSLY_RESERVED_SEEDS = [
    ...
    *[f"E{index}" for index in range(869, 886)],
]
PORT = "3134"
...
E3_BAND8_CAP1 = (
    ...
    "terminal_load_band8_cap1_guard64_dual_window_safe_pareto"
)
E3_BAND8_CAP2 = (
    "faasrank_native_faithful_terminal_ocs_srpt_ready_"
    "terminal_load_band8_cap2_guard64_dual_window_safe_pareto"
)
ARMS = (
    ("v99-e3-anchor", "E3", "anchor", E3_ANCHOR, None, 9),
    ("v99-e3-band8-cap1", "E3", "candidate", E3_BAND8_CAP1, 1, 9),
    ("v99-e3-band8-cap2", "E3", "candidate", E3_BAND8_CAP2, 2, 9),
)
...
def _paths(root: Path) -> dict[str, Path]:
    return {
        "config": root / "v99-e3-training-config.json",
        "source": root / "manifest.v99-e866-e875-full.unbound.json",
        "capture": root / "manifest.v99-tape-capture.unbound.json",
        "prepared": root / "prepared-manifests-v99.json",
    }
...
def _assert_frozen_inputs() -> None:
    for path, expected in (
        (PLAN, PLAN_SHA256),
        (V98_RESULT_RECEIPT, V98_RESULT_RECEIPT_SHA256),
        (V98_SOURCE_RESULT, V98_SOURCE_RESULT_SHA256),
        ...
        if not path.is_file() or file_hash(path) != expected:
            raise RuntimeError(f"frozen V99 input is missing or changed: {path}")
...
def _rewrite(
    ...
    *,
    purpose: str,
    arm: tuple[str, str, str, str, int | None, int] | None = None,
    ...
    marker.update(
        {
            "purpose": purpose,
            "v99_training_plan_sha256": PLAN_SHA256,
            "v99_training_only": True,
            "v99_binary_source_commit": BINARY_SOURCE_COMMIT,
    ...
    if arm is not None:
        ...
        arm_id, experiment_id, role, profile, substitution_cap, expected = arm
        marker.update(
            {
                "v99_arm_id": arm_id,
                "v99_experiment_id": experiment_id,
                "v99_role": role,
                "v99_profile": profile,
                "v99_upper_queue_density_threshold": 64.0
                if role == "candidate"
                else None,
                "v99_nonterminal_queue_density_floor": 8.0
                if role == "candidate"
                else None,
                "v99_substitution_cap": substitution_cap,
                "v99_expected_run_count": expected,
            }
        )
        ...
        metadata.update(
            {
                "v99_training_plan_sha256": PLAN_SHA256,
                "v99_training_only": True,
                "v99_training_seed_metrics_previously_revealed": False,
                "v99_binary_source_commit": BINARY_SOURCE_COMMIT,
                "v99_confirmation_seeds_opened": False,
                "v99_previously_reserved_seeds_opened": False,
                "v99_formal_E01_E20_reexecution": False,
            }
        )
        if arm is not None:
            arm_id, experiment_id, role, profile, substitution_cap, _ = arm
            ...
            metadata.update(
                {
                    "v99_arm_id": arm_id,
                    "v99_arm_role": role,
                    "v99_candidate_profile": profile,
                    "v99_candidate_experiment": experiment_id,
                    "v99_upper_queue_density_threshold": 64.0
                    if role == "candidate"
                    else None,
                    "v99_nonterminal_queue_density_floor": 8.0
                    ...
                    "v99_substitution_cap": substitution_cap,
                    "v99_faasrank_model_artifact_sha256": MODEL_SHA256,
                    "v99_native_faithful_initializer": True,
                    "v99_dual_window_safe_pareto": True,
                    "v99_load_band_substitution_cap_guard": substitution_cap
                    is not None,
                    "v99_terminal_players_included_without_lower_floor": role
                    == "candidate",
                    "v99_srpt_critical_path_player_order": "srpt_ready" in profile,
                }
            )
...
def prepare_v99(root: Path = ROOT) -> dict:
    _assert_frozen_inputs()
    if root.exists():
        raise RuntimeError(f"refusing to overwrite V99 training root: {root}")
    ...
    capture = derive_integration_smoke_shard(
        paths["source"],
        capture_ids,
        purpose=f"V99 fresh balanced-QoS parent tapes; plan_sha256={PLAN_SHA256}",
    )
    _write(
        paths["capture"],
        _rewrite(
    ...
    for arm in ARMS:
        ...
        template = derive_integration_smoke_shard(
            paths["source"],
            selected,
            purpose=f"V99 {arm_id} load-band cap training; plan_sha256={PLAN_SHA256}",
        )
        manifest = _rewrite(
            template,
            purpose=f"V99 {arm_id} load-band cap training; plan_sha256={PLAN_SHA256}",
        ...
        if (
            ...
            raise RuntimeError(f"V99 {arm_id} must have {expected} runs/references")
        if {run["seed"] for run in manifest["runs"]} != TRAINING_SEEDS:
            raise RuntimeError(f"V99 {arm_id} escaped E866-E868")
        keys = {item["key"] for item in manifest["reference_build_dependencies"]}
        if all_reference_keys & keys:
            raise RuntimeError("V99 arm-specific reference keys unexpectedly overlap")
        all_reference_keys.update(keys)
        _write(arm_path(root, arm_id), manifest)
    if len(all_reference_keys) != 27:
        raise RuntimeError("V99 must declare exactly 27 arm-specific references")
    ...
    receipt = {
        "schema_version": "NSE_E3_LOAD_BAND_SUBSTITUTION_CAP_PREPARED_V99_V1",
        "created_at": utc_now(),
        "formal_results_eligible": False,
        ...
        "V98_result_receipt_path": str(V98_RESULT_RECEIPT),
        "V98_result_receipt_sha256": V98_RESULT_RECEIPT_SHA256,
        "V98_source_result_path": str(V98_SOURCE_RESULT),
        "V98_source_result_sha256": V98_SOURCE_RESULT_SHA256,
        ...
        "arms": [
            {
                ...
                "substitution_cap": substitution_cap,
                "run_count": expected,
                "reference_build_count": expected,
            }
            for arm_id, experiment_id, role, profile, substitution_cap, expected in ARMS
        ],
