...
PREPARED_FILE_SHA256 = (
    "67271f2b8b716150a09cfa237a262e0402357326b283e873f13d326ceb475c41"
)
SLA_MANIFEST = ROOT / "manifest.v94-srpt-terminal-dual-training.sla.json"
SLA_MANIFEST_FILE_SHA256 = (
    "a544233883e668923e68bdb8081eda95611a4882f32fe794a88b9ee5f974cbca"
)
SLA_MANIFEST_HASH = "b2b199d88b870172fd65860bb5801dd8955b355b630fec8f6960e13f6a978048"
READY = ROOT / "manifest.v94-srpt-terminal-dual-training.ready.json"
READY_FILE_SHA256 = "e648226ed856c255a969a85a815a947620e0fbd3253736e15fcf7cf9af2635e8"
READY_MANIFEST_HASH = "8a7b0c3568afda65535cbe775e4f70cc8e597903550be1df82ad3d2ac6e3e988"
TAPES = ROOT / "tapes.catalog.json"
TAPES_FILE_SHA256 = "5bc7d23ceeda596fc3b2c15be5a0eb54dd2ad324c8f6cb9696b65ffed315fd9d"
REFERENCES = ROOT / "references.v94.catalog.json"
REFERENCES_FILE_SHA256 = (
    "676a89f27f78317ca0fb3defa827fc8297e0576461f09212d34eb9b08560b666"
)
CAPTURE_WORKSPACE = ROOT / "stages/tape_capture"
REFERENCE_WORKSPACE = ROOT / "stages/references-v94"
WORKSPACE = ROOT / "runs/training-v94"
PAIRING = ROOT / "training-v94-pairing-audit.json"
OUTPUT = ROOT / "joint-blind-audit-v94-training.json"
...
def _tape_evidence(manifest: dict) -> list[dict]:
    ...
    _require(
        ...
        "V94 tape catalog must contain exactly 3 base and 9 derived tapes",
    )
    capture_quarantine = CAPTURE_WORKSPACE / "capture_base_tapes/quarantine"
    _require(
        not capture_quarantine.exists() or not any(capture_quarantine.iterdir()),
    ...
    for key in sorted(entries):
        ...
        if entry["kind"] == "base_steady":
            ...
            _require(
                isinstance(parent_sha256, str)
                and transform.get("parent_sha256") == parent_sha256
...
def _run_evidence(manifest: dict, pairing: dict) -> tuple[list[dict], dict]:
    ...
    for run in sorted(manifest["runs"], key=lambda item: item["run_id"]):
        ...
        _require(
            metadata.get("v94_training_plan_sha256") == PLAN_SHA256
            and metadata.get("v94_training_only") is True
            and metadata.get("v94_training_seed_metrics_previously_revealed") is False
            ...
            and metadata.get("v94_resource_scaling_excluded") is True
            and metadata.get("v94_confirmation_seeds_opened") is False
            and metadata.get("v94_v93_reserved_seeds_opened") is False,
            f"V94 scientific boundary missing: {run_id}",
        )
...
def main() -> None:
    ...
    _require(
        not (ROOT / "training-result-v94.json").exists(),
        "V94 reveal exists before blind audit",
    )
    prepared = load_and_validate_manifest(SLA_MANIFEST)
    manifest = load_and_validate_manifest(READY)
    _require(
        prepared["manifest_hash"] == SLA_MANIFEST_HASH
    ...
    _require(
        ...
        and not (manifest_seeds & CONFIRMATION_SEEDS)
    ...
    _require(capture_events == 3, "tape-capture ledger must contain exactly 3 events")
    _require(reference_events == 12, "reference ledger must contain exactly 12 events")

    audit = {
        "schema_version": "NSE_E3E4_V94_SRPT_TERMINAL_DUAL_TRAINING_BLIND_AUDIT_V1",
        "created_at": utc_now(),
        "status": "passed",
