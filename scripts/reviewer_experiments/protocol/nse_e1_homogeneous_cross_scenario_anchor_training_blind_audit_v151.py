...
from scripts.reviewer_experiments.protocol.nse_e1_homogeneous_cross_scenario_anchor_training_execute_v151 import (
...
OUTPUT_NAME = "joint-blind-audit-v151-training.json"
...
def _audit_nash_log(canonical: Path, run: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if not path.is_file():
        raise RuntimeError(f"V151 Nash log is missing: {run_id}")
    run_config_count = 0
    window_count = 0
    summary_count = 0
    ...
    window_guard_evaluated = 0
    window_guard_accepted = 0
    window_guard_fallback = 0
    initializer_gate_evaluated = 0
    initializer_gate_accepted = 0
    load_least_gate_evaluated = 0
    load_least_gate_accepted = 0
        ...
        for raw in stream:
            ...
            if kind == "run_config":
                ...
                if not (
                    ...
                    raise RuntimeError(
                        "V151 run_config profile/runtime contract changed"
                    )
            elif kind == "window":
                if event.get("frame") != window_count:
                    raise RuntimeError("V151 scheduler window sequence changed")
                solver = event.get("solver")
                decision = event.get("decision")
                if not isinstance(solver, Mapping) or not isinstance(decision, Mapping):
                    raise RuntimeError("V151 scheduler observation is incomplete")
                ...
                if not (
                    ...
                    raise RuntimeError("V151 final assignment hash is invalid")
                window_guard = decision.get("window_safe_guard")
                initializer_gate = decision.get("initializer_dominance_gate")
                load_least_gate = decision.get("load_least_dominance_gate")
                if not all(
                    isinstance(item, Mapping)
                    and item.get("certificate_uses_completion_outcomes") is False
                    for item in (window_guard, initializer_gate, load_least_gate)
                ):
                    raise RuntimeError("V151 guard telemetry is incomplete")
                window_guard_evaluated += int(window_guard.get("evaluated") is True)
                window_guard_accepted += int(window_guard.get("accepted") is True)
                window_guard_fallback += int(
                    window_guard.get("fallback_applied") is True
                )
                initializer_gate_evaluated += int(
                    initializer_gate.get("evaluated") is True
                )
                initializer_gate_accepted += int(
                    initializer_gate.get("accepted") is True
                )
                load_least_gate_evaluated += int(
                    load_least_gate.get("evaluated") is True
                )
                load_least_gate_accepted += int(load_least_gate.get("accepted") is True)
                social = event.get("social")
                if not isinstance(social, Mapping):
                    raise RuntimeError("V151 social observation is incomplete")
                ...
                if state_key is None:
                    if not (
                        ...
                        raise RuntimeError(
                            "V151 unrequested reference reason is not legitimate"
                        )
                    ...
                    if reference_source in {
                        ...
                        raise RuntimeError("V151 bound reference source changed")
                window_count += 1
                ...
                if (
                    ...
                    raise RuntimeError("V151 Nash terminal marker changed")
            elif kind != "function_profile":
                raise RuntimeError(f"unexpected V151 Nash observation kind: {kind}")
    if run_config_count != 1 or window_count != 1000 or summary_count != 1:
        raise RuntimeError("V151 Nash log cardinality changed")
    if reference_requested == 0 or reference_available != reference_requested:
        raise RuntimeError("V151 offline reference replay coverage changed")
    return {
        "run_id": run_id,
        "load": load,
        "seed": run["seed"],
        ...
        "window_safe_guard_evaluated_windows": window_guard_evaluated,
        "window_safe_guard_accepted_windows": window_guard_accepted,
        "window_safe_guard_fallback_windows": window_guard_fallback,
        "initializer_dominance_evaluated_windows": initializer_gate_evaluated,
        "initializer_dominance_accepted_windows": initializer_gate_accepted,
        "load_least_dominance_evaluated_windows": load_least_gate_evaluated,
        "load_least_dominance_accepted_windows": load_least_gate_accepted,
    }
...
def _validate_reference_catalog(
    ...
    if not isinstance(claimed, str) or object_hash(payload) != claimed:
        raise RuntimeError("V151 reference catalog self-hash changed")
    entries = catalog.get("entries")
    if not isinstance(entries, Mapping) or len(entries) != 60:
        raise RuntimeError("V151 reference catalog is not an exact 60-entry product")
    expected_keys = {
        dependency["key"] for dependency in manifest["reference_build_dependencies"]
    }
    if set(entries) != expected_keys:
        raise RuntimeError("V151 reference catalog key set changed")
    for dependency in manifest["reference_build_dependencies"]:
        ...
        for field in (
            ...
            if entry.get(field) != dependency.get(field):
                raise RuntimeError(f"V151 bound reference {field} changed")
        if file_hash(Path(entry["path"])) != entry["sha256"]:
            raise RuntimeError("V151 reference table content hash changed")
        if file_hash(Path(entry["receipt_path"])) != entry["receipt_sha256"]:
            raise RuntimeError("V151 reference receipt content hash changed")
...
def _validate_pairing(manifest: dict[str, Any], root: Path) -> dict[str, Any]:
    ...
    if stored != recomputed or recomputed.get("passed") is not True:
        raise RuntimeError("V151 pairing audit is missing, stale, or failed")
    if recomputed.get("run_count") != 60 or recomputed.get("group_count") != 60:
        raise RuntimeError("V151 pairing product changed")
    return recomputed
...
def run_blind_audit(root: Path = ROOT) -> dict[str, Any]:
    ...
    if output.exists():
        raise RuntimeError(f"V151 blind audit already exists: {output}")
    prepared_path = root / "prepared-manifest-v151.json"
    prepared = read_json(prepared_path)
    prepared_hash = _assert_hashed(prepared, "receipt_hash", "V151 prepared receipt")
    execution_path = root / EXECUTION_RECEIPT_NAME
    execution = read_json(execution_path)
    execution_hash = _assert_hashed(execution, "receipt_hash", "V151 execution receipt")
    ready_schedule_path = root / READY_SCHEDULE_NAME
    ready_schedule = read_json(ready_schedule_path)
    schedule_hash = _assert_hashed(
        ready_schedule, "schedule_hash", "V151 ready schedule"
    )
    ...
    if not (
        ...
        raise RuntimeError("V151 ready product changed")
    for run in manifest["runs"]:
        ...
        if any(
            ...
            raise RuntimeError("V151 ready environment changed")
    ...
    if [item["run_id"] for item in execution["dispatches"]] != expected_order:
        raise RuntimeError("V151 execution order changed after freezing")
    ...
    if actual_dirs != set(expected_order):
        raise RuntimeError("V151 canonical directory product changed")
    quarantine = paths(root)["workspace"] / "quarantine"
    if quarantine.exists() and any(quarantine.rglob("attempt-*")):
        raise RuntimeError("V151 has unexplained quarantined attempts")
    audits = []
    ...
    if len(runtime_identity) != 1:
        raise RuntimeError("V151 runtime identity is not unanimous")
    ...
    if not (
        ...
        raise RuntimeError("V151 runtime identity changed from frozen preparation")
    document = {
        "schema_version": "NSE_E1_HOMOGENEOUS_CROSS_SCENARIO_ANCHOR_BLIND_AUDIT_V151_V1",
        "created_at": utc_now(),
        "status": "pass",
