...
from scripts.reviewer_experiments.protocol.nse_e3_dual_prefix_service_training_prepare_v120 import (
...
OUTPUT = ROOT / "joint-blind-audit-v120-training.json"
RESULT = ROOT / "training-result-v120.json"
...
def _validate_admitted_work_diagnostics(
    ...
    """Validate V120 mechanism evidence without opening any performance summary."""

    path = canonical / "reviewer_records" / run["run_id"] / "nash_metrics.jsonl.gz"
    _require(path.is_file(), f"missing V120 Nash diagnostics: {run['run_id']}")
    ...
    with gzip.open(path, "rt", encoding="utf-8") as handle:
        for line_number, line in enumerate(handle, start=1):
            ...
            _require(type(frame) is int and frame >= 0, "invalid V120 window frame")

            cluster = event.get("cluster")
            _require(isinstance(cluster, dict), "missing V120 cluster diagnostics")
            admitted_fields = (
            ...
            if candidate:
                _require(
                    ...
                    f"V120 admitted-work observation changed: {run['run_id']}:{line_number}",
                )
                ...
                _require(
                    ...
                    f"V120 admitted-work snapshot counts changed: "
                ...
                _require(
                    ...
                    f"V120 admitted-work aggregate changed: {run['run_id']}:{line_number}",
                )
            ...
            _require(
                ...
                f"V120 critical-service contract changed: {run['run_id']}:{line_number}",
            )
            ...
            _require(
                ...
                f"V120 service-directed construction contract changed: "
                f"{run['run_id']}:{line_number}",
            )
            for field in (
                "target_player_count",
                ...
                _require(
                    ...
                    f"V120 service-directed {field} changed: "
                    f"{run['run_id']}:{line_number}",
                )
            _require(
                ...
                f"V120 service-directed substitution partition changed: "
                f"{run['run_id']}:{line_number}",
            )
            if candidate and critical.get("gate_enabled"):
                ...
                _require(
                    ...
                    f"V120 all-player construction coverage changed: "
                    f"{run['run_id']}:{line_number}",
                )
                _require(
                    ...
                    == 0,
                    f"V120 dual-prefix construction coverage changed: "
                    f"{run['run_id']}:{line_number}",
                )
            else:
                _require(
                    ...
                    f"V120 inapplicable service construction was populated: "
                    f"{run['run_id']}:{line_number}",
                )
            ...
            _require(
                ...
                f"V120 componentwise-service contract changed: "
                f"{run['run_id']}:{line_number}",
            )
            for field in (
                "target_player_count",
                ...
                _require(
                    ...
                    f"V120 componentwise {field} changed: "
                    f"{run['run_id']}:{line_number}",
                )
            ...
            _require(
                ...
                f"V120 componentwise acceptance changed: "
                f"{run['run_id']}:{line_number}",
            )
            ...
            _require(
                ...
                f"V120 componentwise reason changed: {run['run_id']}:{line_number}",
            )
            if not componentwise["evaluated"]:
                _require(
                    ...
                    is None,
                    f"V120 unevaluated componentwise evidence was populated: "
                    f"{run['run_id']}:{line_number}",
                )
            ...
            _require(
                ...
                f"V120 componentwise delta changed: {run['run_id']}:{line_number}",
            )
            _require(
                maximum_ratio is None or _finite_nonnegative(maximum_ratio),
                f"V120 componentwise ratio changed: {run['run_id']}:{line_number}",
            )
            if componentwise["evaluated"]:
                _require(
                    componentwise["target_player_count"]
                    == event["decision"]["request_function_players"],
                    f"V120 all-player target coverage changed: "
                    f"{run['run_id']}:{line_number}",
                )
            if componentwise["accepted"] and componentwise["target_player_count"] > 0:
                _require(
                    ...
                    f"V120 accepted componentwise delta is worse: "
                    f"{run['run_id']}:{line_number}",
                )
            _require(
                not critical["accepted"] or componentwise["accepted"],
                f"V120 main certificate bypassed componentwise gate: "
                f"{run['run_id']}:{line_number}",
            )
            ...
            if dominance.get("reason") in componentwise_rejection_reasons:
                _require(
                    ...
                    f"V120 componentwise fallback was not exact anchor: "
                    f"{run['run_id']}:{line_number}",
                )
            ...
            _require(
                ...
                f"V120 re-enabled a failed V114-V116 axis: "
                f"{run['run_id']}:{line_number}",
            )
            for field in (
                ...
                _require(
                    ...
                    f"V120 critical {field} changed: {run['run_id']}:{line_number}",
                )
            _require(
                not critical["accepted"] or critical["evaluated"],
                f"V120 accepted without evaluation: {run['run_id']}:{line_number}",
            )
            if not invoked:
                _require(
                    ...
                    f"V120 non-invoked critical state changed: {run['run_id']}:{line_number}",
                )
            if critical["accepted"]:
                ...
                _require(
                    ...
                    f"V120 accepted evidence is nonfinite: {run['run_id']}:{line_number}",
                )
                ...
                _require(
                    ...
                    f"V120 accepted admitted-work certificate changed: "
                    f"{run['run_id']}:{line_number}",
                )
            ...
            if not candidate:
                _require(
                    ...
                    f"V120 anchor used causal state: {run['run_id']}:{line_number}",
                )
                continue
            if not gate["history_complete"]:
                _require(
                    gate["baseline_count"] == 0 and gate["recent_count"] == 0,
                    f"V120 incomplete history leaked counts: {run['run_id']}:{line_number}",
                )
                continue
            sufficient = (
            ...
            if sufficient and recent_scaled >= baseline_scaled:
                ...
                _require(
                    ...
                    f"V120 exact threshold did not activate: "
                    f"{run['run_id']}:{line_number}",
                )

    _require(
        counts["windows"] == run["simulation"]["total_frame"],
        f"V120 diagnostic window count changed: {run['run_id']}",
    )
    if candidate:
        _require(
            ...
            f"V120 mechanism was not exercised: {run['run_id']}",
        )
    return {
...
def _verify_tape_catalog() -> tuple[list[dict[str, Any]], dict[str, Any]]:
    _require(TAPES.is_file(), f"missing V120 tape catalog: {TAPES}")
    catalog = read_json(TAPES)
    catalog_hash = _assert_hashed_object(catalog, "catalog_hash", "V120 tapes")
    entries = catalog.get("entries")
    _require(
        isinstance(entries, dict) and len(entries) == 12, "V120 tape count changed"
    )
    evidence = []
...
def _verify_reference_stage_ledgers() -> tuple[dict[str, Path], dict[Path, str]]:
    ...
    for arm_id, expected in ARMS.items():
        ...
        root = _stage_root_from_receipts(
            entries, "receipt_path", expected["run_count"], f"V120 reference {arm_id}"
        )
        roots[arm_id] = root
    ...
    for root, expected_count in expected_by_root.items():
        ...
        _assert_ledger_contract(
            ...
            f"V120 reference stage {root}",
        )
        _require(
            not list((root / "quarantine").glob("**/attempt-*")),
            f"V120 reference quarantine is nonempty: {root}",
        )
...
def _validate_run_boundary(
    ...
    required = {
        ...
        "v120_arm_id": arm_id,
        "v120_arm_role": expected["role"],
        "v120_candidate_profile": expected["profile"],
        "v120_candidate_experiment": "E3",
        "v120_shock_rate_ratio": expected["shock_rate_ratio"],
        "v120_shock_threshold_numerator": expected["shock_threshold_numerator"],
        "v120_shock_threshold_denominator": expected["shock_threshold_denominator"],
        ...
        "v120_shock_activation_horizon_frames": expected[
            "shock_activation_horizon_frames"
        ],
        "v120_nonterminal_queue_density_floor": 8.0 if candidate else None,
        "v120_warm_admissibility": "preserve_anchor_warmness" if candidate else None,
        "v120_load_least_window_certificate_mode": (
            "disabled" if candidate else "not_applicable"
        ),
        "v120_critical_service_ratio_numerator": expected[
            "critical_service_threshold_numerator"
        ],
        "v120_critical_service_ratio_denominator": expected[
    ...
    _require(
        ...
        f"V120 run boundary changed: {run['run_id']}",
    )


def run_blind_audit(output: Path = OUTPUT) -> dict[str, Any]:
    _require(not output.exists(), f"V120 blind audit already exists: {output}")
    _require(not RESULT.exists(), "V120 reveal exists before blind audit")
    _require(PLAN.is_file() and file_hash(PLAN) == PLAN_SHA256, "V120 plan changed")
    plan = read_json(PLAN)
    _require(plan.get("formal_results_eligible") is False, "V120 eligibility changed")
    _require(PREPARED.is_file(), f"missing V120 prepared receipt: {PREPARED}")
    prepared = read_json(PREPARED)
    prepared_hash = _assert_hashed_object(
        prepared, "receipt_hash", "V120 prepared receipt"
    )
    _require(
        ...
        and prepared.get("V119_result_receipt_hash") == V119_RESULT_RECEIPT_HASH
        and prepared.get("V119_result_receipt_commit") == V119_RESULT_RECEIPT_COMMIT,
        "V120 prepared scientific boundary changed",
    )
    ...
    for arm_id, expected in ARMS.items():
        ...
        _require(paths["ready"].is_file(), f"missing V120 ready manifest: {arm_id}")
        ...
        _require(
            ...
            f"V120 ready boundary changed: {arm_id}",
        )
        ...
        reference_catalog_hash = _assert_hashed_object(
            references, "catalog_hash", f"V120 references {arm_id}"
        )
        ...
        _require(
            ...
            f"V120 reference key set changed: {arm_id}",
        )
        for key, entry in sorted(entries.items()):
            ...
            _require(
                ...
                f"V120 reference evidence changed: {key}",
            )
        ...
        _require(actual_ids == expected_ids, f"V120 canonical set changed: {arm_id}")
        _require(
            not list((workspace / "quarantine").glob("**/attempt-*")),
            f"V120 online quarantine is nonempty: {arm_id}",
        )
        ...
        for run in manifest["runs"]:
            ...
            _require(
                ...
                f"V120 canonical status changed: {run['run_id']}",
            )
            runtime = {
    ...
    _require(len(run_evidence) == 18, "V120 run evidence count must be 18")
    _require(len(reference_evidence) == 18, "V120 reference evidence count must be 18")
    _require(len(tape_evidence) == 12, "V120 tape evidence count must be 12")
    ...
    _require(
        ...
        > 0,
        "V120 componentwise certificate did not exercise both accept and reject paths",
    )
    _require(
        sum(
        ...
        "V120 all-player componentwise evidence is unavailable or incomplete",
    )
    _require(
        sum(
        ...
        > 0,
        "V120 service-directed construction did not exercise both critical and "
    ...
    _require(
        ...
        "V120 anchor unexpectedly exercised the service construction or certificate",
    )
    for field, expected in EXPECTED_RUNTIME.items():
        _require(
            runtime_values[field] == {expected},
            f"V120 runtime {field} changed: {runtime_values[field]}",
        )
    ...
    _require(
        ...
        f"V120 runtime git identity is not singular: {git_commits}",
    )
    _require(len(paired_inputs) == 9, "V120 paired input product changed")
    for (scenario, seed), rows in paired_inputs.items():
        _require(len(rows) == 2, f"V120 paired arm count changed: {scenario}/{seed}")
        for field in (
            ...
            _require(
                ...
                f"V120 paired {field} changed: {scenario}/{seed}",
            )
        _require(
            rows[0]["common_hpa_hash"] == EXPECTED_COMMON_HPA_SHA256,
            f"V120 common HPA changed: {scenario}/{seed}",
        )

    output_payload = {
        "schema_version": "NSE_E3_DUAL_PREFIX_SERVICE_BLIND_AUDIT_V120_V1",
        "created_at": utc_now(),
        "status": "pass",
