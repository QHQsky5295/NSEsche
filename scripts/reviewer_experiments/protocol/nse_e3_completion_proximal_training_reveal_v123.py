...
from scripts.reviewer_experiments.protocol.nse_e3_completion_proximal_training_blind_audit_v123 import (
    ARMS,
...
OUTPUT = ROOT / "training-result-v123.json"
BLIND_AUDIT_FILE_SHA256 = (
    "16fd7af63a501a701a33dc5471e769a4440249ed7a84346f3d02d59949189e86"
)
BLIND_AUDIT_HASH = "b3339f04cb26562da39fa1b82b1e6e0c0906dc388703ae5a1978270c33f44af9"
EXPECTED_SEEDS = ("E1172", "E1173", "E1174")
EXPECTED_SCENARIOS = (
    "E3.spike5x50ms",
    "E3.sustained3x200ms",
    "E3.pulse4x4x50ms",
...
ANCHOR_ID = "v123-e3-anchor"
CANDIDATE_ID = (
    "v123-e3-causal-shock15-horizon50-admitted-work10-"
    "completion-proximal-depth1-initializer-only"
)
...
def _validate_blind_audit() -> dict[str, Any]:
    ...
    if any(
        ...
        raise RuntimeError("V123 blind audit has not been frozen into the reveal code")
    if not BLIND_AUDIT.is_file() or file_hash(BLIND_AUDIT) != BLIND_AUDIT_FILE_SHA256:
        raise RuntimeError("V123 blind audit is missing or changed")
    ...
    if (
        ...
        raise RuntimeError("V123 blind audit does not authorize reveal")
    return blind
...
def _load_rows(blind: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    if len(admitted) != 18:
        raise RuntimeError("V123 blind audit run evidence is incomplete")
    ...
    for arm_id, expected in ARMS.items():
        ...
        if not manifest_path.is_file() or file_hash(manifest_path) != frozen.get(
            ...
            raise RuntimeError(f"V123 frozen manifest changed: {arm_id}")
        manifest = load_and_validate_manifest(manifest_path)
        if manifest.get("manifest_hash") != frozen.get("manifest_hash"):
            raise RuntimeError(f"V123 manifest hash changed: {arm_id}")
        for run in manifest["runs"]:
            ...
            if (
                ...
                != expected["profile"]
                or metadata.get("v123_training_plan_sha256") != PLAN_SHA256
                or metadata.get("v123_training_only") is not True
                or metadata.get("v123_training_seed_metrics_previously_revealed")
                is not False
                or metadata.get("v123_confirmation_seeds_opened") is not False
                or metadata.get("v123_other_unopened_seeds_opened") is not False
                or metadata.get("v123_arm_id") != arm_id
                or metadata.get("v123_arm_role") != expected["role"]
                or metadata.get("v123_shock_rate_ratio") != expected["shock_rate_ratio"]
                or metadata.get("v123_shock_threshold_numerator")
                != expected["shock_threshold_numerator"]
                or metadata.get("v123_shock_threshold_denominator")
                != expected["shock_threshold_denominator"]
                or metadata.get("v123_shock_activation_horizon_frames")
                != expected["shock_activation_horizon_frames"]
                or metadata.get("v123_critical_service_ratio_numerator")
                != expected["critical_service_threshold_numerator"]
                or metadata.get("v123_critical_service_ratio_denominator")
                != expected["critical_service_threshold_denominator"]
                or metadata.get("v123_service_proxy_work_source")
                != (
                    "admitted_pending_plus_all_resident_remaining_and_same_window_projected_cpu_v1"
                    if candidate
                    else "not_applicable"
                )
                or metadata.get("v123_admitted_work_includes_all_blocked_resident")
                is not candidate
                or metadata.get("v123_completion_proximal_depth1_substitution_scope")
                is not candidate
                or metadata.get("v123_completion_proximal_definition")
                != (
                    "terminal_function_or_nonterminal_function_whose_immutable_DAG_children_are_all_terminal"
                    if candidate
                    else "not_applicable"
                )
                or metadata.get("v123_maximum_remaining_child_depth")
                != (1 if candidate else None)
                or metadata.get(
                    "v123_completion_proximal_critical_frontier_substitution"
                )
                is not candidate
                or metadata.get("v123_deeper_player_exact_anchor") is not candidate
                or metadata.get("v123_outcome_fields_drive_policy") is not False
                or metadata.get("v123_scenario_or_burst_label_used_by_policy")
                is not False
                or metadata.get("v123_completion_or_performance_fields_used_by_policy")
                is not False
                or metadata.get("v123_future_arrivals_used_by_policy") is not False
            ):
                raise RuntimeError(f"V123 arm identity changed: {run['run_id']}")
            run_id = run["run_id"]
            summary_path = (
                ROOT
                / "runs"
            ...
            if not summary_path.is_file() or file_hash(summary_path) != admitted.get(
                ...
                raise RuntimeError(f"V123 summary differs from blind audit: {run_id}")
            ...
            if (
                ...
                raise RuntimeError(
                    f"V123 summary identity/completion mismatch: {run_id}"
                )
            rows.append(
                {
                    "arm_id": arm_id,
...
def execute_reveal() -> dict[str, Any]:
    if OUTPUT.exists():
        raise RuntimeError(f"V123 training result already exists: {OUTPUT}")
    if not PLAN.is_file() or file_hash(PLAN) != PLAN_SHA256:
        raise RuntimeError("frozen V123 plan changed before reveal")
    blind = _validate_blind_audit()
    rows = _load_rows(blind)
    if len(rows) != 18:
        raise RuntimeError("V123 revealed row count differs from sealed plan")
    ...
    result = {
        "schema_version": "NSE_E3_COMPLETION_PROXIMAL_RESULT_V123_V1",
        "created_at": utc_now(),
        ...
        **evaluation,
        "revealed_rows": rows,
        "decision": {
            "authorize_preregistration_of_E1175_E1194_confirmation": joint_pass,
            ...
            "next_action": (
                "freeze V123 with frozen V97 E4 and separately preregister the "
                "complete E1175-E1194 NSESche confirmation before generating inputs"
                if joint_pass
                else (
                    "freeze the complete V123 training cohort, keep E1175-E1194 and "
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Reveal the frozen V123 training cohort exactly once after blind audit."
    )
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    if args.command != "reveal" or not args.execute:
        parser.error("V123 reveal requires the explicit command: reveal --execute")
    execute_reveal()
