...
ROOT = Path(
    "tmp/nse_e1_homogeneous_concurrent3_quota2_cpu_bounded_terminal_low_remaining17_20260901_v171"
)
PLAN = Path(
    "scripts/reviewer_experiments/protocol/"
    "nse_e1_homogeneous_concurrent3_quota2_cpu_bounded_terminal_slack_short_work_pipeline_queue8_low_remaining17_plan_v171.json"
)
PLAN_SHA256 = "b2a423e971ceca40c67ed26d470251e0eefbb14ff4759000dace0bcdb1b5ccbf"
DIAGNOSTIC_SUCCESS = Path(
    "scripts/reviewer_experiments/protocol/"
    "nse_e1_homogeneous_concurrent3_quota2_cpu_bounded_terminal_slack_short_work_pipeline_queue8_low_diagnostic_success_v171.json"
)
DIAGNOSTIC_SUCCESS_SHA256 = (
    "e20b353c1a01f7d12f6384b2cfbe9da2ebe9830e0adffb7459a564b076f7fc6c"
)
DIAGNOSTIC_SUCCESS_HASH = (
    "673ccc895f97bdab3971b95a24a3c80162dc91cc7e549935bf462a0f494f4b9d"
)
DIAGNOSTIC_BLIND_HASH = (
    "4ee9ca96884aed992948d37d9ca791ef13d8cbbb37316887632275f8c26e2254"
)
DIAGNOSTIC_ROOT = v171.ROOT
DIAGNOSTIC_SEEDS = tuple(v171.SEEDS)
...
def paths(root: Path = ROOT) -> dict[str, Path]:
    return {
        ...
        "prepared": root / "prepared-remaining17-v171.json",
        "schedule": root / "frozen-run-order-remaining17-v171.json",
        ...
        "execution": root / "execution-receipt-remaining17-v171.json",
        "pairing": root / "pairing-audit-remaining17-v171.json",
        "blind": root / "joint-blind-audit-remaining17-v171.json",
        "result": root / "complete-training-result-v171.json",
    }


def _assert_frozen_inputs() -> dict[str, Any]:
    source = v171._assert_frozen_inputs()
    for path, sha256, label in (
        (PLAN, PLAN_SHA256, "V171 remaining17 plan"),
        (
            DIAGNOSTIC_SUCCESS,
            DIAGNOSTIC_SUCCESS_SHA256,
            "V171 diagnostic success receipt",
        ),
    ):
        v171._assert_file(path, sha256, label)
    success = read_json(DIAGNOSTIC_SUCCESS)
    if not (
        v171._assert_hashed(success, "receipt_hash", "V171 diagnostic success receipt")
        == DIAGNOSTIC_SUCCESS_HASH
        and success.get("diagnostic_result", {}).get("joint_diagnostic_pass") is True
        and success.get("disposition", {}).get(
            "remaining_seventeen_v171_runs_authorized"
        )
        is True
        and success.get("disposition", {}).get(
        ...
        raise RuntimeError("V171 remaining17 authorization changed")
    diagnostic = v171.paths(DIAGNOSTIC_ROOT)
    for path, sha256, label in (
        (
            diagnostic["ready"],
            success["ready_manifest"]["file_sha256"],
            "V171 diagnostic ready manifest",
        ),
        (
            diagnostic["blind"],
            success["result_blind_audit"]["file_sha256"],
            "V171 diagnostic blind audit",
        ),
        (
            diagnostic["result"],
            success["analysis_correction"]["corrected_result"]["file_sha256"],
            "V171 diagnostic result",
        ),
    ):
        v171._assert_file(path, sha256, label)
    diagnostic_manifest = load_and_validate_manifest(diagnostic["ready"])
    v171._validate_product(diagnostic_manifest, references_bound=True)
    if (
        diagnostic_manifest["manifest_hash"]
        != success["ready_manifest"]["manifest_hash"]
    ):
        raise RuntimeError("V171 diagnostic manifest hash changed")
    diagnostic_blind = read_json(diagnostic["blind"])
    diagnostic_result = read_json(diagnostic["result"])
    if not (
        v171._assert_hashed(
            diagnostic_blind, "blind_audit_hash", "V171 diagnostic blind audit"
        )
        == DIAGNOSTIC_BLIND_HASH
        and v171._assert_hashed(
            diagnostic_result, "result_hash", "V171 corrected diagnostic result"
        )
        == success["analysis_correction"]["corrected_result"]["result_hash"]
        and diagnostic_result.get("reused_v170_candidate_run_count") == 17
        and diagnostic_result.get("joint_diagnostic_pass") is True
    ):
        raise RuntimeError("V171 corrected diagnostic evidence changed")
    return source
...
def _rewrite_remaining17(
    ...
    if [run["seed"] for run in rewritten["runs"]] != list(REMAINING_SEEDS):
        raise RuntimeError("V171 remaining17 source product changed")
    rewritten["execution"]["command_template"][-1] = str(v171.BINARY_PATH.resolve())
    ...
    marker.update(
        {
            "purpose": (
                "V171 fixed remaining-seventeen concurrent3 quota-two CPU-bounded "
                "terminal training completion; never a formal result or paper "
                "superiority claim"
            ),
            "v171_role": "result_blind_remaining17_full_cohort_completion",
            "v171_remaining17_plan_sha256": PLAN_SHA256,
            "v171_diagnostic_success_sha256": DIAGNOSTIC_SUCCESS_SHA256,
            "v171_diagnostic_success_hash": DIAGNOSTIC_SUCCESS_HASH,
            "v171_binary_source_commit": v171.BINARY_SOURCE_COMMIT,
            "v171_protocol_source_commit": protocol_source_commit,
            "v171_binary_sha256": v171.BINARY_SHA256,
            "v171_arm_id": ARM_ID,
            "v171_profile": v171.PROFILE,
            "v171_player_frontier": v171.FRONTIER,
            "v171_single_change_from_v170": v171.SINGLE_CHANGE,
            "v171_cpu_threshold": v171.CPU_THRESHOLD,
            "v171_heavy_player_threshold": v171.HEAVY_PLAYER_THRESHOLD,
            "v171_minimum_active_heavy_players": v171.MINIMUM_ACTIVE_HEAVY_PLAYERS,
            "v171_active_heavy_admission_quota": v171.ACTIVE_HEAVY_QUOTA,
            "v171_quota_selection_order": v171.QUOTA_SELECTION_ORDER,
            "v171_short_work_threshold": v171.SHORT_WORK_THRESHOLD,
            "v171_queue_density_threshold": v171.QUEUE_THRESHOLD,
            "v171_expected_run_count": 17,
            "v171_expected_reference_build_count": 17,
            "v171_fixed_order": list(REMAINING_SEEDS),
            "v171_diagnostic_seeds_reused_without_rerun": list(DIAGNOSTIC_SEEDS),
...
def _validate_product(manifest: Mapping[str, Any], *, references_bound: bool) -> None:
    if not (
        ...
        raise RuntimeError("V171 exact remaining17 product changed")
    for run in manifest["runs"]:
        ...
        if not (
            ...
            and run["environment"].get("NASH_OPERATIONAL_EXPERT_PROXY") == v171.PROFILE
            and run["environment"].get("SERVERLESS_SIM_PORT") == PORT
            and metadata.get("v171_heavy_player_threshold")
            == v171.HEAVY_PLAYER_THRESHOLD
            and metadata.get("v171_minimum_active_heavy_players")
            == v171.MINIMUM_ACTIVE_HEAVY_PLAYERS
            and metadata.get("v171_active_heavy_admission_quota")
            == v171.ACTIVE_HEAVY_QUOTA
            and metadata.get("v171_quota_selection_order") == v171.QUOTA_SELECTION_ORDER
            and metadata.get("v171_diagnostic_seed") is False
        ):
            raise RuntimeError(
                f"V171 remaining17 run contract changed: {run['run_id']}"
            )
...
def prepare_remaining17(root: Path = ROOT) -> dict[str, Any]:
    ...
    if root.exists():
        raise RuntimeError(f"refusing to overwrite V171 remaining17 root: {root}")
    ...
    schedule = {
        "schema_version": "NSE_E1_HOMOGENEOUS_CONCURRENT3_QUOTA2_LOW_REMAINING17_SCHEDULE_V171_V1",
        "created_at": utc_now(),
    ...
    receipt = {
        "schema_version": "NSE_E1_HOMOGENEOUS_CONCURRENT3_QUOTA2_LOW_REMAINING17_PREPARED_V171_V1",
        "created_at": utc_now(),
        "training_only": True,
        ...
        "binary_path": str(v171.BINARY_PATH.resolve()),
        "binary_sha256": v171.BINARY_SHA256,
        "python_sha256": v171.PYTHON_SHA256,
        "cargo_lock_sha256": v171.CARGO_LOCK_SHA256,
        "module_conf_semantic_hash": v171.MODULE_CONF_SEMANTIC_HASH,
...
def execute_remaining17(root: Path = ROOT) -> dict[str, Any]:
    ...
    if output["execution"].exists():
        raise RuntimeError("V171 remaining17 execution receipt already exists")
    prepared = read_json(output["prepared"])
    v171._assert_hashed(prepared, "receipt_hash", "V171 remaining17 prepared receipt")
    ...
    for ordinal, seed in enumerate(REMAINING_SEEDS, start=1):
        ...
        if completed.returncode != 0:
            raise RuntimeError(f"V171 remaining17 dispatch {seed} failed")
        ...
        if not (
            ...
            raise RuntimeError(f"V171 remaining17 {seed} did not QC-pass")
        dispatches.append(
            {
                "ordinal": ordinal,
                "seed": seed,
    ...
    receipt = {
        "schema_version": "NSE_E1_HOMOGENEOUS_CONCURRENT3_QUOTA2_LOW_REMAINING17_EXECUTION_V171_V1",
        "created_at": utc_now(),
        "plan_sha256": PLAN_SHA256,
...
def _remaining17_mechanism_gate(
    ...
    if {audit["seed"] for audit in audits} != set(REMAINING_SEEDS):
        raise RuntimeError("V171 remaining17 blind seed product changed")
    sum_keys = (
    ...
    passed = (
        ...
        and all(audit["frozen_v170_comparison_applicable"] is False for audit in audits)
        ...
        and (
            totals["active_heavy_quota_admitted_players"]
            + int(diagnostic_blind["active_heavy_quota_admitted_players"])
        )
        == 2 * combined_active
        and (
            totals["active_heavy_quota_rejected_excess_players"]
            + int(diagnostic_blind["active_heavy_quota_rejected_excess_players"])
        )
        > 0
        and min(rejected_cpu) > v171.CPU_THRESHOLD
        and diagnostic_blind.get("pass") is True
        and diagnostic_blind.get(
    ...
    return {
        **totals,
        ...
        "combined_twenty_active_heavy_quota_admitted_players": (
            totals["active_heavy_quota_admitted_players"]
            + int(diagnostic_blind["active_heavy_quota_admitted_players"])
        ),
        "combined_twenty_active_heavy_quota_rejected_excess_players": (
            totals["active_heavy_quota_rejected_excess_players"]
            + int(diagnostic_blind["active_heavy_quota_rejected_excess_players"])
        ),
...
def blind_audit_remaining17(root: Path = ROOT) -> dict[str, Any]:
    ...
    if output["blind"].exists():
        raise RuntimeError("V171 remaining17 blind audit already exists")
    _assert_frozen_inputs()
    prepared = read_json(output["prepared"])
    prepared_hash = v171._assert_hashed(
        prepared, "receipt_hash", "V171 remaining17 prepared receipt"
    )
    execution = read_json(output["execution"])
    execution_hash = v171._assert_hashed(
        execution, "receipt_hash", "V171 remaining17 execution receipt"
    )
    manifest = load_and_validate_manifest(output["ready"])
    _validate_product(manifest, references_bound=True)
    ...
    if {path.name for path in canonical_root.iterdir() if path.is_dir()} != {
        ...
        raise RuntimeError("V171 remaining17 canonical product changed")
    quarantine = output["workspace"] / "quarantine"
    if quarantine.exists() and any(quarantine.rglob("attempt-*")):
        raise RuntimeError("V171 remaining17 has quarantined attempts")
    ...
    for run in manifest["runs"]:
        ...
        audits.append(
            v171._audit_nash_log(canonical, run, compare_to_frozen_v170=False)
        )
    if len(identities) != 1:
        raise RuntimeError("V171 remaining17 runtime identity is not unanimous")
    binary, git_commit, python, cargo = next(iter(identities))
    if not (
        binary == v171.BINARY_SHA256
        and git_commit == prepared["protocol_source_commit"]
        and python == v171.PYTHON_SHA256
        and cargo == v171.CARGO_LOCK_SHA256
    ):
        raise RuntimeError("V171 remaining17 runtime identity changed")
    diagnostic_blind = read_json(v171.paths(DIAGNOSTIC_ROOT)["blind"])
    if not (
        v171._assert_hashed(
            diagnostic_blind, "blind_audit_hash", "V171 diagnostic blind audit"
        )
        ...
        raise RuntimeError("V171 diagnostic blind boundary changed")
    mechanism = _remaining17_mechanism_gate(audits, diagnostic_blind)
    if not mechanism["pass"]:
        raise RuntimeError("V171 combined twenty-run blind mechanism gate failed")
    document = {
        "schema_version": "NSE_E1_HOMOGENEOUS_CONCURRENT3_QUOTA2_LOW_REMAINING17_BLIND_AUDIT_V171_V1",
        "created_at": utc_now(),
        "status": "pass",
...
def _load_rows(manifest: Mapping[str, Any], root: Path) -> list[dict[str, Any]]:
    rows = []
    workspace = (
        paths(root)["workspace"] if root == ROOT else v171.paths(root)["workspace"]
    )
    for run in manifest["runs"]:
        summary = (
            workspace
...
def reveal_complete_training(root: Path = ROOT) -> dict[str, Any]:
    ...
    if output["result"].exists():
        raise RuntimeError("V171 complete training result already exists")
    blind = read_json(output["blind"])
    blind_hash = v171._assert_hashed(
        blind, "blind_audit_hash", "V171 remaining17 blind audit"
    )
    if not (
        ...
        raise RuntimeError("V171 remaining17 blind audit did not authorize reveal")
    remaining_manifest = load_and_validate_manifest(output["ready"])
    diagnostic_manifest = load_and_validate_manifest(
        v171.paths(DIAGNOSTIC_ROOT)["ready"]
    )
    ...
    if len(rows) != 20 or {row["seed"] for row in rows} != set(v155.SEEDS):
        raise RuntimeError("V171 complete training cohort changed")
    rows.sort(key=lambda row: row["seed"])
    evaluation = _evaluate_load("low", rows, _load_baselines())
    passed = evaluation["all_three_metric_gates_pass"] and blind["pass"]
    document = {
        "schema_version": "NSE_E1_HOMOGENEOUS_CONCURRENT3_QUOTA2_LOW_COMPLETE_TRAINING_RESULT_V171_V1",
        "created_at": utc_now(),
        "training_only": True,
        "formal_results_eligible": False,
        ...
        "disposition": (
            "freeze_complete_v171_low_cohort_and_authorize_separate_final_data_boundary"
            if passed
            else "retain_all_twenty_valid_v171_runs_and_require_new_causal_plan"
        ),
