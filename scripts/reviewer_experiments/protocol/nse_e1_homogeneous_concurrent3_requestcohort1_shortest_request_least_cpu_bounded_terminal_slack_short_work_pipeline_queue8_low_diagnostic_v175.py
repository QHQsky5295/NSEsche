...
ROOT = Path(
    "tmp/nse_e1_homogeneous_concurrent3_requestcohort1_shortest_request_least_cpu_bounded_terminal_low_diagnostic_20260901_v175"
)
PLAN = Path(
    "scripts/reviewer_experiments/protocol/"
    "nse_e1_homogeneous_concurrent3_requestcohort1_shortest_request_least_cpu_bounded_terminal_low_diagnostic_plan_v175.json"
)
PLAN_SHA256 = "33e03484f7604b45b09cf460a48a902663f1f940d8f3aa922a2170747908db55"
IMPLEMENTATION = Path(
    "scripts/reviewer_experiments/protocol/"
    "nse_e1_homogeneous_concurrent3_requestcohort1_shortest_request_least_cpu_bounded_terminal_slack_short_work_pipeline_queue8_low_diagnostic_implementation_v175.json"
)
IMPLEMENTATION_SHA256 = (
    "b06aff8bebbac29d46f246e3c2f58228a03cb4b4bfa8666c35ed87a7c6aa7cb5"
)
V170_FAILURE = Path(
    "scripts/reviewer_experiments/protocol/"
...
V174_FAILURE = Path(
    "scripts/reviewer_experiments/protocol/"
    "nse_e1_homogeneous_concurrent3_quota1_shortest_request_least_cpu_bounded_terminal_slack_short_work_pipeline_queue8_low_diagnostic_failure_v174.json"
)
V174_FAILURE_SHA256 = "e2fe2203272029058e18eed42a232f7aa79aa8c17bdaa0ad67758376af4a0485"
V174_FAILURE_HASH = "db049512d153010a3c2746753e3dc6b822d69d40c49e09f666201a77a019bf1c"
...
ARM_ID = "v175-low-srpt-slack-concurrent3-requestcohort1-shortest-request-least-cpu-bounded-terminal-short5p5-pipeline-hiku2-ocs-queue8"
PROFILE = "srpt_slack_concurrent3_requestcohort1_shortest_request_least_cpu_bounded_terminal_short5p5_pipeline_hiku2_ocs_queue8"
FRONTIER = "parents_completed_or_concurrent3_requestcohort1_shortest_request_least_cpu_bounded_terminal_or_slack_short_work_parents_scheduled"
SINGLE_CHANGE = (
    "V174_one_heavy_terminal_player_quota_changed_to_all_current_heavy_terminal_"
    "players_in_the_single_selected_shortest_request_cohort"
)
TERMINAL_DEFINITION = (
    "admit_all_parents-completed_players;when_current_heavy_terminal_player_count_"
    "strictly_exceeds_two_select_one_request_by_shortest_current_request_remaining_"
    "work_then_request_minimum_immutable_terminal_function_CPU_work_then_request-id_"
    "and_admit_all_current_heavy_incomplete-parent-terminal_players_in_that_request_"
    "while_applying_the_V168_CPU_bound_to_heavy_players_in_other_requests;otherwise_"
    "retain_V159_terminal_admission;retain_V159_nonterminal_short-work_frontier"
)
CPU_THRESHOLD = 1.0
HEAVY_PLAYER_THRESHOLD = 2
MINIMUM_ACTIVE_HEAVY_PLAYERS = 3
ACTIVE_HEAVY_QUOTA = 1
ACTIVE_HEAVY_QUOTA_UNIT = "request_cohort"
QUOTA_SELECTION_ORDER = (
    "ascending_current_request_remaining_work_then_request_minimum_immutable_terminal_"
    "function_cpu_work_then_request_id_select_all_current_heavy_terminal_players_in_"
    "selected_request"
)
...
BINARY_SOURCE_COMMIT = "5b7d0576429d6402763e579fb750f5b98b3a2f9d"
BINARY_PATH = Path("serverless_sim/target_e1_v175/release/serverless_sim.exe")
BINARY_SHA256 = "a1d04ad7de3a8b98c5478cc60248ea1e44754858ae81cb593a30b11e88a575de"


def paths(root: Path = ROOT) -> dict[str, Path]:
    return {
        "manifest": root / f"manifest.{ARM_ID}.unbound.json",
        "prepared": root / "prepared-v175.json",
        "schedule": root / "frozen-run-order-v175.json",
        ...
        "execution": root / "execution-receipt-v175.json",
        "pairing": root / "pairing-audit-v175.json",
        "blind": root / "joint-blind-audit-v175.json",
        "result": root / "diagnostic-result-v175.json",
    }
...
def _assert_frozen_inputs() -> dict[str, Any]:
    ...
    for path, sha256, label in (
        (PLAN, PLAN_SHA256, "V175 plan"),
        (IMPLEMENTATION, IMPLEMENTATION_SHA256, "V175 implementation receipt"),
        ...
        (V174_FAILURE, V174_FAILURE_SHA256, "V174 sealed diagnostic failure receipt"),
        (
            V170_COMPLETE_RESULT,
            V170_COMPLETE_RESULT_SHA256,
            "V170 sealed complete result",
        ),
        (BINARY_PATH, BINARY_SHA256, "V175 release binary"),
    ...
    if not (
        ...
        and change.get("active_heavy_quota_unit") == ACTIVE_HEAVY_QUOTA_UNIT
        and change.get("request_cohort_selection_order")
        ...
        == "admit_only_the_single_heavy_incomplete_parent_terminal_request_function_player_selected_by_shortest_request_then_least_function_cpu_work"
        and change.get("new_active_heavy_behavior")
        == "select_one_request_cohort_by_shortest_current_request_remaining_work_then_request_minimum_immutable_terminal_function_cpu_work_then_request_id_and_admit_all_current_heavy_incomplete_parent_terminal_players_in_that_request"
        and change.get("selected_cohort_admission")
        == "all_current_heavy_incomplete_parent_terminal_players_in_selected_request"
        and change.get("other_request_active_heavy_behavior")
        ...
        raise RuntimeError("V175 implementation boundary changed")
    failure = read_json(V174_FAILURE)
    if not (
        _assert_hashed(failure, "receipt_hash", "V174 diagnostic failure receipt")
        == V174_FAILURE_HASH
        and failure.get("disposition", {}).get("retain_all_six_valid_v174_runs") is True
        and failure.get("disposition", {}).get(
            "delete_replace_relabel_or_selectively_rerun_any_v174_seed"
        )
        is False
    ):
        raise RuntimeError("V174 diagnostic failure boundary changed")
...
def _rewrite_candidate(
    ...
    marker.update(
        {
            "purpose": (
                "V175 outcome-disclosed six-seed concurrent3 requestcohort1 CPU-bounded "
                "terminal-starvation diagnostic; "
                "never a formal result or paper superiority claim"
            ),
            "v175_role": "result_blind_concurrent3_requestcohort1_shortest_request_least_cpu_bounded_terminal_falsification",
            "v175_plan_sha256": PLAN_SHA256,
            "v175_implementation_sha256": IMPLEMENTATION_SHA256,
            "v175_binary_source_commit": BINARY_SOURCE_COMMIT,
            "v175_protocol_source_commit": protocol_source_commit,
            "v175_binary_sha256": BINARY_SHA256,
            "v175_arm_id": ARM_ID,
            "v175_profile": PROFILE,
            "v175_player_frontier": FRONTIER,
            "v175_single_change_from_v174": SINGLE_CHANGE,
            "v175_cpu_threshold": CPU_THRESHOLD,
            "v175_cpu_boundary": "at_or_below_is_admitted",
            "v175_heavy_player_threshold": HEAVY_PLAYER_THRESHOLD,
            "v175_minimum_active_heavy_players": MINIMUM_ACTIVE_HEAVY_PLAYERS,
            "v175_overload_activation_boundary": "strictly_above_activates",
            "v175_inactive_behavior": "exact_V159_terminal_admission",
            "v175_active_heavy_admission_policy": "deterministic_shortest_request_cohort_one",
            "v175_active_heavy_admission_quota": ACTIVE_HEAVY_QUOTA,
            "v175_active_heavy_admission_quota_unit": ACTIVE_HEAVY_QUOTA_UNIT,
            "v175_quota_selection_order": QUOTA_SELECTION_ORDER,
            "v175_short_work_threshold": SHORT_WORK_THRESHOLD,
            "v175_queue_density_threshold": QUEUE_THRESHOLD,
            "v175_queue_boundary": "below_is_strict",
            "v175_environment": COMMON_ENVIRONMENT,
            "v175_expected_run_count": len(SEEDS),
            "v175_expected_reference_build_count": len(SEEDS),
            "v175_fixed_order": list(SEEDS),
            "v175_candidate_performance_summaries_parsed": 0,
            "v175_remaining_fourteen_authorized": False,
            "v175_confirmation_inputs_generated": False,
        }
    )
    for run in rewritten["runs"]:
        ...
        run["metadata"] = {
            ...
            "v175_queue_density_threshold": QUEUE_THRESHOLD,
            "v175_queue_boundary": "below_is_strict",
            "v175_source_e1_run_id": source_run_id,
            "v175_source_e1_run_spec_hash": source_run_spec_hash,
            "v175_candidate_performance_summaries_parsed_before_run": 0,
            "v175_remaining_fourteen_authorized": False,
            "v175_confirmation_inputs_generated": False,
        }
...
def _validate_product(manifest: Mapping[str, Any], *, references_bound: bool) -> None:
    if not (
        ...
        raise RuntimeError("V175 exact E01/E02/E13/E15/E16/E20 product changed")
    ...
    for run in manifest["runs"]:
        ...
        if not (
            ...
            and metadata.get("v175_profile") == PROFILE
            and metadata.get("v175_player_frontier") == FRONTIER
            and metadata.get("v175_cpu_threshold") == CPU_THRESHOLD
            and metadata.get("v175_cpu_boundary") == "at_or_below_is_admitted"
            and metadata.get("v175_heavy_player_threshold") == HEAVY_PLAYER_THRESHOLD
            and metadata.get("v175_minimum_active_heavy_players")
            == MINIMUM_ACTIVE_HEAVY_PLAYERS
            and metadata.get("v175_overload_activation_boundary")
            == "strictly_above_activates"
            and metadata.get("v175_inactive_behavior")
            == "exact_V159_terminal_admission"
            and metadata.get("v175_active_heavy_admission_policy")
            == "deterministic_shortest_request_cohort_one"
            and metadata.get("v175_active_heavy_admission_quota") == ACTIVE_HEAVY_QUOTA
            and metadata.get("v175_active_heavy_admission_quota_unit")
            == ACTIVE_HEAVY_QUOTA_UNIT
            and metadata.get("v175_quota_selection_order") == QUOTA_SELECTION_ORDER
            and metadata.get("v175_short_work_threshold") == SHORT_WORK_THRESHOLD
            and metadata.get("v175_queue_density_threshold") == QUEUE_THRESHOLD
            and metadata.get("v175_queue_boundary") == "below_is_strict"
        ):
            raise RuntimeError(f"V175 run contract changed: {run.get('run_id')}")


def prepare_v175(root: Path = ROOT) -> dict[str, Any]:
    source = _assert_frozen_inputs()
    if root.exists():
        raise RuntimeError(f"refusing to overwrite V175 root: {root}")
    ...
    schedule = {
        "schema_version": "NSE_E1_HOMOGENEOUS_CONCURRENT3_REQUESTCOHORT1_SHORTEST_REQUEST_LEAST_CPU_BOUNDED_TERMINAL_LOW_SCHEDULE_V175_V1",
        "created_at": utc_now(),
    ...
    receipt = {
        "schema_version": "NSE_E1_HOMOGENEOUS_CONCURRENT3_REQUESTCOHORT1_SHORTEST_REQUEST_LEAST_CPU_BOUNDED_TERMINAL_LOW_PREPARED_V175_V1",
        "created_at": utc_now(),
        ...
        "v174_diagnostic_failure_file_sha256": V174_FAILURE_SHA256,
        "v174_diagnostic_failure_hash": V174_FAILURE_HASH,
        ...
        "active_heavy_admission_quota_unit": ACTIVE_HEAVY_QUOTA_UNIT,
...
def execute_v175(root: Path = ROOT) -> dict[str, Any]:
    output = paths(root)
    if output["execution"].exists():
        raise RuntimeError("V175 execution receipt already exists")
    prepared = read_json(output["prepared"])
    _assert_hashed(prepared, "receipt_hash", "V175 prepared receipt")
    ...
    for ordinal, seed in enumerate(SEEDS, start=1):
        ...
        if completed.returncode != 0:
            raise RuntimeError(f"V175 dispatch {seed} failed: {completed.returncode}")
        ...
        if not (
            ...
            raise RuntimeError(f"V175 canonical is not a QC pass: {run['run_id']}")
        dispatches.append(
            {
                "ordinal": ordinal,
                "seed": seed,
    ...
    receipt = {
        "schema_version": "NSE_E1_HOMOGENEOUS_CAPACITY_OVERLOAD_CPU_BOUNDED_TERMINAL_LOW_EXECUTION_V175_V1",
        "created_at": utc_now(),
        "candidate_performance_summaries_parsed": 0,
        "plan_sha256": PLAN_SHA256,
...
def _audit_nash_log(
    ...
    *,
    ...
    totals = {
        ...
        "active_heavy_selected_request_count": 0,
        "active_heavy_selected_request_candidate_players": 0,
        "active_heavy_request_cohort_expansion_windows": 0,
        "shortest_request_order_certified_active_windows": 0,
        "low_routes": 0,
        "high_routes": 0,
    ...
    extrema: dict[str, float | None] = {
        ...
        "quota_admitted_request_remaining_work_max": None,
        "quota_rejected_request_remaining_work_min": None,
    }
    ...
    with gzip.open(log, "rt", encoding="utf-8") as stream:
        for line in stream:
            ...
            if kind not in counts:
                raise RuntimeError(f"unexpected V175 Nash observation kind: {kind}")
            counts[kind] += 1
            if kind == "run_config":
                ...
                if not (
                    ...
                    == ACTIVE_HEAVY_QUOTA
                    and activation.get("active_heavy_admission_quota_unit")
                    == ACTIVE_HEAVY_QUOTA_UNIT
                    ...
                    raise RuntimeError("V175 run_config contract changed")
            elif kind == "window":
                if event.get("frame") != counts["window"] - 1:
                    raise RuntimeError("V175 scheduler window sequence changed")
                ...
                if (
                    ...
                    raise RuntimeError("V175 decision frontier/hash changed")
                ...
                selected_request_count_now = v159._count(
                    activation.get("active_heavy_selected_request_count"),
                    "active heavy selected request count",
                )
                selected_request_candidates_now = v159._count(
                    activation.get("active_heavy_selected_request_candidate_players"),
                    "active heavy selected request candidate player count",
                )
                quota_admitted_request_work_now = v159._finite_optional(
                    activation.get(
                        "active_heavy_quota_admitted_request_remaining_work_max"
                    ),
                    "active heavy quota admitted request remaining-work maximum",
                )
                quota_rejected_request_work_now = v159._finite_optional(
                    activation.get(
                        "active_heavy_quota_rejected_request_remaining_work_min"
                    ),
                    "active heavy quota rejected request remaining-work minimum",
                )
                ...
                if not (
                    ...
                    and (
                        guard_active_now
                        and selected_request_count_now == ACTIVE_HEAVY_QUOTA
                        and selected_request_candidates_now > 0
                        and quota_selected_now == selected_request_candidates_now
                        and quota_admitted_now == selected_request_candidates_now
                        and quota_admitted_now <= heavy_now
                        ...
                        and quota_admitted_request_work_now is not None
                        and (
                            quota_rejected_now == 0
                            and guard_rejected_ratio_now is None
                            and quota_rejected_request_work_now is None
                            or quota_rejected_now > 0
                            and guard_rejected_ratio_now is not None
                            and quota_rejected_request_work_now is not None
                            and quota_admitted_request_work_now
                            <= quota_rejected_request_work_now
                        )
                        ...
                        and selected_request_count_now == 0
                        and selected_request_candidates_now == 0
                        and quota_selected_now == 0
                        and quota_admitted_now == 0
                        and quota_rejected_now == 0
                        and quota_admitted_request_work_now is None
                        and quota_rejected_request_work_now is None
                    )
                    ...
                    raise RuntimeError("V175 CPU/slack/queue frontier evidence changed")
                ...
                ] += selected_request_count_now
                totals[
                    "active_heavy_selected_request_candidate_players"
                ] += selected_request_candidates_now
                totals["active_heavy_request_cohort_expansion_windows"] += int(
                    guard_active_now and quota_admitted_now > 1
                )
                totals["shortest_request_order_certified_active_windows"] += int(
                    guard_active_now
                    and quota_admitted_request_work_now is not None
                    and (
                        quota_rejected_now == 0
                        or quota_rejected_request_work_now is not None
                        and quota_admitted_request_work_now
                        <= quota_rejected_request_work_now
                    )
                )
                ...
                update_max(
                    "quota_admitted_request_remaining_work_max",
                    quota_admitted_request_work_now,
                )
                update_min(
                    "quota_rejected_request_remaining_work_min",
                    quota_rejected_request_work_now,
                )
                ...
                if not (
                    ...
                    raise RuntimeError("V175 route telemetry is incomplete")
                expected = (
                    LOW_EXPERT if float(density) < QUEUE_THRESHOLD else HIGH_EXPERT
                )
                if route.get("selected_expert") != expected:
                    raise RuntimeError("V175 route does not match queue density")
                ...
                if key is None:
                    if source != "not_requested":
                        raise RuntimeError("V175 unrequested reference reason changed")
                    totals["reference_not_requested"] += 1
                elif source in ("offline_table", "offline_table_nonpositive"):
                    totals["reference_available"] += 1
                else:
                    raise RuntimeError("V175 bound reference source changed")
                ...
                if not (
                    ...
                    raise RuntimeError("V175 Nash terminal marker changed")
    ...
    if (
        ...
        raise RuntimeError("V175 Nash log cardinality changed")
    if (
        totals["reference_available"] + totals["reference_not_requested"]
        != counts["window"]
    ):
        raise RuntimeError("V175 reference replay coverage changed")
    ...
    return {
        ...
        "active_heavy_selected_request_count": totals[
            "active_heavy_selected_request_count"
        ],
        "active_heavy_selected_request_candidate_players": totals[
            "active_heavy_selected_request_candidate_players"
        ],
        "active_heavy_request_cohort_expansion_windows": totals[
            "active_heavy_request_cohort_expansion_windows"
        ],
        "shortest_request_order_certified_active_windows": totals[
            "shortest_request_order_certified_active_windows"
        ],
        "active_heavy_quota_admitted_request_remaining_work_max": extrema[
            "quota_admitted_request_remaining_work_max"
        ],
        "active_heavy_quota_rejected_request_remaining_work_min": extrema[
            "quota_rejected_request_remaining_work_min"
        ],
...
def _mechanism_falsification_gate(
    ...
    if set(by_seed) != set(SEEDS):
        raise RuntimeError("V175 blind audit seed product changed")
    ...
    request_cohort_exercised = (
        ...
        == totals["active_heavy_selected_request_candidate_players"]
        and totals["active_heavy_selected_request_count"]
        == ACTIVE_HEAVY_QUOTA * totals["capacity_overload_guard_active_windows"]
        and totals["active_heavy_quota_admitted_players"]
        >= totals["capacity_overload_guard_active_windows"]
        and totals["active_heavy_quota_rejected_excess_players"] > 0
        and totals["capacity_overload_heavy_incomplete_parent_terminal_players"]
        == totals["active_heavy_quota_selected_players"]
        + totals["active_heavy_quota_rejected_excess_players"]
        + totals["capacity_overload_guard_inactive_heavy_terminal_admissions"]
        and totals["active_heavy_request_cohort_expansion_windows"] > 0
        and totals["shortest_request_order_certified_active_windows"]
        == totals["capacity_overload_guard_active_windows"]
    )
    passed = (
    ...
    return {
        ...
        **extrema,
        ...
        "deterministic_active_heavy_request_cohort_exercised_and_complete": request_cohort_exercised,
        "request_cohort_expansion_exercised": (
            totals["active_heavy_request_cohort_expansion_windows"] > 0
        ),
        "shortest_request_cohort_selection_order_certified_in_every_active_window": (
            totals["shortest_request_order_certified_active_windows"]
            == totals["capacity_overload_guard_active_windows"]
        ),
...
def blind_audit_v175(root: Path = ROOT) -> dict[str, Any]:
    output = paths(root)
    if output["blind"].exists():
        raise RuntimeError("V175 blind audit already exists")
    _assert_frozen_inputs()
    prepared = read_json(output["prepared"])
    prepared_hash = _assert_hashed(prepared, "receipt_hash", "V175 prepared receipt")
    execution = read_json(output["execution"])
    execution_hash = _assert_hashed(execution, "receipt_hash", "V175 execution receipt")
    manifest = load_and_validate_manifest(output["ready"])
    _validate_product(manifest, references_bound=True)
    ...
    if (
        ...
        raise RuntimeError("V175 exact pairing failed")
    ...
    if [item["seed"] for item in execution["dispatches"]] != list(SEEDS):
        raise RuntimeError("V175 execution order changed")
    ...
    if {path.name for path in canonical_root.iterdir() if path.is_dir()} != {
        ...
        raise RuntimeError("V175 canonical product changed")
    quarantine = output["workspace"] / "quarantine"
    if quarantine.exists() and any(quarantine.rglob("attempt-*")):
        raise RuntimeError("V175 has unexplained quarantined attempts")
    ...
    if len(identities) != 1:
        raise RuntimeError("V175 runtime identity is not unanimous")
    ...
    if not (
        ...
        raise RuntimeError("V175 runtime identity changed")
    mechanism = _mechanism_falsification_gate(audits)
    if not mechanism["pass"]:
        raise RuntimeError("V175 mechanism falsification breadth is insufficient")
    document = {
        "schema_version": "NSE_E1_HOMOGENEOUS_CONCURRENT3_CPU_BOUNDED_TERMINAL_LOW_BLIND_AUDIT_V175_V1",
        "created_at": utc_now(),
        "status": "pass",
...
def _load_candidate(
    ...
    if len(rows) != len(SEEDS) or [row["seed"] for row in rows] != list(SEEDS):
        raise RuntimeError("V175 candidate result product changed")
    return rows


def _hybrid_rows_v175(
    v170_rows: Sequence[Mapping[str, Any]],
    v175_rows: Sequence[Mapping[str, Any]],
) -> list[dict[str, Any]]:
    ...
    if len(v175_rows) != len(SEEDS) or {row["seed"] for row in v175_rows} != set(SEEDS):
        raise RuntimeError("V175 diagnostic candidate cohort changed")
    replacements = {row["seed"]: dict(row) for row in v175_rows}
    return [
...
def reveal_v175(root: Path = ROOT) -> dict[str, Any]:
    output = paths(root)
    if output["result"].exists():
        raise RuntimeError("V175 result already exists")
    blind = read_json(output["blind"])
    blind_hash = _assert_hashed(blind, "blind_audit_hash", "V175 blind audit")
    if not (
        ...
        is True
        and blind.get(
            "deterministic_active_heavy_request_cohort_exercised_and_complete"
        )
        is True
        and blind.get("request_cohort_expansion_exercised") is True
        and blind.get(
            "shortest_request_cohort_selection_order_certified_in_every_active_window"
        )
        is True
        ...
        raise RuntimeError("V175 blind audit did not authorize reveal")
    manifest = load_and_validate_manifest(output["ready"])
    candidate = _load_candidate(manifest, root)
    v170_rows = _load_v170_candidate()
    hybrid = _hybrid_rows_v175(v170_rows, candidate)
    ...
    document = {
        "schema_version": "NSE_E1_HOMOGENEOUS_CONCURRENT3_CPU_BOUNDED_TERMINAL_LOW_DIAGNOSTIC_RESULT_V175_V1",
        "created_at": utc_now(),
        "training_only": True,
        "formal_results_eligible": False,
        ...
        "disposition": (
            ...
            if passed
            else "retain_all_six_valid_diagnostic_runs_and_retire_concurrent3_requestcohort1_shortest_request_least_cpu_bounded_terminal_candidate"
        ),
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    if action == "prepare":
        document, key = prepare_v175(), "receipt_hash"
    elif action == "execute":
        document, key = execute_v175(), "receipt_hash"
    elif action == "blind-audit":
        document, key = blind_audit_v175(), "blind_audit_hash"
    else:
        document, key = reveal_v175(), "result_hash"
