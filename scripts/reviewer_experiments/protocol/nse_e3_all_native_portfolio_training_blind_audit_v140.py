...
from scripts.reviewer_experiments.protocol.nse_e3_all_native_portfolio_training_execute_v140 import (
    EXECUTION_RECEIPT,
    READY_SCHEDULE,
    ready_manifest_path,
    workspace_path,
...
OUTPUT = ROOT / "joint-blind-audit-v140-training.json"
RESULT = ROOT / "training-result-v140.json"
...
ARMS = {
    arm_id: {"profile": profile, "portfolio_rule": rule, "run_count": 9}
...
def _verify_tapes() -> tuple[list[dict[str, Any]], dict[str, Any]]:
    _require(TAPES.is_file(), f"missing V140 tape catalog: {TAPES}")
    catalog = read_json(TAPES)
    catalog_hash = _assert_hashed_object(catalog, "catalog_hash", "V140 tapes")
    entries = catalog.get("entries")
    _require(
        isinstance(entries, dict) and len(entries) == 12, "V140 tape count changed"
    )
    evidence = []
    for key, entry in sorted(entries.items()):
        info = inspect_tape(Path(entry["path"]))
        _require(info.sha256 == entry["sha256"], f"V140 tape hash changed: {key}")
        _require(info.workload_seed in TRAINING_SEEDS, f"V140 tape seed changed: {key}")
        evidence.append(
            {
                "key": key,
                "sha256": info.sha256,
    ...
    _require(
        ...
        f"V140 capture ledger changed: {counts}",
    )
    _require(
        not list((capture_root / "quarantine").glob("**/attempt-*")),
        "V140 tape capture quarantine is nonempty",
    )
    return evidence, {
...
def _verify_references() -> tuple[list[dict[str, Any]], dict[str, str]]:
    ...
    for arm_id in ARM_IDS:
        ...
        _require(path.is_file(), f"missing V140 reference catalog: {arm_id}")
        catalog = read_json(path)
        catalog_hash = _assert_hashed_object(
            catalog, "catalog_hash", f"V140 references {arm_id}"
        )
        entries = catalog.get("entries")
        _require(
            isinstance(entries, dict) and len(entries) == 9,
            f"V140 reference count changed: {arm_id}",
        )
        root = _stage_root_from_receipts(
            entries, "receipt_path", 9, f"V140 references {arm_id}"
        )
        rows, last_hash = _read_ledger(root / "ledger.jsonl")
        counts = Counter(row["event_type"] for row in rows)
        _require(
            counts == Counter({"reference_build_canonicalized": 9}),
            f"V140 reference ledger changed: {arm_id}/{counts}",
        )
        _require(
            not list((root / "quarantine").glob("**/attempt-*")),
            f"V140 reference quarantine is nonempty: {arm_id}",
        )
        ...
        for key, entry in sorted(entries.items()):
            ...
            _require(
                ...
                f"V140 reference evidence changed: {key}",
            )
            evidence.append(
                {
                    "arm_id": arm_id,
    ...
    _require(len(evidence) == 27, "V140 reference evidence count changed")
...
def _expected_portfolio_selection(
    ...
    if rule == "minimax_service":
        selected = min(
            indices,
            key=lambda index: (
                _metric_key(candidates[index]["service_max"]),
        ...
        max_ties = [
            ...
            if _metric_equal(
                candidates[index]["service_max"], candidates[selected]["service_max"]
            )
        ]
        if len(max_ties) == 1:
            return selected, "minimum_service_max"
        sum_ties = [
            index
            for index in max_ties
            if _metric_equal(
                candidates[index]["service_sum"], candidates[selected]["service_sum"]
            )
        ]
        if len(sum_ties) == 1:
            return selected, "service_max_tie_minimum_service_sum"
        welfare_ties = [
            index
            for index in sum_ties
            if _metric_equal(
                candidates[index]["paper_welfare"],
                candidates[selected]["paper_welfare"],
            )
        ]
        return selected, (
            "service_and_sum_tie_maximum_paper_welfare"
            if len(welfare_ties) == 1
            else "all_metrics_tie_stable_native_kind"
        )
    if rule == "minsum_service":
        selected = min(
            indices,
            key=lambda index: (
                _metric_key(candidates[index]["service_sum"]),
        ...
        if len(sum_ties) == 1:
            return selected, "minimum_service_sum"
        max_ties = [
            index
            for index in sum_ties
            if _metric_equal(
        ...
        if len(max_ties) == 1:
            return selected, "service_sum_tie_minimum_service_max"
        welfare_ties = [
            index
            for index in max_ties
            if _metric_equal(
    ...
    _require(rule == "service_welfare_borda", f"unknown V140 portfolio rule: {rule}")
    selected = min(indices, key=lambda index: (ranks[index]["rank_sum"], index))
    rank_sum_ties = [
        index
        for index in indices
        if ranks[index]["rank_sum"] == ranks[selected]["rank_sum"]
    ]
    if len(rank_sum_ties) > 1:
        return selected, "ordinal_rank_sum_tie_stable_native_kind"
    metric_tie = any(
        index != selected
        and any(
            _metric_equal(candidates[index][field], candidates[selected][field])
            for field in ("service_max", "service_sum", "paper_welfare")
        )
        for index in indices
    )
    return selected, (
        ...
        if metric_tie
        else "minimum_ordinal_rank_sum"
    )


def _validate_native_diagnostics(
    run: dict[str, Any], canonical: Path, portfolio_rule: str
) -> dict[str, Any]:
    path = canonical / "reviewer_records" / run["run_id"] / "nash_metrics.jsonl.gz"
    _require(path.is_file(), f"missing V140 Nash diagnostics: {run['run_id']}")
    counts = Counter()
    reasons = Counter()
    with gzip.open(path, "rt", encoding="utf-8") as handle:
        for line_number, line in enumerate(handle, start=1):
            ...
            _require(isinstance(decision, dict), "missing V140 decision diagnostics")
            native = decision.get("native_shadow_anchor")
            _require(isinstance(native, dict), "missing V140 native shadow diagnostics")
            portfolio = decision.get("native_portfolio")
            _require(isinstance(portfolio, dict), "missing V140 portfolio diagnostics")
            players = decision.get("request_function_players")
            _require(
                type(players) is int and players >= 0,
                f"invalid V140 player count: {run['run_id']}:{line_number}",
            )
            _require(
                portfolio.get("certificate_uses_completion_outcomes") is False,
                f"V140 portfolio boundary changed: {run['run_id']}:{line_number}",
            )
            _require(
                ...
                f"V140 portfolio certificate definition changed: {run['run_id']}:{line_number}",
            )
            _require(
                ...
                f"V140 configured bandwidth snapshot changed: {run['run_id']}:{line_number}",
            )
            if players > 0:
                ...
                _require(
                    ...
                    f"V140 active portfolio boundary changed: {run['run_id']}:{line_number}",
                )
                ...
                _require(
                    ...
                    and [item.get("kind") for item in candidates] == NATIVE_KINDS,
                    f"V140 portfolio members changed: {run['run_id']}:{line_number}",
                )
                for candidate in candidates:
                    _require(
                        ...
                        f"V140 portfolio candidate invalid: {run['run_id']}:{line_number}",
                    )
                ...
                for candidate, expected_ranks in zip(candidates, ranks):
                    _require(
                        ...
                        f"V140 portfolio ranks changed: {run['run_id']}:{line_number}",
                    )
                selected, reason = _expected_portfolio_selection(
                    portfolio_rule, candidates, ranks
                )
                _require(
                    [item.get("selected") for item in candidates]
                    == [index == selected for index in range(5)]
                    and portfolio.get("selected_kind") == candidates[selected]["kind"]
                    and portfolio.get("deterministic_selection_reason") == reason,
                    f"V140 portfolio selection changed: {run['run_id']}:{line_number}",
                )
                counts[f"selected_{candidates[selected]['kind']}"] += 1
                counts[f"selection_reason_{reason}"] += 1
                _require(
                    native.get("kind") == candidates[selected]["kind"]
                    ...
                    == candidates[selected]["assignment_hash"]
                    and native.get("ordered_command_hash")
                    == candidates[selected]["ordered_command_hash"]
                    and native.get("service_certificate_scope")
                    == "all_feasible_players"
                    ...
                    == candidates[selected]["service_sum"]
                    and native.get("initializer_readiness_service_max")
                    == candidates[selected]["service_max"],
                    f"V140 selected native mismatch: {run['run_id']}:{line_number}",
                )
            _require(
                native.get("certificate_uses_completion_outcomes") is False,
                f"V140 native certificate consulted outcomes: {run['run_id']}",
            )
            ...
            _require(
                ...
                f"V140 native service cohort count invalid: {run['run_id']}:{line_number}",
            )
            guard = decision.get("window_safe_guard")
            _require(isinstance(guard, dict), "missing V140 native guard diagnostics")
            reason = guard.get("reason")
            _require(isinstance(reason, str), "invalid V140 native guard reason")
            if players == 0:
                _require(
                    ...
                    f"V140 empty-window diagnostics mismatch: {run['run_id']}:{line_number}",
                )
                reasons[reason] += 1
                continue
            ...
            if initializer_complete and proposal_complete:
                _require(
                    ...
                    f"V140 native service cohort mismatch: {run['run_id']}:{line_number}",
                )
                ...
                _require(
                    ...
                    f"V140 native service certificate incomplete: {run['run_id']}:{line_number}",
                )
                ...
                _require(
                    ...
                    f"V140 native service deltas changed: {run['run_id']}:{line_number}",
                )
            else:
                _require(
                    guard.get("accepted") is False,
                    f"V140 unavailable native service certificate was accepted: {run['run_id']}:{line_number}",
                )
                if not initializer_complete:
                    _require(
                        initializer_players == 0
                        and reason == "initializer_readiness_service_unavailable",
                        f"V140 initializer service fallback mismatch: {run['run_id']}:{line_number}",
                    )
                else:
                    _require(
                        ...
                        f"V140 proposal service fallback mismatch: {run['run_id']}:{line_number}",
                    )
            ...
            _require(
                ...
                == float(candidates[selected]["paper_welfare"])
                and _f32(float(proposal_welfare) - float(initializer_welfare))
                == float(welfare_delta),
                f"V140 native welfare certificate changed: {run['run_id']}:{line_number}",
            )
            ...
            _require(
                ...
                f"V140 native guard decision changed: {run['run_id']}:{line_number}",
            )
            reasons[reason] += 1
            if guard.get("accepted") is True:
                ...
                _require(
                    ...
                    f"V140 native accepted window violated certificate: {run['run_id']}:{line_number}",
                )
    _require(counts["windows"] == 4000, f"V140 window count changed: {run['run_id']}")
    _require(
        counts["player_windows"] > 0,
        f"V140 has no native player windows: {run['run_id']}",
    )
    return {
...
def _validate_execution_receipt() -> dict[str, Any]:
    ...
    _require(ready_schedule_path.is_file(), "missing V140 ready schedule")
    _require(execution_path.is_file(), "missing V140 execution receipt")
    schedule = read_json(ready_schedule_path)
    schedule_hash = _assert_hashed_object(
        schedule, "schedule_hash", "V140 ready schedule"
    )
    receipt = read_json(execution_path)
    receipt_hash = _assert_hashed_object(
        receipt, "receipt_hash", "V140 execution receipt"
    )
    _require(
        ...
        "V140 execution receipt boundary changed",
    )
    for scheduled, dispatched in zip(schedule["schedule"], receipt["dispatches"]):
        ...
        _require(
            ...
            f"V140 frozen dispatch changed: {scheduled['ordinal']}",
        )
    return {
...
def run_blind_audit(output: Path = OUTPUT) -> dict[str, Any]:
    _require(not output.exists(), f"V140 blind audit already exists: {output}")
    _require(not RESULT.exists(), "V140 reveal exists before blind audit")
    _require(PLAN.is_file() and file_hash(PLAN) == PLAN_SHA256, "V140 plan changed")
    _require(PREPARED.is_file(), "missing V140 prepared receipt")
    prepared = read_json(PREPARED)
    prepared_hash = _assert_hashed_object(
        prepared, "receipt_hash", "V140 prepared receipt"
    )
    _require(
        ...
        and prepared.get("v139_disposition")
        == "aborted_pre_reveal_technical_audit_failure_retained"
        and prepared.get("v139_performance_summaries_parsed") == 0
        and prepared.get("v139_performance_results_consulted") is False
        and prepared.get("v139_reveal_authorized") is False
        and prepared.get("total_online_runs") == 108
        and prepared.get("candidate_reference_builds") == 27,
        "V140 prepared scientific boundary changed",
    )
    _require(
        ...
        == [
            {
                ...
                "native_portfolio_members": NATIVE_KINDS,
                "run_count": 9,
                "reference_build_count": 9,
            }
            for arm_id, profile, rule in PREPARED_ARMS
        ],
        "V140 prepared treatment boundary changed",
    )
    ...
    for manifest_id, expected_count, arm in products:
        ...
        _require(
            ...
            f"V140 ready boundary changed: {manifest_id}",
        )
        ...
        _require(
            ...
            f"V140 pairing changed: {manifest_id}",
        )
        ...
        _require(
            actual_ids == expected_ids, f"V140 canonical set changed: {manifest_id}"
        )
        _require(
            not list((workspace / "quarantine").glob("**/attempt-*")),
            f"V140 online quarantine is nonempty: {manifest_id}",
        )
        ...
        _require(
            ...
            f"V140 online ledger changed: {manifest_id}/{counts}",
        )
        for run in manifest["runs"]:
            if candidate:
                _require(
                    ...
                    == arm["profile"]
                    and run.get("metadata", {}).get("v140_native_portfolio_rule")
                    == arm["portfolio_rule"]
                    and run.get("metadata", {}).get("v140_native_portfolio_members")
                    == NATIVE_KINDS
                    and run.get("metadata", {}).get(
                        "v140_all_native_members_use_all_frontier"
                    )
                    is True
                    and run.get("metadata", {}).get(
                        "v140_service_certificate_state_domain"
                    )
                    == "runtime_existing_aggregates_and_admitted_work"
                    and run.get("metadata", {}).get("v140_paper_welfare_state_domain")
                    == "empty_window_aggregates"
                    and run.get("metadata", {}).get(
                        "v140_selected_welfare_equals_guard_initializer_required"
                    )
                    is True
                    and run.get("metadata", {}).get("v140_outcome_fields_drive_policy")
                    is False
                    and isinstance(run.get("reference_dependency"), dict),
                    f"V140 candidate manifest boundary changed: {run['run_id']}",
                )
            else:
                _require(
                    run.get("method") in BASELINE_METHODS
                    and run.get("reference_dependency") is None,
                    f"V140 baseline manifest boundary changed: {run['run_id']}",
                )
            ...
            _require(
                ...
                f"V140 canonical status changed: {run['run_id']}",
            )
            ...
            diagnostics = (
                _validate_native_diagnostics(run, canonical, str(arm["portfolio_rule"]))
                if candidate
                else None
            )
    ...
    _require(len(run_evidence) == 108, "V140 run evidence count changed")
    for field, expected in EXPECTED_RUNTIME.items():
        _require(
            runtime_values[field] == {expected},
            f"V140 runtime {field} changed: {runtime_values[field]}",
        )
    ...
    _require(
        ...
        f"V140 runtime git identity changed: {git_commits}",
    )
    _require(len(paired_inputs) == 9, "V140 paired block count changed")
    for (scenario, seed), rows in paired_inputs.items():
        _require(
            len(rows) == 12
            and {row["method_label"] for row in rows} == set(METHOD_LABELS),
            f"V140 block product changed: {scenario}/{seed}",
        )
        for field in (
            ...
            _require(
                ...
                f"V140 paired {field} changed: {scenario}/{seed}",
            )
        _require(
            rows[0]["common_hpa_hash"] == EXPECTED_COMMON_HPA_SHA256,
            f"V140 common HPA changed: {scenario}/{seed}",
        )

    payload = {
        "schema_version": "NSE_E3_ALL_NATIVE_PORTFOLIO_BLIND_AUDIT_V140_V1",
        "created_at": utc_now(),
        "status": "pass",
