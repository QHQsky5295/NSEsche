...
from scripts.reviewer_experiments.protocol.nse_e3_resource_headroom_safety_training_blind_audit_v106 import (
    ARMS,
...
OUTPUT = ROOT / "training-result-v106.json"
BLIND_AUDIT_FILE_SHA256 = (
    "974265bbdcca360e02989dafb5b2a4f101a2f948f3212fb6f3fc2c76d4530295"
)
BLIND_AUDIT_HASH = "8d75a0f936f025689771c110181f7b856f614774b573ad79302b7311fa347ba1"
EXPECTED_SEEDS = ("E887", "E888", "E889")
EXPECTED_SCENARIOS = (
    "E3.spike5x50ms",
    "E3.sustained3x200ms",
    "E3.pulse4x4x50ms",
...
ANCHORS = {"E3": "v106-e3-anchor"}
RESOURCE_SAFETY_PREFERENCE = {
    "v106-e3-band8-24-bidirectional-locality-noncritical-frontier-resource-pareto-initializer-only": 0,
    "v106-e3-band8-24-bidirectional-locality-noncritical-frontier-resource-bottleneck-sum-initializer-only": 1,
}
...
def evaluate_training_rows(
    ...
    ranked = sorted(
        ...
        key=lambda item: (
            ...
            RESOURCE_SAFETY_PREFERENCE[item[0]],
            item[0],
        ),
    )
...
def _validate_blind_audit() -> dict[str, Any]:
    if not BLIND_AUDIT.is_file() or file_hash(BLIND_AUDIT) != BLIND_AUDIT_FILE_SHA256:
        raise RuntimeError("V106 blind audit is missing or changed")
    ...
    if (
        ...
        raise RuntimeError("V106 blind audit does not authorize reveal")
    return blind
...
def _load_rows(blind: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    if len(admitted) != 27:
        raise RuntimeError("V106 blind audit run evidence is incomplete")
    ...
    for arm_id, expected in ARMS.items():
        ...
        if not manifest_path.is_file() or file_hash(manifest_path) != frozen.get(
            ...
            raise RuntimeError(f"V106 frozen manifest changed: {arm_id}")
        manifest = load_and_validate_manifest(manifest_path)
        if manifest.get("manifest_hash") != frozen.get("manifest_hash"):
            raise RuntimeError(f"V106 manifest hash changed: {arm_id}")
        for run in manifest["runs"]:
            ...
            if (
                ...
                != expected["profile"]
                or metadata.get("v106_training_plan_sha256") != PLAN_SHA256
                or metadata.get("v106_training_only") is not True
                or metadata.get("v106_training_seed_metrics_previously_revealed")
                is not False
                or metadata.get("v106_confirmation_seeds_opened") is not False
                or metadata.get("v106_other_unopened_seeds_opened") is not False
                or metadata.get("v106_arm_id") != arm_id
                or metadata.get("v106_arm_role") != expected["role"]
                or metadata.get("v106_upper_queue_density_threshold")
                != expected["upper_queue_density_threshold"]
                or metadata.get("v106_nonterminal_queue_density_floor")
                != expected["nonterminal_queue_density_floor"]
                or metadata.get("v106_warm_admissibility")
                != expected["warm_admissibility"]
                or metadata.get("v106_load_least_window_certificate_mode")
                != expected["load_least_window_certificate_mode"]
                or metadata.get("v106_resource_headroom_safety_mode")
                != expected["resource_headroom_safety_mode"]
                or metadata.get("v106_cpu_memory_individual_noninferiority")
                is not expected["cpu_memory_individual_noninferiority"]
                or metadata.get("v106_resource_bottleneck_sum_noninferiority")
                is not expected["resource_bottleneck_sum_noninferiority"]
                or metadata.get("v106_resource_inputs_finite_fail_closed")
                is not (expected["role"] == "candidate")
                or metadata.get("v106_componentwise_faasrank_noninferiority")
                is not False
                or metadata.get(
                ...
                is not False
                or metadata.get("v106_future_child_placement_or_feasibility_used")
                is not False
                or metadata.get("v106_critical_frontier_protection")
                is not (expected["role"] == "candidate")
                or metadata.get("v106_critical_frontier_rank_source")
                != (
                    "immutable_srpt_remaining_critical_path_rank"
                    if expected["role"] == "candidate"
                    else "not_applicable"
                )
                or metadata.get("v106_critical_frontier_tie_rule")
                != (
                ...
                is not (expected["role"] == "candidate")
                or metadata.get("v106_outcome_fields_drive_policy") is not False
            ):
                raise RuntimeError(f"V106 arm identity changed: {run['run_id']}")
            run_id = run["run_id"]
            summary_path = (
                ROOT
                / "runs"
            ...
            if not summary_path.is_file() or file_hash(summary_path) != admitted.get(
                ...
                raise RuntimeError(f"V106 summary differs from blind audit: {run_id}")
            ...
            if (
                ...
                raise RuntimeError(
                    f"V106 summary identity/completion mismatch: {run_id}"
                )
            rows.append(
                {
                    "arm_id": arm_id,
                    ...
                    "resource_headroom_safety_mode": expected[
...
def execute_reveal() -> dict[str, Any]:
    if OUTPUT.exists():
        raise RuntimeError(f"V106 training result already exists: {OUTPUT}")
    if not PLAN.is_file() or file_hash(PLAN) != PLAN_SHA256:
        raise RuntimeError("frozen V106 plan changed before reveal")
    blind = _validate_blind_audit()
    rows = _load_rows(blind)
    if len(rows) != 27:
        raise RuntimeError("V106 revealed row count differs from sealed plan")
    ...
    result = {
        "schema_version": "NSE_E3_RESOURCE_HEADROOM_SAFETY_RESULT_V106_V1",
        "created_at": utc_now(),
        ...
        "selection_rule": {
            ...
            "multiple_passers": (
                ...
                "throughput change, then prefer individual CPU-and-memory Pareto "
                "safety over bottleneck-and-total safety, then arm_id"
            ),
            "frozen_E4": (
                "retain V97 v97-e4-load-guard64 without V106 result-dependent reselection"
            ),
        ...
        **evaluation,
        "revealed_rows": rows,
        "decision": {
            "authorize_preregistration_of_E1006_E1025_confirmation": joint_pass,
            ...
            "next_action": (
                ...
                "separately preregister the complete E1006-E1025 NSESche confirmation "
                "before generating inputs"
                if joint_pass
                else (
                    "freeze the complete V106 training cohort, keep E926-E1005 and "
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Reveal the frozen V106 training cohort exactly once after blind audit."
    )
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    if args.command != "reveal" or not args.execute:
        parser.error("V106 reveal requires the explicit command: reveal --execute")
    execute_reveal()
