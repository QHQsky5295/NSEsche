...
PLAN = Path(
    ...
    "nse_e3_completion_proximal_terminal_child_hpa_anchor_argmin_path_training_plan_v129.json"
)
PLAN_SHA256 = "9749ff730c69ff11d46a63ceb312cd2af759c2cbeefecad6ee0786e8a273c7c3"
V128_RESULT = Path(
    "tmp/nse_e3_completion_proximal_terminal_child_hpa_container_componentwise_path_training_20260829_v128/"
    "training-result-v128.json"
)
V128_RESULT_SHA256 = "b9b09fd777219d51098c3dc527752976bbee81461769b06aae624bf615dcaf5f"
V128_RESULT_HASH = "b399d5ae8c2e4c7e76064bc6938adc2cb954b3c5c9851c830dd313b264d4805c"
V128_RESULT_COMMIT = "ee52de1abd5b6a18b4e6be6cb967a00501523450"
V128_BLIND_AUDIT = Path(
    "tmp/nse_e3_completion_proximal_terminal_child_hpa_container_componentwise_path_training_20260829_v128/"
    "joint-blind-audit-v128-training.json"
)
V128_BLIND_AUDIT_SHA256 = (
    "3b6882047c69420874ca3d13697c5a8aae5e3a4e4741e6f4e44d793c980c45df"
)
V128_BLIND_AUDIT_HASH = (
    "211c76eb7b171b8eca345bd0a573fa4f9f0c2aaa1bbc18e3c33d5329af4c1ab3"
)
V127_RESULT = Path(
...
BINARY_PATH = Path("tmp/nse_v129_build_8508ec0/release/serverless_sim.exe")
BINARY_SHA256 = "3aba2de871e68252b4f21bf39cfd080568c1cbd3ca68856662dbee8a30f29a5c"
BINARY_SOURCE_COMMIT = "8508ec0056b3d4050422faa74b68ba2a01e832a5"

SOURCE_INITIAL_SEEDS = [f"E{index}" for index in range(1310, 1320)]
SOURCE_CI_SEEDS = [f"E{index}" for index in range(1320, 1330)]
TRAINING_SEED_LIST = ["E1310", "E1311", "E1312"]
TRAINING_SEEDS = set(TRAINING_SEED_LIST)
CONFIRMATION_SEEDS = [f"E{index}" for index in range(1313, 1333)]
...
E3_ADMITTED_WORK = (
    ...
    "causal_arrival_shock15_horizon50_admitted_work10_completion_proximal_depth1_componentwise_service_terminal_child_hpa_anchor_argmin_path_pareto_initializer_only_"
    "guard64_dual_window_safe_pareto"
)
ARMS = (
    ("v129-e3-anchor", "anchor", E3_ANCHOR, 9),
    (
        "v129-e3-causal-shock15-horizon50-admitted-work10-completion-proximal-depth1-componentwise-service-terminal-child-hpa-anchor-argmin-path-pareto-initializer-only",
        "candidate",
        E3_ADMITTED_WORK,
        9,
    ),
...
def _paths(root: Path) -> dict[str, Path]:
    return {
        "config": root / "v129-e3-training-config.json",
        "source": root / "manifest.v129-e1310-e1319-full.unbound.json",
        "capture": root / "manifest.v129-tape-capture.unbound.json",
...
def _assert_frozen_inputs() -> None:
    for path, expected in (
        (PLAN, PLAN_SHA256),
        (V128_RESULT, V128_RESULT_SHA256),
        (V128_BLIND_AUDIT, V128_BLIND_AUDIT_SHA256),
        ...
        if not path.is_file() or file_hash(path) != expected:
            raise RuntimeError(f"frozen V129 input is missing or changed: {path}")
    ...
    *,
    ...
    marker.update(
        {
            ...
            "v129_binary_source_commit": BINARY_SOURCE_COMMIT,
    ...
    if arm is not None:
        if frozen_model_config is None:
            raise RuntimeError("V129 arm rewrite lacks frozen model binding")
        arm_id, role, profile, expected = arm
        candidate = role == "candidate"
        marker.update(
            {
                "v129_arm_id": arm_id,
                "v129_experiment_id": "E3",
                "v129_role": role,
                "v129_profile": profile,
                "v129_shock_threshold_numerator": 3 if candidate else None,
                "v129_shock_threshold_denominator": 2 if candidate else None,
                "v129_shock_activation_horizon_frames": 50 if candidate else None,
                "v129_critical_service_ratio_numerator": 9 if candidate else None,
                "v129_critical_service_ratio_denominator": 10 if candidate else None,
                ...
                "v129_expected_run_count": expected,
            ...
            metadata.update(
                {
                    "v129_arm_id": arm_id,
                    "v129_arm_role": role,
                    "v129_candidate_profile": profile,
                    "v129_candidate_experiment": "E3",
                    "v129_shock_rate_ratio": "3/2" if candidate else None,
                    "v129_shock_threshold_numerator": 3 if candidate else None,
                    "v129_shock_threshold_denominator": 2 if candidate else None,
                    ...
                    "v129_faasrank_model_artifact_sha256": MODEL_SHA256,
                    "v129_critical_service_ratio_numerator": 9 if candidate else None,
                    "v129_critical_service_ratio_denominator": 10
                    if candidate
                    else None,
...
def prepare_v129(root: Path = ROOT) -> dict:
    _assert_frozen_inputs()
    if root.exists():
        raise RuntimeError(f"refusing to overwrite V129 training root: {root}")
    ...
    capture = derive_integration_smoke_shard(
        paths["source"], capture_ids, purpose=f"V129 fresh parent tapes {PLAN_SHA256}"
    )
    _write(
        paths["capture"],
        _rewrite(capture, purpose=f"V129 fresh parent tapes {PLAN_SHA256}"),
    )
    ...
    for arm in ARMS:
        ...
        template = derive_integration_smoke_shard(
            paths["source"], selected, purpose=f"V129 {arm_id} {PLAN_SHA256}"
        )
        manifest = _rewrite(
            template,
            purpose=f"V129 {arm_id} {PLAN_SHA256}",
            arm=arm,
        ...
        if (
            ...
            raise RuntimeError(f"V129 {arm_id} must have 9 runs/references")
        if {run["seed"] for run in manifest["runs"]} != TRAINING_SEEDS:
            raise RuntimeError(f"V129 {arm_id} escaped E1310-E1312")
        keys = {item["key"] for item in manifest["reference_build_dependencies"]}
        if all_reference_keys & keys:
            raise RuntimeError("V129 arm-specific reference keys unexpectedly overlap")
        all_reference_keys.update(keys)
        _write(arm_path(root, arm_id), manifest)
    if len(all_reference_keys) != 18:
        raise RuntimeError("V129 must declare exactly 18 arm-specific references")
    ...
    receipt = {
        "schema_version": "NSE_E3_COMPLETION_PROXIMAL_TERMINAL_CHILD_HPA_ANCHOR_ARGMIN_PATH_PREPARED_V129_V1",
        "created_at": utc_now(),
        "formal_results_eligible": False,
        ...
        "V128_result_path": str(V128_RESULT),
        "V128_result_sha256": V128_RESULT_SHA256,
        "V128_result_hash": V128_RESULT_HASH,
        "V128_result_commit": V128_RESULT_COMMIT,
        "V128_blind_audit_path": str(V128_BLIND_AUDIT),
        "V128_blind_audit_sha256": V128_BLIND_AUDIT_SHA256,
        "V128_blind_audit_hash": V128_BLIND_AUDIT_HASH,
        "V128_performance_results_consulted": True,
        ...
        "arms": [
            {
                ...
                "complete_terminal_child_hpa_anchor_argmin_path_service_pareto": role
                == "candidate",
                "terminal_child_hpa_anchor_argmin_path_pairing": (
                    "same_deterministic_anchor_argmin_child_container_node"
                    if role == "candidate"
                    else "not_applicable"
                ),
                "terminal_child_hpa_anchor_argmin_path_aggregation": (
                    "complete_per_child_pareto_on_deterministic_anchor_argmin_witness"
                    if role == "candidate"
                    else "not_applicable"
                ),
                "terminal_child_hpa_anchor_argmin_selection": (
                    "minimum_finite_anchor_path_then_lowest_node_id"
                    if role == "candidate"
                    else "not_applicable"
                ),
                "terminal_child_hpa_anchor_argmin_path_scope": (
                    "every_immutable_immediate_child_of_each_actually_changed_completion_proximal_player"
                    if role == "candidate"
                    else "not_applicable"
                ),
