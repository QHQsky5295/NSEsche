...
OUTPUT = ROOT / "training-result-v145.json"
...
BLIND_AUDIT_FILE_SHA256: str | None = (
    "145a37417a9dc05f69e5f4c99d0f37a088aad2ef8768563d40e971b4378aaf25"
)
BLIND_AUDIT_HASH: str | None = (
    "288fdd2b04c908548a3797b021c8035c13cf5aab0d158549774dd23dbf14970f"
)
...
def evaluate_training_rows(rows: Sequence[Mapping[str, Any]]) -> dict[str, Any]:
    ...
    for row in rows:
        ...
        if run_id in run_ids:
            raise ValueError(f"duplicate V145 run_id: {run_id}")
        run_ids.add(run_id)
    ...
    if len(rows) != 90 or actual != expected:
        raise ValueError(
            "V145 revealed product mismatch: "
...
def _validate_blind_document(blind: Mapping[str, Any], expected_hash: str) -> None:
    ...
    if (
        ...
        raise RuntimeError("V145 blind audit does not authorize reveal")
...
def _validate_blind_audit() -> dict[str, Any]:
    ...
    if any(
        ...
        raise RuntimeError("V145 blind audit has not been frozen into reveal code")
    if not BLIND_AUDIT.is_file() or file_hash(BLIND_AUDIT) != BLIND_AUDIT_FILE_SHA256:
        raise RuntimeError("V145 blind audit is missing or changed")
...
def _load_rows(blind: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    if len(baseline_admitted) != 81:
        raise RuntimeError("V145 blind baseline admission is incomplete")
    baseline_rows = []
    ...
    if not manifest_path.is_file() or file_hash(manifest_path) != frozen_manifest.get(
        ...
        raise RuntimeError("V145 frozen candidate manifest changed")
    manifest = load_and_validate_manifest(manifest_path)
    if manifest.get("manifest_hash") != frozen_manifest.get("manifest_hash"):
        raise RuntimeError("V145 candidate manifest hash changed")
    admitted = {item["run_id"]: item for item in blind["candidate_runs"]}
    if len(admitted) != 9:
        raise RuntimeError("V145 candidate admission is incomplete")
    candidate_rows = []
    for run in manifest["runs"]:
        run_id = run["run_id"]
        evidence = admitted.get(run_id)
        if evidence is None:
            raise RuntimeError(f"V145 run not admitted by blind audit: {run_id}")
        summary_path = (
            workspace_path(ROOT)
            / "canonical"
            / run_id
        ...
        if not summary_path.is_file() or file_hash(summary_path) != evidence.get(
            ...
            raise RuntimeError(f"V145 summary differs from blind audit: {run_id}")
        summary = read_json(summary_path)
        if summary.get("run_complete") is not True or summary.get("run_id") != run_id:
            raise RuntimeError(f"V145 summary identity changed: {run_id}")
...
def execute_reveal() -> dict[str, Any]:
    if OUTPUT.exists():
        raise RuntimeError(f"refusing to overwrite V145 result: {OUTPUT}")
    if not PLAN.is_file() or file_hash(PLAN) != PLAN_SHA256:
        raise RuntimeError("V145 plan changed before reveal")
    blind = _validate_blind_audit()
    rows = _load_rows(blind)
    evaluation = evaluate_training_rows(rows)
    result = {
        "schema_version": "NSE_E3_CAUSAL_RAW_PERSISTENCE_TRAINING_RESULT_V145_V1",
        "created_at": utc_now(),
