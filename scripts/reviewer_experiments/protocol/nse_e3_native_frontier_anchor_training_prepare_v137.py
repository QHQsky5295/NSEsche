...
ROOT = Path("tmp/nse_e3_native_frontier_anchor_training_20260830_v137")
PLAN = Path(
    "scripts/reviewer_experiments/protocol/"
    "nse_e3_native_frontier_anchor_training_plan_v137.json"
)
PLAN_SHA256 = "9ec7336a9a59748e1d1e7346f6ef25df11173db99add3a67f342232b8ce68f4b"

V136_PLAN = Path(
    "scripts/reviewer_experiments/protocol/"
    "nse_e3_all_baseline_closure_diagnostic_plan_v136.json"
)
V136_PLAN_SHA256 = "251146f551243a62579c68a4da89375769aeab450a75cb51375077a589378981"
V136_BLIND_AUDIT = Path(
    "tmp/nse_e3_all_baseline_closure_diagnostic_20260830_v136/"
    "joint-blind-audit-v136.json"
)
V136_BLIND_AUDIT_SHA256 = (
    "02e2f5141abba57a36e2a6ec3cf31b861b962ba2c2e8aee9917ae5f165f7c0db"
)
V136_RESULT = Path(
    "tmp/nse_e3_all_baseline_closure_diagnostic_20260830_v136/"
    "all-baseline-closure-result-v136.json"
)
V136_RESULT_SHA256 = "75a4460e43c4bad1aa048da12b0336c46b9e861a6ddc08ee69c4f380c1c73985"

BINARY_PATH = Path("tmp/nse_v137_build_3f02c44/release/serverless_sim.exe")
BINARY_SHA256 = "81dba279d51fd36a97aba323fd2ab1ea26682810710248235821ff258228e946"
BINARY_SOURCE_COMMIT = "3f02c4489c1ee6f8edeed5a300c59e69396b9a1e"

TRAINING_SEED_LIST = ["E1471", "E1472", "E1473"]
...
PREVIOUS_CONFIRMATION_SEEDS = [f"E{index}" for index in range(1451, 1471)]
NEW_CONFIRMATION_SEEDS = [f"E{index}" for index in range(1474, 1494)]
SEALED_CONFIRMATION_SEEDS = set(PREVIOUS_CONFIRMATION_SEEDS + NEW_CONFIRMATION_SEEDS)
...
ARMS = (
    (
        "v137-e3-native-greedy-all-service-safe-nash",
        "greedy_native_faithful_all_readiness_service_window_safe_pareto",
        "greedy",
    ),
    (
        "v137-e3-native-hiku-all-service-safe-nash",
        "hiku_native_faithful_all_readiness_service_window_safe_pareto",
        "sche_Hiku",
    ),
    (
        "v137-e3-native-ocs-pipeline-service-safe-nash",
...
RUN_ORDER_SEED = "NSE-V137-RCBD-E1471-E1473"
PORT = "3168"
...
def paths(root: Path = ROOT) -> dict[str, Path]:
    return {
        "config": root / "v137-e3-training-config.json",
        "source": root / "manifest.v137-training-source-full.unbound.json",
        "capture": root / "manifest.v137-tape-capture.unbound.json",
        "baselines": root / "manifest.v137-baselines.unbound.json",
        "prepared": root / "prepared-manifests-v137.json",
        "schedule": root / "frozen-run-order-v137.json",
    }
...
def _assert_frozen_inputs() -> None:
    for path, expected in (
        (PLAN, PLAN_SHA256),
        (V136_PLAN, V136_PLAN_SHA256),
        (V136_BLIND_AUDIT, V136_BLIND_AUDIT_SHA256),
        (V136_RESULT, V136_RESULT_SHA256),
        ...
        if not path.is_file() or file_hash(path).lower() != expected.lower():
            raise RuntimeError(f"frozen V137 input is missing or changed: {path}")
    _assert_hashed_object(V136_BLIND_AUDIT, "audit_hash")
    _assert_hashed_object(V136_RESULT, "result_hash")


def _write_config(path: Path) -> None:
    if SEALED_CONFIRMATION_SEEDS & set(SOURCE_INITIAL_SEEDS + SOURCE_CI_SEEDS):
        raise RuntimeError("V137 source seed policy opens a sealed confirmation seed")
...
def _base_marker(rewritten: dict[str, Any], purpose: str) -> dict[str, Any]:
    ...
    marker.update(
        {
            "purpose": purpose,
            "v137_training_plan_sha256": PLAN_SHA256,
            "v137_training_only": True,
            "v137_binary_source_commit": BINARY_SOURCE_COMMIT,
...
def _rewrite_capture(shard: dict[str, Any]) -> dict[str, Any]:
    ...
    marker = _base_marker(rewritten, f"V137 fresh parent tapes {PLAN_SHA256}")
    marker["v137_role"] = "fresh_parent_tape_capture"
        ...
        run.setdefault("metadata", {}).update(
            {
                "v137_training_plan_sha256": PLAN_SHA256,
                "v137_training_only": True,
                "v137_confirmation_inputs_opened": False,
            }
        )
...
def _rewrite_baselines(shard: dict[str, Any]) -> dict[str, Any]:
    rewritten = copy.deepcopy(shard)
    marker = _base_marker(rewritten, f"V137 complete baseline product {PLAN_SHA256}")
    marker.update(
        {
            "v137_role": "paper_baseline",
            "v137_expected_run_count": 81,
            "v137_expected_reference_count": 0,
            "v137_baseline_methods": BASELINE_METHODS,
        }
    )
    for run in rewritten["runs"]:
        source_run_id = run["run_id"]
        source_run_spec_hash = run["run_spec_hash"]
        run["variant"] = "v137-complete-paper-baselines"
        run["environment"]["SERVERLESS_SIM_PORT"] = PORT
        run.setdefault("metadata", {}).update(
            {
                "v137_training_plan_sha256": PLAN_SHA256,
                "v137_training_only": True,
                "v137_role": "paper_baseline",
                "v137_source_run_id": source_run_id,
                "v137_source_run_spec_hash": source_run_spec_hash,
                "v137_performance_consulted_before_execution": False,
                "v137_confirmation_inputs_opened": False,
                "v137_outcome_fields_drive_policy": False,
            }
        )
        _assign_run_identity(run)
...
def _rewrite_arm(
    ...
    arm_id, profile, native_baseline = arm
    rewritten = copy.deepcopy(shard)
    marker = _base_marker(rewritten, f"V137 {arm_id} {PLAN_SHA256}")
    marker.update(
        {
            "v137_role": "candidate",
            "v137_arm_id": arm_id,
            "v137_profile": profile,
            "v137_native_anchor_baseline": native_baseline,
            "v137_expected_run_count": 9,
            "v137_expected_reference_count": 9,
            "v137_native_shadow_exactness_required": True,
            "v137_ready_service_certificate_required": True,
            "v137_outcome_fields_drive_policy": False,
        }
    )
    for run in rewritten["runs"]:
        ...
        run.setdefault("metadata", {}).update(
            {
                ...
                "v137_source_run_spec_hash": source_run_spec_hash,
                "v137_native_shadow_exactness_required": True,
                "v137_player_frontier_and_order_from_native_shadow": True,
                "v137_ready_service_same_nonempty_parent_complete_cohort": True,
                "v137_ready_service_sum_strictly_lower": True,
                "v137_ready_service_max_nonincreasing": True,
                "v137_immutable_baseline_welfare_nonworse": True,
                "v137_no_numeric_tuning": True,
                "v137_outcome_fields_drive_policy": False,
                "v137_scenario_or_burst_label_used_by_policy": False,
                "v137_future_arrivals_used_by_policy": False,
                "v137_confirmation_inputs_opened": False,
            }
        )
        ...
        _assign_run_identity(run)
    _validate_arm_product(rewritten["runs"], arm_id, profile)
    return _finalize(rewritten)
...
def _validate_baseline_product(runs: list[dict[str, Any]]) -> None:
    ...
    if len(runs) != 81 or actual != expected:
        raise RuntimeError("V137 baseline method/scenario/seed product changed")
    if any(run.get("reference_dependency") is not None for run in runs):
        raise RuntimeError("V137 baseline product unexpectedly requires references")


def _validate_arm_product(
    runs: list[dict[str, Any]], arm_id: str, profile: str
) -> None:
    expected = {
    ...
    if len(runs) != 9 or actual != expected:
        raise RuntimeError(f"V137 arm product changed: {arm_id}")
    if {run["method"] for run in runs} != {"sche_nash"}:
        raise RuntimeError(f"V137 arm method changed: {arm_id}")
    if {run["environment"].get("NASH_OPERATIONAL_EXPERT_PROXY") for run in runs} != {
        profile
    }:
        raise RuntimeError(f"V137 arm profile changed: {arm_id}")
...
def _frozen_schedule(
    ...
    if len(schedule) != 108 or len({item["run_id"] for item in schedule}) != 108:
        raise RuntimeError("V137 frozen run order is not an exact 108-run product")
    document = {
        "schema_version": "NSE_E3_NATIVE_FRONTIER_ANCHOR_RUN_ORDER_V137_V1",
        "created_at": utc_now(),
        "performance_results_consulted": False,
        "run_order_seed": RUN_ORDER_SEED,
...
def prepare_v137(root: Path = ROOT) -> dict[str, Any]:
    _assert_frozen_inputs()
    if root.exists():
        raise RuntimeError(f"refusing to overwrite V137 training root: {root}")
    ...
    capture = derive_integration_smoke_shard(
        output["source"], capture_ids, purpose=f"V137 fresh parent tapes {PLAN_SHA256}"
    )
    ...
    baseline_shard = derive_integration_smoke_shard(
        output["source"], baseline_ids, purpose=f"V137 baseline product {PLAN_SHA256}"
    )
    ...
    for arm in ARMS:
        ...
        shard = derive_integration_smoke_shard(
            output["source"], nash_ids, purpose=f"V137 {arm_id} {PLAN_SHA256}"
        )
        ...
        if len(keys) != 9 or all_reference_keys & keys:
            raise RuntimeError("V137 arm reference keys overlap or are incomplete")
    ...
    if len(all_reference_keys) != 27:
        raise RuntimeError("V137 must declare exactly 27 candidate references")
    ...
    receipt = {
        "schema_version": "NSE_E3_NATIVE_FRONTIER_ANCHOR_PREPARED_V137_V1",
        "created_at": utc_now(),
        "formal_results_eligible": False,
        "operational_group_closure_eligible": False,
        "performance_results_consulted": False,
        ...
        "v136_plan_path": str(V136_PLAN),
        "v136_plan_sha256": V136_PLAN_SHA256,
        "v136_blind_audit_path": str(V136_BLIND_AUDIT),
        "v136_blind_audit_sha256": V136_BLIND_AUDIT_SHA256,
        "v136_result_path": str(V136_RESULT),
        "v136_result_sha256": V136_RESULT_SHA256,
        ...
        "arms": [
            {
                "arm_id": arm_id,
                "profile": profile,
                "native_anchor_baseline": baseline,
                "run_count": 9,
                "reference_build_count": 9,
            }
            for arm_id, profile, baseline in ARMS
        ],
        "manifests": manifests,
    }
