...
OUTPUT = ROOT / "joint-blind-audit-v130-training.json"
RESULT = ROOT / "training-result-v130.json"
...
def _validate_admitted_work_diagnostics(
    ...
    """Validate V130 mechanism evidence without opening any performance summary."""

    path = canonical / "reviewer_records" / run["run_id"] / "nash_metrics.jsonl.gz"

    _require(path.is_file(), f"missing V130 Nash diagnostics: {run['run_id']}")
    ...
    with gzip.open(path, "rt", encoding="utf-8") as handle:
        for line_number, line in enumerate(handle, start=1):
            ...
            _require(type(frame) is int and frame >= 0, "invalid V130 window frame")

            cluster = event.get("cluster")
            _require(isinstance(cluster, dict), "missing V130 cluster diagnostics")
            admitted_fields = (
            ...
            if candidate:
                _require(
                    ...
                    f"V130 admitted-work observation changed: {run['run_id']}:{line_number}",
                )
                ...
                _require(
                    ...
                    f"V130 admitted-work snapshot counts changed: "
                ...
                _require(
                    ...
                    f"V130 admitted-work aggregate changed: {run['run_id']}:{line_number}",
                )
            ...
            _require(
                ...
                f"V130 critical-service contract changed: {run['run_id']}:{line_number}",
            )
            ...
            _require(
                ...
                f"V130 componentwise-service contract changed: "
                f"{run['run_id']}:{line_number}",
            )
            for field in (
                "target_player_count",
                ...
                _require(
                    ...
                    f"V130 componentwise {field} changed: "
                    f"{run['run_id']}:{line_number}",
                )
            ...
            _require(
                ...
                f"V130 componentwise acceptance changed: "
                f"{run['run_id']}:{line_number}",
            )
            ...
            _require(
                ...
                f"V130 componentwise reason changed: {run['run_id']}:{line_number}",
            )
            ...
            _require(
                ...
                f"V130 componentwise delta changed: {run['run_id']}:{line_number}",
            )
            _require(
                maximum_ratio is None or _finite_nonnegative(maximum_ratio),
                f"V130 componentwise ratio changed: {run['run_id']}:{line_number}",
            )
            if componentwise["accepted"] and componentwise["target_player_count"] > 0:
                _require(
                    ...
                    f"V130 accepted componentwise delta is worse: "
                    f"{run['run_id']}:{line_number}",
                )
            ...
            _require(
                ...
                f"V130 terminal-child path contract changed: "
                f"{run['run_id']}:{line_number}",
            )
            for field in (
                ...
                _require(
                    ...
                    f"V130 terminal-child path {field} changed: "
                    f"{run['run_id']}:{line_number}",
                )
            _require(
                ...
                f"V130 terminal-child fail-closed reason accounting changed: "
                f"{run['run_id']}:{line_number}",
            )
            _require(
                ...
                f"V130 deterministic anchor-top2 witness accounting changed: "
                f"{run['run_id']}:{line_number}",
            )
            ...
            _require(
                ...
                f"V130 terminal-child path acceptance changed: "
                f"{run['run_id']}:{line_number}",
            )
            ...
            _require(
                ...
                == expected_terminal_child_path_reason,
                f"V130 terminal-child path reason changed: "
                f"{run['run_id']}:{line_number}",
            )
            ...
            _require(
                ...
                f"V130 terminal-child path delta changed: "
                f"{run['run_id']}:{line_number}",
            )
            _require(
                terminal_maximum_ratio is None
                or _finite_nonnegative(terminal_maximum_ratio),
                f"V130 terminal-child path ratio changed: "
                f"{run['run_id']}:{line_number}",
            )
            ...
            if (
                ...
                _require(
                    ...
                    f"V130 accepted terminal-child path is worse: "
                    f"{run['run_id']}:{line_number}",
                )
            _require(
                not critical["accepted"]
                or (componentwise["accepted"] and terminal_child_path["accepted"]),
                f"V130 main certificate bypassed a final service gate: "
                f"{run['run_id']}:{line_number}",
            )
            ...
            if dominance.get("reason") in componentwise_rejection_reasons:
                _require(
                    ...
                    f"V130 componentwise fallback was not exact anchor: "
                    f"{run['run_id']}:{line_number}",
                )
            ...
            if dominance.get("reason") in terminal_child_path_rejection_reasons:
                _require(
                    ...
                    and dominance.get("selected_initializer_assignment_hash")
                    == dominance.get("anchor_assignment_hash"),
                    f"V130 terminal-child path fallback was not exact anchor: "
                    f"{run['run_id']}:{line_number}",
                )
            for field in (
                ...
                _require(
                    ...
                    f"V130 critical {field} changed: {run['run_id']}:{line_number}",
                )
            _require(
                not critical["accepted"] or critical["evaluated"],
                f"V130 accepted without evaluation: {run['run_id']}:{line_number}",
            )
            ...
            _require(
                ...
                f"V130 completion-proximal construction contract changed: "
                f"{run['run_id']}:{line_number}",
            )
            ...
            _require(
                ...
                f"V130 completion-proximal scope diagnostics changed: "
                f"{run['run_id']}:{line_number}",
            )
            proximal_fields = (
            ...
            _require(
                ...
                f"V130 completion-proximal scope counter changed: "
                f"{run['run_id']}:{line_number}",
            )
            if invoked:
                _require(
                    ...
                    f"V130 completion-proximal substitution invariant changed: "
                    f"{run['run_id']}:{line_number}",
                )
            else:
                _require(
                    all(proximal_scope[field] == 0 for field in proximal_fields),
                    f"V130 inactive completion-proximal scope leaked counters: "
                    f"{run['run_id']}:{line_number}",
                )
            if not invoked:
                _require(
                    ...
                    f"V130 non-invoked critical state changed: {run['run_id']}:{line_number}",
                )
            if critical["accepted"]:
                ...
                _require(
                    ...
                    f"V130 accepted evidence is nonfinite: {run['run_id']}:{line_number}",
                )
                ...
                _require(
                    ...
                    f"V130 accepted admitted-work certificate changed: "
                    f"{run['run_id']}:{line_number}",
                )
            ...
            if not candidate:
                _require(
                    ...
                    f"V130 anchor used causal state: {run['run_id']}:{line_number}",
                )
                continue
            if not gate["history_complete"]:
                _require(
                    gate["baseline_count"] == 0 and gate["recent_count"] == 0,
                    f"V130 incomplete history leaked counts: {run['run_id']}:{line_number}",
                )
                continue
            sufficient = (
            ...
            if sufficient and recent_scaled >= baseline_scaled:
                ...
                _require(
                    ...
                    f"V130 exact threshold did not activate: "
                    f"{run['run_id']}:{line_number}",
                )

    _require(
        counts["windows"] == run["simulation"]["total_frame"],
        f"V130 diagnostic window count changed: {run['run_id']}",
    )
    if candidate:
        _require(
            ...
            f"V130 mechanism was not exercised: {run['run_id']}",
        )
    return {
...
def _verify_tape_catalog() -> tuple[list[dict[str, Any]], dict[str, Any]]:
    _require(TAPES.is_file(), f"missing V130 tape catalog: {TAPES}")
    catalog = read_json(TAPES)
    catalog_hash = _assert_hashed_object(catalog, "catalog_hash", "V130 tapes")
    entries = catalog.get("entries")
    _require(
        isinstance(entries, dict) and len(entries) == 12, "V130 tape count changed"
    )
    evidence = []
...
def _verify_reference_stage_ledgers() -> tuple[dict[str, Path], dict[Path, str]]:
    ...
    for arm_id, expected in ARMS.items():
        ...
        root = _stage_root_from_receipts(
            entries, "receipt_path", expected["run_count"], f"V130 reference {arm_id}"
        )
        roots[arm_id] = root
    ...
    for root, expected_count in expected_by_root.items():
        ...
        _assert_ledger_contract(
            ...
            f"V130 reference stage {root}",
        )
        _require(
            not list((root / "quarantine").glob("**/attempt-*")),
            f"V130 reference quarantine is nonempty: {root}",
        )
...
def _validate_run_boundary(
    ...
    required = {
        ...
        "v130_arm_id": arm_id,
        "v130_arm_role": expected["role"],
        "v130_candidate_profile": expected["profile"],
        "v130_candidate_experiment": "E3",
        "v130_shock_rate_ratio": expected["shock_rate_ratio"],
        "v130_shock_threshold_numerator": expected["shock_threshold_numerator"],
        "v130_shock_threshold_denominator": expected["shock_threshold_denominator"],
        ...
        "v130_shock_activation_horizon_frames": expected[
            "shock_activation_horizon_frames"
        ],
        "v130_nonterminal_queue_density_floor": 8.0 if candidate else None,
        "v130_warm_admissibility": "preserve_anchor_warmness" if candidate else None,
        "v130_load_least_window_certificate_mode": (
            "disabled" if candidate else "not_applicable"
        ),
        "v130_critical_service_ratio_numerator": expected[
            "critical_service_threshold_numerator"
        ],
        "v130_critical_service_ratio_denominator": expected[
    ...
    _require(
        ...
        f"V130 run boundary changed: {run['run_id']}",
    )


def run_blind_audit(output: Path = OUTPUT) -> dict[str, Any]:
    _require(not output.exists(), f"V130 blind audit already exists: {output}")
    _require(not RESULT.exists(), "V130 reveal exists before blind audit")
    _require(PLAN.is_file() and file_hash(PLAN) == PLAN_SHA256, "V130 plan changed")
    plan = read_json(PLAN)
    _require(plan.get("formal_results_eligible") is False, "V130 eligibility changed")
    _require(PREPARED.is_file(), f"missing V130 prepared receipt: {PREPARED}")
    prepared = read_json(PREPARED)
    prepared_hash = _assert_hashed_object(
        prepared, "receipt_hash", "V130 prepared receipt"
    )
    _require(
        ...
        "V130 prepared scientific boundary changed",
    )
    ...
    for arm_id, expected in ARMS.items():
        ...
        _require(paths["ready"].is_file(), f"missing V130 ready manifest: {arm_id}")
        ...
        _require(
            ...
            f"V130 ready boundary changed: {arm_id}",
        )
        ...
        reference_catalog_hash = _assert_hashed_object(
            references, "catalog_hash", f"V130 references {arm_id}"
        )
        ...
        _require(
            ...
            f"V130 reference key set changed: {arm_id}",
        )
        for key, entry in sorted(entries.items()):
            ...
            _require(
                ...
                f"V130 reference evidence changed: {key}",
            )
        ...
        _require(actual_ids == expected_ids, f"V130 canonical set changed: {arm_id}")
        _require(
            not list((workspace / "quarantine").glob("**/attempt-*")),
            f"V130 online quarantine is nonempty: {arm_id}",
        )
        ...
        for run in manifest["runs"]:
            ...
            _require(
                ...
                f"V130 canonical status changed: {run['run_id']}",
            )
            runtime = {
    ...
    _require(len(run_evidence) == 18, "V130 run evidence count must be 18")
    _require(len(reference_evidence) == 18, "V130 reference evidence count must be 18")
    _require(len(tape_evidence) == 12, "V130 tape evidence count must be 12")
    for field, expected in EXPECTED_RUNTIME.items():
        _require(
            runtime_values[field] == {expected},
            f"V130 runtime {field} changed: {runtime_values[field]}",
        )
    ...
    _require(
        ...
        f"V130 runtime git identity is not singular: {git_commits}",
    )
    _require(len(paired_inputs) == 9, "V130 paired input product changed")
    for (scenario, seed), rows in paired_inputs.items():
        _require(len(rows) == 2, f"V130 paired arm count changed: {scenario}/{seed}")
        for field in (
            ...
            _require(
                ...
                f"V130 paired {field} changed: {scenario}/{seed}",
            )
        _require(
            rows[0]["common_hpa_hash"] == EXPECTED_COMMON_HPA_SHA256,
            f"V130 common HPA changed: {scenario}/{seed}",
        )
    ...
    _require(
        ...
        "V130 completion-proximal and deeper current-player paths were not both exercised",
    )
    _require(
        completion_proximal_service_choice_evaluated_count > 0
        and completion_proximal_substitution_count > 0,
        "V130 completion-proximal service-choice evaluation/substitution path was not exercised",
    )
    _require(
        deeper_exact_anchor_preserved_count > 0 and deeper_substitution_count == 0,
        "V130 deeper-player exact-anchor preservation contract changed",
    )
    _require(
        proxy_input_unavailable_count == 0,
        "V130 admitted-work proxy inputs were unavailable",
    )
    _require(
        ...
        "V130 componentwise service accept/reject paths were not both exercised",
    )
    _require(
        componentwise_input_unavailable_count == 0,
        "V130 componentwise service inputs were unavailable",
    )
    _require(
        ...
        "V130 terminal-child anchor-top2 accept/reject paths were not both exercised",
    )
    _require(
        terminal_child_path_input_unavailable_count == 0,
        "V130 terminal-child path inputs were unavailable",
    )
    _require(
        ...
        "V130 terminal-child HPA path fail-closed reason count was nonzero",
    )
    _require(
        terminal_child_path_nonterminal_child_count == 0,
        "V130 terminal-child path scope included a nonterminal child",
    )
    _require(
        terminal_child_path_worse_container_path_count > 0,
        "V130 anchor-top2 worse-path rejection was not exercised",
    )

    output_payload = {
        "schema_version": "NSE_E3_COMPLETION_PROXIMAL_TERMINAL_CHILD_HPA_ANCHOR_TOP2_PATH_BLIND_AUDIT_V130_V1",
        "created_at": utc_now(),
        "status": "pass",
        ...
        "terminal_child_anchor_top2_path_mechanism_coverage": {
            "terminal_child_anchor_top2_gate_evaluated_count": terminal_child_path_gate_evaluated_count,
            "terminal_child_anchor_top2_changed_parent_count": terminal_child_path_changed_parent_count,
            "terminal_child_anchor_top2_accepted_changed_parent_count": terminal_child_path_accepted_changed_parent_count,
            "terminal_child_anchor_top2_compared_child_count": terminal_child_path_compared_child_count,
            "terminal_child_anchor_top2_available_container_path_count": terminal_child_path_available_container_path_count,
            "terminal_child_anchor_top2_selected_rank1_path_count": terminal_child_path_selected_rank1_path_count,
            "terminal_child_anchor_top2_selected_rank2_path_count": terminal_child_path_selected_rank2_path_count,
            "terminal_child_anchor_top2_selected_path_count": terminal_child_path_compared_container_path_count,
            "terminal_child_anchor_top2_acceptance_count": terminal_child_path_acceptance_count,
            "terminal_child_anchor_top2_rejection_count": terminal_child_path_rejection_count,
            "terminal_child_anchor_top2_input_unavailable_count": terminal_child_path_input_unavailable_count,
            "terminal_child_anchor_top2_running_container_path_count": terminal_child_path_running_container_count,
            "terminal_child_anchor_top2_starting_container_path_count": terminal_child_path_starting_container_count,
            "terminal_child_anchor_top2_no_container_count": terminal_child_path_no_container_count,
            "terminal_child_anchor_top2_invalid_profile_count": terminal_child_path_invalid_profile_count,
            "terminal_child_anchor_top2_invalid_node_snapshot_count": terminal_child_path_invalid_node_snapshot_count,
            "terminal_child_anchor_top2_invalid_network_count": terminal_child_path_invalid_network_count,
            "terminal_child_anchor_top2_nonterminal_child_count": terminal_child_path_nonterminal_child_count,
            "terminal_child_anchor_top2_worse_child_count": terminal_child_path_worse_child_count,
            "terminal_child_anchor_top2_worse_path_count": terminal_child_path_worse_container_path_count,
            "terminal_child_anchor_top2_maximum_alternative_minus_anchor_for_same_container": terminal_child_path_maximum_same_container_delta,
            "terminal_child_anchor_top2_maximum_alternative_to_anchor_ratio_for_same_container": terminal_child_path_maximum_same_container_ratio,
            "performance_fields_consulted": False,
        },
        "arm_count": len(ARMS),
