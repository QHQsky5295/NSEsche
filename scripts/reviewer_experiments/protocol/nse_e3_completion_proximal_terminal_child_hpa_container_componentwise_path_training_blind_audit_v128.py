...
from scripts.reviewer_experiments.protocol.nse_e3_completion_proximal_terminal_child_hpa_container_componentwise_path_training_prepare_v128 import (
...
OUTPUT = ROOT / "joint-blind-audit-v128-training.json"
RESULT = ROOT / "training-result-v128.json"
...
def _validate_admitted_work_diagnostics(
    ...
    """Validate V128 mechanism evidence without opening any performance summary."""

    path = canonical / "reviewer_records" / run["run_id"] / "nash_metrics.jsonl.gz"

    _require(path.is_file(), f"missing V128 Nash diagnostics: {run['run_id']}")
    ...
    with gzip.open(path, "rt", encoding="utf-8") as handle:
        for line_number, line in enumerate(handle, start=1):
            ...
            _require(type(frame) is int and frame >= 0, "invalid V128 window frame")

            cluster = event.get("cluster")
            _require(isinstance(cluster, dict), "missing V128 cluster diagnostics")
            admitted_fields = (
            ...
            if candidate:
                _require(
                    ...
                    f"V128 admitted-work observation changed: {run['run_id']}:{line_number}",
                )
                ...
                _require(
                    ...
                    f"V128 admitted-work snapshot counts changed: "
                ...
                _require(
                    ...
                    f"V128 admitted-work aggregate changed: {run['run_id']}:{line_number}",
                )
            ...
            _require(
                ...
                f"V128 critical-service contract changed: {run['run_id']}:{line_number}",
            )
            ...
            _require(
                ...
                f"V128 componentwise-service contract changed: "
                f"{run['run_id']}:{line_number}",
            )
            for field in (
                "target_player_count",
                ...
                _require(
                    ...
                    f"V128 componentwise {field} changed: "
                    f"{run['run_id']}:{line_number}",
                )
            ...
            _require(
                ...
                f"V128 componentwise acceptance changed: "
                f"{run['run_id']}:{line_number}",
            )
            ...
            _require(
                ...
                f"V128 componentwise reason changed: {run['run_id']}:{line_number}",
            )
            ...
            _require(
                ...
                f"V128 componentwise delta changed: {run['run_id']}:{line_number}",
            )
            _require(
                maximum_ratio is None or _finite_nonnegative(maximum_ratio),
                f"V128 componentwise ratio changed: {run['run_id']}:{line_number}",
            )
            if componentwise["accepted"] and componentwise["target_player_count"] > 0:
                _require(
                    ...
                    f"V128 accepted componentwise delta is worse: "
                    f"{run['run_id']}:{line_number}",
                )
            ...
            _require(
                ...
                f"V128 terminal-child path contract changed: "
                f"{run['run_id']}:{line_number}",
            )
            for field in (
                ...
                _require(
                    ...
                    f"V128 terminal-child path {field} changed: "
                    f"{run['run_id']}:{line_number}",
                )
            _require(
                ...
                f"V128 terminal-child fail-closed reason accounting changed: "
                f"{run['run_id']}:{line_number}",
            )
            ...
            _require(
                ...
                f"V128 terminal-child path acceptance changed: "
                f"{run['run_id']}:{line_number}",
            )
            ...
            _require(
                ...
                == expected_terminal_child_path_reason,
                f"V128 terminal-child path reason changed: "
                f"{run['run_id']}:{line_number}",
            )
            ...
            _require(
                ...
                f"V128 terminal-child path delta changed: "
                f"{run['run_id']}:{line_number}",
            )
            _require(
                terminal_maximum_ratio is None
                or _finite_nonnegative(terminal_maximum_ratio),
                f"V128 terminal-child path ratio changed: "
                f"{run['run_id']}:{line_number}",
            )
            ...
            if (
                ...
                _require(
                    ...
                    f"V128 accepted terminal-child path is worse: "
                    f"{run['run_id']}:{line_number}",
                )
            _require(
                not critical["accepted"]
                or (componentwise["accepted"] and terminal_child_path["accepted"]),
                f"V128 main certificate bypassed a final service gate: "
                f"{run['run_id']}:{line_number}",
            )
            ...
            if dominance.get("reason") in componentwise_rejection_reasons:
                _require(
                    ...
                    f"V128 componentwise fallback was not exact anchor: "
                    f"{run['run_id']}:{line_number}",
                )
            ...
            if dominance.get("reason") in terminal_child_path_rejection_reasons:
                _require(
                    ...
                    and dominance.get("selected_initializer_assignment_hash")
                    == dominance.get("anchor_assignment_hash"),
                    f"V128 terminal-child path fallback was not exact anchor: "
                    f"{run['run_id']}:{line_number}",
                )
            for field in (
                ...
                _require(
                    ...
                    f"V128 critical {field} changed: {run['run_id']}:{line_number}",
                )
            _require(
                not critical["accepted"] or critical["evaluated"],
                f"V128 accepted without evaluation: {run['run_id']}:{line_number}",
            )
            ...
            _require(
                ...
                f"V128 completion-proximal construction contract changed: "
                f"{run['run_id']}:{line_number}",
            )
            ...
            _require(
                ...
                f"V128 completion-proximal scope diagnostics changed: "
                f"{run['run_id']}:{line_number}",
            )
            proximal_fields = (
            ...
            _require(
                ...
                f"V128 completion-proximal scope counter changed: "
                f"{run['run_id']}:{line_number}",
            )
            if invoked:
                _require(
                    ...
                    f"V128 completion-proximal substitution invariant changed: "
                    f"{run['run_id']}:{line_number}",
                )
            else:
                _require(
                    all(proximal_scope[field] == 0 for field in proximal_fields),
                    f"V128 inactive completion-proximal scope leaked counters: "
                    f"{run['run_id']}:{line_number}",
                )
            if not invoked:
                _require(
                    ...
                    f"V128 non-invoked critical state changed: {run['run_id']}:{line_number}",
                )
            if critical["accepted"]:
                ...
                _require(
                    ...
                    f"V128 accepted evidence is nonfinite: {run['run_id']}:{line_number}",
                )
                ...
                _require(
                    ...
                    f"V128 accepted admitted-work certificate changed: "
                    f"{run['run_id']}:{line_number}",
                )
            ...
            if not candidate:
                _require(
                    ...
                    f"V128 anchor used causal state: {run['run_id']}:{line_number}",
                )
                continue
            if not gate["history_complete"]:
                _require(
                    gate["baseline_count"] == 0 and gate["recent_count"] == 0,
                    f"V128 incomplete history leaked counts: {run['run_id']}:{line_number}",
                )
                continue
            sufficient = (
            ...
            if sufficient and recent_scaled >= baseline_scaled:
                ...
                _require(
                    ...
                    f"V128 exact threshold did not activate: "
                    f"{run['run_id']}:{line_number}",
                )

    _require(
        counts["windows"] == run["simulation"]["total_frame"],
        f"V128 diagnostic window count changed: {run['run_id']}",
    )
    if candidate:
        _require(
            ...
            f"V128 mechanism was not exercised: {run['run_id']}",
        )
    return {
...
def _verify_tape_catalog() -> tuple[list[dict[str, Any]], dict[str, Any]]:
    _require(TAPES.is_file(), f"missing V128 tape catalog: {TAPES}")
    catalog = read_json(TAPES)
    catalog_hash = _assert_hashed_object(catalog, "catalog_hash", "V128 tapes")
    entries = catalog.get("entries")
    _require(
        isinstance(entries, dict) and len(entries) == 12, "V128 tape count changed"
    )
    evidence = []
...
def _verify_reference_stage_ledgers() -> tuple[dict[str, Path], dict[Path, str]]:
    ...
    for arm_id, expected in ARMS.items():
        ...
        root = _stage_root_from_receipts(
            entries, "receipt_path", expected["run_count"], f"V128 reference {arm_id}"
        )
        roots[arm_id] = root
    ...
    for root, expected_count in expected_by_root.items():
        ...
        _assert_ledger_contract(
            ...
            f"V128 reference stage {root}",
        )
        _require(
            not list((root / "quarantine").glob("**/attempt-*")),
            f"V128 reference quarantine is nonempty: {root}",
        )
...
def _validate_run_boundary(
    ...
    required = {
        ...
        "v128_arm_id": arm_id,
        "v128_arm_role": expected["role"],
        "v128_candidate_profile": expected["profile"],
        "v128_candidate_experiment": "E3",
        "v128_shock_rate_ratio": expected["shock_rate_ratio"],
        "v128_shock_threshold_numerator": expected["shock_threshold_numerator"],
        "v128_shock_threshold_denominator": expected["shock_threshold_denominator"],
        ...
        "v128_shock_activation_horizon_frames": expected[
            "shock_activation_horizon_frames"
        ],
        "v128_nonterminal_queue_density_floor": 8.0 if candidate else None,
        "v128_warm_admissibility": "preserve_anchor_warmness" if candidate else None,
        "v128_load_least_window_certificate_mode": (
            "disabled" if candidate else "not_applicable"
        ),
        "v128_critical_service_ratio_numerator": expected[
            "critical_service_threshold_numerator"
        ],
        "v128_critical_service_ratio_denominator": expected[
    ...
    _require(
        ...
        f"V128 run boundary changed: {run['run_id']}",
    )


def run_blind_audit(output: Path = OUTPUT) -> dict[str, Any]:
    _require(not output.exists(), f"V128 blind audit already exists: {output}")
    _require(not RESULT.exists(), "V128 reveal exists before blind audit")
    _require(PLAN.is_file() and file_hash(PLAN) == PLAN_SHA256, "V128 plan changed")
    plan = read_json(PLAN)
    _require(plan.get("formal_results_eligible") is False, "V128 eligibility changed")
    _require(PREPARED.is_file(), f"missing V128 prepared receipt: {PREPARED}")
    prepared = read_json(PREPARED)
    prepared_hash = _assert_hashed_object(
        prepared, "receipt_hash", "V128 prepared receipt"
    )
    _require(
        ...
        "V128 prepared scientific boundary changed",
    )
    ...
    for arm_id, expected in ARMS.items():
        ...
        _require(paths["ready"].is_file(), f"missing V128 ready manifest: {arm_id}")
        ...
        _require(
            ...
            f"V128 ready boundary changed: {arm_id}",
        )
        ...
        reference_catalog_hash = _assert_hashed_object(
            references, "catalog_hash", f"V128 references {arm_id}"
        )
        ...
        _require(
            ...
            f"V128 reference key set changed: {arm_id}",
        )
        for key, entry in sorted(entries.items()):
            ...
            _require(
                ...
                f"V128 reference evidence changed: {key}",
            )
        ...
        _require(actual_ids == expected_ids, f"V128 canonical set changed: {arm_id}")
        _require(
            not list((workspace / "quarantine").glob("**/attempt-*")),
            f"V128 online quarantine is nonempty: {arm_id}",
        )
        ...
        for run in manifest["runs"]:
            ...
            _require(
                ...
                f"V128 canonical status changed: {run['run_id']}",
            )
            runtime = {
    ...
    _require(len(run_evidence) == 18, "V128 run evidence count must be 18")
    _require(len(reference_evidence) == 18, "V128 reference evidence count must be 18")
    _require(len(tape_evidence) == 12, "V128 tape evidence count must be 12")
    for field, expected in EXPECTED_RUNTIME.items():
        _require(
            runtime_values[field] == {expected},
            f"V128 runtime {field} changed: {runtime_values[field]}",
        )
    ...
    _require(
        ...
        f"V128 runtime git identity is not singular: {git_commits}",
    )
    _require(len(paired_inputs) == 9, "V128 paired input product changed")
    for (scenario, seed), rows in paired_inputs.items():
        _require(len(rows) == 2, f"V128 paired arm count changed: {scenario}/{seed}")
        for field in (
            ...
            _require(
                ...
                f"V128 paired {field} changed: {scenario}/{seed}",
            )
        _require(
            rows[0]["common_hpa_hash"] == EXPECTED_COMMON_HPA_SHA256,
            f"V128 common HPA changed: {scenario}/{seed}",
        )
    ...
    _require(
        ...
        "V128 completion-proximal and deeper current-player paths were not both exercised",
    )
    _require(
        completion_proximal_service_choice_evaluated_count > 0
        and completion_proximal_substitution_count > 0,
        "V128 completion-proximal service-choice evaluation/substitution path was not exercised",
    )
    _require(
        deeper_exact_anchor_preserved_count > 0 and deeper_substitution_count == 0,
        "V128 deeper-player exact-anchor preservation contract changed",
    )
    _require(
        proxy_input_unavailable_count == 0,
        "V128 admitted-work proxy inputs were unavailable",
    )
    _require(
        ...
        "V128 componentwise service accept/reject paths were not both exercised",
    )
    _require(
        componentwise_input_unavailable_count == 0,
        "V128 componentwise service inputs were unavailable",
    )
    _require(
        ...
        "V128 terminal-child path accept/reject paths were not both exercised",
    )
    _require(
        terminal_child_path_input_unavailable_count == 0,
        "V128 terminal-child path inputs were unavailable",
    )
    _require(
        ...
        "V128 terminal-child HPA path fail-closed reason count was nonzero",
    )
    _require(
        terminal_child_path_nonterminal_child_count == 0,
        "V128 terminal-child path scope included a nonterminal child",
    )
    _require(
        terminal_child_path_worse_container_path_count > 0,
        "V128 same-container worse-path rejection was not exercised",
    )

    output_payload = {
        "schema_version": "NSE_E3_COMPLETION_PROXIMAL_TERMINAL_CHILD_HPA_CONTAINER_COMPONENTWISE_PATH_BLIND_AUDIT_V128_V1",
        "created_at": utc_now(),
        "status": "pass",
        ...
        "terminal_child_container_componentwise_path_mechanism_coverage": {
            "terminal_child_container_componentwise_gate_evaluated_count": terminal_child_path_gate_evaluated_count,
            "terminal_child_container_componentwise_changed_parent_count": terminal_child_path_changed_parent_count,
            "terminal_child_container_componentwise_compared_child_count": terminal_child_path_compared_child_count,
            "terminal_child_container_componentwise_compared_container_path_count": terminal_child_path_compared_container_path_count,
            "terminal_child_container_componentwise_acceptance_count": terminal_child_path_acceptance_count,
            "terminal_child_container_componentwise_rejection_count": terminal_child_path_rejection_count,
            "terminal_child_container_componentwise_input_unavailable_count": terminal_child_path_input_unavailable_count,
            "terminal_child_container_componentwise_running_container_path_count": terminal_child_path_running_container_count,
            "terminal_child_container_componentwise_starting_container_path_count": terminal_child_path_starting_container_count,
            "terminal_child_container_componentwise_no_container_count": terminal_child_path_no_container_count,
            "terminal_child_container_componentwise_invalid_profile_count": terminal_child_path_invalid_profile_count,
            "terminal_child_container_componentwise_invalid_node_snapshot_count": terminal_child_path_invalid_node_snapshot_count,
            "terminal_child_container_componentwise_invalid_network_count": terminal_child_path_invalid_network_count,
            "terminal_child_container_componentwise_nonterminal_child_count": terminal_child_path_nonterminal_child_count,
            "terminal_child_container_componentwise_worse_child_count": terminal_child_path_worse_child_count,
            "terminal_child_container_componentwise_worse_container_path_count": terminal_child_path_worse_container_path_count,
            "terminal_child_container_componentwise_maximum_alternative_minus_anchor_for_same_container": terminal_child_path_maximum_same_container_delta,
            "terminal_child_container_componentwise_maximum_alternative_to_anchor_ratio_for_same_container": terminal_child_path_maximum_same_container_ratio,
            "performance_fields_consulted": False,
        },
        "arm_count": len(ARMS),
