...
def evaluate_training_rows(rows: Sequence[Mapping[str, Any]]) -> dict[str, Any]:
    ...
    for row in rows:
        ...
        if run_id in run_ids:
            raise ValueError(f"duplicate V144 run_id: {run_id}")
        run_ids.add(run_id)
    ...
    if len(rows) != 90 or actual != expected:
        raise ValueError(
            "V144 revealed product mismatch: "
...
def _validate_blind_document(blind: Mapping[str, Any], expected_hash: str) -> None:
    ...
    if (
        ...
        raise RuntimeError("V144 blind audit does not authorize reveal")
...
def _validate_blind_audit() -> dict[str, Any]:
    ...
    if any(
        ...
        raise RuntimeError("V144 blind audit has not been frozen into reveal code")
    if not BLIND_AUDIT.is_file() or file_hash(BLIND_AUDIT) != BLIND_AUDIT_FILE_SHA256:
        raise RuntimeError("V144 blind audit is missing or changed")
...
def _load_rows(blind: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    if len(baseline_admitted) != 81:
        raise RuntimeError("V144 blind baseline admission is incomplete")
    baseline_rows = []
    ...
    if not manifest_path.is_file() or file_hash(manifest_path) != frozen_manifest.get(
        ...
        raise RuntimeError("V144 frozen candidate manifest changed")
    manifest = load_and_validate_manifest(manifest_path)
    if manifest.get("manifest_hash") != frozen_manifest.get("manifest_hash"):
        raise RuntimeError("V144 candidate manifest hash changed")
    admitted = {item["run_id"]: item for item in blind["candidate_runs"]}
    if len(admitted) != 9:
        raise RuntimeError("V144 candidate admission is incomplete")
    candidate_rows = []
    for run in manifest["runs"]:
        run_id = run["run_id"]
        evidence = admitted.get(run_id)
        if evidence is None:
            raise RuntimeError(f"V144 run not admitted by blind audit: {run_id}")
        summary_path = (
            workspace_path(ROOT)
            / "canonical"
            / run_id
        ...
        if not summary_path.is_file() or file_hash(summary_path) != evidence.get(
            ...
            raise RuntimeError(f"V144 summary differs from blind audit: {run_id}")
        summary = read_json(summary_path)
        if summary.get("run_complete") is not True or summary.get("run_id") != run_id:
            raise RuntimeError(f"V144 summary identity changed: {run_id}")
...
def execute_reveal() -> dict[str, Any]:
    if OUTPUT.exists():
        raise RuntimeError(f"refusing to overwrite V144 result: {OUTPUT}")
    if not PLAN.is_file() or file_hash(PLAN) != PLAN_SHA256:
        raise RuntimeError("V144 plan changed before reveal")
    blind = _validate_blind_audit()
    rows = _load_rows(blind)
    evaluation = evaluate_training_rows(rows)
    result = {
        "schema_version": "NSE_E3_CAUSAL_BURST_MORPHOLOGY_TRAINING_RESULT_V144_V1",
        "created_at": utc_now(),
