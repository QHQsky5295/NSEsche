...
READY_SCHEDULE = ROOT / "frozen-ready-run-order-v147.json"
EXECUTION_RECEIPT = ROOT / "execution-receipt-v147.json"
...
def materialize_ready_schedule(root: Path = ROOT) -> dict[str, Any]:
    ...
    source_hash = _assert_hashed(source, "schedule_hash", "V147 source schedule")
    if (
        source.get("plan_sha256") != PLAN_SHA256
        or source.get("run_order_seed") != RUN_ORDER_SEED
        or len(source.get("schedule", [])) != 9
    ):
        raise RuntimeError("V147 source schedule boundary changed")
    ...
    for run in manifest["runs"]:
        ...
        if key in lookup:
            raise RuntimeError(f"duplicate V147 ready key: {key}")
        lookup[key] = run["run_id"]
    schedule = []
    for item in source["schedule"]:
        key = (item["scenario"], item["seed"])
        if key not in lookup:
            raise RuntimeError(f"missing V147 ready run: {key}")
        schedule.append({**item, "run_id": lookup[key]})
    if len(schedule) != 9 or len({item["run_id"] for item in schedule}) != 9:
        raise RuntimeError("V147 ready schedule is not an exact nine-run product")
    if set(lookup) != {(item["scenario"], item["seed"]) for item in schedule}:
        raise RuntimeError("V147 ready manifest contains unscheduled runs")
    document = {
        "schema_version": "NSE_E3_CAUSAL_RAW_PERSISTENCE_ROUTE_NATIVE_FRONTIER_READY_RUN_ORDER_V147_V1",
        "created_at": utc_now(),
    ...
    if output.exists():
        ...
        _assert_hashed(existing, "schedule_hash", "V147 ready schedule")
        left = dict(existing)
        right = dict(document)
        ...
        if left != right:
            raise RuntimeError("refusing to replace changed V147 ready schedule")
        return existing
    write_json_atomic(output, document)
    return document


def execute_v147(root: Path = ROOT) -> dict[str, Any]:
    output = root / EXECUTION_RECEIPT.name
    if output.exists():
        raise RuntimeError(f"V147 execution receipt already exists: {output}")
    if not PYTHON_PATH.is_file() or file_hash(PYTHON_PATH) != PYTHON_SHA256:
        raise RuntimeError("V147 frozen Python executable is missing or changed")
    ...
    for item in schedule["schedule"]:
        ...
        if canonical.is_dir():
            ...
            if not (
                ...
                raise RuntimeError(
                    f"V147 preexisting canonical is not an attempt-1 QC pass: {item['run_id']}"
                )
            dispatches.append(
                {
        ...
        if completed.returncode != 0:
            failure = {
                "schema_version": "NSE_E3_CAUSAL_RAW_PERSISTENCE_ROUTE_NATIVE_FRONTIER_EXECUTION_FAILURE_V147_V1",
                "created_at": utc_now(),
                "candidate_performance_summaries_parsed": 0,
                "plan_sha256": PLAN_SHA256,
            ...
            write_json_atomic(root / "execution-failure-v147.json", failure)
            raise RuntimeError(
                f"V147 frozen dispatch {item['ordinal']} failed with exit code "
                f"{completed.returncode}"
            )
    receipt = {
        "schema_version": "NSE_E3_CAUSAL_RAW_PERSISTENCE_ROUTE_NATIVE_FRONTIER_EXECUTION_V147_V1",
        "created_at": utc_now(),
        "candidate_performance_summaries_parsed": 0,
        "performance_results_consulted_for_mechanism_design": True,
        "plan_sha256": PLAN_SHA256,
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    document = materialize_ready_schedule() if args.materialize_only else execute_v147()
