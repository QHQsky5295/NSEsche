...
ROOT = Path("tmp/nse_e3e4_low_density_load_pareto_training_20260828_v97")
PLAN = Path(
    "scripts/reviewer_experiments/protocol/"
    "nse_e3e4_low_density_load_pareto_training_plan_v97.json"
)
PLAN_SHA256 = "b9dd0c9be896d68df34b436d90e9fe886cf5e82ae7d0c19470f5b7664d42a342"
V96_RESULT_RECEIPT = Path(
    "scripts/reviewer_experiments/protocol/"
    "nse_e3e4_guarded_anchor_training_result_v96.json"
)
V96_RESULT_RECEIPT_SHA256 = (
    "700224e285fd75f91d2bfbf23f29da9616507c2b8beed671a0692a884ee838d8"
)
V96_SOURCE_RESULT = Path(
    "tmp/nse_e3e4_guarded_anchor_training_20260828_v96/training-result-v96.json"
)
V96_SOURCE_RESULT_SHA256 = (
    "cc5400ce2bbc7f7da9a67be5c385e313bb345f7ff6fd68b11851bc63953ff496"
)
FORMAL_RESULT = Path(
...
BINARY_PATH = Path("tmp/nse_v97_build_18dee09/release/serverless_sim.exe")
BINARY_SHA256 = "df71f47bba25e406734dfc161ed17c7dc62c4b792d90cda415ab81b0ca06b277"
BINARY_SOURCE_COMMIT = "18dee09598cac9fafa6dc2a1ae8c70146cbe1e77"
...
SOURCE_INITIAL_SEEDS = [f"E{index}" for index in range(786, 796)]
SOURCE_CI_SEEDS = [f"E{index}" for index in range(796, 806)]
TRAINING_SEED_LIST = ["E786", "E787", "E788"]
TRAINING_SEEDS = set(TRAINING_SEED_LIST)
CONFIRMATION_SEEDS = [f"E{index}" for index in range(806, 826)]
PREVIOUSLY_RESERVED_SEEDS = [f"E{index}" for index in range(766, 786)]
PORT = "3132"
...
E3_GUARD32 = (
    ...
    "load_least_guard32_dual_window_safe_pareto"
)
E3_GUARD64 = (
    "faasrank_native_faithful_terminal_ocs_srpt_ready_"
    "load_least_guard64_dual_window_safe_pareto"
)
E4_GUARD32 = (
    "faasrank_native_faithful_terminal_ocs_idle_warm_dominance_srpt_ready_"
    "load_least_guard32_dual_window_safe_pareto"
)
E4_GUARD64 = (
    "faasrank_native_faithful_terminal_ocs_idle_warm_dominance_srpt_ready_"
...
ARMS = (
    ("v97-e3-anchor", "E3", "anchor", E3_ANCHOR, None, 9),
    ("v97-e3-load-guard32", "E3", "candidate", E3_GUARD32, 32.0, 9),
    ("v97-e3-load-guard64", "E3", "candidate", E3_GUARD64, 64.0, 9),
    ("v97-e4-anchor", "E4", "anchor", E4_ANCHOR, None, 3),
    ("v97-e4-load-guard32", "E4", "candidate", E4_GUARD32, 32.0, 3),
    ("v97-e4-load-guard64", "E4", "candidate", E4_GUARD64, 64.0, 3),
)
...
def _paths(root: Path) -> dict[str, Path]:
    return {
        "config": root / "v97-e3e4-training-config.json",
        "source": root / "manifest.v97-e786-e795-full.unbound.json",
        "capture": root / "manifest.v97-tape-capture.unbound.json",
        "prepared": root / "prepared-manifests-v97.json",
    }
...
def _assert_frozen_inputs() -> None:
    for path, expected in (
        (PLAN, PLAN_SHA256),
        (V96_RESULT_RECEIPT, V96_RESULT_RECEIPT_SHA256),
        (V96_SOURCE_RESULT, V96_SOURCE_RESULT_SHA256),
        ...
        if not path.is_file() or file_hash(path) != expected:
            raise RuntimeError(f"frozen V97 input is missing or changed: {path}")
...
def _rewrite(
    ...
    *,
    ...
    marker.update(
        {
            "purpose": purpose,
            "v97_training_plan_sha256": PLAN_SHA256,
            "v97_training_only": True,
            "v97_binary_source_commit": BINARY_SOURCE_COMMIT,
    ...
    if arm is not None:
        ...
        arm_id, experiment_id, role, profile, threshold, expected = arm
        marker.update(
            {
                "v97_arm_id": arm_id,
                "v97_experiment_id": experiment_id,
                "v97_role": role,
                "v97_profile": profile,
                "v97_queue_density_threshold": threshold,
                "v97_expected_run_count": expected,
            }
        )
        ...
        metadata.update(
            {
                "v97_training_plan_sha256": PLAN_SHA256,
                "v97_training_only": True,
                "v97_training_seed_metrics_previously_revealed": False,
                "v97_binary_source_commit": BINARY_SOURCE_COMMIT,
                "v97_confirmation_seeds_opened": False,
                "v97_previously_reserved_seeds_opened": False,
                "v97_formal_E01_E20_reexecution": False,
            }
        )
        if arm is not None:
            arm_id, experiment_id, role, profile, threshold, _ = arm
            ...
            metadata.update(
                {
                    "v97_arm_id": arm_id,
                    "v97_arm_role": role,
                    "v97_candidate_profile": profile,
                    "v97_candidate_experiment": experiment_id,
                    "v97_queue_density_threshold": threshold,
                    "v97_faasrank_model_artifact_sha256": MODEL_SHA256,
                    "v97_native_faithful_initializer": True,
                    "v97_dual_window_safe_pareto": True,
                    "v97_low_density_load_least_guard": threshold is not None,
                    "v97_idle_warm_dominance_router": (
                        "idle_warm_dominance" in profile
                    ),
                    "v97_srpt_critical_path_player_order": "srpt_ready" in profile,
                }
            )
...
def prepare_v97(root: Path = ROOT) -> dict:
    _assert_frozen_inputs()
    if root.exists():
        raise RuntimeError(f"refusing to overwrite V97 training root: {root}")
    ...
    capture = derive_integration_smoke_shard(
        paths["source"],
        capture_ids,
        purpose=f"V97 fresh balanced-QoS parent tapes; plan_sha256={PLAN_SHA256}",
    )
    _write(
        paths["capture"],
        _rewrite(
    ...
    for arm in ARMS:
        ...
        template = derive_integration_smoke_shard(
            paths["source"],
            selected,
            purpose=f"V97 {arm_id} low-density-load training; plan_sha256={PLAN_SHA256}",
        )
        manifest = _rewrite(
            template,
            purpose=f"V97 {arm_id} low-density-load training; plan_sha256={PLAN_SHA256}",
        ...
        if (
            ...
            raise RuntimeError(f"V97 {arm_id} must have {expected} runs/references")
        if {run["seed"] for run in manifest["runs"]} != TRAINING_SEEDS:
            raise RuntimeError(f"V97 {arm_id} escaped E786-E788")
        keys = {item["key"] for item in manifest["reference_build_dependencies"]}
        if all_reference_keys & keys:
            raise RuntimeError("V97 arm-specific reference keys unexpectedly overlap")
        all_reference_keys.update(keys)
        _write(arm_path(root, arm_id), manifest)
    if len(all_reference_keys) != 36:
        raise RuntimeError("V97 must declare exactly 36 arm-specific references")
    ...
    receipt = {
        "schema_version": "NSE_E3E4_LOW_DENSITY_LOAD_PARETO_PREPARED_V97_V1",
        "created_at": utc_now(),
        "formal_results_eligible": False,
        ...
        "V96_result_receipt_path": str(V96_RESULT_RECEIPT),
        "V96_result_receipt_sha256": V96_RESULT_RECEIPT_SHA256,
        "V96_source_result_path": str(V96_SOURCE_RESULT),
        "V96_source_result_sha256": V96_SOURCE_RESULT_SHA256,
        ...
        "arms": [
            {
                ...
                "role": role,
                "profile": profile,
                "queue_density_threshold": threshold,
                "run_count": expected,
                "reference_build_count": expected,
            }
            for arm_id, experiment_id, role, profile, threshold, expected in ARMS
        ],
        "base_tape_captures": 3,
        "derived_burst_tapes": 9,
        "arm_reference_builds": 36,
        "arm_online_runs": 36,
