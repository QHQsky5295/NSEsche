...
ROOT = Path("tmp/nse_e3_causal_raw_persistence_router_training_20260830_v145")
PLAN = Path(
    "scripts/reviewer_experiments/protocol/"
    "nse_e3_causal_raw_persistence_router_training_plan_v145.json"
)
PLAN_SHA256 = "753d63a9ca14582a5cb54ea0186cca019d1aada8ed36ab89b1723d104eec5020"
...
BINARY_PATH = Path("tmp/nse_v145_build_c310c86/release/serverless_sim.exe")
BINARY_SHA256 = "0856523e362f18acd15f8649854b7a69685842f1c1b35e2f5fe1fe9ad3bef7f4"
BINARY_SOURCE_COMMIT = "c310c8659445aff99048eb5a1f67752d77662a09"
V145_REVISION = "raw_persistence_v1"

ARM_ID = "v145-e3-causal-raw-persistence-native-router-nash"
PROFILE = "causal_raw_persistence_greedy_faasrank_loadleast_router_nash"
...
RUN_ORDER_SEED = "NSE-V145-RAW-PERSISTENCE-E1526-E1528"
PORT = "3174"
...
V145_SERVICE_STATE_DOMAIN = "runtime_existing_aggregates_and_admitted_work"
...
def paths(root: Path = ROOT) -> dict[str, Path]:
    return {
        ...
        "schedule": root / "frozen-run-order-v145.json",
    }
...
def _assert_frozen_inputs() -> None:
    ...
    for path, expected in frozen:
        if not path.is_file() or file_hash(path).lower() != expected.lower():
            raise RuntimeError(f"frozen V145 input is missing or changed: {path}")
...
def _rewrite_candidate(template: dict[str, Any]) -> dict[str, Any]:
    ...
    marker.update(
        {
            "purpose": f"V145 {ARM_ID} {PLAN_SHA256}",
            "v145_role": "adaptive_training_candidate",
            "v145_training_plan_sha256": PLAN_SHA256,
            "v145_binary_source_commit": BINARY_SOURCE_COMMIT,
            "v145_revision": V145_REVISION,
            "v145_parent_v144_plan_sha256": V144_PLAN_SHA256,
            "v145_parent_v144_blind_sha256": V144_BLIND_SHA256,
            "v145_parent_v144_result_sha256": V144_RESULT_SHA256,
            "v145_arm_id": ARM_ID,
            "v145_profile": PROFILE,
            "v145_native_selection_rule": SELECTION_RULE,
            "v145_native_portfolio_members": NATIVE_MEMBERS,
            "v145_runtime_native_kind_order": RUNTIME_NATIVE_KINDS,
            "v145_shock_threshold_numerator": SHOCK_THRESHOLD_NUMERATOR,
            "v145_shock_threshold_denominator": SHOCK_THRESHOLD_DENOMINATOR,
            "v145_shock_baseline_frames": SHOCK_BASELINE_FRAMES,
            "v145_shock_recent_frames": SHOCK_RECENT_FRAMES,
            "v145_shock_active_frames": SHOCK_ACTIVE_FRAMES,
            "v145_persistence_baseline_frames": PERSISTENCE_BASELINE_FRAMES,
            "v145_persistence_recent_frames": PERSISTENCE_RECENT_FRAMES,
            "v145_persistence_evaluation_age_frames": PERSISTENCE_EVALUATION_AGE_FRAMES,
            "v145_persistence_threshold_numerator": PERSISTENCE_THRESHOLD_NUMERATOR,
            "v145_persistence_threshold_denominator": PERSISTENCE_THRESHOLD_DENOMINATOR,
            "v145_expert_lifecycle": EXPERT_LIFECYCLE,
            "v145_expected_run_count": 9,
            "v145_expected_reference_count": 9,
            "v145_reused_v142_baseline_runs": 81,
            "v145_baseline_rerun_count": 0,
            "v145_performance_results_consulted_for_mechanism_design": True,
            "v145_candidate_performance_summaries_parsed": 0,
            "v145_confirmation_inputs_opened": False,
        ...
        metadata.update(
            {
                ...
                "v145_parent_v144_blind_sha256": V144_BLIND_SHA256,
                "v145_parent_v144_result_sha256": V144_RESULT_SHA256,
                "v145_training_only": True,
                "v145_role": "adaptive_training_candidate",
                "v145_arm_id": ARM_ID,
                "v145_profile": PROFILE,
                "v145_native_selection_rule": SELECTION_RULE,
                "v145_native_portfolio_members": NATIVE_MEMBERS,
                "v145_source_v142_run_id": source_run_id,
                "v145_source_v142_run_spec_hash": source_run_spec_hash,
                "v145_expert_lifecycle": EXPERT_LIFECYCLE,
                "v145_all_three_experts_advanced_every_window": True,
                "v145_shock_detector": "first_seen_arrivals_80_frame_baseline_20_frame_recent_3_over_2_threshold_50_frame_latch_for_episode_onset_only",
                "v145_persistence_test": "freeze_preceding80_raw_arrivals_at_first_onset_then_at_age55_compare_recent5_mean_to_3_over_2_baseline_mean_once_with_checked_integer_arithmetic",
                "v145_quiet_route": "greedy",
                "v145_first_short_route": "greedy",
                "v145_first_sustained_route": "faasrank",
                "v145_recurrent_route": "load_least",
                "v145_complete_native_assignments_required": True,
                "v145_service_certificate_scope": SERVICE_CERTIFICATE_SCOPE,
                "v145_service_certificate_state_domain": V145_SERVICE_STATE_DOMAIN,
                "v145_paper_welfare_state_domain": V145_WELFARE_STATE_DOMAIN,
                "v145_no_numeric_tuning": True,
                "v145_outcome_fields_drive_policy": False,
                "v145_scenario_or_burst_label_used_by_policy": False,
                "v145_future_arrivals_used_by_policy": False,
                "v145_adaptive_training_reuses_v142_results": True,
                "v145_confirmation_inputs_opened": False,
            }
        )
...
def _validate_product(manifest: dict[str, Any]) -> None:
    ...
    if len(runs) != 9 or actual != expected:
        raise RuntimeError("V145 scenario/seed product changed")
    if {run["method"] for run in runs} != {"sche_nash"}:
        raise RuntimeError("V145 method product changed")
    if len(manifest["reference_build_dependencies"]) != 9:
        raise RuntimeError("V145 reference product changed")
    if any(
        run["environment"].get("NASH_OPERATIONAL_EXPERT_PROXY") != PROFILE
        or run.get("metadata", {}).get("v145_native_portfolio_members")
        != NATIVE_MEMBERS
        or run.get("metadata", {}).get("v145_all_three_experts_advanced_every_window")
        is not True
        or run.get("metadata", {}).get("v145_quiet_route") != "greedy"
        or run.get("metadata", {}).get("v145_persistence_test")
        ...
        or run.get("metadata", {}).get("v145_confirmation_inputs_opened") is not False
        for run in runs
    ):
        raise RuntimeError("V145 candidate contract changed")
...
def _frozen_schedule(manifest: dict[str, Any]) -> dict[str, Any]:
    ...
    document = {
        "schema_version": "NSE_E3_CAUSAL_RAW_PERSISTENCE_RUN_ORDER_V145_V1",
        "created_at": utc_now(),
...
def prepare_v145(root: Path = ROOT) -> dict[str, Any]:
    _assert_frozen_inputs()
    if root.exists():
        raise RuntimeError(f"refusing to overwrite V145 training root: {root}")
    ...
    receipt = {
        "schema_version": "NSE_E3_CAUSAL_RAW_PERSISTENCE_PREPARED_V145_V1",
        "created_at": utc_now(),
        ...
        "parent_disposition": "V142_V143_and_V144_complete_training_falsified_no_confirmation_inputs_generated",
        "v145_revision": V145_REVISION,
