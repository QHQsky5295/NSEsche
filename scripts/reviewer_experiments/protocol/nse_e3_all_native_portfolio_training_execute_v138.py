...
READY_SCHEDULE = ROOT / "frozen-ready-run-order-v138.json"
EXECUTION_RECEIPT = ROOT / "execution-receipt-v138.json"
...
def _manifest_ids() -> list[str]:
    return ["v138-baselines", *ARM_IDS]


def materialize_ready_schedule(root: Path = ROOT) -> dict[str, Any]:
    base_path = paths(root)["schedule"]
    if not base_path.is_file():
        raise RuntimeError(f"missing V138 base schedule: {base_path}")
    base = read_json(base_path)
    base_hash = _assert_hashed_object(base, "schedule_hash", "V138 base schedule")
    if (
        base.get("run_order_seed") != RUN_ORDER_SEED
        or base.get("method_labels") != METHOD_LABELS
        or len(base.get("schedule", [])) != 108
    ):
        raise RuntimeError("V138 base schedule boundary changed")
    ...
    for manifest_id in _manifest_ids():
        ...
        label_override = None if manifest_id == "v138-baselines" else manifest_id
        for run in manifest["runs"]:
            label = label_override or run["method"]
            key = (scenario_id(run), run["seed"], label)
            if key in lookup:
                raise RuntimeError(f"duplicate V138 ready schedule key: {key}")
    ...
    for item in base["schedule"]:
        ...
        try:
            ...
            raise RuntimeError(
                f"missing V138 ready run for schedule key: {key}"
            ) from exc
        manifest_id = (
            "v138-baselines"
    ...
    if len(schedule) != 108 or len({item["run_id"] for item in schedule}) != 108:
        raise RuntimeError("V138 ready schedule is not an exact 108-run product")
    if set(lookup) != {
        (item["scenario"], item["seed"], item["method_label"]) for item in schedule
    }:
        raise RuntimeError("V138 ready manifests contain unscheduled runs")

    document = {
        "schema_version": "NSE_E3_ALL_NATIVE_PORTFOLIO_READY_RUN_ORDER_V138_V1",
        "created_at": utc_now(),
    ...
    if output.exists():
        ...
        existing_hash = _assert_hashed_object(
            existing, "schedule_hash", "V138 ready schedule"
        )
        ...
        if comparable_existing != comparable_document:
            raise RuntimeError("refusing to replace changed V138 ready schedule")
...
def execute_v138(root: Path = ROOT) -> dict[str, Any]:
    if EXECUTION_RECEIPT.exists() and root == ROOT:
        raise RuntimeError(
            f"V138 execution receipt already exists: {EXECUTION_RECEIPT}"
        )
    if not PYTHON_PATH.is_file() or file_hash(PYTHON_PATH) != PYTHON_SHA256:
        raise RuntimeError("V138 frozen Python executable is missing or changed")
    ...
    for item in schedule["schedule"]:
        ...
        if canonical.is_dir():
            ...
            if not (
                ...
                raise RuntimeError(
                    f"V138 preexisting canonical is not an attempt-1 QC pass: "
                    f"{item['run_id']}"
                )
            dispatches.append(
                {
        ...
        if completed.returncode != 0:
            failure = {
                "schema_version": "NSE_E3_ALL_NATIVE_PORTFOLIO_EXECUTION_FAILURE_V138_V1",
                "created_at": utc_now(),
                "performance_results_consulted": False,
                "plan_sha256": PLAN_SHA256,
            ...
            write_json_atomic(root / "execution-failure-v138.json", failure)
            raise RuntimeError(
                f"V138 frozen dispatch {item['ordinal']} failed with "
                f"exit code {completed.returncode}"
            )

    receipt = {
        "schema_version": "NSE_E3_ALL_NATIVE_PORTFOLIO_EXECUTION_V138_V1",
        "created_at": utc_now(),
        "performance_results_consulted": False,
        "plan_sha256": PLAN_SHA256,
        "ready_schedule_path": str(schedule_path),
    ...
    if output.exists():
        raise RuntimeError(f"refusing to overwrite V138 execution receipt: {output}")
