...
def _manifest_ids() -> list[str]:
    return ["v137-baselines", *ARM_IDS]


def materialize_ready_schedule(root: Path = ROOT) -> dict[str, Any]:
    base_path = paths(root)["schedule"]
    if not base_path.is_file():
        raise RuntimeError(f"missing V137 base schedule: {base_path}")
    base = read_json(base_path)
    base_hash = _assert_hashed_object(base, "schedule_hash", "V137 base schedule")
    if (
        base.get("run_order_seed") != RUN_ORDER_SEED
        or base.get("method_labels") != METHOD_LABELS
        or len(base.get("schedule", [])) != 108
    ):
        raise RuntimeError("V137 base schedule boundary changed")
    ...
    for manifest_id in _manifest_ids():
        ...
        label_override = None if manifest_id == "v137-baselines" else manifest_id
        for run in manifest["runs"]:
            label = label_override or run["method"]
            key = (scenario_id(run), run["seed"], label)
            if key in lookup:
                raise RuntimeError(f"duplicate V137 ready schedule key: {key}")
    ...
    if len(schedule) != 108 or len({item["run_id"] for item in schedule}) != 108:
        raise RuntimeError("V137 ready schedule is not an exact 108-run product")
    if set(lookup) != {
        (item["scenario"], item["seed"], item["method_label"]) for item in schedule
    }:
        raise RuntimeError("V137 ready manifests contain unscheduled runs")

    document = {
        "schema_version": "NSE_E3_NATIVE_FRONTIER_ANCHOR_READY_RUN_ORDER_V137_V1",
        "created_at": utc_now(),
    ...
    if output.exists():
        ...
        existing_hash = _assert_hashed_object(
            existing, "schedule_hash", "V137 ready schedule"
        )
        ...
        if comparable_existing != comparable_document:
            raise RuntimeError("refusing to replace changed V137 ready schedule")
...
def execute_v137(root: Path = ROOT) -> dict[str, Any]:
    if EXECUTION_RECEIPT.exists() and root == ROOT:
        raise RuntimeError(
            f"V137 execution receipt already exists: {EXECUTION_RECEIPT}"
        )
    ...
    for item in schedule["schedule"]:
        ...
        if completed.returncode != 0:
            failure = {
                "schema_version": "NSE_E3_NATIVE_FRONTIER_ANCHOR_EXECUTION_FAILURE_V137_V1",
                "created_at": utc_now(),
                "performance_results_consulted": False,
                "plan_sha256": PLAN_SHA256,
    ...
    receipt = {
        "schema_version": "NSE_E3_NATIVE_FRONTIER_ANCHOR_EXECUTION_V137_V1",
        "created_at": utc_now(),
        "performance_results_consulted": False,
        "plan_sha256": PLAN_SHA256,
        "ready_schedule_path": str(schedule_path),
    ...
    if output.exists():
        raise RuntimeError(f"refusing to overwrite V137 execution receipt: {output}")
