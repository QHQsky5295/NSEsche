...
V91_RESULT = Path(
    "scripts/reviewer_experiments/protocol/"
    "nse_e3e4_exact_native_per_player_training_result_v91.json"
)
V91_RESULT_SHA256 = "b141c06f894a4488aa0ea3f268296a6007ae4a431a0eebc9983043e504e02ccb"
SOURCE = Path(
...
BINARY = Path("tmp/nse_v92_build_1fbeaa0/release/serverless_sim.exe")
BINARY_SHA256 = "a2be2019e5875719a03d1a6aa72bfc202865a473605850f02680aded3c41d671"
BINARY_SOURCE_COMMIT = "1fbeaa0ddc22aba3a09582fe5a596bc777da1c3a"
OUTPUT = ROOT / "manifest.v92-strict-native-per-player-training.sla.json"
RECEIPT = ROOT / "prepared-training-v92.json"
TRAINING_SEEDS = {"E713", "E714", "E715"}
CONFIRMATION_SEEDS = ["E716", "E717", "E718"]
E3_PROFILE = "ocs_native_exact_pipeline_per_player_strict_pareto"
E4_PROFILE = "jiagu_native_exact_per_player_strict_pareto"


def _assert_inputs() -> None:
    for path, expected in (
        (PLAN, PLAN_SHA256),
        (V91_RESULT, V91_RESULT_SHA256),
        (SOURCE, SOURCE_SHA256),
        (BINARY, BINARY_SHA256),
    ):
        if not path.is_file() or file_hash(path) != expected:
            raise RuntimeError(
                f"frozen V92 training input is missing or changed: {path}"
            )
...
def _rewrite_manifest(source: dict) -> dict:
    ...
    if manifest.get("formal_results_eligible") is not False:
        raise RuntimeError("V92 training source must remain non-formal")
    if len(manifest.get("runs", [])) != 12:
        raise RuntimeError("V92 training source must contain exactly 12 runs")
    if {run.get("seed") for run in manifest["runs"]} != TRAINING_SEEDS:
        raise RuntimeError("V92 training source seed set changed")
    if {run.get("experiment_id") for run in manifest["runs"]} != {"E3", "E4"}:
        raise RuntimeError("V92 training source experiment set changed")
    if any(run.get("method") != "sche_nash" for run in manifest["runs"]):
        raise RuntimeError("V92 training source contains a non-NSESche run")
    ...
    marker["purpose"] = (
        "V92 training-only exact native strict-player Pareto guard on already "
        f"revealed E713-E715; plan_sha256={PLAN_SHA256}"
    )
    marker["v92_training_plan_sha256"] = PLAN_SHA256
    marker["v92_training_only"] = True
    marker["v92_binary_source_commit"] = BINARY_SOURCE_COMMIT
    marker["v92_strict_player_improvement_required"] = True
    ...
    for run in manifest["runs"]:
        ...
        variant = (
            "v92-training-strict-ocs-per-player"
            if run["experiment_id"] == "E3"
            else "v92-training-strict-jiagu-per-player"
        )
        run["variant"] = variant
        run["environment"]["NASH_OPERATIONAL_EXPERT_PROXY"] = profile
        metadata = run.setdefault("metadata", {})
        metadata["v92_training_plan_sha256"] = PLAN_SHA256
        metadata["v92_training_only"] = True
        metadata["v92_training_seed_metrics_previously_revealed"] = True
        metadata["v92_candidate_profile"] = profile
        metadata["v92_candidate_variant"] = variant
        metadata["v92_binary_source_commit"] = BINARY_SOURCE_COMMIT
        metadata["v92_exact_native_initializer"] = True
        metadata["v92_every_player_expert_nonworsening_certificate"] = True
        metadata["v92_at_least_one_player_strict_expert_improvement"] = True
        metadata["v92_resource_scaling_excluded"] = True
        metadata["v92_complete_v91_cohort_retained"] = True
    ...
    if len(candidate_reference_keys) != 12:
        raise RuntimeError("V92 must have exactly 12 candidate-specific references")
    if source_reference_keys & candidate_reference_keys:
        raise RuntimeError(
            "V92 environment-bound references unexpectedly reuse source keys"
        )
...
def main() -> None:
    _assert_inputs()
    if ROOT.exists():
        raise RuntimeError(f"refusing to overwrite V92 training root: {ROOT}")
    ROOT.mkdir(parents=True)
    source = json.loads(SOURCE.read_text(encoding="utf-8"))
    manifest = _rewrite_manifest(source)
    write_json_atomic(OUTPUT, manifest)
    receipt = {
        "schema_version": "NSE_E3E4_STRICT_NATIVE_PER_PLAYER_TRAINING_PREPARED_V92_V1",
        "created_at": utc_now(),
        ...
        "V91_result_path": str(V91_RESULT),
        "V91_result_sha256": V91_RESULT_SHA256,
