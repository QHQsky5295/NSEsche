...
from scripts.reviewer_experiments.protocol.nse_e3_all_native_portfolio_training_blind_audit_v139 import (
    ARMS,
...
OUTPUT = ROOT / "training-result-v139.json"
...
def evaluate_training_rows(rows: Sequence[Mapping[str, Any]]) -> dict[str, Any]:
    ...
    for row in rows:
        ...
        if run_id in run_ids:
            raise ValueError(f"duplicate V139 run_id: {run_id}")
        run_ids.add(run_id)
    ...
    if len(rows) != 108 or actual != expected:
        raise ValueError(
            "V139 revealed product mismatch: "
...
def _validate_blind_audit() -> dict[str, Any]:
    ...
    if any(
        ...
        raise RuntimeError("V139 blind audit has not been frozen into reveal code")
    if not BLIND_AUDIT.is_file() or file_hash(BLIND_AUDIT) != BLIND_AUDIT_FILE_SHA256:
        raise RuntimeError("V139 blind audit is missing or changed")
    ...
    if (
        ...
        raise RuntimeError("V139 blind audit does not authorize reveal")
    return blind
...
def _load_rows(blind: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    if len(admitted) != 108:
        raise RuntimeError("V139 blind audit run evidence is incomplete")
    rows = []
    for manifest_id in ["v139-baselines", *ARM_IDS]:
        ...
        if not manifest_path.is_file() or file_hash(manifest_path) != frozen.get(
            ...
            raise RuntimeError(f"V139 frozen manifest changed: {manifest_id}")
        manifest = load_and_validate_manifest(manifest_path)
        if manifest.get("manifest_hash") != frozen.get("manifest_hash"):
            raise RuntimeError(f"V139 manifest hash changed: {manifest_id}")
        ...
        for run in manifest["runs"]:
            ...
            if evidence is None or evidence.get("manifest_id") != manifest_id:
                raise RuntimeError(f"V139 run not admitted by blind audit: {run_id}")
            summary_path = (
            ...
            if not summary_path.is_file() or file_hash(summary_path) != evidence.get(
                ...
                raise RuntimeError(f"V139 summary differs from blind audit: {run_id}")
            summary = read_json(summary_path)
            if (
                summary.get("run_complete") is not True
                or summary.get("run_id") != run_id
            ):
                raise RuntimeError(f"V139 summary identity changed: {run_id}")
            rows.append(
                {
...
def execute_reveal() -> dict[str, Any]:
    if OUTPUT.exists():
        raise RuntimeError(f"refusing to overwrite V139 result: {OUTPUT}")
    if not PLAN.is_file() or file_hash(PLAN) != PLAN_SHA256:
        raise RuntimeError("V139 plan changed before reveal")
    blind = _validate_blind_audit()
    rows = _load_rows(blind)
    evaluation = evaluate_training_rows(rows)
    result = {
        "schema_version": "NSE_E3_ALL_NATIVE_PORTFOLIO_TRAINING_RESULT_V139_V1",
        "created_at": utc_now(),
