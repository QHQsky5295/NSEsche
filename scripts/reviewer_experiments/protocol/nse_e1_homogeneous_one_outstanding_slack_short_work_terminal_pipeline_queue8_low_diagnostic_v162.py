...
ROOT = Path(
    "tmp/nse_e1_homogeneous_one_outstanding_slack_short_work_terminal_pipeline_queue8_low_diagnostic_20260901_v162"
)
PLAN = Path(
    "scripts/reviewer_experiments/protocol/"
    "nse_e1_homogeneous_one_outstanding_slack_short_work_terminal_pipeline_queue8_low_diagnostic_plan_v162.json"
)
PLAN_SHA256 = "139acc39227e741a608badfdb4e6893d181a7d400a62c038ac9597ce359f7212"
IMPLEMENTATION = Path(
    "scripts/reviewer_experiments/protocol/"
    "nse_e1_homogeneous_one_outstanding_slack_short_work_terminal_pipeline_queue8_low_diagnostic_implementation_v162.json"
)
IMPLEMENTATION_SHA256 = (
    "9005c1730d812b5958040c7cceada3e020f075e02ecadbf8375c71f39c7e1425"
)
V161_RESULT = Path(
    "tmp/nse_e1_homogeneous_jit_parent_tail_slack_short_work_terminal_pipeline_queue8_low_diagnostic_20260831_v161/"
    "diagnostic-result-v161.json"
)
V161_RESULT_SHA256 = "f143731de00977de67678717d85b7cf30eca18652be05765a4b187f65736a4df"
V161_RESULT_HASH = "ffc4abff1ee112b8c84b3f28373fb99b2b3321b1a0d785dec1849236612df56d"

ARM_ID = (
    "v162-low-srpt-slack-one-outstanding-short5p5-" "terminal-pipeline-hiku2-ocs-queue8"
)
PROFILE = "srpt_slack_one_outstanding_short5p5_terminal_pipeline_hiku2_ocs_queue8"
FRONTIER = (
    "parents_completed_or_terminal_or_slack_one_outstanding_"
    "short_work_parents_scheduled"
)
SINGLE_CHANGE = (
    "V159_slack_short_work_pipeline_plus_rolling_one_outstanding_"
    "incomplete-parent_nonterminal_credit_per_request"
)
TERMINAL_DEFINITION = (
    "admit_all_parents-completed_and_terminal_parents-scheduled_players_plus_"
    "at_most_one_deterministic_short_incomplete-parent_nonterminal_player_per_"
    "request_only_when_no_such_assignment_is_already_outstanding"
)
ONE_OUTSTANDING_DEFINITION = (
    "one_assigned_unfinished_nonterminal_function_with_at_least_one_unfinished_"
    "direct_parent_per_request"
)
...
PORT = "3213"
BINARY_SOURCE_COMMIT = "1f5ef720086f3b1a17dfee0024e21095dfcd6837"
BINARY_PATH = Path("serverless_sim/target_e1_v162/release/serverless_sim.exe")
BINARY_SHA256 = "4dac68abe36f4cb90c0cea2d833317c0a3a266595b746c5644e4c393c0f589d2"


def paths(root: Path = ROOT) -> dict[str, Path]:
    return {
        "manifest": root / f"manifest.{ARM_ID}.unbound.json",
        "prepared": root / "prepared-v162.json",
        "schedule": root / "frozen-run-order-v162.json",
        ...
        "execution": root / "execution-receipt-v162.json",
        "pairing": root / "pairing-audit-v162.json",
        "blind": root / "joint-blind-audit-v162.json",
        "result": root / "diagnostic-result-v162.json",
    }
...
def _assert_frozen_inputs() -> dict[str, Any]:
    ...
    for path, sha256, label in (
        (PLAN, PLAN_SHA256, "V162 plan"),
        (IMPLEMENTATION, IMPLEMENTATION_SHA256, "V162 implementation receipt"),
        (V161_RESULT, V161_RESULT_SHA256, "V161 result"),
        (BINARY_PATH, BINARY_SHA256, "V162 release binary"),
    ...
    if not (
        ...
        and change.get("outstanding_definition")
        == "assigned_and_unfinished_nonterminal_function_with_at_least_one_unfinished_direct_parent"
        and change.get("outstanding_limit_per_request") == 1
        and change.get("selection_order") == SELECTION_ORDER
        and change.get("occupied_credit_and_same_window_extra_candidates_fail_closed")
        is True
        and change.get("changes_paper_formula_welfare_pricing_hpa_metrics_or_reference")
        is False
    ):
        raise RuntimeError("V162 implementation boundary changed")
    result = read_json(V161_RESULT)
    if not (
        _assert_hashed(result, "result_hash", "V161 result") == V161_RESULT_HASH
        ...
        raise RuntimeError("V161 result boundary changed")
    _assert_json_semantic(
...
def _rewrite_candidate(
    ...
    marker.update(
        {
            "purpose": (
                "V162 outcome-disclosed three-seed rolling one-outstanding "
                "slack short-work diagnostic; "
                "never a formal result or paper superiority claim"
            ),
            "v162_role": (
                "result_blind_one_outstanding_slack_short_work_falsification"
            ),
            "v162_plan_sha256": PLAN_SHA256,
            "v162_implementation_sha256": IMPLEMENTATION_SHA256,
            "v162_binary_source_commit": BINARY_SOURCE_COMMIT,
            "v162_protocol_source_commit": protocol_source_commit,
            "v162_binary_sha256": BINARY_SHA256,
            "v162_arm_id": ARM_ID,
            "v162_profile": PROFILE,
            "v162_player_frontier": FRONTIER,
            "v162_single_change_from_v159": SINGLE_CHANGE,
            "v162_short_work_threshold": SHORT_WORK_THRESHOLD,
            "v162_queue_density_threshold": QUEUE_THRESHOLD,
            "v162_queue_boundary": "below_is_strict",
            "v162_one_outstanding_definition": ONE_OUTSTANDING_DEFINITION,
            "v162_outstanding_limit_per_request": 1,
            "v162_selection_order": SELECTION_ORDER,
            "v162_occupied_and_same_window_extra_fail_closed": True,
            "v162_environment": COMMON_ENVIRONMENT,
            "v162_expected_run_count": 3,
            "v162_expected_reference_build_count": 3,
            "v162_fixed_order": list(SEEDS),
            "v162_candidate_performance_summaries_parsed": 0,
            "v162_remaining_seventeen_authorized": False,
            "v162_confirmation_inputs_generated": False,
        }
    )
    for run in rewritten["runs"]:
        ...
        run["metadata"] = {
            ...
            "v162_selection_order": SELECTION_ORDER,
            "v162_occupied_and_same_window_extra_fail_closed": True,
            "v162_source_e1_run_id": source_run_id,
            "v162_source_e1_run_spec_hash": source_run_spec_hash,
            "v162_candidate_performance_summaries_parsed_before_run": 0,
            "v162_remaining_seventeen_authorized": False,
            "v162_confirmation_inputs_generated": False,
        }
...
def _validate_product(manifest: Mapping[str, Any], *, references_bound: bool) -> None:
    if not (
        ...
        raise RuntimeError("V162 exact E09/E18/E20 product changed")
    ...
    for run in manifest["runs"]:
        ...
        if not (
            ...
            and metadata.get("v162_profile") == PROFILE
            and metadata.get("v162_player_frontier") == FRONTIER
            and metadata.get("v162_short_work_threshold") == SHORT_WORK_THRESHOLD
            and metadata.get("v162_queue_density_threshold") == QUEUE_THRESHOLD
            and metadata.get("v162_queue_boundary") == "below_is_strict"
            and metadata.get("v162_one_outstanding_definition")
            == ONE_OUTSTANDING_DEFINITION
            and metadata.get("v162_outstanding_limit_per_request") == 1
            and metadata.get("v162_selection_order") == SELECTION_ORDER
            and metadata.get("v162_occupied_and_same_window_extra_fail_closed") is True
        ):
            raise RuntimeError(f"V162 run contract changed: {run.get('run_id')}")


def prepare_v162(root: Path = ROOT) -> dict[str, Any]:
    source = _assert_frozen_inputs()
    if root.exists():
        raise RuntimeError(f"refusing to overwrite V162 root: {root}")
    ...
    schedule = {
        "schema_version": "NSE_E1_HOMOGENEOUS_ONE_OUTSTANDING_SLACK_SHORT_WORK_TERMINAL_PIPELINE_QUEUE8_LOW_SCHEDULE_V162_V1",
        "created_at": utc_now(),
    ...
    receipt = {
        "schema_version": "NSE_E1_HOMOGENEOUS_ONE_OUTSTANDING_SLACK_SHORT_WORK_TERMINAL_PIPELINE_QUEUE8_LOW_PREPARED_V162_V1",
        "created_at": utc_now(),
        ...
        "v161_result_file_sha256": V161_RESULT_SHA256,
        "v161_result_hash": V161_RESULT_HASH,
        ...
        "one_outstanding_definition": ONE_OUTSTANDING_DEFINITION,
        "outstanding_limit_per_request": 1,
        "selection_order": SELECTION_ORDER,
        "occupied_and_same_window_extra_fail_closed": True,
...
def execute_v162(root: Path = ROOT) -> dict[str, Any]:
    output = paths(root)
    if output["execution"].exists():
        raise RuntimeError("V162 execution receipt already exists")
    prepared = read_json(output["prepared"])
    _assert_hashed(prepared, "receipt_hash", "V162 prepared receipt")
    ...
    for ordinal, seed in enumerate(SEEDS, start=1):
        ...
        if completed.returncode != 0:
            raise RuntimeError(f"V162 dispatch {seed} failed: {completed.returncode}")
        ...
        if not (
            ...
            raise RuntimeError(f"V162 canonical is not a QC pass: {run['run_id']}")
        dispatches.append(
            {
                "ordinal": ordinal,
                "seed": seed,
    ...
    receipt = {
        "schema_version": "NSE_E1_HOMOGENEOUS_ONE_OUTSTANDING_SLACK_SHORT_WORK_TERMINAL_PIPELINE_QUEUE8_LOW_EXECUTION_V162_V1",
        "created_at": utc_now(),
        "candidate_performance_summaries_parsed": 0,
        "plan_sha256": PLAN_SHA256,
...
def _count(value: Any, label: str) -> int:
    if isinstance(value, bool) or not isinstance(value, int) or value < 0:
        raise RuntimeError(f"V162 {label} is invalid")
    return value
...
def _finite_optional(value: Any, label: str) -> float | None:
    ...
    if not math.isfinite(result):
        raise RuntimeError(f"V162 {label} is nonfinite")
    return result
...
def _audit_nash_log(canonical: Path, run: Mapping[str, Any]) -> dict[str, Any]:
    ...
    terminal = short = credit_admitted = 0
    rejected = queue_rejected = incomplete = 0
    occupied_rejected = same_window_rejected = released = 0
    requests_observed = occupied_requests = 0
    requests_over_limit_before = projected_requests_over_limit = 0
    outstanding_max_before = projected_outstanding_max = 0
    ...
    with gzip.open(log, "rt", encoding="utf-8") as stream:
        for line in stream:
            ...
            if kind not in counts:
                raise RuntimeError(f"unexpected V162 Nash observation kind: {kind}")
            counts[kind] += 1
            if kind == "run_config":
                ...
                if not (
                    ...
                    raise RuntimeError("V162 run_config contract changed")
            elif kind == "window":
                if event.get("frame") != counts["window"] - 1:
                    raise RuntimeError("V162 scheduler window sequence changed")
                ...
                if (
                    ...
                    raise RuntimeError("V162 decision frontier/hash changed")
                ...
                credit = frontier.get("one_outstanding_short_work_credit", {})
                requests_observed_now = _count(
                    credit.get("requests_observed"), "observed request count"
                )
                occupied_requests_now = _count(
                    credit.get("occupied_requests"), "occupied request count"
                )
                released_now = _count(
                    credit.get("released_requests"), "released request count"
                )
                outstanding_max_now = _count(
                    credit.get("outstanding_max_before"),
                    "outstanding maximum before dispatch",
                )
                over_limit_before_now = _count(
                    credit.get("requests_over_limit_before"),
                    "pre-dispatch credit-limit violation count",
                )
                credit_admitted_now = _count(
                    credit.get("admitted_new_players"), "credit admission count"
                )
                occupied_rejected_now = _count(
                    credit.get("rejected_while_occupied"),
                    "occupied-credit rejection count",
                )
                ...
                projected_over_limit_now = _count(
                    credit.get("projected_requests_over_limit"),
                    "projected credit-limit violation count",
                )
                if not (
                    ...
                    and credit.get("definition") == ONE_OUTSTANDING_DEFINITION
                    and credit.get("credit_limit_per_request") == 1
                    ...
                    and credit_admitted_now == short_now
                    and occupied_rejected_now + same_window_rejected_now <= rejected_now
                    and occupied_requests_now <= requests_observed_now
                    and released_now <= requests_observed_now
                    and outstanding_max_now <= 1
                    and over_limit_before_now == 0
                    and projected_max_now <= 1
                    and projected_over_limit_now == 0
                    ...
                    raise RuntimeError(
                        "V162 one-outstanding slack short-work evidence changed"
                    )
                terminal += terminal_now
                short += short_now
                credit_admitted += credit_admitted_now
                rejected += rejected_now
                queue_rejected += queue_rejected_now
                incomplete += incomplete_now
                occupied_rejected += occupied_rejected_now
                same_window_rejected += same_window_rejected_now
                released += released_now
                requests_observed += requests_observed_now
                occupied_requests += occupied_requests_now
                requests_over_limit_before += over_limit_before_now
                projected_requests_over_limit += projected_over_limit_now
                outstanding_max_before = max(
                    outstanding_max_before, outstanding_max_now
                )
                ...
                if not (
                    ...
                    raise RuntimeError("V162 route telemetry is incomplete")
                expected = (
                    LOW_EXPERT if float(density) < QUEUE_THRESHOLD else HIGH_EXPERT
                )
                if route.get("selected_expert") != expected:
                    raise RuntimeError("V162 route does not match queue density")
                ...
                if key is None:
                    if source != "not_requested":
                        raise RuntimeError("V162 unrequested reference reason changed")
                    reference_not_requested += 1
                elif source in ("offline_table", "offline_table_nonpositive"):
                    reference_available += 1
                else:
                    raise RuntimeError("V162 bound reference source changed")
                ...
                if not (
                    ...
                    raise RuntimeError("V162 Nash terminal marker changed")
    ...
    if (
        ...
        raise RuntimeError("V162 Nash log cardinality changed")
    if reference_available + reference_not_requested != counts["window"]:
        raise RuntimeError("V162 reference replay coverage changed")
    return {
        "run_id": run_id,
        "seed": run["seed"],
        ...
        "occupied_request_observations": occupied_requests,
...
def blind_audit_v162(root: Path = ROOT) -> dict[str, Any]:
    ...
    if output["blind"].exists():
        raise RuntimeError("V162 blind audit already exists")
    _assert_frozen_inputs()
    prepared = read_json(output["prepared"])
    prepared_hash = _assert_hashed(prepared, "receipt_hash", "V162 prepared receipt")
    execution = read_json(output["execution"])
    execution_hash = _assert_hashed(execution, "receipt_hash", "V162 execution receipt")
    manifest = load_and_validate_manifest(output["ready"])
    _validate_product(manifest, references_bound=True)
    ...
    if (
        ...
        raise RuntimeError("V162 exact pairing failed")
    ...
    if [item["seed"] for item in execution["dispatches"]] != list(SEEDS):
        raise RuntimeError("V162 execution order changed")
    ...
    if {path.name for path in canonical_root.iterdir() if path.is_dir()} != {
        ...
        raise RuntimeError("V162 canonical product changed")
    quarantine = output["workspace"] / "quarantine"
    if quarantine.exists() and any(quarantine.rglob("attempt-*")):
        raise RuntimeError("V162 has unexplained quarantined attempts")
    ...
    if len(identities) != 1:
        raise RuntimeError("V162 runtime identity is not unanimous")
...
def _load_candidate(
    ...
    if len(rows) != 3 or [row["seed"] for row in rows] != list(SEEDS):
        raise RuntimeError("V162 candidate result product changed")
    return rows


def reveal_v162(root: Path = ROOT) -> dict[str, Any]:
    output = paths(root)
    if output["result"].exists():
        raise RuntimeError("V162 result already exists")
    blind = read_json(output["blind"])
    blind_hash = _assert_hashed(blind, "blind_audit_hash", "V162 blind audit")
    if not (
        ...
        is True
        and blind.get("one_outstanding_credit_invariants_passed") is True
        and blind.get("work_and_queue_threshold_invariants_passed") is True
        and blind.get("both_routes_exercised") is True
    ):
        raise RuntimeError("V162 blind audit did not authorize reveal")
    manifest = load_and_validate_manifest(output["ready"])
    ...
    mechanism = {
        ...
        for key in (
            ...
            "admitted_one_outstanding_short_work_players",
            "rejected_while_credit_occupied",
            "rejected_same_window_not_selected",
            "released_request_credits",
            "requests_observed",
            "occupied_request_observations",
            "outstanding_max_before",
            "requests_over_limit_before",
    ...
    mechanism["pass"] = (
        ...
        and blind["one_outstanding_credit_invariants_passed"]
    ...
    document = {
        "schema_version": "NSE_E1_HOMOGENEOUS_ONE_OUTSTANDING_SLACK_SHORT_WORK_TERMINAL_PIPELINE_QUEUE8_LOW_DIAGNOSTIC_RESULT_V162_V1",
        "created_at": utc_now(),
        "training_only": True,
        "formal_results_eligible": False,
        ...
        "disposition": (
            ...
            if passed
            else "retain_all_three_valid_diagnostic_runs_and_retire_one_outstanding_candidate"
        ),
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    if action == "prepare":
        document, key = prepare_v162(), "receipt_hash"
    elif action == "execute":
        document, key = execute_v162(), "receipt_hash"
    elif action == "blind-audit":
        document, key = blind_audit_v162(), "blind_audit_hash"
    else:
        document, key = reveal_v162(), "result_hash"
