...
from scripts.reviewer_experiments.protocol.nse_e3_terminal_completion_training_blind_audit_v122 import (
    ARMS,
...
OUTPUT = ROOT / "training-result-v122.json"
BLIND_AUDIT_FILE_SHA256 = ""
BLIND_AUDIT_HASH = ""
EXPECTED_SEEDS = ("E1149", "E1150", "E1151")
EXPECTED_SCENARIOS = (
    "E3.spike5x50ms",
    "E3.sustained3x200ms",
    "E3.pulse4x4x50ms",
...
ANCHOR_ID = "v122-e3-anchor"
CANDIDATE_ID = (
    "v122-e3-causal-shock15-horizon50-admitted-work10-"
    "terminal-completion-only-initializer-only"
)
...
def _validate_blind_audit() -> dict[str, Any]:
    ...
    if any(
        ...
        raise RuntimeError("V122 blind audit has not been frozen into the reveal code")
    if not BLIND_AUDIT.is_file() or file_hash(BLIND_AUDIT) != BLIND_AUDIT_FILE_SHA256:
        raise RuntimeError("V122 blind audit is missing or changed")
    blind = read_json(BLIND_AUDIT)
    payload = dict(blind)
    claimed = payload.pop("audit_hash", None)
    coverage = blind.get("terminal_completion_mechanism_coverage", {})
    if (
        ...
        or coverage.get("terminal_player_count", 0) <= 0
        or coverage.get("nonterminal_player_count", 0) <= 0
        or coverage.get("terminal_service_choice_evaluated_count", 0) <= 0
        or coverage.get("terminal_substitution_count", 0) <= 0
        or coverage.get("nonterminal_exact_anchor_preserved_count", 0) <= 0
        or coverage.get("nonterminal_substitution_count") != 0
        or coverage.get("proxy_input_unavailable_count") != 0
        or coverage.get("performance_fields_consulted") is not False
    ):
        raise RuntimeError("V122 blind audit does not authorize reveal")
    return blind
...
def _load_rows(blind: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    if len(admitted) != 18:
        raise RuntimeError("V122 blind audit run evidence is incomplete")
    ...
    for arm_id, expected in ARMS.items():
        ...
        if not manifest_path.is_file() or file_hash(manifest_path) != frozen.get(
            ...
            raise RuntimeError(f"V122 frozen manifest changed: {arm_id}")
        manifest = load_and_validate_manifest(manifest_path)
        if manifest.get("manifest_hash") != frozen.get("manifest_hash"):
            raise RuntimeError(f"V122 manifest hash changed: {arm_id}")
        for run in manifest["runs"]:
            ...
            if (
                ...
                != expected["profile"]
                or metadata.get("v122_training_plan_sha256") != PLAN_SHA256
                or metadata.get("v122_training_only") is not True
                or metadata.get("v122_training_seed_metrics_previously_revealed")
                is not False
                or metadata.get("v122_confirmation_seeds_opened") is not False
                or metadata.get("v122_other_unopened_seeds_opened") is not False
                or metadata.get("v122_arm_id") != arm_id
                or metadata.get("v122_arm_role") != expected["role"]
                or metadata.get("v122_shock_rate_ratio") != expected["shock_rate_ratio"]
                or metadata.get("v122_shock_threshold_numerator")
                != expected["shock_threshold_numerator"]
                or metadata.get("v122_shock_threshold_denominator")
                != expected["shock_threshold_denominator"]
                or metadata.get("v122_shock_activation_horizon_frames")
                != expected["shock_activation_horizon_frames"]
                or metadata.get("v122_critical_service_ratio_numerator")
                != expected["critical_service_threshold_numerator"]
                or metadata.get("v122_critical_service_ratio_denominator")
                != expected["critical_service_threshold_denominator"]
                or metadata.get("v122_service_proxy_work_source")
                != (
                    "admitted_pending_plus_all_resident_remaining_and_same_window_projected_cpu_v1"
                    if candidate
                    else "not_applicable"
                )
                or metadata.get("v122_admitted_work_includes_all_blocked_resident")
                is not candidate
                or metadata.get("v122_terminal_completion_only_substitution_scope")
                is not candidate
                or metadata.get("v122_terminal_definition")
                != (
                    "function_children_is_empty_in_the_frozen_request_DAG"
                    if candidate
                    else "not_applicable"
                )
                or metadata.get("v122_terminal_critical_frontier_substitution")
                is not candidate
                or metadata.get("v122_nonterminal_exact_anchor") is not candidate
                or metadata.get("v122_outcome_fields_drive_policy") is not False
                or metadata.get("v122_scenario_or_burst_label_used_by_policy")
                is not False
                or metadata.get("v122_completion_or_performance_fields_used_by_policy")
                is not False
                or metadata.get("v122_future_arrivals_used_by_policy") is not False
            ):
                raise RuntimeError(f"V122 arm identity changed: {run['run_id']}")
            run_id = run["run_id"]
            summary_path = (
                ROOT
                / "runs"
            ...
            if not summary_path.is_file() or file_hash(summary_path) != admitted.get(
                ...
                raise RuntimeError(f"V122 summary differs from blind audit: {run_id}")
            ...
            if (
                ...
                raise RuntimeError(
                    f"V122 summary identity/completion mismatch: {run_id}"
                )
            rows.append(
                {
                    "arm_id": arm_id,
...
def execute_reveal() -> dict[str, Any]:
    if OUTPUT.exists():
        raise RuntimeError(f"V122 training result already exists: {OUTPUT}")
    if not PLAN.is_file() or file_hash(PLAN) != PLAN_SHA256:
        raise RuntimeError("frozen V122 plan changed before reveal")
    blind = _validate_blind_audit()
    rows = _load_rows(blind)
    if len(rows) != 18:
        raise RuntimeError("V122 revealed row count differs from sealed plan")
    ...
    result = {
        "schema_version": "NSE_E3_TERMINAL_COMPLETION_RESULT_V122_V1",
        "created_at": utc_now(),
        ...
        **evaluation,
        "revealed_rows": rows,
        "decision": {
            "authorize_preregistration_of_E1152_E1171_confirmation": joint_pass,
            ...
            "next_action": (
                "freeze V122 with frozen V97 E4 and separately preregister the "
                "complete E1152-E1171 NSESche confirmation before generating inputs"
                if joint_pass
                else (
                    "freeze the complete V122 training cohort, keep E1152-E1171 and "
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Reveal the frozen V122 training cohort exactly once after blind audit."
    )
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    if args.command != "reveal" or not args.execute:
        parser.error("V122 reveal requires the explicit command: reveal --execute")
    execute_reveal()
