...
ROOT = Path("tmp/nse_e3_causal_arrival_shock_training_20260829_v107")
PLAN = Path(
    "scripts/reviewer_experiments/protocol/"
    "nse_e3_causal_arrival_shock_training_plan_v107.json"
)
PLAN_SHA256 = "bdd5bb39d932ca5f9d2bbf9c78a151da94c068aacc7a49ec9450a8a3cac7f995"
V106_RESULT_RECEIPT = Path(
    "scripts/reviewer_experiments/protocol/"
    "nse_e3_resource_headroom_safety_training_result_v106.json"
)
V106_RESULT_RECEIPT_SHA256 = (
    "aad57d09ee2fcb6f29c547831df9ada0f9d2f4aa590cffa7282b5fd4ed25f362"
)
V106_SOURCE_RESULT = Path(
    "tmp/nse_e3_resource_headroom_safety_training_20260829_v106/"
    "training-result-v106.json"
)
V106_SOURCE_RESULT_SHA256 = (
    "0c8b917badef622429f19a8658ccfc8eb60d48105fef33102a569e3b173435f9"
)
FORMAL_RESULT = Path(
...
BINARY_PATH = Path("tmp/nse_v107_build_3ba68ca/release/serverless_sim.exe")
BINARY_SHA256 = "8b6edbe5d9fe9123cc411ab0417f83f4b7a20f0e2541350c85e2db19fe2f61f2"
BINARY_SOURCE_COMMIT = "3ba68ca6548ce078f52abcc9107e50f952e10ab1"
...
SOURCE_INITIAL_SEEDS = [f"E{index}" for index in range(890, 900)]
SOURCE_CI_SEEDS = [f"E{index}" for index in range(900, 910)]
TRAINING_SEED_LIST = ["E890", "E891", "E892"]
TRAINING_SEEDS = set(TRAINING_SEED_LIST)
CONFIRMATION_SEEDS = [f"E{index}" for index in range(1026, 1046)]
PREVIOUS_CONFIRMATION_SEEDS = [f"E{index}" for index in range(926, 1026)]
OTHER_UNOPENED_SEEDS = [
    ...
    *[f"E{index}" for index in range(893, 926)],
]
PORT = "3142"

E3_ANCHOR = "faasrank_native_faithful_terminal_ocs_srpt_ready_dual_window_safe_pareto"
E3_CAUSAL_ARRIVAL_SHOCK15_RESOURCE_PARETO_INITIALIZER_ONLY = (
    "faasrank_native_faithful_terminal_ocs_srpt_ready_"
    "causal_arrival_shock15_resource_pareto_initializer_only_guard64_dual_window_safe_pareto"
)
E3_CAUSAL_ARRIVAL_SHOCK20_RESOURCE_PARETO_INITIALIZER_ONLY = (
    "faasrank_native_faithful_terminal_ocs_srpt_ready_"
    "causal_arrival_shock20_resource_pareto_initializer_only_guard64_dual_window_safe_pareto"
)
ARMS = (
    ("v107-e3-anchor", "E3", "anchor", E3_ANCHOR, None, 9),
    (
...
SHOCK_THRESHOLDS = {"3/2": (3, 2), "2/1": (2, 1)}
...
def _paths(root: Path) -> dict[str, Path]:
    return {
        "config": root / "v107-e3-training-config.json",
        "source": root / "manifest.v107-e890-e899-full.unbound.json",
        "capture": root / "manifest.v107-tape-capture.unbound.json",
...
def _assert_frozen_inputs() -> None:
    for path, expected in (
        (PLAN, PLAN_SHA256),
        (V106_RESULT_RECEIPT, V106_RESULT_RECEIPT_SHA256),
        (V106_SOURCE_RESULT, V106_SOURCE_RESULT_SHA256),
        ...
        if not path.is_file() or file_hash(path) != expected:
            raise RuntimeError(f"frozen V107 input is missing or changed: {path}")
...
def _rewrite(
    ...
    *,
    ...
    marker.update(
        {
            "purpose": purpose,
            "v107_training_plan_sha256": PLAN_SHA256,
            "v107_training_only": True,
            "v107_binary_source_commit": BINARY_SOURCE_COMMIT,
    ...
    if arm is not None:
        ...
        marker.update(
            {
                "v107_arm_id": arm_id,
                "v107_experiment_id": experiment_id,
                "v107_role": role,
                "v107_profile": profile,
                "v107_shock_rate_ratio": shock_rate_ratio,
                "v107_shock_threshold_numerator": (
                    shock_threshold[0] if shock_threshold else None
                ),
                "v107_shock_threshold_denominator": (
                    shock_threshold[1] if shock_threshold else None
                ),
                "v107_arrival_history_baseline_frames": 80
                if role == "candidate"
                else None,
                "v107_arrival_history_recent_frames": 20
                if role == "candidate"
                else None,
                "v107_arrival_min_requests_per_window": 20
                if role == "candidate"
                else None,
                "v107_shock_activation_horizon_frames": 100
                if role == "candidate"
                else None,
                "v107_static_upper_density_gate_bypassed_only_while_shock_active": role
                == "candidate",
                "v107_nonterminal_queue_density_floor": (
                    8.0 if role == "candidate" else None
                ),
                "v107_warm_admissibility": (
                    "preserve_anchor_warmness" if role == "candidate" else None
                ),
                "v107_load_least_window_certificate_mode": (
                    "disabled" if role == "candidate" else "not_applicable"
                ),
                "v107_arrival_signal": "first_seen_request_ids_only"
                if role == "candidate"
                else "not_applicable",
                "v107_cpu_memory_individual_noninferiority": role == "candidate",
                "v107_resource_bottleneck_sum_noninferiority": False,
                "v107_resource_inputs_finite_fail_closed": role == "candidate",
                "v107_critical_frontier_protection": role == "candidate",
                "v107_critical_frontier_rank_source": (
                    "immutable_srpt_remaining_critical_path_rank"
                    if role == "candidate"
                    else "not_applicable"
                ),
                "v107_critical_frontier_tie_rule": (
                    "protect_missing_nonfinite_singleton_or_rank_plus_epsilon_ge_request_frontier_maximum"
                    if role == "candidate"
                    else "not_applicable"
                ),
                "v107_expected_run_count": expected,
                "v107_scenario_or_burst_label_used_by_policy": False,
                "v107_completion_or_performance_fields_used_by_policy": False,
                "v107_future_arrivals_used_by_policy": False,
            }
        )
        ...
        metadata.update(
            {
                "v107_training_plan_sha256": PLAN_SHA256,
                "v107_training_only": True,
                "v107_training_seed_metrics_previously_revealed": False,
                "v107_binary_source_commit": BINARY_SOURCE_COMMIT,
                "v107_confirmation_seeds_opened": False,
                "v107_other_unopened_seeds_opened": False,
                "v107_formal_E01_E20_reexecution": False,
            }
        )
        if arm is not None:
            (
            ...
            metadata.update(
                {
                    "v107_arm_id": arm_id,
                    "v107_arm_role": role,
                    "v107_candidate_profile": profile,
                    "v107_candidate_experiment": experiment_id,
                    "v107_shock_rate_ratio": shock_rate_ratio,
                    "v107_shock_threshold_numerator": (
                        shock_threshold[0] if shock_threshold else None
                    ),
                    ...
                    "v107_faasrank_model_artifact_sha256": MODEL_SHA256,
                    "v107_native_faithful_initializer": True,
                    "v107_dual_window_safe_pareto": True,
                    "v107_causal_arrival_shock_initializer_guard": role == "candidate",
                    "v107_terminal_players_included_without_lower_floor": role
                    == "candidate",
                    "v107_srpt_critical_path_player_order": "srpt_ready" in profile,
                    "v107_substitution_cap": None,
                    "v107_load_least_window_certificate_mode": certificate_mode,
                    "v107_arrival_signal": "first_seen_request_ids_only"
                    if role == "candidate"
                    else "not_applicable",
                    "v107_cpu_memory_individual_noninferiority": role == "candidate",
                    "v107_resource_bottleneck_sum_noninferiority": False,
                    "v107_resource_inputs_finite_fail_closed": role == "candidate",
                    "v107_scalar_faasrank_noninferiority": role == "candidate",
                    "v107_input_locality_component_noninferiority": role == "candidate",
                    "v107_componentwise_faasrank_noninferiority": False,
                    "v107_per_child_current_warm_downstream_locality_noninferiority": role
                    == "candidate",
                    "v107_downstream_locality_aggregate_compensation_allowed": False,
                    "v107_future_child_placement_or_feasibility_used": False,
                    "v107_critical_frontier_protection": role == "candidate",
                    "v107_critical_frontier_rank_source": (
                        "immutable_srpt_remaining_critical_path_rank"
                        if role == "candidate"
                    ...
                    "v107_only_strictly_lower_rank_parallel_players_may_substitute": role
                    == "candidate",
                    "v107_outcome_fields_drive_policy": False,
                    "v107_scenario_or_burst_label_used_by_policy": False,
                    "v107_completion_or_performance_fields_used_by_policy": False,
                    "v107_future_arrivals_used_by_policy": False,
                }
...
def prepare_v107(root: Path = ROOT) -> dict:
    _assert_frozen_inputs()
    if root.exists():
        raise RuntimeError(f"refusing to overwrite V107 training root: {root}")
    ...
    capture = derive_integration_smoke_shard(
        paths["source"],
        capture_ids,
        purpose=f"V107 fresh balanced-QoS parent tapes; plan_sha256={PLAN_SHA256}",
    )
    _write(
        paths["capture"],
        _rewrite(
    ...
    for arm in ARMS:
        ...
        template = derive_integration_smoke_shard(
            paths["source"],
            selected,
            purpose=f"V107 {arm_id} causal-arrival-shock training; plan_sha256={PLAN_SHA256}",
        )
        manifest = _rewrite(
            template,
            purpose=f"V107 {arm_id} causal-arrival-shock training; plan_sha256={PLAN_SHA256}",
        ...
        if (
            ...
            raise RuntimeError(f"V107 {arm_id} must have {expected} runs/references")
        if {run["seed"] for run in manifest["runs"]} != TRAINING_SEEDS:
            raise RuntimeError(f"V107 {arm_id} escaped E890-E892")
        keys = {item["key"] for item in manifest["reference_build_dependencies"]}
        if all_reference_keys & keys:
            raise RuntimeError("V107 arm-specific reference keys unexpectedly overlap")
        all_reference_keys.update(keys)
        _write(arm_path(root, arm_id), manifest)
    if len(all_reference_keys) != 27:
        raise RuntimeError("V107 must declare exactly 27 arm-specific references")
    ...
    receipt = {
        "schema_version": "NSE_E3_CAUSAL_ARRIVAL_SHOCK_PREPARED_V107_V1",
        "created_at": utc_now(),
        "formal_results_eligible": False,
        ...
        "V106_result_receipt_path": str(V106_RESULT_RECEIPT),
        "V106_result_receipt_sha256": V106_RESULT_RECEIPT_SHA256,
        "V106_source_result_path": str(V106_SOURCE_RESULT),
        "V106_source_result_sha256": V106_SOURCE_RESULT_SHA256,
