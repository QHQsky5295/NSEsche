"""Frozen zero-result D106--D110 G14 deferral release-valve protocol."""
...
G14_CONTROL = "ready_order"
G14_CANDIDATE = "ready_global_deferral_release_valve"
G14_EFFECTIVE_METHODS = (G14_CONTROL, G14_CANDIDATE)
G14_MANIFEST_SCHEMA = "NSE_G14_DEFERRAL_RELEASE_VALVE_DEVELOPMENT_V1"
G14_REFERENCE_KEY_TAGS = {G14_CONTROL: 1, G14_CANDIDATE: 17}


def _nash_cell(candidate: str, load: str, node_count: int) -> dict[str, Any]:
    return _make_cell(
        "E1",
        f"G14.sche_nash.{candidate}.{load}.homogeneous.n{node_count}",
        "sche_nash",
        ...
        metadata={
            ...
            "g14_role": (
                "deferral_release_valve_candidate"
                if candidate == G14_CANDIDATE
                else "strict_ready_order_control"
            ),
            "paper_equations_changed": False,
            "new_compound_method": candidate == G14_CANDIDATE,
            "strict_best_response": True,
            "utility_guard_relative_regret": 0.0,
            "reference_key_tag": G14_REFERENCE_KEY_TAGS[candidate],
        },
    )
...
def build_g14_deferral_release_valve_manifest(
    simulator_exe: Path,
    source_git_commit: str,
    config_path: Path | None = None,
) -> dict[str, Any]:
    """Build the exact zero-result 30-run G14 candidate/control product."""

    config = load_protocol_config(config_path)
    node_count = int(config["matrix_defaults"]["base_node_count"])
    if node_count != 20:
        raise ProtocolValidationError("G14 requires the frozen 20-node base cluster")
    ...
    cells = [
        ...
        for load, candidate in product(FORMAL_E1_LOADS, G14_EFFECTIVE_METHODS)
    ]
    runs = []
    for cell, seed in product(cells, G14_DEFERRAL_RELEASE_VALVE_SEEDS):
    ...
    marker = {
        "schema_version": G14_MANIFEST_SCHEMA,
        "purpose": "three-load one-bit deferral release-valve control gate",
        "control": G14_CONTROL,
        "candidate": G14_CANDIDATE,
        "loads": list(FORMAL_E1_LOADS),
        "topology": "homogeneous",
        "node_count": node_count,
        "development_seeds": list(G14_DEFERRAL_RELEASE_VALVE_SEEDS),
        "paper_equations_changed": False,
        "strict_eq15_required": True,
        "operational_refinement_schema_version": 11,
        "reference_key_schema_version": 12,
        "reference_key_tags": copy.deepcopy(G14_REFERENCE_KEY_TAGS),
    ...
    manifest: dict[str, Any] = {
        ...
        "created_at": utc_now(),
        "phase": "development",
        "bank_id": "TSCv1.development.G14.deferral-release-valve.D106-D110",
        "formal_results_eligible": False,
        "fixed_seed_bank": {
            "policy": G14_DEFERRAL_RELEASE_VALVE_SAMPLE_POLICY,
            "all_seeds": list(G14_DEFERRAL_RELEASE_VALVE_SEEDS),
            "selected_seeds": list(G14_DEFERRAL_RELEASE_VALVE_SEEDS),
        ...
        "reuse_analyses": [],
        "g14_deferral_release_valve_development": marker,
    }
...
def write_g14_deferral_release_valve_manifest(
    output_path: Path,
    simulator_exe: Path,
    source_git_commit: str,
    config_path: Path | None = None,
    ...
    if output_path.exists():
        raise ProtocolValidationError("refusing to overwrite G14 manifest")
    manifest = build_g14_deferral_release_valve_manifest(
...
def main() -> None:
    ...
    write_g14_deferral_release_valve_manifest(
