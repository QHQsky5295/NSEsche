...
ROOT = Path(
    "tmp/nse_e1_homogeneous_20node_low_cpu_work_shrink_ocs_training_20260901_v186"
)
PLAN = Path(
    "scripts/reviewer_experiments/protocol/"
    "nse_e1_homogeneous_20node_low_cpu_work_shrink_ocs_training_plan_v186.json"
)
PLAN_SHA256 = "a61f2801a866727923a0d281ebd2b8f051457707c54ee3367b08666c8e64be58"
PLAN_HASH = "e1950cb52384dea217b0aa6b96a015e4b217303f5a7e4983061398c6d3eeaf77"
PLAN_COMMIT = "4b1308c9ea225ad9ed9ce45b1dc21762ce23b094"
IMPLEMENTATION_RECEIPT = Path(
    "scripts/reviewer_experiments/protocol/"
    "nse_e1_homogeneous_20node_low_cpu_work_shrink_ocs_training_implementation_v186.json"
)
GOAL = v184.GOAL
...
TEST = Path(
    ...
    "test_nse_e1_low_cpu_work_shrink_ocs_training_v186.py"
)
...
PROFILE = (
    ...
    "pipeline_hiku2_ocs_work_shrink_queue8"
)
FRONTIER = v184.FRONTIER
PORT = "3212"
BINARY_PATH = Path("serverless_sim/target_e1_v186/release/serverless_sim.exe")
BINARY_SHA256 = "24dad33e5b36b08cd87d8fb32465437d1f792611261cf1843b27224c7af034d7"
BINARY_BYTES = 5_897_216
BINARY_SOURCE_COMMIT = "e2cadeca4c93825365da93b7dd7b67077f7e8a80"
SCHEDULER_SOURCE = Path("serverless_sim/src/sche/sche_nash.rs")
SCHEDULER_SOURCE_SHA256 = (
    "043f1ab59f5c4b037c4b07a8825af4e8442b47e6ccf5da762c309d14c87bd1ca"
)
...
def paths(root: Path = ROOT) -> dict[str, Path]:
    return {
        ...
        "prepared": root / "prepared-v186.json",
        ...
        "reference_execution": root / "reference-execution-receipt-v186.json",
        "workspace": root / "formal-runs",
        "execution": root / "execution-receipt-v186.json",
        "inventory": root / "candidate-inventory-audit-v186.json",
        "blind": root / "joint-blind-audit-v186.json",
        "result": root / "training-result-v186.json",
    }
...
def _assert_source_evidence() -> tuple[dict[str, Any], dict[str, Any]]:
    for path, expected, label in (
        ...
        (BINARY_PATH, BINARY_SHA256, "V186 binary"),
        (SCHEDULER_SOURCE, SCHEDULER_SOURCE_SHA256, "V186 scheduler source"),
        (PYTHON_PATH, PYTHON_SHA256, "Python executable"),
        (CARGO_LOCK, CARGO_LOCK_SHA256, "Cargo.lock"),
    ):
        _assert_file(path, expected, label)
    _require(BINARY_PATH.stat().st_size == BINARY_BYTES, "V186 binary size changed")
    v184.v182.v181._assert_json_semantic(
        MODULE_CONF, MODULE_CONF_SEMANTIC_HASH, "module_conf_es.json"
    )
    plan = read_json(PLAN)
    _require(
        _assert_hashed(plan, "plan_hash", "V186 plan") == PLAN_HASH
...
def _assert_frozen_inputs() -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    _require(IMPLEMENTATION_RECEIPT.is_file(), "V186 implementation receipt missing")
    implementation = read_json(IMPLEMENTATION_RECEIPT)
    _assert_hashed(implementation, "receipt_hash", "V186 implementation receipt")
    _require(
        ...
        "V186 implementation receipt does not bind this harness",
    )
    return plan, result
...
def _validate_product(manifest: Mapping[str, Any], *, references_bound: bool) -> None:
    ...
    for run in runs:
        ...
        _require(
            ...
            f"V186 reference binding changed: {run.get('run_id')}",
        )
    _require(len(tape_hashes) == 20, "V186 must reuse 20 distinct V184 tapes")
...
def _rewrite_training(source: Mapping[str, Any]) -> dict[str, Any]:
    ...
    for seed in SEEDS:
        ...
        run["variant"] = "homogeneous-20node-low-v186-cpu-work-shrink-training"
        ...
        metadata.update(
            {
                "v186_training_only": True,
                "v186_plan_sha256": PLAN_SHA256,
                "v186_plan_commit": PLAN_COMMIT,
                "v186_profile": PROFILE,
                "v186_player_frontier": FRONTIER,
                "v186_binary_source_commit": BINARY_SOURCE_COMMIT,
                "v186_binary_sha256": BINARY_SHA256,
                "v186_source_v184_run_id": source_run_id,
                "v186_source_v184_run_spec_hash": source_spec_hash,
                "v186_source_v184_result_hash": SOURCE_RESULT_HASH,
                "v186_same_tape_control": True,
                "v186_baseline_online_runs": 0,
                "v186_performance_fields_parsed_before_run": 0,
                "v186_seed_deletion_replacement_or_selective_rerun": False,
            }
        )
        ...
        lineage.append(
            {
                ...
                "v186_run_id": run["run_id"],
                "v186_run_spec_hash": run["run_spec_hash"],
    ...
    marker["purpose"] = (
        "V186 same-tape mechanism training against the complete frozen V184 control; "
    ...
    marker.update(
        {
            "v186_training_only": True,
            "v186_plan_sha256": PLAN_SHA256,
            "v186_selected_seeds": list(SEEDS),
            "v186_fixed_run_order": [
                {"method": "sche_nash", "seed": seed} for seed in SEEDS
            ],
            "v186_source_to_training_lineage": lineage,
            "v186_expected_reused_base_tapes": 20,
            "v186_expected_references": 20,
            "v186_expected_online_runs": 20,
            "v186_expected_baseline_online_runs": 0,
...
def prepare_v186(root: Path = ROOT) -> dict[str, Any]:
    _assert_frozen_inputs()
    _require(not root.exists(), "V186 root already exists")
    ...
    for run in manifest["runs"]:
        ...
        _require(
            ...
            f"V186 tape binding differs from V184: {run['seed']}",
        )
    ...
    receipt = {
        "schema_version": "NSE_E1_LOW_CPU_WORK_SHRINK_OCS_PREPARED_V186_V1",
        "created_at": utc_now(),
        "training_only": True,
...
def build_references_v186(root: Path = ROOT) -> dict[str, Any]:
    _assert_frozen_inputs()
    output = paths(root)
    ...
    _require(len(catalog.get("entries", {})) == 20, "V186 reference catalog is not 20")
    count, last_hash = verify_ledger(
        output["reference_workspace"] / "reference_builds" / "ledger.jsonl"
    )
    receipt = {
        "schema_version": "NSE_E1_LOW_CPU_WORK_SHRINK_OCS_REFERENCES_V186_V1",
        "created_at": utc_now(),
...
def execute_v186(root: Path = ROOT) -> dict[str, Any]:
    _assert_frozen_inputs()
    output = paths(root)
    ...
    for ordinal, run in enumerate(manifest["runs"], start=1):
        ...
        _require(
            ...
            f"V186 run is not attempt-one QC pass: {run['run_id']}",
        )
        dispatches.append(
            {
                "ordinal": ordinal,
    ...
    receipt = {
        "schema_version": "NSE_E1_LOW_CPU_WORK_SHRINK_OCS_EXECUTION_V186_V1",
        "created_at": utc_now(),
...
def _runtime_identity_v186(audits: Sequence[Mapping[str, Any]]) -> dict[str, Any]:
    identities = {
        (
    ...
    _require(len(identities) == 1, "V186 runtime identity is not unanimous")
...
def blind_audit_v186(root: Path = ROOT) -> dict[str, Any]:
    _assert_frozen_inputs()
    output = paths(root)
    _require(not output["blind"].exists(), "V186 blind audit already exists")
    _require(not output["result"].exists(), "V186 result exists before blind audit")
    manifest = load_and_validate_manifest(output["ready"])
    _validate_product(manifest, references_bound=True)
    ...
    prepared_hash = _assert_hashed(prepared, "receipt_hash", "V186 prepared receipt")
    reference_hash = _assert_hashed(
        reference_execution, "receipt_hash", "V186 reference receipt"
    )
    execution_hash = _assert_hashed(execution, "receipt_hash", "V186 execution receipt")
    _require(
        ...
        "V186 result-blind execution receipts changed",
    )
    ...
    for run in manifest["runs"]:
        ...
        _require(
            ...
            and qc.get("passed") is True
            and qc.get("classification") == "qc_pass",
            f"V186 non-clean canonical: {run['run_id']}",
        )
        audits.append(audit)
        run_evidence.append(
            {
    ...
    inventory = {
        "schema_version": "NSE_E1_LOW_CPU_WORK_SHRINK_OCS_INVENTORY_V186_V1",
        "created_at": utc_now(),
        "status": "pass",
        "method": "sche_nash",
        "seeds": list(SEEDS),
    ...
    document = {
        "schema_version": "NSE_E1_LOW_CPU_WORK_SHRINK_OCS_BLIND_AUDIT_V186_V1",
        "created_at": utc_now(),
        "status": "pass",
        ...
        "runtime_identity": _runtime_identity_v186(audits),
        "ready_manifest": {
...
def _evaluate_training(
    ...
    _require(
        ...
        "V186 rows are not the exact paired 20-seed product",
    )
    gates = {}
    for index, metric in enumerate(METRICS):
        ...
        _require(
            ...
            f"V186 {metric} contains nonfinite values",
        )
        ...
        gates[metric] = {
            ...
            "paired_difference_BCa_95_percent_interval": bca_interval(
                differences, n_resamples=10_000, seed=186_000 + index
            ),
            "two_sided_paired_permutation": _paired_permutation(
                differences, seed=186_100 + index
            ),
...
def reveal_v186(root: Path = ROOT) -> dict[str, Any]:
    output = paths(root)
    _require(not output["result"].exists(), "V186 result already exists")
    blind = read_json(output["blind"])
    blind_hash = _assert_hashed(blind, "blind_audit_hash", "V186 blind audit")
    _require(
        ...
        "V186 blind audit does not authorize reveal",
    )
    _reread_goal()
    manifest = load_and_validate_manifest(output["ready"])
    _validate_product(manifest, references_bound=True)
    ...
    document = {
        "schema_version": "NSE_E1_LOW_CPU_WORK_SHRINK_OCS_TRAINING_RESULT_V186_V1",
        "created_at": utc_now(),
        ...
        "decision": (
            "freeze_V186_training_winner_and_preregister_disjoint_low_confirmation"
            if passed
            else "retain_complete_V186_failure_close_axis_and_do_not_advance_to_middle"
        ),
    }
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    functions = {
        "prepare": prepare_v186,
        "build-references": build_references_v186,
        "execute": execute_v186,
        "blind-audit": blind_audit_v186,
        "reveal": reveal_v186,
    }
