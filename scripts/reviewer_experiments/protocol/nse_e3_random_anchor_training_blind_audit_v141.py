...
from scripts.reviewer_experiments.protocol.nse_e3_random_anchor_training_execute_v141 import (
    EXECUTION_RECEIPT,
    READY_SCHEDULE,
    ready_manifest_path,
    workspace_path,
)
from scripts.reviewer_experiments.protocol.nse_e3_random_anchor_training_prepare_v141 import (
...
OUTPUT = ROOT / "joint-blind-audit-v141-training.json"
RESULT = ROOT / "training-result-v141.json"
...
def _verify_tapes() -> tuple[list[dict[str, Any]], dict[str, Any]]:
    _require(TAPES.is_file(), f"missing V141 tape catalog: {TAPES}")
    catalog = read_json(TAPES)
    catalog_hash = _assert_hashed_object(catalog, "catalog_hash", "V141 tapes")
    entries = catalog.get("entries")
    _require(
        isinstance(entries, dict) and len(entries) == 12, "V141 tape count changed"
    )
    evidence = []
    for key, entry in sorted(entries.items()):
        info = inspect_tape(Path(entry["path"]))
        _require(info.sha256 == entry["sha256"], f"V141 tape hash changed: {key}")
        _require(info.workload_seed in TRAINING_SEEDS, f"V141 tape seed changed: {key}")
        evidence.append(
            {
                "key": key,
                "sha256": info.sha256,
    ...
    _require(
        ...
        f"V141 capture ledger changed: {counts}",
    )
    _require(
        not list((capture_root / "quarantine").glob("**/attempt-*")),
        "V141 tape capture quarantine is nonempty",
    )
    return evidence, {
...
def _verify_references() -> tuple[list[dict[str, Any]], dict[str, str]]:
    ...
    for arm_id in ARM_IDS:
        ...
        _require(path.is_file(), f"missing V141 reference catalog: {arm_id}")
        catalog = read_json(path)
        catalog_hash = _assert_hashed_object(
            catalog, "catalog_hash", f"V141 references {arm_id}"
        )
        entries = catalog.get("entries")
        _require(
            isinstance(entries, dict) and len(entries) == 9,
            f"V141 reference count changed: {arm_id}",
        )
        root = _stage_root_from_receipts(
            entries, "receipt_path", 9, f"V141 references {arm_id}"
        )
        rows, last_hash = _read_ledger(root / "ledger.jsonl")
        counts = Counter(row["event_type"] for row in rows)
        _require(
            counts == Counter({"reference_build_canonicalized": 9}),
            f"V141 reference ledger changed: {arm_id}/{counts}",
        )
        _require(
            not list((root / "quarantine").glob("**/attempt-*")),
            f"V141 reference quarantine is nonempty: {arm_id}",
        )
        ...
        for key, entry in sorted(entries.items()):
            ...
            _require(
                ...
                f"V141 reference evidence changed: {key}",
            )
            evidence.append(
                {
                    "arm_id": arm_id,
    ...
    _require(len(evidence) == 27, "V141 reference evidence count changed")
...
def _expected_portfolio_selection(
    ...
    _require(
        ...
        "V141 Random-default portfolio order changed",
    )
    random = candidates[0]
    epsilon = 1.0e-6
    if rule == "random_default_service_pareto":
        admissible = [
            index
    ...
    _require(
        rule == "random_default_welfare_pareto",
        f"unknown V141 portfolio rule: {rule}",
    )
    admissible = [
        index
        for index in range(1, len(candidates))
...
def _validate_native_diagnostics(
    ...
    _require(path.is_file(), f"missing V141 Nash diagnostics: {run['run_id']}")
    counts = Counter()
    reasons = Counter()
    ...
    with gzip.open(path, "rt", encoding="utf-8") as handle:
        for line_number, line in enumerate(handle, start=1):
            ...
            _require(isinstance(decision, dict), "missing V141 decision diagnostics")
            native = decision.get("native_shadow_anchor")
            _require(isinstance(native, dict), "missing V141 native shadow diagnostics")
            portfolio = decision.get("native_portfolio")
            _require(isinstance(portfolio, dict), "missing V141 portfolio diagnostics")
            players = decision.get("request_function_players")
            _require(
                type(players) is int and players >= 0,
                f"invalid V141 player count: {run['run_id']}:{line_number}",
            )
            _require(
                portfolio.get("certificate_uses_completion_outcomes") is False,
                f"V141 portfolio boundary changed: {run['run_id']}:{line_number}",
            )
            _require(
                ...
                f"V141 portfolio certificate definition changed: {run['run_id']}:{line_number}",
            )
            _require(
                ...
                f"V141 configured bandwidth snapshot changed: {run['run_id']}:{line_number}",
            )
            _require(
                ...
                and portfolio.get("random_shadow_lifecycle")
                == "one_persistent_RandomScheduler_per_algorithm_seed_advanced_once_per_scheduling_window",
                f"V141 Random shadow lifecycle changed: {run['run_id']}:{line_number}",
            )
            ...
            if players > 0:
                ...
                if portfolio_enabled:
                    _require(
                        ...
                        f"V141 active portfolio boundary changed: {run['run_id']}:{line_number}",
                    )
                    ...
                    _require(
                        ...
                        f"V141 portfolio members changed: {run['run_id']}:{line_number}",
                    )
                    for index, candidate in enumerate(candidates):
                        _require(
                            ...
                            f"V141 portfolio candidate invalid: {run['run_id']}:{line_number}",
                        )
                        random = candidates[0]
                        expected_predicates = {
                        ...
                        _require(
                            ...
                            f"V141 Random-relative predicates changed: {run['run_id']}:{line_number}:{index}",
                        )
                    ...
                    for candidate, expected_ranks in zip(candidates, ranks):
                        _require(
                            ...
                            f"V141 portfolio ranks changed: {run['run_id']}:{line_number}",
                        )
                    ...
                    _require(
                        ...
                        f"V141 portfolio selection changed: {run['run_id']}:{line_number}",
                    )
                    ...
                    _require(
                        ...
                        f"V141 exact Random anchor unexpectedly used a portfolio: {run['run_id']}:{line_number}",
                    )
                ...
                _require(
                    ...
                    f"V141 selected native mismatch: {run['run_id']}:{line_number}",
                )
            _require(
                native.get("certificate_uses_completion_outcomes") is False,
                f"V141 native certificate consulted outcomes: {run['run_id']}",
            )
            ...
            _require(
                ...
                f"V141 native service cohort count invalid: {run['run_id']}:{line_number}",
            )
            guard = decision.get("window_safe_guard")
            _require(isinstance(guard, dict), "missing V141 native guard diagnostics")
            reason = guard.get("reason")
            _require(isinstance(reason, str), "invalid V141 native guard reason")
            if players == 0:
                _require(
                    portfolio.get("enabled") is False
                    and portfolio.get("rule") is None
                    ...
                    f"V141 empty-window diagnostics mismatch: {run['run_id']}:{line_number}",
                )
                reasons[reason] += 1
                continue
            ...
            if initializer_complete and proposal_complete:
                _require(
                    ...
                    f"V141 native service cohort mismatch: {run['run_id']}:{line_number}",
                )
                ...
                _require(
                    ...
                    f"V141 native service certificate incomplete: {run['run_id']}:{line_number}",
                )
                ...
                _require(
                    ...
                    f"V141 native service deltas changed: {run['run_id']}:{line_number}",
                )
            else:
                _require(
                    guard.get("accepted") is False,
                    f"V141 unavailable native service certificate was accepted: {run['run_id']}:{line_number}",
                )
                if not initializer_complete:
                    _require(
                        initializer_players == 0
                        and reason == "initializer_readiness_service_unavailable",
                        f"V141 initializer service fallback mismatch: {run['run_id']}:{line_number}",
                    )
                else:
                    _require(
                        ...
                        f"V141 proposal service fallback mismatch: {run['run_id']}:{line_number}",
                    )
            ...
            _require(
                ...
                == float(welfare_delta),
                f"V141 native welfare certificate changed: {run['run_id']}:{line_number}",
            )
            ...
            _require(
                ...
                f"V141 native guard decision changed: {run['run_id']}:{line_number}",
            )
            reasons[reason] += 1
            if guard.get("accepted") is True:
                ...
                _require(
                    ...
                    f"V141 native accepted window violated certificate: {run['run_id']}:{line_number}",
                )
    _require(counts["windows"] == 4000, f"V141 window count changed: {run['run_id']}")
    _require(
        counts["player_windows"] > 0,
        f"V141 has no native player windows: {run['run_id']}",
    )
    return {
...
def _validate_execution_receipt() -> dict[str, Any]:
    ...
    _require(ready_schedule_path.is_file(), "missing V141 ready schedule")
    _require(execution_path.is_file(), "missing V141 execution receipt")
    schedule = read_json(ready_schedule_path)
    schedule_hash = _assert_hashed_object(
        schedule, "schedule_hash", "V141 ready schedule"
    )
    receipt = read_json(execution_path)
    receipt_hash = _assert_hashed_object(
        receipt, "receipt_hash", "V141 execution receipt"
    )
    _require(
        ...
        "V141 execution receipt boundary changed",
    )
    for scheduled, dispatched in zip(schedule["schedule"], receipt["dispatches"]):
        ...
        _require(
            ...
            f"V141 frozen dispatch changed: {scheduled['ordinal']}",
        )
    return {
...
def run_blind_audit(output: Path = OUTPUT) -> dict[str, Any]:
    _require(not output.exists(), f"V141 blind audit already exists: {output}")
    _require(not RESULT.exists(), "V141 reveal exists before blind audit")
    _require(PLAN.is_file() and file_hash(PLAN) == PLAN_SHA256, "V141 plan changed")
    _require(PREPARED.is_file(), "missing V141 prepared receipt")
    prepared = read_json(PREPARED)
    prepared_hash = _assert_hashed_object(
        prepared, "receipt_hash", "V141 prepared receipt"
    )
    _require(
        ...
        and prepared.get("v140_disposition")
        == "complete_training_family_falsified_no_candidate_selected"
        and prepared.get("v140_performance_summaries_parsed_before_blind_audit") == 0
        and prepared.get("v140_performance_results_consulted_after_reveal") is True
        and prepared.get("total_online_runs") == 108
        and prepared.get("candidate_reference_builds") == 27,
        "V141 prepared scientific boundary changed",
    )
    _require(
        ...
        "V141 prepared treatment boundary changed",
    )
    ...
    for manifest_id, expected_count, arm in products:
        ...
        _require(
            ...
            f"V141 ready boundary changed: {manifest_id}",
        )
        ...
        _require(
            ...
            f"V141 pairing changed: {manifest_id}",
        )
        ...
        _require(
            actual_ids == expected_ids, f"V141 canonical set changed: {manifest_id}"
        )
        _require(
            not list((workspace / "quarantine").glob("**/attempt-*")),
            f"V141 online quarantine is nonempty: {manifest_id}",
        )
        ...
        _require(
            ...
            f"V141 online ledger changed: {manifest_id}/{counts}",
        )
        for run in manifest["runs"]:
            if candidate:
                _require(
                    ...
                    == arm["profile"]
                    and run.get("metadata", {}).get("v141_native_selection_rule")
                    == arm["selection_rule"]
                    and run.get("metadata", {}).get("v141_native_portfolio_enabled")
                    is (arm["selection_rule"] != "exact_random_anchor")
                    and run.get("metadata", {}).get("v141_native_portfolio_members")
                    == native_members(arm["selection_rule"])
                    and run.get("metadata", {}).get("v141_random_default_required")
                    is True
                    and run.get("metadata", {}).get(
                        "v141_all_native_members_use_all_frontier"
                    )
                    is True
                    and run.get("metadata", {}).get(
                        "v141_service_certificate_state_domain"
                    )
                    == "runtime_existing_aggregates_and_admitted_work"
                    and run.get("metadata", {}).get("v141_paper_welfare_state_domain")
                    == "empty_window_aggregates"
                    and run.get("metadata", {}).get(
                        "v141_selected_welfare_equals_guard_initializer_required"
                    )
                    is True
                    and run.get("metadata", {}).get("v141_outcome_fields_drive_policy")
                    is False
                    and isinstance(run.get("reference_dependency"), dict),
                    f"V141 candidate manifest boundary changed: {run['run_id']}",
                )
            else:
                _require(
                    run.get("method") in BASELINE_METHODS
                    and run.get("reference_dependency") is None,
                    f"V141 baseline manifest boundary changed: {run['run_id']}",
                )
            ...
            _require(
                ...
                f"V141 canonical status changed: {run['run_id']}",
            )
    ...
    _require(len(run_evidence) == 108, "V141 run evidence count changed")
    for field, expected in EXPECTED_RUNTIME.items():
        _require(
            runtime_values[field] == {expected},
            f"V141 runtime {field} changed: {runtime_values[field]}",
        )
    ...
    _require(
        ...
        f"V141 runtime git identity changed: {git_commits}",
    )
    _require(len(paired_inputs) == 9, "V141 paired block count changed")
    for (scenario, seed), rows in paired_inputs.items():
        _require(
            len(rows) == 12
            and {row["method_label"] for row in rows} == set(METHOD_LABELS),
            f"V141 block product changed: {scenario}/{seed}",
        )
        for field in (
            ...
            _require(
                ...
                f"V141 paired {field} changed: {scenario}/{seed}",
            )
        _require(
            rows[0]["common_hpa_hash"] == EXPECTED_COMMON_HPA_SHA256,
            f"V141 common HPA changed: {scenario}/{seed}",
        )

    payload = {
        "schema_version": "NSE_E3_RANDOM_ANCHOR_BLIND_AUDIT_V141_V1",
        "created_at": utc_now(),
        "status": "pass",
