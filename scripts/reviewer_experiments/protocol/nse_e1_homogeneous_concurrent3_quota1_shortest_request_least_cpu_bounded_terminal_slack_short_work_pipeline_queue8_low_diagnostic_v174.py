...
ROOT = Path(
    "tmp/nse_e1_homogeneous_concurrent3_quota1_shortest_request_least_cpu_bounded_terminal_low_diagnostic_20260901_v174"
)
PLAN = Path(
    "scripts/reviewer_experiments/protocol/"
    "nse_e1_homogeneous_concurrent3_quota1_shortest_request_least_cpu_bounded_terminal_low_diagnostic_plan_v174.json"
)
PLAN_SHA256 = "97c28de4f401ec474587f00ad6c0738cd27e941b6489531e819f04da2b283133"
IMPLEMENTATION = Path(
    "scripts/reviewer_experiments/protocol/"
    "nse_e1_homogeneous_concurrent3_quota1_shortest_request_least_cpu_bounded_terminal_slack_short_work_pipeline_queue8_low_diagnostic_implementation_v174.json"
)
IMPLEMENTATION_SHA256 = (
    "3b5d29f3bddce4fdbb8d600a9890868b346a01f5c6ddfe858baf3b3f0834c35f"
)
V170_FAILURE = Path(
    "scripts/reviewer_experiments/protocol/"
...
ARM_ID = "v174-low-srpt-slack-concurrent3-quota1-shortest-request-least-cpu-bounded-terminal-short5p5-pipeline-hiku2-ocs-queue8"
PROFILE = "srpt_slack_concurrent3_quota1_shortest_request_least_cpu_bounded_terminal_short5p5_pipeline_hiku2_ocs_queue8"
FRONTIER = "parents_completed_or_concurrent3_quota1_shortest_request_least_cpu_bounded_terminal_or_slack_short_work_parents_scheduled"
SINGLE_CHANGE = (
    "V173_least_CPU_quota_one_selection_changed_to_shortest_current_request_"
    "remaining_work_then_least_immutable_function_CPU_work_then_request_id_then_"
    "function_id"
)
TERMINAL_DEFINITION = (
    ...
    "with_shortest_current_request_remaining_work_tied_by_least_immutable_"
    "function_CPU_work_then_request-id-then-function-id_and_apply_the_V168_CPU_"
    "bound_to_the_excess;otherwise_retain_V159_terminal_admission;retain_V159_"
...
QUOTA_SELECTION_ORDER = (
    "ascending_current_request_remaining_work_then_immutable_function_cpu_work_"
    "then_request_id_then_function_id"
)
...
BINARY_SOURCE_COMMIT = "bf21f1d7025389edadc72683e2be92c42b92d0d2"
BINARY_PATH = Path("serverless_sim/target_e1_v174/release/serverless_sim.exe")
BINARY_SHA256 = "e75dcc3c1bbe53af54ec0994e44464ba427ab9dec8c439ad8c829227b875f354"


def paths(root: Path = ROOT) -> dict[str, Path]:
    return {
        "manifest": root / f"manifest.{ARM_ID}.unbound.json",
        "prepared": root / "prepared-v174.json",
        "schedule": root / "frozen-run-order-v174.json",
        ...
        "execution": root / "execution-receipt-v174.json",
        "pairing": root / "pairing-audit-v174.json",
        "blind": root / "joint-blind-audit-v174.json",
        "result": root / "diagnostic-result-v174.json",
    }
...
def _assert_frozen_inputs() -> dict[str, Any]:
    ...
    for path, sha256, label in (
        (PLAN, PLAN_SHA256, "V174 plan"),
        (IMPLEMENTATION, IMPLEMENTATION_SHA256, "V174 implementation receipt"),
        ...
        (BINARY_PATH, BINARY_SHA256, "V174 release binary"),
    ...
    if not (
        ...
        == "admit_the_heavy_incomplete_parent_terminal_player_from_the_current_request_with_shortest_remaining_work"
        ...
        raise RuntimeError("V174 implementation boundary changed")
...
def _rewrite_candidate(
    ...
    marker.update(
        {
            "purpose": (
                "V174 outcome-disclosed six-seed concurrent3 quota1 CPU-bounded "
                "terminal-starvation diagnostic; "
                "never a formal result or paper superiority claim"
            ),
            "v174_role": "result_blind_concurrent3_quota1_shortest_request_least_cpu_bounded_terminal_falsification",
            "v174_plan_sha256": PLAN_SHA256,
            "v174_implementation_sha256": IMPLEMENTATION_SHA256,
            "v174_binary_source_commit": BINARY_SOURCE_COMMIT,
            "v174_protocol_source_commit": protocol_source_commit,
            "v174_binary_sha256": BINARY_SHA256,
            "v174_arm_id": ARM_ID,
            "v174_profile": PROFILE,
            "v174_player_frontier": FRONTIER,
            "v174_single_change_from_v172": SINGLE_CHANGE,
            "v174_cpu_threshold": CPU_THRESHOLD,
            "v174_cpu_boundary": "at_or_below_is_admitted",
            "v174_heavy_player_threshold": HEAVY_PLAYER_THRESHOLD,
            "v174_minimum_active_heavy_players": MINIMUM_ACTIVE_HEAVY_PLAYERS,
            "v174_overload_activation_boundary": "strictly_above_activates",
            "v174_inactive_behavior": "exact_V159_terminal_admission",
            "v174_active_heavy_admission_policy": "deterministic_shortest_request_least_cpu_quota_one",
            "v174_active_heavy_admission_quota": ACTIVE_HEAVY_QUOTA,
            "v174_quota_selection_order": QUOTA_SELECTION_ORDER,
            "v174_short_work_threshold": SHORT_WORK_THRESHOLD,
            "v174_queue_density_threshold": QUEUE_THRESHOLD,
            "v174_queue_boundary": "below_is_strict",
            "v174_environment": COMMON_ENVIRONMENT,
            "v174_expected_run_count": len(SEEDS),
            "v174_expected_reference_build_count": len(SEEDS),
            "v174_fixed_order": list(SEEDS),
            "v174_candidate_performance_summaries_parsed": 0,
            "v174_remaining_fourteen_authorized": False,
            "v174_confirmation_inputs_generated": False,
        }
    )
    for run in rewritten["runs"]:
        ...
        run["metadata"] = {
            ...
            "v174_queue_density_threshold": QUEUE_THRESHOLD,
            "v174_queue_boundary": "below_is_strict",
            "v174_source_e1_run_id": source_run_id,
            "v174_source_e1_run_spec_hash": source_run_spec_hash,
            "v174_candidate_performance_summaries_parsed_before_run": 0,
            "v174_remaining_fourteen_authorized": False,
            "v174_confirmation_inputs_generated": False,
        }
...
def _validate_product(manifest: Mapping[str, Any], *, references_bound: bool) -> None:
    if not (
        ...
        raise RuntimeError("V174 exact E01/E02/E13/E15/E16/E20 product changed")
    ...
    for run in manifest["runs"]:
        ...
        if not (
            ...
            and metadata.get("v174_profile") == PROFILE
            and metadata.get("v174_player_frontier") == FRONTIER
            and metadata.get("v174_cpu_threshold") == CPU_THRESHOLD
            and metadata.get("v174_cpu_boundary") == "at_or_below_is_admitted"
            and metadata.get("v174_heavy_player_threshold") == HEAVY_PLAYER_THRESHOLD
            and metadata.get("v174_minimum_active_heavy_players")
            == MINIMUM_ACTIVE_HEAVY_PLAYERS
            and metadata.get("v174_overload_activation_boundary")
            == "strictly_above_activates"
            and metadata.get("v174_inactive_behavior")
            == "exact_V159_terminal_admission"
            and metadata.get("v174_active_heavy_admission_policy")
            == "deterministic_shortest_request_least_cpu_quota_one"
            and metadata.get("v174_active_heavy_admission_quota") == ACTIVE_HEAVY_QUOTA
            and metadata.get("v174_quota_selection_order") == QUOTA_SELECTION_ORDER
            and metadata.get("v174_short_work_threshold") == SHORT_WORK_THRESHOLD
            and metadata.get("v174_queue_density_threshold") == QUEUE_THRESHOLD
            and metadata.get("v174_queue_boundary") == "below_is_strict"
        ):
            raise RuntimeError(f"V174 run contract changed: {run.get('run_id')}")


def prepare_v174(root: Path = ROOT) -> dict[str, Any]:
    source = _assert_frozen_inputs()
    if root.exists():
        raise RuntimeError(f"refusing to overwrite V174 root: {root}")
    ...
    schedule = {
        "schema_version": "NSE_E1_HOMOGENEOUS_CONCURRENT3_CPU_BOUNDED_TERMINAL_LOW_SCHEDULE_V174_V1",
        "created_at": utc_now(),
    ...
    receipt = {
        "schema_version": "NSE_E1_HOMOGENEOUS_CONCURRENT3_CPU_BOUNDED_TERMINAL_LOW_PREPARED_V174_V1",
        "created_at": utc_now(),
        ...
        "active_heavy_admission_policy": "deterministic_shortest_request_least_cpu_quota_one",
...
def execute_v174(root: Path = ROOT) -> dict[str, Any]:
    output = paths(root)
    if output["execution"].exists():
        raise RuntimeError("V174 execution receipt already exists")
    prepared = read_json(output["prepared"])
    _assert_hashed(prepared, "receipt_hash", "V174 prepared receipt")
    ...
    for ordinal, seed in enumerate(SEEDS, start=1):
        ...
        if completed.returncode != 0:
            raise RuntimeError(f"V174 dispatch {seed} failed: {completed.returncode}")
        ...
        if not (
            ...
            raise RuntimeError(f"V174 canonical is not a QC pass: {run['run_id']}")
        dispatches.append(
            {
                "ordinal": ordinal,
                "seed": seed,
    ...
    receipt = {
        "schema_version": "NSE_E1_HOMOGENEOUS_CAPACITY_OVERLOAD_CPU_BOUNDED_TERMINAL_LOW_EXECUTION_V174_V1",
        "created_at": utc_now(),
        "candidate_performance_summaries_parsed": 0,
        "plan_sha256": PLAN_SHA256,
...
def _audit_nash_log(
    ...
    *,
    ...
    with gzip.open(log, "rt", encoding="utf-8") as stream:
        for line in stream:
            ...
            if kind not in counts:
                raise RuntimeError(f"unexpected V174 Nash observation kind: {kind}")
            counts[kind] += 1
            if kind == "run_config":
                ...
                if not (
                    ...
                    raise RuntimeError("V174 run_config contract changed")
            elif kind == "window":
                if event.get("frame") != counts["window"] - 1:
                    raise RuntimeError("V174 scheduler window sequence changed")
                ...
                if (
                    ...
                    raise RuntimeError("V174 decision frontier/hash changed")
                ...
                if not (
                    ...
                    raise RuntimeError("V174 CPU/slack/queue frontier evidence changed")
                ...
                if not (
                    ...
                    raise RuntimeError("V174 route telemetry is incomplete")
                expected = (
                    LOW_EXPERT if float(density) < QUEUE_THRESHOLD else HIGH_EXPERT
                )
                if route.get("selected_expert") != expected:
                    raise RuntimeError("V174 route does not match queue density")
                ...
                if key is None:
                    if source != "not_requested":
                        raise RuntimeError("V174 unrequested reference reason changed")
                    totals["reference_not_requested"] += 1
                elif source in ("offline_table", "offline_table_nonpositive"):
                    totals["reference_available"] += 1
                else:
                    raise RuntimeError("V174 bound reference source changed")
                ...
                if not (
                    ...
                    raise RuntimeError("V174 Nash terminal marker changed")
    ...
    if (
        ...
        raise RuntimeError("V174 Nash log cardinality changed")
    if (
        totals["reference_available"] + totals["reference_not_requested"]
        != counts["window"]
    ):
        raise RuntimeError("V174 reference replay coverage changed")
...
def _mechanism_falsification_gate(
    ...
    if set(by_seed) != set(SEEDS):
        raise RuntimeError("V174 blind audit seed product changed")
    ...
    return {
        ...
        **extrema,
        ...
        "shortest_request_quota_selection_order_certified_in_every_active_window": (
...
def blind_audit_v174(root: Path = ROOT) -> dict[str, Any]:
    output = paths(root)
    if output["blind"].exists():
        raise RuntimeError("V174 blind audit already exists")
    _assert_frozen_inputs()
    prepared = read_json(output["prepared"])
    prepared_hash = _assert_hashed(prepared, "receipt_hash", "V174 prepared receipt")
    execution = read_json(output["execution"])
    execution_hash = _assert_hashed(execution, "receipt_hash", "V174 execution receipt")
    manifest = load_and_validate_manifest(output["ready"])
    _validate_product(manifest, references_bound=True)
    ...
    if (
        ...
        raise RuntimeError("V174 exact pairing failed")
    ...
    if [item["seed"] for item in execution["dispatches"]] != list(SEEDS):
        raise RuntimeError("V174 execution order changed")
    ...
    if {path.name for path in canonical_root.iterdir() if path.is_dir()} != {
        ...
        raise RuntimeError("V174 canonical product changed")
    quarantine = output["workspace"] / "quarantine"
    if quarantine.exists() and any(quarantine.rglob("attempt-*")):
        raise RuntimeError("V174 has unexplained quarantined attempts")
    ...
    if len(identities) != 1:
        raise RuntimeError("V174 runtime identity is not unanimous")
    ...
    if not (
        ...
        raise RuntimeError("V174 runtime identity changed")
    mechanism = _mechanism_falsification_gate(audits)
    if not mechanism["pass"]:
        raise RuntimeError("V174 mechanism falsification breadth is insufficient")
    document = {
        "schema_version": "NSE_E1_HOMOGENEOUS_CONCURRENT3_CPU_BOUNDED_TERMINAL_LOW_BLIND_AUDIT_V174_V1",
        "created_at": utc_now(),
        "status": "pass",
...
def _load_candidate(
    ...
    if len(rows) != len(SEEDS) or [row["seed"] for row in rows] != list(SEEDS):
        raise RuntimeError("V174 candidate result product changed")
    return rows


def _hybrid_rows_v174(
    v170_rows: Sequence[Mapping[str, Any]],
    v174_rows: Sequence[Mapping[str, Any]],
) -> list[dict[str, Any]]:
    ...
    if len(v174_rows) != len(SEEDS) or {row["seed"] for row in v174_rows} != set(SEEDS):
        raise RuntimeError("V174 diagnostic candidate cohort changed")
    replacements = {row["seed"]: dict(row) for row in v174_rows}
    return [
...
def reveal_v174(root: Path = ROOT) -> dict[str, Any]:
    output = paths(root)
    if output["result"].exists():
        raise RuntimeError("V174 result already exists")
    blind = read_json(output["blind"])
    blind_hash = _assert_hashed(blind, "blind_audit_hash", "V174 blind audit")
    if not (
        ...
        raise RuntimeError("V174 blind audit did not authorize reveal")
    manifest = load_and_validate_manifest(output["ready"])
    candidate = _load_candidate(manifest, root)
    v170_rows = _load_v170_candidate()
    hybrid = _hybrid_rows_v174(v170_rows, candidate)
    ...
    document = {
        "schema_version": "NSE_E1_HOMOGENEOUS_CONCURRENT3_CPU_BOUNDED_TERMINAL_LOW_DIAGNOSTIC_RESULT_V174_V1",
        "created_at": utc_now(),
        "training_only": True,
        "formal_results_eligible": False,
        ...
        "disposition": (
            ...
            if passed
            else "retain_all_six_valid_diagnostic_runs_and_retire_concurrent3_quota1_shortest_request_least_cpu_bounded_terminal_candidate"
        ),
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    if action == "prepare":
        document, key = prepare_v174(), "receipt_hash"
    elif action == "execute":
        document, key = execute_v174(), "receipt_hash"
    elif action == "blind-audit":
        document, key = blind_audit_v174(), "blind_audit_hash"
    else:
        document, key = reveal_v174(), "result_hash"
