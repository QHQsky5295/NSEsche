...
from scripts.reviewer_experiments.protocol.nse_e3_bidirectional_locality_safety_training_blind_audit_v104 import (
    ARMS,
...
ANCHORS = {"E3": "v104-e3-anchor"}
COMPONENT_SAFETY_PREFERENCE = {
    "v104-e3-band8-24-componentwise-bidirectional-locality-pareto-initializer-only": 0,
    "v104-e3-band8-24-bidirectional-locality-pareto-initializer-only": 1,
}
...
def _validate_blind_audit() -> dict[str, Any]:
    if not BLIND_AUDIT.is_file() or file_hash(BLIND_AUDIT) != BLIND_AUDIT_FILE_SHA256:
        raise RuntimeError("V104 blind audit is missing or changed")
    ...
    if (
        ...
        raise RuntimeError("V104 blind audit does not authorize reveal")
    return blind
...
def _load_rows(blind: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    if len(admitted) != 27:
        raise RuntimeError("V104 blind audit run evidence is incomplete")
    ...
    for arm_id, expected in ARMS.items():
        ...
        if not manifest_path.is_file() or file_hash(manifest_path) != frozen.get(
            ...
            raise RuntimeError(f"V104 frozen manifest changed: {arm_id}")
        manifest = load_and_validate_manifest(manifest_path)
        if manifest.get("manifest_hash") != frozen.get("manifest_hash"):
            raise RuntimeError(f"V104 manifest hash changed: {arm_id}")
        for run in manifest["runs"]:
            ...
            if (
                ...
                != expected["profile"]
                or metadata.get("v104_training_plan_sha256") != PLAN_SHA256
                or metadata.get("v104_training_only") is not True
                or metadata.get("v104_training_seed_metrics_previously_revealed")
                is not False
                or metadata.get("v104_confirmation_seeds_opened") is not False
                or metadata.get("v104_other_unopened_seeds_opened") is not False
                or metadata.get("v104_arm_id") != arm_id
                or metadata.get("v104_arm_role") != expected["role"]
                or metadata.get("v104_upper_queue_density_threshold")
                != expected["upper_queue_density_threshold"]
                or metadata.get("v104_nonterminal_queue_density_floor")
                != expected["nonterminal_queue_density_floor"]
                or metadata.get("v104_warm_admissibility")
                != expected["warm_admissibility"]
                or metadata.get("v104_load_least_window_certificate_mode")
                != expected["load_least_window_certificate_mode"]
                or metadata.get("v104_component_safety_mode")
                ...
                is not False
                or metadata.get("v104_future_child_placement_or_feasibility_used")
                is not False
            ):
                raise RuntimeError(f"V104 arm identity changed: {run['run_id']}")
            run_id = run["run_id"]
            summary_path = (
                ROOT
                / "runs"
            ...
            if not summary_path.is_file() or file_hash(summary_path) != admitted.get(
                ...
                raise RuntimeError(f"V104 summary differs from blind audit: {run_id}")
            ...
            if (
                ...
                raise RuntimeError(
                    f"V104 summary identity/completion mismatch: {run_id}"
                )
            rows.append(
                {
                    "arm_id": arm_id,
...
def execute_reveal() -> dict[str, Any]:
    if OUTPUT.exists():
        raise RuntimeError(f"V104 training result already exists: {OUTPUT}")
    if not PLAN.is_file() or file_hash(PLAN) != PLAN_SHA256:
        raise RuntimeError("frozen V104 plan changed before reveal")
    blind = _validate_blind_audit()
    rows = _load_rows(blind)
    if len(rows) != 27:
        raise RuntimeError("V104 revealed row count differs from sealed plan")
    ...
    result = {
        "schema_version": "NSE_E3_BIDIRECTIONAL_LOCALITY_SAFETY_RESULT_V104_V1",
        "created_at": utc_now(),
        ...
        "selection_rule": {
            ...
            "multiple_passers": (
                ...
                "throughput change, then prefer componentwise bidirectional-locality "
                "Pareto over bidirectional-locality Pareto, then arm_id"
            ),
            "frozen_E4": (
                "retain V97 v97-e4-load-guard64 without V104 result-dependent reselection"
            ),
        ...
        **evaluation,
        "revealed_rows": rows,
        "decision": {
            "authorize_preregistration_of_E966_E985_confirmation": joint_pass,
            ...
            "next_action": (
                ...
                "separately preregister the complete E966-E985 NSESche confirmation "
                "before generating inputs"
                if joint_pass
                else (
                    "freeze the complete V104 training cohort, keep E926-E985 and "
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Reveal the frozen V104 training cohort exactly once after blind audit."
    )
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    if args.command != "reveal" or not args.execute:
        parser.error("V104 reveal requires the explicit command: reveal --execute")
    execute_reveal()
