...
PREPARED = ROOT / "prepared-manifests-v134.json"
TAPES = ROOT / "tapes.catalog.json"
OUTPUT = ROOT / "joint-blind-audit-v134-training.json"
RESULT = ROOT / "training-result-v134.json"
...
ARMS = {
    arm_id: {
        ...
        "selected_choice_interference_veto": role == "candidate",
        "run_count": count,
    }
...
def _validate_admitted_work_diagnostics(
    ...
    """Validate V134 mechanism evidence without opening any performance summary."""

    path = canonical / "reviewer_records" / run["run_id"] / "nash_metrics.jsonl.gz"
    _require(path.is_file(), f"missing V134 Nash diagnostics: {run['run_id']}")
    ...
    with gzip.open(path, "rt", encoding="utf-8") as handle:
        for line_number, line in enumerate(handle, start=1):
            ...
            _require(type(frame) is int and frame >= 0, "invalid V134 window frame")

            cluster = event.get("cluster")
            _require(isinstance(cluster, dict), "missing V134 cluster diagnostics")
            admitted_fields = (
            ...
            if candidate:
                _require(
                    ...
                    f"V134 admitted-work observation changed: {run['run_id']}:{line_number}",
                )
                ...
                _require(
                    ...
                    f"V134 admitted-work snapshot counts changed: "
                ...
                _require(
                    ...
                    f"V134 admitted-work aggregate changed: {run['run_id']}:{line_number}",
                )
            ...
            _require(
                ...
                f"V134 critical-service contract changed: {run['run_id']}:{line_number}",
            )
            ...
            _require(
                ...
                f"V134 componentwise-service contract changed: "
                f"{run['run_id']}:{line_number}",
            )
            for field in (
                ...
                _require(
                    ...
                    f"V134 componentwise {field} changed: "
                    f"{run['run_id']}:{line_number}",
                )
            ...
            _require(
                ...
                f"V134 componentwise acceptance changed: "
                f"{run['run_id']}:{line_number}",
            )
            ...
            _require(
                ...
                f"V134 componentwise reason changed: {run['run_id']}:{line_number}",
            )
            if not componentwise["evaluated"]:
                _require(
                    ...
                    is None,
                    f"V134 unevaluated componentwise evidence was populated: "
                    f"{run['run_id']}:{line_number}",
                )
            ...
            _require(
                ...
                f"V134 componentwise delta changed: {run['run_id']}:{line_number}",
            )
            _require(
                maximum_ratio is None or _finite_nonnegative(maximum_ratio),
                f"V134 componentwise ratio changed: {run['run_id']}:{line_number}",
            )
            if componentwise["accepted"] and componentwise["critical_player_count"] > 0:
                _require(
                    maximum_delta is not None and float(maximum_delta) <= 1.000001e-6,
                    f"V134 accepted componentwise delta is worse: "
                    f"{run['run_id']}:{line_number}",
                )
            _require(
                not critical["accepted"] or componentwise["accepted"],
                f"V134 main certificate bypassed componentwise gate: "
                f"{run['run_id']}:{line_number}",
            )
            ...
            if dominance.get("reason") in componentwise_rejection_reasons:
                _require(
                    ...
                    f"V134 componentwise fallback was not exact anchor: "
                    f"{run['run_id']}:{line_number}",
                )
            ...
            _require(
                ...
                is (invoked and expected["componentwise_interference_gate"])
                and interference.get("selected_choice_veto_enabled")
                is (invoked and expected["selected_choice_interference_veto"])
                ...
                and interference.get("source") == "not_applicable"
                and interference.get("uses_completion_outcomes") is False,
                f"V134 selected-choice-interference contract changed: "
                f"{run['run_id']}:{line_number}",
            )
            ...
            _require(
                ...
                f"V134 unexpectedly enabled the complete aggregate interference gate: "
                f"{run['run_id']}:{line_number}",
            )
            ...
            for field in (
                ...
                _require(
                    ...
                    f"V134 critical {field} changed: {run['run_id']}:{line_number}",
                )
            _require(
                not critical["accepted"] or critical["evaluated"],
                f"V134 accepted without evaluation: {run['run_id']}:{line_number}",
            )
            if not invoked:
                _require(
                    ...
                    f"V134 non-invoked critical state changed: {run['run_id']}:{line_number}",
                )
            if critical["accepted"]:
                ...
                _require(
                    ...
                    f"V134 accepted evidence is nonfinite: {run['run_id']}:{line_number}",
                )
                ...
                _require(
                    ...
                    f"V134 accepted admitted-work certificate changed: "
                    f"{run['run_id']}:{line_number}",
                )
            ...
            if not candidate:
                _require(
                    ...
                    f"V134 anchor used causal state: {run['run_id']}:{line_number}",
                )
                continue
            if not gate["history_complete"]:
                _require(
                    gate["baseline_count"] == 0 and gate["recent_count"] == 0,
                    f"V134 incomplete history leaked counts: {run['run_id']}:{line_number}",
                )
                continue
            sufficient = (
            ...
            if sufficient and recent_scaled >= baseline_scaled:
                ...
                _require(
                    ...
                    f"V134 exact threshold did not activate: "
                    f"{run['run_id']}:{line_number}",
                )

    _require(
        counts["windows"] == run["simulation"]["total_frame"],
        f"V134 diagnostic window count changed: {run['run_id']}",
    )
    if candidate:
        _require(
            ...
            f"V134 mechanism was not exercised: {run['run_id']}",
        )
    return {
        ...
        "selected_choice_admitted_interference_veto_enabled": expected[
            "selected_choice_interference_veto"
        ],
        ...
        "admitted_interference_nonworse_window_count": counts[
            "interference_nonworse_windows"
        ],
        "admitted_interference_worse_window_count": counts[
            "interference_worse_windows"
        ],
...
def _verify_tape_catalog() -> tuple[list[dict[str, Any]], dict[str, Any]]:
    _require(TAPES.is_file(), f"missing V134 tape catalog: {TAPES}")
    catalog = read_json(TAPES)
    catalog_hash = _assert_hashed_object(catalog, "catalog_hash", "V134 tapes")
    entries = catalog.get("entries")
    _require(
        isinstance(entries, dict) and len(entries) == 12, "V134 tape count changed"
    )
    evidence = []
...
def _verify_reference_stage_ledgers() -> tuple[dict[str, Path], dict[Path, str]]:
    ...
    for arm_id, expected in ARMS.items():
        ...
        root = _stage_root_from_receipts(
            entries, "receipt_path", expected["run_count"], f"V134 reference {arm_id}"
        )
        roots[arm_id] = root
    ...
    for root, expected_count in expected_by_root.items():
        ...
        _assert_ledger_contract(
            ...
            f"V134 reference stage {root}",
        )
        _require(
            not list((root / "quarantine").glob("**/attempt-*")),
            f"V134 reference quarantine is nonempty: {root}",
        )
...
def _validate_run_boundary(
    ...
    required = {
        "v134_training_plan_sha256": PLAN_SHA256,
        "v134_training_only": True,
        "v134_training_seed_metrics_previously_revealed": False,
        "v134_confirmation_seeds_opened": False,
        "v134_other_unopened_seeds_opened": False,
        "v134_formal_E01_E20_reexecution": False,
        "v134_arm_id": arm_id,
        "v134_arm_role": expected["role"],
        "v134_candidate_profile": expected["profile"],
        "v134_candidate_experiment": "E3",
        "v134_shock_rate_ratio": expected["shock_rate_ratio"],
        "v134_shock_threshold_numerator": expected["shock_threshold_numerator"],
        "v134_shock_threshold_denominator": expected["shock_threshold_denominator"],
        "v134_arrival_history_baseline_frames": 80 if candidate else None,
        "v134_arrival_history_recent_frames": 20 if candidate else None,
        "v134_arrival_min_requests_per_window": 20 if candidate else None,
        "v134_shock_activation_horizon_frames": expected[
            "shock_activation_horizon_frames"
        ],
        "v134_nonterminal_queue_density_floor": 8.0 if candidate else None,
        "v134_warm_admissibility": "preserve_anchor_warmness" if candidate else None,
        "v134_load_least_window_certificate_mode": (
            "disabled" if candidate else "not_applicable"
        ),
        "v134_critical_service_ratio_numerator": expected[
            "critical_service_threshold_numerator"
        ],
        "v134_critical_service_ratio_denominator": expected[
            "critical_service_threshold_denominator"
        ],
        "v134_critical_service_proxy": (
            "remote_parent_transfer_plus_cold_start_plus_admitted_queue_cpu_work"
            if candidate
            else "not_applicable"
        ),
        "v134_service_proxy_work_source": (
            ADMITTED_WORK_SOURCE if candidate else "not_applicable"
        ),
        "v134_admitted_work_includes_all_blocked_resident": candidate,
        "v134_admitted_work_deterministic_f64_sum": candidate,
        "v134_complete_componentwise_service_pareto": candidate,
        "v134_componentwise_service_comparison": (
            "every_current_critical_frontier_alternative_proxy_lte_anchor_plus_epsilon"
            if candidate
            else "not_applicable"
        ),
        "v134_componentwise_service_replay_order": (
            "native_player_order" if candidate else "not_applicable"
        ),
        "v134_componentwise_service_inputs_finite_fail_closed": candidate,
        "v134_componentwise_service_coverage_mismatch_fail_closed": candidate,
        "v134_complete_admitted_interference_pareto": False,
        "v134_componentwise_admitted_interference_pareto": False,
        "v134_selected_choice_interference_veto": candidate,
        "v134_selected_choice_interference_definition": (
            "current_player_sum_of_min_admitted_work_and_player_work_over_node_capacity"
            if candidate
            else "not_applicable"
        ),
        "v134_selected_choice_interference_comparison": (
            "first_otherwise_accepted_player_interference_lte_anchor_plus_epsilon"
            if candidate
            else "not_applicable"
        ),
        "v134_selected_choice_interference_inputs_finite_fail_closed": candidate,
        "v134_arrival_phase_guard": False,
        "v134_critical_service_proxy_inputs_finite_fail_closed": candidate,
        "v134_cpu_memory_individual_noninferiority": candidate,
        "v134_scalar_faasrank_noninferiority": candidate,
        "v134_input_locality_component_noninferiority": candidate,
        "v134_per_child_current_warm_downstream_locality_noninferiority": candidate,
        "v134_critical_frontier_substitution": candidate,
        "v134_critical_frontier_rank_source": (
            "immutable_srpt_remaining_critical_path_rank"
            if candidate
            else "not_applicable"
        ),
        "v134_noncritical_exact_anchor": candidate,
        "v134_complete_summed_critical_service_proxy_strictly_lower": candidate,
        "v134_complete_routed_score_nonworse": candidate,
        "v134_complete_exact_ocs_score_nonworse": candidate,
        "v134_complete_immutable_baseline_welfare_nonworse": candidate,
        "v134_outcome_fields_drive_policy": False,
        "v134_scenario_or_burst_label_used_by_policy": False,
        "v134_completion_or_performance_fields_used_by_policy": False,
        "v134_future_arrivals_used_by_policy": False,
    }
    _require(
        ...
        f"V134 run boundary changed: {run['run_id']}",
    )


def run_blind_audit(output: Path = OUTPUT) -> dict[str, Any]:
    _require(not output.exists(), f"V134 blind audit already exists: {output}")
    _require(not RESULT.exists(), "V134 reveal exists before blind audit")
    _require(PLAN.is_file() and file_hash(PLAN) == PLAN_SHA256, "V134 plan changed")
    plan = read_json(PLAN)
    _require(plan.get("formal_results_eligible") is False, "V134 eligibility changed")
    _require(PREPARED.is_file(), f"missing V134 prepared receipt: {PREPARED}")
    prepared = read_json(PREPARED)
    prepared_hash = _assert_hashed_object(
        prepared, "receipt_hash", "V134 prepared receipt"
    )
    _require(
        ...
        "V134 prepared scientific boundary changed",
    )
    ...
    for arm_id, expected in ARMS.items():
        ...
        _require(paths["ready"].is_file(), f"missing V134 ready manifest: {arm_id}")
        ...
        _require(
            ...
            f"V134 ready boundary changed: {arm_id}",
        )
        ...
        reference_catalog_hash = _assert_hashed_object(
            references, "catalog_hash", f"V134 references {arm_id}"
        )
        ...
        _require(
            ...
            f"V134 reference key set changed: {arm_id}",
        )
        for key, entry in sorted(entries.items()):
            ...
            _require(
                ...
                f"V134 reference evidence changed: {key}",
            )
        ...
        _require(actual_ids == expected_ids, f"V134 canonical set changed: {arm_id}")
        _require(
            not list((workspace / "quarantine").glob("**/attempt-*")),
            f"V134 online quarantine is nonempty: {arm_id}",
        )
        ...
        for run in manifest["runs"]:
            ...
            _require(
                ...
                f"V134 canonical status changed: {run['run_id']}",
            )
            runtime = {
    ...
    _require(len(run_evidence) == 18, "V134 run evidence count must be 18")
    _require(len(reference_evidence) == 18, "V134 reference evidence count must be 18")
    _require(len(tape_evidence) == 12, "V134 tape evidence count must be 12")
    ...
    _require(
        ...
        > 0,
        "V134 componentwise certificate did not exercise both accept and reject paths",
    )
    _require(
        all(
        ...
        "V134 anchor unexpectedly exercised the componentwise certificate",
    )
    _require(
        sum(
            row["admitted_interference_input_unavailable_count"]
            for row in candidate_diagnostics
        )
        == 0
        and sum(
            row["admitted_interference_rejected_candidate_count"]
            for row in candidate_diagnostics
        )
        > 0
        and all(
            row["componentwise_admitted_interference_gate_enabled"] is False
            and row["selected_choice_admitted_interference_veto_enabled"] is True
            and row["admitted_interference_finite_window_count"] == 0
            and row["admitted_interference_accepted_window_count"] == 0
            and row["admitted_interference_nonworse_window_count"] == 0
            and row["admitted_interference_worse_window_count"] == 0
            for row in candidate_diagnostics
        ),
        "V134 selected-choice interference veto was unavailable or not exercised",
    )
    _require(
        all(
            row["admitted_interference_finite_window_count"] == 0
            and row["admitted_interference_accepted_window_count"] == 0
            and row["admitted_interference_rejected_candidate_count"] == 0
            and row["componentwise_admitted_interference_gate_enabled"] is False
            and row["selected_choice_admitted_interference_veto_enabled"] is False
            for row in anchor_diagnostics
        ),
        "V134 anchor unexpectedly exercised an interference certificate",
    )
    for field, expected in EXPECTED_RUNTIME.items():
        _require(
            runtime_values[field] == {expected},
            f"V134 runtime {field} changed: {runtime_values[field]}",
        )
    ...
    _require(
        ...
        f"V134 runtime git identity is not singular: {git_commits}",
    )
    _require(len(paired_inputs) == 9, "V134 paired input product changed")
    for (scenario, seed), rows in paired_inputs.items():
        _require(len(rows) == 2, f"V134 paired arm count changed: {scenario}/{seed}")
        for field in (
            ...
            _require(
                ...
                f"V134 paired {field} changed: {scenario}/{seed}",
            )
        _require(
            rows[0]["common_hpa_hash"] == EXPECTED_COMMON_HPA_SHA256,
            f"V134 common HPA changed: {scenario}/{seed}",
        )

    output_payload = {
        "schema_version": "NSE_E3_COMPONENTWISE_SERVICE_SELECTED_INTERFERENCE_VETO_BLIND_AUDIT_V134_V1",
        "created_at": utc_now(),
        "status": "pass",
