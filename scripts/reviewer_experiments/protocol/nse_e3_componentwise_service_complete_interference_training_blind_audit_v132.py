...
from scripts.reviewer_experiments.protocol.nse_e3_componentwise_service_complete_interference_training_prepare_v132 import (
...
PREPARED = ROOT / "prepared-manifests-v132.json"
TAPES = ROOT / "tapes.catalog.json"
OUTPUT = ROOT / "joint-blind-audit-v132-training.json"
RESULT = ROOT / "training-result-v132.json"
...
def _validate_admitted_work_diagnostics(
    ...
    """Validate V132 mechanism evidence without opening any performance summary."""

    path = canonical / "reviewer_records" / run["run_id"] / "nash_metrics.jsonl.gz"
    _require(path.is_file(), f"missing V132 Nash diagnostics: {run['run_id']}")
    ...
    with gzip.open(path, "rt", encoding="utf-8") as handle:
        for line_number, line in enumerate(handle, start=1):
            ...
            _require(type(frame) is int and frame >= 0, "invalid V132 window frame")

            cluster = event.get("cluster")
            _require(isinstance(cluster, dict), "missing V132 cluster diagnostics")
            admitted_fields = (
            ...
            if candidate:
                _require(
                    ...
                    f"V132 admitted-work observation changed: {run['run_id']}:{line_number}",
                )
                ...
                _require(
                    ...
                    f"V132 admitted-work snapshot counts changed: "
                ...
                _require(
                    ...
                    f"V132 admitted-work aggregate changed: {run['run_id']}:{line_number}",
                )
            ...
            _require(
                ...
                f"V132 critical-service contract changed: {run['run_id']}:{line_number}",
            )
            ...
            _require(
                ...
                f"V132 componentwise-service contract changed: "
                f"{run['run_id']}:{line_number}",
            )
            for field in (
                ...
                _require(
                    ...
                    f"V132 componentwise {field} changed: "
                    f"{run['run_id']}:{line_number}",
                )
            ...
            _require(
                ...
                f"V132 componentwise acceptance changed: "
                f"{run['run_id']}:{line_number}",
            )
            ...
            _require(
                ...
                f"V132 componentwise reason changed: {run['run_id']}:{line_number}",
            )
            if not componentwise["evaluated"]:
                _require(
                    ...
                    is None,
                    f"V132 unevaluated componentwise evidence was populated: "
                    f"{run['run_id']}:{line_number}",
                )
            ...
            _require(
                ...
                f"V132 componentwise delta changed: {run['run_id']}:{line_number}",
            )
            _require(
                maximum_ratio is None or _finite_nonnegative(maximum_ratio),
                f"V132 componentwise ratio changed: {run['run_id']}:{line_number}",
            )
            if componentwise["accepted"] and componentwise["critical_player_count"] > 0:
                _require(
                    ...
                    f"V132 accepted componentwise delta is worse: "
                    f"{run['run_id']}:{line_number}",
                )
            _require(
                not critical["accepted"] or componentwise["accepted"],
                f"V132 main certificate bypassed componentwise gate: "
                f"{run['run_id']}:{line_number}",
            )
            ...
            if dominance.get("reason") in componentwise_rejection_reasons:
                _require(
                    ...
                    f"V132 componentwise fallback was not exact anchor: "
                    f"{run['run_id']}:{line_number}",
                )
            ...
            _require(
                ...
                f"V132 complete-interference contract changed: "
                f"{run['run_id']}:{line_number}",
            )
            ...
            if invoked and all(value is not None for value in interference_values):
                _require(
                    ...
                    f"V132 complete-interference evidence changed: "
                    f"{run['run_id']}:{line_number}",
                )
                ...
                if (
                    float(interference_values[1])
                    <= float(interference_values[0]) + 1.0e-9
                ):
                    counts["interference_nonworse_windows"] += 1
                else:
                    counts["interference_worse_windows"] += 1
            else:
                _require(
                    all(value is None for value in interference_values),
                    f"V132 partial complete-interference evidence: "
                    f"{run['run_id']}:{line_number}",
                )
            if critical["accepted"]:
                _require(
                    ...
                    <= float(interference_values[0]) + 1.0e-9,
                    f"V132 accepted worse complete interference: "
                    f"{run['run_id']}:{line_number}",
                )
                counts["interference_accepted_windows"] += 1
            if dominance.get("reason") == "alternative_admitted_interference_worse":
                _require(
                    all(value is not None for value in interference_values)
                    and float(interference_values[1])
                    > float(interference_values[0]) + 1.0e-9
                    ...
                    and dominance.get("selected_initializer_assignment_hash")
                    == dominance.get("anchor_assignment_hash"),
                    f"V132 complete-interference fallback was not exact anchor: "
                    f"{run['run_id']}:{line_number}",
                )
            ...
            for field in (
                ...
                _require(
                    ...
                    f"V132 critical {field} changed: {run['run_id']}:{line_number}",
                )
            _require(
                not critical["accepted"] or critical["evaluated"],
                f"V132 accepted without evaluation: {run['run_id']}:{line_number}",
            )
            if not invoked:
                _require(
                    ...
                    f"V132 non-invoked critical state changed: {run['run_id']}:{line_number}",
                )
            if critical["accepted"]:
                values = tuple(
                    critical.get(field)
                ...
                _require(
                    ...
                    f"V132 accepted evidence is nonfinite: {run['run_id']}:{line_number}",
                )
                ...
                _require(
                    ...
                    f"V132 accepted admitted-work certificate changed: "
                    f"{run['run_id']}:{line_number}",
                )
            ...
            if not candidate:
                _require(
                    ...
                    f"V132 anchor used causal state: {run['run_id']}:{line_number}",
                )
                continue
            if not gate["history_complete"]:
                _require(
                    gate["baseline_count"] == 0 and gate["recent_count"] == 0,
                    f"V132 incomplete history leaked counts: {run['run_id']}:{line_number}",
                )
                continue
            sufficient = (
            ...
            if sufficient and recent_scaled >= baseline_scaled:
                ...
                _require(
                    ...
                    f"V132 exact threshold did not activate: "
                    f"{run['run_id']}:{line_number}",
                )

    _require(
        counts["windows"] == run["simulation"]["total_frame"],
        f"V132 diagnostic window count changed: {run['run_id']}",
    )
    if candidate:
        _require(
            ...
            f"V132 mechanism was not exercised: {run['run_id']}",
        )
    return {
...
def _verify_tape_catalog() -> tuple[list[dict[str, Any]], dict[str, Any]]:
    _require(TAPES.is_file(), f"missing V132 tape catalog: {TAPES}")
    catalog = read_json(TAPES)
    catalog_hash = _assert_hashed_object(catalog, "catalog_hash", "V132 tapes")
    entries = catalog.get("entries")
    _require(
        isinstance(entries, dict) and len(entries) == 12, "V132 tape count changed"
    )
    evidence = []
...
def _verify_reference_stage_ledgers() -> tuple[dict[str, Path], dict[Path, str]]:
    ...
    for arm_id, expected in ARMS.items():
        ...
        root = _stage_root_from_receipts(
            entries, "receipt_path", expected["run_count"], f"V132 reference {arm_id}"
        )
        roots[arm_id] = root
    ...
    for root, expected_count in expected_by_root.items():
        ...
        _assert_ledger_contract(
            ...
            f"V132 reference stage {root}",
        )
        _require(
            not list((root / "quarantine").glob("**/attempt-*")),
            f"V132 reference quarantine is nonempty: {root}",
        )
...
def _validate_run_boundary(
    ...
    required = {
        "v132_training_plan_sha256": PLAN_SHA256,
        "v132_training_only": True,
        "v132_training_seed_metrics_previously_revealed": False,
        "v132_confirmation_seeds_opened": False,
        "v132_other_unopened_seeds_opened": False,
        "v132_formal_E01_E20_reexecution": False,
        "v132_arm_id": arm_id,
        "v132_arm_role": expected["role"],
        "v132_candidate_profile": expected["profile"],
        "v132_candidate_experiment": "E3",
        "v132_shock_rate_ratio": expected["shock_rate_ratio"],
        "v132_shock_threshold_numerator": expected["shock_threshold_numerator"],
        "v132_shock_threshold_denominator": expected["shock_threshold_denominator"],
        "v132_arrival_history_baseline_frames": 80 if candidate else None,
        "v132_arrival_history_recent_frames": 20 if candidate else None,
        "v132_arrival_min_requests_per_window": 20 if candidate else None,
        "v132_shock_activation_horizon_frames": expected[
            "shock_activation_horizon_frames"
        ],
        "v132_nonterminal_queue_density_floor": 8.0 if candidate else None,
        "v132_warm_admissibility": "preserve_anchor_warmness" if candidate else None,
        "v132_load_least_window_certificate_mode": (
            "disabled" if candidate else "not_applicable"
        ),
        "v132_critical_service_ratio_numerator": expected[
            "critical_service_threshold_numerator"
        ],
        "v132_critical_service_ratio_denominator": expected[
            "critical_service_threshold_denominator"
        ],
        "v132_critical_service_proxy": (
            "remote_parent_transfer_plus_cold_start_plus_admitted_queue_cpu_work"
            if candidate
            else "not_applicable"
        ),
        "v132_service_proxy_work_source": (
            ADMITTED_WORK_SOURCE if candidate else "not_applicable"
        ),
        "v132_admitted_work_includes_all_blocked_resident": candidate,
        "v132_admitted_work_deterministic_f64_sum": candidate,
        "v132_complete_componentwise_service_pareto": candidate,
        "v132_componentwise_service_comparison": (
            "every_current_critical_frontier_alternative_proxy_lte_anchor_plus_epsilon"
            if candidate
            else "not_applicable"
        ),
        "v132_componentwise_service_replay_order": (
            "native_player_order" if candidate else "not_applicable"
        ),
        "v132_componentwise_service_inputs_finite_fail_closed": candidate,
        "v132_componentwise_service_coverage_mismatch_fail_closed": candidate,
        "v132_complete_admitted_interference_pareto": candidate,
        "v132_componentwise_admitted_interference_pareto": False,
        "v132_complete_interference_definition": (
            "sum_over_current_critical_frontier_players_of_min_admitted_work_and_player_work_over_node_capacity"
            if candidate
            else "not_applicable"
        ),
        "v132_complete_interference_comparison": (
            "alternative_complete_interference_lte_anchor_plus_epsilon"
            if candidate
            else "not_applicable"
        ),
        "v132_complete_interference_replay_order": (
            "native_player_order" if candidate else "not_applicable"
        ),
        "v132_complete_interference_inputs_finite_fail_closed": candidate,
        "v132_arrival_phase_guard": False,
        "v132_critical_service_proxy_inputs_finite_fail_closed": candidate,
        "v132_cpu_memory_individual_noninferiority": candidate,
        "v132_scalar_faasrank_noninferiority": candidate,
        "v132_input_locality_component_noninferiority": candidate,
        "v132_per_child_current_warm_downstream_locality_noninferiority": candidate,
        "v132_critical_frontier_substitution": candidate,
        "v132_critical_frontier_rank_source": (
            "immutable_srpt_remaining_critical_path_rank"
            if candidate
            else "not_applicable"
        ),
        "v132_noncritical_exact_anchor": candidate,
        "v132_complete_summed_critical_service_proxy_strictly_lower": candidate,
        "v132_complete_routed_score_nonworse": candidate,
        "v132_complete_exact_ocs_score_nonworse": candidate,
        "v132_complete_immutable_baseline_welfare_nonworse": candidate,
        "v132_outcome_fields_drive_policy": False,
        "v132_scenario_or_burst_label_used_by_policy": False,
        "v132_completion_or_performance_fields_used_by_policy": False,
        "v132_future_arrivals_used_by_policy": False,
    }
    _require(
        ...
        f"V132 run boundary changed: {run['run_id']}",
    )


def run_blind_audit(output: Path = OUTPUT) -> dict[str, Any]:
    _require(not output.exists(), f"V132 blind audit already exists: {output}")
    _require(not RESULT.exists(), "V132 reveal exists before blind audit")
    _require(PLAN.is_file() and file_hash(PLAN) == PLAN_SHA256, "V132 plan changed")
    plan = read_json(PLAN)
    _require(plan.get("formal_results_eligible") is False, "V132 eligibility changed")
    _require(PREPARED.is_file(), f"missing V132 prepared receipt: {PREPARED}")
    prepared = read_json(PREPARED)
    prepared_hash = _assert_hashed_object(
        prepared, "receipt_hash", "V132 prepared receipt"
    )
    _require(
        ...
        "V132 prepared scientific boundary changed",
    )
    ...
    for arm_id, expected in ARMS.items():
        ...
        _require(paths["ready"].is_file(), f"missing V132 ready manifest: {arm_id}")
        ...
        _require(
            ...
            f"V132 ready boundary changed: {arm_id}",
        )
        ...
        reference_catalog_hash = _assert_hashed_object(
            references, "catalog_hash", f"V132 references {arm_id}"
        )
        ...
        _require(
            ...
            f"V132 reference key set changed: {arm_id}",
        )
        for key, entry in sorted(entries.items()):
            ...
            _require(
                ...
                f"V132 reference evidence changed: {key}",
            )
        ...
        _require(actual_ids == expected_ids, f"V132 canonical set changed: {arm_id}")
        _require(
            not list((workspace / "quarantine").glob("**/attempt-*")),
            f"V132 online quarantine is nonempty: {arm_id}",
        )
        ...
        for run in manifest["runs"]:
            ...
            _require(
                ...
                f"V132 canonical status changed: {run['run_id']}",
            )
            runtime = {
    ...
    _require(len(run_evidence) == 18, "V132 run evidence count must be 18")
    _require(len(reference_evidence) == 18, "V132 reference evidence count must be 18")
    _require(len(tape_evidence) == 12, "V132 tape evidence count must be 12")
    ...
    _require(
        ...
        > 0,
        "V132 componentwise certificate did not exercise both accept and reject paths",
    )
    _require(
        all(
        ...
        "V132 anchor unexpectedly exercised the componentwise certificate",
    )
    _require(
        sum(
            row["admitted_interference_finite_window_count"]
        ...
        "V132 complete-interference certificate was unavailable or not exercised",
    )
    _require(
        all(
        ...
        "V132 anchor unexpectedly exercised the complete-interference certificate",
    )
    for field, expected in EXPECTED_RUNTIME.items():
        _require(
            runtime_values[field] == {expected},
            f"V132 runtime {field} changed: {runtime_values[field]}",
        )
    ...
    _require(
        ...
        f"V132 runtime git identity is not singular: {git_commits}",
    )
    _require(len(paired_inputs) == 9, "V132 paired input product changed")
    for (scenario, seed), rows in paired_inputs.items():
        _require(len(rows) == 2, f"V132 paired arm count changed: {scenario}/{seed}")
        for field in (
            ...
            _require(
                ...
                f"V132 paired {field} changed: {scenario}/{seed}",
            )
        _require(
            rows[0]["common_hpa_hash"] == EXPECTED_COMMON_HPA_SHA256,
            f"V132 common HPA changed: {scenario}/{seed}",
        )

    output_payload = {
        "schema_version": "NSE_E3_COMPONENTWISE_SERVICE_COMPLETE_INTERFERENCE_BLIND_AUDIT_V132_V1",
        "created_at": utc_now(),
        "status": "pass",
