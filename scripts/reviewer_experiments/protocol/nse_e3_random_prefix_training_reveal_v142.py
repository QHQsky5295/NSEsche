...
OUTPUT = ROOT / "training-result-v142.json"
...
def evaluate_training_rows(rows: Sequence[Mapping[str, Any]]) -> dict[str, Any]:
    ...
    for row in rows:
        ...
        if run_id in run_ids:
            raise ValueError(f"duplicate V142 run_id: {run_id}")
        run_ids.add(run_id)
    ...
    if len(rows) != 108 or actual != expected:
        raise ValueError(
            "V142 revealed product mismatch: "
...
def _validate_blind_audit() -> dict[str, Any]:
    ...
    if any(
        ...
        raise RuntimeError("V142 blind audit has not been frozen into reveal code")
    if not BLIND_AUDIT.is_file() or file_hash(BLIND_AUDIT) != BLIND_AUDIT_FILE_SHA256:
        raise RuntimeError("V142 blind audit is missing or changed")
    ...
    if (
        ...
        != RETIRED_OPENED_V140_TRAINING_SEEDS
        or blind.get("retired_opened_v141_training_seeds")
        != RETIRED_OPENED_V141_TRAINING_SEEDS
        ...
        raise RuntimeError("V142 blind audit does not authorize reveal")
    return blind
...
def _load_rows(blind: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    if len(admitted) != 108:
        raise RuntimeError("V142 blind audit run evidence is incomplete")
    rows = []
    for manifest_id in ["v142-baselines", *ARM_IDS]:
        ...
        if not manifest_path.is_file() or file_hash(manifest_path) != frozen.get(
            ...
            raise RuntimeError(f"V142 frozen manifest changed: {manifest_id}")
        manifest = load_and_validate_manifest(manifest_path)
        if manifest.get("manifest_hash") != frozen.get("manifest_hash"):
            raise RuntimeError(f"V142 manifest hash changed: {manifest_id}")
        ...
        for run in manifest["runs"]:
            ...
            if evidence is None or evidence.get("manifest_id") != manifest_id:
                raise RuntimeError(f"V142 run not admitted by blind audit: {run_id}")
            summary_path = (
            ...
            if not summary_path.is_file() or file_hash(summary_path) != evidence.get(
                ...
                raise RuntimeError(f"V142 summary differs from blind audit: {run_id}")
            summary = read_json(summary_path)
            if (
                summary.get("run_complete") is not True
                or summary.get("run_id") != run_id
            ):
                raise RuntimeError(f"V142 summary identity changed: {run_id}")
            rows.append(
                {
...
def execute_reveal() -> dict[str, Any]:
    if OUTPUT.exists():
        raise RuntimeError(f"refusing to overwrite V142 result: {OUTPUT}")
    if not PLAN.is_file() or file_hash(PLAN) != PLAN_SHA256:
        raise RuntimeError("V142 plan changed before reveal")
    blind = _validate_blind_audit()
    rows = _load_rows(blind)
    evaluation = evaluate_training_rows(rows)
    result = {
        "schema_version": "NSE_E3_RANDOM_PREFIX_TRAINING_RESULT_V142_V1",
        "created_at": utc_now(),
