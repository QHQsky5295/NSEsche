...
from scripts.reviewer_experiments.protocol.nse_e3_bidirectional_locality_safety_training_prepare_v104 import (
...
OUTPUT = ROOT / "joint-blind-audit-v104-training.json"
RESULT = ROOT / "training-result-v104.json"
CANONICAL_RENAME_RECEIPT = "canonical_rename_receipt_v104.json"
...
def _verify_tape_catalog() -> tuple[list[dict[str, Any]], dict[str, Any]]:
    _require(TAPES.is_file(), f"missing V104 tape catalog: {TAPES}")
    catalog = read_json(TAPES)
    catalog_hash = _assert_hashed_object(catalog, "catalog_hash", "V104 tapes")
    entries = catalog.get("entries")
    _require(
        isinstance(entries, dict) and len(entries) == 12,
        "V104 tape count changed",
    )
    evidence = []
...
def _verify_reference_stage_ledgers() -> tuple[dict[str, Path], dict[Path, str]]:
    ...
    for root, expected_count in expected_by_root.items():
        ...
        _assert_ledger_contract(
            ...
            f"V104 reference stage {root}",
        )
        _require(
            not list((root / "quarantine").glob("**/attempt-*")),
            f"V104 reference quarantine is nonempty: {root}",
        )
...
def _verify_canonical_rename_receipt(
    ...
    _require(receipt_path.is_file(), "missing V104 canonical rename receipt")
    receipt = read_json(receipt_path)
    receipt_hash = _assert_hashed_object(
        receipt, "receipt_hash", "V104 canonical rename receipt"
    )
    run_id = receipt.get("run_id")
    runs = {run["run_id"]: run for run in manifest["runs"]}
    _require(
        receipt.get("schema_version") == "NSE_V104_CANONICAL_RENAME_RECEIPT_V1"
        ...
        "V104 canonical rename boundary changed",
    )
    ...
    _require(
        ...
        "V104 canonical rename filesystem state changed",
    )
    ...
    _require(
        ...
        "V104 canonical rename file inventory malformed",
    )
    expected_files = {
    ...
    _require(
        ...
        "V104 canonical rename content tree changed",
    )
    _require(
        ...
        "V104 canonical rename terminal hashes changed",
    )
    return {
        "arm_id": arm_id,
        "run_id": run_id,
...
def run_blind_audit(output: Path = OUTPUT) -> dict[str, Any]:
    _require(not output.exists(), f"V104 blind audit already exists: {output}")
    _require(not RESULT.exists(), "V104 reveal exists before blind audit")
    _require(PLAN.is_file() and file_hash(PLAN) == PLAN_SHA256, "V104 plan changed")
    plan = read_json(PLAN)
    _require(plan.get("formal_results_eligible") is False, "V104 eligibility changed")
    _require(PREPARED.is_file(), f"missing V104 prepared receipt: {PREPARED}")
    prepared = read_json(PREPARED)
    prepared_hash = _assert_hashed_object(
        prepared, "receipt_hash", "V104 prepared receipt"
    )
    _require(
        ...
        "V104 prepared scientific boundary changed",
    )
    ...
    for arm_id, expected in ARMS.items():
        ...
        _require(paths["ready"].is_file(), f"missing V104 ready manifest: {arm_id}")
        manifest = load_and_validate_manifest(paths["ready"])
        _require(
            len(manifest["runs"]) == expected["run_count"],
            f"V104 run count changed: {arm_id}",
        )
        _require(
            ...
            f"V104 ready flags changed: {arm_id}",
        )
        ...
        reference_catalog_hash = _assert_hashed_object(
            references, "catalog_hash", f"V104 references {arm_id}"
        )
        ...
        _require(
            ...
            f"V104 reference key set changed: {arm_id}",
        )
        for key, entry in sorted(entries.items()):
        ...
        _require(actual_ids == expected_ids, f"V104 canonical set changed: {arm_id}")
        _require(
            not list((workspace / "quarantine").glob("**/attempt-*")),
            f"V104 online quarantine is nonempty: {arm_id}",
        )
        ...
        for run in manifest["runs"]:
            ...
            _require(
                ...
                and metadata.get("v104_arm_id") == arm_id
                and metadata.get("v104_arm_role") == expected["role"]
                and metadata.get("v104_candidate_profile") == expected["profile"]
                and metadata.get("v104_upper_queue_density_threshold")
                == expected["upper_queue_density_threshold"]
                and metadata.get("v104_nonterminal_queue_density_floor")
                == expected["nonterminal_queue_density_floor"]
                and metadata.get("v104_warm_admissibility")
                == expected["warm_admissibility"]
                and metadata.get("v104_load_least_window_certificate_mode")
                == expected["load_least_window_certificate_mode"]
                and metadata.get("v104_component_safety_mode")
                == expected["component_safety_mode"]
                and metadata.get("v104_scalar_faasrank_noninferiority")
                is (expected["role"] == "candidate")
                and metadata.get("v104_input_locality_component_noninferiority")
                is (expected["role"] == "candidate")
                and metadata.get("v104_componentwise_faasrank_noninferiority")
                is expected["component_safety_mode"].startswith("componentwise_")
                and metadata.get(
                    "v104_per_child_current_warm_downstream_locality_noninferiority"
                )
                is (expected["role"] == "candidate")
                and metadata.get(
                    "v104_downstream_locality_aggregate_compensation_allowed"
                )
                is False
                and metadata.get("v104_future_child_placement_or_feasibility_used")
                is False
                and metadata.get("v104_training_seed_metrics_previously_revealed")
                is False
                and metadata.get("v104_confirmation_seeds_opened") is False
                and metadata.get("v104_other_unopened_seeds_opened") is False,
                f"V104 run boundary changed: {run['run_id']}",
            )
            ...
            _require(
                ...
                f"V104 canonical status changed: {run['run_id']}",
            )
            runtime = {
    ...
    _require(len(run_evidence) == 27, "V104 run evidence count must be 27")
    _require(len(reference_evidence) == 27, "V104 reference evidence count must be 27")
    _require(len(tape_evidence) == 12, "V104 tape evidence count must be 12")
    _require(
        len(canonical_rename_receipts) <= len(ARMS),
        "V104 canonical rename receipt count exceeds arm count",
    )
    for field, expected in EXPECTED_RUNTIME.items():
        _require(
            runtime_values[field] == {expected},
            f"V104 runtime {field} changed: {runtime_values[field]}",
        )
    ...
    _require(
        ...
        f"V104 runtime git identity is not singular: {git_commits}",
    )
    for (experiment_id, scenario, seed), rows in paired_inputs.items():
        _require(
            len(rows) == 3,
            f"V104 paired arm count changed: {experiment_id}/{scenario}/{seed}",
        )
        for field in (
            ...
            _require(
                ...
                f"V104 paired {field} changed: {scenario}/{seed}",
            )
        _require(
            rows[0]["common_hpa_hash"] == EXPECTED_COMMON_HPA_SHA256,
            f"V104 common HPA changed: {scenario}/{seed}",
        )

    output_payload = {
        "schema_version": "NSE_E3_BIDIRECTIONAL_LOCALITY_SAFETY_BLIND_AUDIT_V104_V1",
        "created_at": utc_now(),
        "status": "pass",
