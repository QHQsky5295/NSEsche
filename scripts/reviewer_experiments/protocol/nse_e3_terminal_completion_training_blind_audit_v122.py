...
from scripts.reviewer_experiments.protocol.nse_e3_terminal_completion_training_prepare_v122 import (
    ...
    V121_RESULT_RECEIPT_COMMIT,
    V121_RESULT_RECEIPT_HASH,
)
...
PREPARED = ROOT / "prepared-manifests-v122.json"
TAPES = ROOT / "tapes.catalog.json"
OUTPUT = ROOT / "joint-blind-audit-v122-training.json"
RESULT = ROOT / "training-result-v122.json"
...
ARMS = {
    arm_id: {
        ...
        "terminal_completion_only": role == "candidate",
        "run_count": count,
    }
...
def _validate_admitted_work_diagnostics(
    ...
    """Validate V122 mechanism evidence without opening any performance summary."""

    path = canonical / "reviewer_records" / run["run_id"] / "nash_metrics.jsonl.gz"
    _require(path.is_file(), f"missing V122 Nash diagnostics: {run['run_id']}")
    ...
    with gzip.open(path, "rt", encoding="utf-8") as handle:
        for line_number, line in enumerate(handle, start=1):
            ...
            _require(type(frame) is int and frame >= 0, "invalid V122 window frame")

            cluster = event.get("cluster")
            _require(isinstance(cluster, dict), "missing V122 cluster diagnostics")
            admitted_fields = (
            ...
            if candidate:
                _require(
                    ...
                    f"V122 admitted-work observation changed: {run['run_id']}:{line_number}",
                )
                ...
                _require(
                    ...
                    f"V122 admitted-work snapshot counts changed: "
                ...
                _require(
                    ...
                    f"V122 admitted-work aggregate changed: {run['run_id']}:{line_number}",
                )
            ...
            _require(
                ...
                f"V122 critical-service contract changed: {run['run_id']}:{line_number}",
            )
            for field in (
                ...
                _require(
                    ...
                    f"V122 critical {field} changed: {run['run_id']}:{line_number}",
                )
            _require(
                not critical["accepted"] or critical["evaluated"],
                f"V122 accepted without evaluation: {run['run_id']}:{line_number}",
            )
            ...
            _require(
                ...
                == (
                    "current_terminal_critical_frontier_players"
                    if invoked
                    else "not_applicable"
                )
                and construction.get("uses_completion_outcomes") is False,
                f"V122 terminal construction contract changed: "
                f"{run['run_id']}:{line_number}",
            )
            terminal_scope = construction.get("terminal_completion_only")
            _require(
                isinstance(terminal_scope, dict)
                and terminal_scope.get("enabled") is invoked
                and terminal_scope.get("terminal_definition")
                == "function_children_is_empty_in_the_frozen_request_DAG"
                and terminal_scope.get("nonterminal_rule")
                == "exact_anchor_node_is_preserved"
                and terminal_scope.get("uses_completion_outcomes") is False,
                f"V122 terminal scope diagnostics changed: "
                f"{run['run_id']}:{line_number}",
            )
            terminal_fields = (
                "terminal_player_count",
                "nonterminal_player_count",
                "terminal_service_choice_target_player_count",
                "terminal_service_choice_evaluated_player_count",
                "terminal_service_choice_substitution_count",
                "nonterminal_exact_anchor_preserved_player_count",
                "nonterminal_substitution_count",
            )
            _require(
                all(
                    type(terminal_scope.get(field)) is int
                    and terminal_scope[field] >= 0
                    for field in terminal_fields
                ),
                f"V122 terminal scope counter changed: "
                f"{run['run_id']}:{line_number}",
            )
            if invoked:
                _require(
                    terminal_scope["terminal_service_choice_target_player_count"]
                    <= terminal_scope["terminal_player_count"]
                    and terminal_scope["terminal_service_choice_evaluated_player_count"]
                    <= terminal_scope["terminal_service_choice_target_player_count"]
                    and terminal_scope["terminal_service_choice_substitution_count"]
                    <= terminal_scope["terminal_service_choice_evaluated_player_count"]
                    and terminal_scope[
                        "nonterminal_exact_anchor_preserved_player_count"
                    ]
                    <= terminal_scope["nonterminal_player_count"]
                    and terminal_scope["nonterminal_substitution_count"] == 0,
                    f"V122 terminal-only substitution invariant changed: "
                    f"{run['run_id']}:{line_number}",
                )
            else:
                _require(
                    all(terminal_scope[field] == 0 for field in terminal_fields),
                    f"V122 inactive terminal scope leaked counters: "
                    f"{run['run_id']}:{line_number}",
                )
            if not invoked:
                _require(
                    ...
                    f"V122 non-invoked critical state changed: {run['run_id']}:{line_number}",
                )
            if critical["accepted"]:
                ...
                _require(
                    ...
                    f"V122 accepted evidence is nonfinite: {run['run_id']}:{line_number}",
                )
                ...
                _require(
                    ...
                    f"V122 accepted admitted-work certificate changed: "
                    f"{run['run_id']}:{line_number}",
                )
            ...
            counts["terminal_players"] += terminal_scope["terminal_player_count"]
            counts["nonterminal_players"] += terminal_scope["nonterminal_player_count"]
            counts["terminal_service_targets"] += terminal_scope[
                "terminal_service_choice_target_player_count"
            ]
            counts["terminal_service_evaluations"] += terminal_scope[
                "terminal_service_choice_evaluated_player_count"
            ]
            counts["terminal_service_substitutions"] += terminal_scope[
                "terminal_service_choice_substitution_count"
            ]
            counts["nonterminal_anchor_preserved"] += terminal_scope[
                "nonterminal_exact_anchor_preserved_player_count"
            ]
            counts["nonterminal_substitutions"] += terminal_scope[
                "nonterminal_substitution_count"
            ]
            ...
            if not candidate:
                _require(
                    ...
                    f"V122 anchor used causal state: {run['run_id']}:{line_number}",
                )
                continue
            if not gate["history_complete"]:
                _require(
                    gate["baseline_count"] == 0 and gate["recent_count"] == 0,
                    f"V122 incomplete history leaked counts: {run['run_id']}:{line_number}",
                )
                continue
            sufficient = (
            ...
            if sufficient and recent_scaled >= baseline_scaled:
                ...
                _require(
                    ...
                    f"V122 exact threshold did not activate: "
                    f"{run['run_id']}:{line_number}",
                )

    _require(
        counts["windows"] == run["simulation"]["total_frame"],
        f"V122 diagnostic window count changed: {run['run_id']}",
    )
    if candidate:
        _require(
            ...
            f"V122 mechanism was not exercised: {run['run_id']}",
        )
    return {
        ...
        "terminal_player_count": counts["terminal_players"],
        "nonterminal_player_count": counts["nonterminal_players"],
        "terminal_service_choice_target_player_count": counts[
            "terminal_service_targets"
        ],
        "terminal_service_choice_evaluated_player_count": counts[
            "terminal_service_evaluations"
        ],
        "terminal_service_choice_substitution_count": counts[
            "terminal_service_substitutions"
        ],
        "nonterminal_exact_anchor_preserved_player_count": counts[
            "nonterminal_anchor_preserved"
        ],
        "nonterminal_substitution_count": counts["nonterminal_substitutions"],
...
def _verify_tape_catalog() -> tuple[list[dict[str, Any]], dict[str, Any]]:
    _require(TAPES.is_file(), f"missing V122 tape catalog: {TAPES}")
    catalog = read_json(TAPES)
    catalog_hash = _assert_hashed_object(catalog, "catalog_hash", "V122 tapes")
    entries = catalog.get("entries")
    _require(
        isinstance(entries, dict) and len(entries) == 12, "V122 tape count changed"
    )
    evidence = []
...
def _verify_reference_stage_ledgers() -> tuple[dict[str, Path], dict[Path, str]]:
    ...
    for arm_id, expected in ARMS.items():
        ...
        root = _stage_root_from_receipts(
            entries, "receipt_path", expected["run_count"], f"V122 reference {arm_id}"
        )
        roots[arm_id] = root
    ...
    for root, expected_count in expected_by_root.items():
        ...
        _assert_ledger_contract(
            ...
            f"V122 reference stage {root}",
        )
        _require(
            not list((root / "quarantine").glob("**/attempt-*")),
            f"V122 reference quarantine is nonempty: {root}",
        )
...
def _validate_run_boundary(
    ...
    required = {
        "v122_training_plan_sha256": PLAN_SHA256,
        "v122_training_only": True,
        "v122_training_seed_metrics_previously_revealed": False,
        "v122_confirmation_seeds_opened": False,
        "v122_other_unopened_seeds_opened": False,
        "v122_formal_E01_E20_reexecution": False,
        "v122_arm_id": arm_id,
        "v122_arm_role": expected["role"],
        "v122_candidate_profile": expected["profile"],
        "v122_candidate_experiment": "E3",
        "v122_shock_rate_ratio": expected["shock_rate_ratio"],
        "v122_shock_threshold_numerator": expected["shock_threshold_numerator"],
        "v122_shock_threshold_denominator": expected["shock_threshold_denominator"],
        "v122_arrival_history_baseline_frames": 80 if candidate else None,
        "v122_arrival_history_recent_frames": 20 if candidate else None,
        "v122_arrival_min_requests_per_window": 20 if candidate else None,
        "v122_shock_activation_horizon_frames": expected[
            "shock_activation_horizon_frames"
        ],
        "v122_nonterminal_queue_density_floor": 8.0 if candidate else None,
        "v122_warm_admissibility": "preserve_anchor_warmness" if candidate else None,
        "v122_load_least_window_certificate_mode": (
            "disabled" if candidate else "not_applicable"
        ),
        "v122_critical_service_ratio_numerator": expected[
            "critical_service_threshold_numerator"
        ],
        "v122_critical_service_ratio_denominator": expected[
            "critical_service_threshold_denominator"
        ],
        "v122_critical_service_proxy": (
            "remote_parent_transfer_plus_cold_start_plus_admitted_queue_cpu_work"
            if candidate
            else "not_applicable"
        ),
        "v122_service_proxy_work_source": (
            ADMITTED_WORK_SOURCE if candidate else "not_applicable"
        ),
        "v122_admitted_work_includes_all_blocked_resident": candidate,
        "v122_admitted_work_deterministic_f64_sum": candidate,
        "v122_critical_service_proxy_inputs_finite_fail_closed": candidate,
        "v122_cpu_memory_individual_noninferiority": candidate,
        "v122_scalar_faasrank_noninferiority": candidate,
        "v122_input_locality_component_noninferiority": candidate,
        "v122_per_child_current_warm_downstream_locality_noninferiority": candidate,
        "v122_critical_frontier_substitution": candidate,
        "v122_critical_frontier_rank_source": (
            "immutable_srpt_remaining_critical_path_rank"
            if candidate
            else "not_applicable"
        ),
        "v122_terminal_completion_only_substitution_scope": candidate,
        "v122_terminal_definition": (
            "function_children_is_empty_in_the_frozen_request_DAG"
            if candidate
            else "not_applicable"
        ),
        "v122_terminal_critical_frontier_substitution": candidate,
        "v122_nonterminal_exact_anchor": candidate,
        "v122_complete_summed_critical_service_proxy_strictly_lower": candidate,
        "v122_complete_routed_score_nonworse": candidate,
        "v122_complete_exact_ocs_score_nonworse": candidate,
        "v122_complete_immutable_baseline_welfare_nonworse": candidate,
        "v122_outcome_fields_drive_policy": False,
        "v122_scenario_or_burst_label_used_by_policy": False,
        "v122_completion_or_performance_fields_used_by_policy": False,
        "v122_future_arrivals_used_by_policy": False,
    }
    _require(
        ...
        f"V122 run boundary changed: {run['run_id']}",
    )


def run_blind_audit(output: Path = OUTPUT) -> dict[str, Any]:
    _require(not output.exists(), f"V122 blind audit already exists: {output}")
    _require(not RESULT.exists(), "V122 reveal exists before blind audit")
    _require(PLAN.is_file() and file_hash(PLAN) == PLAN_SHA256, "V122 plan changed")
    plan = read_json(PLAN)
    _require(plan.get("formal_results_eligible") is False, "V122 eligibility changed")
    _require(PREPARED.is_file(), f"missing V122 prepared receipt: {PREPARED}")
    prepared = read_json(PREPARED)
    prepared_hash = _assert_hashed_object(
        prepared, "receipt_hash", "V122 prepared receipt"
    )
    _require(
        ...
        and prepared.get("V121_result_receipt_hash") == V121_RESULT_RECEIPT_HASH
        and prepared.get("V121_result_receipt_commit") == V121_RESULT_RECEIPT_COMMIT
        ...
        "V122 prepared scientific boundary changed",
    )
    ...
    for arm_id, expected in ARMS.items():
        ...
        _require(paths["ready"].is_file(), f"missing V122 ready manifest: {arm_id}")
        ...
        _require(
            ...
            f"V122 ready boundary changed: {arm_id}",
        )
        ...
        reference_catalog_hash = _assert_hashed_object(
            references, "catalog_hash", f"V122 references {arm_id}"
        )
        ...
        _require(
            ...
            f"V122 reference key set changed: {arm_id}",
        )
        for key, entry in sorted(entries.items()):
            ...
            _require(
                ...
                f"V122 reference evidence changed: {key}",
            )
        ...
        _require(actual_ids == expected_ids, f"V122 canonical set changed: {arm_id}")
        _require(
            not list((workspace / "quarantine").glob("**/attempt-*")),
            f"V122 online quarantine is nonempty: {arm_id}",
        )
        ...
        for run in manifest["runs"]:
            ...
            _require(
                ...
                f"V122 canonical status changed: {run['run_id']}",
            )
            runtime = {
    ...
    _require(len(run_evidence) == 18, "V122 run evidence count must be 18")
    _require(len(reference_evidence) == 18, "V122 reference evidence count must be 18")
    _require(len(tape_evidence) == 12, "V122 tape evidence count must be 12")
    for field, expected in EXPECTED_RUNTIME.items():
        _require(
            runtime_values[field] == {expected},
            f"V122 runtime {field} changed: {runtime_values[field]}",
        )
    ...
    _require(
        ...
        f"V122 runtime git identity is not singular: {git_commits}",
    )
    _require(len(paired_inputs) == 9, "V122 paired input product changed")
    for (scenario, seed), rows in paired_inputs.items():
        _require(len(rows) == 2, f"V122 paired arm count changed: {scenario}/{seed}")
        for field in (
            ...
            _require(
                ...
                f"V122 paired {field} changed: {scenario}/{seed}",
            )
        _require(
            rows[0]["common_hpa_hash"] == EXPECTED_COMMON_HPA_SHA256,
            f"V122 common HPA changed: {scenario}/{seed}",
        )
    ...
    terminal_player_count = sum(
        row["terminal_player_count"] for row in candidate_diagnostics
    )
    nonterminal_player_count = sum(
        row["nonterminal_player_count"] for row in candidate_diagnostics
    )
    terminal_service_choice_evaluated_count = sum(
        row["terminal_service_choice_evaluated_player_count"]
        for row in candidate_diagnostics
    )
    terminal_substitution_count = sum(
        row["terminal_service_choice_substitution_count"]
        for row in candidate_diagnostics
    )
    nonterminal_exact_anchor_preserved_count = sum(
        row["nonterminal_exact_anchor_preserved_player_count"]
        for row in candidate_diagnostics
    )
    nonterminal_substitution_count = sum(
        row["nonterminal_substitution_count"] for row in candidate_diagnostics
    )
    ...
    _require(
        ...
        and terminal_player_count > 0
        and nonterminal_player_count > 0,
        "V122 terminal and nonterminal current-player paths were not both exercised",
    )
    _require(
        terminal_service_choice_evaluated_count > 0 and terminal_substitution_count > 0,
        "V122 terminal service-choice evaluation/substitution path was not exercised",
    )
    _require(
        nonterminal_exact_anchor_preserved_count > 0
        and nonterminal_substitution_count == 0,
        "V122 nonterminal exact-anchor preservation contract changed",
    )
    _require(
        proxy_input_unavailable_count == 0,
        "V122 admitted-work proxy inputs were unavailable",
    )

    output_payload = {
        "schema_version": "NSE_E3_TERMINAL_COMPLETION_BLIND_AUDIT_V122_V1",
        "created_at": utc_now(),
        "status": "pass",
        ...
        "terminal_completion_mechanism_coverage": {
            "terminal_player_count": terminal_player_count,
            "nonterminal_player_count": nonterminal_player_count,
            "terminal_service_choice_evaluated_count": terminal_service_choice_evaluated_count,
            "terminal_substitution_count": terminal_substitution_count,
            "nonterminal_exact_anchor_preserved_count": nonterminal_exact_anchor_preserved_count,
            "nonterminal_substitution_count": nonterminal_substitution_count,
