...
ROOT = Path("tmp/nse_e3_resource_headroom_safety_training_20260829_v106")
PLAN = Path(
    "scripts/reviewer_experiments/protocol/"
    "nse_e3_resource_headroom_safety_training_plan_v106.json"
)
PLAN_SHA256 = "0db9f43e01f1c9739722751e9416baa5908140a29838ec9b439a24b1ad17a315"
V105_RESULT_RECEIPT = Path(
    "scripts/reviewer_experiments/protocol/"
    "nse_e3_critical_frontier_safety_training_result_v105.json"
)
V105_RESULT_RECEIPT_SHA256 = (
    "bc4e77953efe18be7a106406f21225349f3e9aab63eac67d1de8d147f6b2939c"
)
V105_SOURCE_RESULT = Path(
    "tmp/nse_e3_critical_frontier_safety_training_20260829_v105/"
    "training-result-v105.json"
)
V105_SOURCE_RESULT_SHA256 = (
    "8de9f50657a184e5262422d718ae5959fc33d5e8dadc6cb34197e641f6e57a6f"
)
FORMAL_RESULT = Path(
...
BINARY_PATH = Path("tmp/nse_v106_build_91e001a/release/serverless_sim.exe")
BINARY_SHA256 = "7a2c2f4b1eecbdf175221f6ddbf9801b756081d73ad3dbc5250018dbf0568f88"
BINARY_SOURCE_COMMIT = "91e001a54e0986c92e6c50920726e241ccdd8142"
...
SOURCE_INITIAL_SEEDS = [f"E{index}" for index in range(887, 897)]
SOURCE_CI_SEEDS = [f"E{index}" for index in range(897, 907)]
TRAINING_SEED_LIST = ["E887", "E888", "E889"]
TRAINING_SEEDS = set(TRAINING_SEED_LIST)
CONFIRMATION_SEEDS = [f"E{index}" for index in range(1006, 1026)]
PREVIOUS_CONFIRMATION_SEEDS = [f"E{index}" for index in range(926, 1006)]
OTHER_UNOPENED_SEEDS = [
    ...
    *[f"E{index}" for index in range(890, 926)],
]
PORT = "3141"

E3_ANCHOR = "faasrank_native_faithful_terminal_ocs_srpt_ready_dual_window_safe_pareto"
E3_BAND8_TO24_RESOURCE_PARETO_INITIALIZER_ONLY = (
    "faasrank_native_faithful_terminal_ocs_srpt_ready_"
    "terminal_load_band8_24_bidirectional_locality_noncritical_frontier_resource_pareto_initializer_only_guard64_dual_window_safe_pareto"
)
E3_BAND8_TO24_RESOURCE_BOTTLENECK_SUM_INITIALIZER_ONLY = (
    "faasrank_native_faithful_terminal_ocs_srpt_ready_"
    "terminal_load_band8_24_bidirectional_locality_noncritical_frontier_resource_bottleneck_sum_initializer_only_guard64_dual_window_safe_pareto"
)
ARMS = (
    ("v106-e3-anchor", "E3", "anchor", E3_ANCHOR, None, "not_applicable", 9),
    (
        "v106-e3-band8-24-bidirectional-locality-noncritical-frontier-resource-pareto-initializer-only",
        "E3",
        "candidate",
        E3_BAND8_TO24_RESOURCE_PARETO_INITIALIZER_ONLY,
        24.0,
    ...
    (
        "v106-e3-band8-24-bidirectional-locality-noncritical-frontier-resource-bottleneck-sum-initializer-only",
        "E3",
        "candidate",
        E3_BAND8_TO24_RESOURCE_BOTTLENECK_SUM_INITIALIZER_ONLY,
        24.0,
        "resource_bottleneck_and_total_headroom_each_nonworse",
        9,
    ),
)
...
def _paths(root: Path) -> dict[str, Path]:
    return {
        "config": root / "v106-e3-training-config.json",
        "source": root / "manifest.v106-e887-e896-full.unbound.json",
        "capture": root / "manifest.v106-tape-capture.unbound.json",
        "prepared": root / "prepared-manifests-v106.json",
    }
...
def _assert_frozen_inputs() -> None:
    for path, expected in (
        (PLAN, PLAN_SHA256),
        (V105_RESULT_RECEIPT, V105_RESULT_RECEIPT_SHA256),
        (V105_SOURCE_RESULT, V105_SOURCE_RESULT_SHA256),
        ...
        if not path.is_file() or file_hash(path) != expected:
            raise RuntimeError(f"frozen V106 input is missing or changed: {path}")
...
def _rewrite(
    ...
    *,
    ...
    marker.update(
        {
            "purpose": purpose,
            "v106_training_plan_sha256": PLAN_SHA256,
            "v106_training_only": True,
            "v106_binary_source_commit": BINARY_SOURCE_COMMIT,
    ...
    if arm is not None:
        ...
        marker.update(
            {
                "v106_arm_id": arm_id,
                "v106_experiment_id": experiment_id,
                "v106_role": role,
                "v106_profile": profile,
                "v106_upper_queue_density_threshold": upper_density,
                "v106_nonterminal_queue_density_floor": (
                    8.0 if role == "candidate" else None
                ),
                "v106_warm_admissibility": (
                    "preserve_anchor_warmness" if role == "candidate" else None
                ),
                "v106_load_least_window_certificate_mode": (
                    "disabled" if role == "candidate" else "not_applicable"
                ),
                "v106_resource_headroom_safety_mode": resource_headroom_safety_mode,
                "v106_cpu_memory_individual_noninferiority": (
                    resource_headroom_safety_mode
                    == "cpu_and_memory_headroom_each_nonworse"
                ),
                "v106_resource_bottleneck_sum_noninferiority": (
                    resource_headroom_safety_mode
                    == "resource_bottleneck_and_total_headroom_each_nonworse"
                ),
                "v106_resource_inputs_finite_fail_closed": role == "candidate",
                "v106_critical_frontier_protection": role == "candidate",
                "v106_critical_frontier_rank_source": (
                    "immutable_srpt_remaining_critical_path_rank"
                    if role == "candidate"
                    else "not_applicable"
                ),
                "v106_critical_frontier_tie_rule": (
                    "protect_missing_nonfinite_singleton_or_rank_plus_epsilon_ge_request_frontier_maximum"
                    if role == "candidate"
                    else "not_applicable"
                ),
                "v106_expected_run_count": expected,
            }
        )
        ...
        metadata.update(
            {
                "v106_training_plan_sha256": PLAN_SHA256,
                "v106_training_only": True,
                "v106_training_seed_metrics_previously_revealed": False,
                "v106_binary_source_commit": BINARY_SOURCE_COMMIT,
                "v106_confirmation_seeds_opened": False,
                "v106_other_unopened_seeds_opened": False,
                "v106_formal_E01_E20_reexecution": False,
            }
        )
        if arm is not None:
            (
            ...
            metadata.update(
                {
                    "v106_arm_id": arm_id,
                    "v106_arm_role": role,
                    "v106_candidate_profile": profile,
                    "v106_candidate_experiment": experiment_id,
                    "v106_upper_queue_density_threshold": upper_density,
                    "v106_nonterminal_queue_density_floor": (
                        8.0 if role == "candidate" else None
                    ),
                    ...
                    "v106_faasrank_model_artifact_sha256": MODEL_SHA256,
                    "v106_native_faithful_initializer": True,
                    "v106_dual_window_safe_pareto": True,
                    "v106_density_band_initializer_guard": role == "candidate",
                    "v106_terminal_players_included_without_lower_floor": role
                    == "candidate",
                    "v106_srpt_critical_path_player_order": "srpt_ready" in profile,
                    "v106_substitution_cap": None,
                    "v106_load_least_window_certificate_mode": certificate_mode,
                    "v106_resource_headroom_safety_mode": resource_headroom_safety_mode,
                    "v106_cpu_memory_individual_noninferiority": (
                        resource_headroom_safety_mode
                        == "cpu_and_memory_headroom_each_nonworse"
                    ...
                    "v106_resource_inputs_finite_fail_closed": role == "candidate",
                    "v106_scalar_faasrank_noninferiority": role == "candidate",
                    "v106_input_locality_component_noninferiority": role == "candidate",
                    "v106_componentwise_faasrank_noninferiority": False,
                    "v106_per_child_current_warm_downstream_locality_noninferiority": role
                    == "candidate",
                    "v106_downstream_locality_aggregate_compensation_allowed": False,
                    "v106_future_child_placement_or_feasibility_used": False,
                    "v106_critical_frontier_protection": role == "candidate",
                    "v106_critical_frontier_rank_source": (
                        "immutable_srpt_remaining_critical_path_rank"
                        if role == "candidate"
                    ...
                    "v106_only_strictly_lower_rank_parallel_players_may_substitute": role
                    == "candidate",
                    "v106_outcome_fields_drive_policy": False,
                }
            )
...
def prepare_v106(root: Path = ROOT) -> dict:
    _assert_frozen_inputs()
    if root.exists():
        raise RuntimeError(f"refusing to overwrite V106 training root: {root}")
    ...
    capture = derive_integration_smoke_shard(
        paths["source"],
        capture_ids,
        purpose=f"V106 fresh balanced-QoS parent tapes; plan_sha256={PLAN_SHA256}",
    )
    _write(
        paths["capture"],
        _rewrite(
    ...
    for arm in ARMS:
        ...
        template = derive_integration_smoke_shard(
            paths["source"],
            selected,
            purpose=f"V106 {arm_id} resource-headroom safety training; plan_sha256={PLAN_SHA256}",
        )
        manifest = _rewrite(
            template,
            purpose=f"V106 {arm_id} resource-headroom safety training; plan_sha256={PLAN_SHA256}",
        ...
        if (
            ...
            raise RuntimeError(f"V106 {arm_id} must have {expected} runs/references")
        if {run["seed"] for run in manifest["runs"]} != TRAINING_SEEDS:
            raise RuntimeError(f"V106 {arm_id} escaped E887-E889")
        keys = {item["key"] for item in manifest["reference_build_dependencies"]}
        if all_reference_keys & keys:
            raise RuntimeError("V106 arm-specific reference keys unexpectedly overlap")
        all_reference_keys.update(keys)
        _write(arm_path(root, arm_id), manifest)
    if len(all_reference_keys) != 27:
        raise RuntimeError("V106 must declare exactly 27 arm-specific references")
    ...
    receipt = {
        "schema_version": "NSE_E3_RESOURCE_HEADROOM_SAFETY_PREPARED_V106_V1",
        "created_at": utc_now(),
        "formal_results_eligible": False,
        ...
        "V105_result_receipt_path": str(V105_RESULT_RECEIPT),
        "V105_result_receipt_sha256": V105_RESULT_RECEIPT_SHA256,
        "V105_source_result_path": str(V105_SOURCE_RESULT),
        "V105_source_result_sha256": V105_SOURCE_RESULT_SHA256,
        ...
        "arms": [
            {
                ...
                "resource_headroom_safety_mode": resource_headroom_safety_mode,
            ...
            for arm_id, experiment_id, role, profile, upper_density, resource_headroom_safety_mode, expected in ARMS
        ],
