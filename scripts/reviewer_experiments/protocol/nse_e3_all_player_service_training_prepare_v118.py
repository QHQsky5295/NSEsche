...
ROOT = Path("tmp/nse_e3_all_player_service_training_20260829_v118")
PLAN = Path(
    "scripts/reviewer_experiments/protocol/"
    "nse_e3_all_player_service_training_plan_v118.json"
)
PLAN_SHA256 = "4aa16edfee7a69e8ba27f0615953ace5f3b4886eb46d831dc8ab99c4969c33a0"
...
V117_RESULT_RECEIPT = Path(
    "scripts/reviewer_experiments/protocol/"
    "nse_e3_componentwise_service_training_result_v117.json"
)
V117_RESULT_RECEIPT_SHA256 = (
    "f96af561e6a645c04d70fb8325771ed7907cda7420e11957ef6cb2a0ada00448"
)
BINARY_PATH = Path("tmp/nse_v118_build_133d65c/release/serverless_sim.exe")
BINARY_SHA256 = "f4d6f425342e1fb3272618227fa6676618e0557e2c10a730ba539ac1e23c825c"
BINARY_SOURCE_COMMIT = "133d65c7780a806b517f116e763023fa4d339e54"

SOURCE_INITIAL_SEEDS = [f"E{index}" for index in range(923, 933)]
SOURCE_CI_SEEDS = [f"E{index}" for index in range(933, 943)]
TRAINING_SEED_LIST = ["E923", "E924", "E925"]
...
E3_ALL_PLAYER_SERVICE = (
    ...
    "all_player_componentwise_service_pareto_initializer_only_guard64_dual_window_safe_pareto"
)
ARMS = (
    ("v118-e3-anchor", "anchor", E3_ANCHOR, 9),
    (
        "v118-e3-causal-shock15-horizon50-admitted-work10-all-player-service-initializer-only",
        "candidate",
        E3_ALL_PLAYER_SERVICE,
        9,
    ),
...
def _paths(root: Path) -> dict[str, Path]:
    return {
        "config": root / "v118-e3-training-config.json",
        "source": root / "manifest.v118-e923-e932-full.unbound.json",
        "capture": root / "manifest.v118-tape-capture.unbound.json",
...
def _assert_frozen_inputs() -> None:
    for path, expected in (
        ...
        (V117_RESULT_RECEIPT, V117_RESULT_RECEIPT_SHA256),
        ...
        if not path.is_file() or file_hash(path) != expected:
            raise RuntimeError(f"frozen V118 input is missing or changed: {path}")
    ...
    *,
    ...
    marker.update(
        {
            ...
            "v118_binary_source_commit": BINARY_SOURCE_COMMIT,
    ...
    if arm is not None:
        if frozen_model_config is None:
            raise RuntimeError("V118 arm rewrite lacks frozen model binding")
        arm_id, role, profile, expected = arm
        candidate = role == "candidate"
        marker.update(
            {
                "v118_arm_id": arm_id,
                "v118_experiment_id": "E3",
                "v118_role": role,
                "v118_profile": profile,
                "v118_shock_threshold_numerator": 3 if candidate else None,
                "v118_shock_threshold_denominator": 2 if candidate else None,
                "v118_shock_activation_horizon_frames": 50 if candidate else None,
                "v118_critical_service_ratio_numerator": 9 if candidate else None,
                "v118_critical_service_ratio_denominator": 10 if candidate else None,
                ...
                "v118_expected_run_count": expected,
            ...
            metadata.update(
                {
                    "v118_arm_id": arm_id,
                    "v118_arm_role": role,
                    "v118_candidate_profile": profile,
                    "v118_candidate_experiment": "E3",
                    "v118_shock_rate_ratio": "3/2" if candidate else None,
                    "v118_shock_threshold_numerator": 3 if candidate else None,
                    "v118_shock_threshold_denominator": 2 if candidate else None,
                    ...
                    "v118_faasrank_model_artifact_sha256": MODEL_SHA256,
                    "v118_critical_service_ratio_numerator": 9 if candidate else None,
                    "v118_critical_service_ratio_denominator": 10
                    if candidate
                    else None,
...
def prepare_v118(root: Path = ROOT) -> dict:
    _assert_frozen_inputs()
    if root.exists():
        raise RuntimeError(f"refusing to overwrite V118 training root: {root}")
    ...
    capture = derive_integration_smoke_shard(
        paths["source"], capture_ids, purpose=f"V118 fresh parent tapes {PLAN_SHA256}"
    )
    _write(
        paths["capture"],
        _rewrite(capture, purpose=f"V118 fresh parent tapes {PLAN_SHA256}"),
    )
    ...
    for arm in ARMS:
        ...
        template = derive_integration_smoke_shard(
            paths["source"], selected, purpose=f"V118 {arm_id} {PLAN_SHA256}"
        )
        manifest = _rewrite(
            template,
            purpose=f"V118 {arm_id} {PLAN_SHA256}",
            arm=arm,
        ...
        if (
            ...
            raise RuntimeError(f"V118 {arm_id} must have 9 runs/references")
        if {run["seed"] for run in manifest["runs"]} != TRAINING_SEEDS:
            raise RuntimeError(f"V118 {arm_id} escaped E923-E925")
        keys = {item["key"] for item in manifest["reference_build_dependencies"]}
        if all_reference_keys & keys:
            raise RuntimeError("V118 arm-specific reference keys unexpectedly overlap")
        all_reference_keys.update(keys)
        _write(arm_path(root, arm_id), manifest)
    if len(all_reference_keys) != 18:
        raise RuntimeError("V118 must declare exactly 18 arm-specific references")
    ...
    receipt = {
        "schema_version": "NSE_E3_ALL_PLAYER_SERVICE_PREPARED_V118_V1",
        "created_at": utc_now(),
        "formal_results_eligible": False,
        ...
        "V117_result_receipt_path": str(V117_RESULT_RECEIPT),
        "V117_result_receipt_sha256": V117_RESULT_RECEIPT_SHA256,
