...
from scripts.reviewer_experiments.protocol.nse_e3_readiness_stratified_work_training_prepare_v135 import (
...
PREPARED = ROOT / "prepared-manifests-v135.json"
TAPES = ROOT / "tapes.catalog.json"
OUTPUT = ROOT / "joint-blind-audit-v135-training.json"
RESULT = ROOT / "training-result-v135.json"
...
READINESS_STRATIFIED_WORK_SOURCE = "pending_immutable_plus_runnable_immutable_plus_blocked_resident_remaining_plus_same_window_projected_immutable_cpu_v1"
...
ARMS = {
    arm_id: {
        ...
        "work_source": READINESS_STRATIFIED_WORK_SOURCE
        if role == "candidate"
...
def _validate_readiness_stratified_work_diagnostics(
    run: dict[str, Any], canonical: Path, expected: dict[str, Any]
) -> dict[str, Any]:
    """Validate V135 mechanism evidence without opening any performance summary."""

    path = canonical / "reviewer_records" / run["run_id"] / "nash_metrics.jsonl.gz"
    _require(path.is_file(), f"missing V135 Nash diagnostics: {run['run_id']}")
    candidate = expected["role"] == "candidate"
    counts = Counter()
    pending_immutable_total_sum = 0.0
    runnable_immutable_total_sum = 0.0
    runnable_remaining_total_sum = 0.0
    resident_remaining_total_sum = 0.0
    blocked_resident_remaining_total_sum = 0.0
    readiness_stratified_total_sum = 0.0
    with gzip.open(path, "rt", encoding="utf-8") as handle:
        for line_number, line in enumerate(handle, start=1):
            ...
            _require(type(frame) is int and frame >= 0, "invalid V135 window frame")

            cluster = event.get("cluster")
            _require(isinstance(cluster, dict), "missing V135 cluster diagnostics")
            work_fields = (
                "queue_pending_cpu_work_total",
                "queue_pending_cpu_work_max",
                "queue_runnable_cpu_work_total",
                "queue_runnable_cpu_work_max",
                "queue_runnable_remaining_cpu_work_total",
                "queue_runnable_remaining_cpu_work_max",
                ...
                "queue_readiness_stratified_cpu_work_total",
                "queue_readiness_stratified_cpu_work_max",
            )
            if candidate:
                _require(
                    cluster.get("queue_cpu_work_observation_complete") is True
                    and cluster.get("queue_admitted_work_observation_complete") is True
                    and cluster.get("queue_remaining_cpu_work_observation_complete")
                    is True
                    and cluster.get(
                        "queue_readiness_stratified_work_observation_complete"
                    )
                    is True
                    and all(
                        _finite_nonnegative(cluster.get(field)) for field in work_fields
                    ),
                    f"V135 readiness-stratified-work observation changed: {run['run_id']}:{line_number}",
                )
                pending_count = cluster.get("queue_pending_cpu_value_count")
                runnable_count = cluster.get("queue_runnable_remaining_cpu_value_count")
                ...
                _require(
                    ...
                    and type(runnable_count) is int
                    and runnable_count >= 0
                    and runnable_count == cluster.get("queue_runnable_total")
                    and type(resident_count) is int
                    and resident_count >= 0
                    and resident_count == cluster.get("queue_resident_total"),
                    f"V135 readiness-stratified-work snapshot counts changed: "
                    f"{run['run_id']}:{line_number}",
                )
                _require(
                    all(
                        float(cluster[total]) + 1.0e-9 >= float(cluster[maximum])
                    ...
                    f"V135 readiness-stratified-work aggregate changed: {run['run_id']}:{line_number}",
                )
                pending = float(cluster["queue_pending_cpu_work_total"])
                runnable = float(cluster["queue_runnable_cpu_work_total"])
                runnable_remaining = float(
                    cluster["queue_runnable_remaining_cpu_work_total"]
                )
                resident_remaining = float(
                    cluster["queue_resident_remaining_cpu_work_total"]
                )
                blocked_remaining = float(
                    cluster["queue_blocked_resident_remaining_cpu_work_total"]
                )
                stratified = float(cluster["queue_readiness_stratified_cpu_work_total"])
                _require(
                    math.isclose(
                        blocked_remaining,
                        resident_remaining - runnable_remaining,
                        rel_tol=1.0e-12,
                        abs_tol=1.0e-8,
                    )
                    and math.isclose(
                        stratified,
                        pending + runnable + blocked_remaining,
                        rel_tol=1.0e-12,
                        abs_tol=1.0e-8,
                    ),
                    f"V135 readiness partition arithmetic changed: "
                    f"{run['run_id']}:{line_number}",
                )
                pending_immutable_total_sum += pending
                runnable_immutable_total_sum += runnable
                runnable_remaining_total_sum += runnable_remaining
                resident_remaining_total_sum += resident_remaining
                blocked_resident_remaining_total_sum += blocked_remaining
                readiness_stratified_total_sum += stratified
                counts["readiness_stratified_work_complete_windows"] += 1
                counts["pending_cpu_value_count"] += pending_count
                counts["runnable_remaining_cpu_value_count"] += runnable_count
            ...
            _require(
                ...
                f"V135 critical-service contract changed: {run['run_id']}:{line_number}",
            )
            for field in (
                ...
                _require(
                    ...
                    f"V135 critical {field} changed: {run['run_id']}:{line_number}",
                )
            _require(
                not critical["accepted"] or critical["evaluated"],
                f"V135 accepted without evaluation: {run['run_id']}:{line_number}",
            )
            if not invoked:
                _require(
                    ...
                    f"V135 non-invoked critical state changed: {run['run_id']}:{line_number}",
                )
            if critical["accepted"]:
                ...
                _require(
                    ...
                    f"V135 accepted evidence is nonfinite: {run['run_id']}:{line_number}",
                )
                ...
                _require(
                    ...
                    f"V135 accepted readiness-stratified-work certificate changed: "
                    f"{run['run_id']}:{line_number}",
                )
            ...
            if not candidate:
                _require(
                    ...
                    f"V135 anchor used causal state: {run['run_id']}:{line_number}",
                )
                continue
            if not gate["history_complete"]:
                _require(
                    gate["baseline_count"] == 0 and gate["recent_count"] == 0,
                    f"V135 incomplete history leaked counts: {run['run_id']}:{line_number}",
                )
                continue
            sufficient = (
            ...
            if sufficient and recent_scaled >= baseline_scaled:
                ...
                _require(
                    ...
                    f"V135 exact threshold did not activate: "
                    f"{run['run_id']}:{line_number}",
                )

    _require(
        counts["windows"] == run["simulation"]["total_frame"],
        f"V135 diagnostic window count changed: {run['run_id']}",
    )
    if candidate:
        _require(
            counts["readiness_stratified_work_complete_windows"] == counts["windows"]
            and counts["threshold_met_windows"] > 0
            and counts["active_windows"] > 0
            and counts["critical_evaluated_windows"] > 0,
            f"V135 mechanism was not exercised: {run['run_id']}",
        )
    return {
        "file_sha256": file_hash(path),
        "window_count": counts["windows"],
        "readiness_stratified_work_complete_window_count": counts[
            "readiness_stratified_work_complete_windows"
        ],
        "pending_cpu_value_count": counts["pending_cpu_value_count"],
        "runnable_remaining_cpu_value_count": counts[
            "runnable_remaining_cpu_value_count"
        ],
        "resident_remaining_cpu_value_count": counts[
            "resident_remaining_cpu_value_count"
        ],
        "pending_immutable_cpu_work_total_sum": pending_immutable_total_sum,
        "runnable_immutable_cpu_work_total_sum": runnable_immutable_total_sum,
        "runnable_remaining_cpu_work_total_sum": runnable_remaining_total_sum,
        "resident_remaining_cpu_work_total_sum": resident_remaining_total_sum,
        "blocked_resident_remaining_cpu_work_total_sum": blocked_resident_remaining_total_sum,
        "readiness_stratified_cpu_work_total_sum": readiness_stratified_total_sum,
...
def _verify_tape_catalog() -> tuple[list[dict[str, Any]], dict[str, Any]]:
    _require(TAPES.is_file(), f"missing V135 tape catalog: {TAPES}")
    catalog = read_json(TAPES)
    catalog_hash = _assert_hashed_object(catalog, "catalog_hash", "V135 tapes")
    entries = catalog.get("entries")
    _require(
        isinstance(entries, dict) and len(entries) == 12, "V135 tape count changed"
    )
    evidence = []
...
def _verify_reference_stage_ledgers() -> tuple[dict[str, Path], dict[Path, str]]:
    ...
    for arm_id, expected in ARMS.items():
        ...
        root = _stage_root_from_receipts(
            entries, "receipt_path", expected["run_count"], f"V135 reference {arm_id}"
        )
        roots[arm_id] = root
    ...
    for root, expected_count in expected_by_root.items():
        ...
        _assert_ledger_contract(
            ...
            f"V135 reference stage {root}",
        )
        _require(
            not list((root / "quarantine").glob("**/attempt-*")),
            f"V135 reference quarantine is nonempty: {root}",
        )
...
def _validate_run_boundary(
    ...
    required = {
        "v135_training_plan_sha256": PLAN_SHA256,
        "v135_training_only": True,
        "v135_training_seed_metrics_previously_revealed": False,
        "v135_confirmation_seeds_opened": False,
        "v135_other_unopened_seeds_opened": False,
        "v135_formal_E01_E20_reexecution": False,
        "v135_arm_id": arm_id,
        "v135_arm_role": expected["role"],
        "v135_candidate_profile": expected["profile"],
        "v135_candidate_experiment": "E3",
        "v135_shock_rate_ratio": expected["shock_rate_ratio"],
        "v135_shock_threshold_numerator": expected["shock_threshold_numerator"],
        "v135_shock_threshold_denominator": expected["shock_threshold_denominator"],
        "v135_arrival_history_baseline_frames": 80 if candidate else None,
        "v135_arrival_history_recent_frames": 20 if candidate else None,
        "v135_arrival_min_requests_per_window": 20 if candidate else None,
        "v135_shock_activation_horizon_frames": expected[
            "shock_activation_horizon_frames"
        ],
        "v135_nonterminal_queue_density_floor": 8.0 if candidate else None,
        "v135_warm_admissibility": "preserve_anchor_warmness" if candidate else None,
        "v135_load_least_window_certificate_mode": (
            "disabled" if candidate else "not_applicable"
        ),
        "v135_critical_service_ratio_numerator": expected[
            "critical_service_threshold_numerator"
        ],
        "v135_critical_service_ratio_denominator": expected[
            "critical_service_threshold_denominator"
        ],
        "v135_critical_service_proxy": (
            "remote_parent_transfer_plus_cold_start_plus_readiness_stratified_queue_cpu_work"
            if candidate
            else "not_applicable"
        ),
        "v135_service_proxy_work_source": (
            READINESS_STRATIFIED_WORK_SOURCE if candidate else "not_applicable"
        ),
        "v135_readiness_stratified_work_includes_all_blocked_resident": candidate,
        "v135_readiness_stratified_work_runnable_uses_immutable_cpu": candidate,
        "v135_readiness_stratified_work_blocked_uses_remaining_cpu": candidate,
        "v135_readiness_stratified_work_disjoint_task_counting": candidate,
        "v135_readiness_stratified_work_deterministic_f64_sum": candidate,
        "v135_critical_service_proxy_inputs_finite_fail_closed": candidate,
        "v135_cpu_memory_individual_noninferiority": candidate,
        "v135_scalar_faasrank_noninferiority": candidate,
        "v135_input_locality_component_noninferiority": candidate,
        "v135_per_child_current_warm_downstream_locality_noninferiority": candidate,
        "v135_critical_frontier_substitution": candidate,
        "v135_critical_frontier_rank_source": (
            "immutable_srpt_remaining_critical_path_rank"
            if candidate
            else "not_applicable"
        ),
        "v135_noncritical_exact_anchor": candidate,
        "v135_complete_summed_critical_service_proxy_strictly_lower": candidate,
        "v135_complete_routed_score_nonworse": candidate,
        "v135_complete_exact_ocs_score_nonworse": candidate,
        "v135_complete_immutable_baseline_welfare_nonworse": candidate,
        "v135_outcome_fields_drive_policy": False,
        "v135_scenario_or_burst_label_used_by_policy": False,
        "v135_completion_or_performance_fields_used_by_policy": False,
        "v135_future_arrivals_used_by_policy": False,
    }
    _require(
        ...
        f"V135 run boundary changed: {run['run_id']}",
    )


def run_blind_audit(output: Path = OUTPUT) -> dict[str, Any]:
    _require(not output.exists(), f"V135 blind audit already exists: {output}")
    _require(not RESULT.exists(), "V135 reveal exists before blind audit")
    _require(PLAN.is_file() and file_hash(PLAN) == PLAN_SHA256, "V135 plan changed")
    plan = read_json(PLAN)
    _require(plan.get("formal_results_eligible") is False, "V135 eligibility changed")
    _require(PREPARED.is_file(), f"missing V135 prepared receipt: {PREPARED}")
    prepared = read_json(PREPARED)
    prepared_hash = _assert_hashed_object(
        prepared, "receipt_hash", "V135 prepared receipt"
    )
    _require(
        ...
        "V135 prepared scientific boundary changed",
    )
    ...
    for arm_id, expected in ARMS.items():
        ...
        _require(paths["ready"].is_file(), f"missing V135 ready manifest: {arm_id}")
        ...
        _require(
            ...
            f"V135 ready boundary changed: {arm_id}",
        )
        ...
        reference_catalog_hash = _assert_hashed_object(
            references, "catalog_hash", f"V135 references {arm_id}"
        )
        ...
        _require(
            ...
            f"V135 reference key set changed: {arm_id}",
        )
        for key, entry in sorted(entries.items()):
            ...
            _require(
                ...
                f"V135 reference evidence changed: {key}",
            )
        ...
        _require(actual_ids == expected_ids, f"V135 canonical set changed: {arm_id}")
        _require(
            not list((workspace / "quarantine").glob("**/attempt-*")),
            f"V135 online quarantine is nonempty: {arm_id}",
        )
        ...
        for run in manifest["runs"]:
            ...
            diagnostics = _validate_readiness_stratified_work_diagnostics(
            ...
            _require(
                ...
                f"V135 canonical status changed: {run['run_id']}",
            )
            runtime = {
            ...
            run_evidence.append(
                {
                    ...
                    "readiness_stratified_work_diagnostics": diagnostics,
                    "ledger_last_hash": ledger_last_hash,
                    "reference_ledger_last_hash": reference_last_hash,
                }
            )

    _require(len(run_evidence) == 18, "V135 run evidence count must be 18")
    _require(len(reference_evidence) == 18, "V135 reference evidence count must be 18")
    _require(len(tape_evidence) == 12, "V135 tape evidence count must be 12")
    for field, expected in EXPECTED_RUNTIME.items():
        _require(
            runtime_values[field] == {expected},
            f"V135 runtime {field} changed: {runtime_values[field]}",
        )
    ...
    _require(
        ...
        f"V135 runtime git identity is not singular: {git_commits}",
    )
    _require(len(paired_inputs) == 9, "V135 paired input product changed")
    for (scenario, seed), rows in paired_inputs.items():
        _require(len(rows) == 2, f"V135 paired arm count changed: {scenario}/{seed}")
        for field in (
            ...
            _require(
                ...
                f"V135 paired {field} changed: {scenario}/{seed}",
            )
        _require(
            rows[0]["common_hpa_hash"] == EXPECTED_COMMON_HPA_SHA256,
            f"V135 common HPA changed: {scenario}/{seed}",
        )

    output_payload = {
        "schema_version": "NSE_E3_READINESS_STRATIFIED_WORK_BLIND_AUDIT_V135_V1",
        "created_at": utc_now(),
        "status": "pass",
        ...
        "mechanism_diagnostic_windows_parsed": sum(
            run["readiness_stratified_work_diagnostics"]["window_count"]
