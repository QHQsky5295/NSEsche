...
from scripts.reviewer_experiments.protocol.nse_e3_componentwise_service_componentwise_interference_training_prepare_v133 import (
...
OUTPUT = ROOT / "joint-blind-audit-v133-training.json"
RESULT = ROOT / "training-result-v133.json"
...
ARMS = {
    arm_id: {
        ...
        "componentwise_interference_gate": role == "candidate",
        "run_count": count,
    }
...
def _validate_admitted_work_diagnostics(
    ...
    """Validate V133 mechanism evidence without opening any performance summary."""

    path = canonical / "reviewer_records" / run["run_id"] / "nash_metrics.jsonl.gz"
    _require(path.is_file(), f"missing V133 Nash diagnostics: {run['run_id']}")
    ...
    with gzip.open(path, "rt", encoding="utf-8") as handle:
        for line_number, line in enumerate(handle, start=1):
            ...
            _require(type(frame) is int and frame >= 0, "invalid V133 window frame")

            cluster = event.get("cluster")
            _require(isinstance(cluster, dict), "missing V133 cluster diagnostics")
            admitted_fields = (
            ...
            if candidate:
                _require(
                    ...
                    f"V133 admitted-work observation changed: {run['run_id']}:{line_number}",
                )
                ...
                _require(
                    ...
                    f"V133 admitted-work snapshot counts changed: "
                ...
                _require(
                    ...
                    f"V133 admitted-work aggregate changed: {run['run_id']}:{line_number}",
                )
            ...
            _require(
                ...
                f"V133 critical-service contract changed: {run['run_id']}:{line_number}",
            )
            ...
            _require(
                ...
                f"V133 componentwise-service contract changed: "
                f"{run['run_id']}:{line_number}",
            )
            for field in (
                ...
                _require(
                    ...
                    f"V133 componentwise {field} changed: "
                    f"{run['run_id']}:{line_number}",
                )
            ...
            _require(
                ...
                f"V133 componentwise acceptance changed: "
                f"{run['run_id']}:{line_number}",
            )
            ...
            _require(
                ...
                f"V133 componentwise reason changed: {run['run_id']}:{line_number}",
            )
            if not componentwise["evaluated"]:
                _require(
                    ...
                    is None,
                    f"V133 unevaluated componentwise evidence was populated: "
                    f"{run['run_id']}:{line_number}",
                )
            ...
            _require(
                ...
                f"V133 componentwise delta changed: {run['run_id']}:{line_number}",
            )
            _require(
                maximum_ratio is None or _finite_nonnegative(maximum_ratio),
                f"V133 componentwise ratio changed: {run['run_id']}:{line_number}",
            )
            if componentwise["accepted"] and componentwise["critical_player_count"] > 0:
                _require(
                    ...
                    f"V133 accepted componentwise delta is worse: "
                    f"{run['run_id']}:{line_number}",
                )
            _require(
                not critical["accepted"] or componentwise["accepted"],
                f"V133 main certificate bypassed componentwise gate: "
                f"{run['run_id']}:{line_number}",
            )
            ...
            if dominance.get("reason") in componentwise_rejection_reasons:
                _require(
                    ...
                    f"V133 componentwise fallback was not exact anchor: "
                    f"{run['run_id']}:{line_number}",
                )
            ...
            _require(
                ...
                f"V133 componentwise-interference contract changed: "
                f"{run['run_id']}:{line_number}",
            )
            ...
            _require(
                ...
                f"V133 unexpectedly enabled the complete aggregate interference gate: "
                f"{run['run_id']}:{line_number}",
            )
            ...
            for field in (
                ...
                _require(
                    ...
                    f"V133 critical {field} changed: {run['run_id']}:{line_number}",
                )
            _require(
                not critical["accepted"] or critical["evaluated"],
                f"V133 accepted without evaluation: {run['run_id']}:{line_number}",
            )
            if not invoked:
                _require(
                    ...
                    f"V133 non-invoked critical state changed: {run['run_id']}:{line_number}",
                )
            if critical["accepted"]:
                ...
                _require(
                    ...
                    f"V133 accepted evidence is nonfinite: {run['run_id']}:{line_number}",
                )
                ...
                _require(
                    ...
                    f"V133 accepted admitted-work certificate changed: "
                    f"{run['run_id']}:{line_number}",
                )
            ...
            if not candidate:
                _require(
                    ...
                    f"V133 anchor used causal state: {run['run_id']}:{line_number}",
                )
                continue
            if not gate["history_complete"]:
                _require(
                    gate["baseline_count"] == 0 and gate["recent_count"] == 0,
                    f"V133 incomplete history leaked counts: {run['run_id']}:{line_number}",
                )
                continue
            sufficient = (
            ...
            if sufficient and recent_scaled >= baseline_scaled:
                ...
                _require(
                    ...
                    f"V133 exact threshold did not activate: "
                    f"{run['run_id']}:{line_number}",
                )

    _require(
        counts["windows"] == run["simulation"]["total_frame"],
        f"V133 diagnostic window count changed: {run['run_id']}",
    )
    if candidate:
        _require(
            ...
            f"V133 mechanism was not exercised: {run['run_id']}",
        )
    return {
...
def _verify_tape_catalog() -> tuple[list[dict[str, Any]], dict[str, Any]]:
    _require(TAPES.is_file(), f"missing V133 tape catalog: {TAPES}")
    catalog = read_json(TAPES)
    catalog_hash = _assert_hashed_object(catalog, "catalog_hash", "V133 tapes")
    entries = catalog.get("entries")
    _require(
        isinstance(entries, dict) and len(entries) == 12, "V133 tape count changed"
    )
    evidence = []
...
def _verify_reference_stage_ledgers() -> tuple[dict[str, Path], dict[Path, str]]:
    ...
    for arm_id, expected in ARMS.items():
        ...
        root = _stage_root_from_receipts(
            entries, "receipt_path", expected["run_count"], f"V133 reference {arm_id}"
        )
        roots[arm_id] = root
    ...
    for root, expected_count in expected_by_root.items():
        ...
        _assert_ledger_contract(
            ...
            f"V133 reference stage {root}",
        )
        _require(
            not list((root / "quarantine").glob("**/attempt-*")),
            f"V133 reference quarantine is nonempty: {root}",
        )
...
def _validate_run_boundary(
    ...
    required = {
        ...
        "v133_arm_id": arm_id,
        "v133_arm_role": expected["role"],
        "v133_candidate_profile": expected["profile"],
        "v133_candidate_experiment": "E3",
        "v133_shock_rate_ratio": expected["shock_rate_ratio"],
        "v133_shock_threshold_numerator": expected["shock_threshold_numerator"],
        "v133_shock_threshold_denominator": expected["shock_threshold_denominator"],
        ...
        "v133_shock_activation_horizon_frames": expected[
            "shock_activation_horizon_frames"
        ],
        "v133_nonterminal_queue_density_floor": 8.0 if candidate else None,
        "v133_warm_admissibility": "preserve_anchor_warmness" if candidate else None,
        "v133_load_least_window_certificate_mode": (
            "disabled" if candidate else "not_applicable"
        ),
        "v133_critical_service_ratio_numerator": expected[
            "critical_service_threshold_numerator"
        ],
        "v133_critical_service_ratio_denominator": expected[
    ...
    _require(
        ...
        f"V133 run boundary changed: {run['run_id']}",
    )


def run_blind_audit(output: Path = OUTPUT) -> dict[str, Any]:
    _require(not output.exists(), f"V133 blind audit already exists: {output}")
    _require(not RESULT.exists(), "V133 reveal exists before blind audit")
    _require(PLAN.is_file() and file_hash(PLAN) == PLAN_SHA256, "V133 plan changed")
    plan = read_json(PLAN)
    _require(plan.get("formal_results_eligible") is False, "V133 eligibility changed")
    _require(PREPARED.is_file(), f"missing V133 prepared receipt: {PREPARED}")
    prepared = read_json(PREPARED)
    prepared_hash = _assert_hashed_object(
        prepared, "receipt_hash", "V133 prepared receipt"
    )
    _require(
        ...
        "V133 prepared scientific boundary changed",
    )
    ...
    for arm_id, expected in ARMS.items():
        ...
        _require(paths["ready"].is_file(), f"missing V133 ready manifest: {arm_id}")
        ...
        _require(
            ...
            f"V133 ready boundary changed: {arm_id}",
        )
        ...
        reference_catalog_hash = _assert_hashed_object(
            references, "catalog_hash", f"V133 references {arm_id}"
        )
        ...
        _require(
            ...
            f"V133 reference key set changed: {arm_id}",
        )
        for key, entry in sorted(entries.items()):
            ...
            _require(
                ...
                f"V133 reference evidence changed: {key}",
            )
        ...
        _require(actual_ids == expected_ids, f"V133 canonical set changed: {arm_id}")
        _require(
            not list((workspace / "quarantine").glob("**/attempt-*")),
            f"V133 online quarantine is nonempty: {arm_id}",
        )
        ...
        for run in manifest["runs"]:
            ...
            _require(
                ...
                f"V133 canonical status changed: {run['run_id']}",
            )
            runtime = {
    ...
    _require(len(run_evidence) == 18, "V133 run evidence count must be 18")
    _require(len(reference_evidence) == 18, "V133 reference evidence count must be 18")
    _require(len(tape_evidence) == 12, "V133 tape evidence count must be 12")
    ...
    _require(
        ...
        > 0,
        "V133 componentwise certificate did not exercise both accept and reject paths",
    )
    _require(
        all(
        ...
        "V133 anchor unexpectedly exercised the componentwise certificate",
    )
    _require(
        sum(
        ...
        > 0
        and all(
            row["componentwise_admitted_interference_gate_enabled"] is True
        ...
        "V133 componentwise-interference certificate was unavailable or not exercised",
    )
    _require(
        all(
        ...
        "V133 anchor unexpectedly exercised an interference certificate",
    )
    for field, expected in EXPECTED_RUNTIME.items():
        _require(
            runtime_values[field] == {expected},
            f"V133 runtime {field} changed: {runtime_values[field]}",
        )
    ...
    _require(
        ...
        f"V133 runtime git identity is not singular: {git_commits}",
    )
    _require(len(paired_inputs) == 9, "V133 paired input product changed")
    for (scenario, seed), rows in paired_inputs.items():
        _require(len(rows) == 2, f"V133 paired arm count changed: {scenario}/{seed}")
        for field in (
            ...
            _require(
                ...
                f"V133 paired {field} changed: {scenario}/{seed}",
            )
        _require(
            rows[0]["common_hpa_hash"] == EXPECTED_COMMON_HPA_SHA256,
            f"V133 common HPA changed: {scenario}/{seed}",
        )

    output_payload = {
        "schema_version": "NSE_E3_COMPONENTWISE_SERVICE_COMPONENTWISE_INTERFERENCE_BLIND_AUDIT_V133_V1",
        "created_at": utc_now(),
        "status": "pass",
