...
ROOT = Path("tmp/nse_e3_initializer_component_safety_training_20260828_v103")
PLAN = Path(
    "scripts/reviewer_experiments/protocol/"
    "nse_e3_initializer_component_safety_training_plan_v103.json"
)
PLAN_SHA256 = "3f582e08372cbf2540f77114799411167c026a84c079c5efcfe9dc08e2511173"
V102_RESULT_RECEIPT = Path(
    "scripts/reviewer_experiments/protocol/"
    "nse_e3_load_initializer_certificate_factorial_training_result_v102.json"
)
V102_RESULT_RECEIPT_SHA256 = (
    "a8c47ebdb1f75206761e3d05e23f30ed3707d413de0f72cb2de42a8f613164a3"
)
V102_SOURCE_RESULT = Path(
    "tmp/nse_e3_load_initializer_certificate_factorial_training_20260828_v102/"
    "training-result-v102.json"
)
V102_SOURCE_RESULT_SHA256 = (
    "a50d59941154e931c2d56111b484c85263931f1afc455c6b8f25f2260683fd0e"
)
FORMAL_RESULT = Path(
...
BINARY_PATH = Path("tmp/nse_v103_build_a8dcba9/release/serverless_sim.exe")
BINARY_SHA256 = "9928fb69dfae81e38d3dcc7a99da9e5230779cc267fd55df2a99dbb3c98fa15c"
BINARY_SOURCE_COMMIT = "a8dcba99a1631c932545f68845bca34b9f715186"
...
SOURCE_INITIAL_SEEDS = [f"E{index}" for index in range(878, 888)]
SOURCE_CI_SEEDS = [f"E{index}" for index in range(888, 898)]
TRAINING_SEED_LIST = ["E878", "E879", "E880"]
TRAINING_SEEDS = set(TRAINING_SEED_LIST)
CONFIRMATION_SEEDS = [f"E{index}" for index in range(946, 966)]
PREVIOUS_CONFIRMATION_SEEDS = [f"E{index}" for index in range(926, 946)]
OTHER_UNOPENED_SEEDS = [
    ...
    *[f"E{index}" for index in range(881, 926)],
]
PORT = "3138"

E3_ANCHOR = "faasrank_native_faithful_terminal_ocs_srpt_ready_dual_window_safe_pareto"
E3_BAND8_TO24_INPUT_LOCALITY_PARETO_INITIALIZER_ONLY = (
    "faasrank_native_faithful_terminal_ocs_srpt_ready_"
    "terminal_load_band8_24_input_locality_pareto_initializer_only_guard64_dual_window_safe_pareto"
)
E3_BAND8_TO24_COMPONENTWISE_PARETO_INITIALIZER_ONLY = (
    "faasrank_native_faithful_terminal_ocs_srpt_ready_"
    "terminal_load_band8_24_componentwise_pareto_initializer_only_guard64_dual_window_safe_pareto"
)
ARMS = (
    ("v103-e3-anchor", "E3", "anchor", E3_ANCHOR, None, "not_applicable", 9),
    (
        "v103-e3-band8-24-input-locality-pareto-initializer-only",
        "E3",
        "candidate",
        E3_BAND8_TO24_INPUT_LOCALITY_PARETO_INITIALIZER_ONLY,
        24.0,
        "scalar_plus_input_locality_noninferiority",
        9,
    ),
    (
        "v103-e3-band8-24-componentwise-pareto-initializer-only",
        "E3",
        "candidate",
        E3_BAND8_TO24_COMPONENTWISE_PARETO_INITIALIZER_ONLY,
        24.0,
        "componentwise_cpu_memory_locality_warm_diversity_noninferiority",
        9,
    ),
)
...
def _paths(root: Path) -> dict[str, Path]:
    return {
        "config": root / "v103-e3-training-config.json",
        "source": root / "manifest.v103-e878-e887-full.unbound.json",
        "capture": root / "manifest.v103-tape-capture.unbound.json",
        "prepared": root / "prepared-manifests-v103.json",
    }
...
def _assert_frozen_inputs() -> None:
    for path, expected in (
        (PLAN, PLAN_SHA256),
        (V102_RESULT_RECEIPT, V102_RESULT_RECEIPT_SHA256),
        (V102_SOURCE_RESULT, V102_SOURCE_RESULT_SHA256),
        ...
        if not path.is_file() or file_hash(path) != expected:
            raise RuntimeError(f"frozen V103 input is missing or changed: {path}")
...
def _rewrite(
    ...
    *,
    ...
    marker.update(
        {
            "purpose": purpose,
            "v103_training_plan_sha256": PLAN_SHA256,
            "v103_training_only": True,
            "v103_binary_source_commit": BINARY_SOURCE_COMMIT,
    ...
    if arm is not None:
        ...
        marker.update(
            {
                "v103_arm_id": arm_id,
                "v103_experiment_id": experiment_id,
                "v103_role": role,
                "v103_profile": profile,
                "v103_upper_queue_density_threshold": upper_density,
                "v103_nonterminal_queue_density_floor": (
                    8.0 if role == "candidate" else None
                ),
                "v103_warm_admissibility": (
                    "preserve_anchor_warmness" if role == "candidate" else None
                ),
                "v103_load_least_window_certificate_mode": (
                    "disabled" if role == "candidate" else "not_applicable"
                ),
                "v103_component_safety_mode": component_safety_mode,
                "v103_expected_run_count": expected,
            }
        )
        ...
        metadata.update(
            {
                "v103_training_plan_sha256": PLAN_SHA256,
                "v103_training_only": True,
                "v103_training_seed_metrics_previously_revealed": False,
                "v103_binary_source_commit": BINARY_SOURCE_COMMIT,
                "v103_confirmation_seeds_opened": False,
                "v103_other_unopened_seeds_opened": False,
                "v103_formal_E01_E20_reexecution": False,
            }
        )
        if arm is not None:
            (
            ...
            metadata.update(
                {
                    "v103_arm_id": arm_id,
                    "v103_arm_role": role,
                    "v103_candidate_profile": profile,
                    "v103_candidate_experiment": experiment_id,
                    "v103_upper_queue_density_threshold": upper_density,
                    "v103_nonterminal_queue_density_floor": (
                        8.0 if role == "candidate" else None
                    ),
                    ...
                    "v103_faasrank_model_artifact_sha256": MODEL_SHA256,
                    "v103_native_faithful_initializer": True,
                    "v103_dual_window_safe_pareto": True,
                    "v103_density_band_initializer_guard": role == "candidate",
                    "v103_terminal_players_included_without_lower_floor": role
                    == "candidate",
                    "v103_srpt_critical_path_player_order": "srpt_ready" in profile,
                    "v103_substitution_cap": None,
                    "v103_load_least_window_certificate_mode": certificate_mode,
                    "v103_component_safety_mode": component_safety_mode,
                    "v103_scalar_faasrank_noninferiority": role == "candidate",
                    "v103_input_locality_component_noninferiority": role == "candidate",
                    "v103_componentwise_faasrank_noninferiority": (
                        component_safety_mode
                        == "componentwise_cpu_memory_locality_warm_diversity_noninferiority"
                    ),
                }
...
def prepare_v103(root: Path = ROOT) -> dict:
    _assert_frozen_inputs()
    if root.exists():
        raise RuntimeError(f"refusing to overwrite V103 training root: {root}")
    ...
    capture = derive_integration_smoke_shard(
        paths["source"],
        capture_ids,
        purpose=f"V103 fresh balanced-QoS parent tapes; plan_sha256={PLAN_SHA256}",
    )
    _write(
        paths["capture"],
        _rewrite(
    ...
    for arm in ARMS:
        ...
        template = derive_integration_smoke_shard(
            paths["source"],
            selected,
            purpose=f"V103 {arm_id} initializer component-safety training; plan_sha256={PLAN_SHA256}",
        )
        manifest = _rewrite(
            template,
            purpose=f"V103 {arm_id} initializer component-safety training; plan_sha256={PLAN_SHA256}",
        ...
        if (
            ...
            raise RuntimeError(f"V103 {arm_id} must have {expected} runs/references")
        if {run["seed"] for run in manifest["runs"]} != TRAINING_SEEDS:
            raise RuntimeError(f"V103 {arm_id} escaped E878-E880")
        keys = {item["key"] for item in manifest["reference_build_dependencies"]}
        if all_reference_keys & keys:
            raise RuntimeError("V103 arm-specific reference keys unexpectedly overlap")
        all_reference_keys.update(keys)
        _write(arm_path(root, arm_id), manifest)
    if len(all_reference_keys) != 27:
        raise RuntimeError("V103 must declare exactly 27 arm-specific references")
    ...
    receipt = {
        "schema_version": "NSE_E3_INITIALIZER_COMPONENT_SAFETY_PREPARED_V103_V1",
        "created_at": utc_now(),
        "formal_results_eligible": False,
        ...
        "V102_result_receipt_path": str(V102_RESULT_RECEIPT),
        "V102_result_receipt_sha256": V102_RESULT_RECEIPT_SHA256,
        "V102_source_result_path": str(V102_SOURCE_RESULT),
        "V102_source_result_sha256": V102_SOURCE_RESULT_SHA256,
