...
from scripts.reviewer_experiments.protocol.nse_e3_completion_proximal_terminal_child_hpa_anchor_argmin_path_training_prepare_v129 import (
    ...
    ROOT,
    TRAINING_SEEDS,
    V128_BLIND_AUDIT_HASH,
    V128_BLIND_AUDIT_SHA256,
    V128_RESULT_COMMIT,
    V128_RESULT_HASH,
    V128_RESULT_SHA256,
    V127_RESULT_COMMIT,
...
PREPARED = ROOT / "prepared-manifests-v129.json"
TAPES = ROOT / "tapes.catalog.json"
OUTPUT = ROOT / "joint-blind-audit-v129-training.json"
RESULT = ROOT / "training-result-v129.json"
...
def _validate_admitted_work_diagnostics(
    ...
    """Validate V129 mechanism evidence without opening any performance summary."""

    path = canonical / "reviewer_records" / run["run_id"] / "nash_metrics.jsonl.gz"

    _require(path.is_file(), f"missing V129 Nash diagnostics: {run['run_id']}")
    ...
    with gzip.open(path, "rt", encoding="utf-8") as handle:
        for line_number, line in enumerate(handle, start=1):
            ...
            _require(type(frame) is int and frame >= 0, "invalid V129 window frame")

            cluster = event.get("cluster")
            _require(isinstance(cluster, dict), "missing V129 cluster diagnostics")
            admitted_fields = (
            ...
            if candidate:
                _require(
                    ...
                    f"V129 admitted-work observation changed: {run['run_id']}:{line_number}",
                )
                ...
                _require(
                    ...
                    f"V129 admitted-work snapshot counts changed: "
                ...
                _require(
                    ...
                    f"V129 admitted-work aggregate changed: {run['run_id']}:{line_number}",
                )
            ...
            _require(
                ...
                f"V129 critical-service contract changed: {run['run_id']}:{line_number}",
            )
            ...
            _require(
                ...
                f"V129 componentwise-service contract changed: "
                f"{run['run_id']}:{line_number}",
            )
            for field in (
                "target_player_count",
                ...
                _require(
                    ...
                    f"V129 componentwise {field} changed: "
                    f"{run['run_id']}:{line_number}",
                )
            ...
            _require(
                ...
                f"V129 componentwise acceptance changed: "
                f"{run['run_id']}:{line_number}",
            )
            ...
            _require(
                ...
                f"V129 componentwise reason changed: {run['run_id']}:{line_number}",
            )
            ...
            _require(
                ...
                f"V129 componentwise delta changed: {run['run_id']}:{line_number}",
            )
            _require(
                maximum_ratio is None or _finite_nonnegative(maximum_ratio),
                f"V129 componentwise ratio changed: {run['run_id']}:{line_number}",
            )
            if componentwise["accepted"] and componentwise["target_player_count"] > 0:
                _require(
                    ...
                    f"V129 accepted componentwise delta is worse: "
                    f"{run['run_id']}:{line_number}",
                )
            ...
            _require(
                ...
                == (
                    "every_terminal_child_alternative_path_to_the_deterministic_anchor_argmin_hpa_container_less_than_or_equal_to_anchor"
                ...
                == (
                    "same_deterministic_anchor_argmin_child_container_node"
                    if expected["terminal_child_path_service"]
                    else "independent_minimum_path"
                )
                and terminal_child_path.get("aggregation")
                == (
                    "complete_per_child_pareto_on_deterministic_anchor_argmin_witness"
                    if expected["terminal_child_path_service"]
                    else "minimum_path_per_child"
                )
                and terminal_child_path.get("uses_completion_outcomes") is False,
                f"V129 terminal-child path contract changed: "
                f"{run['run_id']}:{line_number}",
            )
            for field in (
                ...
                _require(
                    ...
                    f"V129 terminal-child path {field} changed: "
                    f"{run['run_id']}:{line_number}",
                )
            _require(
                ...
                f"V129 terminal-child fail-closed reason accounting changed: "
                f"{run['run_id']}:{line_number}",
            )
            _require(
                ...
                and (
                    ...
                    or terminal_child_path["compared_container_path_count"]
                ...
                f"V129 deterministic anchor-argmin witness accounting changed: "
                f"{run['run_id']}:{line_number}",
            )
            ...
            _require(
                ...
                f"V129 terminal-child path acceptance changed: "
                f"{run['run_id']}:{line_number}",
            )
            ...
            _require(
                ...
                == expected_terminal_child_path_reason,
                f"V129 terminal-child path reason changed: "
                f"{run['run_id']}:{line_number}",
            )
            ...
            _require(
                ...
                f"V129 terminal-child path delta changed: "
                f"{run['run_id']}:{line_number}",
            )
            _require(
                terminal_maximum_ratio is None
                or _finite_nonnegative(terminal_maximum_ratio),
                f"V129 terminal-child path ratio changed: "
                f"{run['run_id']}:{line_number}",
            )
            ...
            if (
                ...
                _require(
                    ...
                    f"V129 accepted terminal-child path is worse: "
                    f"{run['run_id']}:{line_number}",
                )
            _require(
                not critical["accepted"]
                or (componentwise["accepted"] and terminal_child_path["accepted"]),
                f"V129 main certificate bypassed a final service gate: "
                f"{run['run_id']}:{line_number}",
            )
            ...
            if dominance.get("reason") in componentwise_rejection_reasons:
                _require(
                    ...
                    f"V129 componentwise fallback was not exact anchor: "
                    f"{run['run_id']}:{line_number}",
                )
            ...
            if dominance.get("reason") in terminal_child_path_rejection_reasons:
                _require(
                    ...
                    and dominance.get("selected_initializer_assignment_hash")
                    == dominance.get("anchor_assignment_hash"),
                    f"V129 terminal-child path fallback was not exact anchor: "
                    f"{run['run_id']}:{line_number}",
                )
            for field in (
                ...
                _require(
                    ...
                    f"V129 critical {field} changed: {run['run_id']}:{line_number}",
                )
            _require(
                not critical["accepted"] or critical["evaluated"],
                f"V129 accepted without evaluation: {run['run_id']}:{line_number}",
            )
            ...
            _require(
                ...
                f"V129 completion-proximal construction contract changed: "
                f"{run['run_id']}:{line_number}",
            )
            ...
            _require(
                ...
                f"V129 completion-proximal scope diagnostics changed: "
                f"{run['run_id']}:{line_number}",
            )
            proximal_fields = (
            ...
            _require(
                ...
                f"V129 completion-proximal scope counter changed: "
                f"{run['run_id']}:{line_number}",
            )
            if invoked:
                _require(
                    ...
                    f"V129 completion-proximal substitution invariant changed: "
                    f"{run['run_id']}:{line_number}",
                )
            else:
                _require(
                    all(proximal_scope[field] == 0 for field in proximal_fields),
                    f"V129 inactive completion-proximal scope leaked counters: "
                    f"{run['run_id']}:{line_number}",
                )
            if not invoked:
                _require(
                    ...
                    f"V129 non-invoked critical state changed: {run['run_id']}:{line_number}",
                )
            if critical["accepted"]:
                ...
                _require(
                    ...
                    f"V129 accepted evidence is nonfinite: {run['run_id']}:{line_number}",
                )
                ...
                _require(
                    ...
                    f"V129 accepted admitted-work certificate changed: "
                    f"{run['run_id']}:{line_number}",
                )
            ...
            if not candidate:
                _require(
                    ...
                    f"V129 anchor used causal state: {run['run_id']}:{line_number}",
                )
                continue
            if not gate["history_complete"]:
                _require(
                    gate["baseline_count"] == 0 and gate["recent_count"] == 0,
                    f"V129 incomplete history leaked counts: {run['run_id']}:{line_number}",
                )
                continue
            sufficient = (
            ...
            if sufficient and recent_scaled >= baseline_scaled:
                ...
                _require(
                    ...
                    f"V129 exact threshold did not activate: "
                    f"{run['run_id']}:{line_number}",
                )

    _require(
        counts["windows"] == run["simulation"]["total_frame"],
        f"V129 diagnostic window count changed: {run['run_id']}",
    )
    if candidate:
        _require(
            ...
            f"V129 mechanism was not exercised: {run['run_id']}",
        )
    return {
...
def _verify_tape_catalog() -> tuple[list[dict[str, Any]], dict[str, Any]]:
    _require(TAPES.is_file(), f"missing V129 tape catalog: {TAPES}")
    catalog = read_json(TAPES)
    catalog_hash = _assert_hashed_object(catalog, "catalog_hash", "V129 tapes")
    entries = catalog.get("entries")
    _require(
        isinstance(entries, dict) and len(entries) == 12, "V129 tape count changed"
    )
    evidence = []
...
def _verify_reference_stage_ledgers() -> tuple[dict[str, Path], dict[Path, str]]:
    ...
    for arm_id, expected in ARMS.items():
        ...
        root = _stage_root_from_receipts(
            entries, "receipt_path", expected["run_count"], f"V129 reference {arm_id}"
        )
        roots[arm_id] = root
    ...
    for root, expected_count in expected_by_root.items():
        ...
        _assert_ledger_contract(
            ...
            f"V129 reference stage {root}",
        )
        _require(
            not list((root / "quarantine").glob("**/attempt-*")),
            f"V129 reference quarantine is nonempty: {root}",
        )
...
def _validate_run_boundary(
    ...
    required = {
        "v129_training_plan_sha256": PLAN_SHA256,
        "v129_training_only": True,
        "v129_training_seed_metrics_previously_revealed": False,
        "v129_confirmation_seeds_opened": False,
        "v129_other_unopened_seeds_opened": False,
        "v129_formal_E01_E20_reexecution": False,
        "v129_arm_id": arm_id,
        "v129_arm_role": expected["role"],
        "v129_candidate_profile": expected["profile"],
        "v129_candidate_experiment": "E3",
        "v129_shock_rate_ratio": expected["shock_rate_ratio"],
        "v129_shock_threshold_numerator": expected["shock_threshold_numerator"],
        "v129_shock_threshold_denominator": expected["shock_threshold_denominator"],
        "v129_arrival_history_baseline_frames": 80 if candidate else None,
        "v129_arrival_history_recent_frames": 20 if candidate else None,
        "v129_arrival_min_requests_per_window": 20 if candidate else None,
        "v129_shock_activation_horizon_frames": expected[
            "shock_activation_horizon_frames"
        ],
        "v129_nonterminal_queue_density_floor": 8.0 if candidate else None,
        "v129_warm_admissibility": "preserve_anchor_warmness" if candidate else None,
        "v129_load_least_window_certificate_mode": (
            "disabled" if candidate else "not_applicable"
        ),
        "v129_critical_service_ratio_numerator": expected[
            "critical_service_threshold_numerator"
        ],
        "v129_critical_service_ratio_denominator": expected[
            "critical_service_threshold_denominator"
        ],
        "v129_critical_service_proxy": (
            "remote_parent_transfer_plus_cold_start_plus_admitted_queue_cpu_work"
            if candidate
            else "not_applicable"
        ),
        "v129_service_proxy_work_source": (
            ADMITTED_WORK_SOURCE if candidate else "not_applicable"
        ),
        "v129_admitted_work_includes_all_blocked_resident": candidate,
        "v129_admitted_work_deterministic_f64_sum": candidate,
        "v129_critical_service_proxy_inputs_finite_fail_closed": candidate,
        "v129_cpu_memory_individual_noninferiority": candidate,
        "v129_scalar_faasrank_noninferiority": candidate,
        "v129_input_locality_component_noninferiority": candidate,
        "v129_per_child_current_warm_downstream_locality_noninferiority": candidate,
        "v129_critical_frontier_substitution": candidate,
        "v129_critical_frontier_rank_source": (
            "immutable_srpt_remaining_critical_path_rank"
            if candidate
            else "not_applicable"
        ),
        "v129_completion_proximal_depth1_substitution_scope": candidate,
        "v129_completion_proximal_definition": (
            "terminal_function_or_nonterminal_function_whose_immutable_DAG_children_are_all_terminal"
            if candidate
            else "not_applicable"
        ),
        "v129_maximum_remaining_child_depth": 1 if candidate else None,
        "v129_completion_proximal_critical_frontier_substitution": candidate,
        "v129_deeper_player_exact_anchor": candidate,
        "v129_complete_componentwise_service_pareto": candidate,
        "v129_componentwise_service_scope": (
            "all_current_critical_frontier_players" if candidate else "not_applicable"
        ),
        "v129_componentwise_service_comparison": (
            "every_current_critical_player_alternative_less_than_or_equal_to_anchor"
            if candidate
            else "not_applicable"
        ),
        "v129_componentwise_service_replay_order": (
            "frozen_native_player_order_with_independent_assignment_prefixes"
            if candidate
            else "not_applicable"
        ),
        "v129_complete_terminal_child_hpa_anchor_argmin_path_service_pareto": candidate,
        "v129_terminal_child_hpa_anchor_argmin_path_scope": (
            "every_immutable_immediate_child_of_each_actually_changed_completion_proximal_player"
            if candidate
            else "not_applicable"
        ),
        "v129_terminal_child_hpa_anchor_argmin_path_proxy": (
            "parent_output_transfer_plus_full_cold_start_if_starting_plus_child_node_admitted_cpu_service"
            if candidate
            else "not_applicable"
        ),
        "v129_terminal_child_hpa_container_node_scope": (
            "all_current_HPA_owned_child_containers_running_or_starting"
            if candidate
            else "not_applicable"
        ),
        "v129_terminal_child_hpa_anchor_argmin_path_comparison": (
            "every_terminal_child_alternative_path_to_the_deterministic_anchor_argmin_hpa_container_less_than_or_equal_to_anchor"
            if candidate
            else "not_applicable"
        ),
        "v129_terminal_child_hpa_anchor_argmin_path_pairing": (
            "same_deterministic_anchor_argmin_child_container_node"
            if candidate
            else "not_applicable"
        ),
        "v129_terminal_child_hpa_anchor_argmin_path_aggregation": (
            "complete_per_child_pareto_on_deterministic_anchor_argmin_witness"
            if candidate
            else "not_applicable"
        ),
        "v129_terminal_child_hpa_anchor_argmin_selection": (
            "minimum_finite_anchor_path_then_lowest_node_id"
            if candidate
            else "not_applicable"
        ),
        "v129_terminal_child_hpa_anchor_argmin_path_input_fail_closed": candidate,
        "v129_complete_summed_critical_service_proxy_strictly_lower": candidate,
        "v129_complete_routed_score_nonworse": candidate,
        "v129_complete_exact_ocs_score_nonworse": candidate,
        "v129_complete_immutable_baseline_welfare_nonworse": candidate,
        "v129_outcome_fields_drive_policy": False,
        "v129_scenario_or_burst_label_used_by_policy": False,
        "v129_completion_or_performance_fields_used_by_policy": False,
        "v129_future_arrivals_used_by_policy": False,
    }
    _require(
        ...
        f"V129 run boundary changed: {run['run_id']}",
    )


def run_blind_audit(output: Path = OUTPUT) -> dict[str, Any]:
    _require(not output.exists(), f"V129 blind audit already exists: {output}")
    _require(not RESULT.exists(), "V129 reveal exists before blind audit")
    _require(PLAN.is_file() and file_hash(PLAN) == PLAN_SHA256, "V129 plan changed")
    plan = read_json(PLAN)
    _require(plan.get("formal_results_eligible") is False, "V129 eligibility changed")
    _require(PREPARED.is_file(), f"missing V129 prepared receipt: {PREPARED}")
    prepared = read_json(PREPARED)
    prepared_hash = _assert_hashed_object(
        prepared, "receipt_hash", "V129 prepared receipt"
    )
    _require(
        ...
        and prepared.get("V128_result_sha256") == V128_RESULT_SHA256
        and prepared.get("V128_result_hash") == V128_RESULT_HASH
        and prepared.get("V128_result_commit") == V128_RESULT_COMMIT
        and prepared.get("V128_blind_audit_sha256") == V128_BLIND_AUDIT_SHA256
        and prepared.get("V128_blind_audit_hash") == V128_BLIND_AUDIT_HASH
        and prepared.get("V128_performance_results_consulted") is True
        ...
        "V129 prepared scientific boundary changed",
    )
    ...
    for arm_id, expected in ARMS.items():
        ...
        _require(paths["ready"].is_file(), f"missing V129 ready manifest: {arm_id}")
        ...
        _require(
            ...
            f"V129 ready boundary changed: {arm_id}",
        )
        ...
        reference_catalog_hash = _assert_hashed_object(
            references, "catalog_hash", f"V129 references {arm_id}"
        )
        ...
        _require(
            ...
            f"V129 reference key set changed: {arm_id}",
        )
        for key, entry in sorted(entries.items()):
            ...
            _require(
                ...
                f"V129 reference evidence changed: {key}",
            )
        ...
        _require(actual_ids == expected_ids, f"V129 canonical set changed: {arm_id}")
        _require(
            not list((workspace / "quarantine").glob("**/attempt-*")),
            f"V129 online quarantine is nonempty: {arm_id}",
        )
        ...
        for run in manifest["runs"]:
            ...
            _require(
                ...
                f"V129 canonical status changed: {run['run_id']}",
            )
            runtime = {
    ...
    _require(len(run_evidence) == 18, "V129 run evidence count must be 18")
    _require(len(reference_evidence) == 18, "V129 reference evidence count must be 18")
    _require(len(tape_evidence) == 12, "V129 tape evidence count must be 12")
    for field, expected in EXPECTED_RUNTIME.items():
        _require(
            runtime_values[field] == {expected},
            f"V129 runtime {field} changed: {runtime_values[field]}",
        )
    ...
    _require(
        ...
        f"V129 runtime git identity is not singular: {git_commits}",
    )
    _require(len(paired_inputs) == 9, "V129 paired input product changed")
    for (scenario, seed), rows in paired_inputs.items():
        _require(len(rows) == 2, f"V129 paired arm count changed: {scenario}/{seed}")
        for field in (
            ...
            _require(
                ...
                f"V129 paired {field} changed: {scenario}/{seed}",
            )
        _require(
            rows[0]["common_hpa_hash"] == EXPECTED_COMMON_HPA_SHA256,
            f"V129 common HPA changed: {scenario}/{seed}",
        )
    ...
    _require(
        ...
        "V129 completion-proximal and deeper current-player paths were not both exercised",
    )
    _require(
        completion_proximal_service_choice_evaluated_count > 0
        and completion_proximal_substitution_count > 0,
        "V129 completion-proximal service-choice evaluation/substitution path was not exercised",
    )
    _require(
        deeper_exact_anchor_preserved_count > 0 and deeper_substitution_count == 0,
        "V129 deeper-player exact-anchor preservation contract changed",
    )
    _require(
        proxy_input_unavailable_count == 0,
        "V129 admitted-work proxy inputs were unavailable",
    )
    _require(
        ...
        "V129 componentwise service accept/reject paths were not both exercised",
    )
    _require(
        componentwise_input_unavailable_count == 0,
        "V129 componentwise service inputs were unavailable",
    )
    _require(
        ...
        and terminal_child_path_compared_container_path_count > 0
        ...
        "V129 terminal-child anchor-argmin accept/reject paths were not both exercised",
    )
    _require(
        terminal_child_path_input_unavailable_count == 0,
        "V129 terminal-child path inputs were unavailable",
    )
    _require(
        ...
        "V129 terminal-child HPA path fail-closed reason count was nonzero",
    )
    _require(
        terminal_child_path_nonterminal_child_count == 0,
        "V129 terminal-child path scope included a nonterminal child",
    )
    _require(
        terminal_child_path_worse_container_path_count > 0,
        "V129 anchor-argmin worse-path rejection was not exercised",
    )

    output_payload = {
        "schema_version": "NSE_E3_COMPLETION_PROXIMAL_TERMINAL_CHILD_HPA_ANCHOR_ARGMIN_PATH_BLIND_AUDIT_V129_V1",
        "created_at": utc_now(),
        "status": "pass",
        ...
        "terminal_child_anchor_argmin_path_mechanism_coverage": {
            "terminal_child_anchor_argmin_gate_evaluated_count": terminal_child_path_gate_evaluated_count,
            "terminal_child_anchor_argmin_changed_parent_count": terminal_child_path_changed_parent_count,
            "terminal_child_anchor_argmin_accepted_changed_parent_count": terminal_child_path_accepted_changed_parent_count,
            "terminal_child_anchor_argmin_compared_child_count": terminal_child_path_compared_child_count,
            "terminal_child_anchor_argmin_selected_path_count": terminal_child_path_compared_container_path_count,
            "terminal_child_anchor_argmin_acceptance_count": terminal_child_path_acceptance_count,
            "terminal_child_anchor_argmin_rejection_count": terminal_child_path_rejection_count,
            "terminal_child_anchor_argmin_input_unavailable_count": terminal_child_path_input_unavailable_count,
            "terminal_child_anchor_argmin_running_container_path_count": terminal_child_path_running_container_count,
            "terminal_child_anchor_argmin_starting_container_path_count": terminal_child_path_starting_container_count,
            "terminal_child_anchor_argmin_no_container_count": terminal_child_path_no_container_count,
            "terminal_child_anchor_argmin_invalid_profile_count": terminal_child_path_invalid_profile_count,
            "terminal_child_anchor_argmin_invalid_node_snapshot_count": terminal_child_path_invalid_node_snapshot_count,
            "terminal_child_anchor_argmin_invalid_network_count": terminal_child_path_invalid_network_count,
            "terminal_child_anchor_argmin_nonterminal_child_count": terminal_child_path_nonterminal_child_count,
            "terminal_child_anchor_argmin_worse_child_count": terminal_child_path_worse_child_count,
            "terminal_child_anchor_argmin_worse_path_count": terminal_child_path_worse_container_path_count,
            "terminal_child_anchor_argmin_maximum_alternative_minus_anchor_for_same_container": terminal_child_path_maximum_same_container_delta,
            "terminal_child_anchor_argmin_maximum_alternative_to_anchor_ratio_for_same_container": terminal_child_path_maximum_same_container_ratio,
            "performance_fields_consulted": False,
        },
        "arm_count": len(ARMS),
