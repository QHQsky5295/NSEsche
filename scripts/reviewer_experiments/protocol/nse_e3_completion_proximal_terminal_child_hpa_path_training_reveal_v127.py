...
from scripts.reviewer_experiments.protocol.nse_e3_completion_proximal_terminal_child_hpa_path_training_blind_audit_v127 import (
    ARMS,
...
ANCHOR_ID = "v127-e3-anchor"
CANDIDATE_ID = (
    "v127-e3-causal-shock15-horizon50-admitted-work10-"
    "completion-proximal-depth1-componentwise-service-terminal-child-hpa-container-path-pareto-"
    "initializer-only"
)
...
def _validate_blind_audit() -> dict[str, Any]:
    ...
    if any(
        ...
        raise RuntimeError("V127 blind audit has not been frozen into the reveal code")
    if not BLIND_AUDIT.is_file() or file_hash(BLIND_AUDIT) != BLIND_AUDIT_FILE_SHA256:
        raise RuntimeError("V127 blind audit is missing or changed")
    ...
    if (
        ...
        raise RuntimeError("V127 blind audit does not authorize reveal")
    return blind
...
def _load_rows(blind: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    if len(admitted) != 18:
        raise RuntimeError("V127 blind audit run evidence is incomplete")
    ...
    for arm_id, expected in ARMS.items():
        ...
        if not manifest_path.is_file() or file_hash(manifest_path) != frozen.get(
            ...
            raise RuntimeError(f"V127 frozen manifest changed: {arm_id}")
        manifest = load_and_validate_manifest(manifest_path)
        if manifest.get("manifest_hash") != frozen.get("manifest_hash"):
            raise RuntimeError(f"V127 manifest hash changed: {arm_id}")
        for run in manifest["runs"]:
            ...
            if (
                ...
                != expected["profile"]
                or metadata.get("v127_training_plan_sha256") != PLAN_SHA256
                or metadata.get("v127_training_only") is not True
                or metadata.get("v127_training_seed_metrics_previously_revealed")
                is not False
                or metadata.get("v127_confirmation_seeds_opened") is not False
                or metadata.get("v127_other_unopened_seeds_opened") is not False
                or metadata.get("v127_arm_id") != arm_id
                or metadata.get("v127_arm_role") != expected["role"]
                or metadata.get("v127_shock_rate_ratio") != expected["shock_rate_ratio"]
                or metadata.get("v127_shock_threshold_numerator")
                != expected["shock_threshold_numerator"]
                or metadata.get("v127_shock_threshold_denominator")
                != expected["shock_threshold_denominator"]
                or metadata.get("v127_shock_activation_horizon_frames")
                != expected["shock_activation_horizon_frames"]
                or metadata.get("v127_critical_service_ratio_numerator")
                != expected["critical_service_threshold_numerator"]
                or metadata.get("v127_critical_service_ratio_denominator")
                != expected["critical_service_threshold_denominator"]
                or metadata.get("v127_service_proxy_work_source")
                != (
                    "admitted_pending_plus_all_resident_remaining_and_same_window_projected_cpu_v1"
                    if candidate
                    else "not_applicable"
                )
                or metadata.get("v127_admitted_work_includes_all_blocked_resident")
                is not candidate
                or metadata.get("v127_completion_proximal_depth1_substitution_scope")
                is not candidate
                or metadata.get("v127_completion_proximal_definition")
                != (
                    "terminal_function_or_nonterminal_function_whose_immutable_DAG_children_are_all_terminal"
                    if candidate
                    else "not_applicable"
                )
                or metadata.get("v127_maximum_remaining_child_depth")
                != (1 if candidate else None)
                or metadata.get(
                    "v127_completion_proximal_critical_frontier_substitution"
                )
                is not candidate
                or metadata.get("v127_deeper_player_exact_anchor") is not candidate
                or metadata.get("v127_complete_componentwise_service_pareto")
                is not candidate
                or metadata.get("v127_componentwise_service_scope")
                != (
                    "all_current_critical_frontier_players"
                    if candidate
                    else "not_applicable"
                )
                or metadata.get("v127_componentwise_service_comparison")
                != (
                ...
                is not False
                or metadata.get("v127_completion_or_performance_fields_used_by_policy")
                is not False
                or metadata.get("v127_future_arrivals_used_by_policy") is not False
            ):
                raise RuntimeError(f"V127 arm identity changed: {run['run_id']}")
            run_id = run["run_id"]
            summary_path = (
                ROOT
                / "runs"
            ...
            if not summary_path.is_file() or file_hash(summary_path) != admitted.get(
                ...
                raise RuntimeError(f"V127 summary differs from blind audit: {run_id}")
            ...
            if (
                ...
                raise RuntimeError(
                    f"V127 summary identity/completion mismatch: {run_id}"
                )
            rows.append(
                {
                    "arm_id": arm_id,
...
def execute_reveal() -> dict[str, Any]:
    if OUTPUT.exists():
        raise RuntimeError(f"V127 training result already exists: {OUTPUT}")
    if not PLAN.is_file() or file_hash(PLAN) != PLAN_SHA256:
        raise RuntimeError("frozen V127 plan changed before reveal")
    blind = _validate_blind_audit()
    rows = _load_rows(blind)
    if len(rows) != 18:
        raise RuntimeError("V127 revealed row count differs from sealed plan")
    ...
    result = {
        "schema_version": "NSE_E3_COMPLETION_PROXIMAL_TERMINAL_CHILD_HPA_PATH_RESULT_V127_V1",
        "created_at": utc_now(),
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Reveal the frozen V127 training cohort exactly once after blind audit."
    )
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    if args.command != "reveal" or not args.execute:
        parser.error("V127 reveal requires the explicit command: reveal --execute")
    execute_reveal()
