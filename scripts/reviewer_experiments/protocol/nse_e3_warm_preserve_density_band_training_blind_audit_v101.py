...
from scripts.reviewer_experiments.protocol.nse_e3_warm_preserve_density_band_training_prepare_v101 import (
...
OUTPUT = ROOT / "joint-blind-audit-v101-training.json"
RESULT = ROOT / "training-result-v101.json"
...
ARMS = {
    ...
    for arm_id, experiment_id, role, profile, upper_density, count in PREPARED_ARMS
}
...
def _verify_tape_catalog() -> tuple[list[dict[str, Any]], dict[str, Any]]:
    _require(TAPES.is_file(), f"missing V101 tape catalog: {TAPES}")
    catalog = read_json(TAPES)
    catalog_hash = _assert_hashed_object(catalog, "catalog_hash", "V101 tapes")
    entries = catalog.get("entries")
    _require(
        isinstance(entries, dict) and len(entries) == 12,
        "V101 tape count changed",
    )
    evidence = []
...
def _verify_reference_stage_ledgers() -> tuple[dict[str, Path], dict[Path, str]]:
    ...
    for root, expected_count in expected_by_root.items():
        ...
        _assert_ledger_contract(
            ...
            f"V101 reference stage {root}",
        )
        _require(
            not list((root / "quarantine").glob("**/attempt-*")),
            f"V101 reference quarantine is nonempty: {root}",
        )
...
def run_blind_audit(output: Path = OUTPUT) -> dict[str, Any]:
    _require(not output.exists(), f"V101 blind audit already exists: {output}")
    _require(not RESULT.exists(), "V101 reveal exists before blind audit")
    _require(PLAN.is_file() and file_hash(PLAN) == PLAN_SHA256, "V101 plan changed")
    plan = read_json(PLAN)
    _require(plan.get("formal_results_eligible") is False, "V101 eligibility changed")
    _require(PREPARED.is_file(), f"missing V101 prepared receipt: {PREPARED}")
    prepared = read_json(PREPARED)
    prepared_hash = _assert_hashed_object(
        prepared, "receipt_hash", "V101 prepared receipt"
    )
    _require(
        ...
        "V101 prepared scientific boundary changed",
    )
    ...
    for arm_id, expected in ARMS.items():
        ...
        _require(paths["ready"].is_file(), f"missing V101 ready manifest: {arm_id}")
        manifest = load_and_validate_manifest(paths["ready"])
        _require(
            len(manifest["runs"]) == expected["run_count"],
            f"V101 run count changed: {arm_id}",
        )
        _require(
            ...
            f"V101 ready flags changed: {arm_id}",
        )
        ...
        reference_catalog_hash = _assert_hashed_object(
            references, "catalog_hash", f"V101 references {arm_id}"
        )
        ...
        _require(
            ...
            f"V101 reference key set changed: {arm_id}",
        )
        for key, entry in sorted(entries.items()):
        ...
        _require(actual_ids == expected_ids, f"V101 canonical set changed: {arm_id}")
        _require(
            not list((workspace / "quarantine").glob("**/attempt-*")),
            f"V101 online quarantine is nonempty: {arm_id}",
        )
        ...
        for run in manifest["runs"]:
            ...
            _require(
                ...
                and metadata.get("v101_arm_id") == arm_id
                and metadata.get("v101_arm_role") == expected["role"]
                and metadata.get("v101_candidate_profile") == expected["profile"]
                and metadata.get("v101_upper_queue_density_threshold")
                == expected["upper_queue_density_threshold"]
                and metadata.get("v101_nonterminal_queue_density_floor")
                == expected["nonterminal_queue_density_floor"]
                and metadata.get("v101_warm_admissibility")
                == expected["warm_admissibility"]
                and metadata.get("v101_training_seed_metrics_previously_revealed")
                is False
                and metadata.get("v101_confirmation_seeds_opened") is False
                and metadata.get("v101_other_unopened_seeds_opened") is False,
                f"V101 run boundary changed: {run['run_id']}",
            )
            ...
            _require(
                ...
                f"V101 canonical status changed: {run['run_id']}",
            )
            runtime = {
    ...
    _require(len(run_evidence) == 27, "V101 run evidence count must be 27")
    _require(len(reference_evidence) == 27, "V101 reference evidence count must be 27")
    _require(len(tape_evidence) == 12, "V101 tape evidence count must be 12")
    for field, expected in EXPECTED_RUNTIME.items():
        _require(
            runtime_values[field] == {expected},
            f"V101 runtime {field} changed: {runtime_values[field]}",
        )
    ...
    _require(
        ...
        f"V101 runtime git identity is not singular: {git_commits}",
    )
    for (experiment_id, scenario, seed), rows in paired_inputs.items():
        _require(
            len(rows) == 3,
            f"V101 paired arm count changed: {experiment_id}/{scenario}/{seed}",
        )
        for field in (
            ...
            _require(
                ...
                f"V101 paired {field} changed: {scenario}/{seed}",
            )
        _require(
            rows[0]["common_hpa_hash"] == EXPECTED_COMMON_HPA_SHA256,
            f"V101 common HPA changed: {scenario}/{seed}",
        )

    output_payload = {
        "schema_version": "NSE_E3_WARM_PRESERVE_DENSITY_BAND_BLIND_AUDIT_V101_V1",
        "created_at": utc_now(),
        "status": "pass",
