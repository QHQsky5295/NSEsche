...
from scripts.reviewer_experiments.protocol.nse_e3_completion_proximal_terminal_child_hpa_anchor_running_first_top2_path_training_prepare_v131 import (
    ...
    ROOT,
    TRAINING_SEEDS,
    V130_BLIND_AUDIT_COMMIT,
    V130_BLIND_AUDIT_HASH,
    V130_BLIND_AUDIT_SHA256,
    V130_RESULT_COMMIT,
    V130_RESULT_HASH,
    V130_RESULT_SHA256,
    V129_BLIND_AUDIT_COMMIT,
...
OUTPUT = ROOT / "joint-blind-audit-v131-training.json"
RESULT = ROOT / "training-result-v131.json"
...
ARMS = {
    arm_id: {
        ...
        "terminal_child_path_service": role == "candidate",
        "run_count": count,
    }
...
def _validate_admitted_work_diagnostics(
    ...
    """Validate V131 mechanism evidence without opening any performance summary."""

    path = canonical / "reviewer_records" / run["run_id"] / "nash_metrics.jsonl.gz"

    _require(path.is_file(), f"missing V131 Nash diagnostics: {run['run_id']}")
    candidate = expected["role"] == "candidate"
    counts = Counter()
    exact_pending_total_sum = 0.0
    resident_remaining_total_sum = 0.0
    maximum_same_container_delta: float | None = None
    maximum_same_container_ratio: float | None = None
    with gzip.open(path, "rt", encoding="utf-8") as handle:
        for line_number, line in enumerate(handle, start=1):
            ...
            _require(type(frame) is int and frame >= 0, "invalid V131 window frame")

            cluster = event.get("cluster")
            _require(isinstance(cluster, dict), "missing V131 cluster diagnostics")
            admitted_fields = (
            ...
            if candidate:
                _require(
                    ...
                    f"V131 admitted-work observation changed: {run['run_id']}:{line_number}",
                )
                ...
                _require(
                    ...
                    f"V131 admitted-work snapshot counts changed: "
                ...
                _require(
                    ...
                    f"V131 admitted-work aggregate changed: {run['run_id']}:{line_number}",
                )
            ...
            _require(
                ...
                f"V131 critical-service contract changed: {run['run_id']}:{line_number}",
            )
            ...
            _require(
                ...
                f"V131 componentwise-service contract changed: "
                f"{run['run_id']}:{line_number}",
            )
            for field in (
                "target_player_count",
                ...
                _require(
                    ...
                    f"V131 componentwise {field} changed: "
                    f"{run['run_id']}:{line_number}",
                )
            ...
            _require(
                ...
                f"V131 componentwise acceptance changed: "
                f"{run['run_id']}:{line_number}",
            )
            ...
            _require(
                ...
                f"V131 componentwise reason changed: {run['run_id']}:{line_number}",
            )
            ...
            _require(
                ...
                f"V131 componentwise delta changed: {run['run_id']}:{line_number}",
            )
            _require(
                maximum_ratio is None or _finite_nonnegative(maximum_ratio),
                f"V131 componentwise ratio changed: {run['run_id']}:{line_number}",
            )
            if componentwise["accepted"] and componentwise["target_player_count"] > 0:
                _require(
                    ...
                    f"V131 accepted componentwise delta is worse: "
                    f"{run['run_id']}:{line_number}",
                )
            terminal_child_path = critical.get("terminal_child_path_pareto")
            terminal_child_path_invoked = (
                expected["terminal_child_path_service"] and invoked
            )
            _require(
                isinstance(terminal_child_path, dict)
                and terminal_child_path.get("gate_enabled")
                is terminal_child_path_invoked
                and type(terminal_child_path.get("evaluated")) is bool
                and type(terminal_child_path.get("accepted")) is bool
                and terminal_child_path.get("scope")
                == (
                ...
                and terminal_child_path.get("container_pairing")
                == (
                    "same_first_two_running_first_deterministic_anchor_ranked_child_container_nodes"
                    if expected["terminal_child_path_service"]
                    else "independent_minimum_path"
                )
                and terminal_child_path.get("aggregation")
                == (
                    "complete_per_child_pareto_on_up_to_two_running_first_deterministic_anchor_ranked_witnesses"
                    if expected["terminal_child_path_service"]
                    else "minimum_path_per_child"
                )
                and terminal_child_path.get("uses_completion_outcomes") is False,
                f"V131 terminal-child path contract changed: "
                f"{run['run_id']}:{line_number}",
            )
            for field in (
                "changed_parent_count",
                "accepted_changed_parent_count",
                "compared_child_count",
                "compared_container_path_count",
                "available_container_path_count",
                "selected_rank1_path_count",
                "selected_rank2_path_count",
                "running_available_but_no_running_selected_child_count",
                "input_unavailable_count",
                "running_container_count",
                "starting_container_count",
                "no_container_count",
                "invalid_profile_count",
                "invalid_node_snapshot_count",
                "invalid_network_count",
                "nonterminal_child_count",
                "worse_child_count",
                "worse_container_path_count",
            ):
                _require(
                    type(terminal_child_path.get(field)) is int
                    and terminal_child_path[field] >= 0,
                    f"V131 terminal-child path {field} changed: "
                    f"{run['run_id']}:{line_number}",
                )
            _require(
                terminal_child_path["input_unavailable_count"]
                == terminal_child_path["no_container_count"]
                + terminal_child_path["invalid_profile_count"]
                + terminal_child_path["invalid_node_snapshot_count"]
                + terminal_child_path["invalid_network_count"],
                f"V131 terminal-child fail-closed reason accounting changed: "
                f"{run['run_id']}:{line_number}",
            )
            _require(
                terminal_child_path["accepted_changed_parent_count"]
                == (
                    terminal_child_path["changed_parent_count"]
                    if terminal_child_path["accepted"]
                    else 0
                )
                and (
                    not terminal_child_path_invoked
                    or (
                        terminal_child_path["selected_rank1_path_count"]
                        == terminal_child_path["compared_child_count"]
                        and terminal_child_path["selected_rank2_path_count"]
                        <= terminal_child_path["compared_child_count"]
                        and terminal_child_path["compared_container_path_count"]
                        == terminal_child_path["selected_rank1_path_count"]
                        + terminal_child_path["selected_rank2_path_count"]
                        and terminal_child_path["available_container_path_count"]
                        >= terminal_child_path["compared_container_path_count"]
                    )
                )
                and terminal_child_path["running_container_count"]
                + terminal_child_path["starting_container_count"]
                == terminal_child_path["compared_container_path_count"]
                and terminal_child_path.get("protected_anchor_path_witness_count")
                == (2 if expected["terminal_child_path_service"] else None),
                f"V131 deterministic anchor-top2 witness accounting changed: "
                f"{run['run_id']}:{line_number}",
            )
            expected_terminal_child_path_accept = (
                terminal_child_path["evaluated"]
                and terminal_child_path["input_unavailable_count"] == 0
                and terminal_child_path["nonterminal_child_count"] == 0
                and terminal_child_path["worse_child_count"] == 0
                and terminal_child_path["worse_container_path_count"] == 0
            )
            _require(
                terminal_child_path["accepted"] is expected_terminal_child_path_accept,
                f"V131 terminal-child path acceptance changed: "
                f"{run['run_id']}:{line_number}",
            )
            expected_terminal_child_path_reason = (
                ...
                if terminal_child_path["nonterminal_child_count"] > 0
                else "terminal_child_path_service_worse"
                if terminal_child_path["worse_child_count"] > 0
                or terminal_child_path["worse_container_path_count"] > 0
                else "accepted"
            )
            _require(
                terminal_child_path.get("reason")
                == expected_terminal_child_path_reason,
                f"V131 terminal-child path reason changed: "
                f"{run['run_id']}:{line_number}",
            )
            terminal_maximum_delta = terminal_child_path.get(
                "maximum_alternative_minus_anchor"
            )
            terminal_maximum_ratio = terminal_child_path.get(
                "maximum_alternative_to_anchor_ratio"
            )
            _require(
                terminal_maximum_delta is None
                or (
                    type(terminal_maximum_delta) in (int, float)
                    and not isinstance(terminal_maximum_delta, bool)
                    and math.isfinite(float(terminal_maximum_delta))
                ),
                f"V131 terminal-child path delta changed: "
                f"{run['run_id']}:{line_number}",
            )
            _require(
                terminal_maximum_ratio is None
                or _finite_nonnegative(terminal_maximum_ratio),
                f"V131 terminal-child path ratio changed: "
                f"{run['run_id']}:{line_number}",
            )
            if terminal_maximum_delta is not None:
                terminal_delta = float(terminal_maximum_delta)
                maximum_same_container_delta = (
                    terminal_delta
                    if maximum_same_container_delta is None
                    else max(maximum_same_container_delta, terminal_delta)
                )
            if terminal_maximum_ratio is not None:
                terminal_ratio = float(terminal_maximum_ratio)
                maximum_same_container_ratio = (
                    terminal_ratio
                    if maximum_same_container_ratio is None
                    else max(maximum_same_container_ratio, terminal_ratio)
                )
            if (
                terminal_child_path["accepted"]
                and terminal_child_path["compared_child_count"] > 0
            ):
                _require(
                    terminal_maximum_delta is not None
                    and float(terminal_maximum_delta) <= 1.000001e-6,
                    f"V131 accepted terminal-child path is worse: "
                    f"{run['run_id']}:{line_number}",
                )
            _require(
                not critical["accepted"]
                or (componentwise["accepted"] and terminal_child_path["accepted"]),
                f"V131 main certificate bypassed a final service gate: "
                f"{run['run_id']}:{line_number}",
            )
            ...
            if dominance.get("reason") in componentwise_rejection_reasons:
                _require(
                    ...
                    f"V131 componentwise fallback was not exact anchor: "
                    f"{run['run_id']}:{line_number}",
                )
            terminal_child_path_rejection_reasons = {
                "terminal_child_path_input_unavailable",
                "terminal_child_path_nonterminal_child",
                "terminal_child_path_service_worse",
            }
            if dominance.get("reason") in terminal_child_path_rejection_reasons:
                _require(
                    terminal_child_path["accepted"] is False
                    and dominance.get("accepted") is False
                    and dominance.get("selected_initializer_assignment_hash")
                    == dominance.get("anchor_assignment_hash"),
                    f"V131 terminal-child path fallback was not exact anchor: "
                    f"{run['run_id']}:{line_number}",
                )
            for field in (
                ...
                _require(
                    ...
                    f"V131 critical {field} changed: {run['run_id']}:{line_number}",
                )
            _require(
                not critical["accepted"] or critical["evaluated"],
                f"V131 accepted without evaluation: {run['run_id']}:{line_number}",
            )
            ...
            _require(
                ...
                f"V131 completion-proximal construction contract changed: "
                f"{run['run_id']}:{line_number}",
            )
            ...
            _require(
                ...
                f"V131 completion-proximal scope diagnostics changed: "
                f"{run['run_id']}:{line_number}",
            )
            proximal_fields = (
            ...
            _require(
                ...
                f"V131 completion-proximal scope counter changed: "
                f"{run['run_id']}:{line_number}",
            )
            if invoked:
                _require(
                    ...
                    f"V131 completion-proximal substitution invariant changed: "
                    f"{run['run_id']}:{line_number}",
                )
            else:
                _require(
                    all(proximal_scope[field] == 0 for field in proximal_fields),
                    f"V131 inactive completion-proximal scope leaked counters: "
                    f"{run['run_id']}:{line_number}",
                )
            if not invoked:
                _require(
                    ...
                    f"V131 non-invoked critical state changed: {run['run_id']}:{line_number}",
                )
            if critical["accepted"]:
                ...
                _require(
                    ...
                    f"V131 accepted evidence is nonfinite: {run['run_id']}:{line_number}",
                )
                ...
                _require(
                    ...
                    f"V131 accepted admitted-work certificate changed: "
                    f"{run['run_id']}:{line_number}",
                )
            ...
            counts["terminal_child_path_evaluated_windows"] += int(
                terminal_child_path["evaluated"]
            )
            counts["terminal_child_path_accepted_windows"] += int(
                terminal_child_path["accepted"]
            )
            counts["terminal_child_path_rejected_windows"] += int(
                terminal_child_path["evaluated"] and not terminal_child_path["accepted"]
            )
            counts["terminal_child_path_changed_parents"] += terminal_child_path[
                "changed_parent_count"
            ]
            counts[
                "terminal_child_path_accepted_changed_parents"
            ] += terminal_child_path["accepted_changed_parent_count"]
            counts["terminal_child_path_compared_children"] += terminal_child_path[
                "compared_child_count"
            ]
            counts[
                "terminal_child_path_compared_container_paths"
            ] += terminal_child_path["compared_container_path_count"]
            counts[
                "terminal_child_path_available_container_paths"
            ] += terminal_child_path["available_container_path_count"]
            counts["terminal_child_path_selected_rank1_paths"] += terminal_child_path[
                "selected_rank1_path_count"
            ]
            counts["terminal_child_path_selected_rank2_paths"] += terminal_child_path[
                "selected_rank2_path_count"
            ]
            counts[
                "terminal_child_path_running_available_but_no_running_selected_children"
            ] += terminal_child_path[
                "running_available_but_no_running_selected_child_count"
            ]
            counts["terminal_child_path_unavailable"] += terminal_child_path[
                "input_unavailable_count"
            ]
            counts["terminal_child_path_running_containers"] += terminal_child_path[
                "running_container_count"
            ]
            counts["terminal_child_path_starting_containers"] += terminal_child_path[
                "starting_container_count"
            ]
            counts["terminal_child_path_no_container"] += terminal_child_path[
                "no_container_count"
            ]
            counts["terminal_child_path_invalid_profile"] += terminal_child_path[
                "invalid_profile_count"
            ]
            counts["terminal_child_path_invalid_node_snapshot"] += terminal_child_path[
                "invalid_node_snapshot_count"
            ]
            counts["terminal_child_path_invalid_network"] += terminal_child_path[
                "invalid_network_count"
            ]
            counts["terminal_child_path_nonterminal_children"] += terminal_child_path[
                "nonterminal_child_count"
            ]
            counts["terminal_child_path_worse_children"] += terminal_child_path[
                "worse_child_count"
            ]
            counts["terminal_child_path_worse_container_paths"] += terminal_child_path[
                "worse_container_path_count"
            ]
            ...
            if not candidate:
                _require(
                    ...
                    f"V131 anchor used causal state: {run['run_id']}:{line_number}",
                )
                continue
            if not gate["history_complete"]:
                _require(
                    gate["baseline_count"] == 0 and gate["recent_count"] == 0,
                    f"V131 incomplete history leaked counts: {run['run_id']}:{line_number}",
                )
                continue
            sufficient = (
            ...
            if sufficient and recent_scaled >= baseline_scaled:
                ...
                _require(
                    ...
                    f"V131 exact threshold did not activate: "
                    f"{run['run_id']}:{line_number}",
                )

    _require(
        counts["windows"] == run["simulation"]["total_frame"],
        f"V131 diagnostic window count changed: {run['run_id']}",
    )
    if candidate:
        _require(
            ...
            f"V131 mechanism was not exercised: {run['run_id']}",
        )
    return {
        ...
        "terminal_child_path_evaluated_window_count": counts[
            "terminal_child_path_evaluated_windows"
        ],
        "terminal_child_path_accepted_window_count": counts[
            "terminal_child_path_accepted_windows"
        ],
        "terminal_child_path_rejected_window_count": counts[
            "terminal_child_path_rejected_windows"
        ],
        "terminal_child_path_changed_parent_count": counts[
            "terminal_child_path_changed_parents"
        ],
        "terminal_child_path_accepted_changed_parent_count": counts[
            "terminal_child_path_accepted_changed_parents"
        ],
        "terminal_child_path_compared_child_count": counts[
            "terminal_child_path_compared_children"
        ],
        "terminal_child_path_compared_container_path_count": counts[
            "terminal_child_path_compared_container_paths"
        ],
        "terminal_child_path_available_container_path_count": counts[
            "terminal_child_path_available_container_paths"
        ],
        "terminal_child_path_selected_rank1_path_count": counts[
            "terminal_child_path_selected_rank1_paths"
        ],
        "terminal_child_path_selected_rank2_path_count": counts[
            "terminal_child_path_selected_rank2_paths"
        ],
        "terminal_child_path_running_available_but_no_running_selected_child_count": counts[
            "terminal_child_path_running_available_but_no_running_selected_children"
        ],
        "terminal_child_path_input_unavailable_count": counts[
            "terminal_child_path_unavailable"
        ],
        "terminal_child_path_running_container_count": counts[
            "terminal_child_path_running_containers"
        ],
        "terminal_child_path_starting_container_count": counts[
            "terminal_child_path_starting_containers"
        ],
        "terminal_child_path_no_container_count": counts[
            "terminal_child_path_no_container"
        ],
        "terminal_child_path_invalid_profile_count": counts[
            "terminal_child_path_invalid_profile"
        ],
        "terminal_child_path_invalid_node_snapshot_count": counts[
            "terminal_child_path_invalid_node_snapshot"
        ],
        "terminal_child_path_invalid_network_count": counts[
            "terminal_child_path_invalid_network"
        ],
        "terminal_child_path_nonterminal_child_count": counts[
            "terminal_child_path_nonterminal_children"
        ],
        "terminal_child_path_worse_child_count": counts[
            "terminal_child_path_worse_children"
        ],
        "terminal_child_path_worse_container_path_count": counts[
            "terminal_child_path_worse_container_paths"
        ],
        "terminal_child_path_maximum_same_container_alternative_minus_anchor": maximum_same_container_delta,
        "terminal_child_path_maximum_same_container_alternative_to_anchor_ratio": maximum_same_container_ratio,
...
def _verify_tape_catalog() -> tuple[list[dict[str, Any]], dict[str, Any]]:
    _require(TAPES.is_file(), f"missing V131 tape catalog: {TAPES}")
    catalog = read_json(TAPES)
    catalog_hash = _assert_hashed_object(catalog, "catalog_hash", "V131 tapes")
    entries = catalog.get("entries")
    _require(
        isinstance(entries, dict) and len(entries) == 12, "V131 tape count changed"
    )
    evidence = []
...
def _verify_reference_stage_ledgers() -> tuple[dict[str, Path], dict[Path, str]]:
    ...
    for arm_id, expected in ARMS.items():
        ...
        root = _stage_root_from_receipts(
            entries, "receipt_path", expected["run_count"], f"V131 reference {arm_id}"
        )
        roots[arm_id] = root
    ...
    for root, expected_count in expected_by_root.items():
        ...
        _assert_ledger_contract(
            ...
            f"V131 reference stage {root}",
        )
        _require(
            not list((root / "quarantine").glob("**/attempt-*")),
            f"V131 reference quarantine is nonempty: {root}",
        )
...
def _validate_run_boundary(
    ...
    required = {
        ...
        "v131_arm_id": arm_id,
        "v131_arm_role": expected["role"],
        "v131_candidate_profile": expected["profile"],
        "v131_candidate_experiment": "E3",
        "v131_shock_rate_ratio": expected["shock_rate_ratio"],
        "v131_shock_threshold_numerator": expected["shock_threshold_numerator"],
        "v131_shock_threshold_denominator": expected["shock_threshold_denominator"],
        ...
        "v131_shock_activation_horizon_frames": expected[
            "shock_activation_horizon_frames"
        ],
        "v131_nonterminal_queue_density_floor": 8.0 if candidate else None,
        "v131_warm_admissibility": "preserve_anchor_warmness" if candidate else None,
        "v131_load_least_window_certificate_mode": (
            "disabled" if candidate else "not_applicable"
        ),
        "v131_critical_service_ratio_numerator": expected[
            "critical_service_threshold_numerator"
        ],
        "v131_critical_service_ratio_denominator": expected[
    ...
    _require(
        ...
        f"V131 run boundary changed: {run['run_id']}",
    )


def run_blind_audit(output: Path = OUTPUT) -> dict[str, Any]:
    _require(not output.exists(), f"V131 blind audit already exists: {output}")
    _require(not RESULT.exists(), "V131 reveal exists before blind audit")
    _require(PLAN.is_file() and file_hash(PLAN) == PLAN_SHA256, "V131 plan changed")
    plan = read_json(PLAN)
    _require(plan.get("formal_results_eligible") is False, "V131 eligibility changed")
    _require(PREPARED.is_file(), f"missing V131 prepared receipt: {PREPARED}")
    prepared = read_json(PREPARED)
    prepared_hash = _assert_hashed_object(
        prepared, "receipt_hash", "V131 prepared receipt"
    )
    _require(
        ...
        and prepared.get("V130_result_sha256") == V130_RESULT_SHA256
        and prepared.get("V130_result_hash") == V130_RESULT_HASH
        and prepared.get("V130_result_commit") == V130_RESULT_COMMIT
        and prepared.get("V130_blind_audit_sha256") == V130_BLIND_AUDIT_SHA256
        and prepared.get("V130_blind_audit_hash") == V130_BLIND_AUDIT_HASH
        and prepared.get("V130_blind_audit_commit") == V130_BLIND_AUDIT_COMMIT
        and prepared.get("V130_performance_results_consulted") is True
        and prepared.get("V129_result_sha256") == V129_RESULT_SHA256
        and prepared.get("V129_result_hash") == V129_RESULT_HASH
        and prepared.get("V129_result_commit") == V129_RESULT_COMMIT
        and prepared.get("V129_blind_audit_sha256") == V129_BLIND_AUDIT_SHA256
        and prepared.get("V129_blind_audit_hash") == V129_BLIND_AUDIT_HASH
        and prepared.get("V129_blind_audit_commit") == V129_BLIND_AUDIT_COMMIT
        and prepared.get("V129_performance_results_consulted") is True
        and prepared.get("V127_result_sha256") == V127_RESULT_SHA256
        and prepared.get("V127_result_hash") == V127_RESULT_HASH
        and prepared.get("V127_result_commit") == V127_RESULT_COMMIT
        and prepared.get("V127_performance_results_consulted") is True
        and prepared.get("V126_blind_failure_sha256") == V126_FAILURE_SHA256
        and prepared.get("V126_blind_failure_hash") == V126_FAILURE_HASH
        and prepared.get("V126_blind_failure_commit") == V126_FAILURE_COMMIT
        and prepared.get("V126_performance_results_consulted") is False
        and prepared.get("V124_result_hash") == V124_RESULT_HASH
        and prepared.get("V124_result_commit") == V124_RESULT_COMMIT
        and prepared.get("V125_invalidation_sha256") == V125_INVALIDATION_SHA256
        ...
        "V131 prepared scientific boundary changed",
    )
    ...
    for arm_id, expected in ARMS.items():
        ...
        _require(paths["ready"].is_file(), f"missing V131 ready manifest: {arm_id}")
        ...
        _require(
            ...
            f"V131 ready boundary changed: {arm_id}",
        )
        ...
        reference_catalog_hash = _assert_hashed_object(
            references, "catalog_hash", f"V131 references {arm_id}"
        )
        ...
        _require(
            ...
            f"V131 reference key set changed: {arm_id}",
        )
        for key, entry in sorted(entries.items()):
            ...
            _require(
                ...
                f"V131 reference evidence changed: {key}",
            )
        ...
        _require(actual_ids == expected_ids, f"V131 canonical set changed: {arm_id}")
        _require(
            not list((workspace / "quarantine").glob("**/attempt-*")),
            f"V131 online quarantine is nonempty: {arm_id}",
        )
        ...
        for run in manifest["runs"]:
            ...
            _require(
                ...
                f"V131 canonical status changed: {run['run_id']}",
            )
            runtime = {
    ...
    _require(len(run_evidence) == 18, "V131 run evidence count must be 18")
    _require(len(reference_evidence) == 18, "V131 reference evidence count must be 18")
    _require(len(tape_evidence) == 12, "V131 tape evidence count must be 12")
    for field, expected in EXPECTED_RUNTIME.items():
        _require(
            runtime_values[field] == {expected},
            f"V131 runtime {field} changed: {runtime_values[field]}",
        )
    ...
    _require(
        ...
        f"V131 runtime git identity is not singular: {git_commits}",
    )
    _require(len(paired_inputs) == 9, "V131 paired input product changed")
    for (scenario, seed), rows in paired_inputs.items():
        _require(len(rows) == 2, f"V131 paired arm count changed: {scenario}/{seed}")
        for field in (
            ...
            _require(
                ...
                f"V131 paired {field} changed: {scenario}/{seed}",
            )
        _require(
            rows[0]["common_hpa_hash"] == EXPECTED_COMMON_HPA_SHA256,
            f"V131 common HPA changed: {scenario}/{seed}",
        )
    ...
    terminal_child_path_gate_evaluated_count = sum(
        row["terminal_child_path_evaluated_window_count"]
        for row in candidate_diagnostics
    )
    terminal_child_path_acceptance_count = sum(
        row["terminal_child_path_accepted_window_count"]
        for row in candidate_diagnostics
    )
    terminal_child_path_rejection_count = sum(
        row["terminal_child_path_rejected_window_count"]
        for row in candidate_diagnostics
    )
    terminal_child_path_changed_parent_count = sum(
        row["terminal_child_path_changed_parent_count"] for row in candidate_diagnostics
    )
    terminal_child_path_accepted_changed_parent_count = sum(
        row["terminal_child_path_accepted_changed_parent_count"]
        for row in candidate_diagnostics
    )
    terminal_child_path_compared_child_count = sum(
        row["terminal_child_path_compared_child_count"] for row in candidate_diagnostics
    )
    terminal_child_path_compared_container_path_count = sum(
        row["terminal_child_path_compared_container_path_count"]
        for row in candidate_diagnostics
    )
    terminal_child_path_available_container_path_count = sum(
        row["terminal_child_path_available_container_path_count"]
        for row in candidate_diagnostics
    )
    terminal_child_path_selected_rank1_path_count = sum(
        row["terminal_child_path_selected_rank1_path_count"]
        for row in candidate_diagnostics
    )
    terminal_child_path_selected_rank2_path_count = sum(
        row["terminal_child_path_selected_rank2_path_count"]
        for row in candidate_diagnostics
    )
    terminal_child_path_running_available_but_no_running_selected_child_count = sum(
        row["terminal_child_path_running_available_but_no_running_selected_child_count"]
        for row in candidate_diagnostics
    )
    terminal_child_path_input_unavailable_count = sum(
        row["terminal_child_path_input_unavailable_count"]
        for row in candidate_diagnostics
    )
    terminal_child_path_running_container_count = sum(
        row["terminal_child_path_running_container_count"]
        for row in candidate_diagnostics
    )
    terminal_child_path_starting_container_count = sum(
        row["terminal_child_path_starting_container_count"]
        for row in candidate_diagnostics
    )
    terminal_child_path_no_container_count = sum(
        row["terminal_child_path_no_container_count"] for row in candidate_diagnostics
    )
    terminal_child_path_invalid_profile_count = sum(
        row["terminal_child_path_invalid_profile_count"]
        for row in candidate_diagnostics
    )
    terminal_child_path_invalid_node_snapshot_count = sum(
        row["terminal_child_path_invalid_node_snapshot_count"]
        for row in candidate_diagnostics
    )
    terminal_child_path_invalid_network_count = sum(
        row["terminal_child_path_invalid_network_count"]
        for row in candidate_diagnostics
    )
    terminal_child_path_nonterminal_child_count = sum(
        row["terminal_child_path_nonterminal_child_count"]
        for row in candidate_diagnostics
    )
    terminal_child_path_worse_child_count = sum(
        row["terminal_child_path_worse_child_count"] for row in candidate_diagnostics
    )
    terminal_child_path_worse_container_path_count = sum(
        row["terminal_child_path_worse_container_path_count"]
        for row in candidate_diagnostics
    )
    terminal_child_path_maximum_same_container_delta = max(
        (
            row["terminal_child_path_maximum_same_container_alternative_minus_anchor"]
            for row in candidate_diagnostics
            if row[
                "terminal_child_path_maximum_same_container_alternative_minus_anchor"
            ]
            is not None
        ),
        default=None,
    )
    terminal_child_path_maximum_same_container_ratio = max(
        (
            row[
                "terminal_child_path_maximum_same_container_alternative_to_anchor_ratio"
            ]
            for row in candidate_diagnostics
            if row[
                "terminal_child_path_maximum_same_container_alternative_to_anchor_ratio"
    ...
    _require(
        ...
        "V131 completion-proximal and deeper current-player paths were not both exercised",
    )
    _require(
        completion_proximal_service_choice_evaluated_count > 0
        and completion_proximal_substitution_count > 0,
        "V131 completion-proximal service-choice evaluation/substitution path was not exercised",
    )
    _require(
        deeper_exact_anchor_preserved_count > 0 and deeper_substitution_count == 0,
        "V131 deeper-player exact-anchor preservation contract changed",
    )
    _require(
        proxy_input_unavailable_count == 0,
        "V131 admitted-work proxy inputs were unavailable",
    )
    _require(
        ...
        "V131 componentwise service accept/reject paths were not both exercised",
    )
    _require(
        componentwise_input_unavailable_count == 0,
        "V131 componentwise service inputs were unavailable",
    )
    _require(
        terminal_child_path_gate_evaluated_count > 0
        and terminal_child_path_changed_parent_count > 0
        and terminal_child_path_accepted_changed_parent_count > 0
        and terminal_child_path_compared_child_count > 0
        and terminal_child_path_available_container_path_count
        >= terminal_child_path_compared_container_path_count
        and terminal_child_path_selected_rank1_path_count
        == terminal_child_path_compared_child_count
        and terminal_child_path_selected_rank2_path_count > 0
        and terminal_child_path_compared_container_path_count
        == terminal_child_path_selected_rank1_path_count
        + terminal_child_path_selected_rank2_path_count
        and terminal_child_path_maximum_same_container_delta is not None
        and terminal_child_path_maximum_same_container_ratio is not None
        and terminal_child_path_running_container_count > 0
        and terminal_child_path_starting_container_count > 0
        and terminal_child_path_running_available_but_no_running_selected_child_count
        == 0
        and terminal_child_path_acceptance_count > 0
        and terminal_child_path_rejection_count > 0,
        "V131 terminal-child anchor-top2 accept/reject paths were not both exercised",
    )
    _require(
        terminal_child_path_input_unavailable_count == 0,
        "V131 terminal-child path inputs were unavailable",
    )
    _require(
        terminal_child_path_no_container_count == 0
        and terminal_child_path_invalid_profile_count == 0
        and terminal_child_path_invalid_node_snapshot_count == 0
        and terminal_child_path_invalid_network_count == 0,
        "V131 terminal-child HPA path fail-closed reason count was nonzero",
    )
    _require(
        terminal_child_path_nonterminal_child_count == 0,
        "V131 terminal-child path scope included a nonterminal child",
    )
    _require(
        terminal_child_path_worse_container_path_count > 0,
        "V131 anchor-top2 worse-path rejection was not exercised",
    )

    output_payload = {
        "schema_version": "NSE_E3_COMPLETION_PROXIMAL_TERMINAL_CHILD_HPA_ANCHOR_RUNNING_FIRST_TOP2_PATH_BLIND_AUDIT_V131_V1",
        "created_at": utc_now(),
        "status": "pass",
        ...
        "terminal_child_anchor_running_first_top2_path_mechanism_coverage": {
            "terminal_child_anchor_running_first_top2_gate_evaluated_count": terminal_child_path_gate_evaluated_count,
            "terminal_child_anchor_running_first_top2_changed_parent_count": terminal_child_path_changed_parent_count,
            "terminal_child_anchor_running_first_top2_accepted_changed_parent_count": terminal_child_path_accepted_changed_parent_count,
            "terminal_child_anchor_running_first_top2_compared_child_count": terminal_child_path_compared_child_count,
            "terminal_child_anchor_running_first_top2_available_container_path_count": terminal_child_path_available_container_path_count,
            "terminal_child_anchor_running_first_top2_selected_rank1_path_count": terminal_child_path_selected_rank1_path_count,
            "terminal_child_anchor_running_first_top2_selected_rank2_path_count": terminal_child_path_selected_rank2_path_count,
            "terminal_child_anchor_running_first_top2_selected_path_count": terminal_child_path_compared_container_path_count,
            "terminal_child_anchor_running_first_top2_running_available_but_no_running_selected_child_count": terminal_child_path_running_available_but_no_running_selected_child_count,
            "terminal_child_anchor_running_first_top2_acceptance_count": terminal_child_path_acceptance_count,
            "terminal_child_anchor_running_first_top2_rejection_count": terminal_child_path_rejection_count,
            "terminal_child_anchor_running_first_top2_input_unavailable_count": terminal_child_path_input_unavailable_count,
            "terminal_child_anchor_running_first_top2_running_container_path_count": terminal_child_path_running_container_count,
            "terminal_child_anchor_running_first_top2_starting_container_path_count": terminal_child_path_starting_container_count,
            "terminal_child_anchor_running_first_top2_no_container_count": terminal_child_path_no_container_count,
            "terminal_child_anchor_running_first_top2_invalid_profile_count": terminal_child_path_invalid_profile_count,
            "terminal_child_anchor_running_first_top2_invalid_node_snapshot_count": terminal_child_path_invalid_node_snapshot_count,
            "terminal_child_anchor_running_first_top2_invalid_network_count": terminal_child_path_invalid_network_count,
            "terminal_child_anchor_running_first_top2_nonterminal_child_count": terminal_child_path_nonterminal_child_count,
            "terminal_child_anchor_running_first_top2_worse_child_count": terminal_child_path_worse_child_count,
            "terminal_child_anchor_running_first_top2_worse_path_count": terminal_child_path_worse_container_path_count,
            "terminal_child_anchor_running_first_top2_maximum_alternative_minus_anchor_for_same_container": terminal_child_path_maximum_same_container_delta,
            "terminal_child_anchor_running_first_top2_maximum_alternative_to_anchor_ratio_for_same_container": terminal_child_path_maximum_same_container_ratio,
            "performance_fields_consulted": False,
        },
        "arm_count": len(ARMS),
