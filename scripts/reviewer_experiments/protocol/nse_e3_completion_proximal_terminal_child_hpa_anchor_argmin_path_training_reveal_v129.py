...
from scripts.reviewer_experiments.protocol.nse_e3_completion_proximal_terminal_child_hpa_anchor_argmin_path_training_blind_audit_v129 import (
    ARMS,
...
OUTPUT = ROOT / "training-result-v129.json"
BLIND_AUDIT_FILE_SHA256 = "__FREEZE_AFTER_V129_BLIND_AUDIT__"
BLIND_AUDIT_HASH = "__FREEZE_AFTER_V129_BLIND_AUDIT__"
EXPECTED_SEEDS = ("E1310", "E1311", "E1312")
EXPECTED_SCENARIOS = (
    "E3.spike5x50ms",
    "E3.sustained3x200ms",
    "E3.pulse4x4x50ms",
...
ANCHOR_ID = "v129-e3-anchor"
CANDIDATE_ID = (
    "v129-e3-causal-shock15-horizon50-admitted-work10-"
    "completion-proximal-depth1-componentwise-service-terminal-child-hpa-anchor-argmin-path-pareto-"
    "initializer-only"
)
...
def _validate_blind_audit() -> dict[str, Any]:
    ...
    if any(
        ...
        raise RuntimeError("V129 blind audit has not been frozen into the reveal code")
    if not BLIND_AUDIT.is_file() or file_hash(BLIND_AUDIT) != BLIND_AUDIT_FILE_SHA256:
        raise RuntimeError("V129 blind audit is missing or changed")
    blind = read_json(BLIND_AUDIT)
    payload = dict(blind)
    claimed = payload.pop("audit_hash", None)
    coverage = blind.get("completion_proximal_depth1_mechanism_coverage", {})
    componentwise = blind.get("componentwise_service_mechanism_coverage", {})
    terminal_child_path = blind.get(
    ...
    if (
        ...
        or coverage.get("completion_proximal_player_count", 0) <= 0
        or coverage.get("deeper_player_count", 0) <= 0
        or coverage.get("completion_proximal_service_choice_evaluated_count", 0) <= 0
        or coverage.get("completion_proximal_substitution_count", 0) <= 0
        or coverage.get("deeper_exact_anchor_preserved_count", 0) <= 0
        or coverage.get("deeper_substitution_count") != 0
        or coverage.get("proxy_input_unavailable_count") != 0
        or coverage.get("performance_fields_consulted") is not False
        or not isinstance(componentwise, dict)
        or componentwise.get("componentwise_service_gate_evaluated_count", 0) <= 0
        or componentwise.get("componentwise_service_compared_player_count", 0) <= 0
        or componentwise.get("componentwise_service_acceptance_count", 0) <= 0
        or componentwise.get("componentwise_service_rejection_count", 0) <= 0
        or componentwise.get("componentwise_input_unavailable_count") != 0
        or componentwise.get("performance_fields_consulted") is not False
        or not isinstance(terminal_child_path, dict)
        or terminal_child_path.get(
            "terminal_child_anchor_argmin_gate_evaluated_count", 0
        )
        <= 0
        or terminal_child_path.get(
            "terminal_child_anchor_argmin_changed_parent_count", 0
        )
        <= 0
        or terminal_child_path.get(
            "terminal_child_anchor_argmin_accepted_changed_parent_count", 0
        )
        <= 0
        or terminal_child_path.get(
            "terminal_child_anchor_argmin_compared_child_count", 0
        )
        <= 0
        or terminal_child_path.get(
            "terminal_child_anchor_argmin_selected_path_count", 0
        )
        <= 0
        or terminal_child_path.get(
            "terminal_child_anchor_argmin_running_container_path_count", 0
        )
        <= 0
        or terminal_child_path.get(
            "terminal_child_anchor_argmin_starting_container_path_count", 0
        )
        <= 0
        or terminal_child_path.get("terminal_child_anchor_argmin_acceptance_count", 0)
        <= 0
        or terminal_child_path.get("terminal_child_anchor_argmin_rejection_count", 0)
        <= 0
        or terminal_child_path.get(
            "terminal_child_anchor_argmin_input_unavailable_count"
        )
        != 0
        or terminal_child_path.get(
            "terminal_child_anchor_argmin_nonterminal_child_count"
        )
        != 0
        or terminal_child_path.get("terminal_child_anchor_argmin_no_container_count")
        != 0
        or terminal_child_path.get("terminal_child_anchor_argmin_invalid_profile_count")
        != 0
        or terminal_child_path.get(
            "terminal_child_anchor_argmin_invalid_node_snapshot_count"
        )
        != 0
        or terminal_child_path.get("terminal_child_anchor_argmin_invalid_network_count")
        != 0
        or terminal_child_path.get("terminal_child_anchor_argmin_worse_path_count", 0)
        <= 0
        or not _finite(
            terminal_child_path.get(
                "terminal_child_anchor_argmin_maximum_alternative_minus_anchor_for_same_container"
            )
        )
        or not _finite(
            terminal_child_path.get(
                "terminal_child_anchor_argmin_maximum_alternative_to_anchor_ratio_for_same_container"
            )
        )
        or terminal_child_path.get("performance_fields_consulted") is not False
    ):
        raise RuntimeError("V129 blind audit does not authorize reveal")
    return blind
...
def _load_rows(blind: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    if len(admitted) != 18:
        raise RuntimeError("V129 blind audit run evidence is incomplete")
    ...
    for arm_id, expected in ARMS.items():
        ...
        if not manifest_path.is_file() or file_hash(manifest_path) != frozen.get(
            ...
            raise RuntimeError(f"V129 frozen manifest changed: {arm_id}")
        manifest = load_and_validate_manifest(manifest_path)
        if manifest.get("manifest_hash") != frozen.get("manifest_hash"):
            raise RuntimeError(f"V129 manifest hash changed: {arm_id}")
        for run in manifest["runs"]:
            ...
            if (
                ...
                != expected["profile"]
                or metadata.get("v129_training_plan_sha256") != PLAN_SHA256
                or metadata.get("v129_training_only") is not True
                or metadata.get("v129_training_seed_metrics_previously_revealed")
                is not False
                or metadata.get("v129_confirmation_seeds_opened") is not False
                or metadata.get("v129_other_unopened_seeds_opened") is not False
                or metadata.get("v129_arm_id") != arm_id
                or metadata.get("v129_arm_role") != expected["role"]
                or metadata.get("v129_shock_rate_ratio") != expected["shock_rate_ratio"]
                or metadata.get("v129_shock_threshold_numerator")
                != expected["shock_threshold_numerator"]
                or metadata.get("v129_shock_threshold_denominator")
                != expected["shock_threshold_denominator"]
                or metadata.get("v129_shock_activation_horizon_frames")
                != expected["shock_activation_horizon_frames"]
                or metadata.get("v129_critical_service_ratio_numerator")
                != expected["critical_service_threshold_numerator"]
                or metadata.get("v129_critical_service_ratio_denominator")
                != expected["critical_service_threshold_denominator"]
                or metadata.get("v129_service_proxy_work_source")
                != (
                    "admitted_pending_plus_all_resident_remaining_and_same_window_projected_cpu_v1"
                    if candidate
                    else "not_applicable"
                )
                or metadata.get("v129_admitted_work_includes_all_blocked_resident")
                is not candidate
                or metadata.get("v129_completion_proximal_depth1_substitution_scope")
                is not candidate
                or metadata.get("v129_completion_proximal_definition")
                != (
                    "terminal_function_or_nonterminal_function_whose_immutable_DAG_children_are_all_terminal"
                    if candidate
                    else "not_applicable"
                )
                or metadata.get("v129_maximum_remaining_child_depth")
                != (1 if candidate else None)
                or metadata.get(
                    "v129_completion_proximal_critical_frontier_substitution"
                )
                is not candidate
                or metadata.get("v129_deeper_player_exact_anchor") is not candidate
                or metadata.get("v129_complete_componentwise_service_pareto")
                is not candidate
                or metadata.get("v129_componentwise_service_scope")
                != (
                    "all_current_critical_frontier_players"
                    if candidate
                    else "not_applicable"
                )
                or metadata.get("v129_componentwise_service_comparison")
                != (
                ...
                or metadata.get(
                    "v129_complete_terminal_child_hpa_anchor_argmin_path_service_pareto"
                )
                is not candidate
                or metadata.get("v129_terminal_child_hpa_anchor_argmin_path_scope")
                != (
                    "every_immutable_immediate_child_of_each_actually_changed_completion_proximal_player"
                    if candidate
                    else "not_applicable"
                )
                or metadata.get("v129_terminal_child_hpa_anchor_argmin_path_proxy")
                != (
                    "parent_output_transfer_plus_full_cold_start_if_starting_plus_child_node_admitted_cpu_service"
                    if candidate
                    else "not_applicable"
                )
                or metadata.get("v129_terminal_child_hpa_container_node_scope")
                != (
                    "all_current_HPA_owned_child_containers_running_or_starting"
                    if candidate
                    else "not_applicable"
                )
                or metadata.get("v129_terminal_child_hpa_anchor_argmin_path_comparison")
                != (
                    "every_terminal_child_alternative_path_to_the_deterministic_anchor_argmin_hpa_container_less_than_or_equal_to_anchor"
                    if candidate
                    else "not_applicable"
                )
                or metadata.get("v129_terminal_child_hpa_anchor_argmin_path_pairing")
                != (
                ...
                or metadata.get("v129_terminal_child_hpa_anchor_argmin_selection")
                != (
                ...
                or metadata.get(
                    "v129_terminal_child_hpa_anchor_argmin_path_input_fail_closed"
                )
                is not candidate
                or metadata.get("v129_outcome_fields_drive_policy") is not False
                or metadata.get("v129_scenario_or_burst_label_used_by_policy")
                is not False
                or metadata.get("v129_completion_or_performance_fields_used_by_policy")
                is not False
                or metadata.get("v129_future_arrivals_used_by_policy") is not False
            ):
                raise RuntimeError(f"V129 arm identity changed: {run['run_id']}")
            run_id = run["run_id"]
            summary_path = (
                ROOT
                / "runs"
            ...
            if not summary_path.is_file() or file_hash(summary_path) != admitted.get(
                ...
                raise RuntimeError(f"V129 summary differs from blind audit: {run_id}")
            ...
            if (
                ...
                raise RuntimeError(
                    f"V129 summary identity/completion mismatch: {run_id}"
                )
            rows.append(
                {
                    "arm_id": arm_id,
...
def execute_reveal() -> dict[str, Any]:
    if OUTPUT.exists():
        raise RuntimeError(f"V129 training result already exists: {OUTPUT}")
    if not PLAN.is_file() or file_hash(PLAN) != PLAN_SHA256:
        raise RuntimeError("frozen V129 plan changed before reveal")
    blind = _validate_blind_audit()
    rows = _load_rows(blind)
    if len(rows) != 18:
        raise RuntimeError("V129 revealed row count differs from sealed plan")
    ...
    result = {
        "schema_version": "NSE_E3_COMPLETION_PROXIMAL_TERMINAL_CHILD_HPA_ANCHOR_ARGMIN_PATH_RESULT_V129_V1",
        "created_at": utc_now(),
        ...
        **evaluation,
        "revealed_rows": rows,
        "decision": {
            "authorize_preregistration_of_E1313_E1332_confirmation": joint_pass,
            ...
            "next_action": (
                "freeze V129 with frozen V97 E4 and separately preregister the "
                "complete E1313-E1332 NSESche confirmation before generating inputs"
                if joint_pass
                else (
                    "freeze the complete V129 training cohort, keep E1313-E1332 and "
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Reveal the frozen V129 training cohort exactly once after blind audit."
    )
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    if args.command != "reveal" or not args.execute:
        parser.error("V129 reveal requires the explicit command: reveal --execute")
    execute_reveal()
