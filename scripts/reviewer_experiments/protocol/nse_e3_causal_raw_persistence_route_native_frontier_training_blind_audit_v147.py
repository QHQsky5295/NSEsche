...
from scripts.reviewer_experiments.protocol.nse_e3_causal_raw_persistence_route_native_frontier_training_execute_v147 import (
    EXECUTION_RECEIPT,
    READY_SCHEDULE,
)
from scripts.reviewer_experiments.protocol.nse_e3_causal_raw_persistence_route_native_frontier_training_prepare_v147 import (
    ...
    V146_PLAN_SHA256,
    V147_SERVICE_STATE_DOMAIN,
    V147_WELFARE_STATE_DOMAIN,
    pairing_path,
    paths,
    ready_manifest_path,
...
PREPARED = ROOT / "prepared-manifest-v147.json"
OUTPUT = ROOT / "joint-blind-audit-v147-training.json"
RESULT = ROOT / "training-result-v147.json"
EXPECTED_RUNTIME = {
...
def _persistence_comparison(recent: int, baseline: int) -> dict[str, Any]:
    if (
        ...
        raise ValueError("V147 persistence counts must be nonnegative integers")
    recent_scaled = (
...
def _validate_guard(
    ...
    _require(isinstance(reason, str), "invalid V147 guard reason")
    ...
    _require(
        ...
        f"V147 native service boundary changed: {run_id}:{line_number}",
    )
    if players == 0:
        _require(
            selected is None
            ...
            f"V147 empty guard changed: {run_id}:{line_number}",
        )
        return reason, False
    ...
    _require(
        ...
        f"V147 selected initializer changed: {run_id}:{line_number}",
    )
    ...
    if proposal_complete:
        _require(
            ...
            f"V147 proposal service changed: {run_id}:{line_number}",
        )
        ...
        _require(
            ...
            f"V147 proposal deltas changed: {run_id}:{line_number}",
        )
    else:
        _require(
            proposal_players == 0
            ...
            f"V147 unavailable proposal changed: {run_id}:{line_number}",
        )
    ...
    _require(
        ...
        == float(welfare_delta),
        f"V147 welfare certificate changed: {run_id}:{line_number}",
    )
    ...
    _require(
        ...
        f"V147 guard disposition changed: {run_id}:{line_number}",
    )
    return reason, accepted


def _validate_v147_native_diagnostics(
    run: dict[str, Any], canonical: Path
) -> dict[str, Any]:
    path = canonical / "reviewer_records" / run["run_id"] / "nash_metrics.jsonl.gz"
    _require(path.is_file(), f"missing V147 Nash diagnostics: {run['run_id']}")
    counts = Counter()
    reasons = Counter()
        ...
        for line_number, line in enumerate(handle, start=1):
            ...
            if event.get("kind") == "run_config":
                ...
                _require(
                    ...
                    and contract.get("version") == "V147"
                    and contract.get("router")
                    == "frozen_V145_causal_raw_persistence_state_machine"
                    and contract.get("native_frontiers") == ROUTE_FRONTIERS
                    and contract.get("cross_expert_tail_fill") is False
                    and contract.get("uses_completed_request_outcomes") is False
                    and contract.get("reference_policy_independent") is True,
                    f"V147 run-config changed: {run['run_id']}:{line_number}",
                )
                continue
            ...
            _require(isinstance(decision, dict), "missing V147 decision diagnostics")
            ...
            route_native_frontier = (
                portfolio.get("v147_causal_raw_persistence_route_native_frontier")
                if isinstance(portfolio, dict)
                else None
            )
            retired_full_frontier = (
            ...
            _require(
                ...
                f"missing V147 persistence diagnostics: {run['run_id']}:{line_number}",
            )
            ...
            _require(
                ...
                is False,
                f"V147 causal input boundary changed: {run['run_id']}:{line_number}",
            )
            ...
            _require(
                ...
                f"V147 detector changed: {run['run_id']}:{line_number}",
            )
            ...
            if started:
                ...
                if expected_episode_count == 1:
                    ...
                    _require(
                        ...
                        f"V147 first episode lacks preceding baseline: {run['run_id']}:{line_number}",
                    )
                    ...
                    _require(
                        ...
                        f"V147 first episode baseline is ineligible: {run['run_id']}:{line_number}",
                    )
            expected_first_age = (
            ...
            if (
                ...
                _require(
                    ...
                    f"V147 persistence baseline disappeared: {run['run_id']}:{line_number}",
                )
            ...
            if persistence_evaluated:
                _require(
                    ...
                    f"V147 evaluated persistence state is incomplete: {run['run_id']}:{line_number}",
                )
            ...
            _require(
                ...
                f"V147 state transition changed: {run['run_id']}:{line_number}",
            )
            ...
            _require(
                ...
                f"V147 player count changed: {run['run_id']}:{line_number}",
            )
            frontier_counts = route_native_frontier.get("native_frontier_player_counts")
            native_counts = route_native_frontier.get("native_command_counts")
            native_hashes = route_native_frontier.get("native_ordered_command_hashes")
            selected_frontier = route_native_frontier.get("selected_frontier")
            selected_frontier_count = route_native_frontier.get(
                "selected_frontier_player_count"
            )
            _require(
                route_native_frontier.get("enabled") is True
                and route_native_frontier.get("frame") == frame
                and route_native_frontier.get("router_selected_kind") == selected_kind
                and route_native_frontier.get("router_selection_reason")
                == selection_reason
                and route_native_frontier.get("native_frontiers") == ROUTE_FRONTIERS
                and selected_frontier == ROUTE_FRONTIERS[selected_kind]
                and selected_frontier_count == players
                and type(
                    route_native_frontier.get("selected_frontier_player_order_hash")
                )
                is int
                and route_native_frontier.get(
                    "every_native_capture_complete_on_own_frontier"
                )
                is True
                and route_native_frontier.get("no_cross_expert_cohort_tail_fill")
                is True
                and route_native_frontier.get("shadow_invocations_this_window")
                == {kind: 1 for kind in RUNTIME_NATIVE_KINDS}
                and route_native_frontier.get(
                    "all_three_shadows_advanced_exactly_once_this_window"
                )
                is True
                and retired_full_frontier.get("enabled") is False
                and all(
                    isinstance(item, dict)
                    for item in (
                        frontier_counts,
                ...
                f"V147 route-native-frontier boundary changed: {run['run_id']}:{line_number}",
            )
            for kind in RUNTIME_NATIVE_KINDS:
                frontier_count = frontier_counts.get(kind)
                native_count = native_counts.get(kind)
                _require(
                    type(frontier_count) is int
                    and frontier_count >= 0
                    and native_count == frontier_count
                    and type(native_hashes.get(kind)) is int,
                    f"V147 {kind} native frontier changed: {run['run_id']}:{line_number}",
                )
                counts[f"frontier_{kind}_players"] += frontier_count
                counts[f"native_{kind}_commands"] += native_count
            _require(
                frontier_counts[selected_kind] == players
                and native_counts[selected_kind] == players,
                f"V147 selected frontier count changed: {run['run_id']}:{line_number}",
            )
            ...
            if players == 0:
                _require(
                    ...
                    f"V147 empty portfolio changed: {run['run_id']}:{line_number}",
                )
                _require(
                    route_native_frontier.get(
                        "selected_capture_complete_on_own_frontier"
                    )
                    is False,
                    f"V147 empty selected capture changed: {run['run_id']}:{line_number}",
                )
            else:
                active_player_windows += 1
                _require(
                    portfolio.get("enabled") is True
                    and portfolio.get("rule")
                    == "causal_raw_persistence_route_native_frontier"
                    and portfolio.get("deterministic_selection_reason")
                    == selection_reason
                    and portfolio.get("candidate_count") == 1
                    and isinstance(candidates, list)
                    and [item.get("kind") for item in candidates] == [selected_kind]
                    and [item.get("selected") for item in candidates] == [True]
                    and portfolio.get("selected_kind") == selected_kind,
                    f"V147 portfolio selection changed: {run['run_id']}:{line_number}",
                )
                for candidate in candidates:
                    _require(
                ...
                _require(
                    ...
                    is True,
                    f"V147 selected capture changed: {run['run_id']}:{line_number}",
                )
            ...
            _require(
                ...
                is True,
                f"V147 expert lifecycle changed: {run['run_id']}:{line_number}",
            )
            if selected_candidate is None:
                ...
                _require(
                    ...
                    is False,
                    f"V147 empty dispatch changed: {run['run_id']}:{line_number}",
                )
            else:
                _require(
                    ...
                    == selected_candidate["ordered_command_hash"]
                    and route_native_frontier.get("selected_native_assignment_hash")
                    == selected_candidate["assignment_hash"],
                    f"V147 selected hashes changed: {run['run_id']}:{line_number}",
                )
            ...
            if players > 0:
                _require(
                    ...
                    and route_native_frontier.get("final_assignment_hash")
                    == persistence.get("final_assignment_hash"),
                    f"V147 final dispatch changed: {run['run_id']}:{line_number}",
                )
            if accepted:
                counts["accepted_windows"] += 1

    _require(
        counts["run_config"] == 1, f"V147 run-config count changed: {run['run_id']}"
    )
    _require(counts["windows"] == 4000, f"V147 window count changed: {run['run_id']}")
    _require(active_player_windows > 0, f"V147 has no player windows: {run['run_id']}")
    return {
        ...
        "native_frontier_player_totals": {
            kind: counts[f"frontier_{kind}_players"] for kind in RUNTIME_NATIVE_KINDS
        },
        "native_command_totals": {
            kind: counts[f"native_{kind}_commands"] for kind in RUNTIME_NATIVE_KINDS
        },
        "cross_expert_tail_fill_observed": False,
...
def _verify_references(candidate_manifest: Mapping[str, Any]) -> dict[str, Any]:
    ...
    _require(path.is_file(), "missing V147 reference catalog")
    catalog = read_json(path)
    catalog_hash = _assert_hashed_object(catalog, "catalog_hash", "V147 references")
    ...
    _require(
        ...
        "V147 reference product changed",
    )
    stage_root = _stage_root_from_receipts(
        entries, "receipt_path", 9, "V147 references"
    )
    ...
    _require(
        ...
        f"V147 reference ledger changed: {counts}",
    )
    bound = {
    ...
    for key, entry in sorted(entries.items()):
        ...
        _require(
            ...
            f"V147 reference evidence changed: {key}",
        )
        evidence.append(
            {
                "key": key,
...
def _validate_execution_receipt() -> dict[str, Any]:
    ...
    _require(schedule_path.is_file(), "missing V147 ready schedule")
    _require(receipt_path.is_file(), "missing V147 execution receipt")
    schedule = read_json(schedule_path)
    schedule_hash = _assert_hashed_object(
        schedule, "schedule_hash", "V147 ready schedule"
    )
    ...
    _require(
        ...
        "V147 ready schedule binding changed",
    )
    receipt = read_json(receipt_path)
    receipt_hash = _assert_hashed_object(
        receipt, "receipt_hash", "V147 execution receipt"
    )
    _require(
        ...
        "V147 execution boundary changed",
    )
    for scheduled, dispatched in zip(schedule["schedule"], receipt["dispatches"]):
        _require(
            ...
            f"V147 frozen dispatch changed: {scheduled['ordinal']}",
        )
    return {
...
def run_blind_audit(output: Path = OUTPUT) -> dict[str, Any]:
    _require(not output.exists(), f"V147 blind audit already exists: {output}")
    _require(not RESULT.exists(), "V147 reveal exists before blind audit")
    _require(PLAN.is_file() and file_hash(PLAN) == PLAN_SHA256, "V147 plan changed")
    _require(PREPARED.is_file(), "missing V147 prepared receipt")
    prepared = read_json(PREPARED)
    prepared_hash = _assert_hashed_object(
        prepared, "receipt_hash", "V147 prepared receipt"
    )
    ...
    _require(
        ...
        and prepared.get("route_frontiers") == ROUTE_FRONTIERS
        and prepared.get("single_factor_change")
        == "universal_all_unscheduled_to_route_selected_expert_native_frontier"
        and prepared.get("native_frontier_command_evidence_required") is True
        ...
        and prepared.get("v146_plan_file_sha256") == V146_PLAN_SHA256
        and prepared.get("v146_invalidation_file_sha256") == V146_INVALIDATION_SHA256
        and prepared.get("v146_invalidation_hash") == V146_INVALIDATION_HASH
        and prepared.get("parent_disposition")
        == "V146_retired_before_online_execution_after_all_nine_reference_dependencies_blocked"
        and prepared.get("training_seeds") == TRAINING_SEED_LIST
        and prepared.get("sealed_confirmation_seeds") == NEW_CONFIRMATION_SEEDS,
        "V147 prepared boundary changed",
    )
    _require(
        ...
        "V147 prepared unbound manifest changed",
    )
    ...
    _require(
        ...
        "V147 ready manifest boundary changed",
    )
    _require(
        ...
        "V147 ready schedule no longer matches the manifest",
    )
    ...
    _require(actual_ids == expected_ids, "V147 canonical set changed")
    _require(
        not list((workspace / "quarantine").glob("**/attempt-*")),
        "V147 online quarantine is nonempty",
    )
    ...
    _require(
        ...
        f"V147 online ledger changed: {ledger_counts}",
    )
    ...
    for run in candidate_manifest["runs"]:
        ...
        _require(
            ...
            == PROFILE
            and metadata.get("v147_native_selection_rule") == SELECTION_RULE
            and metadata.get("v147_native_portfolio_members") == NATIVE_MEMBERS
            and metadata.get("v147_all_three_experts_advanced_every_window") is True
            and metadata.get("v147_shock_detector")
            == "first_seen_arrivals_80_frame_baseline_20_frame_recent_3_over_2_threshold_50_frame_latch_for_episode_onset_only"
            and metadata.get("v147_persistence_test")
            ...
            and metadata.get("v147_player_frontier") == PLAYER_FRONTIER
            and metadata.get("v147_route_frontiers") == ROUTE_FRONTIERS
            and metadata.get("v147_single_factor_change")
            == "universal_all_unscheduled_to_route_selected_expert_native_frontier"
            and metadata.get("v147_native_frontier_command_evidence_required") is True
            and metadata.get("v147_parent_v146_plan_sha256") == V146_PLAN_SHA256
            and metadata.get("v147_parent_v146_invalidation_sha256")
            == V146_INVALIDATION_SHA256
            and metadata.get("v147_parent_v146_invalidation_hash")
            == V146_INVALIDATION_HASH
            and metadata.get("v147_quiet_route") == "greedy"
            and metadata.get("v147_first_short_route") == "greedy"
            and metadata.get("v147_first_sustained_route") == "faasrank"
            and metadata.get("v147_recurrent_route") == "load_least"
            and metadata.get("v147_service_certificate_scope")
            == SERVICE_CERTIFICATE_SCOPE
            and metadata.get("v147_service_certificate_state_domain")
            == V147_SERVICE_STATE_DOMAIN
            and metadata.get("v147_paper_welfare_state_domain")
            == V147_WELFARE_STATE_DOMAIN
            and metadata.get("v147_outcome_fields_drive_policy") is False
            and metadata.get("v147_scenario_or_burst_label_used_by_policy") is False
            and metadata.get("v147_future_arrivals_used_by_policy") is False
            and metadata.get("v147_confirmation_inputs_opened") is False
            and isinstance(run.get("reference_dependency"), dict),
            f"V147 candidate manifest boundary changed: {run['run_id']}",
        )
        ...
        _require(
            ...
            f"V147 canonical status changed: {run['run_id']}",
        )
        runtime = _runtime_evidence(audit)
        for field, value in runtime.items():
            runtime_values[field].add(value)
        diagnostics = _validate_v147_native_diagnostics(run, canonical)
        paired = {
            "method_label": ARM_ID,
            "run_id": run["run_id"],
    ...
    for field, expected in EXPECTED_RUNTIME.items():
        _require(
            ...
            f"V147 runtime {field} changed: {runtime_values[field]}",
        )
    ...
    _require(
        ...
        f"V147 runtime git identity changed: {git_commits}",
    )
    _require(len(paired_inputs) == 9, "V147 paired block count changed")
    for (scenario, seed), rows in paired_inputs.items():
        _require(
            len(rows) == 10
            and {row["method_label"] for row in rows} == {*BASELINE_METHODS, ARM_ID},
            f"V147 paired product changed: {scenario}/{seed}",
        )
        for field in (
            ...
            _require(
                ...
                f"V147 paired {field} changed: {scenario}/{seed}",
            )
        _require(
            rows[0]["common_hpa_hash"] == EXPECTED_COMMON_HPA_SHA256,
            f"V147 common HPA changed: {scenario}/{seed}",
        )

    payload = {
        "schema_version": "NSE_E3_CAUSAL_RAW_PERSISTENCE_ROUTE_NATIVE_FRONTIER_BLIND_AUDIT_V147_V1",
        "created_at": utc_now(),
        "status": "pass",
