...
from scripts.reviewer_experiments.protocol.nse_e3_all_native_portfolio_training_execute_v139 import (
    EXECUTION_RECEIPT,
    READY_SCHEDULE,
    ready_manifest_path,
    workspace_path,
...
OUTPUT = ROOT / "joint-blind-audit-v139-training.json"
RESULT = ROOT / "training-result-v139.json"
...
def _verify_tapes() -> tuple[list[dict[str, Any]], dict[str, Any]]:
    _require(TAPES.is_file(), f"missing V139 tape catalog: {TAPES}")
    catalog = read_json(TAPES)
    catalog_hash = _assert_hashed_object(catalog, "catalog_hash", "V139 tapes")
    entries = catalog.get("entries")
    _require(
        isinstance(entries, dict) and len(entries) == 12, "V139 tape count changed"
    )
    evidence = []
    for key, entry in sorted(entries.items()):
        info = inspect_tape(Path(entry["path"]))
        _require(info.sha256 == entry["sha256"], f"V139 tape hash changed: {key}")
        _require(info.workload_seed in TRAINING_SEEDS, f"V139 tape seed changed: {key}")
        evidence.append(
            {
                "key": key,
                "sha256": info.sha256,
    ...
    _require(
        ...
        f"V139 capture ledger changed: {counts}",
    )
    _require(
        not list((capture_root / "quarantine").glob("**/attempt-*")),
        "V139 tape capture quarantine is nonempty",
    )
    return evidence, {
...
def _verify_references() -> tuple[list[dict[str, Any]], dict[str, str]]:
    ...
    for arm_id in ARM_IDS:
        ...
        _require(path.is_file(), f"missing V139 reference catalog: {arm_id}")
        catalog = read_json(path)
        catalog_hash = _assert_hashed_object(
            catalog, "catalog_hash", f"V139 references {arm_id}"
        )
        entries = catalog.get("entries")
        _require(
            isinstance(entries, dict) and len(entries) == 9,
            f"V139 reference count changed: {arm_id}",
        )
        root = _stage_root_from_receipts(
            entries, "receipt_path", 9, f"V139 references {arm_id}"
        )
        rows, last_hash = _read_ledger(root / "ledger.jsonl")
        counts = Counter(row["event_type"] for row in rows)
        _require(
            counts == Counter({"reference_build_canonicalized": 9}),
            f"V139 reference ledger changed: {arm_id}/{counts}",
        )
        _require(
            not list((root / "quarantine").glob("**/attempt-*")),
            f"V139 reference quarantine is nonempty: {arm_id}",
        )
        ...
        for key, entry in sorted(entries.items()):
            ...
            _require(
                ...
                f"V139 reference evidence changed: {key}",
            )
            evidence.append(
                {
                    "arm_id": arm_id,
    ...
    _require(len(evidence) == 27, "V139 reference evidence count changed")
...
def _expected_portfolio_selection(
    ...
    _require(rule == "service_welfare_borda", f"unknown V139 portfolio rule: {rule}")
...
def _validate_native_diagnostics(
    ...
    _require(path.is_file(), f"missing V139 Nash diagnostics: {run['run_id']}")
    counts = Counter()
    reasons = Counter()
    with gzip.open(path, "rt", encoding="utf-8") as handle:
        for line_number, line in enumerate(handle, start=1):
            ...
            _require(isinstance(decision, dict), "missing V139 decision diagnostics")
            native = decision.get("native_shadow_anchor")
            _require(isinstance(native, dict), "missing V139 native shadow diagnostics")
            portfolio = decision.get("native_portfolio")
            _require(isinstance(portfolio, dict), "missing V139 portfolio diagnostics")
            ...
            _require(
                ...
                f"V139 portfolio certificate definition changed: {run['run_id']}:{line_number}",
            )
            _require(
                ...
                f"V139 configured bandwidth snapshot changed: {run['run_id']}:{line_number}",
            )
            if players > 0:
                ...
                _require(
                    ...
                    f"V139 portfolio members changed: {run['run_id']}:{line_number}",
                )
                for candidate in candidates:
                    _require(
                        ...
                        f"V139 portfolio candidate invalid: {run['run_id']}:{line_number}",
                    )
                ...
                for candidate, expected_ranks in zip(candidates, ranks):
                    _require(
                        ...
                        f"V139 portfolio ranks changed: {run['run_id']}:{line_number}",
                    )
                ...
                _require(
                    ...
                    f"V139 portfolio selection changed: {run['run_id']}:{line_number}",
                )
                ...
                _require(
                    ...
                    f"V139 selected native mismatch: {run['run_id']}:{line_number}",
                )
            _require(
                native.get("certificate_uses_completion_outcomes") is False,
                f"V139 native certificate consulted outcomes: {run['run_id']}",
            )
            ...
            _require(
                ...
                f"V139 native service cohort count invalid: {run['run_id']}:{line_number}",
            )
            guard = decision.get("window_safe_guard")
            _require(isinstance(guard, dict), "missing V139 native guard diagnostics")
            ...
            if players == 0:
                _require(
                    ...
                    f"V139 empty-window diagnostics mismatch: {run['run_id']}:{line_number}",
                )
                reasons[reason] += 1
                continue
            ...
            if initializer_complete and proposal_complete:
                _require(
                    ...
                    f"V139 native service cohort mismatch: {run['run_id']}:{line_number}",
                )
                ...
                _require(
                    ...
                    f"V139 native service certificate incomplete: {run['run_id']}:{line_number}",
                )
                ...
                _require(
                    ...
                    f"V139 native service deltas changed: {run['run_id']}:{line_number}",
                )
            else:
                _require(
                    guard.get("accepted") is False,
                    f"V139 unavailable native service certificate was accepted: {run['run_id']}:{line_number}",
                )
                if not initializer_complete:
                    _require(
                        initializer_players == 0
                        and reason == "initializer_readiness_service_unavailable",
                        f"V139 initializer service fallback mismatch: {run['run_id']}:{line_number}",
                    )
                else:
                    _require(
                        ...
                        f"V139 proposal service fallback mismatch: {run['run_id']}:{line_number}",
                    )
            ...
            _require(
                ...
                == float(welfare_delta),
                f"V139 native welfare certificate changed: {run['run_id']}:{line_number}",
            )
            ...
            _require(
                ...
                f"V139 native guard decision changed: {run['run_id']}:{line_number}",
            )
            reasons[reason] += 1
            if guard.get("accepted") is True:
                ...
                _require(
                    ...
                    f"V139 native accepted window violated certificate: {run['run_id']}:{line_number}",
                )
    _require(counts["windows"] == 4000, f"V139 window count changed: {run['run_id']}")
    _require(
        counts["player_windows"] > 0,
        f"V139 has no native player windows: {run['run_id']}",
    )
    return {
...
def _validate_execution_receipt() -> dict[str, Any]:
    ...
    _require(ready_schedule_path.is_file(), "missing V139 ready schedule")
    _require(execution_path.is_file(), "missing V139 execution receipt")
    schedule = read_json(ready_schedule_path)
    schedule_hash = _assert_hashed_object(
        schedule, "schedule_hash", "V139 ready schedule"
    )
    receipt = read_json(execution_path)
    receipt_hash = _assert_hashed_object(
        receipt, "receipt_hash", "V139 execution receipt"
    )
    _require(
        ...
        "V139 execution receipt boundary changed",
    )
    for scheduled, dispatched in zip(schedule["schedule"], receipt["dispatches"]):
        ...
        _require(
            ...
            f"V139 frozen dispatch changed: {scheduled['ordinal']}",
        )
    return {
...
def run_blind_audit(output: Path = OUTPUT) -> dict[str, Any]:
    _require(not output.exists(), f"V139 blind audit already exists: {output}")
    _require(not RESULT.exists(), "V139 reveal exists before blind audit")
    _require(PLAN.is_file() and file_hash(PLAN) == PLAN_SHA256, "V139 plan changed")
    _require(PREPARED.is_file(), "missing V139 prepared receipt")
    prepared = read_json(PREPARED)
    prepared_hash = _assert_hashed_object(
        prepared, "receipt_hash", "V139 prepared receipt"
    )
    _require(
        ...
        and prepared.get("v138_disposition")
        == "aborted_implementation_readiness_batch_retained"
        and prepared.get(
            "v138_partial_reference_records_consulted_for_technical_diagnosis"
        )
        is True
        and prepared.get("v138_completed_online_performance_results_consulted") is False
        and prepared.get("total_online_runs") == 108
        and prepared.get("candidate_reference_builds") == 27,
        "V139 prepared scientific boundary changed",
    )
    _require(
        ...
        "V139 prepared treatment boundary changed",
    )
    ...
    for manifest_id, expected_count, arm in products:
        ...
        _require(
            ...
            f"V139 ready boundary changed: {manifest_id}",
        )
        ...
        _require(
            ...
            f"V139 pairing changed: {manifest_id}",
        )
        ...
        _require(
            actual_ids == expected_ids, f"V139 canonical set changed: {manifest_id}"
        )
        _require(
            not list((workspace / "quarantine").glob("**/attempt-*")),
            f"V139 online quarantine is nonempty: {manifest_id}",
        )
        ...
        _require(
            ...
            f"V139 online ledger changed: {manifest_id}/{counts}",
        )
        for run in manifest["runs"]:
            if candidate:
                _require(
                    ...
                    == arm["profile"]
                    and run.get("metadata", {}).get("v139_native_portfolio_rule")
                    == arm["portfolio_rule"]
                    and run.get("metadata", {}).get("v139_native_portfolio_members")
                    == NATIVE_KINDS
                    and run.get("metadata", {}).get(
                        "v139_all_native_members_use_all_frontier"
                    )
                    is True
                    and run.get("metadata", {}).get("v139_outcome_fields_drive_policy")
                    is False
                    and isinstance(run.get("reference_dependency"), dict),
                    f"V139 candidate manifest boundary changed: {run['run_id']}",
                )
            else:
                _require(
                    run.get("method") in BASELINE_METHODS
                    and run.get("reference_dependency") is None,
                    f"V139 baseline manifest boundary changed: {run['run_id']}",
                )
            ...
            _require(
                ...
                f"V139 canonical status changed: {run['run_id']}",
            )
    ...
    _require(len(run_evidence) == 108, "V139 run evidence count changed")
    for field, expected in EXPECTED_RUNTIME.items():
        _require(
            runtime_values[field] == {expected},
            f"V139 runtime {field} changed: {runtime_values[field]}",
        )
    ...
    _require(
        ...
        f"V139 runtime git identity changed: {git_commits}",
    )
    _require(len(paired_inputs) == 9, "V139 paired block count changed")
    for (scenario, seed), rows in paired_inputs.items():
        _require(
            len(rows) == 12
            and {row["method_label"] for row in rows} == set(METHOD_LABELS),
            f"V139 block product changed: {scenario}/{seed}",
        )
        for field in (
            ...
            _require(
                ...
                f"V139 paired {field} changed: {scenario}/{seed}",
            )
        _require(
            rows[0]["common_hpa_hash"] == EXPECTED_COMMON_HPA_SHA256,
            f"V139 common HPA changed: {scenario}/{seed}",
        )

    payload = {
        "schema_version": "NSE_E3_ALL_NATIVE_PORTFOLIO_BLIND_AUDIT_V139_V1",
        "created_at": utc_now(),
        "status": "pass",
