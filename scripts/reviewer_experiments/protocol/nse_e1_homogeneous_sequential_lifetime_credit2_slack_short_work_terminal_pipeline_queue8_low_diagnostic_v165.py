...
ROOT = Path(
    "tmp/nse_e1_homogeneous_sequential_lifetime_credit2_slack_short_work_terminal_pipeline_queue8_low_diagnostic_20260901_v165"
)
PLAN = Path(
    "scripts/reviewer_experiments/protocol/"
    "nse_e1_homogeneous_sequential_lifetime_credit2_slack_short_work_terminal_pipeline_queue8_low_diagnostic_plan_v165.json"
)
PLAN_SHA256 = "73dc748da7e7b15f82e9024ba796ef0b5275facd24217e14581214462ee2dd6b"
IMPLEMENTATION = Path(
    "scripts/reviewer_experiments/protocol/"
    "nse_e1_homogeneous_sequential_lifetime_credit2_slack_short_work_terminal_pipeline_queue8_low_diagnostic_implementation_v165.json"
)
IMPLEMENTATION_SHA256 = (
    "29a2da70473c8eabb51f45af4186260c0bb209d5802b9b71fc64d52135893864"
)
V164_FAILURE = Path(
    "scripts/reviewer_experiments/protocol/"
    "nse_e1_homogeneous_ready_antihotspot_lifetime_credit_slack_short_work_terminal_pipeline_queue8_low_diagnostic_failure_v164.json"
)
V164_FAILURE_SHA256 = "41adb91d7f8118663844ea78b46fc7253edcb63c12635e3aff80eb7b8ede8a9d"
V164_FAILURE_HASH = "366aa4901228b7c787a6b8fbc2e711d6ae2f0dcca70f362edd86be921a2b043b"

ARM_ID = "v165-low-srpt-slack-sequential-lifetime-credit2-short5p5-terminal-pipeline-hiku2-ocs-queue8"
PROFILE = (
    "srpt_slack_sequential_lifetime_credit2_short5p5_terminal_pipeline_"
    "hiku2_ocs_queue8"
)
FRONTIER = (
    "parents_completed_or_terminal_or_slack_sequential_lifetime_credit2_"
    "short_work_parents_scheduled"
)
SINGLE_CHANGE = (
    "V163_plus_one_additional_strictly_sequential_nonreusable_lifetime_"
...
TERMINAL_DEFINITION = (
    ...
    "the_second_requires_zero_current_outstanding_speculation"
)
LIFETIME_CREDIT_DEFINITION = (
    "at_most_two_nonreusable_incomplete-parent_nonterminal_admissions_per_"
    "request_lifetime_with_zero_outstanding_required_before_the_second"
)
LIFETIME_CREDIT_CONTRACT_DEFINITION = (
    "at_most_two_nonreusable_incomplete-parent_nonterminal_admissions_per_"
    "request_lifetime;the_second_requires_zero_current_outstanding_speculation;"
...
PORT = "3216"
BINARY_SOURCE_COMMIT = "e848289d13329d40146a0f7baff38dd99b5b425c"
BINARY_PATH = Path("serverless_sim/target_e1_v165/release/serverless_sim.exe")
BINARY_SHA256 = "8049b61ee7a1aa0fad4f5c929bdb142afd8fe166baaa32a7dec2e1b2b3a81809"


def paths(root: Path = ROOT) -> dict[str, Path]:
    return {
        "manifest": root / f"manifest.{ARM_ID}.unbound.json",
        "prepared": root / "prepared-v165.json",
        "schedule": root / "frozen-run-order-v165.json",
        ...
        "execution": root / "execution-receipt-v165.json",
        "pairing": root / "pairing-audit-v165.json",
        "blind": root / "joint-blind-audit-v165.json",
        "result": root / "diagnostic-result-v165.json",
    }
...
def _assert_frozen_inputs() -> dict[str, Any]:
    ...
    for path, sha256, label in (
        (PLAN, PLAN_SHA256, "V165 plan"),
        (IMPLEMENTATION, IMPLEMENTATION_SHA256, "V165 implementation receipt"),
        (V164_FAILURE, V164_FAILURE_SHA256, "V164 joint-gate failure receipt"),
        (BINARY_PATH, BINARY_SHA256, "V165 release binary"),
    ...
    if not (
        ...
        == "one additional nonreusable admission only after the first is consumed and current outstanding incomplete-parent nonterminal speculation is zero"
        and change.get("credit_cap_per_request_lifetime") == 2
        and change.get("selected_credit_players_per_request_per_window_cap") == 1
        and change.get("projected_outstanding_speculation_cap") == 1
        ...
        raise RuntimeError("V165 implementation boundary changed")
    failure = read_json(V164_FAILURE)
    if not (
        _assert_hashed(failure, "receipt_hash", "V164 failure receipt")
        == V164_FAILURE_HASH
        ...
        and failure.get("disposition", {}).get(
            "remaining_seventeen_v164_runs_authorized"
        )
        is False
    ):
        raise RuntimeError("V164 sealed failure boundary changed")
    _assert_json_semantic(
...
def _rewrite_candidate(
    ...
    for key in list(marker):
        if key.startswith("v163_"):
            marker.pop(key)
    contract = {
        "v165_training_only": True,
        "v165_role": "result_blind_sequential_lifetime_credit2_falsification",
        "v165_plan_sha256": PLAN_SHA256,
        "v165_implementation_sha256": IMPLEMENTATION_SHA256,
        "v165_binary_source_commit": BINARY_SOURCE_COMMIT,
        "v165_protocol_source_commit": protocol_source_commit,
        "v165_binary_sha256": BINARY_SHA256,
        "v165_arm_id": ARM_ID,
        "v165_profile": PROFILE,
        "v165_player_frontier": FRONTIER,
        "v165_single_change_from_v163": SINGLE_CHANGE,
        "v165_short_work_threshold": SHORT_WORK_THRESHOLD,
        "v165_queue_density_threshold": QUEUE_THRESHOLD,
        "v165_queue_boundary": "below_is_strict",
        "v165_lifetime_credit_definition": LIFETIME_CREDIT_DEFINITION,
        "v165_lifetime_credit_contract_definition": (
            LIFETIME_CREDIT_CONTRACT_DEFINITION
        ),
        "v165_lifetime_credit_limit_per_request": 2,
        "v165_selection_order": SELECTION_ORDER,
        "v165_credit_reuse_after_parent_or_function_completion": False,
        "v165_second_credit_requires_zero_outstanding_speculation": True,
        "v165_selected_credit_players_per_request_per_window_cap": 1,
        "v165_projected_outstanding_speculation_cap": 1,
        "v165_ready_antihotspot_enabled": False,
        "v165_operational_score": "exact_V163_router_for_every_player",
        "v165_remaining_seventeen_authorized": False,
        "v165_confirmation_inputs_generated": False,
    }
    marker.update(
        {
            "purpose": (
                "V165 outcome-disclosed three-seed sequential lifetime-credit2 "
                "diagnostic on the frozen V163 router; never a formal result or "
                "paper superiority claim"
            ),
            **contract,
            "v165_environment": COMMON_ENVIRONMENT,
            "v165_expected_run_count": 3,
            "v165_expected_reference_build_count": 3,
            "v165_fixed_order": list(SEEDS),
            "v165_candidate_performance_summaries_parsed": 0,
        }
    )
    for run in rewritten["runs"]:
        ...
        run["metadata"] = {
            **contract,
            "v165_source_e1_run_id": old.get("v163_source_e1_run_id"),
            "v165_source_e1_run_spec_hash": old.get("v163_source_e1_run_spec_hash"),
            "v165_candidate_performance_summaries_parsed_before_run": 0,
        }
...
def _validate_product(manifest: Mapping[str, Any], *, references_bound: bool) -> None:
    if not (
        ...
        raise RuntimeError("V165 exact E09/E18/E20 product changed")
    ...
    for run in manifest["runs"]:
        ...
        if not (
            ...
            and metadata.get("v165_profile") == PROFILE
            and metadata.get("v165_player_frontier") == FRONTIER
            and metadata.get("v165_short_work_threshold") == SHORT_WORK_THRESHOLD
            and metadata.get("v165_queue_density_threshold") == QUEUE_THRESHOLD
            and metadata.get("v165_queue_boundary") == "below_is_strict"
            and metadata.get("v165_lifetime_credit_definition")
            == LIFETIME_CREDIT_DEFINITION
            and metadata.get("v165_lifetime_credit_contract_definition")
            == LIFETIME_CREDIT_CONTRACT_DEFINITION
            and metadata.get("v165_lifetime_credit_limit_per_request") == 2
            and metadata.get("v165_selection_order") == SELECTION_ORDER
            and metadata.get("v165_credit_reuse_after_parent_or_function_completion")
            is False
            and metadata.get("v165_second_credit_requires_zero_outstanding_speculation")
            is True
            and metadata.get("v165_selected_credit_players_per_request_per_window_cap")
            == 1
            and metadata.get("v165_projected_outstanding_speculation_cap") == 1
            and metadata.get("v165_ready_antihotspot_enabled") is False
            and metadata.get("v165_operational_score")
            == "exact_V163_router_for_every_player"
        ):
            raise RuntimeError(f"V165 run contract changed: {run.get('run_id')}")


def prepare_v165(root: Path = ROOT) -> dict[str, Any]:
    source = _assert_frozen_inputs()
    if root.exists():
        raise RuntimeError(f"refusing to overwrite V165 root: {root}")
    ...
    schedule = {
        "schema_version": "NSE_E1_HOMOGENEOUS_SEQUENTIAL_LIFETIME_CREDIT2_LOW_SCHEDULE_V165_V1",
        "created_at": utc_now(),
    ...
    receipt = {
        "schema_version": "NSE_E1_HOMOGENEOUS_SEQUENTIAL_LIFETIME_CREDIT2_LOW_PREPARED_V165_V1",
        "created_at": utc_now(),
        ...
        "v164_failure_receipt_file_sha256": V164_FAILURE_SHA256,
        "v164_failure_receipt_hash": V164_FAILURE_HASH,
        "v163_and_v164_performance_revealed_and_used_for_training_diagnosis": True,
        ...
        "second_credit_requires_zero_outstanding_speculation": True,
        "selected_credit_players_per_request_per_window_cap": 1,
        "projected_outstanding_speculation_cap": 1,
...
def execute_v165(root: Path = ROOT) -> dict[str, Any]:
    output = paths(root)
    if output["execution"].exists():
        raise RuntimeError("V165 execution receipt already exists")
    prepared = read_json(output["prepared"])
    _assert_hashed(prepared, "receipt_hash", "V165 prepared receipt")
    ...
    for ordinal, seed in enumerate(SEEDS, start=1):
        ...
        if completed.returncode != 0:
            raise RuntimeError(f"V165 dispatch {seed} failed: {completed.returncode}")
        ...
        if not (
            ...
            raise RuntimeError(f"V165 canonical is not a QC pass: {run['run_id']}")
        dispatches.append(
            {
                "ordinal": ordinal,
                "seed": seed,
    ...
    receipt = {
        "schema_version": "NSE_E1_HOMOGENEOUS_SEQUENTIAL_LIFETIME_CREDIT2_LOW_EXECUTION_V165_V1",
        "created_at": utc_now(),
        "candidate_performance_summaries_parsed": 0,
        "plan_sha256": PLAN_SHA256,
...
def _count(value: Any, label: str) -> int:
    if isinstance(value, bool) or not isinstance(value, int) or value < 0:
        raise RuntimeError(f"V165 {label} is invalid")
    return value
...
def _finite_optional(value: Any, label: str) -> float | None:
    ...
    if not math.isfinite(result):
        raise RuntimeError(f"V165 {label} is nonfinite")
    return result
...
def _audit_nash_log(canonical: Path, run: Mapping[str, Any]) -> dict[str, Any]:
    ...
    with gzip.open(log, "rt", encoding="utf-8") as stream:
        for line in stream:
            ...
            if kind not in counts:
                raise RuntimeError(f"unexpected V165 Nash observation kind: {kind}")
            counts[kind] += 1
            if kind == "run_config":
                ...
                if not (
                    ...
                    raise RuntimeError("V165 run_config contract changed")
            elif kind == "window":
                if event.get("frame") != counts["window"] - 1:
                    raise RuntimeError("V165 scheduler window sequence changed")
                ...
                if (
                    ...
                    raise RuntimeError("V165 decision frontier/hash changed")
                ...
                if not (
                    ...
                    raise RuntimeError(
                        "V165 sequential lifetime-credit2 evidence changed"
                    )
                ...
                if not (
                    ...
                    raise RuntimeError("V165 route telemetry is incomplete")
                expected = (
                    LOW_EXPERT if float(density) < QUEUE_THRESHOLD else HIGH_EXPERT
                )
                if route.get("selected_expert") != expected:
                    raise RuntimeError("V165 route does not match queue density")
                ...
                if key is None:
                    if source != "not_requested":
                        raise RuntimeError("V165 unrequested reference reason changed")
                    reference_not_requested += 1
                elif source in ("offline_table", "offline_table_nonpositive"):
                    reference_available += 1
                else:
                    raise RuntimeError("V165 bound reference source changed")
                ...
                if not (
                    ...
                    raise RuntimeError("V165 Nash terminal marker changed")
    ...
    if (
        ...
        raise RuntimeError("V165 Nash log cardinality changed")
    if reference_available + reference_not_requested != counts["window"]:
        raise RuntimeError("V165 reference replay coverage changed")
    return {
        "run_id": run_id,
        "seed": run["seed"],
...
def _mechanism_falsification_gate(
    ...
    breadth_paths = {
        ...
        "second_sequential_lifetime_credit_admission": second_admissions > 0,
        "second_credit_blocked_by_outstanding": outstanding_rejections > 0,
        "lifetime_credit_cap_exhausted": cap_rejections > 0,
    ...
    if not sequential_invariants:
        failure_reasons.append("sequential_lifetime_credit_invariant_failed")
    ...
    return {
        ...
        "sequential_lifetime_credit_invariants_passed": sequential_invariants,
        "work_and_queue_threshold_invariants_passed": work_invariants,
        "both_routes_exercised": both_routes,
    }


def blind_audit_v165(root: Path = ROOT) -> dict[str, Any]:
    output = paths(root)
    if output["blind"].exists():
        raise RuntimeError("V165 blind audit already exists")
    _assert_frozen_inputs()
    prepared = read_json(output["prepared"])
    prepared_hash = _assert_hashed(prepared, "receipt_hash", "V165 prepared receipt")
    execution = read_json(output["execution"])
    execution_hash = _assert_hashed(execution, "receipt_hash", "V165 execution receipt")
    manifest = load_and_validate_manifest(output["ready"])
    _validate_product(manifest, references_bound=True)
    ...
    if (
        ...
        raise RuntimeError("V165 exact pairing failed")
    ...
    if [item["seed"] for item in execution["dispatches"]] != list(SEEDS):
        raise RuntimeError("V165 execution order changed")
    ...
    if {path.name for path in canonical_root.iterdir() if path.is_dir()} != {
        ...
        raise RuntimeError("V165 canonical product changed")
    quarantine = output["workspace"] / "quarantine"
    if quarantine.exists() and any(quarantine.rglob("attempt-*")):
        raise RuntimeError("V165 has unexplained quarantined attempts")
    ...
    if len(identities) != 1:
        raise RuntimeError("V165 runtime identity is not unanimous")
    ...
    if not (
        ...
        raise RuntimeError("V165 runtime identity changed")
    mechanism = _mechanism_falsification_gate(audits)
    document = {
        "schema_version": "NSE_E1_HOMOGENEOUS_SEQUENTIAL_LIFETIME_CREDIT2_LOW_BLIND_AUDIT_V165_V1",
        "created_at": utc_now(),
        ...
        **mechanism["metrics"],
        ...
        "sequential_lifetime_credit_invariants_passed": mechanism[
            "sequential_lifetime_credit_invariants_passed"
        ],
...
def _load_candidate(
    ...
    if len(rows) != 3 or [row["seed"] for row in rows] != list(SEEDS):
        raise RuntimeError("V165 candidate result product changed")
    return rows


def reveal_v165(root: Path = ROOT) -> dict[str, Any]:
    output = paths(root)
    if output["result"].exists():
        raise RuntimeError("V165 result already exists")
    blind = read_json(output["blind"])
    blind_hash = _assert_hashed(blind, "blind_audit_hash", "V165 blind audit")
    if not (
        ...
        and blind.get("sequential_lifetime_credit_invariants_passed") is True
        ...
        raise RuntimeError("V165 blind audit did not authorize reveal")
    manifest = load_and_validate_manifest(output["ready"])
    ...
    mechanism["pass"] = (
        ...
        and blind["sequential_lifetime_credit_invariants_passed"]
    ...
    document = {
        "schema_version": "NSE_E1_HOMOGENEOUS_SEQUENTIAL_LIFETIME_CREDIT2_LOW_DIAGNOSTIC_RESULT_V165_V1",
        "created_at": utc_now(),
        "training_only": True,
        "formal_results_eligible": False,
        ...
        "disposition": (
            ...
            if passed
            else "retain_all_three_valid_diagnostic_runs_and_retire_sequential_lifetime_credit2_candidate"
        ),
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    if action == "prepare":
        document, key = prepare_v165(), "receipt_hash"
    elif action == "execute":
        document, key = execute_v165(), "receipt_hash"
    elif action == "blind-audit":
        document, key = blind_audit_v165(), "blind_audit_hash"
    else:
        document, key = reveal_v165(), "result_hash"
