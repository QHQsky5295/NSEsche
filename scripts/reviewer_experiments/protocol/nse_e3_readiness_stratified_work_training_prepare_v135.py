...
ROOT = Path("tmp/nse_e3_readiness_stratified_work_training_20260830_v135")
PLAN = Path(
    "scripts/reviewer_experiments/protocol/"
    "nse_e3_readiness_stratified_work_training_plan_v135.json"
)
PLAN_SHA256 = "ab5e576fd6ce302eda3687dee79c317d328274376083f2ac9427cdfd883404c4"
...
BINARY_PATH = Path("tmp/nse_v135_build_702e5a4/release/serverless_sim.exe")
BINARY_SHA256 = "8eceb2f8066b083739740e6cf3b9c66ba88bb8fcff7d1f3bf8684a5793da35e8"
BINARY_SOURCE_COMMIT = "702e5a4af97fa462750acd4f83820da9e069168c"

TRAINING_SEED_LIST = ["E1448", "E1449", "E1450"]
...
CONFIRMATION_SEEDS = [f"E{index}" for index in range(1451, 1471)]
PREVIOUS_CONFIRMATION_SEEDS = [
    *[f"E{index}" for index in range(1428, 1448)],
...
E3_READINESS_STRATIFIED_WORK = (
    "faasrank_native_faithful_terminal_ocs_srpt_ready_"
    "causal_arrival_shock15_horizon50_readiness_stratified_work10_initializer_only_"
    "guard64_dual_window_safe_pareto"
)
ARMS = (
    ("v135-e3-anchor", "anchor", E3_ANCHOR, 9),
    (
        "v135-e3-causal-shock15-horizon50-readiness-stratified-work10-initializer-only",
        "candidate",
        E3_READINESS_STRATIFIED_WORK,
        9,
    ),
...
def _paths(root: Path) -> dict[str, Path]:
    return {
        "config": root / "v135-e3-training-config.json",
        "source": root / "manifest.v135-training-source-full.unbound.json",
        "capture": root / "manifest.v135-tape-capture.unbound.json",
...
def _assert_frozen_inputs() -> None:
    for path, expected in (
        ...
        if not path.is_file() or file_hash(path) != expected:
            raise RuntimeError(f"frozen V135 input is missing or changed: {path}")
    ...
    *,
    ...
    marker.update(
        {
            ...
            "v135_binary_source_commit": BINARY_SOURCE_COMMIT,
    ...
    if arm is not None:
        if frozen_model_config is None:
            raise RuntimeError("V135 arm rewrite lacks frozen model binding")
        arm_id, role, profile, expected = arm
        candidate = role == "candidate"
        marker.update(
            {
                "v135_arm_id": arm_id,
                "v135_experiment_id": "E3",
                "v135_role": role,
                "v135_profile": profile,
                "v135_shock_threshold_numerator": 3 if candidate else None,
                "v135_shock_threshold_denominator": 2 if candidate else None,
                "v135_shock_activation_horizon_frames": 50 if candidate else None,
                "v135_critical_service_ratio_numerator": 9 if candidate else None,
                "v135_critical_service_ratio_denominator": 10 if candidate else None,
                ...
                "v135_expected_run_count": expected,
            ...
            metadata.update(
                {
                    "v135_arm_id": arm_id,
                    "v135_arm_role": role,
                    "v135_candidate_profile": profile,
                    "v135_candidate_experiment": "E3",
                    "v135_shock_rate_ratio": "3/2" if candidate else None,
                    "v135_shock_threshold_numerator": 3 if candidate else None,
                    "v135_shock_threshold_denominator": 2 if candidate else None,
                    ...
                    "v135_faasrank_model_artifact_sha256": MODEL_SHA256,
                    "v135_critical_service_ratio_numerator": 9 if candidate else None,
                    "v135_critical_service_ratio_denominator": 10
                    if candidate
                    else None,
...
def prepare_v135(root: Path = ROOT) -> dict:
    _assert_frozen_inputs()
    if root.exists():
        raise RuntimeError(f"refusing to overwrite V135 training root: {root}")
    ...
    capture = derive_integration_smoke_shard(
        paths["source"], capture_ids, purpose=f"V135 fresh parent tapes {PLAN_SHA256}"
    )
    _write(
        paths["capture"],
        _rewrite(capture, purpose=f"V135 fresh parent tapes {PLAN_SHA256}"),
    )
    ...
    for arm in ARMS:
        ...
        template = derive_integration_smoke_shard(
            paths["source"], selected, purpose=f"V135 {arm_id} {PLAN_SHA256}"
        )
        manifest = _rewrite(
            template,
            purpose=f"V135 {arm_id} {PLAN_SHA256}",
            arm=arm,
        ...
        if (
            ...
            raise RuntimeError(f"V135 {arm_id} must have 9 runs/references")
        if {run["seed"] for run in manifest["runs"]} != TRAINING_SEEDS:
            raise RuntimeError(f"V135 {arm_id} escaped E1448-E1450")
        keys = {item["key"] for item in manifest["reference_build_dependencies"]}
        if all_reference_keys & keys:
            raise RuntimeError("V135 arm-specific reference keys unexpectedly overlap")
        all_reference_keys.update(keys)
        _write(arm_path(root, arm_id), manifest)
    if len(all_reference_keys) != 18:
        raise RuntimeError("V135 must declare exactly 18 arm-specific references")
    ...
    receipt = {
        "schema_version": "NSE_E3_READINESS_STRATIFIED_WORK_PREPARED_V135_V1",
        "created_at": utc_now(),
        "formal_results_eligible": False,
