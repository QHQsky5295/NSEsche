...
ROOT = Path(
    "tmp/nse_e1_homogeneous_cpu_bounded_terminal_slack_short_work_pipeline_queue8_low_diagnostic_20260901_v168"
)
PLAN = Path(
    "scripts/reviewer_experiments/protocol/"
    "nse_e1_homogeneous_cpu_bounded_terminal_slack_short_work_pipeline_queue8_low_diagnostic_plan_v168.json"
)
PLAN_SHA256 = "b8c459afa699b52421a23d5e32e5496b5ca031240e44d25dee059d2014ac5909"
IMPLEMENTATION = Path(
    "scripts/reviewer_experiments/protocol/"
    "nse_e1_homogeneous_cpu_bounded_terminal_slack_short_work_pipeline_queue8_low_diagnostic_implementation_v168.json"
)
IMPLEMENTATION_SHA256 = (
    "83d621c494ccd0f9d715db81f4e21b1596467f0d1e1cab1cb4d43253a296df63"
)
V167_FAILURE = Path(
    "scripts/reviewer_experiments/protocol/"
    "nse_e1_homogeneous_aged_dual_lifetime_credit2_slack_short_work_terminal_pipeline_queue8_low_diagnostic_failure_v167.json"
)
V167_FAILURE_SHA256 = "546971efe52d936b53f137186a2a73562b084be72e3cd993bc780222c5760a0d"
V167_FAILURE_HASH = "62970c1b25bb832eb2b26dbd69b675acb297cb5986408cdb915b3d624b87b1ba"

ARM_ID = "v168-low-srpt-slack-cpu-bounded-terminal-short5p5-pipeline-hiku2-ocs-queue8"
PROFILE = "srpt_slack_cpu_bounded_terminal_short5p5_pipeline_hiku2_ocs_queue8"
FRONTIER = (
    "parents_completed_or_cpu_bounded_terminal_or_slack_short_work_parents_scheduled"
)
SINGLE_CHANGE = (
    "V159_frontier_plus_incomplete_parent_terminal_immutable_cpu_work_at_most_"
    "mean_node_cpu_capacity"
)
TERMINAL_DEFINITION = (
    "admit_all_parents-completed_players;admit_incomplete-parent_terminal_players_"
    "only_when_immutable_function_cpu_work_divided_by_mean_node_cpu_capacity_is_"
    "at_most_one;retain_V159_nonterminal_short-work_frontier"
)
CPU_THRESHOLD = 1.0
...
BINARY_SOURCE_COMMIT = "c387a3ab77f69f53c729ee0f59f3f68300c727d8"
BINARY_PATH = Path("serverless_sim/target_e1_v168/release/serverless_sim.exe")
BINARY_SHA256 = "f8238388dcb2f5e92375707be0ede7e264ddfe8c163e562f175b0c86307a9287"


def paths(root: Path = ROOT) -> dict[str, Path]:
    return {
        "manifest": root / f"manifest.{ARM_ID}.unbound.json",
        "prepared": root / "prepared-v168.json",
        "schedule": root / "frozen-run-order-v168.json",
        ...
        "execution": root / "execution-receipt-v168.json",
        "pairing": root / "pairing-audit-v168.json",
        "blind": root / "joint-blind-audit-v168.json",
        "result": root / "diagnostic-result-v168.json",
    }
...
def _assert_frozen_inputs() -> dict[str, Any]:
    ...
    for path, sha256, label in (
        (PLAN, PLAN_SHA256, "V168 plan"),
        (IMPLEMENTATION, IMPLEMENTATION_SHA256, "V168 implementation receipt"),
        (V167_FAILURE, V167_FAILURE_SHA256, "V167 sealed failure receipt"),
        (BINARY_PATH, BINARY_SHA256, "V168 release binary"),
    ...
    if not (
        ...
        and change.get("normalized_cpu_threshold") == CPU_THRESHOLD
        and change.get("boundary") == "at_or_below_is_admitted"
        ...
        raise RuntimeError("V168 implementation boundary changed")
    failure = read_json(V167_FAILURE)
    if not (
        _assert_hashed(failure, "receipt_hash", "V167 failure receipt")
        == V167_FAILURE_HASH
        and failure.get("disposition", {}).get(
            "remaining_seventeen_v167_runs_authorized"
        )
        is False
        and failure.get("disposition", {}).get("retain_all_three_valid_diagnostic_runs")
        is True
    ):
        raise RuntimeError("V167 failure boundary changed")
    _assert_json_semantic(
...
def _rewrite_candidate(
    ...
    marker.update(
        {
            "purpose": (
                "V168 outcome-disclosed three-seed CPU-bounded terminal diagnostic; "
                "never a formal result or paper superiority claim"
            ),
            "v168_role": "result_blind_cpu_bounded_terminal_falsification",
            "v168_plan_sha256": PLAN_SHA256,
            "v168_implementation_sha256": IMPLEMENTATION_SHA256,
            "v168_binary_source_commit": BINARY_SOURCE_COMMIT,
            "v168_protocol_source_commit": protocol_source_commit,
            "v168_binary_sha256": BINARY_SHA256,
            "v168_arm_id": ARM_ID,
            "v168_profile": PROFILE,
            "v168_player_frontier": FRONTIER,
            "v168_single_change_from_v159": SINGLE_CHANGE,
            "v168_cpu_threshold": CPU_THRESHOLD,
            "v168_cpu_boundary": "at_or_below_is_admitted",
            "v168_short_work_threshold": SHORT_WORK_THRESHOLD,
            "v168_queue_density_threshold": QUEUE_THRESHOLD,
            "v168_queue_boundary": "below_is_strict",
            "v168_environment": COMMON_ENVIRONMENT,
            "v168_expected_run_count": 3,
            "v168_expected_reference_build_count": 3,
            "v168_fixed_order": list(SEEDS),
            "v168_candidate_performance_summaries_parsed": 0,
            "v168_remaining_seventeen_authorized": False,
            "v168_confirmation_inputs_generated": False,
        }
    )
    for run in rewritten["runs"]:
        ...
        run["metadata"] = {
            ...
            "v168_queue_density_threshold": QUEUE_THRESHOLD,
            "v168_queue_boundary": "below_is_strict",
            "v168_source_e1_run_id": source_run_id,
            "v168_source_e1_run_spec_hash": source_run_spec_hash,
            "v168_candidate_performance_summaries_parsed_before_run": 0,
            "v168_remaining_seventeen_authorized": False,
            "v168_confirmation_inputs_generated": False,
        }
...
def _validate_product(manifest: Mapping[str, Any], *, references_bound: bool) -> None:
    if not (
        ...
        raise RuntimeError("V168 exact E09/E18/E20 product changed")
    ...
    for run in manifest["runs"]:
        ...
        if not (
            ...
            and metadata.get("v168_profile") == PROFILE
            and metadata.get("v168_player_frontier") == FRONTIER
            and metadata.get("v168_cpu_threshold") == CPU_THRESHOLD
            and metadata.get("v168_cpu_boundary") == "at_or_below_is_admitted"
            and metadata.get("v168_short_work_threshold") == SHORT_WORK_THRESHOLD
            and metadata.get("v168_queue_density_threshold") == QUEUE_THRESHOLD
            and metadata.get("v168_queue_boundary") == "below_is_strict"
        ):
            raise RuntimeError(f"V168 run contract changed: {run.get('run_id')}")


def prepare_v168(root: Path = ROOT) -> dict[str, Any]:
    source = _assert_frozen_inputs()
    if root.exists():
        raise RuntimeError(f"refusing to overwrite V168 root: {root}")
    ...
    schedule = {
        "schema_version": "NSE_E1_HOMOGENEOUS_CPU_BOUNDED_TERMINAL_LOW_SCHEDULE_V168_V1",
        "created_at": utc_now(),
    ...
    receipt = {
        "schema_version": "NSE_E1_HOMOGENEOUS_CPU_BOUNDED_TERMINAL_LOW_PREPARED_V168_V1",
        "created_at": utc_now(),
        ...
        "v167_failure_file_sha256": V167_FAILURE_SHA256,
        "v167_failure_hash": V167_FAILURE_HASH,
...
def execute_v168(root: Path = ROOT) -> dict[str, Any]:
    output = paths(root)
    if output["execution"].exists():
        raise RuntimeError("V168 execution receipt already exists")
    prepared = read_json(output["prepared"])
    _assert_hashed(prepared, "receipt_hash", "V168 prepared receipt")
    ...
    for ordinal, seed in enumerate(SEEDS, start=1):
        ...
        if completed.returncode != 0:
            raise RuntimeError(f"V168 dispatch {seed} failed: {completed.returncode}")
        ...
        if not (
            ...
            raise RuntimeError(f"V168 canonical is not a QC pass: {run['run_id']}")
        dispatches.append(
            {
                "ordinal": ordinal,
                "seed": seed,
    ...
    receipt = {
        "schema_version": "NSE_E1_HOMOGENEOUS_CPU_BOUNDED_TERMINAL_LOW_EXECUTION_V168_V1",
        "created_at": utc_now(),
        "candidate_performance_summaries_parsed": 0,
        "plan_sha256": PLAN_SHA256,
...
def _audit_nash_log(canonical: Path, run: Mapping[str, Any]) -> dict[str, Any]:
    ...
    with gzip.open(log, "rt", encoding="utf-8") as stream:
        for line in stream:
            ...
            if kind not in counts:
                raise RuntimeError(f"unexpected V168 Nash observation kind: {kind}")
            counts[kind] += 1
            if kind == "run_config":
                ...
                if not (
                    ...
                    raise RuntimeError("V168 run_config contract changed")
            elif kind == "window":
                if event.get("frame") != counts["window"] - 1:
                    raise RuntimeError("V168 scheduler window sequence changed")
                ...
                if (
                    ...
                    raise RuntimeError("V168 decision frontier/hash changed")
                ...
                if not (
                    ...
                    and (
                        guard_admitted_ratio_now is None
                    ...
                    raise RuntimeError("V168 CPU/slack/queue frontier evidence changed")
                ...
                if not (
                    ...
                    raise RuntimeError("V168 route telemetry is incomplete")
                expected = (
                    LOW_EXPERT if float(density) < QUEUE_THRESHOLD else HIGH_EXPERT
                )
                if route.get("selected_expert") != expected:
                    raise RuntimeError("V168 route does not match queue density")
                ...
                if key is None:
                    if source != "not_requested":
                        raise RuntimeError("V168 unrequested reference reason changed")
                    totals["reference_not_requested"] += 1
                elif source in ("offline_table", "offline_table_nonpositive"):
                    totals["reference_available"] += 1
                else:
                    raise RuntimeError("V168 bound reference source changed")
                ...
                if not (
                    ...
                    raise RuntimeError("V168 Nash terminal marker changed")
    ...
    if (
        ...
        raise RuntimeError("V168 Nash log cardinality changed")
    if (
        totals["reference_available"] + totals["reference_not_requested"]
        != counts["window"]
    ):
        raise RuntimeError("V168 reference replay coverage changed")
    return {
        "run_id": run_id,
        "seed": run["seed"],
...
def _mechanism_falsification_gate(
    ...
    passed = (
        ...
        and extrema["cpu_guard_admitted_normalized_cpu_max"] <= CPU_THRESHOLD
    ...
    return {
        ...
        **extrema,
        ...
        "cpu_guard_boundary_invariants_passed": (
            extrema["cpu_guard_admitted_normalized_cpu_max"] is not None
            and extrema["cpu_guard_admitted_normalized_cpu_max"] <= CPU_THRESHOLD
...
def blind_audit_v168(root: Path = ROOT) -> dict[str, Any]:
    output = paths(root)
    if output["blind"].exists():
        raise RuntimeError("V168 blind audit already exists")
    _assert_frozen_inputs()
    prepared = read_json(output["prepared"])
    prepared_hash = _assert_hashed(prepared, "receipt_hash", "V168 prepared receipt")
    execution = read_json(output["execution"])
    execution_hash = _assert_hashed(execution, "receipt_hash", "V168 execution receipt")
    manifest = load_and_validate_manifest(output["ready"])
    _validate_product(manifest, references_bound=True)
    ...
    if (
        ...
        raise RuntimeError("V168 exact pairing failed")
    ...
    if [item["seed"] for item in execution["dispatches"]] != list(SEEDS):
        raise RuntimeError("V168 execution order changed")
    ...
    if {path.name for path in canonical_root.iterdir() if path.is_dir()} != {
        ...
        raise RuntimeError("V168 canonical product changed")
    quarantine = output["workspace"] / "quarantine"
    if quarantine.exists() and any(quarantine.rglob("attempt-*")):
        raise RuntimeError("V168 has unexplained quarantined attempts")
    ...
    if len(identities) != 1:
        raise RuntimeError("V168 runtime identity is not unanimous")
    ...
    if not (
        ...
        raise RuntimeError("V168 runtime identity changed")
    mechanism = _mechanism_falsification_gate(audits)
    if not mechanism["pass"]:
        raise RuntimeError("V168 mechanism falsification breadth is insufficient")
    document = {
        "schema_version": "NSE_E1_HOMOGENEOUS_CPU_BOUNDED_TERMINAL_LOW_BLIND_AUDIT_V168_V1",
        "created_at": utc_now(),
        "status": "pass",
...
def _load_candidate(
    ...
    if len(rows) != 3 or [row["seed"] for row in rows] != list(SEEDS):
        raise RuntimeError("V168 candidate result product changed")
    return rows


def reveal_v168(root: Path = ROOT) -> dict[str, Any]:
    output = paths(root)
    if output["result"].exists():
        raise RuntimeError("V168 result already exists")
    blind = read_json(output["blind"])
    blind_hash = _assert_hashed(blind, "blind_audit_hash", "V168 blind audit")
    if not (
        ...
        raise RuntimeError("V168 blind audit did not authorize reveal")
    manifest = load_and_validate_manifest(output["ready"])
    ...
    document = {
        "schema_version": "NSE_E1_HOMOGENEOUS_CPU_BOUNDED_TERMINAL_LOW_DIAGNOSTIC_RESULT_V168_V1",
        "created_at": utc_now(),
        "training_only": True,
        "formal_results_eligible": False,
        ...
        "disposition": (
            ...
            if passed
            else "retain_all_three_valid_diagnostic_runs_and_retire_cpu_bounded_terminal_candidate"
        ),
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    if action == "prepare":
        document, key = prepare_v168(), "receipt_hash"
    elif action == "execute":
        document, key = execute_v168(), "receipt_hash"
    elif action == "blind-audit":
        document, key = blind_audit_v168(), "blind_audit_hash"
    else:
        document, key = reveal_v168(), "result_hash"
