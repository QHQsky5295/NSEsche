...
READY_SCHEDULE_NAME = "frozen-ready-run-order-v150.json"
EXECUTION_RECEIPT_NAME = "execution-receipt-v150.json"
...
def materialize_ready_schedule(root: Path = ROOT) -> dict[str, Any]:
    ...
    source_hash = _assert_hashed(source, "schedule_hash", "V150 source schedule")
    if (
        source.get("plan_sha256") != PLAN_SHA256
        or source.get("run_order_seed") != RUN_ORDER_SEED
        or len(source.get("schedule", [])) != 60
    ):
        raise RuntimeError("V150 source schedule boundary changed")
    ...
    if (
        ...
        raise RuntimeError("V150 ready manifest is not fully reference-bound")
    lookup: dict[tuple[str, str], str] = {}
    for run in manifest["runs"]:
        key = (run["workload"]["request_freq"], run["seed"])
        if key in lookup:
            raise RuntimeError(f"duplicate V150 ready key: {key}")
    ...
    for item in source["schedule"]:
        ...
        if key not in lookup or item.get("profile") != PROFILES[item["load"]]:
            raise RuntimeError(f"missing or changed V150 ready run: {key}")
        schedule.append({**item, "run_id": lookup[key]})
    if len(schedule) != 60 or len({item["run_id"] for item in schedule}) != 60:
        raise RuntimeError("V150 ready schedule is not an exact 60-run product")
    if set(lookup) != {(item["load"], item["seed"]) for item in schedule}:
        raise RuntimeError("V150 ready manifest contains unscheduled runs")
    document = {
        "schema_version": "NSE_E1_HOMOGENEOUS_LEGACY_PROFILE_READY_RUN_ORDER_V150_V1",
        "created_at": utc_now(),
    ...
    if output.exists():
        ...
        _assert_hashed(existing, "schedule_hash", "V150 ready schedule")
        left = dict(existing)
        right = dict(document)
        ...
        if left != right:
            raise RuntimeError("refusing to replace changed V150 ready schedule")
        return existing
    write_json_atomic(output, document)
    return document


def execute_v150(root: Path = ROOT) -> dict[str, Any]:
    output = root / EXECUTION_RECEIPT_NAME
    if output.exists():
        raise RuntimeError(f"V150 execution receipt already exists: {output}")
    if not PYTHON_PATH.is_file() or file_hash(PYTHON_PATH) != PYTHON_SHA256:
        raise RuntimeError("V150 frozen Python executable is missing or changed")
    ...
    for item in schedule["schedule"]:
        ...
        if canonical.is_dir():
            ...
            if completed.returncode != 0:
                failure = {
                    "schema_version": "NSE_E1_HOMOGENEOUS_LEGACY_PROFILE_EXECUTION_FAILURE_V150_V1",
                    "created_at": utc_now(),
                    "candidate_performance_summaries_parsed": 0,
                    "plan_sha256": PLAN_SHA256,
                ...
                write_json_atomic(root / "execution-failure-v150.json", failure)
                raise RuntimeError(
                    f"V150 frozen dispatch {item['ordinal']} failed with exit code "
        ...
        if not (
            ...
            raise RuntimeError(f"V150 canonical is not a QC pass: {item['run_id']}")
        dispatch = {
    ...
    receipt = {
        "schema_version": "NSE_E1_HOMOGENEOUS_LEGACY_PROFILE_EXECUTION_V150_V1",
        "created_at": utc_now(),
        "candidate_performance_summaries_parsed": 0,
        "performance_results_consulted_for_design": True,
        "plan_sha256": PLAN_SHA256,
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    document = materialize_ready_schedule() if args.materialize_only else execute_v150()
