...
from scripts.reviewer_experiments.protocol.nse_e3_processor_sharing_work_training_blind_audit_v112 import (
    ARMS,
...
OUTPUT = ROOT / "training-result-v112.json"
BLIND_AUDIT_FILE_SHA256 = "UNFROZEN_V112_BLIND_AUDIT_FILE_SHA256"
BLIND_AUDIT_HASH = "UNFROZEN_V112_BLIND_AUDIT_HASH"
EXPECTED_SEEDS = ("E905", "E906", "E907")
EXPECTED_SCENARIOS = (
    "E3.spike5x50ms",
    "E3.sustained3x200ms",
    "E3.pulse4x4x50ms",
...
def _validate_blind_audit() -> dict[str, Any]:
    ...
    if any(
        ...
        raise RuntimeError("V112 blind audit has not been frozen into the reveal code")
    if not BLIND_AUDIT.is_file() or file_hash(BLIND_AUDIT) != BLIND_AUDIT_FILE_SHA256:
        raise RuntimeError("V112 blind audit is missing or changed")
    ...
    if (
        ...
        raise RuntimeError("V112 blind audit does not authorize reveal")
    return blind
...
def _load_rows(blind: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    if len(admitted) != 18:
        raise RuntimeError("V112 blind audit run evidence is incomplete")
    ...
    for arm_id, expected in ARMS.items():
        ...
        if not manifest_path.is_file() or file_hash(manifest_path) != frozen.get(
            ...
            raise RuntimeError(f"V112 frozen manifest changed: {arm_id}")
        manifest = load_and_validate_manifest(manifest_path)
        if manifest.get("manifest_hash") != frozen.get("manifest_hash"):
            raise RuntimeError(f"V112 manifest hash changed: {arm_id}")
        for run in manifest["runs"]:
            ...
            if (
                ...
                != expected["profile"]
                or metadata.get("v112_training_plan_sha256") != PLAN_SHA256
                or metadata.get("v112_training_only") is not True
                or metadata.get("v112_training_seed_metrics_previously_revealed")
                is not False
                or metadata.get("v112_confirmation_seeds_opened") is not False
                or metadata.get("v112_other_unopened_seeds_opened") is not False
                or metadata.get("v112_arm_id") != arm_id
                or metadata.get("v112_arm_role") != expected["role"]
                or metadata.get("v112_shock_rate_ratio") != expected["shock_rate_ratio"]
                or metadata.get("v112_shock_threshold_numerator")
                != expected["shock_threshold_numerator"]
                or metadata.get("v112_shock_threshold_denominator")
                != expected["shock_threshold_denominator"]
                or metadata.get("v112_shock_activation_horizon_frames")
                != expected["shock_activation_horizon_frames"]
                or metadata.get("v112_critical_service_ratio_numerator")
                != expected["critical_service_threshold_numerator"]
                or metadata.get("v112_critical_service_ratio_denominator")
                != expected["critical_service_threshold_denominator"]
                or metadata.get("v112_service_proxy_work_source")
                != (
                    "processor_sharing_capped_pending_runnable_remaining_and_same_window_projected_cpu_v1"
                    if candidate
                    else "not_applicable"
                )
                or metadata.get("v112_processor_sharing_work_excludes_blocked_resident")
                is not candidate
                or metadata.get("v112_outcome_fields_drive_policy") is not False
                or metadata.get("v112_scenario_or_burst_label_used_by_policy")
                is not False
                or metadata.get("v112_completion_or_performance_fields_used_by_policy")
                is not False
                or metadata.get("v112_future_arrivals_used_by_policy") is not False
            ):
                raise RuntimeError(f"V112 arm identity changed: {run['run_id']}")
            run_id = run["run_id"]
            summary_path = (
                ROOT
                / "runs"
            ...
            if not summary_path.is_file() or file_hash(summary_path) != admitted.get(
                ...
                raise RuntimeError(f"V112 summary differs from blind audit: {run_id}")
            ...
            if (
                ...
                raise RuntimeError(
                    f"V112 summary identity/completion mismatch: {run_id}"
                )
            rows.append(
                {
                    "arm_id": arm_id,
...
def execute_reveal() -> dict[str, Any]:
    if OUTPUT.exists():
        raise RuntimeError(f"V112 training result already exists: {OUTPUT}")
    if not PLAN.is_file() or file_hash(PLAN) != PLAN_SHA256:
        raise RuntimeError("frozen V112 plan changed before reveal")
    blind = _validate_blind_audit()
    rows = _load_rows(blind)
    if len(rows) != 18:
        raise RuntimeError("V112 revealed row count differs from sealed plan")
    ...
    result = {
        "schema_version": "NSE_E3_PROCESSOR_SHARING_WORK_RESULT_V112_V1",
        "created_at": utc_now(),
        ...
        **evaluation,
        ...
        "decision": {
            ...
            "next_action": (
                "freeze V112 with frozen V97 E4 and separately preregister the "
                "complete E1066-E1085 NSESche confirmation before generating inputs"
                if joint_pass
                else (
                    "freeze the complete V112 training cohort, keep E926-E1085 and "
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Reveal the frozen V112 training cohort exactly once after blind audit."
    )
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    if args.command != "reveal" or not args.execute:
        parser.error("V112 reveal requires the explicit command: reveal --execute")
    execute_reveal()
