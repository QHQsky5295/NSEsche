...
from scripts.reviewer_experiments.protocol.nse_e3_terminal_load_band_training_blind_audit_v98 import (
    ARMS,
...
OUTPUT = ROOT / "training-result-v98.json"
BLIND_AUDIT_FILE_SHA256 = (
    "0c4e35e4303e65ab6670cf7f9049025ba21011facd5dd377f245fb970adf59be"
)
BLIND_AUDIT_HASH = "631d0dbe6889e1ada3a7945fd2333509b6e8d11625789a65238ed5fbb0141f38"
EXPECTED_SEEDS = ("E826", "E827", "E828")
EXPECTED_SCENARIOS = (
    "E3.spike5x50ms",
    "E3.sustained3x200ms",
    "E3.pulse4x4x50ms",
...
def _validate_blind_audit() -> dict[str, Any]:
    if not BLIND_AUDIT.is_file() or file_hash(BLIND_AUDIT) != BLIND_AUDIT_FILE_SHA256:
        raise RuntimeError("V98 blind audit is missing or changed")
    ...
    if (
        ...
        raise RuntimeError("V98 blind audit does not authorize reveal")
    return blind
...
def _load_rows(blind: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    if len(admitted) != 27:
        raise RuntimeError("V98 blind audit run evidence is incomplete")
    ...
    for arm_id, expected in ARMS.items():
        ...
        if not manifest_path.is_file() or file_hash(manifest_path) != frozen.get(
            ...
            raise RuntimeError(f"V98 frozen manifest changed: {arm_id}")
        manifest = load_and_validate_manifest(manifest_path)
        if manifest.get("manifest_hash") != frozen.get("manifest_hash"):
            raise RuntimeError(f"V98 manifest hash changed: {arm_id}")
        for run in manifest["runs"]:
            ...
            if (
                ...
                != expected["profile"]
                or metadata.get("v98_training_plan_sha256") != PLAN_SHA256
                or metadata.get("v98_training_only") is not True
                or metadata.get("v98_training_seed_metrics_previously_revealed")
                is not False
                or metadata.get("v98_confirmation_seeds_opened") is not False
                or metadata.get("v98_previously_reserved_seeds_opened") is not False
                or metadata.get("v98_arm_id") != arm_id
                or metadata.get("v98_arm_role") != expected["role"]
                or metadata.get("v98_upper_queue_density_threshold")
                != expected["upper_queue_density_threshold"]
                or metadata.get("v98_nonterminal_queue_density_floor")
                != expected["nonterminal_queue_density_floor"]
            ):
                raise RuntimeError(f"V98 arm identity changed: {run['run_id']}")
            run_id = run["run_id"]
            summary_path = (
                ROOT
                / "runs"
            ...
            if not summary_path.is_file() or file_hash(summary_path) != admitted.get(
                ...
                raise RuntimeError(f"V98 summary differs from blind audit: {run_id}")
            ...
            if (
                ...
                raise RuntimeError(
                    f"V98 summary identity/completion mismatch: {run_id}"
                )
            rows.append(
                {
                    "arm_id": arm_id,
...
def execute_reveal() -> dict[str, Any]:
    if OUTPUT.exists():
        raise RuntimeError(f"V98 training result already exists: {OUTPUT}")
    if not PLAN.is_file() or file_hash(PLAN) != PLAN_SHA256:
        raise RuntimeError("frozen V98 plan changed before reveal")
    blind = _validate_blind_audit()
    rows = _load_rows(blind)
    if len(rows) != 27:
        raise RuntimeError("V98 revealed row count differs from sealed plan")
    ...
    result = {
        "schema_version": "NSE_E3_TERMINAL_LOAD_BAND_RESULT_V98_V1",
        "created_at": utc_now(),
        ...
        "selection_rule": {
            ...
            "frozen_E4": (
                "retain V97 v97-e4-load-guard64 without V98 result-dependent reselection"
            ),
        ...
        **evaluation,
        "revealed_rows": rows,
        "decision": {
            "authorize_preregistration_of_E846_E865_confirmation": joint_pass,
            ...
            "next_action": (
                ...
                "separately preregister the complete E846-E865 NSESche confirmation "
                "before generating inputs"
                if joint_pass
                else (
                    "freeze the complete V98 training cohort, keep E846-E865, "
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Reveal the frozen V98 training cohort exactly once after blind audit."
    )
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    if args.command != "reveal" or not args.execute:
        parser.error("V98 reveal requires the explicit command: reveal --execute")
    execute_reveal()
