...
BLIND_AUDIT = ROOT / "joint-blind-audit-v93-training.json"
MANIFEST = ROOT / "manifest.v93-srpt-ready-native-training.ready.json"
MANIFEST_FILE_SHA256 = (
    "30871586a87787ae79353e79df72d8b25629370e4956acc5c3eca8808c207c68"
)
MANIFEST_HASH = "07b0b1aeef0573e78180455a83937afb76594f5f2a1015388a4c9136c18695ef"
CANONICAL = ROOT / "runs/training-v93/canonical"
OUTPUT = ROOT / "training-result-v93.json"
...
def _load_rows() -> list[dict]:
    ...
    for run in manifest["runs"]:
        ...
        if (
            ...
            or metadata.get("v93_training_plan_sha256") != PLAN_SHA256
            or metadata.get("v93_training_only") is not True
            or metadata.get("v93_exact_native_initializer") is not True
            or metadata.get("v93_every_player_expert_nonworsening_certificate")
            is not True
            or metadata.get("v93_at_least_one_player_strict_expert_improvement")
            is not True
            or metadata.get("v93_parent_complete_ready_frontier") is not True
            or metadata.get("v93_srpt_player_order") is not True
            or metadata.get("v93_native_expert_demand_context_retained") is not True
            or metadata.get("v93_complete_v92_cohort_retained") is not True
        ):
...
def _validate_blind_audit() -> dict:
    if not BLIND_AUDIT.is_file():
        raise RuntimeError("V93 blind audit is missing")
    ...
    if (
        ...
        raise RuntimeError("V93 blind audit does not authorize reveal")
    return blind


def main() -> None:
    if OUTPUT.exists():
        raise RuntimeError(f"V93 training result already exists: {OUTPUT}")
    ...
    rows = _load_rows()
    if len(rows) != 12:
        raise RuntimeError("V93 revealed row count differs from sealed plan")
    ...
    result = {
        "schema_version": "NSE_E3E4_SRPT_READY_NATIVE_TRAINING_RESULT_V93_V1",
        "created_at": utc_now(),
        ...
        "revealed_rows": rows,
        "decision": {
            "authorize_v93_confirmation_on_E716_E718": all_pass,
            ...
            "next_action": (
                "freeze V93 profiles and preregister exactly E716-E718"
                if all_pass
                else "retain V93 cohort, keep E716-E718 sealed, and return to mechanism diagnosis"
            ),
        },
    }
