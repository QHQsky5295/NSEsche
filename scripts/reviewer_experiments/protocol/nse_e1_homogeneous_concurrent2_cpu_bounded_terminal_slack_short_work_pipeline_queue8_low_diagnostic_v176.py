...
ROOT = Path(
    "tmp/nse_e1_homogeneous_concurrent2_cpu_bounded_terminal_low_diagnostic_20260901_v176"
)
PLAN = Path(
    "scripts/reviewer_experiments/protocol/"
    "nse_e1_homogeneous_concurrent2_cpu_bounded_terminal_low_diagnostic_plan_v176.json"
)
PLAN_SHA256 = "7912c25ff074120d3dbb2a1df675c558a2a228bf0ec4687344dfd0dc41d55341"
IMPLEMENTATION = Path(
    "scripts/reviewer_experiments/protocol/"
    "nse_e1_homogeneous_concurrent2_cpu_bounded_terminal_slack_short_work_pipeline_queue8_low_diagnostic_implementation_v176.json"
)
IMPLEMENTATION_SHA256 = (
    "84497cacd548c0f677d397083212a70cd22797863385427f6dbe3b9fde9c8005"
)
V170_FAILURE = v175.V170_FAILURE
V170_FAILURE_SHA256 = v175.V170_FAILURE_SHA256
V170_FAILURE_HASH = v175.V170_FAILURE_HASH
...
V175_FAILURE = Path(
    "scripts/reviewer_experiments/protocol/"
    "nse_e1_homogeneous_concurrent3_requestcohort1_shortest_request_least_cpu_bounded_terminal_slack_short_work_pipeline_queue8_low_diagnostic_failure_v175.json"
)
V175_FAILURE_SHA256 = "b031df6c786021ebd9ddd0a752f6e7af0bd52823a28f58da99a82fdbcbcdef94"
V175_FAILURE_HASH = "f23bc7eab89e466fa7d0fbcdf1cb8caefcfde13825f8682718bc676f877accd7"

SEEDS = ("E01", "E05", "E07", "E08", "E10", "E14")
THROUGHPUT_SIX_SEED_SUM_GATE = 8.674999999999997
QPR_SIX_SEED_SUM_GATE = 0.41630826952008215
THROUGHPUT_SIX_SEED_PAIRED_WIN_GATE = 4
QPR_SIX_SEED_PAIRED_WIN_GATE = 3

ARM_ID = "v176-low-srpt-slack-concurrent2-cpu-bounded-terminal-short5p5-pipeline-hiku2-ocs-queue8"
PROFILE = (
    "srpt_slack_concurrent2_cpu_bounded_terminal_short5p5_pipeline_hiku2_ocs_queue8"
)
FRONTIER = (
    "parents_completed_or_concurrent2_cpu_bounded_terminal_or_slack_short_work_"
    "parents_scheduled"
)
SINGLE_CHANGE = (
    "V170_all_or_none_heavy_terminal_rejection_activated_when_current_heavy_"
...
MINIMUM_ACTIVE_HEAVY_PLAYERS = 2
...
BINARY_SOURCE_COMMIT = "f314643dae0b96f5cd3b7bd955d1a7a363f02413"
BINARY_PATH = Path("serverless_sim/target_e1_v176/release/serverless_sim.exe")
BINARY_SHA256 = "ebff14215d4e31dd1efb24cf104b8413bbbdbe700fbf3028e4ece1be42beb6fd"


def paths(root: Path = ROOT) -> dict[str, Path]:
    return {
        "manifest": root / f"manifest.{ARM_ID}.unbound.json",
        "prepared": root / "prepared-v176.json",
        "schedule": root / "frozen-run-order-v176.json",
        ...
        "execution": root / "execution-receipt-v176.json",
        "pairing": root / "pairing-audit-v176.json",
        "blind": root / "joint-blind-audit-v176.json",
        "result": root / "diagnostic-result-v176.json",
    }
...
def _assert_frozen_inputs() -> dict[str, Any]:
    ...
    for path, sha256, label in (
        (PLAN, PLAN_SHA256, "V176 plan"),
        (IMPLEMENTATION, IMPLEMENTATION_SHA256, "V176 implementation receipt"),
        (V170_FAILURE, V170_FAILURE_SHA256, "V170 failure receipt"),
        (V170_COMPLETE_RESULT, V170_COMPLETE_RESULT_SHA256, "V170 complete result"),
        (V175_FAILURE, V175_FAILURE_SHA256, "V175 failure receipt"),
        (BINARY_PATH, BINARY_SHA256, "V176 release binary"),
    ...
    if not (
        _assert_hashed(implementation, "receipt_hash", "V176 implementation receipt")
        == "38b0fb546f30fb9a8d8c55235ce4d6456c7e1c8f8b11485a825ca44ca0526687"
        ...
        and change.get("new_minimum_active_count") == MINIMUM_ACTIVE_HEAVY_PLAYERS
        ...
        and telemetry.get("version") == "V176"
        ...
        raise RuntimeError("V176 implementation boundary changed")
    failure = read_json(V170_FAILURE)
    if not (
        _assert_hashed(failure, "receipt_hash", "V170 failure receipt")
        == V170_FAILURE_HASH
        ...
        raise RuntimeError("V170 frozen failure boundary changed")
    failure = read_json(V175_FAILURE)
    if not (
        _assert_hashed(failure, "receipt_hash", "V175 failure receipt")
        == V175_FAILURE_HASH
        and failure.get("diagnostic_result", {}).get("joint_pass") is False
        and failure.get("disposition", {}).get("retain_all_six_valid_v175_runs") is True
        and failure.get("disposition", {}).get(
            "delete_replace_relabel_or_selectively_rerun_any_v175_seed"
        )
        is False
    ):
        raise RuntimeError("V175 frozen failure boundary changed")
...
def _metadata(protocol_source_commit: str, source: Mapping[str, Any]) -> dict[str, Any]:
    return {
        "v176_training_only": True,
        "v176_role": "result_blind_concurrent2_cpu_bounded_terminal_falsification",
        "v176_plan_sha256": PLAN_SHA256,
        "v176_implementation_sha256": IMPLEMENTATION_SHA256,
        "v176_binary_source_commit": BINARY_SOURCE_COMMIT,
        "v176_protocol_source_commit": protocol_source_commit,
        "v176_binary_sha256": BINARY_SHA256,
        "v176_arm_id": ARM_ID,
        "v176_profile": PROFILE,
        "v176_player_frontier": FRONTIER,
        "v176_single_change_from_v170": SINGLE_CHANGE,
        "v176_cpu_threshold": CPU_THRESHOLD,
        "v176_cpu_boundary": "at_or_below_is_admitted",
        "v176_heavy_player_threshold": HEAVY_PLAYER_THRESHOLD,
        "v176_minimum_active_heavy_players": MINIMUM_ACTIVE_HEAVY_PLAYERS,
        "v176_overload_activation_boundary": "strictly_above_activates",
        "v176_inactive_behavior": "exact_V159_terminal_admission",
        "v176_active_heavy_admission_policy": "all_current_heavy_incomplete_parent_terminal_players_rejected",
        "v176_active_heavy_admission_quota": None,
        "v176_short_work_threshold": SHORT_WORK_THRESHOLD,
        "v176_queue_density_threshold": QUEUE_THRESHOLD,
        "v176_queue_boundary": "below_is_strict",
        "v176_source_e1_run_id": source.get("v155_source_e1_run_id"),
        "v176_source_e1_run_spec_hash": source.get("v155_source_e1_run_spec_hash"),
        "v176_candidate_performance_summaries_parsed_before_run": 0,
        "v176_remaining_fourteen_authorized": False,
        "v176_confirmation_inputs_generated": False,
    }
...
def _rewrite_candidate(
    ...
    marker.update(
        {
            "purpose": "V176 outcome-disclosed six-seed training diagnosis; never a formal result or paper superiority claim",
            "v176_role": "result_blind_concurrent2_cpu_bounded_terminal_falsification",
            "v176_plan_sha256": PLAN_SHA256,
            "v176_implementation_sha256": IMPLEMENTATION_SHA256,
            "v176_binary_source_commit": BINARY_SOURCE_COMMIT,
            ...
            "v176_queue_density_threshold": QUEUE_THRESHOLD,
            "v176_queue_boundary": "below_is_strict",
            "v176_environment": COMMON_ENVIRONMENT,
            "v176_expected_run_count": len(SEEDS),
            "v176_expected_reference_build_count": len(SEEDS),
            "v176_fixed_order": list(SEEDS),
            "v176_candidate_performance_summaries_parsed": 0,
            "v176_remaining_fourteen_authorized": False,
            "v176_confirmation_inputs_generated": False,
        }
    )
...
def _validate_product(manifest: Mapping[str, Any], *, references_bound: bool) -> None:
    if not (
        ...
        raise RuntimeError("V176 exact E01/E05/E07/E08/E10/E14 product changed")
    ...
    for run in manifest["runs"]:
        ...
        if not (
            ...
            and metadata.get("v176_profile") == PROFILE
            and metadata.get("v176_player_frontier") == FRONTIER
            and metadata.get("v176_cpu_threshold") == CPU_THRESHOLD
            and metadata.get("v176_heavy_player_threshold") == HEAVY_PLAYER_THRESHOLD
            and metadata.get("v176_minimum_active_heavy_players")
            == MINIMUM_ACTIVE_HEAVY_PLAYERS
            and metadata.get("v176_overload_activation_boundary")
            == "strictly_above_activates"
            and metadata.get("v176_inactive_behavior")
            == "exact_V159_terminal_admission"
            and metadata.get("v176_active_heavy_admission_policy")
            == "all_current_heavy_incomplete_parent_terminal_players_rejected"
            and metadata.get("v176_active_heavy_admission_quota") is None
            and metadata.get("v176_short_work_threshold") == SHORT_WORK_THRESHOLD
            and metadata.get("v176_queue_density_threshold") == QUEUE_THRESHOLD
        ):
            raise RuntimeError(f"V176 run contract changed: {run.get('run_id')}")


def prepare_v176(root: Path = ROOT) -> dict[str, Any]:
    source = _assert_frozen_inputs()
    if root.exists():
        raise RuntimeError(f"refusing to overwrite V176 root: {root}")
    ...
    schedule = {
        "schema_version": "NSE_E1_HOMOGENEOUS_CONCURRENT2_CPU_BOUNDED_TERMINAL_LOW_SCHEDULE_V176_V1",
        "created_at": utc_now(),
    ...
    receipt = {
        "schema_version": "NSE_E1_HOMOGENEOUS_CONCURRENT2_CPU_BOUNDED_TERMINAL_LOW_PREPARED_V176_V1",
        "created_at": utc_now(),
        ...
        "v175_diagnostic_failure_file_sha256": V175_FAILURE_SHA256,
        "v175_diagnostic_failure_hash": V175_FAILURE_HASH,
...
def execute_v176(root: Path = ROOT) -> dict[str, Any]:
    output = paths(root)
    if output["execution"].exists():
        raise RuntimeError("V176 execution receipt already exists")
    prepared = read_json(output["prepared"])
    _assert_hashed(prepared, "receipt_hash", "V176 prepared receipt")
    ...
    for ordinal, seed in enumerate(SEEDS, start=1):
        ...
        if completed.returncode != 0:
            raise RuntimeError(f"V176 dispatch {seed} failed: {completed.returncode}")
        ...
        if not (
            ...
            raise RuntimeError(f"V176 canonical is not a QC pass: {run['run_id']}")
        dispatches.append(
            {
                "ordinal": ordinal,
                "seed": seed,
    ...
    receipt = {
        "schema_version": "NSE_E1_HOMOGENEOUS_CONCURRENT2_CPU_BOUNDED_TERMINAL_LOW_EXECUTION_V176_V1",
        "created_at": utc_now(),
        "candidate_performance_summaries_parsed": 0,
        "plan_sha256": PLAN_SHA256,
...
def _audit_nash_log(
    ...
    run: Mapping[str, Any],
    *,
    frozen_v170: Sequence[int] | None = None,
) -> dict[str, Any]:
    run_id = run["run_id"]
    ...
    first_exact_two_frame: int | None = None
    ...
    with gzip.open(log, "rt", encoding="utf-8") as stream:
        for line in stream:
            ...
            if kind not in counts:
                raise RuntimeError(f"unexpected V176 Nash observation kind: {kind}")
            counts[kind] += 1
            if kind == "run_config":
                ...
                checks = (
                    ...
                    ("version", contract.get("version"), "V176"),
                    (
                        "queue_threshold",
                    ...
                    (
                        ...
                        "heavy_player_count_strictly_above_one",
                    ),
                    (
                        "inactive_behavior",
                ...
                if mismatches:
                    raise RuntimeError(
                        "V176 run_config contract changed: " + ",".join(mismatches)
                    )
                continue
                if False and not (
                    activation.get("heavy_player_definition")
                    ...
                    raise RuntimeError("V176 run_config contract changed")
                continue
            if kind == "run_summary":
                if not (
                    ...
                    raise RuntimeError("V176 Nash terminal marker changed")
                continue
            ...
            if event.get("frame") != frame:
                raise RuntimeError("V176 scheduler window sequence changed")
            ...
            if (
                ...
                raise RuntimeError("V176 decision frontier/hash changed")
            ...
            if not (
                ...
                and activation.get("activation_boundary") == "strictly_above_activates"
                and activation.get("active_heavy_admission_policy") is None
                and activation.get("active_heavy_admission_quota") is None
                and activation.get("active_heavy_admission_quota_unit") is None
                and activation.get("quota_selection_order") is None
                ...
                and active is (heavy > HEAVY_PLAYER_THRESHOLD)
                ...
                raise RuntimeError("V176 CPU/slack/queue frontier evidence changed")
            if heavy == 2:
                totals["exact_two_windows"] += 1
                totals["exact_two_heavy_rejections"] += guard_rejected
                if first_exact_two_frame is None:
                    first_exact_two_frame = frame
            if heavy == 1:
                totals["exact_one_windows"] += 1
                totals["exact_one_heavy_admissions"] += inactive_admissions
            ...
            if not (
                ...
                raise RuntimeError("V176 route telemetry is incomplete")
            expected = LOW_EXPERT if float(density) < QUEUE_THRESHOLD else HIGH_EXPERT
            if route.get("selected_expert") != expected:
                raise RuntimeError("V176 route does not match queue density")
            ...
            if key is None:
                if source != "not_requested":
                    raise RuntimeError("V176 unrequested reference reason changed")
                totals["reference_not_requested"] += 1
            elif source in ("offline_table", "offline_table_nonpositive"):
                totals["reference_available"] += 1
            else:
                raise RuntimeError("V176 bound reference source changed")
    ...
    if (
        ...
        raise RuntimeError("V176 Nash log cardinality changed")
    if totals["reference_available"] + totals["reference_not_requested"] != 1000:
        raise RuntimeError("V176 reference replay coverage changed")
    if frozen_v170 is None:
        frozen_hash = None
        prefix_matches = None
        first_exact_two_differs = None
        first_mismatch = None
        mismatch_count = None
    else:
        if len(frozen_v170) != 1000:
            raise RuntimeError("frozen V170 assignment cardinality changed")
        frozen_hash = _assignment_sequence_sha256(frozen_v170)
        mismatch_frames = [
            index
            for index, (current, frozen) in enumerate(zip(decision_hashes, frozen_v170))
            if current != frozen
        ]
        first_mismatch = mismatch_frames[0] if mismatch_frames else None
        mismatch_count = len(mismatch_frames)
        prefix_matches = first_exact_two_frame is not None and tuple(
            decision_hashes[:first_exact_two_frame]
        ) == tuple(frozen_v170[:first_exact_two_frame])
        first_exact_two_differs = (
            first_exact_two_frame is not None
            and decision_hashes[first_exact_two_frame]
            != frozen_v170[first_exact_two_frame]
            and first_mismatch == first_exact_two_frame
        )
    return {
        "run_id": run_id,
        "seed": run["seed"],
        ...
        "frozen_v170_assignment_sequence_sha256": frozen_hash,
        "assignment_mismatch_count_vs_v170": mismatch_count,
        "first_assignment_mismatch_frame_vs_v170": first_mismatch,
        "first_exact_two_frame": first_exact_two_frame,
        "pre_first_exact_two_assignment_prefix_matches_v170": prefix_matches,
        "first_exact_two_frame_assignment_differs_from_v170": first_exact_two_differs,
        ...
        "capacity_overload_exact_one_heavy_terminal_admissions": totals[
            "exact_one_heavy_admissions"
        ],
...
def _mechanism_falsification_gate(
    ...
    sum_keys = (
        ...
        "capacity_overload_exact_one_heavy_terminal_admissions",
    ...
    if set(by_seed) != set(SEEDS):
        raise RuntimeError("V176 blind audit seed product changed")
    selected_exact_two = all(
        by_seed[seed]["capacity_overload_exact_two_windows"] > 0
        and by_seed[seed]["first_exact_two_frame"] is not None
        and by_seed[seed]["pre_first_exact_two_assignment_prefix_matches_v170"] is True
        and by_seed[seed]["first_exact_two_frame_assignment_differs_from_v170"] is True
        and by_seed[seed]["first_assignment_mismatch_frame_vs_v170"]
        == by_seed[seed]["first_exact_two_frame"]
        for seed in SEEDS
    )
    boundary_exact = (
        totals["capacity_overload_exact_one_windows"] > 0
        and totals["capacity_overload_exact_one_heavy_terminal_admissions"]
        == totals["capacity_overload_exact_one_windows"]
    ...
    return {
        **totals,
        **extrema,
        "every_selected_seed_exercised_exact_two_boundary": all(
            by_seed[seed]["capacity_overload_exact_two_windows"] > 0 for seed in SEEDS
        ),
        "selected_seeds_match_v170_until_first_exact_two_then_diverge": selected_exact_two,
        "selected_seed_first_exact_two_frames": {
            seed: by_seed[seed]["first_exact_two_frame"] for seed in SEEDS
        },
        "selected_seed_first_assignment_mismatch_frames_vs_v170": {
            seed: by_seed[seed]["first_assignment_mismatch_frame_vs_v170"]
            for seed in SEEDS
        },
        "exact_one_inactive_and_exact_two_all_rejected": boundary_exact,
...
def blind_audit_v176(root: Path = ROOT) -> dict[str, Any]:
    output = paths(root)
    if output["blind"].exists():
        raise RuntimeError("V176 blind audit already exists")
    _assert_frozen_inputs()
    prepared = read_json(output["prepared"])
    prepared_hash = _assert_hashed(prepared, "receipt_hash", "V176 prepared receipt")
    execution = read_json(output["execution"])
    execution_hash = _assert_hashed(execution, "receipt_hash", "V176 execution receipt")
    manifest = load_and_validate_manifest(output["ready"])
    _validate_product(manifest, references_bound=True)
    ...
    if not (
        ...
        raise RuntimeError("V176 exact pairing failed")
    ...
    if [item["seed"] for item in execution["dispatches"]] != list(SEEDS):
        raise RuntimeError("V176 execution order changed")
    ...
    if {path.name for path in canonical_root.iterdir() if path.is_dir()} != {
        ...
        raise RuntimeError("V176 canonical product changed")
    quarantine = output["workspace"] / "quarantine"
    if quarantine.exists() and any(quarantine.rglob("attempt-*")):
        raise RuntimeError("V176 has unexplained quarantined attempts")
    ...
    for run in manifest["runs"]:
        ...
        audits.append(
            _audit_nash_log(
                canonical,
                run,
                frozen_v170=v175._frozen_v170_assignment_hashes(run["seed"]),
            )
        )
    if len(identities) != 1:
        raise RuntimeError("V176 runtime identity is not unanimous")
    ...
    if not (
        ...
        raise RuntimeError("V176 runtime identity changed")
    mechanism = _mechanism_falsification_gate(audits)
    if not mechanism["pass"]:
        raise RuntimeError("V176 mechanism falsification breadth is insufficient")
    document = {
        "schema_version": "NSE_E1_HOMOGENEOUS_CONCURRENT2_CPU_BOUNDED_TERMINAL_LOW_BLIND_AUDIT_V176_V1",
        "created_at": utc_now(),
        "status": "pass",
...
def _load_candidate(
    ...
    if len(rows) != len(SEEDS) or [row["seed"] for row in rows] != list(SEEDS):
        raise RuntimeError("V176 candidate result product changed")
    return rows
...
def _hybrid_rows(
    ...
    if len(v176_rows) != len(SEEDS) or {row.get("seed") for row in v176_rows} != set(
        SEEDS
    ):
        raise RuntimeError("V176 diagnostic candidate cohort changed")
    replacements = {row["seed"]: dict(row) for row in v176_rows}
    return [
...
def reveal_v176(root: Path = ROOT) -> dict[str, Any]:
    output = paths(root)
    if output["result"].exists():
        raise RuntimeError("V176 result already exists")
    blind = read_json(output["blind"])
    blind_hash = _assert_hashed(blind, "blind_audit_hash", "V176 blind audit")
    if not (
        ...
        and blind.get("selected_seeds_match_v170_until_first_exact_two_then_diverge")
        is True
        and blind.get("exact_one_inactive_and_exact_two_all_rejected") is True
        ...
        raise RuntimeError("V176 blind audit did not authorize reveal")
    manifest = load_and_validate_manifest(output["ready"])
    candidate = _load_candidate(manifest, root)
    v170_rows = v175._load_v170_candidate()
    hybrid = _hybrid_rows(v170_rows, candidate)
    ...
    mechanism_keys = (
        ...
        "capacity_overload_exact_two_windows",
        "capacity_overload_exact_two_heavy_terminal_rejections",
        "selected_seeds_match_v170_until_first_exact_two_then_diverge",
        "selected_seed_first_exact_two_frames",
        "selected_seed_first_assignment_mismatch_frames_vs_v170",
        "exact_one_inactive_and_exact_two_all_rejected",
    ...
    document = {
        "schema_version": "NSE_E1_HOMOGENEOUS_CONCURRENT2_CPU_BOUNDED_TERMINAL_LOW_DIAGNOSTIC_RESULT_V176_V1",
        "created_at": utc_now(),
        "training_only": True,
        "formal_results_eligible": False,
        ...
        "disposition": (
            "authorize_separately_committed_remaining_fourteen_training_plan_without_rerunning_E01_E05_E07_E08_E10_E14"
            if passed
            else "retain_all_six_valid_diagnostic_runs_and_retire_concurrent2_cpu_bounded_terminal_candidate"
        ),
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    if action == "prepare":
        document, key = prepare_v176(), "receipt_hash"
    elif action == "execute":
        document, key = execute_v176(), "receipt_hash"
    elif action == "blind-audit":
        document, key = blind_audit_v176(), "blind_audit_hash"
    else:
        document, key = reveal_v176(), "result_hash"
