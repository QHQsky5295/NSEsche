...
from scripts.reviewer_experiments.protocol.nse_e3_random_anchor_training_blind_audit_v141 import (
    ARMS,
...
OUTPUT = ROOT / "training-result-v141.json"
...
def evaluate_training_rows(rows: Sequence[Mapping[str, Any]]) -> dict[str, Any]:
    ...
    for row in rows:
        ...
        if run_id in run_ids:
            raise ValueError(f"duplicate V141 run_id: {run_id}")
        run_ids.add(run_id)
    ...
    if len(rows) != 108 or actual != expected:
        raise ValueError(
            "V141 revealed product mismatch: "
    ...
    for arm_id in ARM_IDS:
        ...
        score = {
            ...
            "native_selection_rule": ARMS[arm_id]["selection_rule"],
...
def _validate_blind_audit() -> dict[str, Any]:
    ...
    if any(
        ...
        raise RuntimeError("V141 blind audit has not been frozen into reveal code")
    if not BLIND_AUDIT.is_file() or file_hash(BLIND_AUDIT) != BLIND_AUDIT_FILE_SHA256:
        raise RuntimeError("V141 blind audit is missing or changed")
    ...
    if (
        ...
        or blind.get("retired_unmaterialized_v137_confirmation_seeds")
        != RETIRED_V137_CONFIRMATION_SEEDS
        or blind.get("retired_opened_v138_training_seeds")
        != RETIRED_OPENED_V138_TRAINING_SEEDS
        or blind.get("retired_opened_v139_training_seeds")
        != RETIRED_OPENED_V139_TRAINING_SEEDS
        or blind.get("retired_opened_v140_training_seeds")
        != RETIRED_OPENED_V140_TRAINING_SEEDS
        ...
        raise RuntimeError("V141 blind audit does not authorize reveal")
    return blind
...
def _load_rows(blind: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    if len(admitted) != 108:
        raise RuntimeError("V141 blind audit run evidence is incomplete")
    rows = []
    for manifest_id in ["v141-baselines", *ARM_IDS]:
        ...
        if not manifest_path.is_file() or file_hash(manifest_path) != frozen.get(
            ...
            raise RuntimeError(f"V141 frozen manifest changed: {manifest_id}")
        manifest = load_and_validate_manifest(manifest_path)
        if manifest.get("manifest_hash") != frozen.get("manifest_hash"):
            raise RuntimeError(f"V141 manifest hash changed: {manifest_id}")
        ...
        for run in manifest["runs"]:
            ...
            if evidence is None or evidence.get("manifest_id") != manifest_id:
                raise RuntimeError(f"V141 run not admitted by blind audit: {run_id}")
            summary_path = (
            ...
            if not summary_path.is_file() or file_hash(summary_path) != evidence.get(
                ...
                raise RuntimeError(f"V141 summary differs from blind audit: {run_id}")
            summary = read_json(summary_path)
            if (
                summary.get("run_complete") is not True
                or summary.get("run_id") != run_id
            ):
                raise RuntimeError(f"V141 summary identity changed: {run_id}")
            rows.append(
                {
...
def execute_reveal() -> dict[str, Any]:
    if OUTPUT.exists():
        raise RuntimeError(f"refusing to overwrite V141 result: {OUTPUT}")
    if not PLAN.is_file() or file_hash(PLAN) != PLAN_SHA256:
        raise RuntimeError("V141 plan changed before reveal")
    blind = _validate_blind_audit()
    rows = _load_rows(blind)
    evaluation = evaluate_training_rows(rows)
    result = {
        "schema_version": "NSE_E3_RANDOM_ANCHOR_TRAINING_RESULT_V141_V1",
        "created_at": utc_now(),
