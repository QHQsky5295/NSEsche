...
from scripts.reviewer_experiments.protocol.nse_e3_load_band_substitution_cap_training_prepare_v99 import (
...
OUTPUT = ROOT / "joint-blind-audit-v99-training.json"
RESULT = ROOT / "training-result-v99.json"
...
ARMS = {
    ...
    for arm_id, experiment_id, role, profile, cap, count in PREPARED_ARMS
}
...
def _verify_tape_catalog() -> tuple[list[dict[str, Any]], dict[str, Any]]:
    _require(TAPES.is_file(), f"missing V99 tape catalog: {TAPES}")
    catalog = read_json(TAPES)
    catalog_hash = _assert_hashed_object(catalog, "catalog_hash", "V99 tapes")
    entries = catalog.get("entries")
    _require(isinstance(entries, dict) and len(entries) == 12, "V99 tape count changed")
    evidence = []
    ...
    _require(
        ...
        "V99 tape kind boundary changed",
    )
    ...
    _assert_ledger_contract(
        rows, Counter({"capture_canonicalized": 3}), "V99 tape capture"
    )
    _require(
        not list(
        ...
        "V99 tape capture has quarantined attempts",
    )
    return evidence, {
...
def run_blind_audit(output: Path = OUTPUT) -> dict[str, Any]:
    _require(not output.exists(), f"V99 blind audit already exists: {output}")
    _require(not RESULT.exists(), "V99 reveal exists before blind audit")
    _require(PLAN.is_file() and file_hash(PLAN) == PLAN_SHA256, "V99 plan changed")
    plan = read_json(PLAN)
    _require(plan.get("formal_results_eligible") is False, "V99 eligibility changed")
    _require(PREPARED.is_file(), f"missing V99 prepared receipt: {PREPARED}")
    prepared = read_json(PREPARED)
    prepared_hash = _assert_hashed_object(
        prepared, "receipt_hash", "V99 prepared receipt"
    )
    _require(
        ...
        "V99 prepared scientific boundary changed",
    )
    ...
    for arm_id, expected in ARMS.items():
        ...
        _require(paths["ready"].is_file(), f"missing V99 ready manifest: {arm_id}")
        manifest = load_and_validate_manifest(paths["ready"])
        _require(
            len(manifest["runs"]) == expected["run_count"],
            f"V99 run count changed: {arm_id}",
        )
        _require(
            ...
            f"V99 ready flags changed: {arm_id}",
        )
        ...
        reference_catalog_hash = _assert_hashed_object(
            references, "catalog_hash", f"V99 references {arm_id}"
        )
        ...
        _require(
            ...
            f"V99 reference key set changed: {arm_id}",
        )
        for key, entry in sorted(entries.items()):
        ...
        _require(
            ...
            f"V99 reference quarantine is nonempty: {arm_id}",
        )
        ...
        _require(actual_ids == expected_ids, f"V99 canonical set changed: {arm_id}")
        _require(
            not list((workspace / "quarantine").glob("**/attempt-*")),
            f"V99 online quarantine is nonempty: {arm_id}",
        )
        ...
        for run in manifest["runs"]:
            ...
            _require(
                ...
                and metadata.get("v99_arm_id") == arm_id
                and metadata.get("v99_arm_role") == expected["role"]
                and metadata.get("v99_candidate_profile") == expected["profile"]
                and metadata.get("v99_upper_queue_density_threshold")
                == expected["upper_queue_density_threshold"]
                and metadata.get("v99_nonterminal_queue_density_floor")
                == expected["nonterminal_queue_density_floor"]
                and metadata.get("v99_substitution_cap") == expected["substitution_cap"]
                and metadata.get("v99_training_seed_metrics_previously_revealed")
                is False
                and metadata.get("v99_confirmation_seeds_opened") is False
                and metadata.get("v99_previously_reserved_seeds_opened") is False,
                f"V99 run boundary changed: {run['run_id']}",
            )
            ...
            _require(
                ...
                f"V99 canonical status changed: {run['run_id']}",
            )
            runtime = {
    ...
    _require(len(run_evidence) == 27, "V99 run evidence count must be 27")
    _require(len(reference_evidence) == 27, "V99 reference evidence count must be 27")
    _require(len(tape_evidence) == 12, "V99 tape evidence count must be 12")
    for field, expected in EXPECTED_RUNTIME.items():
        _require(
            runtime_values[field] == {expected},
            f"V99 runtime {field} changed: {runtime_values[field]}",
        )
    ...
    _require(
        ...
        f"V99 runtime git identity is not singular: {git_commits}",
    )
    for (experiment_id, scenario, seed), rows in paired_inputs.items():
        _require(
            len(rows) == 3,
            f"V99 paired arm count changed: {experiment_id}/{scenario}/{seed}",
        )
        for field in (
            ...
            _require(
                ...
                f"V99 paired {field} changed: {scenario}/{seed}",
            )
        _require(
            rows[0]["common_hpa_hash"] == EXPECTED_COMMON_HPA_SHA256,
            f"V99 common HPA changed: {scenario}/{seed}",
        )
    ...
    output_payload = {
        "schema_version": "NSE_E3_LOAD_BAND_SUBSTITUTION_CAP_BLIND_AUDIT_V99_V1",
        "created_at": utc_now(),
        "status": "pass",
