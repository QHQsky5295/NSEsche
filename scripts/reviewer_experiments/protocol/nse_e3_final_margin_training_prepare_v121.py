...
ROOT = Path("tmp/nse_e3_final_margin_training_20260829_v121")
PLAN = Path(
    "scripts/reviewer_experiments/protocol/"
    "nse_e3_final_margin_training_plan_v121.json"
)
PLAN_SHA256 = "1391a43d4660edec1957bcfe5ea602ae41f5c19b264efb7a87037a5a42181a26"
...
V120_RESULT_RECEIPT = Path(
    "scripts/reviewer_experiments/protocol/"
    "nse_e3_dual_prefix_service_training_result_v120.json"
)
V120_RESULT_RECEIPT_SHA256 = (
    "c1197cc377bbe8eb9204519b98c6b2b69bd2cda5e7835c31d92109ba396d809f"
)
V120_RESULT_RECEIPT_HASH = (
    "600b2872a45146989f248ea7d2098f3695f83a8ee57328c865a89bf1eb81de9e"
)
V120_RESULT_RECEIPT_COMMIT = "fc31849"
BINARY_PATH = Path("tmp/nse_v121_build_3b21400/release/serverless_sim.exe")
BINARY_SHA256 = "565b66d8ce06928a52b154c400ab051018199d9b0d374eeeb52df1314812ffd7"
BINARY_SOURCE_COMMIT = "3b214004b9d5e03039b89bc22e1554f054c39eda"

SOURCE_INITIAL_SEEDS = [f"E{index}" for index in range(1126, 1136)]
SOURCE_CI_SEEDS = [f"E{index}" for index in range(1136, 1146)]
TRAINING_SEED_LIST = ["E1126", "E1127", "E1128"]
TRAINING_SEEDS = set(TRAINING_SEED_LIST)
CONFIRMATION_SEEDS = [f"E{index}" for index in range(1129, 1149)]
PREVIOUS_CONFIRMATION_SEEDS = [f"E{index}" for index in range(932, 1126)]
...
E3_FINAL_MARGIN = (
    ...
    "all_player_service_directed_dual_prefix_changed_player_margin_componentwise_pareto_initializer_only_guard64_dual_window_safe_pareto"
)
ARMS = (
    ("v121-e3-anchor", "anchor", E3_ANCHOR, 9),
    (
        "v121-e3-causal-shock15-horizon50-admitted-work10-all-player-dual-prefix-final-margin-service-directed-initializer-only",
        "candidate",
        E3_FINAL_MARGIN,
        9,
    ),
...
def _paths(root: Path) -> dict[str, Path]:
    return {
        "config": root / "v121-e3-training-config.json",
        "source": root / "manifest.v121-e1126-e1135-full.unbound.json",
        "capture": root / "manifest.v121-tape-capture.unbound.json",
...
def _assert_frozen_inputs() -> None:
    for path, expected in (
        ...
        (V120_RESULT_RECEIPT, V120_RESULT_RECEIPT_SHA256),
        ...
        if not path.is_file() or file_hash(path) != expected:
            raise RuntimeError(f"frozen V121 input is missing or changed: {path}")
    parent = json.loads(V120_RESULT_RECEIPT.read_text(encoding="utf-8"))
    if parent.get("receipt_hash") != V120_RESULT_RECEIPT_HASH:
        raise RuntimeError("frozen V120 receipt object hash changed")
    ...
    *,
    ...
    marker.update(
        {
            ...
            "v121_binary_source_commit": BINARY_SOURCE_COMMIT,
    ...
    if arm is not None:
        if frozen_model_config is None:
            raise RuntimeError("V121 arm rewrite lacks frozen model binding")
        arm_id, role, profile, expected = arm
        candidate = role == "candidate"
        marker.update(
            {
                "v121_arm_id": arm_id,
                "v121_experiment_id": "E3",
                "v121_role": role,
                "v121_profile": profile,
                "v121_shock_threshold_numerator": 3 if candidate else None,
                "v121_shock_threshold_denominator": 2 if candidate else None,
                "v121_shock_activation_horizon_frames": 50 if candidate else None,
                "v121_critical_service_ratio_numerator": 9 if candidate else None,
                "v121_critical_service_ratio_denominator": 10 if candidate else None,
                ...
                "v121_completed_changed_player_margin_numerator": 9
                if candidate
                else None,
                "v121_completed_changed_player_margin_denominator": 10
                if candidate
                else None,
                "v121_noncritical_exact_anchor": False,
                "v121_expected_run_count": expected,
            ...
            metadata.update(
                {
                    "v121_arm_id": arm_id,
                    "v121_arm_role": role,
                    "v121_candidate_profile": profile,
                    "v121_candidate_experiment": "E3",
                    "v121_shock_rate_ratio": "3/2" if candidate else None,
                    "v121_shock_threshold_numerator": 3 if candidate else None,
                    "v121_shock_threshold_denominator": 2 if candidate else None,
                    ...
                    "v121_faasrank_model_artifact_sha256": MODEL_SHA256,
                    "v121_critical_service_ratio_numerator": 9 if candidate else None,
                    "v121_critical_service_ratio_denominator": 10
                    if candidate
                    else None,
                    ...
                    "v121_completed_changed_player_margin_enabled": candidate,
                    "v121_completed_changed_player_margin_numerator": (
                        9 if candidate else None
                    ),
                    "v121_completed_changed_player_margin_denominator": (
...
def prepare_v121(root: Path = ROOT) -> dict:
    _assert_frozen_inputs()
    if root.exists():
        raise RuntimeError(f"refusing to overwrite V121 training root: {root}")
    ...
    capture = derive_integration_smoke_shard(
        paths["source"], capture_ids, purpose=f"V121 fresh parent tapes {PLAN_SHA256}"
    )
    _write(
        paths["capture"],
        _rewrite(capture, purpose=f"V121 fresh parent tapes {PLAN_SHA256}"),
    )
    ...
    for arm in ARMS:
        ...
        template = derive_integration_smoke_shard(
            paths["source"], selected, purpose=f"V121 {arm_id} {PLAN_SHA256}"
        )
        manifest = _rewrite(
            template,
            purpose=f"V121 {arm_id} {PLAN_SHA256}",
            arm=arm,
        ...
        if (
            ...
            raise RuntimeError(f"V121 {arm_id} must have 9 runs/references")
        if {run["seed"] for run in manifest["runs"]} != TRAINING_SEEDS:
            raise RuntimeError(f"V121 {arm_id} escaped E1126-E1128")
        keys = {item["key"] for item in manifest["reference_build_dependencies"]}
        if all_reference_keys & keys:
            raise RuntimeError("V121 arm-specific reference keys unexpectedly overlap")
        all_reference_keys.update(keys)
        _write(arm_path(root, arm_id), manifest)
    if len(all_reference_keys) != 18:
        raise RuntimeError("V121 must declare exactly 18 arm-specific references")
    ...
    receipt = {
        "schema_version": "NSE_E3_FINAL_MARGIN_PREPARED_V121_V1",
        "created_at": utc_now(),
        "formal_results_eligible": False,
        ...
        "V120_result_receipt_path": str(V120_RESULT_RECEIPT),
        "V120_result_receipt_sha256": V120_RESULT_RECEIPT_SHA256,
        "V120_result_receipt_hash": V120_RESULT_RECEIPT_HASH,
        "V120_result_receipt_commit": V120_RESULT_RECEIPT_COMMIT,
        ...
        "arms": [
            {
                ...
                "completed_changed_player_margin_enabled": role == "candidate",
                "completed_changed_player_margin": (
                    "9/10" if role == "candidate" else None
                ),
