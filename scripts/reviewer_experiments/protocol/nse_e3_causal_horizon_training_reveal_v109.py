...
from scripts.reviewer_experiments.protocol.nse_e3_causal_horizon_training_blind_audit_v109 import (
    ARMS,
...
OUTPUT = ROOT / "training-result-v109.json"
BLIND_AUDIT_FILE_SHA256 = "PENDING_FROZEN_V109_BLIND_AUDIT_FILE_SHA256"
BLIND_AUDIT_HASH = "PENDING_FROZEN_V109_BLIND_AUDIT_HASH"
EXPECTED_SEEDS = ("E896", "E897", "E898")
EXPECTED_SCENARIOS = (
    "E3.spike5x50ms",
    "E3.sustained3x200ms",
    "E3.pulse4x4x50ms",
...
ANCHORS = {"E3": "v109-e3-anchor"}
HORIZON_PREFERENCE = {
    "v109-e3-causal-shock15-horizon25-critical-service10-initializer-only": 0,
    "v109-e3-causal-shock15-horizon50-critical-service10-initializer-only": 1,
}
...
def evaluate_training_rows(
    ...
    for arm_id, arm in ARMS.items():
        ...
        dimension_scores["E3"][arm_id] = {
            ...
            "activation_horizon_frames": arm["shock_activation_horizon_frames"],
            "maximum_candidate_to_anchor_service_proxy_ratio": (
                f"{arm['critical_service_threshold_numerator']}/"
                f"{arm['critical_service_threshold_denominator']}"
            ),
            "anchor_arm_id": anchor_id,
...
def _validate_blind_audit() -> dict[str, Any]:
    if not BLIND_AUDIT.is_file() or file_hash(BLIND_AUDIT) != BLIND_AUDIT_FILE_SHA256:
        raise RuntimeError("V109 blind audit is missing or changed")
    ...
    if (
        ...
        raise RuntimeError("V109 blind audit does not authorize reveal")
    return blind
...
def _load_rows(blind: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    if len(admitted) != 27:
        raise RuntimeError("V109 blind audit run evidence is incomplete")
    ...
    for arm_id, expected in ARMS.items():
        ...
        if not manifest_path.is_file() or file_hash(manifest_path) != frozen.get(
            ...
            raise RuntimeError(f"V109 frozen manifest changed: {arm_id}")
        manifest = load_and_validate_manifest(manifest_path)
        if manifest.get("manifest_hash") != frozen.get("manifest_hash"):
            raise RuntimeError(f"V109 manifest hash changed: {arm_id}")
        for run in manifest["runs"]:
            ...
            if (
                ...
                != expected["profile"]
                or metadata.get("v109_training_plan_sha256") != PLAN_SHA256
                or metadata.get("v109_training_only") is not True
                or metadata.get("v109_training_seed_metrics_previously_revealed")
                is not False
                or metadata.get("v109_confirmation_seeds_opened") is not False
                or metadata.get("v109_other_unopened_seeds_opened") is not False
                or metadata.get("v109_arm_id") != arm_id
                or metadata.get("v109_arm_role") != expected["role"]
                or metadata.get("v109_shock_rate_ratio") != expected["shock_rate_ratio"]
                or metadata.get("v109_shock_threshold_numerator")
                != expected["shock_threshold_numerator"]
                or metadata.get("v109_shock_threshold_denominator")
                != expected["shock_threshold_denominator"]
                or metadata.get("v109_arrival_history_baseline_frames")
                != expected["arrival_history_baseline_frames"]
                or metadata.get("v109_arrival_history_recent_frames")
                != expected["arrival_history_recent_frames"]
                or metadata.get("v109_arrival_min_requests_per_window")
                != expected["arrival_min_requests_per_window"]
                or metadata.get("v109_shock_activation_horizon_frames")
                ...
                or metadata.get("v109_nonterminal_queue_density_floor")
                != expected["nonterminal_queue_density_floor"]
                or metadata.get("v109_warm_admissibility")
                != expected["warm_admissibility"]
                or metadata.get("v109_load_least_window_certificate_mode")
                != expected["load_least_window_certificate_mode"]
                or metadata.get("v109_arrival_signal") != expected["arrival_signal"]
                or metadata.get("v109_cpu_memory_individual_noninferiority")
                is not expected["cpu_memory_individual_noninferiority"]
                or metadata.get("v109_resource_bottleneck_sum_noninferiority")
                is not expected["resource_bottleneck_sum_noninferiority"]
                or metadata.get("v109_resource_inputs_finite_fail_closed")
                is not (expected["role"] == "candidate")
                or metadata.get("v109_critical_service_ratio_numerator")
                != expected["critical_service_threshold_numerator"]
                or metadata.get("v109_critical_service_ratio_denominator")
                != expected["critical_service_threshold_denominator"]
                or metadata.get("v109_critical_service_proxy")
                != (
                    "remote_parent_transfer_plus_cold_start_plus_projected_cpu_service"
                    if expected["role"] == "candidate"
                    else "not_applicable"
                )
                or metadata.get("v109_critical_service_proxy_inputs_finite_fail_closed")
                is not (expected["role"] == "candidate")
                or metadata.get("v109_componentwise_faasrank_noninferiority")
                is not False
                or metadata.get(
                ...
                is not False
                or metadata.get("v109_future_child_placement_or_feasibility_used")
                is not False
                or metadata.get("v109_critical_frontier_substitution")
                is not (expected["role"] == "candidate")
                or metadata.get("v109_critical_frontier_rank_source")
                != (
                    "immutable_srpt_remaining_critical_path_rank"
                    if expected["role"] == "candidate"
                    else "not_applicable"
                )
                or metadata.get("v109_critical_frontier_tie_rule")
                != (
                    "every_exact_tied_current_request_frontier_maximum"
                    if expected["role"] == "candidate"
                    else "not_applicable"
                )
                or metadata.get("v109_noncritical_exact_anchor")
                is not (expected["role"] == "candidate")
                or metadata.get(
                    "v109_complete_summed_critical_service_proxy_strictly_lower"
                )
                is not (expected["role"] == "candidate")
                or metadata.get("v109_complete_routed_score_nonworse")
                is not (expected["role"] == "candidate")
                or metadata.get("v109_complete_exact_ocs_score_nonworse")
                is not (expected["role"] == "candidate")
                or metadata.get("v109_complete_immutable_baseline_welfare_nonworse")
                is not (expected["role"] == "candidate")
                or metadata.get("v109_load_least_complete_score_used_as_gate")
                is not False
                or metadata.get("v109_outcome_fields_drive_policy") is not False
                or metadata.get("v109_scenario_or_burst_label_used_by_policy")
                is not False
                or metadata.get("v109_completion_or_performance_fields_used_by_policy")
                is not False
                or metadata.get("v109_future_arrivals_used_by_policy") is not False
            ):
                raise RuntimeError(f"V109 arm identity changed: {run['run_id']}")
            run_id = run["run_id"]
            summary_path = (
                ROOT
                / "runs"
            ...
            if not summary_path.is_file() or file_hash(summary_path) != admitted.get(
                ...
                raise RuntimeError(f"V109 summary differs from blind audit: {run_id}")
            ...
            if (
                ...
                raise RuntimeError(
                    f"V109 summary identity/completion mismatch: {run_id}"
                )
            rows.append(
                {
                    "arm_id": arm_id,
                    ...
                    "critical_service_threshold_numerator": expected[
                        "critical_service_threshold_numerator"
                    ],
                    "critical_service_threshold_denominator": expected[
...
def execute_reveal() -> dict[str, Any]:
    if OUTPUT.exists():
        raise RuntimeError(f"V109 training result already exists: {OUTPUT}")
    if not PLAN.is_file() or file_hash(PLAN) != PLAN_SHA256:
        raise RuntimeError("frozen V109 plan changed before reveal")
    blind = _validate_blind_audit()
    rows = _load_rows(blind)
    if len(rows) != 27:
        raise RuntimeError("V109 revealed row count differs from sealed plan")
    ...
    result = {
        "schema_version": "NSE_E3_CAUSAL_HORIZON_RESULT_V109_V1",
        "created_at": utc_now(),
        ...
        "selection_rule": {
            ...
            "multiple_passers": (
                ...
                "throughput change, then prefer the shorter 25-frame causal "
                "activation horizon, then arm_id"
            ),
            "frozen_E4": (
                "retain V97 v97-e4-load-guard64 without V109 result-dependent reselection"
            ),
        ...
        **evaluation,
        "revealed_rows": rows,
        "decision": {
            "authorize_preregistration_of_E1066_E1085_confirmation": joint_pass,
            ...
            "next_action": (
                ...
                "separately preregister the complete E1066-E1085 NSESche confirmation "
                "before generating inputs"
                if joint_pass
                else (
                    "freeze the complete V109 training cohort, keep E926-E1065 and "
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Reveal the frozen V109 training cohort exactly once after blind audit."
    )
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    if args.command != "reveal" or not args.execute:
        parser.error("V109 reveal requires the explicit command: reveal --execute")
    execute_reveal()
