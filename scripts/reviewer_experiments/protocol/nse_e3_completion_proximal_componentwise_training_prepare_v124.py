...
ROOT = Path("tmp/nse_e3_completion_proximal_componentwise_training_20260829_v124")
PLAN = Path(
    "scripts/reviewer_experiments/protocol/"
    "nse_e3_completion_proximal_componentwise_training_plan_v124.json"
)
PLAN_SHA256 = "9c17ac207539dda71393bf576fc825a6096e6a8a1d8ff9edb687ef1d3016ccb0"
...
BINARY_PATH = Path("tmp/nse_v124_build_c448969/release/serverless_sim.exe")
BINARY_SHA256 = "bc4e61b7d3b513b772d0af46828f835b7f3a4ff439320da14db407b4e3ae0807"
BINARY_SOURCE_COMMIT = "c4489691293507fc13d9db7084e179c7d78fd4b1"

SOURCE_INITIAL_SEEDS = [f"E{index}" for index in range(1195, 1205)]
SOURCE_CI_SEEDS = [f"E{index}" for index in range(1205, 1215)]
TRAINING_SEED_LIST = ["E1195", "E1196", "E1197"]
TRAINING_SEEDS = set(TRAINING_SEED_LIST)
CONFIRMATION_SEEDS = [f"E{index}" for index in range(1198, 1218)]
PREVIOUS_CONFIRMATION_SEEDS = [f"E{index}" for index in range(932, 1195)]
...
E3_ADMITTED_WORK = (
    ...
    "causal_arrival_shock15_horizon50_admitted_work10_completion_proximal_depth1_componentwise_service_pareto_initializer_only_"
    "guard64_dual_window_safe_pareto"
)
ARMS = (
    ("v124-e3-anchor", "anchor", E3_ANCHOR, 9),
    (
        "v124-e3-causal-shock15-horizon50-admitted-work10-completion-proximal-depth1-componentwise-service-pareto-initializer-only",
        "candidate",
        E3_ADMITTED_WORK,
        9,
    ),
...
def _paths(root: Path) -> dict[str, Path]:
    return {
        "config": root / "v124-e3-training-config.json",
        "source": root / "manifest.v124-e1195-e1204-full.unbound.json",
        "capture": root / "manifest.v124-tape-capture.unbound.json",
...
def _assert_frozen_inputs() -> None:
    for path, expected in (
        ...
        if not path.is_file() or file_hash(path) != expected:
            raise RuntimeError(f"frozen V124 input is missing or changed: {path}")
    ...
    *,
    ...
    marker.update(
        {
            ...
            "v124_binary_source_commit": BINARY_SOURCE_COMMIT,
    ...
    if arm is not None:
        if frozen_model_config is None:
            raise RuntimeError("V124 arm rewrite lacks frozen model binding")
        arm_id, role, profile, expected = arm
        candidate = role == "candidate"
        marker.update(
            {
                "v124_arm_id": arm_id,
                "v124_experiment_id": "E3",
                "v124_role": role,
                "v124_profile": profile,
                "v124_shock_threshold_numerator": 3 if candidate else None,
                "v124_shock_threshold_denominator": 2 if candidate else None,
                "v124_shock_activation_horizon_frames": 50 if candidate else None,
                "v124_critical_service_ratio_numerator": 9 if candidate else None,
                "v124_critical_service_ratio_denominator": 10 if candidate else None,
                ...
                "v124_expected_run_count": expected,
            ...
            metadata.update(
                {
                    "v124_arm_id": arm_id,
                    "v124_arm_role": role,
                    "v124_candidate_profile": profile,
                    "v124_candidate_experiment": "E3",
                    "v124_shock_rate_ratio": "3/2" if candidate else None,
                    "v124_shock_threshold_numerator": 3 if candidate else None,
                    "v124_shock_threshold_denominator": 2 if candidate else None,
                    ...
                    "v124_faasrank_model_artifact_sha256": MODEL_SHA256,
                    "v124_critical_service_ratio_numerator": 9 if candidate else None,
                    "v124_critical_service_ratio_denominator": 10
                    if candidate
                    else None,
...
def prepare_v124(root: Path = ROOT) -> dict:
    _assert_frozen_inputs()
    if root.exists():
        raise RuntimeError(f"refusing to overwrite V124 training root: {root}")
    ...
    capture = derive_integration_smoke_shard(
        paths["source"], capture_ids, purpose=f"V124 fresh parent tapes {PLAN_SHA256}"
    )
    _write(
        paths["capture"],
        _rewrite(capture, purpose=f"V124 fresh parent tapes {PLAN_SHA256}"),
    )
    ...
    for arm in ARMS:
        ...
        template = derive_integration_smoke_shard(
            paths["source"], selected, purpose=f"V124 {arm_id} {PLAN_SHA256}"
        )
        manifest = _rewrite(
            template,
            purpose=f"V124 {arm_id} {PLAN_SHA256}",
            arm=arm,
        ...
        if (
            ...
            raise RuntimeError(f"V124 {arm_id} must have 9 runs/references")
        if {run["seed"] for run in manifest["runs"]} != TRAINING_SEEDS:
            raise RuntimeError(f"V124 {arm_id} escaped E1195-E1197")
        keys = {item["key"] for item in manifest["reference_build_dependencies"]}
        if all_reference_keys & keys:
            raise RuntimeError("V124 arm-specific reference keys unexpectedly overlap")
        all_reference_keys.update(keys)
        _write(arm_path(root, arm_id), manifest)
    if len(all_reference_keys) != 18:
        raise RuntimeError("V124 must declare exactly 18 arm-specific references")
    ...
    receipt = {
        "schema_version": "NSE_E3_COMPLETION_PROXIMAL_COMPONENTWISE_PREPARED_V124_V1",
        "created_at": utc_now(),
        "formal_results_eligible": False,
