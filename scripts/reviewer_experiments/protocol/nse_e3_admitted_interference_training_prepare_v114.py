...
ROOT = Path("tmp/nse_e3_admitted_interference_training_20260829_v114")
PLAN = Path(
    "scripts/reviewer_experiments/protocol/"
    "nse_e3_admitted_interference_training_plan_v114.json"
)
PLAN_SHA256 = "2291806d0e54241a560ce8ba502372701765eb9729fe1a77f8182a2e1db79b21"
...
BINARY_PATH = Path("tmp/nse_v114_build_a318f41/release/serverless_sim.exe")
BINARY_SHA256 = "5acd4d379d5e9dc701396c847ff411e74f37b0b6f9aad432cbde36bd06071916"
BINARY_SOURCE_COMMIT = "a318f412cfe449e651386c004c3350e894a4b700"

SOURCE_INITIAL_SEEDS = [f"E{index}" for index in range(911, 921)]
SOURCE_CI_SEEDS = [f"E{index}" for index in range(921, 931)]
TRAINING_SEED_LIST = ["E911", "E912", "E913"]
TRAINING_SEEDS = set(TRAINING_SEED_LIST)
CONFIRMATION_SEEDS = [f"E{index}" for index in range(1086, 1106)]
PREVIOUS_CONFIRMATION_SEEDS = [f"E{index}" for index in range(926, 1086)]
OTHER_UNOPENED_SEEDS = [
    ...
    *[f"E{index}" for index in range(914, 926)],
]
PORT = "3147"

E3_ANCHOR = "faasrank_native_faithful_terminal_ocs_srpt_ready_dual_window_safe_pareto"
E3_ADMITTED_INTERFERENCE = (
    "faasrank_native_faithful_terminal_ocs_srpt_ready_"
    "causal_arrival_shock15_horizon50_admitted_work10_interference_pareto_"
    "initializer_only_guard64_dual_window_safe_pareto"
)
ARMS = (
    ("v114-e3-anchor", "anchor", E3_ANCHOR, 9),
    (
        "v114-e3-causal-shock15-horizon50-admitted-work10-interference-pareto-initializer-only",
        "candidate",
        E3_ADMITTED_INTERFERENCE,
        9,
    ),
...
def _paths(root: Path) -> dict[str, Path]:
    return {
        "config": root / "v114-e3-training-config.json",
        "source": root / "manifest.v114-e911-e920-full.unbound.json",
        "capture": root / "manifest.v114-tape-capture.unbound.json",
...
def _assert_frozen_inputs() -> None:
    for path, expected in (
        ...
        if not path.is_file() or file_hash(path) != expected:
            raise RuntimeError(f"frozen V114 input is missing or changed: {path}")
    ...
    *,
    ...
    marker.update(
        {
            ...
            "v114_binary_source_commit": BINARY_SOURCE_COMMIT,
    ...
    if arm is not None:
        if frozen_model_config is None:
            raise RuntimeError("V114 arm rewrite lacks frozen model binding")
        arm_id, role, profile, expected = arm
        candidate = role == "candidate"
        marker.update(
            {
                "v114_arm_id": arm_id,
                "v114_experiment_id": "E3",
                "v114_role": role,
                "v114_profile": profile,
                "v114_shock_threshold_numerator": 3 if candidate else None,
                "v114_shock_threshold_denominator": 2 if candidate else None,
                "v114_shock_activation_horizon_frames": 50 if candidate else None,
                "v114_critical_service_ratio_numerator": 9 if candidate else None,
                "v114_critical_service_ratio_denominator": 10 if candidate else None,
                ...
                "v114_expected_run_count": expected,
            ...
            metadata.update(
                {
                    "v114_arm_id": arm_id,
                    "v114_arm_role": role,
                    "v114_candidate_profile": profile,
                    "v114_candidate_experiment": "E3",
                    "v114_shock_rate_ratio": "3/2" if candidate else None,
                    "v114_shock_threshold_numerator": 3 if candidate else None,
                    "v114_shock_threshold_denominator": 2 if candidate else None,
                    ...
                    "v114_faasrank_model_artifact_sha256": MODEL_SHA256,
                    "v114_critical_service_ratio_numerator": 9 if candidate else None,
                    "v114_critical_service_ratio_denominator": 10
                    if candidate
                    else None,
...
def prepare_v114(root: Path = ROOT) -> dict:
    _assert_frozen_inputs()
    if root.exists():
        raise RuntimeError(f"refusing to overwrite V114 training root: {root}")
    ...
    capture = derive_integration_smoke_shard(
        paths["source"], capture_ids, purpose=f"V114 fresh parent tapes {PLAN_SHA256}"
    )
    _write(
        paths["capture"],
        _rewrite(capture, purpose=f"V114 fresh parent tapes {PLAN_SHA256}"),
    )
    ...
    for arm in ARMS:
        ...
        template = derive_integration_smoke_shard(
            paths["source"], selected, purpose=f"V114 {arm_id} {PLAN_SHA256}"
        )
        manifest = _rewrite(
            template,
            purpose=f"V114 {arm_id} {PLAN_SHA256}",
            arm=arm,
        ...
        if (
            ...
            raise RuntimeError(f"V114 {arm_id} must have 9 runs/references")
        if {run["seed"] for run in manifest["runs"]} != TRAINING_SEEDS:
            raise RuntimeError(f"V114 {arm_id} escaped E911-E913")
        keys = {item["key"] for item in manifest["reference_build_dependencies"]}
        if all_reference_keys & keys:
            raise RuntimeError("V114 arm-specific reference keys unexpectedly overlap")
        all_reference_keys.update(keys)
        _write(arm_path(root, arm_id), manifest)
    if len(all_reference_keys) != 18:
        raise RuntimeError("V114 must declare exactly 18 arm-specific references")
    ...
    receipt = {
        "schema_version": "NSE_E3_ADMITTED_INTERFERENCE_PREPARED_V114_V1",
        "created_at": utc_now(),
        "formal_results_eligible": False,
