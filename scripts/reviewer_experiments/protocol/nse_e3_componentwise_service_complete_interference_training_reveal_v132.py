...
from scripts.reviewer_experiments.protocol.nse_e3_componentwise_service_complete_interference_training_blind_audit_v132 import (
    ARMS,
...
OUTPUT = ROOT / "training-result-v132.json"
BLIND_AUDIT_FILE_SHA256 = (
    "e6ed1e3ac069f9090f5b987fac63ed42ed9f7b9c6ac9e0f4b9b034a23fbf48dd"
)
BLIND_AUDIT_HASH = "3ed824e37a5f61a7b5d44e4d2ad244dd25cc354e3862f0faa0bd9076f3c9511f"
EXPECTED_SEEDS = ("E1379", "E1380", "E1381")
EXPECTED_SCENARIOS = (
    "E3.spike5x50ms",
    "E3.sustained3x200ms",
    "E3.pulse4x4x50ms",
...
ANCHOR_ID = "v132-e3-anchor"
CANDIDATE_ID = (
    "v132-e3-causal-shock15-horizon50-admitted-work10-"
    "componentwise-service-complete-interference-initializer-only"
)
...
def _validate_blind_audit() -> dict[str, Any]:
    ...
    if any(
        ...
        raise RuntimeError("V132 blind audit has not been frozen into the reveal code")
    if not BLIND_AUDIT.is_file() or file_hash(BLIND_AUDIT) != BLIND_AUDIT_FILE_SHA256:
        raise RuntimeError("V132 blind audit is missing or changed")
    ...
    if (
        ...
        raise RuntimeError("V132 blind audit does not authorize reveal")
    return blind
...
def _load_rows(blind: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    if len(admitted) != 18:
        raise RuntimeError("V132 blind audit run evidence is incomplete")
    ...
    for arm_id, expected in ARMS.items():
        ...
        if not manifest_path.is_file() or file_hash(manifest_path) != frozen.get(
            ...
            raise RuntimeError(f"V132 frozen manifest changed: {arm_id}")
        manifest = load_and_validate_manifest(manifest_path)
        if manifest.get("manifest_hash") != frozen.get("manifest_hash"):
            raise RuntimeError(f"V132 manifest hash changed: {arm_id}")
        for run in manifest["runs"]:
            ...
            if (
                ...
                != expected["profile"]
                or metadata.get("v132_training_plan_sha256") != PLAN_SHA256
                or metadata.get("v132_training_only") is not True
                or metadata.get("v132_training_seed_metrics_previously_revealed")
                is not False
                or metadata.get("v132_confirmation_seeds_opened") is not False
                or metadata.get("v132_other_unopened_seeds_opened") is not False
                or metadata.get("v132_arm_id") != arm_id
                or metadata.get("v132_arm_role") != expected["role"]
                or metadata.get("v132_shock_rate_ratio") != expected["shock_rate_ratio"]
                or metadata.get("v132_shock_threshold_numerator")
                != expected["shock_threshold_numerator"]
                or metadata.get("v132_shock_threshold_denominator")
                != expected["shock_threshold_denominator"]
                or metadata.get("v132_shock_activation_horizon_frames")
                != expected["shock_activation_horizon_frames"]
                or metadata.get("v132_critical_service_ratio_numerator")
                != expected["critical_service_threshold_numerator"]
                or metadata.get("v132_critical_service_ratio_denominator")
                != expected["critical_service_threshold_denominator"]
                or metadata.get("v132_service_proxy_work_source")
                != (
                    "admitted_pending_plus_all_resident_remaining_and_same_window_projected_cpu_v1"
                    if candidate
                    else "not_applicable"
                )
                or metadata.get("v132_admitted_work_includes_all_blocked_resident")
                is not candidate
                or metadata.get("v132_complete_componentwise_service_pareto")
                is not candidate
                or metadata.get("v132_complete_admitted_interference_pareto")
                is not candidate
                or metadata.get("v132_componentwise_admitted_interference_pareto")
                is not False
                or metadata.get("v132_complete_interference_comparison")
                != (
                    "alternative_complete_interference_lte_anchor_plus_epsilon"
                    if candidate
                    else "not_applicable"
                )
                or metadata.get("v132_complete_interference_inputs_finite_fail_closed")
                is not candidate
                or metadata.get("v132_outcome_fields_drive_policy") is not False
                or metadata.get("v132_scenario_or_burst_label_used_by_policy")
                is not False
                or metadata.get("v132_completion_or_performance_fields_used_by_policy")
                is not False
                or metadata.get("v132_future_arrivals_used_by_policy") is not False
            ):
                raise RuntimeError(f"V132 arm identity changed: {run['run_id']}")
            run_id = run["run_id"]
            summary_path = (
                ROOT
                / "runs"
            ...
            if not summary_path.is_file() or file_hash(summary_path) != admitted.get(
                ...
                raise RuntimeError(f"V132 summary differs from blind audit: {run_id}")
            ...
            if (
                ...
                raise RuntimeError(
                    f"V132 summary identity/completion mismatch: {run_id}"
                )
            rows.append(
                {
                    "arm_id": arm_id,
...
def execute_reveal() -> dict[str, Any]:
    if OUTPUT.exists():
        raise RuntimeError(f"V132 training result already exists: {OUTPUT}")
    if not PLAN.is_file() or file_hash(PLAN) != PLAN_SHA256:
        raise RuntimeError("frozen V132 plan changed before reveal")
    blind = _validate_blind_audit()
    rows = _load_rows(blind)
    if len(rows) != 18:
        raise RuntimeError("V132 revealed row count differs from sealed plan")
    ...
    result = {
        "schema_version": "NSE_E3_COMPONENTWISE_SERVICE_COMPLETE_INTERFERENCE_RESULT_V132_V1",
        "created_at": utc_now(),
        ...
        **evaluation,
        "revealed_rows": rows,
        "decision": {
            "authorize_preregistration_of_E1382_E1401_confirmation": joint_pass,
            ...
            "next_action": (
                "freeze V132 with frozen V97 E4 and separately preregister the "
                "complete E1382-E1401 NSESche confirmation before generating inputs"
                if joint_pass
                else (
                    "freeze the complete V132 training cohort, keep E1359-E1378 and "
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Reveal the frozen V132 training cohort exactly once after blind audit."
    )
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    if args.command != "reveal" or not args.execute:
        parser.error("V132 reveal requires the explicit command: reveal --execute")
    execute_reveal()
