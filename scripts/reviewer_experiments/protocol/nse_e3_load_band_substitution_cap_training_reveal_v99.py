...
OUTPUT = ROOT / "training-result-v99.json"
BLIND_AUDIT_FILE_SHA256 = (
    "6a4038ac5f83db4b9ebe6733b3b390be4ab9a7648b43304f33ae738a7a8109f6"
)
BLIND_AUDIT_HASH = "e92396145aa7f59f69ad0e388c622e0975dd9ef599a4db17d28139db9a85474c"
EXPECTED_SEEDS = ("E866", "E867", "E868")
EXPECTED_SCENARIOS = (
    "E3.spike5x50ms",
    "E3.sustained3x200ms",
    "E3.pulse4x4x50ms",
...
def evaluate_training_rows(
    ...
    for arm_id, arm in ARMS.items():
        ...
        dimension_scores[dimension][arm_id] = {
            "profile": arm["profile"],
            "substitution_cap": arm["substitution_cap"],
            "anchor_arm_id": anchor_id,
...
def _validate_blind_audit() -> dict[str, Any]:
    if not BLIND_AUDIT.is_file() or file_hash(BLIND_AUDIT) != BLIND_AUDIT_FILE_SHA256:
        raise RuntimeError("V99 blind audit is missing or changed")
    ...
    if (
        ...
        raise RuntimeError("V99 blind audit does not authorize reveal")
    return blind
...
def _load_rows(blind: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    if len(admitted) != 27:
        raise RuntimeError("V99 blind audit run evidence is incomplete")
    ...
    for arm_id, expected in ARMS.items():
        ...
        if not manifest_path.is_file() or file_hash(manifest_path) != frozen.get(
            ...
            raise RuntimeError(f"V99 frozen manifest changed: {arm_id}")
        manifest = load_and_validate_manifest(manifest_path)
        if manifest.get("manifest_hash") != frozen.get("manifest_hash"):
            raise RuntimeError(f"V99 manifest hash changed: {arm_id}")
        for run in manifest["runs"]:
            ...
            if (
                ...
                != expected["profile"]
                or metadata.get("v99_training_plan_sha256") != PLAN_SHA256
                or metadata.get("v99_training_only") is not True
                or metadata.get("v99_training_seed_metrics_previously_revealed")
                is not False
                or metadata.get("v99_confirmation_seeds_opened") is not False
                or metadata.get("v99_previously_reserved_seeds_opened") is not False
                or metadata.get("v99_arm_id") != arm_id
                or metadata.get("v99_arm_role") != expected["role"]
                or metadata.get("v99_upper_queue_density_threshold")
                != expected["upper_queue_density_threshold"]
                or metadata.get("v99_nonterminal_queue_density_floor")
                != expected["nonterminal_queue_density_floor"]
                or metadata.get("v99_substitution_cap") != expected["substitution_cap"]
            ):
                raise RuntimeError(f"V99 arm identity changed: {run['run_id']}")
            run_id = run["run_id"]
            summary_path = (
                ROOT
                / "runs"
            ...
            if not summary_path.is_file() or file_hash(summary_path) != admitted.get(
                ...
                raise RuntimeError(f"V99 summary differs from blind audit: {run_id}")
            ...
            if (
                ...
                raise RuntimeError(
                    f"V99 summary identity/completion mismatch: {run_id}"
                )
            rows.append(
                {
                    "arm_id": arm_id,
                    ...
                    "substitution_cap": expected["substitution_cap"],
...
def execute_reveal() -> dict[str, Any]:
    if OUTPUT.exists():
        raise RuntimeError(f"V99 training result already exists: {OUTPUT}")
    if not PLAN.is_file() or file_hash(PLAN) != PLAN_SHA256:
        raise RuntimeError("frozen V99 plan changed before reveal")
    blind = _validate_blind_audit()
    rows = _load_rows(blind)
    if len(rows) != 27:
        raise RuntimeError("V99 revealed row count differs from sealed plan")
    ...
    result = {
        "schema_version": "NSE_E3_LOAD_BAND_SUBSTITUTION_CAP_RESULT_V99_V1",
        "created_at": utc_now(),
        ...
        "selection_rule": {
            ...
            "frozen_E4": (
                "retain V97 v97-e4-load-guard64 without V99 result-dependent reselection"
            ),
        ...
        **evaluation,
        "revealed_rows": rows,
        "decision": {
            "authorize_preregistration_of_E886_E905_confirmation": joint_pass,
            ...
            "next_action": (
                ...
                "separately preregister the complete E886-E905 NSESche confirmation "
                "before generating inputs"
                if joint_pass
                else (
                    "freeze the complete V99 training cohort, keep E886-E905, "
                    "E806-E825, and E766-E785 untouched, and return to mechanism "
                    "diagnosis"
                )
            ),
        },
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Reveal the frozen V99 training cohort exactly once after blind audit."
    )
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    if args.command != "reveal" or not args.execute:
        parser.error("V99 reveal requires the explicit command: reveal --execute")
    execute_reveal()
