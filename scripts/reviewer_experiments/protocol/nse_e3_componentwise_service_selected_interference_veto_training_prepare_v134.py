...
ROOT = Path(
    "tmp/nse_e3_componentwise_service_selected_interference_veto_training_20260830_v134"
)
PLAN = Path(
    "scripts/reviewer_experiments/protocol/"
    "nse_e3_componentwise_service_selected_interference_veto_training_plan_v134.json"
)
PLAN_SHA256 = "d4397bf24441f6f8570981b18806a63f851b0a576723b2c1ec77ab05d4f4af48"
V117_PLAN = Path(
    "scripts/reviewer_experiments/protocol/"
...
V133_PLAN_SHA256 = "6cee0e71f268d0ec947f257c5a79223d5662d5ed7e5dc3888c1d7aba69177283"
V133_BLIND_AUDIT = Path(
    "tmp/nse_e3_componentwise_service_componentwise_interference_training_20260830_v133/"
    "joint-blind-audit-v133-training.json"
)
V133_BLIND_AUDIT_SHA256 = (
    "b58e6a8b6424170baeb4acdbca4d6da55162756c00fc8f23405b0a7602acc6b8"
)
BINARY_PATH = Path("tmp/nse_v134_build_12a7648/release/serverless_sim.exe")
BINARY_SHA256 = "c771a542cce46697082b721d863db3e1c0553eec6a393ebe166b418c1eb31f9b"
BINARY_SOURCE_COMMIT = "12a76489f718b444b1858cb724b788dd857cce9c"

TRAINING_SEED_LIST = ["E1425", "E1426", "E1427"]
...
CONFIRMATION_SEEDS = [f"E{index}" for index in range(1428, 1448)]
PREVIOUS_CONFIRMATION_SEEDS = [
    *[f"E{index}" for index in range(1405, 1425)],
...
E3_COMPONENTWISE_SERVICE_SELECTED_INTERFERENCE_VETO = (
    "faasrank_native_faithful_terminal_ocs_srpt_ready_"
    "causal_arrival_shock15_horizon50_admitted_work10_"
    "componentwise_service_selected_interference_veto_pareto_initializer_only_guard64_dual_window_safe_pareto"
)
ARMS = (
    ("v134-e3-anchor", "anchor", E3_ANCHOR, 9),
    (
        "v134-e3-causal-shock15-horizon50-admitted-work10-componentwise-service-selected-interference-veto-initializer-only",
        "candidate",
        E3_COMPONENTWISE_SERVICE_SELECTED_INTERFERENCE_VETO,
        9,
    ),
...
def _paths(root: Path) -> dict[str, Path]:
    return {
        "config": root / "v134-e3-training-config.json",
        "source": root / "manifest.v134-training-source-full.unbound.json",
        "capture": root / "manifest.v134-tape-capture.unbound.json",
...
def _assert_frozen_inputs() -> None:
    for path, expected in (
        ...
        (V133_PLAN, V133_PLAN_SHA256),
        (V133_BLIND_AUDIT, V133_BLIND_AUDIT_SHA256),
        ...
        if not path.is_file() or file_hash(path) != expected:
            raise RuntimeError(f"frozen V134 input is missing or changed: {path}")
    ...
    *,
    ...
    marker.update(
        {
            ...
            "v134_binary_source_commit": BINARY_SOURCE_COMMIT,
    ...
    if arm is not None:
        if frozen_model_config is None:
            raise RuntimeError("V134 arm rewrite lacks frozen model binding")
        arm_id, role, profile, expected = arm
        candidate = role == "candidate"
        marker.update(
            {
                "v134_arm_id": arm_id,
                "v134_experiment_id": "E3",
                "v134_role": role,
                "v134_profile": profile,
                "v134_shock_threshold_numerator": 3 if candidate else None,
                "v134_shock_threshold_denominator": 2 if candidate else None,
                "v134_shock_activation_horizon_frames": 50 if candidate else None,
                "v134_critical_service_ratio_numerator": 9 if candidate else None,
                "v134_critical_service_ratio_denominator": 10 if candidate else None,
                ...
                "v134_expected_run_count": expected,
            ...
            metadata.update(
                {
                    "v134_arm_id": arm_id,
                    "v134_arm_role": role,
                    "v134_candidate_profile": profile,
                    "v134_candidate_experiment": "E3",
                    "v134_shock_rate_ratio": "3/2" if candidate else None,
                    "v134_shock_threshold_numerator": 3 if candidate else None,
                    "v134_shock_threshold_denominator": 2 if candidate else None,
                    ...
                    "v134_faasrank_model_artifact_sha256": MODEL_SHA256,
                    "v134_critical_service_ratio_numerator": 9 if candidate else None,
                    "v134_critical_service_ratio_denominator": 10
                    if candidate
                    else None,
...
def prepare_v134(root: Path = ROOT) -> dict:
    _assert_frozen_inputs()
    if root.exists():
        raise RuntimeError(f"refusing to overwrite V134 training root: {root}")
    ...
    capture = derive_integration_smoke_shard(
        paths["source"], capture_ids, purpose=f"V134 fresh parent tapes {PLAN_SHA256}"
    )
    _write(
        paths["capture"],
        _rewrite(capture, purpose=f"V134 fresh parent tapes {PLAN_SHA256}"),
    )
    ...
    for arm in ARMS:
        ...
        template = derive_integration_smoke_shard(
            paths["source"], selected, purpose=f"V134 {arm_id} {PLAN_SHA256}"
        )
        manifest = _rewrite(
            template,
            purpose=f"V134 {arm_id} {PLAN_SHA256}",
            arm=arm,
        ...
        if (
            ...
            raise RuntimeError(f"V134 {arm_id} must have 9 runs/references")
        if {run["seed"] for run in manifest["runs"]} != TRAINING_SEEDS:
            raise RuntimeError(f"V134 {arm_id} escaped E1425-E1427")
        keys = {item["key"] for item in manifest["reference_build_dependencies"]}
        if all_reference_keys & keys:
            raise RuntimeError("V134 arm-specific reference keys unexpectedly overlap")
        all_reference_keys.update(keys)
        _write(arm_path(root, arm_id), manifest)
    if len(all_reference_keys) != 18:
        raise RuntimeError("V134 must declare exactly 18 arm-specific references")
    ...
    receipt = {
        "schema_version": "NSE_E3_COMPONENTWISE_SERVICE_SELECTED_INTERFERENCE_VETO_PREPARED_V134_V1",
        "created_at": utc_now(),
        "formal_results_eligible": False,
        ...
        "V133_plan_path": str(V133_PLAN),
        "V133_plan_sha256": V133_PLAN_SHA256,
        "V133_blind_audit_path": str(V133_BLIND_AUDIT),
        "V133_blind_audit_sha256": V133_BLIND_AUDIT_SHA256,
        "V133_performance_result_reopened": False,
        ...
        "arms": [
            {
                ...
                "selected_choice_interference_veto_rule": (
                    "first_otherwise_accepted_player_interference_lte_anchor_plus_epsilon"
                    if role == "candidate"
                    else "not_applicable"
                ),
                "componentwise_enumeration_interference": False,
