...
PLAN_SHA256 = "5fc758ad2ce1a90aa26884d94d5338d4739277c9d75f12c0f37eaf07788747ab"
V95_RESULT_RECEIPT = Path(
    "scripts/reviewer_experiments/protocol/"
    "nse_e3e4_qpr_recovery_training_result_v95.json"
)
V95_RESULT_RECEIPT_SHA256 = (
    "eb16602215d385fb7dbd74bbff94c46c3919807ac98cd8c769bf616a0438fb1b"
)
V95_SOURCE_RESULT = Path(
    "tmp/nse_e3e4_qpr_recovery_training_20260828_v95/training-result-v95.json"
)
V95_SOURCE_RESULT_SHA256 = (
    "d5cef8b427838966a4b5fb406a0c33b4250432db6afb2ff7ecb6c12904615097"
)
FORMAL_RESULT = Path(
...
MECHANISM_INPUTS = (
    (
        Path(
            "scripts/reviewer_experiments/protocol/"
            "nse_e3e4_formal_initial_v94_plan.json"
        ),
        "14982f64b9725ca6dca767284d9620db79b839592cdb07c04a0c3fd515bae243",
    ),
    (
        Path(
            "scripts/reviewer_experiments/protocol/"
            "nse_idle_warm_dominance_pareto_implementation_v85.json"
        ),
        "ca9f18960bf8c097d0215845acc8eb95250506558a2bec320c0f14c64d83f9bc",
    ),
)

SOURCE_INITIAL_SEEDS = [f"E{index}" for index in range(746, 756)]
SOURCE_CI_SEEDS = [f"E{index}" for index in range(756, 766)]
TRAINING_SEED_LIST = ["E746", "E747", "E748"]
TRAINING_SEEDS = set(TRAINING_SEED_LIST)
CONFIRMATION_SEEDS = [f"E{index}" for index in range(766, 786)]
PORT = "3131"

E3_ANCHOR = (
    "faasrank_native_faithful_terminal_ocs_" "srpt_ready_dual_window_safe_pareto"
)
IDLE_WARM_SRPT = (
...
ARMS = (
    ("v96-e3-anchor", "E3", "anchor", E3_ANCHOR, 9),
    ("v96-e3-idle-warm-srpt", "E3", "candidate", IDLE_WARM_SRPT, 9),
    (
    ...
    ("v96-e4-anchor", "E4", "anchor", IDLE_WARM_SRPT, 3),
    (
...
def _paths(root: Path) -> dict[str, Path]:
    return {
        "config": root / "v96-e3e4-training-config.json",
        "source": root / "manifest.v96-e746-e755-full.unbound.json",
        "capture": root / "manifest.v96-tape-capture.unbound.json",
...
def _assert_frozen_inputs() -> None:
    inputs = (
        (PLAN, PLAN_SHA256),
        (V95_RESULT_RECEIPT, V95_RESULT_RECEIPT_SHA256),
        (V95_SOURCE_RESULT, V95_SOURCE_RESULT_SHA256),
    ...
    for path, expected in inputs:
        if not path.is_file() or file_hash(path) != expected:
            raise RuntimeError(f"frozen V96 input is missing or changed: {path}")
...
def _rewrite(
    ...
    *,
    purpose: str,
    arm: tuple[str, str, str, str, int] | None = None,
    ...
    marker.update(
        {
            "purpose": purpose,
            "v96_training_plan_sha256": PLAN_SHA256,
            "v96_training_only": True,
            "v96_binary_source_commit": BINARY_SOURCE_COMMIT,
    ...
    if arm is not None:
        ...
        arm_id, experiment_id, role, profile, expected = arm
        marker.update(
            {
                "v96_arm_id": arm_id,
                "v96_experiment_id": experiment_id,
                "v96_role": role,
                "v96_profile": profile,
                "v96_expected_run_count": expected,
            }
        )
        ...
        metadata.update(
            {
                "v96_training_plan_sha256": PLAN_SHA256,
                "v96_training_only": True,
                "v96_training_seed_metrics_previously_revealed": False,
                "v96_binary_source_commit": BINARY_SOURCE_COMMIT,
                "v96_confirmation_seeds_opened": False,
                "v96_formal_E01_E20_reexecution": False,
            }
        )
        if arm is not None:
            arm_id, experiment_id, role, profile, _ = arm
            ...
            metadata.update(
                {
                    "v96_arm_id": arm_id,
                    "v96_arm_role": role,
                    "v96_candidate_profile": profile,
                    "v96_candidate_experiment": experiment_id,
                    "v96_faasrank_model_artifact_sha256": MODEL_SHA256,
                    "v96_native_faithful_initializer": True,
                    "v96_dual_window_safe_pareto": True,
                    "v96_idle_warm_dominance_router": (
                        "idle_warm_dominance" in profile
                    ),
                    "v96_srpt_critical_path_player_order": "srpt_ready" in profile,
                }
            )
...
def prepare_v96(root: Path = ROOT) -> dict:
    _assert_frozen_inputs()
    if root.exists():
        raise RuntimeError(f"refusing to overwrite V96 training root: {root}")
    ...
    capture = derive_integration_smoke_shard(
        paths["source"],
        capture_ids,
        purpose=f"V96 fresh balanced-QoS parent tapes; plan_sha256={PLAN_SHA256}",
    )
    _write(
        paths["capture"],
        _rewrite(
    ...
    for arm in ARMS:
        arm_id, experiment_id, _, _, expected = arm
        ...
        template = derive_integration_smoke_shard(
            paths["source"],
            selected,
            purpose=f"V96 {arm_id} guarded-anchor training; plan_sha256={PLAN_SHA256}",
        )
        manifest = _rewrite(
            template,
            purpose=f"V96 {arm_id} guarded-anchor training; plan_sha256={PLAN_SHA256}",
        ...
        if (
            ...
            raise RuntimeError(f"V96 {arm_id} must have {expected} runs/references")
        if {run["seed"] for run in manifest["runs"]} != TRAINING_SEEDS:
            raise RuntimeError(f"V96 {arm_id} escaped E746-E748")
        keys = {item["key"] for item in manifest["reference_build_dependencies"]}
        if all_reference_keys & keys:
            raise RuntimeError("V96 arm-specific reference keys unexpectedly overlap")
        all_reference_keys.update(keys)
        _write(arm_path(root, arm_id), manifest)
        arm_manifests[arm_id] = manifest
    if len(all_reference_keys) != 33:
        raise RuntimeError("V96 must declare exactly 33 arm-specific references")

    manifests: dict[str, dict] = {}
    manifest_paths = [paths["source"], paths["capture"]] + [
        arm_path(root, arm_id) for arm_id, _, _, _, _ in ARMS
    ]
    ...
    receipt = {
        "schema_version": "NSE_E3E4_GUARDED_ANCHOR_TRAINING_PREPARED_V96_V1",
        "created_at": utc_now(),
        "formal_results_eligible": False,
        ...
        "V95_result_receipt_path": str(V95_RESULT_RECEIPT),
        "V95_result_receipt_sha256": V95_RESULT_RECEIPT_SHA256,
        "V95_source_result_path": str(V95_SOURCE_RESULT),
        "V95_source_result_sha256": V95_SOURCE_RESULT_SHA256,
        ...
        "arms": [
            ...
            for arm_id, experiment_id, role, profile, expected in ARMS
        ],
        "base_tape_captures": 3,
        "derived_burst_tapes": 9,
        "arm_reference_builds": 33,
        "arm_online_runs": 33,
