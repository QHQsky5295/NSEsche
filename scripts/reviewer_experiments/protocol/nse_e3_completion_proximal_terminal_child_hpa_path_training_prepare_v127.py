...
PLAN = Path(
    ...
    "nse_e3_completion_proximal_terminal_child_hpa_path_training_plan_v127.json"
)
PLAN_SHA256 = "0bfee6a14bf490af0075f6819f5c1f0412a5cd0052daa84812021b73cac95564"
V124_RESULT = Path(
...
BINARY_PATH = Path("tmp/nse_v127_build_2d0a0ea/release/serverless_sim.exe")
BINARY_SHA256 = "6dbe79e08a13b179c9f811859c5d079bdf49daf9a92ad8f664410d91e61c3d93"
BINARY_SOURCE_COMMIT = "2d0a0eaa99ae86df094d14c327d4e1e56d4e3ed0"

SOURCE_INITIAL_SEEDS = [f"E{index}" for index in range(1264, 1274)]
SOURCE_CI_SEEDS = [f"E{index}" for index in range(1274, 1284)]
TRAINING_SEED_LIST = ["E1264", "E1265", "E1266"]
TRAINING_SEEDS = set(TRAINING_SEED_LIST)
CONFIRMATION_SEEDS = [f"E{index}" for index in range(1267, 1287)]
PREVIOUS_CONFIRMATION_SEEDS = [f"E{index}" for index in range(1244, 1264)]
...
E3_ADMITTED_WORK = (
    ...
    "causal_arrival_shock15_horizon50_admitted_work10_completion_proximal_depth1_componentwise_service_terminal_child_hpa_container_path_pareto_initializer_only_"
    "guard64_dual_window_safe_pareto"
)
ARMS = (
    ("v127-e3-anchor", "anchor", E3_ANCHOR, 9),
    (
        "v127-e3-causal-shock15-horizon50-admitted-work10-completion-proximal-depth1-componentwise-service-terminal-child-hpa-container-path-pareto-initializer-only",
        "candidate",
        E3_ADMITTED_WORK,
        9,
    ),
...
def _paths(root: Path) -> dict[str, Path]:
    return {
        "config": root / "v127-e3-training-config.json",
        "source": root / "manifest.v127-e1264-e1273-full.unbound.json",
        "capture": root / "manifest.v127-tape-capture.unbound.json",
...
def _assert_frozen_inputs() -> None:
    for path, expected in (
        ...
        if not path.is_file() or file_hash(path) != expected:
            raise RuntimeError(f"frozen V127 input is missing or changed: {path}")
    ...
    *,
    ...
    marker.update(
        {
            ...
            "v127_binary_source_commit": BINARY_SOURCE_COMMIT,
    ...
    if arm is not None:
        if frozen_model_config is None:
            raise RuntimeError("V127 arm rewrite lacks frozen model binding")
        arm_id, role, profile, expected = arm
        candidate = role == "candidate"
        marker.update(
            {
                "v127_arm_id": arm_id,
                "v127_experiment_id": "E3",
                "v127_role": role,
                "v127_profile": profile,
                "v127_shock_threshold_numerator": 3 if candidate else None,
                "v127_shock_threshold_denominator": 2 if candidate else None,
                "v127_shock_activation_horizon_frames": 50 if candidate else None,
                "v127_critical_service_ratio_numerator": 9 if candidate else None,
                "v127_critical_service_ratio_denominator": 10 if candidate else None,
                ...
                "v127_expected_run_count": expected,
            ...
            metadata.update(
                {
                    "v127_arm_id": arm_id,
                    "v127_arm_role": role,
                    "v127_candidate_profile": profile,
                    "v127_candidate_experiment": "E3",
                    "v127_shock_rate_ratio": "3/2" if candidate else None,
                    "v127_shock_threshold_numerator": 3 if candidate else None,
                    "v127_shock_threshold_denominator": 2 if candidate else None,
                    ...
                    "v127_faasrank_model_artifact_sha256": MODEL_SHA256,
                    "v127_critical_service_ratio_numerator": 9 if candidate else None,
                    "v127_critical_service_ratio_denominator": 10
                    if candidate
                    else None,
...
def prepare_v127(root: Path = ROOT) -> dict:
    _assert_frozen_inputs()
    if root.exists():
        raise RuntimeError(f"refusing to overwrite V127 training root: {root}")
    ...
    capture = derive_integration_smoke_shard(
        paths["source"], capture_ids, purpose=f"V127 fresh parent tapes {PLAN_SHA256}"
    )
    _write(
        paths["capture"],
        _rewrite(capture, purpose=f"V127 fresh parent tapes {PLAN_SHA256}"),
    )
    ...
    for arm in ARMS:
        ...
        template = derive_integration_smoke_shard(
            paths["source"], selected, purpose=f"V127 {arm_id} {PLAN_SHA256}"
        )
        manifest = _rewrite(
            template,
            purpose=f"V127 {arm_id} {PLAN_SHA256}",
            arm=arm,
        ...
        if (
            ...
            raise RuntimeError(f"V127 {arm_id} must have 9 runs/references")
        if {run["seed"] for run in manifest["runs"]} != TRAINING_SEEDS:
            raise RuntimeError(f"V127 {arm_id} escaped E1264-E1266")
        keys = {item["key"] for item in manifest["reference_build_dependencies"]}
        if all_reference_keys & keys:
            raise RuntimeError("V127 arm-specific reference keys unexpectedly overlap")
        all_reference_keys.update(keys)
        _write(arm_path(root, arm_id), manifest)
    if len(all_reference_keys) != 18:
        raise RuntimeError("V127 must declare exactly 18 arm-specific references")
    ...
    receipt = {
        "schema_version": "NSE_E3_COMPLETION_PROXIMAL_TERMINAL_CHILD_HPA_PATH_PREPARED_V127_V1",
        "created_at": utc_now(),
        "formal_results_eligible": False,
        ...
        "arms": [
            {
                ...
                "complete_terminal_child_hpa_container_path_service_pareto": role
                == "candidate",
                "terminal_child_hpa_container_path_scope": (
                    "every_immutable_immediate_child_of_each_actually_changed_completion_proximal_player"
                    if role == "candidate"
                    else "not_applicable"
                ),
