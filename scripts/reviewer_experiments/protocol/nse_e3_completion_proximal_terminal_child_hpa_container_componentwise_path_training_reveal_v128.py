...
OUTPUT = ROOT / "training-result-v128.json"
BLIND_AUDIT_FILE_SHA256 = (
    "3b6882047c69420874ca3d13697c5a8aae5e3a4e4741e6f4e44d793c980c45df"
)
BLIND_AUDIT_HASH = "211c76eb7b171b8eca345bd0a573fa4f9f0c2aaa1bbc18e3c33d5329af4c1ab3"
EXPECTED_SEEDS = ("E1287", "E1288", "E1289")
EXPECTED_SCENARIOS = (
    "E3.spike5x50ms",
    "E3.sustained3x200ms",
    "E3.pulse4x4x50ms",
...
ANCHOR_ID = "v128-e3-anchor"
CANDIDATE_ID = (
    "v128-e3-causal-shock15-horizon50-admitted-work10-"
    "completion-proximal-depth1-componentwise-service-terminal-child-hpa-container-componentwise-path-pareto-"
    "initializer-only"
)
...
def _validate_blind_audit() -> dict[str, Any]:
    ...
    if any(
        ...
        raise RuntimeError("V128 blind audit has not been frozen into the reveal code")
    if not BLIND_AUDIT.is_file() or file_hash(BLIND_AUDIT) != BLIND_AUDIT_FILE_SHA256:
        raise RuntimeError("V128 blind audit is missing or changed")
    ...
    if (
        ...
        or terminal_child_path.get(
            "terminal_child_container_componentwise_gate_evaluated_count", 0
        )
        <= 0
        or terminal_child_path.get(
            "terminal_child_container_componentwise_changed_parent_count", 0
        )
        <= 0
        or terminal_child_path.get(
            "terminal_child_container_componentwise_compared_child_count", 0
        )
        <= 0
        or terminal_child_path.get(
            "terminal_child_container_componentwise_compared_container_path_count", 0
        )
        <= 0
        or terminal_child_path.get(
            "terminal_child_container_componentwise_running_container_path_count", 0
        )
        <= 0
        or terminal_child_path.get(
            "terminal_child_container_componentwise_starting_container_path_count", 0
        )
        <= 0
        or terminal_child_path.get(
            "terminal_child_container_componentwise_acceptance_count", 0
        )
        <= 0
        or terminal_child_path.get(
            "terminal_child_container_componentwise_rejection_count", 0
        )
        <= 0
        or terminal_child_path.get(
            "terminal_child_container_componentwise_input_unavailable_count"
        )
        != 0
        or terminal_child_path.get(
            "terminal_child_container_componentwise_nonterminal_child_count"
        )
        != 0
        or terminal_child_path.get(
            "terminal_child_container_componentwise_no_container_count"
        )
        != 0
        or terminal_child_path.get(
            "terminal_child_container_componentwise_invalid_profile_count"
        )
        != 0
        or terminal_child_path.get(
            "terminal_child_container_componentwise_invalid_node_snapshot_count"
        )
        != 0
        or terminal_child_path.get(
            "terminal_child_container_componentwise_invalid_network_count"
        )
        != 0
        or terminal_child_path.get(
            "terminal_child_container_componentwise_worse_container_path_count", 0
        )
        <= 0
        or not _finite(
            terminal_child_path.get(
                "terminal_child_container_componentwise_maximum_alternative_minus_anchor_for_same_container"
            )
        )
        or not _finite(
            terminal_child_path.get(
                "terminal_child_container_componentwise_maximum_alternative_to_anchor_ratio_for_same_container"
            )
        )
        or terminal_child_path.get("performance_fields_consulted") is not False
    ):
        raise RuntimeError("V128 blind audit does not authorize reveal")
    return blind
...
def _load_rows(blind: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    if len(admitted) != 18:
        raise RuntimeError("V128 blind audit run evidence is incomplete")
    ...
    for arm_id, expected in ARMS.items():
        ...
        if not manifest_path.is_file() or file_hash(manifest_path) != frozen.get(
            ...
            raise RuntimeError(f"V128 frozen manifest changed: {arm_id}")
        manifest = load_and_validate_manifest(manifest_path)
        if manifest.get("manifest_hash") != frozen.get("manifest_hash"):
            raise RuntimeError(f"V128 manifest hash changed: {arm_id}")
        for run in manifest["runs"]:
            ...
            if (
                ...
                != expected["profile"]
                or metadata.get("v128_training_plan_sha256") != PLAN_SHA256
                or metadata.get("v128_training_only") is not True
                or metadata.get("v128_training_seed_metrics_previously_revealed")
                is not False
                or metadata.get("v128_confirmation_seeds_opened") is not False
                or metadata.get("v128_other_unopened_seeds_opened") is not False
                or metadata.get("v128_arm_id") != arm_id
                or metadata.get("v128_arm_role") != expected["role"]
                or metadata.get("v128_shock_rate_ratio") != expected["shock_rate_ratio"]
                or metadata.get("v128_shock_threshold_numerator")
                != expected["shock_threshold_numerator"]
                or metadata.get("v128_shock_threshold_denominator")
                != expected["shock_threshold_denominator"]
                or metadata.get("v128_shock_activation_horizon_frames")
                != expected["shock_activation_horizon_frames"]
                or metadata.get("v128_critical_service_ratio_numerator")
                != expected["critical_service_threshold_numerator"]
                or metadata.get("v128_critical_service_ratio_denominator")
                != expected["critical_service_threshold_denominator"]
                or metadata.get("v128_service_proxy_work_source")
                != (
                    "admitted_pending_plus_all_resident_remaining_and_same_window_projected_cpu_v1"
                    if candidate
                    else "not_applicable"
                )
                or metadata.get("v128_admitted_work_includes_all_blocked_resident")
                is not candidate
                or metadata.get("v128_completion_proximal_depth1_substitution_scope")
                is not candidate
                or metadata.get("v128_completion_proximal_definition")
                != (
                    "terminal_function_or_nonterminal_function_whose_immutable_DAG_children_are_all_terminal"
                    if candidate
                    else "not_applicable"
                )
                or metadata.get("v128_maximum_remaining_child_depth")
                != (1 if candidate else None)
                or metadata.get(
                    "v128_completion_proximal_critical_frontier_substitution"
                )
                is not candidate
                or metadata.get("v128_deeper_player_exact_anchor") is not candidate
                or metadata.get("v128_complete_componentwise_service_pareto")
                is not candidate
                or metadata.get("v128_componentwise_service_scope")
                != (
                    "all_current_critical_frontier_players"
                    if candidate
                    else "not_applicable"
                )
                or metadata.get("v128_componentwise_service_comparison")
                != (
                ...
                or metadata.get(
                    "v128_complete_terminal_child_hpa_container_componentwise_path_service_pareto"
                )
                is not candidate
                or metadata.get(
                ...
                or metadata.get("v128_terminal_child_hpa_container_node_scope")
                != (
                ...
                or metadata.get(
                    "v128_terminal_child_hpa_container_componentwise_path_input_fail_closed"
                )
                is not candidate
                or metadata.get("v128_outcome_fields_drive_policy") is not False
                or metadata.get("v128_scenario_or_burst_label_used_by_policy")
                is not False
                or metadata.get("v128_completion_or_performance_fields_used_by_policy")
                is not False
                or metadata.get("v128_future_arrivals_used_by_policy") is not False
            ):
                raise RuntimeError(f"V128 arm identity changed: {run['run_id']}")
            run_id = run["run_id"]
            summary_path = (
                ROOT
                / "runs"
            ...
            if not summary_path.is_file() or file_hash(summary_path) != admitted.get(
                ...
                raise RuntimeError(f"V128 summary differs from blind audit: {run_id}")
            ...
            if (
                ...
                raise RuntimeError(
                    f"V128 summary identity/completion mismatch: {run_id}"
                )
            rows.append(
                {
                    "arm_id": arm_id,
...
def execute_reveal() -> dict[str, Any]:
    if OUTPUT.exists():
        raise RuntimeError(f"V128 training result already exists: {OUTPUT}")
    if not PLAN.is_file() or file_hash(PLAN) != PLAN_SHA256:
        raise RuntimeError("frozen V128 plan changed before reveal")
    blind = _validate_blind_audit()
    rows = _load_rows(blind)
    if len(rows) != 18:
        raise RuntimeError("V128 revealed row count differs from sealed plan")
    ...
    result = {
        "schema_version": "NSE_E3_COMPLETION_PROXIMAL_TERMINAL_CHILD_HPA_CONTAINER_COMPONENTWISE_PATH_RESULT_V128_V1",
        "created_at": utc_now(),
        ...
        **evaluation,
        "revealed_rows": rows,
        "decision": {
            "authorize_preregistration_of_E1290_E1309_confirmation": joint_pass,
            ...
            "next_action": (
                "freeze V128 with frozen V97 E4 and separately preregister the "
                "complete E1290-E1309 NSESche confirmation before generating inputs"
                if joint_pass
                else (
                    "freeze the complete V128 training cohort, keep E1290-E1309 and "
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Reveal the frozen V128 training cohort exactly once after blind audit."
    )
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    if args.command != "reveal" or not args.execute:
        parser.error("V128 reveal requires the explicit command: reveal --execute")
    execute_reveal()
