...
OUTPUT = ROOT / "joint-blind-audit-v109-training.json"
RESULT = ROOT / "training-result-v109.json"
CANONICAL_RENAME_RECEIPT = "canonical_rename_receipt_v109.json"
...
ARMS = {
    arm_id: {
        ...
        "diagnostic_activation_horizon_frames": (
            ACTIVATION_HORIZONS[arm_id] if role == "candidate" else 100
        ),
    ...
    with gzip.open(path, "rt", encoding="utf-8") as handle:
        for line_number, line in enumerate(handle, start=1):
            ...
            _require(
                ...
                f"missing V109 causal diagnostics: {run['run_id']}:{line_number}",
            )
            _require(
                ...
                f"V109 causal diagnostic contract changed: {run['run_id']}:{line_number}",
            )
            for field in (
                ...
                _require(
                    ...
                    f"V109 causal diagnostic {field} changed: "
            ...
            _require(
                ...
                f"V109 causal diagnostic types changed: {run['run_id']}:{line_number}",
            )
            until_frame = gate.get("until_frame")
            _require(
                until_frame is None or (type(until_frame) is int and until_frame >= 0),
                f"V109 causal activation horizon changed: {run['run_id']}:{line_number}",
            )
            _require(
                gate["active"] is (until_frame is not None and frame <= until_frame),
                f"V109 causal active flag changed: {run['run_id']}:{line_number}",
            )
            critical = (
                event.get("decision", {})
                .get("load_least_dominance_gate", {})
            ...
            _require(
                ...
                f"missing V109 critical-service diagnostics: "
                f"{run['run_id']}:{line_number}",
            )
            ...
            _require(
                ...
                f"V109 critical-service contract changed: "
                f"{run['run_id']}:{line_number}",
            )
            for field in (
                ...
                _require(
                    type(value) is int and value >= 0,
                    f"V109 critical-service {field} changed: "
                    f"{run['run_id']}:{line_number}",
                )
            _require(
                not critical["accepted"] or critical["evaluated"],
                f"V109 critical-service accepted without evaluation: "
                f"{run['run_id']}:{line_number}",
            )
            if not critical_initializer_invoked:
                _require(
                    ...
                    f"V109 non-invoked window unexpectedly used critical-service state: "
                    f"{run['run_id']}:{line_number}",
                )
                if candidate:
                    _require(
                        ...
                        f"V109 candidate critical-service bypass changed: "
                        f"{run['run_id']}:{line_number}",
                    )
            if critical["accepted"]:
                ...
                _require(
                    ...
                    f"V109 accepted critical-service evidence is nonfinite: "
                    f"{run['run_id']}:{line_number}",
                )
                expected_ratio = (
                    expected["critical_service_threshold_numerator"]
                ...
                _require(
                    ...
                    f"V109 accepted critical-service certificate changed: "
                    f"{run['run_id']}:{line_number}",
                )
            ...
            if not candidate:
                _require(
                    ...
                    f"V109 anchor unexpectedly used causal state: "
                    f"{run['run_id']}:{line_number}",
                )
                continue
            if not gate["history_complete"]:
                _require(
                    ...
                    f"V109 incomplete causal history leaked counts: "
                    f"{run['run_id']}:{line_number}",
                )
                continue
            sufficient = (
            ...
            if sufficient and recent_scaled >= baseline_scaled:
                ...
                _require(
                    ...
                    f"V109 exact causal threshold did not activate: "
                    f"{run['run_id']}:{line_number}",
                )
            active_window_count += int(gate["active"])
    _require(
        window_count == run["simulation"]["total_frame"],
        f"V109 diagnostic window count changed: {run['run_id']}",
    )
    if candidate:
        _require(
            threshold_met_window_count > 0 and active_window_count > 0,
            f"V109 causal gate never activated: {run['run_id']}",
        )
        _require(
            critical_evaluated_window_count > 0,
            f"V109 critical-service gate was never evaluated: {run['run_id']}",
        )
    return {
...
def _verify_tape_catalog() -> tuple[list[dict[str, Any]], dict[str, Any]]:
    _require(TAPES.is_file(), f"missing V109 tape catalog: {TAPES}")
    catalog = read_json(TAPES)
    catalog_hash = _assert_hashed_object(catalog, "catalog_hash", "V109 tapes")
    entries = catalog.get("entries")
    _require(
        isinstance(entries, dict) and len(entries) == 12,
        "V109 tape count changed",
    )
    evidence = []
...
def _verify_reference_stage_ledgers() -> tuple[dict[str, Path], dict[Path, str]]:
    ...
    for root, expected_count in expected_by_root.items():
        ...
        _assert_ledger_contract(
            ...
            f"V109 reference stage {root}",
        )
        _require(
            not list((root / "quarantine").glob("**/attempt-*")),
            f"V109 reference quarantine is nonempty: {root}",
        )
...
def _verify_canonical_rename_receipt(
    ...
    _require(receipt_path.is_file(), "missing V109 canonical rename receipt")
    receipt = read_json(receipt_path)
    receipt_hash = _assert_hashed_object(
        receipt, "receipt_hash", "V109 canonical rename receipt"
    )
    run_id = receipt.get("run_id")
    runs = {run["run_id"]: run for run in manifest["runs"]}
    _require(
        receipt.get("schema_version") == "NSE_V109_CANONICAL_RENAME_RECEIPT_V1"
        ...
        "V109 canonical rename boundary changed",
    )
    ...
    _require(
        ...
        "V109 canonical rename filesystem state changed",
    )
    ...
    _require(
        ...
        "V109 canonical rename file inventory malformed",
    )
    expected_files = {
    ...
    _require(
        ...
        "V109 canonical rename content tree changed",
    )
    _require(
        ...
        "V109 canonical rename terminal hashes changed",
    )
    return {
        "arm_id": arm_id,
        "run_id": run_id,
...
def run_blind_audit(output: Path = OUTPUT) -> dict[str, Any]:
    _require(not output.exists(), f"V109 blind audit already exists: {output}")
    _require(not RESULT.exists(), "V109 reveal exists before blind audit")
    _require(PLAN.is_file() and file_hash(PLAN) == PLAN_SHA256, "V109 plan changed")
    plan = read_json(PLAN)
    _require(plan.get("formal_results_eligible") is False, "V109 eligibility changed")
    _require(PREPARED.is_file(), f"missing V109 prepared receipt: {PREPARED}")
    prepared = read_json(PREPARED)
    prepared_hash = _assert_hashed_object(
        prepared, "receipt_hash", "V109 prepared receipt"
    )
    _require(
        ...
        "V109 prepared scientific boundary changed",
    )
    ...
    for arm_id, expected in ARMS.items():
        ...
        _require(paths["ready"].is_file(), f"missing V109 ready manifest: {arm_id}")
        manifest = load_and_validate_manifest(paths["ready"])
        _require(
            len(manifest["runs"]) == expected["run_count"],
            f"V109 run count changed: {arm_id}",
        )
        _require(
            ...
            f"V109 ready flags changed: {arm_id}",
        )
        ...
        reference_catalog_hash = _assert_hashed_object(
            references, "catalog_hash", f"V109 references {arm_id}"
        )
        ...
        _require(
            ...
            f"V109 reference key set changed: {arm_id}",
        )
        for key, entry in sorted(entries.items()):
        ...
        _require(actual_ids == expected_ids, f"V109 canonical set changed: {arm_id}")
        _require(
            not list((workspace / "quarantine").glob("**/attempt-*")),
            f"V109 online quarantine is nonempty: {arm_id}",
        )
        ...
        for run in manifest["runs"]:
            ...
            _require(
                ...
                and metadata.get("v109_arm_id") == arm_id
                and metadata.get("v109_arm_role") == expected["role"]
                and metadata.get("v109_candidate_profile") == expected["profile"]
                and metadata.get("v109_shock_rate_ratio")
                == expected["shock_rate_ratio"]
                and metadata.get("v109_shock_threshold_numerator")
                == expected["shock_threshold_numerator"]
                and metadata.get("v109_shock_threshold_denominator")
                == expected["shock_threshold_denominator"]
                and metadata.get("v109_arrival_history_baseline_frames")
                == expected["arrival_history_baseline_frames"]
                and metadata.get("v109_arrival_history_recent_frames")
                == expected["arrival_history_recent_frames"]
                and metadata.get("v109_arrival_min_requests_per_window")
                == expected["arrival_min_requests_per_window"]
                and metadata.get("v109_shock_activation_horizon_frames")
                == expected["shock_activation_horizon_frames"]
                and metadata.get(
                    "v109_static_upper_density_gate_bypassed_only_while_shock_active"
                )
                is (expected["role"] == "candidate")
                and metadata.get("v109_nonterminal_queue_density_floor")
                == expected["nonterminal_queue_density_floor"]
                and metadata.get("v109_warm_admissibility")
                == expected["warm_admissibility"]
                and metadata.get("v109_load_least_window_certificate_mode")
                == expected["load_least_window_certificate_mode"]
                and metadata.get("v109_arrival_signal") == expected["arrival_signal"]
                and metadata.get("v109_cpu_memory_individual_noninferiority")
                is expected["cpu_memory_individual_noninferiority"]
                and metadata.get("v109_resource_bottleneck_sum_noninferiority")
                is expected["resource_bottleneck_sum_noninferiority"]
                and metadata.get("v109_resource_inputs_finite_fail_closed")
                is (expected["role"] == "candidate")
                and metadata.get("v109_critical_service_ratio_numerator")
                == expected["critical_service_threshold_numerator"]
                and metadata.get("v109_critical_service_ratio_denominator")
                == expected["critical_service_threshold_denominator"]
                and metadata.get("v109_critical_service_proxy")
                == (
                ...
                and metadata.get(
                    "v109_critical_service_proxy_inputs_finite_fail_closed"
                )
                is (expected["role"] == "candidate")
                and metadata.get("v109_scalar_faasrank_noninferiority")
                is (expected["role"] == "candidate")
                and metadata.get("v109_input_locality_component_noninferiority")
                is (expected["role"] == "candidate")
                and metadata.get("v109_componentwise_faasrank_noninferiority") is False
                and metadata.get(
                    "v109_per_child_current_warm_downstream_locality_noninferiority"
                )
                is (expected["role"] == "candidate")
                and metadata.get(
                    "v109_downstream_locality_aggregate_compensation_allowed"
                )
                is False
                and metadata.get("v109_future_child_placement_or_feasibility_used")
                is False
                and metadata.get("v109_critical_frontier_substitution")
                is expected["critical_frontier_substitution"]
                and metadata.get("v109_critical_frontier_rank_source")
                == expected["critical_frontier_rank_source"]
                and metadata.get("v109_critical_frontier_tie_rule")
                == expected["critical_frontier_tie_rule"]
                and metadata.get("v109_noncritical_exact_anchor")
                is expected["noncritical_exact_anchor"]
                and metadata.get(
                    "v109_complete_summed_critical_service_proxy_strictly_lower"
                )
                is (expected["role"] == "candidate")
                and metadata.get("v109_complete_routed_score_nonworse")
                is (expected["role"] == "candidate")
                and metadata.get("v109_complete_exact_ocs_score_nonworse")
                is (expected["role"] == "candidate")
                and metadata.get("v109_complete_immutable_baseline_welfare_nonworse")
                is (expected["role"] == "candidate")
                and metadata.get("v109_load_least_complete_score_used_as_gate") is False
                and metadata.get("v109_outcome_fields_drive_policy") is False
                and metadata.get("v109_scenario_or_burst_label_used_by_policy") is False
                and metadata.get("v109_completion_or_performance_fields_used_by_policy")
                is False
                and metadata.get("v109_future_arrivals_used_by_policy") is False
                and metadata.get("v109_training_seed_metrics_previously_revealed")
                is False
                and metadata.get("v109_confirmation_seeds_opened") is False
                and metadata.get("v109_other_unopened_seeds_opened") is False,
                f"V109 run boundary changed: {run['run_id']}",
            )
            ...
            mechanism_diagnostics = _validate_causal_horizon_diagnostics(
            ...
            _require(
                ...
                f"V109 canonical status changed: {run['run_id']}",
            )
            runtime = {
            ...
            run_evidence.append(
                {
                    ...
                    "causal_horizon_diagnostics": mechanism_diagnostics,
                    "ledger_last_hash": ledger_last_hash,
                    "reference_ledger_last_hash": reference_last_hash,
                }
            )

    _require(len(run_evidence) == 27, "V109 run evidence count must be 27")
    _require(len(reference_evidence) == 27, "V109 reference evidence count must be 27")
    _require(len(tape_evidence) == 12, "V109 tape evidence count must be 12")
    _require(
        len(canonical_rename_receipts) <= len(ARMS),
        "V109 canonical rename receipt count exceeds arm count",
    )
    for field, expected in EXPECTED_RUNTIME.items():
        _require(
            runtime_values[field] == {expected},
            f"V109 runtime {field} changed: {runtime_values[field]}",
        )
    ...
    _require(
        ...
        f"V109 runtime git identity is not singular: {git_commits}",
    )
    for (experiment_id, scenario, seed), rows in paired_inputs.items():
        _require(
            len(rows) == 3,
            f"V109 paired arm count changed: {experiment_id}/{scenario}/{seed}",
        )
        for field in (
            ...
            _require(
                ...
                f"V109 paired {field} changed: {scenario}/{seed}",
            )
        _require(
            rows[0]["common_hpa_hash"] == EXPECTED_COMMON_HPA_SHA256,
            f"V109 common HPA changed: {scenario}/{seed}",
        )

    output_payload = {
        "schema_version": "NSE_E3_CAUSAL_HORIZON_BLIND_AUDIT_V109_V1",
        "created_at": utc_now(),
        "status": "pass",
        ...
        "mechanism_diagnostic_windows_parsed": sum(
            run["causal_horizon_diagnostics"]["window_count"] for run in run_evidence
        ),
