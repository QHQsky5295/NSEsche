...
def _audit_random_prefix_window(event: Mapping[str, Any]) -> dict[str, int]:
    ...
    zero_command_prefix = prefix == 0
    if zero_command_prefix:
        if not (
            ...
            and ((feasible == 0 and not early) or (feasible > 0 and early))
        ):
            raise RuntimeError("V153 zero-command Random-prefix certificate changed")
