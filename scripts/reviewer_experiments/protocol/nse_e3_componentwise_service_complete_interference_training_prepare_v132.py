...
PLAN_SHA256 = "27619d4ebd8217334ed078f0f72315d49c295ae69cf082f068609b46c0444b34"
V117_PLAN = Path(
    "scripts/reviewer_experiments/protocol/"
...
V131_PLAN_SHA256 = "45510e542b0c8a97301d4f301b65a93935e3e2c09d904efd109ed05580d63805"
V131_BLIND_AUDIT = Path(
    "tmp/nse_e3_completion_proximal_terminal_child_hpa_anchor_running_first_top2_path_training_20260829_v131/"
    "joint-blind-audit-v131-training.json"
)
V131_BLIND_AUDIT_SHA256 = (
    "f1741919d3b09e2857129610fe7403b803a488f56ab527377c8ebedb5f0bad2e"
)
BINARY_PATH = Path("tmp/nse_v132_build_91c3ebf/release/serverless_sim.exe")
BINARY_SHA256 = "b5ba2be29e48faa7d74fefbf61c425e4566dac1fa1d1f4ac43ef33cd9359935a"
BINARY_SOURCE_COMMIT = "91c3ebff886aa7a815c70f6dec4aad3fcbdb40a3"

TRAINING_SEED_LIST = ["E1379", "E1380", "E1381"]
...
CONFIRMATION_SEEDS = [f"E{index}" for index in range(1382, 1402)]
...
E3_COMPONENTWISE_SERVICE_COMPLETE_INTERFERENCE = (
    "faasrank_native_faithful_terminal_ocs_srpt_ready_"
    "causal_arrival_shock15_horizon50_admitted_work10_"
    "componentwise_service_complete_interference_pareto_initializer_only_guard64_dual_window_safe_pareto"
)
ARMS = (
    ("v132-e3-anchor", "anchor", E3_ANCHOR, 9),
    (
        "v132-e3-causal-shock15-horizon50-admitted-work10-componentwise-service-complete-interference-initializer-only",
        "candidate",
        E3_COMPONENTWISE_SERVICE_COMPLETE_INTERFERENCE,
        9,
    ),
...
def _paths(root: Path) -> dict[str, Path]:
    return {
        "config": root / "v132-e3-training-config.json",
        "source": root / "manifest.v132-training-source-full.unbound.json",
        "capture": root / "manifest.v132-tape-capture.unbound.json",
...
def _assert_frozen_inputs() -> None:
    for path, expected in (
        ...
        (V131_PLAN, V131_PLAN_SHA256),
        (V131_BLIND_AUDIT, V131_BLIND_AUDIT_SHA256),
        ...
        if not path.is_file() or file_hash(path) != expected:
            raise RuntimeError(f"frozen V132 input is missing or changed: {path}")
    ...
    *,
    ...
    marker.update(
        {
            ...
            "v132_binary_source_commit": BINARY_SOURCE_COMMIT,
    ...
    if arm is not None:
        if frozen_model_config is None:
            raise RuntimeError("V132 arm rewrite lacks frozen model binding")
        arm_id, role, profile, expected = arm
        candidate = role == "candidate"
        marker.update(
            {
                "v132_arm_id": arm_id,
                "v132_experiment_id": "E3",
                "v132_role": role,
                "v132_profile": profile,
                "v132_shock_threshold_numerator": 3 if candidate else None,
                "v132_shock_threshold_denominator": 2 if candidate else None,
                "v132_shock_activation_horizon_frames": 50 if candidate else None,
                "v132_critical_service_ratio_numerator": 9 if candidate else None,
                "v132_critical_service_ratio_denominator": 10 if candidate else None,
                ...
                "v132_expected_run_count": expected,
            ...
            metadata.update(
                {
                    "v132_arm_id": arm_id,
                    "v132_arm_role": role,
                    "v132_candidate_profile": profile,
                    "v132_candidate_experiment": "E3",
                    "v132_shock_rate_ratio": "3/2" if candidate else None,
                    "v132_shock_threshold_numerator": 3 if candidate else None,
                    "v132_shock_threshold_denominator": 2 if candidate else None,
                    ...
                    "v132_faasrank_model_artifact_sha256": MODEL_SHA256,
                    "v132_critical_service_ratio_numerator": 9 if candidate else None,
                    "v132_critical_service_ratio_denominator": 10
                    if candidate
                    else None,
...
def prepare_v132(root: Path = ROOT) -> dict:
    _assert_frozen_inputs()
    if root.exists():
        raise RuntimeError(f"refusing to overwrite V132 training root: {root}")
    ...
    capture = derive_integration_smoke_shard(
        paths["source"], capture_ids, purpose=f"V132 fresh parent tapes {PLAN_SHA256}"
    )
    _write(
        paths["capture"],
        _rewrite(capture, purpose=f"V132 fresh parent tapes {PLAN_SHA256}"),
    )
    ...
    for arm in ARMS:
        ...
        template = derive_integration_smoke_shard(
            paths["source"], selected, purpose=f"V132 {arm_id} {PLAN_SHA256}"
        )
        manifest = _rewrite(
            template,
            purpose=f"V132 {arm_id} {PLAN_SHA256}",
            arm=arm,
        ...
        if (
            ...
            raise RuntimeError(f"V132 {arm_id} must have 9 runs/references")
        if {run["seed"] for run in manifest["runs"]} != TRAINING_SEEDS:
            raise RuntimeError(f"V132 {arm_id} escaped E1379-E1381")
        keys = {item["key"] for item in manifest["reference_build_dependencies"]}
        if all_reference_keys & keys:
            raise RuntimeError("V132 arm-specific reference keys unexpectedly overlap")
        all_reference_keys.update(keys)
        _write(arm_path(root, arm_id), manifest)
    if len(all_reference_keys) != 18:
        raise RuntimeError("V132 must declare exactly 18 arm-specific references")
    ...
    receipt = {
        "schema_version": "NSE_E3_COMPONENTWISE_SERVICE_COMPLETE_INTERFERENCE_PREPARED_V132_V1",
        "created_at": utc_now(),
        "formal_results_eligible": False,
        ...
        "V131_plan_path": str(V131_PLAN),
        "V131_plan_sha256": V131_PLAN_SHA256,
        "V131_blind_audit_path": str(V131_BLIND_AUDIT),
        "V131_blind_audit_sha256": V131_BLIND_AUDIT_SHA256,
        "V131_performance_result_reopened": False,
        ...
        "arms": [
            {
                ...
                "complete_interference_rule": (
                    "alternative_complete_interference_lte_anchor_plus_epsilon"
                    if role == "candidate"
                    else "not_applicable"
                ),
                "componentwise_per_choice_interference": False,
                "run_count": expected,
