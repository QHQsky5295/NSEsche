...
ROOT = Path("tmp/nse_e3_critical_service_time_training_20260829_v108")
PLAN = Path(
    "scripts/reviewer_experiments/protocol/"
    "nse_e3_critical_service_time_training_plan_v108.json"
)
PLAN_SHA256 = "878acf7d1e2dd8515314d83b4b5b61c8f424dd5adaec7ff05f6a0c7b66ba5854"
V107_RESULT_RECEIPT = Path(
    "scripts/reviewer_experiments/protocol/"
    "nse_e3_causal_arrival_shock_training_result_v107.json"
)
V107_RESULT_RECEIPT_SHA256 = (
    "a0baf1313dceb2ace59d7c814943196a5aea1d7251c6d2bb937eddc0200ca643"
)
V107_SOURCE_RESULT = Path(
    "tmp/nse_e3_causal_arrival_shock_training_20260829_v107/"
    "training-result-v107.json"
)
V107_SOURCE_RESULT_SHA256 = (
    "e44667ed0d6b6c85bd2bc36cd5a22cbcf054381070dac7aa7d63eda9b3db95a4"
)
FORMAL_RESULT = Path(
...
BINARY_PATH = Path("tmp/nse_v108_build_1311c55/release/serverless_sim.exe")
BINARY_SHA256 = "ff16138756e98f44f28c83abdf6f104e3c1e6b9053ac3b9d04c429412ff7a849"
BINARY_SOURCE_COMMIT = "1311c55981fc1a92e66d751048345a0c2ddbcece"
...
SOURCE_INITIAL_SEEDS = [f"E{index}" for index in range(893, 903)]
SOURCE_CI_SEEDS = [f"E{index}" for index in range(903, 913)]
TRAINING_SEED_LIST = ["E893", "E894", "E895"]
TRAINING_SEEDS = set(TRAINING_SEED_LIST)
CONFIRMATION_SEEDS = [f"E{index}" for index in range(1046, 1066)]
PREVIOUS_CONFIRMATION_SEEDS = [f"E{index}" for index in range(926, 1046)]
OTHER_UNOPENED_SEEDS = [
    ...
    *[f"E{index}" for index in range(896, 926)],
]
PORT = "3142"

E3_ANCHOR = "faasrank_native_faithful_terminal_ocs_srpt_ready_dual_window_safe_pareto"
E3_CAUSAL_ARRIVAL_SHOCK15_CRITICAL_SERVICE10_INITIALIZER_ONLY = (
    "faasrank_native_faithful_terminal_ocs_srpt_ready_"
    "causal_arrival_shock15_critical_service10_initializer_only_guard64_dual_window_safe_pareto"
)
E3_CAUSAL_ARRIVAL_SHOCK15_CRITICAL_SERVICE20_INITIALIZER_ONLY = (
    "faasrank_native_faithful_terminal_ocs_srpt_ready_"
    "causal_arrival_shock15_critical_service20_initializer_only_guard64_dual_window_safe_pareto"
)
ARMS = (
    ("v108-e3-anchor", "E3", "anchor", E3_ANCHOR, None, 9),
    (
        "v108-e3-causal-shock15-critical-service10-initializer-only",
        "E3",
        "candidate",
        E3_CAUSAL_ARRIVAL_SHOCK15_CRITICAL_SERVICE10_INITIALIZER_ONLY,
        "3/2",
    ...
    (
        "v108-e3-causal-shock15-critical-service20-initializer-only",
        "E3",
        "candidate",
        E3_CAUSAL_ARRIVAL_SHOCK15_CRITICAL_SERVICE20_INITIALIZER_ONLY,
        "3/2",
...
CRITICAL_SERVICE_RATIOS = {
    "v108-e3-causal-shock15-critical-service10-initializer-only": (9, 10),
    "v108-e3-causal-shock15-critical-service20-initializer-only": (4, 5),
}
...
def _paths(root: Path) -> dict[str, Path]:
    return {
        "config": root / "v108-e3-training-config.json",
        "source": root / "manifest.v108-e893-e902-full.unbound.json",
        "capture": root / "manifest.v108-tape-capture.unbound.json",
...
def _assert_frozen_inputs() -> None:
    for path, expected in (
        (PLAN, PLAN_SHA256),
        (V107_RESULT_RECEIPT, V107_RESULT_RECEIPT_SHA256),
        (V107_SOURCE_RESULT, V107_SOURCE_RESULT_SHA256),
        ...
        if not path.is_file() or file_hash(path) != expected:
            raise RuntimeError(f"frozen V108 input is missing or changed: {path}")
...
def _rewrite(
    ...
    *,
    ...
    marker.update(
        {
            "purpose": purpose,
            "v108_training_plan_sha256": PLAN_SHA256,
            "v108_training_only": True,
            "v108_binary_source_commit": BINARY_SOURCE_COMMIT,
    ...
    if arm is not None:
        ...
        marker.update(
            {
                "v108_arm_id": arm_id,
                "v108_experiment_id": experiment_id,
                "v108_role": role,
                "v108_profile": profile,
                "v108_shock_rate_ratio": shock_rate_ratio,
                "v108_shock_threshold_numerator": (
                    shock_threshold[0] if shock_threshold else None
                ),
                "v108_shock_threshold_denominator": (
                    shock_threshold[1] if shock_threshold else None
                ),
                "v108_arrival_history_baseline_frames": 80
                if role == "candidate"
                else None,
                "v108_arrival_history_recent_frames": 20
                if role == "candidate"
                else None,
                "v108_arrival_min_requests_per_window": 20
                if role == "candidate"
                else None,
                "v108_shock_activation_horizon_frames": 100
                if role == "candidate"
                else None,
                "v108_static_upper_density_gate_bypassed_only_while_shock_active": role
                == "candidate",
                "v108_nonterminal_queue_density_floor": (
                    8.0 if role == "candidate" else None
                ),
                "v108_warm_admissibility": (
                    "preserve_anchor_warmness" if role == "candidate" else None
                ),
                "v108_load_least_window_certificate_mode": (
                    "disabled" if role == "candidate" else "not_applicable"
                ),
                "v108_arrival_signal": "first_seen_request_ids_only"
                if role == "candidate"
                else "not_applicable",
                "v108_cpu_memory_individual_noninferiority": role == "candidate",
                "v108_resource_bottleneck_sum_noninferiority": False,
                "v108_resource_inputs_finite_fail_closed": role == "candidate",
                "v108_critical_service_ratio_numerator": (
                    critical_service_ratio[0] if critical_service_ratio else None
                ),
                "v108_critical_service_ratio_denominator": (
                    critical_service_ratio[1] if critical_service_ratio else None
                ),
                "v108_critical_frontier_substitution": role == "candidate",
                "v108_critical_frontier_rank_source": (
                    "immutable_srpt_remaining_critical_path_rank"
                    if role == "candidate"
                    else "not_applicable"
                ),
                "v108_critical_frontier_tie_rule": (
                    "every_exact_tied_current_request_frontier_maximum"
                    if role == "candidate"
                    else "not_applicable"
                ),
                "v108_noncritical_exact_anchor": role == "candidate",
                "v108_critical_service_proxy_inputs_finite_fail_closed": role
                == "candidate",
                "v108_complete_summed_critical_service_proxy_strictly_lower": role
                == "candidate",
                "v108_expected_run_count": expected,
                "v108_scenario_or_burst_label_used_by_policy": False,
                "v108_completion_or_performance_fields_used_by_policy": False,
                "v108_future_arrivals_used_by_policy": False,
            }
        )
        ...
        metadata.update(
            {
                "v108_training_plan_sha256": PLAN_SHA256,
                "v108_training_only": True,
                "v108_training_seed_metrics_previously_revealed": False,
                "v108_binary_source_commit": BINARY_SOURCE_COMMIT,
                "v108_confirmation_seeds_opened": False,
                "v108_other_unopened_seeds_opened": False,
                "v108_formal_E01_E20_reexecution": False,
            }
        )
        if arm is not None:
            (
            ...
            metadata.update(
                {
                    "v108_arm_id": arm_id,
                    "v108_arm_role": role,
                    "v108_candidate_profile": profile,
                    "v108_candidate_experiment": experiment_id,
                    "v108_shock_rate_ratio": shock_rate_ratio,
                    "v108_shock_threshold_numerator": (
                        shock_threshold[0] if shock_threshold else None
                    ),
                    ...
                    "v108_faasrank_model_artifact_sha256": MODEL_SHA256,
                    "v108_native_faithful_initializer": True,
                    "v108_dual_window_safe_pareto": True,
                    "v108_causal_arrival_shock_initializer_guard": role == "candidate",
                    "v108_terminal_players_included_without_lower_floor": role
                    == "candidate",
                    "v108_srpt_critical_path_player_order": "srpt_ready" in profile,
                    "v108_substitution_cap": None,
                    "v108_load_least_window_certificate_mode": certificate_mode,
                    "v108_arrival_signal": "first_seen_request_ids_only"
                    if role == "candidate"
                    else "not_applicable",
                    "v108_cpu_memory_individual_noninferiority": role == "candidate",
                    ...
                    "v108_critical_service_proxy_inputs_finite_fail_closed": role
                    == "candidate",
                    "v108_scalar_faasrank_noninferiority": role == "candidate",
                    "v108_input_locality_component_noninferiority": role == "candidate",
                    "v108_componentwise_faasrank_noninferiority": False,
                    "v108_per_child_current_warm_downstream_locality_noninferiority": role
                    == "candidate",
                    "v108_downstream_locality_aggregate_compensation_allowed": False,
                    "v108_future_child_placement_or_feasibility_used": False,
                    "v108_critical_frontier_substitution": role == "candidate",
                    "v108_critical_frontier_rank_source": (
                        "immutable_srpt_remaining_critical_path_rank"
                        if role == "candidate"
                    ...
                    "v108_complete_summed_critical_service_proxy_strictly_lower": role
                    == "candidate",
                    "v108_complete_routed_score_nonworse": role == "candidate",
                    "v108_complete_exact_ocs_score_nonworse": role == "candidate",
                    "v108_complete_immutable_baseline_welfare_nonworse": role
                    == "candidate",
                    "v108_load_least_complete_score_used_as_gate": False,
                    "v108_outcome_fields_drive_policy": False,
                    "v108_scenario_or_burst_label_used_by_policy": False,
                    "v108_completion_or_performance_fields_used_by_policy": False,
                    "v108_future_arrivals_used_by_policy": False,
                }
...
def prepare_v108(root: Path = ROOT) -> dict:
    _assert_frozen_inputs()
    if root.exists():
        raise RuntimeError(f"refusing to overwrite V108 training root: {root}")
    ...
    capture = derive_integration_smoke_shard(
        paths["source"],
        capture_ids,
        purpose=f"V108 fresh balanced-QoS parent tapes; plan_sha256={PLAN_SHA256}",
    )
    _write(
        paths["capture"],
        _rewrite(
    ...
    for arm in ARMS:
        ...
        template = derive_integration_smoke_shard(
            paths["source"],
            selected,
            purpose=f"V108 {arm_id} critical-service-time training; plan_sha256={PLAN_SHA256}",
        )
        manifest = _rewrite(
            template,
            purpose=f"V108 {arm_id} critical-service-time training; plan_sha256={PLAN_SHA256}",
        ...
        if (
            ...
            raise RuntimeError(f"V108 {arm_id} must have {expected} runs/references")
        if {run["seed"] for run in manifest["runs"]} != TRAINING_SEEDS:
            raise RuntimeError(f"V108 {arm_id} escaped E893-E895")
        keys = {item["key"] for item in manifest["reference_build_dependencies"]}
        if all_reference_keys & keys:
            raise RuntimeError("V108 arm-specific reference keys unexpectedly overlap")
        all_reference_keys.update(keys)
        _write(arm_path(root, arm_id), manifest)
    if len(all_reference_keys) != 27:
        raise RuntimeError("V108 must declare exactly 27 arm-specific references")
    ...
    receipt = {
        "schema_version": "NSE_E3_CRITICAL_SERVICE_TIME_PREPARED_V108_V1",
        "created_at": utc_now(),
        "formal_results_eligible": False,
        ...
        "V107_result_receipt_path": str(V107_RESULT_RECEIPT),
        "V107_result_receipt_sha256": V107_RESULT_RECEIPT_SHA256,
        "V107_source_result_path": str(V107_SOURCE_RESULT),
        "V107_source_result_sha256": V107_SOURCE_RESULT_SHA256,
