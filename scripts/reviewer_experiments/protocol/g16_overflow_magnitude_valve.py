"""Frozen zero-result D111--D115 G16 overflow-magnitude valve protocol."""
...
G16_CONTROL = "ready_order"
G16_CANDIDATE = "ready_global_overflow_magnitude_release_valve"
G16_EFFECTIVE_METHODS = (G16_CONTROL, G16_CANDIDATE)
G16_MANIFEST_SCHEMA = "NSE_G16_OVERFLOW_MAGNITUDE_VALVE_DEVELOPMENT_V1"
G16_REFERENCE_KEY_TAGS = {G16_CONTROL: 1, G16_CANDIDATE: 18}


def _nash_cell(candidate: str, load: str, node_count: int) -> dict[str, Any]:
    return _make_cell(
        "E1",
        f"G16.sche_nash.{candidate}.{load}.homogeneous.n{node_count}",
        "sche_nash",
        ...
        metadata={
            ...
            "g16_role": (
                ...
                if candidate == G16_CANDIDATE
                else "strict_ready_order_control"
            ),
            "paper_equations_changed": False,
            "new_compound_method": candidate == G16_CANDIDATE,
            "strict_best_response": True,
            "utility_guard_relative_regret": 0.0,
            "reference_key_tag": G16_REFERENCE_KEY_TAGS[candidate],
        },
    )
...
def build_g16_overflow_magnitude_valve_manifest(
    simulator_exe: Path,
    source_git_commit: str,
    config_path: Path | None = None,
) -> dict[str, Any]:
    """Build the exact zero-result 30-run G16 candidate/control product."""

    config = load_protocol_config(config_path)
    node_count = int(config["matrix_defaults"]["base_node_count"])
    if node_count != 20:
        raise ProtocolValidationError("G16 requires the frozen 20-node base cluster")
    ...
    cells = [
        ...
        for load, candidate in product(FORMAL_E1_LOADS, G16_EFFECTIVE_METHODS)
    ]
    runs = []
    for cell, seed in product(cells, G16_OVERFLOW_MAGNITUDE_VALVE_SEEDS):
    ...
    marker = {
        "schema_version": G16_MANIFEST_SCHEMA,
        "purpose": "three-load overflow-magnitude-gated release-valve control gate",
        "control": G16_CONTROL,
        "candidate": G16_CANDIDATE,
        "loads": list(FORMAL_E1_LOADS),
        "topology": "homogeneous",
        "node_count": node_count,
        "development_seeds": list(G16_OVERFLOW_MAGNITUDE_VALVE_SEEDS),
        "paper_equations_changed": False,
        "strict_eq15_required": True,
        "operational_refinement_schema_version": 12,
        "reference_key_schema_version": 13,
        "reference_key_tags": copy.deepcopy(G16_REFERENCE_KEY_TAGS),
    ...
    manifest: dict[str, Any] = {
        ...
        "created_at": utc_now(),
        "phase": "development",
        "bank_id": "TSCv1.development.G16.overflow-magnitude-valve.D111-D115",
        "formal_results_eligible": False,
        "fixed_seed_bank": {
            "policy": G16_OVERFLOW_MAGNITUDE_VALVE_SAMPLE_POLICY,
            "all_seeds": list(G16_OVERFLOW_MAGNITUDE_VALVE_SEEDS),
            "selected_seeds": list(G16_OVERFLOW_MAGNITUDE_VALVE_SEEDS),
        ...
        "reuse_analyses": [],
        "g16_overflow_magnitude_valve_development": marker,
    }
...
def write_g16_overflow_magnitude_valve_manifest(
    output_path: Path,
    simulator_exe: Path,
    source_git_commit: str,
    config_path: Path | None = None,
    ...
    if output_path.exists():
        raise ProtocolValidationError("refusing to overwrite G16 manifest")
    manifest = build_g16_overflow_magnitude_valve_manifest(
