...
from scripts.reviewer_experiments.protocol.nse_e3_admitted_interference_training_prepare_v114 import (
...
PREPARED = ROOT / "prepared-manifests-v114.json"
TAPES = ROOT / "tapes.catalog.json"
OUTPUT = ROOT / "joint-blind-audit-v114-training.json"
RESULT = ROOT / "training-result-v114.json"
...
def _validate_admitted_work_diagnostics(
    ...
    """Validate V114 mechanism evidence without opening any performance summary."""

    path = canonical / "reviewer_records" / run["run_id"] / "nash_metrics.jsonl.gz"
    _require(path.is_file(), f"missing V114 Nash diagnostics: {run['run_id']}")
    ...
    with gzip.open(path, "rt", encoding="utf-8") as handle:
        for line_number, line in enumerate(handle, start=1):
            ...
            _require(type(frame) is int and frame >= 0, "invalid V114 window frame")

            cluster = event.get("cluster")
            _require(isinstance(cluster, dict), "missing V114 cluster diagnostics")
            admitted_fields = (
            ...
            if candidate:
                _require(
                    ...
                    f"V114 admitted-work observation changed: {run['run_id']}:{line_number}",
                )
                ...
                _require(
                    ...
                    f"V114 admitted-work snapshot counts changed: "
                ...
                _require(
                    ...
                    f"V114 admitted-work aggregate changed: {run['run_id']}:{line_number}",
                )
            ...
            _require(
                ...
                f"V114 critical-service contract changed: {run['run_id']}:{line_number}",
            )
            for field in (
                ...
                _require(
                    ...
                    f"V114 critical {field} changed: {run['run_id']}:{line_number}",
                )
            _require(
                not critical["accepted"] or critical["evaluated"],
                f"V114 accepted without evaluation: {run['run_id']}:{line_number}",
            )
            ...
            _require(
                ...
                f"V114 admitted-interference contract changed: "
                f"{run['run_id']}:{line_number}",
            )
            ...
            if invoked and all(value is not None for value in interference_values):
                _require(
                    ...
                    f"V114 admitted-interference evidence changed: "
                    f"{run['run_id']}:{line_number}",
                )
                ...
                _require(
                    ...
                    f"V114 partial admitted-interference evidence: "
                    f"{run['run_id']}:{line_number}",
                )
            if critical["accepted"]:
                _require(
                    ...
                    f"V114 accepted worse admitted interference: "
                    f"{run['run_id']}:{line_number}",
                )
            ...
            if not invoked:
                _require(
                    ...
                    f"V114 non-invoked critical state changed: {run['run_id']}:{line_number}",
                )
                _require(
                    interference["rejected_candidate_count"] == 0
                    and interference["input_unavailable_count"] == 0,
                    f"V114 non-invoked interference state changed: "
                    f"{run['run_id']}:{line_number}",
                )
            if critical["accepted"]:
                values = tuple(
                ...
                _require(
                    ...
                    f"V114 accepted evidence is nonfinite: {run['run_id']}:{line_number}",
                )
                ...
                _require(
                    ...
                    f"V114 accepted admitted-work certificate changed: "
                    f"{run['run_id']}:{line_number}",
                )
            ...
            if not candidate:
                _require(
                    ...
                    f"V114 anchor used causal state: {run['run_id']}:{line_number}",
                )
                continue
            if not gate["history_complete"]:
                _require(
                    gate["baseline_count"] == 0 and gate["recent_count"] == 0,
                    f"V114 incomplete history leaked counts: {run['run_id']}:{line_number}",
                )
                continue
            sufficient = (
            ...
            if sufficient and recent_scaled >= baseline_scaled:
                ...
                _require(
                    ...
                    f"V114 exact threshold did not activate: "
                    f"{run['run_id']}:{line_number}",
                )

    _require(
        counts["windows"] == run["simulation"]["total_frame"],
        f"V114 diagnostic window count changed: {run['run_id']}",
    )
    if candidate:
        _require(
            ...
            f"V114 mechanism was not exercised: {run['run_id']}",
        )
    return {
...
def _verify_tape_catalog() -> tuple[list[dict[str, Any]], dict[str, Any]]:
    _require(TAPES.is_file(), f"missing V114 tape catalog: {TAPES}")
    catalog = read_json(TAPES)
    catalog_hash = _assert_hashed_object(catalog, "catalog_hash", "V114 tapes")
    entries = catalog.get("entries")
    _require(
        isinstance(entries, dict) and len(entries) == 12, "V114 tape count changed"
    )
    evidence = []
...
def _verify_reference_stage_ledgers() -> tuple[dict[str, Path], dict[Path, str]]:
    ...
    for arm_id, expected in ARMS.items():
        ...
        root = _stage_root_from_receipts(
            entries, "receipt_path", expected["run_count"], f"V114 reference {arm_id}"
        )
        roots[arm_id] = root
    ...
    for root, expected_count in expected_by_root.items():
        ...
        _assert_ledger_contract(
            ...
            f"V114 reference stage {root}",
        )
        _require(
            not list((root / "quarantine").glob("**/attempt-*")),
            f"V114 reference quarantine is nonempty: {root}",
        )
...
def _validate_run_boundary(
    ...
    required = {
        "v114_training_plan_sha256": PLAN_SHA256,
        "v114_training_only": True,
        "v114_training_seed_metrics_previously_revealed": False,
        "v114_confirmation_seeds_opened": False,
        "v114_other_unopened_seeds_opened": False,
        "v114_formal_E01_E20_reexecution": False,
        "v114_arm_id": arm_id,
        "v114_arm_role": expected["role"],
        "v114_candidate_profile": expected["profile"],
        "v114_candidate_experiment": "E3",
        "v114_shock_rate_ratio": expected["shock_rate_ratio"],
        "v114_shock_threshold_numerator": expected["shock_threshold_numerator"],
        "v114_shock_threshold_denominator": expected["shock_threshold_denominator"],
        "v114_arrival_history_baseline_frames": 80 if candidate else None,
        "v114_arrival_history_recent_frames": 20 if candidate else None,
        "v114_arrival_min_requests_per_window": 20 if candidate else None,
        "v114_shock_activation_horizon_frames": expected[
            "shock_activation_horizon_frames"
        ],
        "v114_nonterminal_queue_density_floor": 8.0 if candidate else None,
        "v114_warm_admissibility": "preserve_anchor_warmness" if candidate else None,
        "v114_load_least_window_certificate_mode": (
            "disabled" if candidate else "not_applicable"
        ),
        "v114_critical_service_ratio_numerator": expected[
            "critical_service_threshold_numerator"
        ],
        "v114_critical_service_ratio_denominator": expected[
            "critical_service_threshold_denominator"
        ],
        "v114_critical_service_proxy": (
            "remote_parent_transfer_plus_cold_start_plus_admitted_queue_cpu_work"
            if candidate
            else "not_applicable"
        ),
        "v114_service_proxy_work_source": (
            ADMITTED_WORK_SOURCE if candidate else "not_applicable"
        ),
        "v114_admitted_work_includes_all_blocked_resident": candidate,
        "v114_admitted_work_deterministic_f64_sum": candidate,
        "v114_admitted_interference_pareto": candidate,
        "v114_admitted_interference_source": (
            ADMITTED_INTERFERENCE_SOURCE if candidate else "not_applicable"
        ),
        "v114_admitted_interference_inputs_finite_fail_closed": candidate,
        "v114_complete_admitted_interference_nonworse": candidate,
        "v114_critical_service_proxy_inputs_finite_fail_closed": candidate,
        "v114_cpu_memory_individual_noninferiority": candidate,
        "v114_scalar_faasrank_noninferiority": candidate,
        "v114_input_locality_component_noninferiority": candidate,
        "v114_per_child_current_warm_downstream_locality_noninferiority": candidate,
        "v114_critical_frontier_substitution": candidate,
        "v114_critical_frontier_rank_source": (
            "immutable_srpt_remaining_critical_path_rank"
            if candidate
            else "not_applicable"
        ),
        "v114_noncritical_exact_anchor": candidate,
        "v114_complete_summed_critical_service_proxy_strictly_lower": candidate,
        "v114_complete_routed_score_nonworse": candidate,
        "v114_complete_exact_ocs_score_nonworse": candidate,
        "v114_complete_immutable_baseline_welfare_nonworse": candidate,
        "v114_outcome_fields_drive_policy": False,
        "v114_scenario_or_burst_label_used_by_policy": False,
        "v114_completion_or_performance_fields_used_by_policy": False,
        "v114_future_arrivals_used_by_policy": False,
    }
    _require(
        ...
        f"V114 run boundary changed: {run['run_id']}",
    )


def run_blind_audit(output: Path = OUTPUT) -> dict[str, Any]:
    _require(not output.exists(), f"V114 blind audit already exists: {output}")
    _require(not RESULT.exists(), "V114 reveal exists before blind audit")
    _require(PLAN.is_file() and file_hash(PLAN) == PLAN_SHA256, "V114 plan changed")
    plan = read_json(PLAN)
    _require(plan.get("formal_results_eligible") is False, "V114 eligibility changed")
    _require(PREPARED.is_file(), f"missing V114 prepared receipt: {PREPARED}")
    prepared = read_json(PREPARED)
    prepared_hash = _assert_hashed_object(
        prepared, "receipt_hash", "V114 prepared receipt"
    )
    _require(
        ...
        "V114 prepared scientific boundary changed",
    )
    ...
    for arm_id, expected in ARMS.items():
        ...
        _require(paths["ready"].is_file(), f"missing V114 ready manifest: {arm_id}")
        ...
        _require(
            ...
            f"V114 ready boundary changed: {arm_id}",
        )
        ...
        reference_catalog_hash = _assert_hashed_object(
            references, "catalog_hash", f"V114 references {arm_id}"
        )
        ...
        _require(
            ...
            f"V114 reference key set changed: {arm_id}",
        )
        for key, entry in sorted(entries.items()):
            ...
            _require(
                ...
                f"V114 reference evidence changed: {key}",
            )
        ...
        _require(actual_ids == expected_ids, f"V114 canonical set changed: {arm_id}")
        _require(
            not list((workspace / "quarantine").glob("**/attempt-*")),
            f"V114 online quarantine is nonempty: {arm_id}",
        )
        ...
        for run in manifest["runs"]:
            ...
            _require(
                ...
                f"V114 canonical status changed: {run['run_id']}",
            )
            runtime = {
    ...
    _require(len(run_evidence) == 18, "V114 run evidence count must be 18")
    _require(len(reference_evidence) == 18, "V114 reference evidence count must be 18")
    _require(len(tape_evidence) == 12, "V114 tape evidence count must be 12")
    for field, expected in EXPECTED_RUNTIME.items():
        _require(
            runtime_values[field] == {expected},
            f"V114 runtime {field} changed: {runtime_values[field]}",
        )
    ...
    _require(
        ...
        f"V114 runtime git identity is not singular: {git_commits}",
    )
    _require(len(paired_inputs) == 9, "V114 paired input product changed")
    for (scenario, seed), rows in paired_inputs.items():
        _require(len(rows) == 2, f"V114 paired arm count changed: {scenario}/{seed}")
        for field in (
            ...
            _require(
                ...
                f"V114 paired {field} changed: {scenario}/{seed}",
            )
        _require(
            rows[0]["common_hpa_hash"] == EXPECTED_COMMON_HPA_SHA256,
            f"V114 common HPA changed: {scenario}/{seed}",
        )

    output_payload = {
        "schema_version": "NSE_E3_ADMITTED_INTERFERENCE_BLIND_AUDIT_V114_V1",
        "created_at": utc_now(),
        "status": "pass",
