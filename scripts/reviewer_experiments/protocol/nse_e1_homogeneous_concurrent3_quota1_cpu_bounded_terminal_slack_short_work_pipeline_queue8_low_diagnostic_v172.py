...
ROOT = Path(
    "tmp/nse_e1_homogeneous_concurrent3_quota1_cpu_bounded_terminal_low_diagnostic_20260901_v172"
)
PLAN = Path(
    "scripts/reviewer_experiments/protocol/"
    "nse_e1_homogeneous_concurrent3_quota1_cpu_bounded_terminal_low_diagnostic_plan_v172.json"
)
PLAN_SHA256 = "f21b33141e3151de841072d91d76bf32fe6a9e7d58d860aee7a12de32061bbe1"
IMPLEMENTATION = Path(
    "scripts/reviewer_experiments/protocol/"
    "nse_e1_homogeneous_concurrent3_quota1_cpu_bounded_terminal_slack_short_work_pipeline_queue8_low_diagnostic_implementation_v172.json"
)
IMPLEMENTATION_SHA256 = (
    "2e7764b25bcca471141144f2c754b9a070f416f01a5c3362f3d99596c46a45e0"
)
V170_FAILURE = Path(
    "scripts/reviewer_experiments/protocol/"
...
ARM_ID = "v172-low-srpt-slack-concurrent3-quota1-cpu-bounded-terminal-short5p5-pipeline-hiku2-ocs-queue8"
PROFILE = "srpt_slack_concurrent3_quota1_cpu_bounded_terminal_short5p5_pipeline_hiku2_ocs_queue8"
FRONTIER = "parents_completed_or_concurrent3_quota1_cpu_bounded_terminal_or_slack_short_work_parents_scheduled"
SINGLE_CHANGE = (
    "V171_quota_two_reduced_to_one_deterministic_active_heavy_incomplete-parent_"
    "terminal_player_ordered_by_request_id_then_function_id"
)
TERMINAL_DEFINITION = (
    "admit_all_parents-completed_players;when_current_heavy_terminal_player_count_"
    "strictly_exceeds_two_admit_at_most_one_heavy_incomplete-parent-terminal-"
    "player_in_ascending_request-id-then-function-id_order_and_apply_the_V168_"
...
BINARY_SOURCE_COMMIT = "cfc8bda507ca6075c3df66f69178242611fae245"
BINARY_PATH = Path("serverless_sim/target_e1_v172/release/serverless_sim.exe")
BINARY_SHA256 = "2da5db5c6c813d334500d2dceaf50567edacd72a9d78afe2bba0e8cca7fc327e"


def paths(root: Path = ROOT) -> dict[str, Path]:
    return {
        "manifest": root / f"manifest.{ARM_ID}.unbound.json",
        "prepared": root / "prepared-v172.json",
        "schedule": root / "frozen-run-order-v172.json",
        ...
        "execution": root / "execution-receipt-v172.json",
        "pairing": root / "pairing-audit-v172.json",
        "blind": root / "joint-blind-audit-v172.json",
        "result": root / "diagnostic-result-v172.json",
    }
...
def _assert_frozen_inputs() -> dict[str, Any]:
    ...
    for path, sha256, label in (
        (PLAN, PLAN_SHA256, "V172 plan"),
        (IMPLEMENTATION, IMPLEMENTATION_SHA256, "V172 implementation receipt"),
        ...
        (BINARY_PATH, BINARY_SHA256, "V172 release binary"),
    ...
    if not (
        ...
        and change.get("old_active_heavy_quota") == 2
        and change.get("new_active_heavy_quota") == ACTIVE_HEAVY_QUOTA
        ...
        == "admit_at_most_one_heavy_incomplete_parent_terminal_player"
        ...
        raise RuntimeError("V172 implementation boundary changed")
...
def _rewrite_candidate(
    ...
    marker.update(
        {
            "purpose": (
                "V172 outcome-disclosed six-seed concurrent3 quota1 CPU-bounded "
                "terminal-starvation diagnostic; "
                "never a formal result or paper superiority claim"
            ),
            "v172_role": "result_blind_concurrent3_quota1_cpu_bounded_terminal_falsification",
            "v172_plan_sha256": PLAN_SHA256,
            "v172_implementation_sha256": IMPLEMENTATION_SHA256,
            "v172_binary_source_commit": BINARY_SOURCE_COMMIT,
            "v172_protocol_source_commit": protocol_source_commit,
            "v172_binary_sha256": BINARY_SHA256,
            "v172_arm_id": ARM_ID,
            "v172_profile": PROFILE,
            "v172_player_frontier": FRONTIER,
            "v172_single_change_from_v171": SINGLE_CHANGE,
            "v172_cpu_threshold": CPU_THRESHOLD,
            "v172_cpu_boundary": "at_or_below_is_admitted",
            "v172_heavy_player_threshold": HEAVY_PLAYER_THRESHOLD,
            "v172_minimum_active_heavy_players": MINIMUM_ACTIVE_HEAVY_PLAYERS,
            "v172_overload_activation_boundary": "strictly_above_activates",
            "v172_inactive_behavior": "exact_V159_terminal_admission",
            "v172_active_heavy_admission_policy": "deterministic_quota_one",
            "v172_active_heavy_admission_quota": ACTIVE_HEAVY_QUOTA,
            "v172_quota_selection_order": QUOTA_SELECTION_ORDER,
            "v172_short_work_threshold": SHORT_WORK_THRESHOLD,
            "v172_queue_density_threshold": QUEUE_THRESHOLD,
            "v172_queue_boundary": "below_is_strict",
            "v172_environment": COMMON_ENVIRONMENT,
            "v172_expected_run_count": len(SEEDS),
            "v172_expected_reference_build_count": len(SEEDS),
            "v172_fixed_order": list(SEEDS),
            "v172_candidate_performance_summaries_parsed": 0,
            "v172_remaining_fourteen_authorized": False,
            "v172_confirmation_inputs_generated": False,
        }
    )
    for run in rewritten["runs"]:
        ...
        run["metadata"] = {
            ...
            "v172_queue_density_threshold": QUEUE_THRESHOLD,
            "v172_queue_boundary": "below_is_strict",
            "v172_source_e1_run_id": source_run_id,
            "v172_source_e1_run_spec_hash": source_run_spec_hash,
            "v172_candidate_performance_summaries_parsed_before_run": 0,
            "v172_remaining_fourteen_authorized": False,
            "v172_confirmation_inputs_generated": False,
        }
...
def _validate_product(manifest: Mapping[str, Any], *, references_bound: bool) -> None:
    if not (
        ...
        raise RuntimeError("V172 exact E01/E02/E13/E15/E16/E20 product changed")
    ...
    for run in manifest["runs"]:
        ...
        if not (
            ...
            and metadata.get("v172_profile") == PROFILE
            and metadata.get("v172_player_frontier") == FRONTIER
            and metadata.get("v172_cpu_threshold") == CPU_THRESHOLD
            and metadata.get("v172_cpu_boundary") == "at_or_below_is_admitted"
            and metadata.get("v172_heavy_player_threshold") == HEAVY_PLAYER_THRESHOLD
            and metadata.get("v172_minimum_active_heavy_players")
            == MINIMUM_ACTIVE_HEAVY_PLAYERS
            and metadata.get("v172_overload_activation_boundary")
            == "strictly_above_activates"
            and metadata.get("v172_inactive_behavior")
            == "exact_V159_terminal_admission"
            and metadata.get("v172_active_heavy_admission_policy")
            == "deterministic_quota_one"
            and metadata.get("v172_active_heavy_admission_quota") == ACTIVE_HEAVY_QUOTA
            and metadata.get("v172_quota_selection_order") == QUOTA_SELECTION_ORDER
            and metadata.get("v172_short_work_threshold") == SHORT_WORK_THRESHOLD
            and metadata.get("v172_queue_density_threshold") == QUEUE_THRESHOLD
            and metadata.get("v172_queue_boundary") == "below_is_strict"
        ):
            raise RuntimeError(f"V172 run contract changed: {run.get('run_id')}")


def prepare_v172(root: Path = ROOT) -> dict[str, Any]:
    source = _assert_frozen_inputs()
    if root.exists():
        raise RuntimeError(f"refusing to overwrite V172 root: {root}")
    ...
    schedule = {
        "schema_version": "NSE_E1_HOMOGENEOUS_CONCURRENT3_CPU_BOUNDED_TERMINAL_LOW_SCHEDULE_V172_V1",
        "created_at": utc_now(),
    ...
    receipt = {
        "schema_version": "NSE_E1_HOMOGENEOUS_CONCURRENT3_CPU_BOUNDED_TERMINAL_LOW_PREPARED_V172_V1",
        "created_at": utc_now(),
        ...
        "active_heavy_admission_policy": "deterministic_quota_one",
...
def execute_v172(root: Path = ROOT) -> dict[str, Any]:
    output = paths(root)
    if output["execution"].exists():
        raise RuntimeError("V172 execution receipt already exists")
    prepared = read_json(output["prepared"])
    _assert_hashed(prepared, "receipt_hash", "V172 prepared receipt")
    ...
    for ordinal, seed in enumerate(SEEDS, start=1):
        ...
        if completed.returncode != 0:
            raise RuntimeError(f"V172 dispatch {seed} failed: {completed.returncode}")
        ...
        if not (
            ...
            raise RuntimeError(f"V172 canonical is not a QC pass: {run['run_id']}")
        dispatches.append(
            {
                "ordinal": ordinal,
                "seed": seed,
    ...
    receipt = {
        "schema_version": "NSE_E1_HOMOGENEOUS_CAPACITY_OVERLOAD_CPU_BOUNDED_TERMINAL_LOW_EXECUTION_V172_V1",
        "created_at": utc_now(),
        "candidate_performance_summaries_parsed": 0,
        "plan_sha256": PLAN_SHA256,
...
def _audit_nash_log(
    ...
    *,
    ...
    with gzip.open(log, "rt", encoding="utf-8") as stream:
        for line in stream:
            ...
            if kind not in counts:
                raise RuntimeError(f"unexpected V172 Nash observation kind: {kind}")
            counts[kind] += 1
            if kind == "run_config":
                ...
                if not (
                    ...
                    raise RuntimeError("V172 run_config contract changed")
            elif kind == "window":
                if event.get("frame") != counts["window"] - 1:
                    raise RuntimeError("V172 scheduler window sequence changed")
                ...
                if (
                    ...
                    raise RuntimeError("V172 decision frontier/hash changed")
                ...
                if not (
                    ...
                    raise RuntimeError("V172 CPU/slack/queue frontier evidence changed")
                ...
                if not (
                    ...
                    raise RuntimeError("V172 route telemetry is incomplete")
                expected = (
                    LOW_EXPERT if float(density) < QUEUE_THRESHOLD else HIGH_EXPERT
                )
                if route.get("selected_expert") != expected:
                    raise RuntimeError("V172 route does not match queue density")
                ...
                if key is None:
                    if source != "not_requested":
                        raise RuntimeError("V172 unrequested reference reason changed")
                    totals["reference_not_requested"] += 1
                elif source in ("offline_table", "offline_table_nonpositive"):
                    totals["reference_available"] += 1
                else:
                    raise RuntimeError("V172 bound reference source changed")
                ...
                if not (
                    ...
                    raise RuntimeError("V172 Nash terminal marker changed")
    ...
    if (
        ...
        raise RuntimeError("V172 Nash log cardinality changed")
    if (
        totals["reference_available"] + totals["reference_not_requested"]
        != counts["window"]
    ):
        raise RuntimeError("V172 reference replay coverage changed")
...
def _mechanism_falsification_gate(
    ...
    if set(by_seed) != set(SEEDS):
        raise RuntimeError("V172 blind audit seed product changed")
...
def blind_audit_v172(root: Path = ROOT) -> dict[str, Any]:
    output = paths(root)
    if output["blind"].exists():
        raise RuntimeError("V172 blind audit already exists")
    _assert_frozen_inputs()
    prepared = read_json(output["prepared"])
    prepared_hash = _assert_hashed(prepared, "receipt_hash", "V172 prepared receipt")
    execution = read_json(output["execution"])
    execution_hash = _assert_hashed(execution, "receipt_hash", "V172 execution receipt")
    manifest = load_and_validate_manifest(output["ready"])
    _validate_product(manifest, references_bound=True)
    ...
    if (
        ...
        raise RuntimeError("V172 exact pairing failed")
    ...
    if [item["seed"] for item in execution["dispatches"]] != list(SEEDS):
        raise RuntimeError("V172 execution order changed")
    ...
    if {path.name for path in canonical_root.iterdir() if path.is_dir()} != {
        ...
        raise RuntimeError("V172 canonical product changed")
    quarantine = output["workspace"] / "quarantine"
    if quarantine.exists() and any(quarantine.rglob("attempt-*")):
        raise RuntimeError("V172 has unexplained quarantined attempts")
    ...
    if len(identities) != 1:
        raise RuntimeError("V172 runtime identity is not unanimous")
    ...
    if not (
        ...
        raise RuntimeError("V172 runtime identity changed")
    mechanism = _mechanism_falsification_gate(audits)
    if not mechanism["pass"]:
        raise RuntimeError("V172 mechanism falsification breadth is insufficient")
    document = {
        "schema_version": "NSE_E1_HOMOGENEOUS_CONCURRENT3_CPU_BOUNDED_TERMINAL_LOW_BLIND_AUDIT_V172_V1",
        "created_at": utc_now(),
        "status": "pass",
...
def _load_candidate(
    ...
    if len(rows) != len(SEEDS) or [row["seed"] for row in rows] != list(SEEDS):
        raise RuntimeError("V172 candidate result product changed")
    return rows


def _hybrid_rows_v172(
    v170_rows: Sequence[Mapping[str, Any]],
    v172_rows: Sequence[Mapping[str, Any]],
) -> list[dict[str, Any]]:
    ...
    if len(v172_rows) != len(SEEDS) or {row["seed"] for row in v172_rows} != set(SEEDS):
        raise RuntimeError("V172 diagnostic candidate cohort changed")
    replacements = {row["seed"]: dict(row) for row in v172_rows}
    return [
...
def reveal_v172(root: Path = ROOT) -> dict[str, Any]:
    output = paths(root)
    if output["result"].exists():
        raise RuntimeError("V172 result already exists")
    blind = read_json(output["blind"])
    blind_hash = _assert_hashed(blind, "blind_audit_hash", "V172 blind audit")
    if not (
        ...
        raise RuntimeError("V172 blind audit did not authorize reveal")
    manifest = load_and_validate_manifest(output["ready"])
    candidate = _load_candidate(manifest, root)
    v170_rows = _load_v170_candidate()
    hybrid = _hybrid_rows_v172(v170_rows, candidate)
    ...
    document = {
        "schema_version": "NSE_E1_HOMOGENEOUS_CONCURRENT3_CPU_BOUNDED_TERMINAL_LOW_DIAGNOSTIC_RESULT_V172_V1",
        "created_at": utc_now(),
        "training_only": True,
        "formal_results_eligible": False,
        ...
        "disposition": (
            ...
            if passed
            else "retain_all_six_valid_diagnostic_runs_and_retire_concurrent3_quota1_cpu_bounded_terminal_candidate"
        ),
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    if action == "prepare":
        document, key = prepare_v172(), "receipt_hash"
    elif action == "execute":
        document, key = execute_v172(), "receipt_hash"
    elif action == "blind-audit":
        document, key = blind_audit_v172(), "blind_audit_hash"
    else:
        document, key = reveal_v172(), "result_hash"
