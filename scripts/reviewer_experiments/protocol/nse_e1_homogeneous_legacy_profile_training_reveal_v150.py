...
OUTPUT_NAME = "training-result-v150.json"
...
def _load_candidate(
    ...
    if len(rows) != 60 or {(row["load"], row["seed"]) for row in rows} != expected:
        raise RuntimeError("V150 candidate result product changed")
    return rows
...
def execute_reveal(root: Path = ROOT) -> dict[str, Any]:
    ...
    if output.exists():
        raise RuntimeError(f"V150 training result already exists: {output}")
    ...
    if not (
        ...
        raise RuntimeError(
            "V150 blind audit is absent, changed, or did not authorize reveal"
        )
    ...
    document = {
        "schema_version": "NSE_E1_HOMOGENEOUS_LEGACY_PROFILE_TRAINING_RESULT_V150_V1",
        "created_at": utc_now(),
        "training_only": True,
        ...
        "disposition": (
            ...
            if passed
            else "retain_all_v150_runs_and_retire_failed_load_profiles_without_confirmation_inputs"
        ),
