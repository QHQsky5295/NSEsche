...
BLIND_AUDIT = ROOT / "joint-blind-audit-v91-training.json"
MANIFEST = ROOT / "manifest.v91-exact-native-per-player-training.ready.json"
MANIFEST_FILE_SHA256 = (
    "b9200fb2cb3e83cd6ab38b043e148d1bdeafdbd245e85fc7661811495b1a2d0d"
)
MANIFEST_HASH = "77e7a052e6fb0ff08c03f94f2f6ade1f3f131f2157c66c7df059ff474a774c7e"
CANONICAL = ROOT / "runs/training-v91/canonical"
OUTPUT = ROOT / "training-result-v91.json"
...
def _load_rows() -> list[dict]:
    ...
    for run in manifest["runs"]:
        ...
        if (
            ...
            or metadata.get("v91_training_plan_sha256") != PLAN_SHA256
            or metadata.get("v91_training_only") is not True
        ):
...
def _validate_blind_audit() -> dict:
    if not BLIND_AUDIT.is_file():
        raise RuntimeError("V91 blind audit is missing")
    ...
    if (
        ...
        raise RuntimeError("V91 blind audit does not authorize reveal")
    return blind


def main() -> None:
    if OUTPUT.exists():
        raise RuntimeError(f"V91 training result already exists: {OUTPUT}")
    ...
    rows = _load_rows()
    if len(rows) != 12:
        raise RuntimeError("V91 revealed row count differs from sealed plan")
    ...
    result = {
        "schema_version": "NSE_E3E4_EXACT_NATIVE_PER_PLAYER_TRAINING_RESULT_V91_V1",
        "created_at": utc_now(),
        ...
        "revealed_rows": rows,
        "decision": {
            "authorize_v91_confirmation_on_E716_E718": all_pass,
            ...
            "next_action": (
                "freeze V91 profiles and preregister exactly E716-E718"
                if all_pass
                else "retain V91 cohort, keep E716-E718 sealed, and return to mechanism diagnosis"
            ),
        },
    }
