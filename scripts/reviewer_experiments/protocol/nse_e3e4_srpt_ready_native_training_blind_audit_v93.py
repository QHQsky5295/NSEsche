...
PREPARED = ROOT / "manifest.v93-srpt-ready-native-training.sla.json"
PREPARED_FILE_SHA256 = (
    "38be4c1defcef420530d43f7c619c7f3ab92d69d6b24fbc754f945590e27eec0"
)
PREPARED_MANIFEST_HASH = (
    "8d4e6c3ccca29a7ff9aa0bcc43b6dbdb5de8151c0dbae7b7844e81229152df60"
)
READY = ROOT / "manifest.v93-srpt-ready-native-training.ready.json"
READY_FILE_SHA256 = "30871586a87787ae79353e79df72d8b25629370e4956acc5c3eca8808c207c68"
READY_MANIFEST_HASH = "07b0b1aeef0573e78180455a83937afb76594f5f2a1015388a4c9136c18695ef"
WORKSPACE = ROOT / "runs/training-v93"
PAIRING = ROOT / "training-v93-pairing-audit.json"
REFERENCES = ROOT / "references.v93.catalog.json"
REFERENCES_FILE_SHA256 = (
    "18bba37aaddf1886c030cc57981c26f19dedf79df3fcfce4b361303a1be9b836"
)
REFERENCE_WORKSPACE = ROOT / "reference-builds-v93"
OUTPUT = ROOT / "joint-blind-audit-v93-training.json"
EXPECTED_BINARY_SHA256 = (
    "5c3fc338eda895fd7fcb34bdc0c8056d8bb17e756a03c36437ac5b83ff188eab"
)
...
EXPECTED_PROFILES = {
    "E3": "ocs_native_exact_srpt_ready_per_player_strict_pareto",
    "E4": "jiagu_native_exact_srpt_ready_per_player_strict_pareto",
}
...
def _run_evidence(manifest: dict, pairing: dict) -> tuple[list[dict], dict]:
    ...
    for run in sorted(manifest["runs"], key=lambda item: item["run_id"]):
        ...
        _require(
            metadata.get("v93_training_plan_sha256") == PLAN_SHA256
            and metadata.get("v93_training_only") is True
            and metadata.get("v93_training_seed_metrics_previously_revealed") is True
            and metadata.get("v93_exact_native_initializer") is True
            and metadata.get("v93_every_player_expert_nonworsening_certificate") is True
            and metadata.get("v93_at_least_one_player_strict_expert_improvement")
            is True
            and metadata.get("v93_parent_complete_ready_frontier") is True
            and metadata.get("v93_srpt_player_order") is True
            and metadata.get("v93_native_expert_demand_context_retained") is True
            and metadata.get("v93_resource_scaling_excluded") is True
            and metadata.get("v93_complete_v92_cohort_retained") is True,
            f"V93 scientific boundary missing: {run_id}",
        )
...
def main() -> None:
    ...
    _require(
        not (ROOT / "training-result-v93.json").exists(),
        "V93 reveal exists before blind audit",
    )
    ...
    audit = {
        "schema_version": "NSE_E3E4_V93_SRPT_READY_TRAINING_BLIND_AUDIT_V1",
        "created_at": utc_now(),
        "status": "passed",
