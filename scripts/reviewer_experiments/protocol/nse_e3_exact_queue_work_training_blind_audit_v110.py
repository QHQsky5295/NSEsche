...
from scripts.reviewer_experiments.protocol.nse_e3_exact_queue_work_training_prepare_v110 import (
...
PREPARED = ROOT / "prepared-manifests-v110.json"
TAPES = ROOT / "tapes.catalog.json"
OUTPUT = ROOT / "joint-blind-audit-v110-training.json"
RESULT = ROOT / "training-result-v110.json"
...
ARMS = {
    arm_id: {
        ...
        "work_source": EXACT_WORK_SOURCE if role == "candidate" else LEGACY_WORK_SOURCE,
        "run_count": count,
    }
...
def _validate_exact_queue_work_diagnostics(
    run: dict[str, Any], canonical: Path, expected: dict[str, Any]
) -> dict[str, Any]:
    """Validate V110 mechanism evidence without opening any performance summary."""

    path = canonical / "reviewer_records" / run["run_id"] / "nash_metrics.jsonl.gz"
    _require(path.is_file(), f"missing V110 Nash diagnostics: {run['run_id']}")
    candidate = expected["role"] == "candidate"
    counts = Counter()
    exact_pending_total_sum = 0.0
    exact_runnable_total_sum = 0.0
    with gzip.open(path, "rt", encoding="utf-8") as handle:
        for line_number, line in enumerate(handle, start=1):
            ...
            _require(type(frame) is int and frame >= 0, "invalid V110 window frame")

            cluster = event.get("cluster")
            _require(isinstance(cluster, dict), "missing V110 cluster diagnostics")
            exact_fields = (
            ...
            if candidate:
                _require(
                    ...
                    f"V110 exact-work observation changed: {run['run_id']}:{line_number}",
                )
                _require(
                    float(cluster["queue_pending_cpu_work_total"]) + 1.0e-9
                    >= float(cluster["queue_pending_cpu_work_max"])
                    and float(cluster["queue_runnable_cpu_work_total"]) + 1.0e-9
                    >= float(cluster["queue_runnable_cpu_work_max"]),
                    f"V110 exact-work aggregate changed: {run['run_id']}:{line_number}",
                )
                exact_pending_total_sum += float(
                    cluster["queue_pending_cpu_work_total"]
                )
                exact_runnable_total_sum += float(
                    cluster["queue_runnable_cpu_work_total"]
                )
                counts["exact_work_complete_windows"] += 1
            ...
            _require(
                ...
                f"V110 critical-service contract changed: {run['run_id']}:{line_number}",
            )
            for field in (
                ...
                _require(
                    ...
                    f"V110 critical {field} changed: {run['run_id']}:{line_number}",
                )
            _require(
                not critical["accepted"] or critical["evaluated"],
                f"V110 accepted without evaluation: {run['run_id']}:{line_number}",
            )
            if not invoked:
                _require(
                    ...
                    f"V110 non-invoked critical state changed: {run['run_id']}:{line_number}",
                )
            if critical["accepted"]:
                ...
                _require(
                    ...
                    f"V110 accepted evidence is nonfinite: {run['run_id']}:{line_number}",
                )
                ...
                _require(
                    ...
                    f"V110 accepted exact-work certificate changed: "
            ...
            if not candidate:
                _require(
                    ...
                    f"V110 anchor used causal state: {run['run_id']}:{line_number}",
                )
                continue
            if not gate["history_complete"]:
                _require(
                    gate["baseline_count"] == 0 and gate["recent_count"] == 0,
                    f"V110 incomplete history leaked counts: {run['run_id']}:{line_number}",
                )
                continue
            sufficient = (
            ...
            if sufficient and recent_scaled >= baseline_scaled:
                ...
                _require(
                    ...
                    f"V110 exact threshold did not activate: "
                    f"{run['run_id']}:{line_number}",
                )

    _require(
        counts["windows"] == run["simulation"]["total_frame"],
        f"V110 diagnostic window count changed: {run['run_id']}",
    )
    if candidate:
        _require(
            counts["exact_work_complete_windows"] == counts["windows"]
            and counts["threshold_met_windows"] > 0
            and counts["active_windows"] > 0
            and counts["critical_evaluated_windows"] > 0,
            f"V110 mechanism was not exercised: {run['run_id']}",
        )
    return {
        "file_sha256": file_hash(path),
        "window_count": counts["windows"],
        "exact_work_complete_window_count": counts["exact_work_complete_windows"],
        "exact_pending_cpu_work_total_sum": exact_pending_total_sum,
        "exact_runnable_cpu_work_total_sum": exact_runnable_total_sum,
...
def _verify_tape_catalog() -> tuple[list[dict[str, Any]], dict[str, Any]]:
    _require(TAPES.is_file(), f"missing V110 tape catalog: {TAPES}")
    catalog = read_json(TAPES)
    catalog_hash = _assert_hashed_object(catalog, "catalog_hash", "V110 tapes")
    entries = catalog.get("entries")
    _require(
        isinstance(entries, dict) and len(entries) == 12, "V110 tape count changed"
    )
    evidence = []
...
def _verify_reference_stage_ledgers() -> tuple[dict[str, Path], dict[Path, str]]:
    ...
    for arm_id, expected in ARMS.items():
        ...
        root = _stage_root_from_receipts(
            entries, "receipt_path", expected["run_count"], f"V110 reference {arm_id}"
        )
        roots[arm_id] = root
    ...
    for root, expected_count in expected_by_root.items():
        ...
        _assert_ledger_contract(
            ...
            f"V110 reference stage {root}",
        )
        _require(
            not list((root / "quarantine").glob("**/attempt-*")),
            f"V110 reference quarantine is nonempty: {root}",
        )
...
def _validate_run_boundary(
    ...
    required = {
        "v110_training_plan_sha256": PLAN_SHA256,
        "v110_training_only": True,
        "v110_training_seed_metrics_previously_revealed": False,
        "v110_confirmation_seeds_opened": False,
        "v110_other_unopened_seeds_opened": False,
        "v110_formal_E01_E20_reexecution": False,
        "v110_arm_id": arm_id,
        "v110_arm_role": expected["role"],
        "v110_candidate_profile": expected["profile"],
        "v110_candidate_experiment": "E3",
        "v110_shock_rate_ratio": expected["shock_rate_ratio"],
        "v110_shock_threshold_numerator": expected["shock_threshold_numerator"],
        "v110_shock_threshold_denominator": expected["shock_threshold_denominator"],
        "v110_arrival_history_baseline_frames": 80 if candidate else None,
        "v110_arrival_history_recent_frames": 20 if candidate else None,
        "v110_arrival_min_requests_per_window": 20 if candidate else None,
        "v110_shock_activation_horizon_frames": expected[
            "shock_activation_horizon_frames"
        ],
        "v110_nonterminal_queue_density_floor": 8.0 if candidate else None,
        "v110_warm_admissibility": "preserve_anchor_warmness" if candidate else None,
        "v110_load_least_window_certificate_mode": (
            "disabled" if candidate else "not_applicable"
        ),
        "v110_critical_service_ratio_numerator": expected[
            "critical_service_threshold_numerator"
        ],
        "v110_critical_service_ratio_denominator": expected[
            "critical_service_threshold_denominator"
        ],
        "v110_critical_service_proxy": (
            "remote_parent_transfer_plus_cold_start_plus_exact_queue_cpu_work"
            if candidate
            else "not_applicable"
        ),
        "v110_service_proxy_work_source": (
            EXACT_WORK_SOURCE if candidate else "not_applicable"
        ),
        "v110_exact_queue_work_excludes_blocked_resident": candidate,
        "v110_exact_queue_work_deterministic_f64_sum": candidate,
        "v110_critical_service_proxy_inputs_finite_fail_closed": candidate,
        "v110_cpu_memory_individual_noninferiority": candidate,
        "v110_scalar_faasrank_noninferiority": candidate,
        "v110_input_locality_component_noninferiority": candidate,
        "v110_per_child_current_warm_downstream_locality_noninferiority": candidate,
        "v110_critical_frontier_substitution": candidate,
        "v110_critical_frontier_rank_source": (
            "immutable_srpt_remaining_critical_path_rank"
            if candidate
            else "not_applicable"
        ),
        "v110_noncritical_exact_anchor": candidate,
        "v110_complete_summed_critical_service_proxy_strictly_lower": candidate,
        "v110_complete_routed_score_nonworse": candidate,
        "v110_complete_exact_ocs_score_nonworse": candidate,
        "v110_complete_immutable_baseline_welfare_nonworse": candidate,
        "v110_outcome_fields_drive_policy": False,
        "v110_scenario_or_burst_label_used_by_policy": False,
        "v110_completion_or_performance_fields_used_by_policy": False,
        "v110_future_arrivals_used_by_policy": False,
    }
    _require(
        ...
        f"V110 run boundary changed: {run['run_id']}",
    )


def run_blind_audit(output: Path = OUTPUT) -> dict[str, Any]:
    _require(not output.exists(), f"V110 blind audit already exists: {output}")
    _require(not RESULT.exists(), "V110 reveal exists before blind audit")
    _require(PLAN.is_file() and file_hash(PLAN) == PLAN_SHA256, "V110 plan changed")
    plan = read_json(PLAN)
    _require(plan.get("formal_results_eligible") is False, "V110 eligibility changed")
    _require(PREPARED.is_file(), f"missing V110 prepared receipt: {PREPARED}")
    prepared = read_json(PREPARED)
    prepared_hash = _assert_hashed_object(
        prepared, "receipt_hash", "V110 prepared receipt"
    )
    _require(
        ...
        "V110 prepared scientific boundary changed",
    )
    ...
    for arm_id, expected in ARMS.items():
        ...
        _require(paths["ready"].is_file(), f"missing V110 ready manifest: {arm_id}")
        ...
        _require(
            ...
            f"V110 ready boundary changed: {arm_id}",
        )
        ...
        reference_catalog_hash = _assert_hashed_object(
            references, "catalog_hash", f"V110 references {arm_id}"
        )
        ...
        _require(
            ...
            f"V110 reference key set changed: {arm_id}",
        )
        for key, entry in sorted(entries.items()):
            ...
            _require(
                ...
                f"V110 reference evidence changed: {key}",
            )
        ...
        _require(actual_ids == expected_ids, f"V110 canonical set changed: {arm_id}")
        _require(
            not list((workspace / "quarantine").glob("**/attempt-*")),
            f"V110 online quarantine is nonempty: {arm_id}",
        )
        ...
        for run in manifest["runs"]:
            ...
            diagnostics = _validate_exact_queue_work_diagnostics(
            ...
            _require(
                ...
                f"V110 canonical status changed: {run['run_id']}",
            )
            runtime = {
            ...
            run_evidence.append(
                {
                    ...
                    "exact_queue_work_diagnostics": diagnostics,
                    "ledger_last_hash": ledger_last_hash,
                    "reference_ledger_last_hash": reference_last_hash,
                }
            )

    _require(len(run_evidence) == 18, "V110 run evidence count must be 18")
    _require(len(reference_evidence) == 18, "V110 reference evidence count must be 18")
    _require(len(tape_evidence) == 12, "V110 tape evidence count must be 12")
    for field, expected in EXPECTED_RUNTIME.items():
        _require(
            runtime_values[field] == {expected},
            f"V110 runtime {field} changed: {runtime_values[field]}",
        )
    ...
    _require(
        ...
        f"V110 runtime git identity is not singular: {git_commits}",
    )
    _require(len(paired_inputs) == 9, "V110 paired input product changed")
    for (scenario, seed), rows in paired_inputs.items():
        _require(len(rows) == 2, f"V110 paired arm count changed: {scenario}/{seed}")
        for field in (
            ...
            _require(
                ...
                f"V110 paired {field} changed: {scenario}/{seed}",
            )
        _require(
            rows[0]["common_hpa_hash"] == EXPECTED_COMMON_HPA_SHA256,
            f"V110 common HPA changed: {scenario}/{seed}",
        )

    output_payload = {
        "schema_version": "NSE_E3_EXACT_QUEUE_WORK_BLIND_AUDIT_V110_V1",
        "created_at": utc_now(),
        "status": "pass",
        ...
        "mechanism_diagnostic_windows_parsed": sum(
            run["exact_queue_work_diagnostics"]["window_count"] for run in run_evidence
        ),
