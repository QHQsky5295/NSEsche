...
PLAN = Path(
    ...
    "nse_e1_homogeneous_20node_low_earliest_executable_finish_training_plan_v189.json"
)
PLAN_SHA256 = "4f2bd5a05014d56ed86d288940fbc298386528a4cb8b191fe7ab23a35760d74d"
PLAN_HASH = "f8dd4019248de5a7f136146e045315206f4b968bb89a9d48e9ec62adfd602376"
PLAN_COMMIT = "5a1ce78980f5ba61268030290852dc9e0a70cb4a"
FAILURE_RECEIPT = Path(
    "scripts/reviewer_experiments/protocol/"
    "nse_e1_homogeneous_20node_low_native_clearance_response_training_failure_v188.json"
)
FAILURE_RECEIPT_SHA256 = (
    "8f87f2ce7e0fa5cdbface8d8ef777e4c68eb02989afcf49d260cf4f8a152114e"
)
FAILURE_RECEIPT_HASH = (
    "8993db5d5981e563d0794eb979177d5702bba7fcbdbcbb2744d210a01a172de8"
)
IMPLEMENTATION_RECEIPT = Path(
    "scripts/reviewer_experiments/protocol/"
    "nse_e1_homogeneous_20node_low_earliest_executable_finish_training_implementation_v189.json"
)
MODULE = Path(__file__)
TEST = Path(
    "scripts/reviewer_experiments/protocol/tests/"
    "test_nse_e1_low_earliest_executable_finish_training_v189.py"
)
GOAL = Path(
...
CANDIDATE_PROFILE = (
    ...
    "pipeline_earliest_executable_finish_queue8"
)
PORT = "3215"
BINARY_PATH = Path("serverless_sim/target_e1_v189/release/serverless_sim.exe")
BINARY_SHA256 = "cf7f9f4d1f76d6e98844145b88e60dc02f16bc0a1b77d2cdea2eb0204e5dbb59"
BINARY_BYTES = 5_954_560
BINARY_SOURCE_COMMIT = "549dbe4f7a8484fe430209e12707cd9bd732e0dd"
...
V188_RESULT = Path(
    ...
    "training-result-v188.json"
)
V188_RESULT_SHA256 = "41d6df2947a387f01d65600074dd55a9a4edeb61b333a0d7245036501a2e47eb"
V188_RESULT_HASH = "da82d8f581c082f9ddb7ab6b46161b486cf5aff2d754ef30ef8d586020f5bdcc"


def paths(root: Path = ROOT) -> dict[str, Path]:
    return {
        "candidate_tapes": root / "manifest.candidate.tapes.json",
        "schedule": root / "schedule-v189.json",
        "prepared": root / "prepared-v189.json",
        ...
        "reference_execution": root / "reference-execution-receipt-v189.json",
        "candidate_workspace": root / "formal-runs-candidate",
        "execution": root / "execution-receipt-v189.json",
        "inventory": root / "inventory-audit-v189.json",
        "blind": root / "blind-audit-v189.json",
        "result": root / "training-result-v189.json",
    }
...
def _assert_plan() -> dict[str, Any]:
    _assert_file(PLAN, PLAN_SHA256, "V189 plan")
    _assert_file(FAILURE_RECEIPT, FAILURE_RECEIPT_SHA256, "V188 failure receipt")
    plan = read_json(PLAN)
    failure = read_json(FAILURE_RECEIPT)
    _require(
        _assert_hashed(plan, "plan_hash", "V189 plan") == PLAN_HASH
        ...
        is False
        and plan.get("mechanistic_hypothesis", {}).get("uses_queue_density_router")
        is False
        and plan.get("mechanistic_hypothesis", {}).get("uses_ordinal_borda") is False
        and plan.get("result_blind_execution", {}).get("all_valid_rows_retained")
        is True,
        "V189 plan boundary changed",
    )
    _require(
        _assert_hashed(failure, "receipt_hash", "V188 failure receipt")
        == FAILURE_RECEIPT_HASH
        and failure.get("status")
        == "complete_same_tape_training_failed_throughput_and_qpr_gates"
        and failure.get("technical_integrity", {}).get("all_valid_rows_retained")
        is True,
        "V188 failure evidence changed",
    )
    return plan
...
def _assert_frozen_inputs(*, require_implementation: bool) -> dict[str, Any]:
    ...
    for path, expected, label in (
        (BINARY_PATH, BINARY_SHA256, "V189 binary"),
        (SCHEDULER_SOURCE, SCHEDULER_SOURCE_SHA256, "V189 scheduler source"),
        (PYTHON_PATH, PYTHON_SHA256, "Python executable"),
        (CARGO_LOCK, CARGO_LOCK_SHA256, "Cargo.lock"),
        (V188_RESULT, V188_RESULT_SHA256, "V188 result"),
    ):
        _assert_file(path, expected, label)
    _require(BINARY_PATH.stat().st_size == BINARY_BYTES, "V189 binary size changed")
    v184.v182.v181._assert_json_semantic(
        MODULE_CONF, MODULE_CONF_SEMANTIC_HASH, "module_conf_es.json"
    )
    result = read_json(V188_RESULT)
    _require(
        _assert_hashed(result, "result_hash", "V188 result") == V188_RESULT_HASH
        and result.get("status") == "training_fail"
        and result.get("valid_seed_deletion_replacement_relabeling_or_selective_rerun")
        is False,
        "V188 result boundary changed",
    )
    _source_manifests()
    if require_implementation:
        _require(
            IMPLEMENTATION_RECEIPT.is_file(), "V189 implementation receipt missing"
        )
        receipt = read_json(IMPLEMENTATION_RECEIPT)
        _assert_hashed(receipt, "receipt_hash", "V189 implementation receipt")
        _require(
            ...
            "V189 implementation receipt does not bind this harness",
        )
    return plan
...
def _build_tape_bound_manifest(protocol_commit: str) -> dict[str, Any]:
    ...
    _require(set(source_by_seed) == set(SEEDS), "V189 source product is not exact 20")
    ...
    for seed in SEEDS:
        ...
        _require(
            ...
            f"V189 {seed} source arms do not share the frozen tape",
        )
        run["variant"] = "homogeneous-20node-low-v189-native-training"
        ...
        run["metadata"] = {
            "v189_training_only": True,
            "v189_plan_sha256": PLAN_SHA256,
            "v189_plan_hash": PLAN_HASH,
            "v189_plan_commit": PLAN_COMMIT,
            "v189_protocol_source_commit": protocol_commit,
            "v189_profile": CANDIDATE_PROFILE,
            "v189_player_frontier": v187.FRONTIER,
            "v189_binary_source_commit": BINARY_SOURCE_COMMIT,
            "v189_binary_sha256": BINARY_SHA256,
            "v189_earliest_executable_finish_formula": "max(cold_start_readiness,parent_transfer_readiness)+processor_sharing_compute",
            "v189_uses_queue_density_router": False,
            "v189_uses_ordinal_borda": False,
            "v189_source_v187_candidate_run_id": source_run_id,
            "v189_source_v187_candidate_run_spec_hash": source_spec_hash,
            "v189_source_v187_control_run_id": control_run["run_id"],
            "v189_source_tape_reused": True,
            "v189_external_baseline_scores_allowed": False,
            "v189_new_base_tape_captures": 0,
            "v189_new_control_or_baseline_runs": 0,
            "v189_seed_deletion_replacement_or_selective_result_rerun": False,
        }
        ...
        lineage.append(
            {
                ...
                "v189_run_id": run["run_id"],
                "v189_run_spec_hash": run["run_spec_hash"],
            }
        )
    ...
    marker.update(
        {
            "purpose": "V189 NSESche-native candidate on frozen V187 E1610-E1629 tapes",
            "v189_training_only": True,
            "v189_plan_sha256": PLAN_SHA256,
            "v189_plan_hash": PLAN_HASH,
            "v189_profile": CANDIDATE_PROFILE,
            ...
            "v189_uses_queue_density_router": False,
            "v189_uses_ordinal_borda": False,
            "v189_source_to_candidate_lineage": lineage,
            "v189_expected_runs": 20,
            "v189_expected_references": 20,
            "v189_new_base_tapes": 0,
            "v189_new_control_or_baseline_runs": 0,
...
def _validate_candidate(manifest: Mapping[str, Any], *, references_bound: bool) -> None:
    ...
    _require(
        ...
        "V189 exact ordered product changed",
    )
    _require(
        ...
        "V189 binding state changed",
    )
    for run in runs:
        reference = run.get("reference_dependency", {})
        _require(
            v187._selected(run)
            and run.get("variant") == "homogeneous-20node-low-v189-native-training"
            and run.get("environment", {}).get("NASH_OPERATIONAL_EXPERT_PROXY")
            == CANDIDATE_PROFILE
            and run.get("environment", {}).get("SERVERLESS_SIM_PORT") == PORT
            and run.get("metadata", {}).get("v189_external_baseline_scores_allowed")
            is False
            and run.get("metadata", {}).get("v189_uses_queue_density_router") is False
            and run.get("metadata", {}).get("v189_uses_ordinal_borda") is False
            and run.get("metadata", {}).get("v189_new_control_or_baseline_runs") == 0
            ...
            f"V189 run contract changed: {run.get('run_id')}",
        )


def prepare_v189(root: Path = ROOT) -> dict[str, Any]:
    _assert_frozen_inputs(require_implementation=True)
    _require(not root.exists(), f"refusing to overwrite V189 root: {root}")
    ...
    schedule = {
        "schema_version": "NSE_E1_LOW_EARLIEST_EXECUTABLE_FINISH_SCHEDULE_V189_V1",
        "created_at": utc_now(),
    ...
    receipt = {
        "schema_version": "NSE_E1_LOW_EARLIEST_EXECUTABLE_FINISH_PREPARED_V189_V1",
        "created_at": utc_now(),
        "training_only": True,
...
def build_references_v189(root: Path = ROOT) -> dict[str, Any]:
    _assert_frozen_inputs(require_implementation=True)
    output = paths(root)
    ...
    _require(len(catalog.get("entries", {})) == 20, "V189 reference catalog is not 20")
    count, last_hash = verify_ledger(
        output["reference_workspace"] / "reference_builds" / "ledger.jsonl"
    )
    receipt = {
        "schema_version": "NSE_E1_LOW_EARLIEST_EXECUTABLE_FINISH_REFERENCES_V189_V1",
        "created_at": utc_now(),
...
def execute_v189(root: Path = ROOT) -> dict[str, Any]:
    _assert_frozen_inputs(require_implementation=True)
    output = paths(root)
    ...
    for ordinal, seed in enumerate(SEEDS, start=1):
        ...
        _require(
            ...
            f"V189 run is not canonical QC pass: {run['run_id']}",
        )
        dispatches.append(
            {
                "ordinal": ordinal,
    ...
    receipt = {
        "schema_version": "NSE_E1_LOW_EARLIEST_EXECUTABLE_FINISH_EXECUTION_V189_V1",
        "created_at": utc_now(),
...
def _runtime_identity(
    ...
    _require(len(identities) == 1, "V189 runtime identity is not unanimous")
...
def blind_audit_v189(root: Path = ROOT) -> dict[str, Any]:
    _assert_frozen_inputs(require_implementation=True)
    output = paths(root)
    _require(not output["blind"].exists(), "V189 blind audit already exists")
    _require(not output["result"].exists(), "V189 result exists before blind audit")
    prepared = read_json(output["prepared"])
    references = read_json(output["reference_execution"])
    execution = read_json(output["execution"])
    receipt_hashes = {
        "prepared": _assert_hashed(prepared, "receipt_hash", "V189 prepared"),
        "references": _assert_hashed(references, "receipt_hash", "V189 references"),
        "execution": _assert_hashed(execution, "receipt_hash", "V189 execution"),
    }
    _require(
        ...
        "V189 result-blind receipts changed",
    )
    manifest = load_and_validate_manifest(output["candidate_ready"])
    _validate_candidate(manifest, references_bound=True)
    ...
    _require(actual_ids == expected_ids, "V189 canonical set changed")
    audits = []
    run_evidence = []
    ...
    for run in manifest["runs"]:
        ...
        _require(
            ...
            and qc.get("passed") is True
            and qc.get("classification") == "qc_pass",
            f"V189 non-clean canonical: {run['run_id']}",
        )
        audits.append(audit)
        run_evidence.append(
            {
    ...
    for run in control["runs"]:
        ...
        _require(
            ...
            f"V189 {run['seed']} candidate/control tape mismatch",
        )
    ...
    if quarantine_root.exists():
        for attempt_path in sorted(quarantine_root.rglob("attempt.json")):
            ...
            _require(
                attempt.get("classification") != "qc_pass",
                f"V189 QC-pass attempt incorrectly quarantined: {attempt_path}",
            )
    ...
    inventory = {
        "schema_version": "NSE_E1_LOW_EARLIEST_EXECUTABLE_FINISH_INVENTORY_V189_V1",
        "created_at": utc_now(),
        "status": "pass",
        "seeds": list(SEEDS),
    ...
    document = {
        "schema_version": "NSE_E1_LOW_EARLIEST_EXECUTABLE_FINISH_BLIND_AUDIT_V189_V1",
        "created_at": utc_now(),
        "status": "pass",
...
def _evaluate_training(rows: Sequence[Mapping[str, Any]]) -> dict[str, Any]:
    ...
    _require(
        ...
        "V189 training rows contain nonfinite values",
    )
    return evaluation


def reveal_v189(root: Path = ROOT) -> dict[str, Any]:
    output = paths(root)
    _require(not output["result"].exists(), "V189 result already exists")
    blind = read_json(output["blind"])
    blind_hash = _assert_hashed(blind, "blind_audit_hash", "V189 blind audit")
    _require(
        ...
        "V189 blind audit does not authorize reveal",
    )
    _reread_goal()
    ...
    document = {
        "schema_version": "NSE_E1_LOW_EARLIEST_EXECUTABLE_FINISH_TRAINING_RESULT_V189_V1",
        "created_at": utc_now(),
        ...
        "decision": (
            "freeze_V189_training_winner_and_preregister_disjoint_low_confirmation"
            if passed
            else "retain_complete_V189_failure_close_axis_and_do_not_advance_to_middle"
        ),
    }
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    functions = {
        "prepare": prepare_v189,
        "build-references": build_references_v189,
        "execute": execute_v189,
        "blind-audit": blind_audit_v189,
        "reveal": reveal_v189,
    }
