...
OUTPUT = ROOT / "joint-blind-audit-v102-training.json"
RESULT = ROOT / "training-result-v102.json"
...
def _verify_tape_catalog() -> tuple[list[dict[str, Any]], dict[str, Any]]:
    _require(TAPES.is_file(), f"missing V102 tape catalog: {TAPES}")
    catalog = read_json(TAPES)
    catalog_hash = _assert_hashed_object(catalog, "catalog_hash", "V102 tapes")
    entries = catalog.get("entries")
    _require(
        isinstance(entries, dict) and len(entries) == 12,
        "V102 tape count changed",
    )
    evidence = []
...
def _verify_reference_stage_ledgers() -> tuple[dict[str, Path], dict[Path, str]]:
    ...
    for root, expected_count in expected_by_root.items():
        ...
        _assert_ledger_contract(
            ...
            f"V102 reference stage {root}",
        )
        _require(
            not list((root / "quarantine").glob("**/attempt-*")),
            f"V102 reference quarantine is nonempty: {root}",
        )
...
def run_blind_audit(output: Path = OUTPUT) -> dict[str, Any]:
    _require(not output.exists(), f"V102 blind audit already exists: {output}")
    _require(not RESULT.exists(), "V102 reveal exists before blind audit")
    _require(PLAN.is_file() and file_hash(PLAN) == PLAN_SHA256, "V102 plan changed")
    plan = read_json(PLAN)
    _require(plan.get("formal_results_eligible") is False, "V102 eligibility changed")
    _require(PREPARED.is_file(), f"missing V102 prepared receipt: {PREPARED}")
    prepared = read_json(PREPARED)
    prepared_hash = _assert_hashed_object(
        prepared, "receipt_hash", "V102 prepared receipt"
    )
    _require(
        ...
        "V102 prepared scientific boundary changed",
    )
    ...
    for arm_id, expected in ARMS.items():
        ...
        _require(paths["ready"].is_file(), f"missing V102 ready manifest: {arm_id}")
        manifest = load_and_validate_manifest(paths["ready"])
        _require(
            len(manifest["runs"]) == expected["run_count"],
            f"V102 run count changed: {arm_id}",
        )
        _require(
            ...
            f"V102 ready flags changed: {arm_id}",
        )
        ...
        reference_catalog_hash = _assert_hashed_object(
            references, "catalog_hash", f"V102 references {arm_id}"
        )
        ...
        _require(
            ...
            f"V102 reference key set changed: {arm_id}",
        )
        for key, entry in sorted(entries.items()):
        ...
        _require(actual_ids == expected_ids, f"V102 canonical set changed: {arm_id}")
        _require(
            not list((workspace / "quarantine").glob("**/attempt-*")),
            f"V102 online quarantine is nonempty: {arm_id}",
        )
        ...
        for run in manifest["runs"]:
            ...
            _require(
                ...
                and metadata.get("v102_arm_id") == arm_id
                and metadata.get("v102_arm_role") == expected["role"]
                and metadata.get("v102_candidate_profile") == expected["profile"]
                and metadata.get("v102_upper_queue_density_threshold")
                == expected["upper_queue_density_threshold"]
                and metadata.get("v102_nonterminal_queue_density_floor")
                == expected["nonterminal_queue_density_floor"]
                and metadata.get("v102_warm_admissibility")
                == expected["warm_admissibility"]
                and metadata.get("v102_load_least_window_certificate_mode")
                == expected["load_least_window_certificate_mode"]
                and metadata.get("v102_training_seed_metrics_previously_revealed")
                is False
                and metadata.get("v102_confirmation_seeds_opened") is False
                and metadata.get("v102_other_unopened_seeds_opened") is False,
                f"V102 run boundary changed: {run['run_id']}",
            )
            ...
            _require(
                ...
                f"V102 canonical status changed: {run['run_id']}",
            )
            runtime = {
    ...
    _require(len(run_evidence) == 27, "V102 run evidence count must be 27")
    _require(len(reference_evidence) == 27, "V102 reference evidence count must be 27")
    _require(len(tape_evidence) == 12, "V102 tape evidence count must be 12")
    for field, expected in EXPECTED_RUNTIME.items():
        _require(
            runtime_values[field] == {expected},
            f"V102 runtime {field} changed: {runtime_values[field]}",
        )
    ...
    _require(
        ...
        f"V102 runtime git identity is not singular: {git_commits}",
    )
    for (experiment_id, scenario, seed), rows in paired_inputs.items():
        _require(
            len(rows) == 3,
            f"V102 paired arm count changed: {experiment_id}/{scenario}/{seed}",
        )
        for field in (
            ...
            _require(
                ...
                f"V102 paired {field} changed: {scenario}/{seed}",
            )
        _require(
            rows[0]["common_hpa_hash"] == EXPECTED_COMMON_HPA_SHA256,
            f"V102 common HPA changed: {scenario}/{seed}",
        )

    output_payload = {
        "schema_version": "NSE_E3_LOAD_INITIALIZER_CERTIFICATE_FACTORIAL_BLIND_AUDIT_V102_V1",
        "created_at": utc_now(),
        "status": "pass",
