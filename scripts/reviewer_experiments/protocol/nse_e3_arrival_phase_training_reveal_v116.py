...
from scripts.reviewer_experiments.protocol.nse_e3_arrival_phase_training_blind_audit_v116 import (
    ARMS,
...
ANCHOR_ID = "v116-e3-anchor"
CANDIDATE_ID = (
    "v116-e3-causal-shock15-horizon50-admitted-work10-"
    "complete-interference-rising-phase-initializer-only"
)
...
def _validate_blind_audit() -> dict[str, Any]:
    ...
    if any(
        ...
        raise RuntimeError("V116 blind audit has not been frozen into the reveal code")
    if not BLIND_AUDIT.is_file() or file_hash(BLIND_AUDIT) != BLIND_AUDIT_FILE_SHA256:
        raise RuntimeError("V116 blind audit is missing or changed")
    ...
    if (
        ...
        raise RuntimeError("V116 blind audit does not authorize reveal")
    return blind
...
def _load_rows(blind: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    if len(admitted) != 18:
        raise RuntimeError("V116 blind audit run evidence is incomplete")
    ...
    for arm_id, expected in ARMS.items():
        ...
        if not manifest_path.is_file() or file_hash(manifest_path) != frozen.get(
            ...
            raise RuntimeError(f"V116 frozen manifest changed: {arm_id}")
        manifest = load_and_validate_manifest(manifest_path)
        if manifest.get("manifest_hash") != frozen.get("manifest_hash"):
            raise RuntimeError(f"V116 manifest hash changed: {arm_id}")
        for run in manifest["runs"]:
            ...
            if (
                ...
                != expected["profile"]
                or metadata.get("v116_training_plan_sha256") != PLAN_SHA256
                or metadata.get("v116_training_only") is not True
                or metadata.get("v116_training_seed_metrics_previously_revealed")
                is not False
                or metadata.get("v116_confirmation_seeds_opened") is not False
                or metadata.get("v116_other_unopened_seeds_opened") is not False
                or metadata.get("v116_arm_id") != arm_id
                or metadata.get("v116_arm_role") != expected["role"]
                or metadata.get("v116_shock_rate_ratio") != expected["shock_rate_ratio"]
                or metadata.get("v116_shock_threshold_numerator")
                != expected["shock_threshold_numerator"]
                or metadata.get("v116_shock_threshold_denominator")
                != expected["shock_threshold_denominator"]
                or metadata.get("v116_shock_activation_horizon_frames")
                != expected["shock_activation_horizon_frames"]
                or metadata.get("v116_critical_service_ratio_numerator")
                != expected["critical_service_threshold_numerator"]
                or metadata.get("v116_critical_service_ratio_denominator")
                != expected["critical_service_threshold_denominator"]
                or metadata.get("v116_service_proxy_work_source")
                != (
                    "admitted_pending_plus_all_resident_remaining_and_same_window_projected_cpu_v1"
                    if candidate
                    else "not_applicable"
                )
                or metadata.get("v116_admitted_work_includes_all_blocked_resident")
                is not candidate
                or metadata.get("v116_admitted_interference_pareto") is not candidate
                or metadata.get("v116_componentwise_admitted_interference_pareto")
                is not False
                or metadata.get("v116_admitted_interference_source")
                != (
                    "admitted_processor_sharing_interference_pending_resident_and_projected_cpu_v1"
                    if candidate
                    else "not_applicable"
                )
                or metadata.get("v116_complete_admitted_interference_nonworse")
                is not candidate
                or metadata.get("v116_arrival_phase_guard") is not candidate
                or metadata.get("v116_arrival_phase_window_frames")
                != (20 if candidate else None)
                or metadata.get("v116_arrival_phase_previous_history_offsets")
                != ([60, 79] if candidate else None)
                or metadata.get("v116_arrival_phase_current_history_offsets")
                != ([80, 99] if candidate else None)
                or metadata.get("v116_arrival_phase_comparison")
                != (
                    "current_greater_than_or_equal_to_previous"
                    if candidate
                    else "not_applicable"
                )
                or metadata.get("v116_arrival_phase_windows_adjacent_and_disjoint")
                is not candidate
                or metadata.get("v116_arrival_phase_incomplete_or_overflow_fail_closed")
                is not candidate
                or metadata.get("v116_outcome_fields_drive_policy") is not False
                or metadata.get("v116_scenario_or_burst_label_used_by_policy")
                is not False
                or metadata.get("v116_completion_or_performance_fields_used_by_policy")
                is not False
                or metadata.get("v116_future_arrivals_used_by_policy") is not False
            ):
                raise RuntimeError(f"V116 arm identity changed: {run['run_id']}")
            run_id = run["run_id"]
            summary_path = (
                ROOT
                / "runs"
            ...
            if not summary_path.is_file() or file_hash(summary_path) != admitted.get(
                ...
                raise RuntimeError(f"V116 summary differs from blind audit: {run_id}")
            ...
            if (
                ...
                raise RuntimeError(
                    f"V116 summary identity/completion mismatch: {run_id}"
                )
            rows.append(
                {
                    "arm_id": arm_id,
...
def execute_reveal() -> dict[str, Any]:
    if OUTPUT.exists():
        raise RuntimeError(f"V116 training result already exists: {OUTPUT}")
    if not PLAN.is_file() or file_hash(PLAN) != PLAN_SHA256:
        raise RuntimeError("frozen V116 plan changed before reveal")
    blind = _validate_blind_audit()
    rows = _load_rows(blind)
    if len(rows) != 18:
        raise RuntimeError("V116 revealed row count differs from sealed plan")
    ...
    result = {
        "schema_version": "NSE_E3_ARRIVAL_PHASE_RESULT_V116_V1",
        "created_at": utc_now(),
        ...
        **evaluation,
        "revealed_rows": rows,
        "decision": {
            "authorize_preregistration_of_E1106_E1125_confirmation": joint_pass,
            ...
            "next_action": (
                "freeze V116 with frozen V97 E4 and separately preregister the "
                "complete E1106-E1125 NSESche confirmation before generating inputs"
                if joint_pass
                else (
                    "freeze the complete V116 training cohort, keep E926-E1125 and "
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Reveal the frozen V116 training cohort exactly once after blind audit."
    )
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    if args.command != "reveal" or not args.execute:
        parser.error("V116 reveal requires the explicit command: reveal --execute")
    execute_reveal()
