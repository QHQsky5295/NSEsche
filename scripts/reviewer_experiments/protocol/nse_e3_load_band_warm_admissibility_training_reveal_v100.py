...
OUTPUT = ROOT / "training-result-v100.json"
BLIND_AUDIT_FILE_SHA256 = (
    "016899d95e8055c09ab3397034d7073626875defa9fb2142f6ea7cb078abb574"
)
BLIND_AUDIT_HASH = "6fde99788372decc90bb0d2880a8e546515b0a9b8b44c45fa5b27e59a837fa18"
EXPECTED_SEEDS = ("E869", "E870", "E871")
EXPECTED_SCENARIOS = (
    "E3.spike5x50ms",
    "E3.sustained3x200ms",
    "E3.pulse4x4x50ms",
...
ANCHORS = {"E3": "v100-e3-anchor"}
WARM_ADMISSIBILITY_PREFERENCE = {
    "preserve_anchor_warmness": 0,
    "candidate_warm_only": 1,
}
...
def evaluate_training_rows(
    ...
    for arm_id, arm in ARMS.items():
        ...
        dimension_scores[dimension][arm_id] = {
            "profile": arm["profile"],
            "warm_admissibility": arm["warm_admissibility"],
            "anchor_arm_id": anchor_id,
    ...
    for dimension in ("E3",):
        ...
        ranked = sorted(
            ...
            key=lambda item: (
                ...
                WARM_ADMISSIBILITY_PREFERENCE[item[1]["warm_admissibility"]],
                item[0],
            ),
        )
...
def _validate_blind_audit() -> dict[str, Any]:
    if not BLIND_AUDIT.is_file() or file_hash(BLIND_AUDIT) != BLIND_AUDIT_FILE_SHA256:
        raise RuntimeError("V100 blind audit is missing or changed")
    ...
    if (
        ...
        raise RuntimeError("V100 blind audit does not authorize reveal")
    return blind
...
def _load_rows(blind: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    if len(admitted) != 27:
        raise RuntimeError("V100 blind audit run evidence is incomplete")
    ...
    for arm_id, expected in ARMS.items():
        ...
        if not manifest_path.is_file() or file_hash(manifest_path) != frozen.get(
            ...
            raise RuntimeError(f"V100 frozen manifest changed: {arm_id}")
        manifest = load_and_validate_manifest(manifest_path)
        if manifest.get("manifest_hash") != frozen.get("manifest_hash"):
            raise RuntimeError(f"V100 manifest hash changed: {arm_id}")
        for run in manifest["runs"]:
            ...
            if (
                ...
                != expected["profile"]
                or metadata.get("v100_training_plan_sha256") != PLAN_SHA256
                or metadata.get("v100_training_only") is not True
                or metadata.get("v100_training_seed_metrics_previously_revealed")
                is not False
                or metadata.get("v100_confirmation_seeds_opened") is not False
                or metadata.get("v100_previously_reserved_seeds_opened") is not False
                or metadata.get("v100_arm_id") != arm_id
                or metadata.get("v100_arm_role") != expected["role"]
                or metadata.get("v100_upper_queue_density_threshold")
                != expected["upper_queue_density_threshold"]
                or metadata.get("v100_nonterminal_queue_density_floor")
                != expected["nonterminal_queue_density_floor"]
                or metadata.get("v100_warm_admissibility")
                != expected["warm_admissibility"]
            ):
                raise RuntimeError(f"V100 arm identity changed: {run['run_id']}")
            run_id = run["run_id"]
            summary_path = (
                ROOT
                / "runs"
            ...
            if not summary_path.is_file() or file_hash(summary_path) != admitted.get(
                ...
                raise RuntimeError(f"V100 summary differs from blind audit: {run_id}")
            ...
            if (
                ...
                raise RuntimeError(
                    f"V100 summary identity/completion mismatch: {run_id}"
                )
            rows.append(
                {
                    "arm_id": arm_id,
...
def execute_reveal() -> dict[str, Any]:
    if OUTPUT.exists():
        raise RuntimeError(f"V100 training result already exists: {OUTPUT}")
    if not PLAN.is_file() or file_hash(PLAN) != PLAN_SHA256:
        raise RuntimeError("frozen V100 plan changed before reveal")
    blind = _validate_blind_audit()
    rows = _load_rows(blind)
    if len(rows) != 27:
        raise RuntimeError("V100 revealed row count differs from sealed plan")
    ...
    result = {
        "schema_version": "NSE_E3_LOAD_BAND_WARM_ADMISSIBILITY_RESULT_V100_V1",
        "created_at": utc_now(),
        ...
        "selection_rule": {
            ...
            "multiple_passers": (
                ...
                "throughput change, then prefer warm-preserve over warm-only"
            ),
            "frozen_E4": (
                "retain V97 v97-e4-load-guard64 without V100 result-dependent reselection"
            ),
        ...
        **evaluation,
        "revealed_rows": rows,
        "decision": {
            "authorize_preregistration_of_E906_E925_confirmation": joint_pass,
            ...
            "next_action": (
                ...
                "separately preregister the complete E906-E925 NSESche confirmation "
                "before generating inputs"
                if joint_pass
                else (
                    "freeze the complete V100 training cohort, keep E906-E925 and "
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Reveal the frozen V100 training cohort exactly once after blind audit."
    )
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    if args.command != "reveal" or not args.execute:
        parser.error("V100 reveal requires the explicit command: reveal --execute")
    execute_reveal()
