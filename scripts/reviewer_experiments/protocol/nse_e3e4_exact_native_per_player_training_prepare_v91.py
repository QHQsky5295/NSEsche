...
BINARY = Path("tmp/nse_v91_build_3618ce0/release/serverless_sim.exe")
BINARY_SHA256 = "ece299706c4ce9e2c418c1b71e741faaa7b2deba385ed5d9b0e8e1f00e6d0331"
BINARY_SOURCE_COMMIT = "3618ce02091f1701d98ac5c405365c3c8c088251"
OUTPUT = ROOT / "manifest.v91-exact-native-per-player-training.sla.json"
RECEIPT = ROOT / "prepared-training-v91.json"
TRAINING_SEEDS = {"E713", "E714", "E715"}
CONFIRMATION_SEEDS = ["E716", "E717", "E718"]
E3_PROFILE = "ocs_native_exact_pipeline_per_player_pareto"
E4_PROFILE = "jiagu_native_exact_per_player_pareto"
...
def _assert_inputs() -> None:
    for path, expected in (
        ...
        if not path.is_file() or file_hash(path) != expected:
            raise RuntimeError(
                f"frozen V91 training input is missing or changed: {path}"
            )
...
def _rewrite_manifest(source: dict) -> dict:
    ...
    if manifest.get("formal_results_eligible") is not False:
        raise RuntimeError("V91 training source must remain non-formal")
    if len(manifest.get("runs", [])) != 12:
        raise RuntimeError("V91 training source must contain exactly 12 runs")
    if {run.get("seed") for run in manifest["runs"]} != TRAINING_SEEDS:
        raise RuntimeError("V91 training source seed set changed")
    if {run.get("experiment_id") for run in manifest["runs"]} != {"E3", "E4"}:
        raise RuntimeError("V91 training source experiment set changed")
    if any(run.get("method") != "sche_nash" for run in manifest["runs"]):
        raise RuntimeError("V91 training source contains a non-NSESche run")
    ...
    marker["purpose"] = (
        "V91 training-only exact native initializer and every-player guard on "
        f"already revealed E713-E715; plan_sha256={PLAN_SHA256}"
    )
    marker["v91_training_plan_sha256"] = PLAN_SHA256
    marker["v91_training_only"] = True
    marker["v91_binary_source_commit"] = BINARY_SOURCE_COMMIT
    marker["v91_every_player_certificate"] = True
    ...
    for run in manifest["runs"]:
        ...
        variant = (
            "v91-training-exact-ocs-per-player"
            if run["experiment_id"] == "E3"
            else "v91-training-exact-jiagu-per-player"
        )
        run["variant"] = variant
        run["environment"]["NASH_OPERATIONAL_EXPERT_PROXY"] = profile
        metadata = run.setdefault("metadata", {})
        metadata["v91_training_plan_sha256"] = PLAN_SHA256
        metadata["v91_training_only"] = True
        metadata["v91_training_seed_metrics_previously_revealed"] = True
        metadata["v91_candidate_profile"] = profile
        metadata["v91_candidate_variant"] = variant
        metadata["v91_binary_source_commit"] = BINARY_SOURCE_COMMIT
        metadata["v91_exact_native_initializer"] = True
        metadata["v91_every_player_expert_nonworsening_certificate"] = True
        metadata["v91_resource_scaling_excluded"] = True
    ...
    if len(candidate_reference_keys) != 12:
        raise RuntimeError("V91 must have exactly 12 candidate-specific references")
    if source_reference_keys & candidate_reference_keys:
        raise RuntimeError(
            "V91 environment-bound references unexpectedly reuse source keys"
        )
...
def main() -> None:
    _assert_inputs()
    if ROOT.exists():
        raise RuntimeError(f"refusing to overwrite V91 training root: {ROOT}")
    ROOT.mkdir(parents=True)
    source = json.loads(SOURCE.read_text(encoding="utf-8"))
    manifest = _rewrite_manifest(source)
    write_json_atomic(OUTPUT, manifest)
    receipt = {
        "schema_version": "NSE_E3E4_EXACT_NATIVE_PER_PLAYER_TRAINING_PREPARED_V91_V1",
        "created_at": utc_now(),
