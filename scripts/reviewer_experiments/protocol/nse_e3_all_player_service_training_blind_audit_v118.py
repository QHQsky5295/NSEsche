...
from scripts.reviewer_experiments.protocol.nse_e3_all_player_service_training_prepare_v118 import (
...
PREPARED = ROOT / "prepared-manifests-v118.json"
TAPES = ROOT / "tapes.catalog.json"
OUTPUT = ROOT / "joint-blind-audit-v118-training.json"
RESULT = ROOT / "training-result-v118.json"
...
def _validate_admitted_work_diagnostics(
    ...
    """Validate V118 mechanism evidence without opening any performance summary."""

    path = canonical / "reviewer_records" / run["run_id"] / "nash_metrics.jsonl.gz"
    _require(path.is_file(), f"missing V118 Nash diagnostics: {run['run_id']}")
    ...
    with gzip.open(path, "rt", encoding="utf-8") as handle:
        for line_number, line in enumerate(handle, start=1):
            ...
            _require(type(frame) is int and frame >= 0, "invalid V118 window frame")

            cluster = event.get("cluster")
            _require(isinstance(cluster, dict), "missing V118 cluster diagnostics")
            admitted_fields = (
            ...
            if candidate:
                _require(
                    ...
                    f"V118 admitted-work observation changed: {run['run_id']}:{line_number}",
                )
                ...
                _require(
                    ...
                    f"V118 admitted-work snapshot counts changed: "
                ...
                _require(
                    ...
                    f"V118 admitted-work aggregate changed: {run['run_id']}:{line_number}",
                )
            ...
            _require(
                ...
                f"V118 critical-service contract changed: {run['run_id']}:{line_number}",
            )
            ...
            _require(
                ...
                == (
                    "every_current_player_alternative_less_than_or_equal_to_anchor"
                    if candidate
                ...
                f"V118 componentwise-service contract changed: "
                f"{run['run_id']}:{line_number}",
            )
            for field in (
                "target_player_count",
                ...
                _require(
                    ...
                    f"V118 componentwise {field} changed: "
                    f"{run['run_id']}:{line_number}",
                )
            ...
            _require(
                ...
                f"V118 componentwise acceptance changed: "
                f"{run['run_id']}:{line_number}",
            )
            ...
            _require(
                ...
                f"V118 componentwise reason changed: {run['run_id']}:{line_number}",
            )
            if not componentwise["evaluated"]:
                _require(
                    ...
                    is None,
                    f"V118 unevaluated componentwise evidence was populated: "
                    f"{run['run_id']}:{line_number}",
                )
            ...
            _require(
                ...
                f"V118 componentwise delta changed: {run['run_id']}:{line_number}",
            )
            _require(
                maximum_ratio is None or _finite_nonnegative(maximum_ratio),
                f"V118 componentwise ratio changed: {run['run_id']}:{line_number}",
            )
            if componentwise["evaluated"]:
                _require(
                    componentwise["target_player_count"]
                    == event["decision"]["request_function_players"],
                    f"V118 all-player target coverage changed: "
                    f"{run['run_id']}:{line_number}",
                )
            if componentwise["accepted"] and componentwise["target_player_count"] > 0:
                _require(
                    ...
                    f"V118 accepted componentwise delta is worse: "
                    f"{run['run_id']}:{line_number}",
                )
            _require(
                not critical["accepted"] or componentwise["accepted"],
                f"V118 main certificate bypassed componentwise gate: "
                f"{run['run_id']}:{line_number}",
            )
            ...
            if dominance.get("reason") in componentwise_rejection_reasons:
                _require(
                    ...
                    f"V118 componentwise fallback was not exact anchor: "
                    f"{run['run_id']}:{line_number}",
                )
            ...
            _require(
                ...
                f"V118 re-enabled a failed V114-V116 axis: "
                f"{run['run_id']}:{line_number}",
            )
            for field in (
                ...
                _require(
                    ...
                    f"V118 critical {field} changed: {run['run_id']}:{line_number}",
                )
            _require(
                not critical["accepted"] or critical["evaluated"],
                f"V118 accepted without evaluation: {run['run_id']}:{line_number}",
            )
            if not invoked:
                _require(
                    ...
                    f"V118 non-invoked critical state changed: {run['run_id']}:{line_number}",
                )
            if critical["accepted"]:
                ...
                _require(
                    ...
                    f"V118 accepted evidence is nonfinite: {run['run_id']}:{line_number}",
                )
                ...
                _require(
                    ...
                    f"V118 accepted admitted-work certificate changed: "
                    f"{run['run_id']}:{line_number}",
                )
            ...
            if not candidate:
                _require(
                    ...
                    f"V118 anchor used causal state: {run['run_id']}:{line_number}",
                )
                continue
            if not gate["history_complete"]:
                _require(
                    gate["baseline_count"] == 0 and gate["recent_count"] == 0,
                    f"V118 incomplete history leaked counts: {run['run_id']}:{line_number}",
                )
                continue
            sufficient = (
            ...
            if sufficient and recent_scaled >= baseline_scaled:
                ...
                _require(
                    ...
                    f"V118 exact threshold did not activate: "
                    f"{run['run_id']}:{line_number}",
                )

    _require(
        counts["windows"] == run["simulation"]["total_frame"],
        f"V118 diagnostic window count changed: {run['run_id']}",
    )
    if candidate:
        _require(
            ...
            f"V118 mechanism was not exercised: {run['run_id']}",
        )
    return {
...
def _verify_tape_catalog() -> tuple[list[dict[str, Any]], dict[str, Any]]:
    _require(TAPES.is_file(), f"missing V118 tape catalog: {TAPES}")
    catalog = read_json(TAPES)
    catalog_hash = _assert_hashed_object(catalog, "catalog_hash", "V118 tapes")
    entries = catalog.get("entries")
    _require(
        isinstance(entries, dict) and len(entries) == 12, "V118 tape count changed"
    )
    evidence = []
...
def _verify_reference_stage_ledgers() -> tuple[dict[str, Path], dict[Path, str]]:
    ...
    for arm_id, expected in ARMS.items():
        ...
        root = _stage_root_from_receipts(
            entries, "receipt_path", expected["run_count"], f"V118 reference {arm_id}"
        )
        roots[arm_id] = root
    ...
    for root, expected_count in expected_by_root.items():
        ...
        _assert_ledger_contract(
            ...
            f"V118 reference stage {root}",
        )
        _require(
            not list((root / "quarantine").glob("**/attempt-*")),
            f"V118 reference quarantine is nonempty: {root}",
        )
...
def _validate_run_boundary(
    ...
    required = {
        "v118_training_plan_sha256": PLAN_SHA256,
        "v118_training_only": True,
        "v118_training_seed_metrics_previously_revealed": False,
        "v118_confirmation_seeds_opened": False,
        "v118_other_unopened_seeds_opened": False,
        "v118_formal_E01_E20_reexecution": False,
        "v118_arm_id": arm_id,
        "v118_arm_role": expected["role"],
        "v118_candidate_profile": expected["profile"],
        "v118_candidate_experiment": "E3",
        "v118_shock_rate_ratio": expected["shock_rate_ratio"],
        "v118_shock_threshold_numerator": expected["shock_threshold_numerator"],
        "v118_shock_threshold_denominator": expected["shock_threshold_denominator"],
        "v118_arrival_history_baseline_frames": 80 if candidate else None,
        "v118_arrival_history_recent_frames": 20 if candidate else None,
        "v118_arrival_min_requests_per_window": 20 if candidate else None,
        "v118_shock_activation_horizon_frames": expected[
            "shock_activation_horizon_frames"
        ],
        "v118_nonterminal_queue_density_floor": 8.0 if candidate else None,
        "v118_warm_admissibility": "preserve_anchor_warmness" if candidate else None,
        "v118_load_least_window_certificate_mode": (
            "disabled" if candidate else "not_applicable"
        ),
        "v118_critical_service_ratio_numerator": expected[
            "critical_service_threshold_numerator"
        ],
        "v118_critical_service_ratio_denominator": expected[
            "critical_service_threshold_denominator"
        ],
        "v118_critical_service_proxy": (
            "remote_parent_transfer_plus_cold_start_plus_admitted_queue_cpu_work"
            if candidate
            else "not_applicable"
        ),
        "v118_service_proxy_work_source": (
            ADMITTED_WORK_SOURCE if candidate else "not_applicable"
        ),
        "v118_admitted_work_includes_all_blocked_resident": candidate,
        "v118_admitted_work_deterministic_f64_sum": candidate,
        "v118_complete_componentwise_service_pareto": candidate,
        "v118_componentwise_service_scope": (
            "all_current_players" if candidate else "not_applicable"
        ),
        "v118_componentwise_service_comparison": (
            "every_current_player_alternative_proxy_lte_anchor_plus_epsilon"
            if candidate
            else "not_applicable"
        ),
        "v118_componentwise_service_replay_order": (
            "native_player_order" if candidate else "not_applicable"
        ),
        "v118_componentwise_service_inputs_finite_fail_closed": candidate,
        "v118_componentwise_service_coverage_mismatch_fail_closed": candidate,
        "v118_interference_constraints": False,
        "v118_arrival_phase_guard": False,
        "v118_critical_service_proxy_inputs_finite_fail_closed": candidate,
        "v118_cpu_memory_individual_noninferiority": candidate,
        "v118_scalar_faasrank_noninferiority": candidate,
        "v118_input_locality_component_noninferiority": candidate,
        "v118_per_child_current_warm_downstream_locality_noninferiority": candidate,
        "v118_critical_frontier_substitution": candidate,
        "v118_critical_frontier_rank_source": (
            "immutable_srpt_remaining_critical_path_rank"
            if candidate
            else "not_applicable"
        ),
        "v118_noncritical_exact_anchor": candidate,
        "v118_complete_summed_critical_service_proxy_strictly_lower": candidate,
        "v118_complete_routed_score_nonworse": candidate,
        "v118_complete_exact_ocs_score_nonworse": candidate,
        "v118_complete_immutable_baseline_welfare_nonworse": candidate,
        "v118_outcome_fields_drive_policy": False,
        "v118_scenario_or_burst_label_used_by_policy": False,
        "v118_completion_or_performance_fields_used_by_policy": False,
        "v118_future_arrivals_used_by_policy": False,
    }
    _require(
        ...
        f"V118 run boundary changed: {run['run_id']}",
    )


def run_blind_audit(output: Path = OUTPUT) -> dict[str, Any]:
    _require(not output.exists(), f"V118 blind audit already exists: {output}")
    _require(not RESULT.exists(), "V118 reveal exists before blind audit")
    _require(PLAN.is_file() and file_hash(PLAN) == PLAN_SHA256, "V118 plan changed")
    plan = read_json(PLAN)
    _require(plan.get("formal_results_eligible") is False, "V118 eligibility changed")
    _require(PREPARED.is_file(), f"missing V118 prepared receipt: {PREPARED}")
    prepared = read_json(PREPARED)
    prepared_hash = _assert_hashed_object(
        prepared, "receipt_hash", "V118 prepared receipt"
    )
    _require(
        ...
        "V118 prepared scientific boundary changed",
    )
    ...
    for arm_id, expected in ARMS.items():
        ...
        _require(paths["ready"].is_file(), f"missing V118 ready manifest: {arm_id}")
        ...
        _require(
            ...
            f"V118 ready boundary changed: {arm_id}",
        )
        ...
        reference_catalog_hash = _assert_hashed_object(
            references, "catalog_hash", f"V118 references {arm_id}"
        )
        ...
        _require(
            ...
            f"V118 reference key set changed: {arm_id}",
        )
        for key, entry in sorted(entries.items()):
            ...
            _require(
                ...
                f"V118 reference evidence changed: {key}",
            )
        ...
        _require(actual_ids == expected_ids, f"V118 canonical set changed: {arm_id}")
        _require(
            not list((workspace / "quarantine").glob("**/attempt-*")),
            f"V118 online quarantine is nonempty: {arm_id}",
        )
        ...
        for run in manifest["runs"]:
            ...
            _require(
                ...
                f"V118 canonical status changed: {run['run_id']}",
            )
            runtime = {
    ...
    _require(len(run_evidence) == 18, "V118 run evidence count must be 18")
    _require(len(reference_evidence) == 18, "V118 reference evidence count must be 18")
    _require(len(tape_evidence) == 12, "V118 tape evidence count must be 12")
    ...
    _require(
        ...
        > 0,
        "V118 componentwise certificate did not exercise both accept and reject paths",
    )
    _require(
        sum(
        ...
        "V118 all-player componentwise evidence is unavailable or incomplete",
    )
    _require(
        all(
        ...
        "V118 anchor unexpectedly exercised the componentwise certificate",
    )
    for field, expected in EXPECTED_RUNTIME.items():
        _require(
            runtime_values[field] == {expected},
            f"V118 runtime {field} changed: {runtime_values[field]}",
        )
    ...
    _require(
        ...
        f"V118 runtime git identity is not singular: {git_commits}",
    )
    _require(len(paired_inputs) == 9, "V118 paired input product changed")
    for (scenario, seed), rows in paired_inputs.items():
        _require(len(rows) == 2, f"V118 paired arm count changed: {scenario}/{seed}")
        for field in (
            ...
            _require(
                ...
                f"V118 paired {field} changed: {scenario}/{seed}",
            )
        _require(
            rows[0]["common_hpa_hash"] == EXPECTED_COMMON_HPA_SHA256,
            f"V118 common HPA changed: {scenario}/{seed}",
        )

    output_payload = {
        "schema_version": "NSE_E3_ALL_PLAYER_SERVICE_BLIND_AUDIT_V118_V1",
        "created_at": utc_now(),
        "status": "pass",
