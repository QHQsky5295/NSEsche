...
PREPARED = ROOT / "prepared-manifests-v127.json"
TAPES = ROOT / "tapes.catalog.json"
OUTPUT = ROOT / "joint-blind-audit-v127-training.json"
RESULT = ROOT / "training-result-v127.json"
...
def _validate_admitted_work_diagnostics(
    ...
    """Validate V127 mechanism evidence without opening any performance summary."""

    path = canonical / "reviewer_records" / run["run_id"] / "nash_metrics.jsonl.gz"
    _require(path.is_file(), f"missing V127 Nash diagnostics: {run['run_id']}")
    ...
    with gzip.open(path, "rt", encoding="utf-8") as handle:
        for line_number, line in enumerate(handle, start=1):
            ...
            _require(type(frame) is int and frame >= 0, "invalid V127 window frame")

            cluster = event.get("cluster")
            _require(isinstance(cluster, dict), "missing V127 cluster diagnostics")
            admitted_fields = (
            ...
            if candidate:
                _require(
                    ...
                    f"V127 admitted-work observation changed: {run['run_id']}:{line_number}",
                )
                ...
                _require(
                    ...
                    f"V127 admitted-work snapshot counts changed: "
                ...
                _require(
                    ...
                    f"V127 admitted-work aggregate changed: {run['run_id']}:{line_number}",
                )
            ...
            _require(
                ...
                f"V127 critical-service contract changed: {run['run_id']}:{line_number}",
            )
            ...
            _require(
                ...
                f"V127 componentwise-service contract changed: "
                f"{run['run_id']}:{line_number}",
            )
            for field in (
                "target_player_count",
                ...
                _require(
                    ...
                    f"V127 componentwise {field} changed: "
                    f"{run['run_id']}:{line_number}",
                )
            ...
            _require(
                ...
                f"V127 componentwise acceptance changed: "
                f"{run['run_id']}:{line_number}",
            )
            ...
            _require(
                ...
                f"V127 componentwise reason changed: {run['run_id']}:{line_number}",
            )
            ...
            _require(
                ...
                f"V127 componentwise delta changed: {run['run_id']}:{line_number}",
            )
            _require(
                maximum_ratio is None or _finite_nonnegative(maximum_ratio),
                f"V127 componentwise ratio changed: {run['run_id']}:{line_number}",
            )
            if componentwise["accepted"] and componentwise["target_player_count"] > 0:
                _require(
                    ...
                    f"V127 accepted componentwise delta is worse: "
                    f"{run['run_id']}:{line_number}",
                )
            ...
            _require(
                ...
                f"V127 terminal-child path contract changed: "
                f"{run['run_id']}:{line_number}",
            )
            for field in (
                ...
                _require(
                    ...
                    f"V127 terminal-child path {field} changed: "
                    f"{run['run_id']}:{line_number}",
                )
            _require(
                ...
                f"V127 terminal-child fail-closed reason accounting changed: "
                f"{run['run_id']}:{line_number}",
            )
            ...
            _require(
                ...
                f"V127 terminal-child path acceptance changed: "
                f"{run['run_id']}:{line_number}",
            )
            ...
            _require(
                ...
                == expected_terminal_child_path_reason,
                f"V127 terminal-child path reason changed: "
                f"{run['run_id']}:{line_number}",
            )
            ...
            _require(
                ...
                f"V127 terminal-child path delta changed: "
                f"{run['run_id']}:{line_number}",
            )
            _require(
                terminal_maximum_ratio is None
                or _finite_nonnegative(terminal_maximum_ratio),
                f"V127 terminal-child path ratio changed: "
                f"{run['run_id']}:{line_number}",
            )
            if (
                ...
                _require(
                    ...
                    f"V127 accepted terminal-child path is worse: "
                    f"{run['run_id']}:{line_number}",
                )
            _require(
                not critical["accepted"]
                or (componentwise["accepted"] and terminal_child_path["accepted"]),
                f"V127 main certificate bypassed a final service gate: "
                f"{run['run_id']}:{line_number}",
            )
            ...
            if dominance.get("reason") in componentwise_rejection_reasons:
                _require(
                    ...
                    f"V127 componentwise fallback was not exact anchor: "
                    f"{run['run_id']}:{line_number}",
                )
            ...
            if dominance.get("reason") in terminal_child_path_rejection_reasons:
                _require(
                    ...
                    and dominance.get("selected_initializer_assignment_hash")
                    == dominance.get("anchor_assignment_hash"),
                    f"V127 terminal-child path fallback was not exact anchor: "
                    f"{run['run_id']}:{line_number}",
                )
            for field in (
                ...
                _require(
                    ...
                    f"V127 critical {field} changed: {run['run_id']}:{line_number}",
                )
            _require(
                not critical["accepted"] or critical["evaluated"],
                f"V127 accepted without evaluation: {run['run_id']}:{line_number}",
            )
            ...
            _require(
                ...
                f"V127 completion-proximal construction contract changed: "
                f"{run['run_id']}:{line_number}",
            )
            ...
            _require(
                ...
                f"V127 completion-proximal scope diagnostics changed: "
                f"{run['run_id']}:{line_number}",
            )
            proximal_fields = (
            ...
            _require(
                ...
                f"V127 completion-proximal scope counter changed: "
                f"{run['run_id']}:{line_number}",
            )
            if invoked:
                _require(
                    ...
                    f"V127 completion-proximal substitution invariant changed: "
                    f"{run['run_id']}:{line_number}",
                )
            else:
                _require(
                    all(proximal_scope[field] == 0 for field in proximal_fields),
                    f"V127 inactive completion-proximal scope leaked counters: "
                    f"{run['run_id']}:{line_number}",
                )
            if not invoked:
                _require(
                    ...
                    f"V127 non-invoked critical state changed: {run['run_id']}:{line_number}",
                )
            if critical["accepted"]:
                ...
                _require(
                    ...
                    f"V127 accepted evidence is nonfinite: {run['run_id']}:{line_number}",
                )
                ...
                _require(
                    ...
                    f"V127 accepted admitted-work certificate changed: "
                    f"{run['run_id']}:{line_number}",
                )
            ...
            if not candidate:
                _require(
                    ...
                    f"V127 anchor used causal state: {run['run_id']}:{line_number}",
                )
                continue
            if not gate["history_complete"]:
                _require(
                    gate["baseline_count"] == 0 and gate["recent_count"] == 0,
                    f"V127 incomplete history leaked counts: {run['run_id']}:{line_number}",
                )
                continue
            sufficient = (
            ...
            if sufficient and recent_scaled >= baseline_scaled:
                ...
                _require(
                    ...
                    f"V127 exact threshold did not activate: "
                    f"{run['run_id']}:{line_number}",
                )

    _require(
        counts["windows"] == run["simulation"]["total_frame"],
        f"V127 diagnostic window count changed: {run['run_id']}",
    )
    if candidate:
        _require(
            ...
            f"V127 mechanism was not exercised: {run['run_id']}",
        )
    return {
...
def _verify_tape_catalog() -> tuple[list[dict[str, Any]], dict[str, Any]]:
    _require(TAPES.is_file(), f"missing V127 tape catalog: {TAPES}")
    catalog = read_json(TAPES)
    catalog_hash = _assert_hashed_object(catalog, "catalog_hash", "V127 tapes")
    entries = catalog.get("entries")
    _require(
        isinstance(entries, dict) and len(entries) == 12, "V127 tape count changed"
    )
    evidence = []
...
def _verify_reference_stage_ledgers() -> tuple[dict[str, Path], dict[Path, str]]:
    ...
    for arm_id, expected in ARMS.items():
        ...
        root = _stage_root_from_receipts(
            entries, "receipt_path", expected["run_count"], f"V127 reference {arm_id}"
        )
        roots[arm_id] = root
    ...
    for root, expected_count in expected_by_root.items():
        ...
        _assert_ledger_contract(
            ...
            f"V127 reference stage {root}",
        )
        _require(
            not list((root / "quarantine").glob("**/attempt-*")),
            f"V127 reference quarantine is nonempty: {root}",
        )
...
def _validate_run_boundary(
    ...
    required = {
        "v127_training_plan_sha256": PLAN_SHA256,
        "v127_training_only": True,
        "v127_training_seed_metrics_previously_revealed": False,
        "v127_confirmation_seeds_opened": False,
        "v127_other_unopened_seeds_opened": False,
        "v127_formal_E01_E20_reexecution": False,
        "v127_arm_id": arm_id,
        "v127_arm_role": expected["role"],
        "v127_candidate_profile": expected["profile"],
        "v127_candidate_experiment": "E3",
        "v127_shock_rate_ratio": expected["shock_rate_ratio"],
        "v127_shock_threshold_numerator": expected["shock_threshold_numerator"],
        "v127_shock_threshold_denominator": expected["shock_threshold_denominator"],
        "v127_arrival_history_baseline_frames": 80 if candidate else None,
        "v127_arrival_history_recent_frames": 20 if candidate else None,
        "v127_arrival_min_requests_per_window": 20 if candidate else None,
        "v127_shock_activation_horizon_frames": expected[
            "shock_activation_horizon_frames"
        ],
        "v127_nonterminal_queue_density_floor": 8.0 if candidate else None,
        "v127_warm_admissibility": "preserve_anchor_warmness" if candidate else None,
        "v127_load_least_window_certificate_mode": (
            "disabled" if candidate else "not_applicable"
        ),
        "v127_critical_service_ratio_numerator": expected[
            "critical_service_threshold_numerator"
        ],
        "v127_critical_service_ratio_denominator": expected[
            "critical_service_threshold_denominator"
        ],
        "v127_critical_service_proxy": (
            "remote_parent_transfer_plus_cold_start_plus_admitted_queue_cpu_work"
            if candidate
            else "not_applicable"
        ),
        "v127_service_proxy_work_source": (
            ADMITTED_WORK_SOURCE if candidate else "not_applicable"
        ),
        "v127_admitted_work_includes_all_blocked_resident": candidate,
        "v127_admitted_work_deterministic_f64_sum": candidate,
        "v127_critical_service_proxy_inputs_finite_fail_closed": candidate,
        "v127_cpu_memory_individual_noninferiority": candidate,
        "v127_scalar_faasrank_noninferiority": candidate,
        "v127_input_locality_component_noninferiority": candidate,
        "v127_per_child_current_warm_downstream_locality_noninferiority": candidate,
        "v127_critical_frontier_substitution": candidate,
        "v127_critical_frontier_rank_source": (
            "immutable_srpt_remaining_critical_path_rank"
            if candidate
            else "not_applicable"
        ),
        "v127_completion_proximal_depth1_substitution_scope": candidate,
        "v127_completion_proximal_definition": (
            "terminal_function_or_nonterminal_function_whose_immutable_DAG_children_are_all_terminal"
            if candidate
            else "not_applicable"
        ),
        "v127_maximum_remaining_child_depth": 1 if candidate else None,
        "v127_completion_proximal_critical_frontier_substitution": candidate,
        "v127_deeper_player_exact_anchor": candidate,
        "v127_complete_componentwise_service_pareto": candidate,
        "v127_componentwise_service_scope": (
            "all_current_critical_frontier_players" if candidate else "not_applicable"
        ),
        "v127_componentwise_service_comparison": (
            "every_current_critical_player_alternative_less_than_or_equal_to_anchor"
            if candidate
            else "not_applicable"
        ),
        "v127_componentwise_service_replay_order": (
            "frozen_native_player_order_with_independent_assignment_prefixes"
            if candidate
            else "not_applicable"
        ),
        "v127_complete_terminal_child_hpa_container_path_service_pareto": candidate,
        "v127_terminal_child_hpa_container_path_scope": (
            "every_immutable_immediate_child_of_each_actually_changed_completion_proximal_player"
            if candidate
            else "not_applicable"
        ),
        "v127_terminal_child_hpa_container_path_proxy": (
            "parent_output_transfer_plus_full_cold_start_if_starting_plus_child_node_admitted_cpu_service"
            if candidate
            else "not_applicable"
        ),
        "v127_terminal_child_hpa_container_node_scope": (
            "all_current_HPA_owned_child_containers_running_or_starting"
            if candidate
            else "not_applicable"
        ),
        "v127_terminal_child_hpa_container_path_comparison": (
            "every_alternative_terminal_child_hpa_container_path_less_than_or_equal_to_anchor"
            if candidate
            else "not_applicable"
        ),
        "v127_terminal_child_hpa_container_path_input_fail_closed": candidate,
        "v127_complete_summed_critical_service_proxy_strictly_lower": candidate,
        "v127_complete_routed_score_nonworse": candidate,
        "v127_complete_exact_ocs_score_nonworse": candidate,
        "v127_complete_immutable_baseline_welfare_nonworse": candidate,
        "v127_outcome_fields_drive_policy": False,
        "v127_scenario_or_burst_label_used_by_policy": False,
        "v127_completion_or_performance_fields_used_by_policy": False,
        "v127_future_arrivals_used_by_policy": False,
    }
    _require(
        ...
        f"V127 run boundary changed: {run['run_id']}",
    )


def run_blind_audit(output: Path = OUTPUT) -> dict[str, Any]:
    _require(not output.exists(), f"V127 blind audit already exists: {output}")
    _require(not RESULT.exists(), "V127 reveal exists before blind audit")
    _require(PLAN.is_file() and file_hash(PLAN) == PLAN_SHA256, "V127 plan changed")
    plan = read_json(PLAN)
    _require(plan.get("formal_results_eligible") is False, "V127 eligibility changed")
    _require(PREPARED.is_file(), f"missing V127 prepared receipt: {PREPARED}")
    prepared = read_json(PREPARED)
    prepared_hash = _assert_hashed_object(
        prepared, "receipt_hash", "V127 prepared receipt"
    )
    _require(
        ...
        "V127 prepared scientific boundary changed",
    )
    ...
    for arm_id, expected in ARMS.items():
        ...
        _require(paths["ready"].is_file(), f"missing V127 ready manifest: {arm_id}")
        ...
        _require(
            ...
            f"V127 ready boundary changed: {arm_id}",
        )
        ...
        reference_catalog_hash = _assert_hashed_object(
            references, "catalog_hash", f"V127 references {arm_id}"
        )
        ...
        _require(
            ...
            f"V127 reference key set changed: {arm_id}",
        )
        for key, entry in sorted(entries.items()):
            ...
            _require(
                ...
                f"V127 reference evidence changed: {key}",
            )
        ...
        _require(actual_ids == expected_ids, f"V127 canonical set changed: {arm_id}")
        _require(
            not list((workspace / "quarantine").glob("**/attempt-*")),
            f"V127 online quarantine is nonempty: {arm_id}",
        )
        ...
        for run in manifest["runs"]:
            ...
            _require(
                ...
                f"V127 canonical status changed: {run['run_id']}",
            )
            runtime = {
    ...
    _require(len(run_evidence) == 18, "V127 run evidence count must be 18")
    _require(len(reference_evidence) == 18, "V127 reference evidence count must be 18")
    _require(len(tape_evidence) == 12, "V127 tape evidence count must be 12")
    for field, expected in EXPECTED_RUNTIME.items():
        _require(
            runtime_values[field] == {expected},
            f"V127 runtime {field} changed: {runtime_values[field]}",
        )
    ...
    _require(
        ...
        f"V127 runtime git identity is not singular: {git_commits}",
    )
    _require(len(paired_inputs) == 9, "V127 paired input product changed")
    for (scenario, seed), rows in paired_inputs.items():
        _require(len(rows) == 2, f"V127 paired arm count changed: {scenario}/{seed}")
        for field in (
            ...
            _require(
                ...
                f"V127 paired {field} changed: {scenario}/{seed}",
            )
        _require(
            rows[0]["common_hpa_hash"] == EXPECTED_COMMON_HPA_SHA256,
            f"V127 common HPA changed: {scenario}/{seed}",
        )
    ...
    _require(
        ...
        "V127 completion-proximal and deeper current-player paths were not both exercised",
    )
    _require(
        completion_proximal_service_choice_evaluated_count > 0
        and completion_proximal_substitution_count > 0,
        "V127 completion-proximal service-choice evaluation/substitution path was not exercised",
    )
    _require(
        deeper_exact_anchor_preserved_count > 0 and deeper_substitution_count == 0,
        "V127 deeper-player exact-anchor preservation contract changed",
    )
    _require(
        proxy_input_unavailable_count == 0,
        "V127 admitted-work proxy inputs were unavailable",
    )
    _require(
        ...
        "V127 componentwise service accept/reject paths were not both exercised",
    )
    _require(
        componentwise_input_unavailable_count == 0,
        "V127 componentwise service inputs were unavailable",
    )
    _require(
        ...
        "V127 terminal-child path accept/reject paths were not both exercised",
    )
    _require(
        terminal_child_path_input_unavailable_count == 0,
        "V127 terminal-child path inputs were unavailable",
    )
    _require(
        ...
        "V127 terminal-child HPA path fail-closed reason count was nonzero",
    )
    _require(
        terminal_child_path_nonterminal_child_count == 0,
        "V127 terminal-child path scope included a nonterminal child",
    )

    output_payload = {
        "schema_version": "NSE_E3_COMPLETION_PROXIMAL_TERMINAL_CHILD_HPA_PATH_BLIND_AUDIT_V127_V1",
        "created_at": utc_now(),
        "status": "pass",
        ...
        "terminal_child_path_mechanism_coverage": {
            "terminal_child_path_gate_evaluated_count": terminal_child_path_gate_evaluated_count,
            "terminal_child_path_changed_parent_count": terminal_child_path_changed_parent_count,
            "terminal_child_path_compared_child_count": terminal_child_path_compared_child_count,
            "terminal_child_path_acceptance_count": terminal_child_path_acceptance_count,
            "terminal_child_path_rejection_count": terminal_child_path_rejection_count,
            "terminal_child_path_input_unavailable_count": terminal_child_path_input_unavailable_count,
            "terminal_child_path_running_container_count": terminal_child_path_running_container_count,
            "terminal_child_path_starting_container_count": terminal_child_path_starting_container_count,
            "terminal_child_path_no_container_count": terminal_child_path_no_container_count,
            "terminal_child_path_invalid_profile_count": terminal_child_path_invalid_profile_count,
            "terminal_child_path_invalid_node_snapshot_count": terminal_child_path_invalid_node_snapshot_count,
            "terminal_child_path_invalid_network_count": terminal_child_path_invalid_network_count,
            "terminal_child_path_nonterminal_child_count": terminal_child_path_nonterminal_child_count,
            "terminal_child_path_worse_child_count": terminal_child_path_worse_child_count,
            "performance_fields_consulted": False,
        },
        "arm_count": len(ARMS),
