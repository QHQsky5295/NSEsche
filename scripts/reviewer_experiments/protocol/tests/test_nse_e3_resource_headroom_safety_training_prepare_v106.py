...
class ResourceHeadroomSafetyPreparationV106Tests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.temporary = tempfile.TemporaryDirectory()
        cls.root = Path(cls.temporary.name) / "v106"
        cls.receipt = prepare_v106(cls.root)
        cls.paths = _paths(cls.root)
        cls.arms = {
            arm_id: json.loads(arm_path(cls.root, arm_id).read_text(encoding="utf-8"))
            for arm_id, _, _, _, _, _, _ in ARMS
        }
    ...
    def test_exact_arms_profiles_density_bounds_and_runtime(self) -> None:
        expected = {
            ...
            for arm_id, experiment_id, role, profile, upper_density, resource_headroom_safety_mode, count in ARMS
        }
        ...
        for arm_id, manifest in self.arms.items():
            ...
            self.assertEqual(
                {
                    run["metadata"]["v106_upper_queue_density_threshold"]
                    for run in manifest["runs"]
                },
                {upper_density},
            )
            self.assertTrue(
                all(
                    run["metadata"]["v106_nonterminal_queue_density_floor"]
                    == (8.0 if role == "candidate" else None)
                    and run["metadata"]["v106_warm_admissibility"]
                    == ("preserve_anchor_warmness" if role == "candidate" else None)
                    and run["metadata"]["v106_density_band_initializer_guard"]
                    is (role == "candidate")
                    and run["metadata"]["v106_load_least_window_certificate_mode"]
                    == ("disabled" if role == "candidate" else "not_applicable")
                    and run["metadata"]["v106_resource_headroom_safety_mode"]
                    == resource_headroom_safety_mode
                    and run["metadata"]["v106_cpu_memory_individual_noninferiority"]
                    is (
                        resource_headroom_safety_mode
                        == "cpu_and_memory_headroom_each_nonworse"
                    )
                    and run["metadata"]["v106_resource_bottleneck_sum_noninferiority"]
                    is (
                        resource_headroom_safety_mode
                        == "resource_bottleneck_and_total_headroom_each_nonworse"
                    )
                    and run["metadata"]["v106_resource_inputs_finite_fail_closed"]
                    is (role == "candidate")
                    and run["metadata"]["v106_scalar_faasrank_noninferiority"]
                    is (role == "candidate")
                    and run["metadata"]["v106_input_locality_component_noninferiority"]
                    is (role == "candidate")
                    and run["metadata"]["v106_componentwise_faasrank_noninferiority"]
                    is False
                    and run["metadata"][
                    ...
                    is False
                    and run["metadata"][
                        "v106_future_child_placement_or_feasibility_used"
                    ]
                    is False
                    and run["metadata"]["v106_critical_frontier_protection"]
                    is (role == "candidate")
                    and run["metadata"]["v106_critical_frontier_rank_source"]
                    == (
                        "immutable_srpt_remaining_critical_path_rank"
                        if role == "candidate"
                        else "not_applicable"
                    )
                    and run["metadata"]["v106_critical_frontier_tie_rule"]
                    == (
                    ...
                    is (role == "candidate")
                    and run["metadata"]["v106_outcome_fields_drive_policy"] is False
                    and run["metadata"]["v106_substitution_cap"] is None
                    and run["metadata"]["v106_confirmation_seeds_opened"] is False
                    and run["metadata"]["v106_other_unopened_seeds_opened"] is False
    ...
    def test_plan_is_frozen_and_matches_exact_seed_boundary(self) -> None:
        ...
        self.assertEqual(
            PREVIOUS_CONFIRMATION_SEEDS, [f"E{i}" for i in range(926, 1006)]
        )
        ...
        self.assertEqual(
            ...
            },
            {
                ...
                for arm_id, _, role, profile, _, _, count in ARMS
            },
        )
        self.assertFalse(
