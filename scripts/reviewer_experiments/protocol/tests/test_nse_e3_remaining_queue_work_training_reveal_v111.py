...
import scripts.reviewer_experiments.protocol.nse_e3_remaining_queue_work_training_reveal_v111 as reveal_module
from scripts.reviewer_experiments.protocol.nse_e3_remaining_queue_work_training_reveal_v111 import (
    ARMS,
    CANDIDATE_ID,
    EXPECTED_SCENARIOS,
    EXPECTED_SEEDS,
...
class RemainingQueueWorkRevealV111Tests(unittest.TestCase):
    ...
    def test_single_candidate_must_pass_all_nine_frozen_gates(self) -> None:
        ...
        self.assertFalse(diagnostic["used_for_V111_selection"])
    ...
    def test_one_large_outlier_cannot_satisfy_two_of_three_rule(self) -> None:
        ...
        for row in rows:
            if row["arm_id"] == CANDIDATE_ID and row["scenario_id"] == "E3.spike5x50ms":
                value = 10.0 if row["seed"] == "E904" else 0.9
