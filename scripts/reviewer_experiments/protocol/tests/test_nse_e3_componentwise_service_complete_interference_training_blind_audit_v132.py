...
def _critical(
    ...
    return {
        ...
        "admitted_interference_pareto": {
            ...
            "input_unavailable_count": 0,
            "anchor_sum": 4.0 if enabled else None,
            "alternative_sum": 3.5 if enabled else None,
            "alternative_minus_anchor": -0.5 if enabled else None,
            "source": ADMITTED_INTERFERENCE_SOURCE if enabled else "not_applicable",
            "uses_completion_outcomes": False,
        },
...
class ComponentwiseServiceCompleteInterferenceBlindAuditV132Tests(unittest.TestCase):
    def test_frozen_arm_seed_and_runtime_boundary(self) -> None:
        self.assertEqual(TRAINING_SEEDS, {"E1379", "E1380", "E1381"})
        self.assertEqual(CONFIRMATION_SEEDS, [f"E{i}" for i in range(1382, 1402)])
        self.assertEqual(
            PREVIOUS_CONFIRMATION_SEEDS,
    ...
    def test_diagnostics_require_admitted_snapshot_and_ratio_certificate(
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual(evidence["admitted_interference_finite_window_count"], 2)
            self.assertEqual(evidence["admitted_interference_accepted_window_count"], 1)
            self.assertEqual(evidence["admitted_interference_nonworse_window_count"], 2)
            ...
            windows[2]["decision"]["load_least_dominance_gate"][
                ...
                {"alternative_sum": 4.1, "alternative_minus_anchor": 0.1}
            )
            with gzip.open(path, "wt", encoding="utf-8") as handle:
                for event in windows:
                    handle.write(json.dumps(event) + "\n")
            with self.assertRaisesRegex(
                RuntimeError, "accepted worse complete interference"
            ):
                _validate_admitted_work_diagnostics(
                    {"run_id": run_id, "simulation": {"total_frame": 3}},
                    canonical,
