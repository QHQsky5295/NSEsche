...
class FinalMarginPreparationV121Tests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.temporary = tempfile.TemporaryDirectory()
        cls.root = Path(cls.temporary.name) / "v121"
        cls.receipt = prepare_v121(cls.root)
    ...
    def test_two_arms_lock_exact_profiles_runtime_and_seed_product(self) -> None:
        ...
        for arm_id, role, profile, count in ARMS:
            ...
            for run in manifest["runs"]:
                ...
                self.assertEqual(metadata["v121_arm_id"], arm_id)
                self.assertEqual(metadata["v121_arm_role"], role)
                self.assertEqual(
                    metadata["v121_shock_activation_horizon_frames"],
                    50 if candidate else None,
                )
                self.assertEqual(
                    metadata["v121_critical_service_ratio_numerator"],
                    9 if candidate else None,
                )
                self.assertEqual(
                    metadata["v121_critical_service_ratio_denominator"],
                    10 if candidate else None,
                )
                self.assertEqual(
                    metadata["v121_service_proxy_work_source"],
                    (
                ...
                self.assertIs(
                    metadata["v121_admitted_work_includes_all_blocked_resident"],
                    candidate,
                )
                self.assertIs(
                    metadata["v121_complete_componentwise_service_pareto"],
                    candidate,
                )
                self.assertEqual(
                    metadata["v121_componentwise_service_scope"],
                    "all_current_players" if candidate else "not_applicable",
                )
                self.assertEqual(
                    metadata["v121_componentwise_service_comparison"],
                    (
                ...
                self.assertEqual(
                    metadata["v121_componentwise_service_replay_order"],
                    "native_player_order" if candidate else "not_applicable",
                )
                self.assertIs(metadata["v121_service_directed_construction"], candidate)
                self.assertEqual(
                    metadata["v121_service_choice_scope"],
                    "all_current_players" if candidate else "not_applicable",
                )
                self.assertIs(metadata["v121_dual_prefix_anchor_enabled"], candidate)
                self.assertEqual(
                    metadata["v121_anchor_service_baseline_state"],
                    (
                ...
                self.assertEqual(
                    metadata["v121_candidate_service_state"],
                    (
                ...
                self.assertIs(
                    metadata["v121_completed_changed_player_margin_enabled"],
                    candidate,
                )
                self.assertEqual(
                    metadata["v121_completed_changed_player_margin_numerator"],
                    9 if candidate else None,
                )
                self.assertEqual(
                    metadata["v121_completed_changed_player_margin_denominator"],
                    10 if candidate else None,
                )
                self.assertEqual(
                    metadata["v121_completed_changed_player_definition"],
                    (
                ...
                self.assertFalse(metadata["v121_noncritical_exact_anchor"])
                self.assertFalse(metadata["v121_interference_constraints"])
                self.assertFalse(metadata["v121_arrival_phase_guard"])
                self.assertFalse(metadata["v121_outcome_fields_drive_policy"])
    ...
    def test_plan_is_frozen_and_seed_boundaries_remain_sealed(self) -> None:
        ...
        self.assertEqual(plan["paired_design"]["online_runs_per_arm"] * 2, 18)
        self.assertEqual(plan["paired_design"]["reference_builds_per_arm"] * 2, 18)
        self.assertEqual(
            plan["paired_design"]["candidate_profile"],
            ARMS[1][2],
        )

    def test_existing_root_is_never_overwritten(self) -> None:
        with self.assertRaisesRegex(RuntimeError, "refusing to overwrite"):
            prepare_v121(self.root)
