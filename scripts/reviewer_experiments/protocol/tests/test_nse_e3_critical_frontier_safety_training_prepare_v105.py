...
class CriticalFrontierSafetyPreparationV105Tests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.temporary = tempfile.TemporaryDirectory()
        cls.root = Path(cls.temporary.name) / "v105"
        cls.receipt = prepare_v105(cls.root)
    ...
    def test_exact_arms_profiles_density_bounds_and_runtime(self) -> None:
        ...
        for arm_id, manifest in self.arms.items():
            ...
            self.assertEqual(
                {
                    run["metadata"]["v105_upper_queue_density_threshold"]
                    for run in manifest["runs"]
                },
                {upper_density},
            )
            self.assertTrue(
                all(
                    run["metadata"]["v105_nonterminal_queue_density_floor"]
                    == (8.0 if role == "candidate" else None)
                    and run["metadata"]["v105_warm_admissibility"]
                    == ("preserve_anchor_warmness" if role == "candidate" else None)
                    and run["metadata"]["v105_density_band_initializer_guard"]
                    is (role == "candidate")
                    and run["metadata"]["v105_load_least_window_certificate_mode"]
                    == ("disabled" if role == "candidate" else "not_applicable")
                    and run["metadata"]["v105_component_safety_mode"]
                    == component_safety_mode
                    and run["metadata"]["v105_scalar_faasrank_noninferiority"]
                    is (role == "candidate")
                    and run["metadata"]["v105_input_locality_component_noninferiority"]
                    is (role == "candidate")
                    and run["metadata"]["v105_componentwise_faasrank_noninferiority"]
                    ...
                    is False
                    and run["metadata"][
                        "v105_future_child_placement_or_feasibility_used"
                    ]
                    is False
                    and run["metadata"]["v105_critical_frontier_protection"]
                    is (role == "candidate")
                    and run["metadata"]["v105_critical_frontier_rank_source"]
                    == (
                        "immutable_srpt_remaining_critical_path_rank"
                        if role == "candidate"
                        else "not_applicable"
                    )
                    and run["metadata"]["v105_critical_frontier_tie_rule"]
                    == (
                    ...
                    is (role == "candidate")
                    and run["metadata"]["v105_outcome_fields_drive_policy"] is False
                    and run["metadata"]["v105_substitution_cap"] is None
                    and run["metadata"]["v105_confirmation_seeds_opened"] is False
                    and run["metadata"]["v105_other_unopened_seeds_opened"] is False
