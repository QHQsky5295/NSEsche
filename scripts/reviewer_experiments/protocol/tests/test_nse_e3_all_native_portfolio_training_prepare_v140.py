...
class AllNativePortfolioPreparationV140Tests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.temporary = tempfile.TemporaryDirectory()
        cls.root = Path(cls.temporary.name) / "v140"
        cls.receipt = prepare_v140(cls.root)
    ...
    def test_exact_complete_products_profiles_and_portfolio_metadata(self) -> None:
        ...
        members = ["greedy", "hiku", "jiagu", "orion", "load_least"]
        all_reference_keys: set[str] = set()
        for arm_id, profile, portfolio_rule in ARMS:
            ...
            for run in manifest["runs"]:
                ...
                self.assertEqual(metadata["v140_arm_id"], arm_id)
                self.assertEqual(metadata["v140_native_portfolio_rule"], portfolio_rule)
                self.assertEqual(metadata["v140_native_portfolio_members"], members)
                self.assertTrue(metadata["v140_native_shadow_exactness_required"])
                self.assertTrue(metadata["v140_all_native_members_use_all_frontier"])
                self.assertTrue(
                    metadata["v140_all_player_service_complete_assignment_required"]
                )
                self.assertEqual(
                    metadata["v140_service_certificate_state_domain"],
                    "runtime_existing_aggregates_and_admitted_work",
                )
                self.assertEqual(
                    metadata["v140_paper_welfare_state_domain"],
                    "empty_window_aggregates",
                )
                self.assertTrue(
                    metadata["v140_selected_welfare_equals_guard_initializer_required"]
                )
                self.assertFalse(metadata["v140_outcome_fields_drive_policy"])
    ...
    def test_pairing_and_confirmation_information_boundary(self) -> None:
        ...
        self.assertEqual(
            self.receipt["v139_disposition"],
            "aborted_pre_reveal_technical_audit_failure_retained",
        )
        self.assertEqual(self.receipt["v139_performance_summaries_parsed"], 0)
        self.assertFalse(self.receipt["v139_performance_results_consulted"])
        self.assertFalse(self.receipt["v139_reveal_authorized"])
