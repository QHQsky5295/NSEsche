...
class TerminalLoadBandTrainingBlindAuditV98Tests(unittest.TestCase):
    def test_frozen_arm_seed_and_runtime_boundary(self) -> None:
        self.assertEqual(TRAINING_SEEDS, {"E826", "E827", "E828"})
        self.assertEqual(CONFIRMATION_SEEDS, [f"E{i}" for i in range(846, 866)])
        self.assertEqual(
            PREVIOUSLY_RESERVED_SEEDS,
            [f"E{i}" for i in range(766, 786)] + [f"E{i}" for i in range(806, 826)],
        )
