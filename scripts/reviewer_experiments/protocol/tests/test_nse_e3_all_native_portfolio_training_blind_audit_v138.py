...
class AllNativePortfolioBlindAuditV138Tests(unittest.TestCase):
    ...
    def test_preregistered_product_and_seed_boundaries_are_exact(self) -> None:
        self.assertEqual(TRAINING_SEED_LIST, ["E1494", "E1495", "E1496"])
        self.assertEqual(
        ...
        self.assertFalse(
            ...
            & set(RETIRED_V137_CONFIRMATION_SEEDS + NEW_CONFIRMATION_SEEDS)
        )
    ...
    def test_unavailable_proposal_certificate_is_audited_fallback(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            run_id = "synthetic-v138-unavailable-proposal"
            events = [window_event(frame) for frame in range(4000)]
            for event in events:
    ...
    def test_empty_window_not_applicable_diagnostics_pass(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            run_id = "synthetic-v138-empty-window"
            events = [window_event(frame) for frame in range(4000)]
