...
class CompletionProximalHpaContainerComponentwisePathPreparationV128Tests(
    unittest.TestCase
):
    @classmethod
    def setUpClass(cls) -> None:
        cls.temporary = tempfile.TemporaryDirectory()
        cls.root = Path(cls.temporary.name) / "v128"
        cls.receipt = prepare_v128(cls.root)
    ...
    def test_two_arms_lock_exact_profiles_runtime_and_seed_product(self) -> None:
        ...
        for arm_id, role, profile, count in ARMS:
            ...
            for run in manifest["runs"]:
                ...
                self.assertEqual(metadata["v128_arm_id"], arm_id)
                self.assertEqual(metadata["v128_arm_role"], role)
                self.assertEqual(
                    metadata["v128_shock_activation_horizon_frames"],
                    50 if candidate else None,
                )
                self.assertEqual(
                    metadata["v128_critical_service_ratio_numerator"],
                    9 if candidate else None,
                )
                self.assertEqual(
                    metadata["v128_critical_service_ratio_denominator"],
                    10 if candidate else None,
                )
                self.assertEqual(
                    metadata["v128_service_proxy_work_source"],
                    (
                ...
                self.assertIs(
                    metadata["v128_admitted_work_includes_all_blocked_resident"],
                    candidate,
                )
                self.assertIs(
                    metadata["v128_completion_proximal_depth1_substitution_scope"],
                    candidate,
                )
                self.assertEqual(
                    metadata["v128_completion_proximal_definition"],
                    (
                ...
                self.assertEqual(
                    metadata["v128_maximum_remaining_child_depth"],
                    1 if candidate else None,
                )
                self.assertIs(
                    metadata["v128_completion_proximal_critical_frontier_substitution"],
                    candidate,
                )
                self.assertIs(metadata["v128_deeper_player_exact_anchor"], candidate)
                self.assertIs(
                    metadata["v128_complete_componentwise_service_pareto"],
                    candidate,
                )
                self.assertEqual(
                    metadata["v128_componentwise_service_scope"],
                    (
                ...
                self.assertEqual(
                    metadata["v128_componentwise_service_comparison"],
                    (
                ...
                self.assertFalse(metadata["v128_outcome_fields_drive_policy"])
    ...
    def test_plan_is_frozen_and_seed_boundaries_remain_sealed(self) -> None:
        ...
        self.assertEqual(
            ...
            "complete_per_changed_parent_terminal_child_same_hpa_container_componentwise_path_service_pareto",
        )

    def test_existing_root_is_never_overwritten(self) -> None:
        with self.assertRaisesRegex(RuntimeError, "refusing to overwrite"):
            prepare_v128(self.root)
