...
class ComponentwiseServiceComponentwiseInterferencePreparationV133Tests(
    unittest.TestCase
):
    @classmethod
    def setUpClass(cls) -> None:
        cls.temporary = tempfile.TemporaryDirectory()
        cls.root = Path(cls.temporary.name) / "v133"
        cls.receipt = prepare_v133(cls.root)
    ...
    def test_two_arms_lock_exact_profiles_runtime_and_seed_product(self) -> None:
        ...
        for arm_id, role, profile, count in ARMS:
            ...
            for run in manifest["runs"]:
                ...
                self.assertEqual(metadata["v133_arm_id"], arm_id)
                self.assertEqual(metadata["v133_arm_role"], role)
                self.assertEqual(
                    metadata["v133_shock_activation_horizon_frames"],
                    50 if candidate else None,
                )
                self.assertEqual(
                    metadata["v133_critical_service_ratio_numerator"],
                    9 if candidate else None,
                )
                self.assertEqual(
                    metadata["v133_critical_service_ratio_denominator"],
                    10 if candidate else None,
                )
                self.assertEqual(
                    metadata["v133_service_proxy_work_source"],
                    (
                ...
                self.assertIs(
                    metadata["v133_admitted_work_includes_all_blocked_resident"],
                    candidate,
                )
                self.assertIs(
                    metadata["v133_complete_componentwise_service_pareto"],
                    candidate,
                )
                self.assertEqual(
                    metadata["v133_componentwise_service_replay_order"],
                    "native_player_order" if candidate else "not_applicable",
                )
                self.assertFalse(metadata["v133_complete_admitted_interference_pareto"])
                self.assertIs(
                    metadata["v133_componentwise_admitted_interference_pareto"],
                    candidate,
                )
                self.assertEqual(
                    metadata["v133_componentwise_interference_comparison"],
                    (
                ...
                self.assertFalse(metadata["v133_arrival_phase_guard"])
                self.assertFalse(metadata["v133_outcome_fields_drive_policy"])
    ...
    def test_plan_is_frozen_and_seed_boundaries_remain_sealed(self) -> None:
        ...
        self.assertEqual(
            plan["training_design"]["sealed_confirmation_seeds"], "E1405-E1424"
        )
        self.assertEqual(
            PREVIOUS_CONFIRMATION_SEEDS,
