...
from scripts.reviewer_experiments.protocol import (
    nse_e1_homogeneous_concurrent3_quota1_shortest_request_least_cpu_bounded_terminal_slack_short_work_pipeline_queue8_low_diagnostic_v174 as v174,
)
from scripts.reviewer_experiments.protocol.util import file_hash, read_json


class V174Concurrent3Quota1ShortestRequestLeastCpuBoundedTerminalDiagnosticTests(
    unittest.TestCase
):
    def test_frozen_inputs_and_exact_product(self) -> None:
        self.assertEqual(file_hash(v174.PLAN), v174.PLAN_SHA256)
        self.assertEqual(file_hash(v174.IMPLEMENTATION), v174.IMPLEMENTATION_SHA256)
        self.assertEqual(file_hash(v174.BINARY_PATH), v174.BINARY_SHA256)
        self.assertEqual(
            file_hash(v174.V170_COMPLETE_RESULT),
            v174.V170_COMPLETE_RESULT_SHA256,
        )
        plan = read_json(v174.PLAN)
        implementation = read_json(v174.IMPLEMENTATION)
        self.assertEqual(plan["diagnostic_design"]["seeds"], list(v174.SEEDS))
        ...
        self.assertEqual(
            change["new_quota_selection_order"], v174.QUOTA_SELECTION_ORDER
        )
        self.assertEqual(
            v174.TERMINAL_DEFINITION,
            ...
            "terminal-player_with_shortest_current_request_remaining_work_tied_by_"
            "least_immutable_function_CPU_work_then_request-id-then-function-id_and_"
        ...
        manifest = v174._rewrite_candidate(v174._assert_frozen_inputs(), "c" * 40)
        v174._validate_product(manifest, references_bound=False)
        self.assertEqual([run["seed"] for run in manifest["runs"]], list(v174.SEEDS))
        self.assertEqual(len(manifest["reference_build_dependencies"]), len(v174.SEEDS))
        self.assertTrue(
            all(
                run["environment"]["NASH_OPERATIONAL_EXPERT_PROXY"] == v174.PROFILE
                and run["metadata"]["v174_cpu_threshold"] == 1.0
                and run["metadata"]["v174_heavy_player_threshold"] == 2
                and run["metadata"]["v174_minimum_active_heavy_players"] == 3
                and run["metadata"]["v174_overload_activation_boundary"]
                == "strictly_above_activates"
                and run["metadata"]["v174_active_heavy_admission_quota"] == 1
                and run["metadata"]["v174_quota_selection_order"]
                == v174.QUOTA_SELECTION_ORDER
                and run["metadata"]["v174_remaining_fourteen_authorized"] is False
    ...
    def test_hybrid_replaces_only_selected_rows_from_sealed_v170(self) -> None:
        ...
        hybrid = v174._hybrid_rows_v174(frozen, replacements)
        self.assertEqual(
            [row["seed"] for row in hybrid], [f"E{i:02d}" for i in range(1, 21)]
        )
        by_seed = {row["seed"]: row for row in hybrid}
        self.assertTrue(all(by_seed[seed]["source"] == "V174" for seed in v174.SEEDS))
        self.assertTrue(
            all(
                by_seed[f"E{index:02d}"]["source"] == "V170"
                for index in range(1, 21)
                if f"E{index:02d}" not in v174.SEEDS
            )
        )
        self.assertEqual(len(v174._load_v170_candidate()), 20)
    ...
    def _run_config() -> dict:
        return {
            "kind": "run_config",
            "scheduler": "sche_nash",
            "operational_expert_proxy": v174.PROFILE,
            "reference": {"mode": "offline_required", "offline_load_ok": True},
            "operational_expert_proxy_contract": {
                "version": "V174",
                "queue_density_threshold": v174.QUEUE_THRESHOLD,
                "below_threshold_expert": v174.LOW_EXPERT,
                "at_or_above_threshold_expert": v174.HIGH_EXPERT,
                "player_frontier": v174.FRONTIER,
                "single_change_from_v155": v174.SINGLE_CHANGE,
                "terminal_pipeline_definition": v174.TERMINAL_DEFINITION,
                "short_work_pipeline_remaining_work_threshold": v174.SHORT_WORK_THRESHOLD,
                "short_work_pipeline_queue_density_threshold": v174.QUEUE_THRESHOLD,
                "short_work_pipeline_queue_boundary": "below_is_strict",
                "short_work_definition": v174.WORK_DEFINITION,
        ...
        return {
            ...
            "decision": {
                ...
                "terminal_pipeline_frontier": {
                    "enabled": True,
                    "definition": v174.FRONTIER,
                    "short_work_remaining_work_threshold": v174.SHORT_WORK_THRESHOLD,
                ...
                "srpt_hiku2_ocs_queue_router": {
                    ...
                    "selected_expert": (
                        v174.LOW_EXPERT if low_route else v174.HIGH_EXPERT
                    ),
                    "player_frontier": v174.FRONTIER,
    ...
    def test_blind_gate_requires_quota_and_selected_seed_divergence(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory, mock.patch.object(
            v174, "_frozen_v170_assignment_hashes", return_value=frozen
        ):
            root = Path(directory)
            audits = []
            for ordinal, seed in enumerate(v174.SEEDS):
            ...
            gate = v174._mechanism_falsification_gate(audits)
            ...
            self.assertEqual(
                ...
                {seed: 24 for seed in v174.SEEDS},
            )
            self.assertGreater(
    ...
    def test_blind_audit_rejects_active_quota_above_one(self) -> None:
        with tempfile.TemporaryDirectory() as directory, mock.patch.object(
            v174, "_frozen_v170_assignment_hashes", return_value=tuple(range(1000))
        ):
            ...
            with self.assertRaisesRegex(RuntimeError, "frontier evidence changed"):
                v174._audit_nash_log(
                    canonical, {"run_id": "synthetic-v174", "seed": "E20"}
                )
    ...
    def test_blind_audit_rejects_activation_at_exact_threshold(self) -> None:
        with tempfile.TemporaryDirectory() as directory, mock.patch.object(
            ...
            with self.assertRaisesRegex(RuntimeError, "frontier evidence changed"):
                v174._audit_nash_log(
                    canonical, {"run_id": "synthetic-v174", "seed": "E02"}
                )
    ...
    def test_blind_audit_can_skip_unavailable_v159_sequence_for_remaining_seeds(
        ...
        with tempfile.TemporaryDirectory() as directory, mock.patch.object(
            ...
            evidence = v174._audit_nash_log(
                canonical,
                {"run_id": "synthetic-v174", "seed": "E01"},
    ...
    def test_mechanism_gate_rejects_incomplete_active_quota(self) -> None:
        ...
        for seed in v174.SEEDS:
            ...
            audits.append(audit)
        self.assertFalse(v174._mechanism_falsification_gate(audits)["pass"])
