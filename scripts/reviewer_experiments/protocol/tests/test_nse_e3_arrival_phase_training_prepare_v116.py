...
class ArrivalPhasePreparationV116Tests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.temporary = tempfile.TemporaryDirectory()
        cls.root = Path(cls.temporary.name) / "v116"
        cls.receipt = prepare_v116(cls.root)
    ...
    def test_two_arms_lock_exact_profiles_runtime_and_seed_product(self) -> None:
        ...
        for arm_id, role, profile, count in ARMS:
            ...
            for run in manifest["runs"]:
                ...
                self.assertEqual(metadata["v116_arm_id"], arm_id)
                self.assertEqual(metadata["v116_arm_role"], role)
                self.assertEqual(
                    metadata["v116_shock_activation_horizon_frames"],
                    50 if candidate else None,
                )
                self.assertEqual(
                    metadata["v116_critical_service_ratio_numerator"],
                    9 if candidate else None,
                )
                self.assertEqual(
                    metadata["v116_critical_service_ratio_denominator"],
                    10 if candidate else None,
                )
                self.assertEqual(
                    metadata["v116_service_proxy_work_source"],
                    (
                ...
                self.assertIs(
                    metadata["v116_admitted_work_includes_all_blocked_resident"],
                    candidate,
                )
                self.assertIs(metadata["v116_admitted_interference_pareto"], candidate)
                self.assertFalse(
                    metadata["v116_componentwise_admitted_interference_pareto"]
                )
                self.assertEqual(
                    metadata["v116_admitted_interference_source"],
                    (
                ...
                self.assertIs(
                    metadata["v116_complete_admitted_interference_nonworse"],
                    candidate,
                )
                self.assertIs(metadata["v116_arrival_phase_guard"], candidate)
                self.assertEqual(
                    metadata["v116_arrival_phase_window_frames"],
                    20 if candidate else None,
                )
                self.assertEqual(
                    metadata["v116_arrival_phase_previous_history_offsets"],
                    [60, 79] if candidate else None,
                )
                self.assertEqual(
                    metadata["v116_arrival_phase_current_history_offsets"],
                    [80, 99] if candidate else None,
                )
                self.assertEqual(
                    metadata["v116_arrival_phase_comparison"],
                    (
                ...
                self.assertIs(
                    metadata["v116_arrival_phase_windows_adjacent_and_disjoint"],
                    candidate,
                )
                self.assertIs(
                    metadata["v116_arrival_phase_incomplete_or_overflow_fail_closed"],
                    candidate,
                )
                self.assertFalse(metadata["v116_outcome_fields_drive_policy"])
