...
class AdmittedInterferencePreparationV114Tests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.temporary = tempfile.TemporaryDirectory()
        cls.root = Path(cls.temporary.name) / "v114"
        cls.receipt = prepare_v114(cls.root)
    ...
    def test_two_arms_lock_exact_profiles_runtime_and_seed_product(self) -> None:
        ...
        for arm_id, role, profile, count in ARMS:
            ...
            for run in manifest["runs"]:
                ...
                self.assertEqual(metadata["v114_arm_id"], arm_id)
                self.assertEqual(metadata["v114_arm_role"], role)
                self.assertEqual(
                    metadata["v114_shock_activation_horizon_frames"],
                    50 if candidate else None,
                )
                self.assertEqual(
                    metadata["v114_critical_service_ratio_numerator"],
                    9 if candidate else None,
                )
                self.assertEqual(
                    metadata["v114_critical_service_ratio_denominator"],
                    10 if candidate else None,
                )
                self.assertEqual(
                    metadata["v114_service_proxy_work_source"],
                    (
                ...
                self.assertIs(
                    metadata["v114_admitted_work_includes_all_blocked_resident"],
                    candidate,
                )
                self.assertIs(metadata["v114_admitted_interference_pareto"], candidate)
                self.assertEqual(
                    metadata["v114_admitted_interference_source"],
                    (
                ...
                self.assertIs(
                    metadata["v114_complete_admitted_interference_nonworse"],
                    candidate,
                )
                self.assertFalse(metadata["v114_outcome_fields_drive_policy"])
    ...
    def test_plan_is_frozen_and_seed_boundaries_remain_sealed(self) -> None:
        ...
        self.assertEqual(
            plan["training_design"]["sealed_confirmation_seeds"], "E1086-E1105"
        )
        self.assertEqual(
            PREVIOUS_CONFIRMATION_SEEDS, [f"E{i}" for i in range(926, 1086)]
        )
