...
class LoadBandSubstitutionCapTrainingPreparationV99Tests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.temporary = tempfile.TemporaryDirectory()
        cls.root = Path(cls.temporary.name) / "v99"
        cls.receipt = prepare_v99(cls.root)
    ...
    def test_exact_arms_profiles_thresholds_and_runtime(self) -> None:
        ...
        for arm_id, manifest in self.arms.items():
            ...
            self.assertEqual(
                {
                    run["metadata"]["v99_nonterminal_queue_density_floor"]
                    for run in manifest["runs"]
                },
                {8.0 if role == "candidate" else None},
            )
            self.assertEqual(
                {run["metadata"]["v99_substitution_cap"] for run in manifest["runs"]},
                {cap},
            )
            self.assertTrue(
                all(
                    run["metadata"]["v99_load_band_substitution_cap_guard"]
                    is (cap is not None)
                    and run["metadata"]["v99_upper_queue_density_threshold"]
                    == (64.0 if role == "candidate" else None)
                    and run["metadata"]["v99_confirmation_seeds_opened"] is False
                    and run["metadata"]["v99_previously_reserved_seeds_opened"] is False
    ...
    def test_arm_references_are_unique_and_tapes_are_paired(self) -> None:
        ...
        e3_sets = [keys for arm, keys in tape_sets.items() if arm.startswith("v99-e3")]
