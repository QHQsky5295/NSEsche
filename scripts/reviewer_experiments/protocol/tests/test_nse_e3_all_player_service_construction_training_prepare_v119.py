...
class AllPlayerServicePreparationV119Tests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.temporary = tempfile.TemporaryDirectory()
        cls.root = Path(cls.temporary.name) / "v119"
        cls.receipt = prepare_v119(cls.root)
    ...
    def test_two_arms_lock_exact_profiles_runtime_and_seed_product(self) -> None:
        ...
        for arm_id, role, profile, count in ARMS:
            ...
            for run in manifest["runs"]:
                ...
                self.assertEqual(metadata["v119_arm_id"], arm_id)
                self.assertEqual(metadata["v119_arm_role"], role)
                self.assertEqual(
                    metadata["v119_shock_activation_horizon_frames"],
                    50 if candidate else None,
                )
                self.assertEqual(
                    metadata["v119_critical_service_ratio_numerator"],
                    9 if candidate else None,
                )
                self.assertEqual(
                    metadata["v119_critical_service_ratio_denominator"],
                    10 if candidate else None,
                )
                self.assertEqual(
                    metadata["v119_service_proxy_work_source"],
                    (
                ...
                self.assertIs(
                    metadata["v119_admitted_work_includes_all_blocked_resident"],
                    candidate,
                )
                self.assertIs(
                    metadata["v119_complete_componentwise_service_pareto"],
                    candidate,
                )
                self.assertEqual(
                    metadata["v119_componentwise_service_scope"],
                    "all_current_players" if candidate else "not_applicable",
                )
                self.assertEqual(
                    metadata["v119_componentwise_service_comparison"],
                    (
                ...
                self.assertEqual(
                    metadata["v119_componentwise_service_replay_order"],
                    "native_player_order" if candidate else "not_applicable",
                )
                self.assertIs(metadata["v119_service_directed_construction"], candidate)
                self.assertEqual(
                    metadata["v119_service_choice_scope"],
                    "all_current_players" if candidate else "not_applicable",
                )
                self.assertFalse(metadata["v119_noncritical_exact_anchor"])
                self.assertFalse(metadata["v119_interference_constraints"])
                self.assertFalse(metadata["v119_arrival_phase_guard"])
                self.assertFalse(metadata["v119_outcome_fields_drive_policy"])
    ...
    def test_plan_is_frozen_and_seed_boundaries_remain_sealed(self) -> None:
        ...
        self.assertEqual(
            PREVIOUS_CONFIRMATION_SEEDS, [f"E{i}" for i in range(929, 1106)]
        )
