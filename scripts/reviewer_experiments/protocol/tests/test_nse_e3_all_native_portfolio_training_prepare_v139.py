...
class AllNativePortfolioPreparationV139Tests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.temporary = tempfile.TemporaryDirectory()
        cls.root = Path(cls.temporary.name) / "v139"
        cls.receipt = prepare_v139(cls.root)
    ...
    def test_exact_complete_products_profiles_and_portfolio_metadata(self) -> None:
        ...
        for arm_id, profile, portfolio_rule in ARMS:
            ...
            for run in manifest["runs"]:
                ...
                self.assertEqual(metadata["v139_arm_id"], arm_id)
                self.assertEqual(metadata["v139_native_portfolio_rule"], portfolio_rule)
                self.assertEqual(metadata["v139_native_portfolio_members"], members)
                self.assertTrue(metadata["v139_native_shadow_exactness_required"])
                self.assertTrue(metadata["v139_all_native_members_use_all_frontier"])
                self.assertTrue(
                    metadata["v139_all_player_service_complete_assignment_required"]
                )
                self.assertFalse(metadata["v139_outcome_fields_drive_policy"])
    ...
    def test_pairing_and_confirmation_information_boundary(self) -> None:
        ...
        self.assertFalse(
            self.receipt["v138_completed_online_performance_results_consulted"]
        )
