...
class ArrivalPhaseBlindAuditV116Tests(unittest.TestCase):
    def test_frozen_arm_seed_and_runtime_boundary(self) -> None:
        self.assertEqual(TRAINING_SEEDS, {"E917", "E918", "E919"})
    ...
    def test_diagnostics_require_admitted_snapshot_and_ratio_certificate(
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
                        ...
                        "decision": {
                            "load_least_dominance_gate": {
                                "causal_arrival_shock": {
                                    "gate_enabled": True,
                                    "active": invoked,
                                    "history_complete": phase_complete,
                                    "first_seen_current_frame": 1 if frame < 80 else 2,
                                    "baseline_frames": 80,
                                    "recent_frames": 20,
                                    "min_requests_per_window": 20,
                                    "baseline_count": 80 if phase_complete else 0,
                                    "recent_count": 40 if phase_complete else 0,
                                    "threshold_numerator": 3,
                                    "threshold_denominator": 2,
                                    "active_frames": 50,
                                    "until_frame": 148 if invoked else None,
                                    ...
                                    "non_decreasing_phase": {
                                        ...
                                        "current_count": 40 if phase_complete else None,
                                        "non_decreasing": phase_complete,
                                        "comparison": "current_greater_than_or_equal_to_previous",
                                        "windows_are_adjacent_and_disjoint": True,
                                        "uses_first_seen_request_ids_only": True,
                                    },
                                },
            ...
            evidence = _validate_admitted_work_diagnostics(
                {"run_id": run_id, "simulation": {"total_frame": 100}},
                canonical,
                expected,
            )
            self.assertEqual(evidence["window_count"], 100)
            self.assertEqual(evidence["admitted_work_complete_window_count"], 100)
            self.assertEqual(evidence["pending_cpu_value_count"], 200)
            self.assertEqual(evidence["resident_remaining_cpu_value_count"], 300)
            self.assertEqual(evidence["threshold_met_window_count"], 1)
            self.assertEqual(evidence["arrival_phase_complete_window_count"], 1)
            self.assertEqual(evidence["arrival_phase_non_decreasing_window_count"], 1)
            self.assertEqual(evidence["arrival_phase_declining_window_count"], 0)
            ...
            phase = windows[-1]["decision"]["load_least_dominance_gate"][
                "causal_arrival_shock"
            ]["non_decreasing_phase"]
            phase["current_count"] = 39
            with gzip.open(path, "wt", encoding="utf-8") as handle:
                for event in windows:
                    handle.write(json.dumps(event) + "\n")
            with self.assertRaisesRegex(RuntimeError, "arrival-phase reconstruction"):
                _validate_admitted_work_diagnostics(
                    {"run_id": run_id, "simulation": {"total_frame": 100}},
                    canonical,
                    expected,
                )
            phase["current_count"] = 40
