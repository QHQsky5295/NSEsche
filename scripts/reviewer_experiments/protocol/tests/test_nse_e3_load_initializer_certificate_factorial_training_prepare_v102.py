...
class LoadInitializerCertificateFactorialPreparationV102Tests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.temporary = tempfile.TemporaryDirectory()
        cls.root = Path(cls.temporary.name) / "v102"
        cls.receipt = prepare_v102(cls.root)
    ...
    def test_exact_arms_profiles_density_bounds_and_runtime(self) -> None:
        expected = {
            arm_id: (experiment_id, role, profile, upper_density, count)
            for arm_id, experiment_id, role, profile, upper_density, count in ARMS
        }
        ...
        for arm_id, manifest in self.arms.items():
            experiment_id, role, profile, upper_density, count = expected[arm_id]
            ...
            self.assertEqual(
                {
                    run["metadata"]["v102_upper_queue_density_threshold"]
                    for run in manifest["runs"]
                },
                {upper_density},
            )
            self.assertTrue(
                all(
                    run["metadata"]["v102_nonterminal_queue_density_floor"]
                    == (8.0 if role == "candidate" else None)
                    and run["metadata"]["v102_warm_admissibility"]
                    == ("preserve_anchor_warmness" if role == "candidate" else None)
                    and run["metadata"]["v102_density_band_warm_preserve_guard"]
                    is (role == "candidate")
                    and run["metadata"]["v102_load_least_window_certificate_mode"]
                    == (
                        "conditional_on_accepted_load_least_initializer"
                        if arm_id.endswith("conditional-certificate")
                        else (
                            "disabled"
                            if arm_id.endswith("initializer-only")
                            else "not_applicable"
                        )
                    )
                    and run["metadata"]["v102_substitution_cap"] is None
                    and run["metadata"]["v102_confirmation_seeds_opened"] is False
                    and run["metadata"]["v102_other_unopened_seeds_opened"] is False
