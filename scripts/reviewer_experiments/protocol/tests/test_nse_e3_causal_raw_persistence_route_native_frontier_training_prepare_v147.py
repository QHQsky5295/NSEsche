...
class V147RouteNativeFrontierPrepareTests(unittest.TestCase):
    def test_rewrite_is_exact_nine_cell_route_native_frontier_product(self) -> None:
        ...
        for run in manifest["runs"]:
            ...
            self.assertEqual(metadata["v147_native_portfolio_members"], NATIVE_MEMBERS)
            self.assertTrue(metadata["v147_all_three_experts_advanced_every_window"])
            self.assertEqual(metadata["v147_quiet_route"], "greedy")
            self.assertFalse(metadata["v147_outcome_fields_drive_policy"])
            self.assertEqual(
                metadata["v147_service_certificate_scope"],
                SERVICE_CERTIFICATE_SCOPE,
            )
            self.assertEqual(metadata["v147_player_frontier"], PLAYER_FRONTIER)
            self.assertEqual(metadata["v147_route_frontiers"], ROUTE_FRONTIERS)
            self.assertEqual(
                metadata["v147_single_factor_change"],
                "universal_all_unscheduled_to_route_selected_expert_native_frontier",
            )
            self.assertTrue(metadata["v147_native_frontier_command_evidence_required"])
    ...
    def test_prepare_seals_v146_failure_and_single_factor_change(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            root = Path(temp) / "v147"
            receipt = prepare_v147(root)
            self.assertEqual(receipt["plan_sha256"], PLAN_SHA256)
            self.assertEqual(receipt["candidate_online_runs"], 9)
            self.assertEqual(receipt["v147_revision"], V147_REVISION)
            self.assertEqual(receipt["v145_result_file_sha256"], V145_RESULT_SHA256)
            self.assertEqual(receipt["v146_invalidation_hash"], V146_INVALIDATION_HASH)
            self.assertEqual(receipt["player_frontier"], PLAYER_FRONTIER)
            self.assertEqual(receipt["route_frontiers"], ROUTE_FRONTIERS)
            self.assertEqual(
                receipt["single_factor_change"],
                "universal_all_unscheduled_to_route_selected_expert_native_frontier",
            )
            self.assertTrue(receipt["native_frontier_command_evidence_required"])
