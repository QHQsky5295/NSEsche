...
import scripts.reviewer_experiments.protocol.nse_e3_load_band_substitution_cap_training_reveal_v99 as reveal_module
from scripts.reviewer_experiments.protocol.nse_e3_load_band_substitution_cap_training_reveal_v99 import (
    ARMS,
    EXPECTED_SCENARIOS,
    EXPECTED_SEEDS,
...
class LoadBandSubstitutionCapTrainingRevealV99Tests(unittest.TestCase):
    ...
    def test_joint_gate_selects_only_direction_consistent_candidates(self) -> None:
        ...
        self.assertFalse(
            result["dimension_scores"]["E3"]["v99-e3-band8-cap2"][
                "all_required_gates_pass"
            ]
        )
        diagnostic = result["arm_aggregates"]["v99-e3-band8-cap1"]["E3.spike5x50ms"][
            "formal_absolute_diagnostics"
        ]["qpr_finite_only"]
        self.assertFalse(diagnostic["used_for_V99_selection"])

    def test_one_large_outlier_cannot_satisfy_two_of_three_rule(self) -> None:
        rows = rows_fixture()
        for row in rows:
            if (
                row["arm_id"] == "v99-e3-band8-cap1"
                and row["scenario_id"] == "E3.spike5x50ms"
            ):
                value = 10.0 if row["seed"] == "E868" else 0.9
                row["qpr_finite_only"] = value
                row["qpr_zero_completed_as_zero"] = value
        result = evaluate_training_rows(rows, THRESHOLDS)
        gate = result["paired_candidate_results"]["v99-e3-band8-cap1"][
            "E3.spike5x50ms"
