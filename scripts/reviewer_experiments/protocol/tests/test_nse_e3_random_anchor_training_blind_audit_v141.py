...
def _run(run_id: str) -> dict:
    return {
        "run_id": run_id,
        "simulator_experiment": {"algorithm_seed": 1523},
    }
...
def window_event(
    frame: int, selection_rule: str = "random_default_service_pareto"
) -> dict:
    ...
    return {
        ...
        "decision": {
            ...
            "native_portfolio": {
                ...
                "random_shadow_algorithm_seed": 1523,
...
class RandomAnchorBlindAuditV141Tests(unittest.TestCase):
    ...
    def test_preregistered_product_and_seed_boundaries_are_exact(self) -> None:
        self.assertEqual(TRAINING_SEED_LIST, ["E1523", "E1524", "E1525"])
        self.assertEqual(
    ...
    def test_exact_random_anchor_and_both_portfolios_pass(self) -> None:
        for rule, expected_kind in (
            ("exact_random_anchor", "random"),
            ("random_default_service_pareto", "load_least"),
            ("random_default_welfare_pareto", "hiku"),
        ):
            with self.subTest(rule=rule), tempfile.TemporaryDirectory() as temporary:
                run_id = f"synthetic-v141-{rule}"
    ...
    def test_random_predicate_or_candidate_alignment_error_fails_closed(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with self.assertRaisesRegex(RuntimeError, "Random-relative predicates"):
                _validate_native_diagnostics(
                    _run(run_id), canonical, "random_default_service_pareto"
                )
    ...
    def test_unavailable_proposal_certificate_is_audited_fallback(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            run_id = "synthetic-v141-unavailable-proposal"
            events = [window_event(frame) for frame in range(4000)]
            for event in events:
    ...
    def test_empty_window_not_applicable_and_enabled_portfolio_fails(self) -> None:
        for corrupt in (False, True):
            with self.subTest(
                ...
                run_id = f"synthetic-v141-empty-{corrupt}"
                events = [window_event(frame) for frame in range(4000)]
