...
import scripts.reviewer_experiments.protocol.nse_e3_terminal_load_band_training_reveal_v98 as reveal_module
from scripts.reviewer_experiments.protocol.nse_e3_terminal_load_band_training_reveal_v98 import (
    ARMS,
    EXPECTED_SCENARIOS,
    EXPECTED_SEEDS,
    ...
    for arm_id, arm in ARMS.items():
        ...
        for scenario in scenarios:
            for index, seed in enumerate(EXPECTED_SEEDS):
                ...
                elif arm_id == "v98-e3-terminal-load-band4":
...
class TerminalLoadBandTrainingRevealV98Tests(unittest.TestCase):
    ...
    def test_joint_gate_selects_only_direction_consistent_candidates(self) -> None:
        ...
        self.assertEqual(
            ...
            "v98-e3-terminal-load-band4",
        )
        self.assertFalse(
            result["dimension_scores"]["E3"]["v98-e3-terminal-load-band8"][
                "all_required_gates_pass"
            ]
        )
        diagnostic = result["arm_aggregates"]["v98-e3-terminal-load-band4"][
            "E3.spike5x50ms"
        ]["formal_absolute_diagnostics"]["qpr_finite_only"]
        self.assertFalse(diagnostic["used_for_V98_selection"])

    def test_one_large_outlier_cannot_satisfy_two_of_three_rule(self) -> None:
        rows = rows_fixture()
        for row in rows:
            if (
                row["arm_id"] == "v98-e3-terminal-load-band4"
                and row["scenario_id"] == "E3.spike5x50ms"
            ):
                value = 10.0 if row["seed"] == "E828" else 0.9
                row["qpr_finite_only"] = value
                row["qpr_zero_completed_as_zero"] = value
        result = evaluate_training_rows(rows, THRESHOLDS)
        gate = result["paired_candidate_results"]["v98-e3-terminal-load-band4"][
            "E3.spike5x50ms"
