...
from scripts.reviewer_experiments.protocol import (
    nse_e1_homogeneous_concurrent3_quota1_cpu_bounded_terminal_slack_short_work_pipeline_queue8_low_diagnostic_v172 as v172,
)
from scripts.reviewer_experiments.protocol.util import file_hash, read_json


class V172Concurrent3Quota1CpuBoundedTerminalDiagnosticTests(unittest.TestCase):
    def test_frozen_inputs_and_exact_product(self) -> None:
        self.assertEqual(file_hash(v172.PLAN), v172.PLAN_SHA256)
        self.assertEqual(file_hash(v172.IMPLEMENTATION), v172.IMPLEMENTATION_SHA256)
        self.assertEqual(file_hash(v172.BINARY_PATH), v172.BINARY_SHA256)
        self.assertEqual(
            file_hash(v172.V170_COMPLETE_RESULT),
            v172.V170_COMPLETE_RESULT_SHA256,
        )
        plan = read_json(v172.PLAN)
        implementation = read_json(v172.IMPLEMENTATION)
        self.assertEqual(plan["diagnostic_design"]["seeds"], list(v172.SEEDS))
        ...
        self.assertEqual(change["old_active_heavy_quota"], 2)
        self.assertEqual(change["new_active_heavy_quota"], 1)
        self.assertEqual(
            change["quota_selection_order"], "ascending_request_id_then_function_id"
        )
        self.assertEqual(
            v172.TERMINAL_DEFINITION,
            "admit_all_parents-completed_players;when_current_heavy_terminal_player_"
            "count_strictly_exceeds_two_admit_at_most_one_heavy_incomplete-parent-"
            "terminal-player_in_ascending_request-id-then-function-id_order_and_"
        ...
        manifest = v172._rewrite_candidate(v172._assert_frozen_inputs(), "c" * 40)
        v172._validate_product(manifest, references_bound=False)
        self.assertEqual([run["seed"] for run in manifest["runs"]], list(v172.SEEDS))
        self.assertEqual(len(manifest["reference_build_dependencies"]), len(v172.SEEDS))
        self.assertTrue(
            all(
                run["environment"]["NASH_OPERATIONAL_EXPERT_PROXY"] == v172.PROFILE
                and run["metadata"]["v172_cpu_threshold"] == 1.0
                and run["metadata"]["v172_heavy_player_threshold"] == 2
                and run["metadata"]["v172_minimum_active_heavy_players"] == 3
                and run["metadata"]["v172_overload_activation_boundary"]
                == "strictly_above_activates"
                and run["metadata"]["v172_active_heavy_admission_quota"] == 1
                and run["metadata"]["v172_quota_selection_order"]
                == "ascending_request_id_then_function_id"
                and run["metadata"]["v172_remaining_fourteen_authorized"] is False
    ...
    def test_hybrid_replaces_only_selected_rows_from_sealed_v170(self) -> None:
        ...
        replacements = [
            ...
            for ordinal, seed in enumerate(v172.SEEDS)
        ]
        hybrid = v172._hybrid_rows_v172(frozen, replacements)
        self.assertEqual(
            [row["seed"] for row in hybrid], [f"E{i:02d}" for i in range(1, 21)]
        )
        by_seed = {row["seed"]: row for row in hybrid}
        self.assertTrue(all(by_seed[seed]["source"] == "V172" for seed in v172.SEEDS))
        self.assertTrue(
            all(
                by_seed[f"E{index:02d}"]["source"] == "V170"
                for index in range(1, 21)
                if f"E{index:02d}" not in v172.SEEDS
            )
        )
        self.assertEqual(len(v172._load_v170_candidate()), 20)
    ...
    def _run_config() -> dict:
        return {
            "kind": "run_config",
            "scheduler": "sche_nash",
            "operational_expert_proxy": v172.PROFILE,
            "reference": {"mode": "offline_required", "offline_load_ok": True},
            "operational_expert_proxy_contract": {
                "version": "V172",
                "queue_density_threshold": v172.QUEUE_THRESHOLD,
                "below_threshold_expert": v172.LOW_EXPERT,
                "at_or_above_threshold_expert": v172.HIGH_EXPERT,
                "player_frontier": v172.FRONTIER,
                "single_change_from_v155": v172.SINGLE_CHANGE,
                "terminal_pipeline_definition": v172.TERMINAL_DEFINITION,
                "short_work_pipeline_remaining_work_threshold": v172.SHORT_WORK_THRESHOLD,
                "short_work_pipeline_queue_density_threshold": v172.QUEUE_THRESHOLD,
                "short_work_pipeline_queue_boundary": "below_is_strict",
                "short_work_definition": v172.WORK_DEFINITION,
        ...
        return {
            ...
            "decision": {
                ...
                "terminal_pipeline_frontier": {
                    "enabled": True,
                    "definition": v172.FRONTIER,
                    "short_work_remaining_work_threshold": v172.SHORT_WORK_THRESHOLD,
                ...
                "srpt_hiku2_ocs_queue_router": {
                    ...
                    "selected_expert": (
                        v172.LOW_EXPERT if low_route else v172.HIGH_EXPERT
                    ),
                    "player_frontier": v172.FRONTIER,
    ...
    def test_blind_gate_requires_quota_and_selected_seed_divergence(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory, mock.patch.object(
            v172, "_frozen_v170_assignment_hashes", return_value=frozen
        ):
            root = Path(directory)
            audits = []
            for seed in v172.SEEDS:
                run_id = f"synthetic-v172-{seed}"
                canonical = root / seed
                self._write_log(
                    canonical,
                    run_id,
                ...
                audits.append(
                    v172._audit_nash_log(canonical, {"run_id": run_id, "seed": seed})
                )
            gate = v172._mechanism_falsification_gate(audits)
            ...
            self.assertEqual(
                ...
                {seed: 24 for seed in v172.SEEDS},
            )
            self.assertGreater(
    ...
    def test_blind_audit_rejects_active_quota_above_one(self) -> None:
        with tempfile.TemporaryDirectory() as directory, mock.patch.object(
            v172, "_frozen_v170_assignment_hashes", return_value=tuple(range(1000))
        ):
            ...
            with self.assertRaisesRegex(RuntimeError, "frontier evidence changed"):
                v172._audit_nash_log(
                    canonical, {"run_id": "synthetic-v172", "seed": "E20"}
                )
    ...
    def test_blind_audit_rejects_activation_at_exact_threshold(self) -> None:
        with tempfile.TemporaryDirectory() as directory, mock.patch.object(
            ...
            with self.assertRaisesRegex(RuntimeError, "frontier evidence changed"):
                v172._audit_nash_log(
                    canonical, {"run_id": "synthetic-v172", "seed": "E02"}
                )
    ...
    def test_blind_audit_can_skip_unavailable_v159_sequence_for_remaining_seeds(
        ...
        with tempfile.TemporaryDirectory() as directory, mock.patch.object(
            ...
            evidence = v172._audit_nash_log(
                canonical,
                {"run_id": "synthetic-v172", "seed": "E01"},
    ...
    def test_mechanism_gate_rejects_incomplete_active_quota(self) -> None:
        ...
        for seed in v172.SEEDS:
            ...
            audits.append(audit)
        self.assertFalse(v172._mechanism_falsification_gate(audits)["pass"])
