...
class V147RouteNativeFrontierExecuteTests(unittest.TestCase):
    def test_ready_schedule_maps_all_nine_bound_run_ids_once(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            ...
            write_json_atomic(root / "frozen-run-order-v147.json", source)
            ...
            _assert_hashed(ready, "schedule_hash", "test V147 ready schedule")
    ...
    def test_changed_existing_ready_schedule_is_rejected(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            ...
            manifest = _rewrite_candidate(read_json(V142_TEMPLATE))
            write_json_atomic(
                root / "frozen-run-order-v147.json", _frozen_schedule(manifest)
            )
            write_json_atomic(root / f"manifest.{ARM_ID}.ready.json", manifest)
            ready = materialize_ready_schedule(root)
            ...
            write_json_atomic(root / "frozen-ready-run-order-v147.json", ready)
