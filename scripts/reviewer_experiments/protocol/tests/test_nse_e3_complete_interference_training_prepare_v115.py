...
class CompleteInterferencePreparationV115Tests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.temporary = tempfile.TemporaryDirectory()
        cls.root = Path(cls.temporary.name) / "v115"
        cls.receipt = prepare_v115(cls.root)
    ...
    def test_two_arms_lock_exact_profiles_runtime_and_seed_product(self) -> None:
        ...
        for arm_id, role, profile, count in ARMS:
            ...
            for run in manifest["runs"]:
                ...
                self.assertEqual(metadata["v115_arm_id"], arm_id)
                self.assertEqual(metadata["v115_arm_role"], role)
                self.assertEqual(
                    metadata["v115_shock_activation_horizon_frames"],
                    50 if candidate else None,
                )
                self.assertEqual(
                    metadata["v115_critical_service_ratio_numerator"],
                    9 if candidate else None,
                )
                self.assertEqual(
                    metadata["v115_critical_service_ratio_denominator"],
                    10 if candidate else None,
                )
                self.assertEqual(
                    metadata["v115_service_proxy_work_source"],
                    (
                ...
                self.assertIs(
                    metadata["v115_admitted_work_includes_all_blocked_resident"],
                    candidate,
                )
                self.assertIs(metadata["v115_admitted_interference_pareto"], candidate)
                self.assertFalse(
                    metadata["v115_componentwise_admitted_interference_pareto"]
                )
                self.assertEqual(
                    metadata["v115_admitted_interference_source"],
                    (
                ...
                self.assertIs(
                    metadata["v115_complete_admitted_interference_nonworse"],
                    candidate,
                )
                self.assertFalse(metadata["v115_outcome_fields_drive_policy"])
