...
class CausalHorizonPreparationV109Tests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.temporary = tempfile.TemporaryDirectory()
        cls.root = Path(cls.temporary.name) / "v109"
        cls.receipt = prepare_v109(cls.root)
    ...
    def test_exact_arms_profiles_density_bounds_and_runtime(self) -> None:
        ...
        for arm_id, manifest in self.arms.items():
            ...
            self.assertEqual(
                {run["metadata"]["v109_shock_rate_ratio"] for run in manifest["runs"]},
                {shock_rate_ratio},
            )
            self.assertTrue(
                all(
                    run["metadata"]["v109_nonterminal_queue_density_floor"]
                    == (8.0 if role == "candidate" else None)
                    and run["metadata"]["v109_warm_admissibility"]
                    == ("preserve_anchor_warmness" if role == "candidate" else None)
                    and run["metadata"]["v109_causal_arrival_shock_initializer_guard"]
                    is (role == "candidate")
                    and run["metadata"]["v109_load_least_window_certificate_mode"]
                    == ("disabled" if role == "candidate" else "not_applicable")
                    and run["metadata"]["v109_arrival_signal"]
                    == (
                        "first_seen_request_ids_only"
                        if role == "candidate"
                        else "not_applicable"
                    )
                    and run["metadata"]["v109_cpu_memory_individual_noninferiority"]
                    is (role == "candidate")
                    and run["metadata"]["v109_resource_bottleneck_sum_noninferiority"]
                    is False
                    and run["metadata"]["v109_shock_threshold_numerator"]
                    == ({"3/2": 3}.get(shock_rate_ratio))
                    and run["metadata"]["v109_shock_threshold_denominator"]
                    == ({"3/2": 2}.get(shock_rate_ratio))
                    and run["metadata"]["v109_arrival_history_baseline_frames"]
                    == (80 if role == "candidate" else None)
                    and run["metadata"]["v109_arrival_history_recent_frames"]
                    == (20 if role == "candidate" else None)
                    and run["metadata"]["v109_arrival_min_requests_per_window"]
                    == (20 if role == "candidate" else None)
                    and run["metadata"]["v109_shock_activation_horizon_frames"]
                    == ACTIVATION_HORIZONS.get(arm_id)
                    and run["metadata"]["v109_resource_inputs_finite_fail_closed"]
                    is (role == "candidate")
                    and run["metadata"]["v109_scalar_faasrank_noninferiority"]
                    is (role == "candidate")
                    and run["metadata"]["v109_input_locality_component_noninferiority"]
                    is (role == "candidate")
                    and run["metadata"]["v109_componentwise_faasrank_noninferiority"]
                    is False
                    and run["metadata"][
                    ...
                    is False
                    and run["metadata"][
                        "v109_future_child_placement_or_feasibility_used"
                    ]
                    is False
                    and run["metadata"]["v109_critical_frontier_substitution"]
                    is (role == "candidate")
                    and run["metadata"]["v109_critical_frontier_rank_source"]
                    == (
                        "immutable_srpt_remaining_critical_path_rank"
                        if role == "candidate"
                        else "not_applicable"
                    )
                    and run["metadata"]["v109_critical_frontier_tie_rule"]
                    == (
                        "every_exact_tied_current_request_frontier_maximum"
                        if role == "candidate"
                        else "not_applicable"
                    )
                    and run["metadata"]["v109_noncritical_exact_anchor"]
                    is (role == "candidate")
                    and run["metadata"]["v109_critical_service_ratio_numerator"]
                    == (
                        CRITICAL_SERVICE_RATIOS[arm_id][0]
                        if role == "candidate"
                        else None
                    )
                    and run["metadata"]["v109_critical_service_ratio_denominator"]
                    == (
                    ...
                    is (role == "candidate")
                    and run["metadata"]["v109_load_least_complete_score_used_as_gate"]
                    is False
                    and run["metadata"]["v109_outcome_fields_drive_policy"] is False
                    and run["metadata"]["v109_scenario_or_burst_label_used_by_policy"]
                    is False
                    and run["metadata"][
                        "v109_completion_or_performance_fields_used_by_policy"
                    ]
                    is False
                    and run["metadata"]["v109_future_arrivals_used_by_policy"] is False
                    and run["metadata"]["v109_substitution_cap"] is None
                    and run["metadata"]["v109_confirmation_seeds_opened"] is False
                    and run["metadata"]["v109_other_unopened_seeds_opened"] is False
    ...
    def test_plan_is_frozen_and_matches_exact_seed_boundary(self) -> None:
        ...
        self.assertEqual(
            {
                item.get("activation_horizon_frames")
