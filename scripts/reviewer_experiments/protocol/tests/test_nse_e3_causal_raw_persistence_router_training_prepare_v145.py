...
class V145CausalRawPersistencePrepareTests(unittest.TestCase):
    def test_rewrite_is_exact_nine_cell_raw_persistence_product(self) -> None:
        ...
        for run in manifest["runs"]:
            ...
            self.assertEqual(
                run["metadata"]["v145_native_portfolio_members"], NATIVE_MEMBERS
            )
            self.assertTrue(
                run["metadata"]["v145_all_three_experts_advanced_every_window"]
            )
            self.assertEqual(run["metadata"]["v145_quiet_route"], "greedy")
            self.assertFalse(run["metadata"]["v145_outcome_fields_drive_policy"])
            self.assertEqual(
                run["metadata"]["v145_service_certificate_scope"],
                SERVICE_CERTIFICATE_SCOPE,
            )
    ...
    def test_prepare_seals_parent_failures_and_generates_no_confirmation(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            root = Path(temp) / "v145"
            receipt = prepare_v145(root)
            self.assertEqual(receipt["plan_sha256"], PLAN_SHA256)
            self.assertEqual(receipt["candidate_online_runs"], 9)
            self.assertEqual(receipt["v145_revision"], V145_REVISION)
            self.assertEqual(receipt["v144_result_file_sha256"], V144_RESULT_SHA256)
