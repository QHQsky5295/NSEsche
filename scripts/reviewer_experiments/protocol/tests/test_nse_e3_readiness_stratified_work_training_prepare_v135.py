...
class ReadinessStratifiedWorkPreparationV135Tests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.temporary = tempfile.TemporaryDirectory()
        cls.root = Path(cls.temporary.name) / "v135"
        cls.receipt = prepare_v135(cls.root)
    ...
    def test_two_arms_lock_exact_profiles_runtime_and_seed_product(self) -> None:
        ...
        for arm_id, role, profile, count in ARMS:
            ...
            for run in manifest["runs"]:
                ...
                self.assertEqual(metadata["v135_arm_id"], arm_id)
                self.assertEqual(metadata["v135_arm_role"], role)
                self.assertEqual(
                    metadata["v135_shock_activation_horizon_frames"],
                    50 if candidate else None,
                )
                self.assertEqual(
                    metadata["v135_critical_service_ratio_numerator"],
                    9 if candidate else None,
                )
                self.assertEqual(
                    metadata["v135_critical_service_ratio_denominator"],
                    10 if candidate else None,
                )
                self.assertEqual(
                    metadata["v135_service_proxy_work_source"],
                    (
                ...
                self.assertIs(
                    metadata["v135_readiness_stratified_work_disjoint_task_counting"],
                    candidate,
                )
                self.assertIs(
                    metadata["v135_readiness_stratified_work_deterministic_f64_sum"],
                    candidate,
                )
                self.assertFalse(metadata["v135_outcome_fields_drive_policy"])
    ...
    def test_plan_is_frozen_and_seed_boundaries_remain_sealed(self) -> None:
        ...
        self.assertEqual(
            plan["training_design"]["sealed_confirmation_seeds"], "E1451-E1470"
        )
        self.assertEqual(
            PREVIOUS_CONFIRMATION_SEEDS,
            [
