...
class InitializerComponentSafetyBlindAuditV103Tests(unittest.TestCase):
    def test_frozen_arm_seed_density_and_runtime_boundary(self) -> None:
        self.assertEqual(TRAINING_SEEDS, {"E878", "E879", "E880"})
        self.assertEqual(CONFIRMATION_SEEDS, [f"E{i}" for i in range(946, 966)])
        self.assertEqual(
            PREVIOUS_CONFIRMATION_SEEDS, [f"E{i}" for i in range(926, 946)]
        )
        self.assertEqual(
            OTHER_UNOPENED_SEEDS,
