...
class AdmittedWorkPreparationV122Tests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.temporary = tempfile.TemporaryDirectory()
        cls.root = Path(cls.temporary.name) / "v122"
        cls.receipt = prepare_v122(cls.root)
    ...
    def test_two_arms_lock_exact_profiles_runtime_and_seed_product(self) -> None:
        ...
        for arm_id, role, profile, count in ARMS:
            ...
            for run in manifest["runs"]:
                ...
                self.assertEqual(metadata["v122_arm_id"], arm_id)
                self.assertEqual(metadata["v122_arm_role"], role)
                self.assertEqual(
                    metadata["v122_shock_activation_horizon_frames"],
                    50 if candidate else None,
                )
                self.assertEqual(
                    metadata["v122_critical_service_ratio_numerator"],
                    9 if candidate else None,
                )
                self.assertEqual(
                    metadata["v122_critical_service_ratio_denominator"],
                    10 if candidate else None,
                )
                self.assertEqual(
                    metadata["v122_service_proxy_work_source"],
                    (
                ...
                self.assertIs(
                    metadata["v122_admitted_work_includes_all_blocked_resident"],
                    candidate,
                )
                self.assertIs(
                    metadata["v122_terminal_completion_only_substitution_scope"],
                    candidate,
                )
                self.assertEqual(
                    metadata["v122_terminal_definition"],
                    (
                ...
                self.assertIs(
                    metadata["v122_terminal_critical_frontier_substitution"],
                    candidate,
                )
                self.assertIs(metadata["v122_nonterminal_exact_anchor"], candidate)
                self.assertFalse(metadata["v122_outcome_fields_drive_policy"])
    ...
    def test_plan_is_frozen_and_seed_boundaries_remain_sealed(self) -> None:
        ...
        self.assertEqual(
            ...
            [f"E{i}" for i in range(1152, 1172)],
        )
        self.assertEqual(
