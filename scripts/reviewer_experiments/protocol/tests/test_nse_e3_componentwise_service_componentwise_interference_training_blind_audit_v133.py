...
def _critical(
    ...
    return {
        ...
        "admitted_interference_pareto": {
            "gate_enabled": False,
            "componentwise_gate_enabled": enabled,
...
class ComponentwiseServiceComponentwiseInterferenceBlindAuditV133Tests(
    unittest.TestCase
):
    def test_frozen_arm_seed_and_runtime_boundary(self) -> None:
        self.assertEqual(TRAINING_SEEDS, {"E1402", "E1403", "E1404"})
        self.assertEqual(CONFIRMATION_SEEDS, [f"E{i}" for i in range(1405, 1425)])
        self.assertEqual(
            PREVIOUS_CONFIRMATION_SEEDS,
    ...
    def test_diagnostics_require_admitted_snapshot_and_ratio_certificate(
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            ]["admitted_interference_pareto"]["componentwise_gate_enabled"] = False
            with gzip.open(path, "wt", encoding="utf-8") as handle:
                for event in windows:
                    handle.write(json.dumps(event) + "\n")
            with self.assertRaisesRegex(
                RuntimeError, "componentwise-interference contract changed"
            ):
                _validate_admitted_work_diagnostics(
                    {"run_id": run_id, "simulation": {"total_frame": 3}},
                    canonical,
