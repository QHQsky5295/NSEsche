...
class RemainingQueueWorkPreparationV111Tests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.temporary = tempfile.TemporaryDirectory()
        cls.root = Path(cls.temporary.name) / "v111"
        cls.receipt = prepare_v111(cls.root)
    ...
    def test_two_arms_lock_exact_profiles_runtime_and_seed_product(self) -> None:
        ...
        for arm_id, role, profile, count in ARMS:
            ...
            for run in manifest["runs"]:
                ...
                self.assertEqual(metadata["v111_arm_id"], arm_id)
                self.assertEqual(metadata["v111_arm_role"], role)
                self.assertEqual(
                    metadata["v111_shock_activation_horizon_frames"],
                    50 if candidate else None,
                )
                self.assertEqual(
                    metadata["v111_critical_service_ratio_numerator"],
                    9 if candidate else None,
                )
                self.assertEqual(
                    metadata["v111_critical_service_ratio_denominator"],
                    10 if candidate else None,
                )
                self.assertEqual(
                    metadata["v111_service_proxy_work_source"],
                    (
                ...
                self.assertIs(
                    metadata["v111_remaining_queue_work_excludes_blocked_resident"],
                    candidate,
                )
                self.assertFalse(metadata["v111_outcome_fields_drive_policy"])
