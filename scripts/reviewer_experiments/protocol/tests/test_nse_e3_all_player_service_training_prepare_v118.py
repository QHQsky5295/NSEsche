...
class AllPlayerServicePreparationV118Tests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.temporary = tempfile.TemporaryDirectory()
        cls.root = Path(cls.temporary.name) / "v118"
        cls.receipt = prepare_v118(cls.root)
    ...
    def test_two_arms_lock_exact_profiles_runtime_and_seed_product(self) -> None:
        ...
        for arm_id, role, profile, count in ARMS:
            ...
            for run in manifest["runs"]:
                ...
                self.assertEqual(metadata["v118_arm_id"], arm_id)
                self.assertEqual(metadata["v118_arm_role"], role)
                self.assertEqual(
                    metadata["v118_shock_activation_horizon_frames"],
                    50 if candidate else None,
                )
                self.assertEqual(
                    metadata["v118_critical_service_ratio_numerator"],
                    9 if candidate else None,
                )
                self.assertEqual(
                    metadata["v118_critical_service_ratio_denominator"],
                    10 if candidate else None,
                )
                self.assertEqual(
                    metadata["v118_service_proxy_work_source"],
                    (
                ...
                self.assertIs(
                    metadata["v118_admitted_work_includes_all_blocked_resident"],
                    candidate,
                )
                self.assertIs(
                    metadata["v118_complete_componentwise_service_pareto"],
                    candidate,
                )
                self.assertEqual(
                    metadata["v118_componentwise_service_scope"],
                    "all_current_players" if candidate else "not_applicable",
                )
                self.assertEqual(
                    metadata["v118_componentwise_service_comparison"],
                    (
                ...
                self.assertEqual(
                    metadata["v118_componentwise_service_replay_order"],
                    "native_player_order" if candidate else "not_applicable",
                )
                self.assertFalse(metadata["v118_interference_constraints"])
                self.assertFalse(metadata["v118_arrival_phase_guard"])
                self.assertFalse(metadata["v118_outcome_fields_drive_policy"])
