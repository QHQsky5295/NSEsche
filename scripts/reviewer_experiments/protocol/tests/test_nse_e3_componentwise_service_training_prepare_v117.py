...
class ComponentwiseServicePreparationV117Tests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.temporary = tempfile.TemporaryDirectory()
        cls.root = Path(cls.temporary.name) / "v117"
        cls.receipt = prepare_v117(cls.root)
    ...
    def test_two_arms_lock_exact_profiles_runtime_and_seed_product(self) -> None:
        ...
        for arm_id, role, profile, count in ARMS:
            ...
            for run in manifest["runs"]:
                ...
                self.assertEqual(metadata["v117_arm_id"], arm_id)
                self.assertEqual(metadata["v117_arm_role"], role)
                self.assertEqual(
                    metadata["v117_shock_activation_horizon_frames"],
                    50 if candidate else None,
                )
                self.assertEqual(
                    metadata["v117_critical_service_ratio_numerator"],
                    9 if candidate else None,
                )
                self.assertEqual(
                    metadata["v117_critical_service_ratio_denominator"],
                    10 if candidate else None,
                )
                self.assertEqual(
                    metadata["v117_service_proxy_work_source"],
                    (
                ...
                self.assertIs(
                    metadata["v117_admitted_work_includes_all_blocked_resident"],
                    candidate,
                )
                self.assertIs(
                    metadata["v117_complete_componentwise_service_pareto"],
                    candidate,
                )
                self.assertEqual(
                    metadata["v117_componentwise_service_replay_order"],
                    "native_player_order" if candidate else "not_applicable",
                )
                self.assertFalse(metadata["v117_interference_constraints"])
                self.assertFalse(metadata["v117_arrival_phase_guard"])
                self.assertFalse(metadata["v117_outcome_fields_drive_policy"])
