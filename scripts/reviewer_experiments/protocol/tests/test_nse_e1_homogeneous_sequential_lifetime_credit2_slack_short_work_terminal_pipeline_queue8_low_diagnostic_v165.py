...
from scripts.reviewer_experiments.protocol import (
    nse_e1_homogeneous_sequential_lifetime_credit2_slack_short_work_terminal_pipeline_queue8_low_diagnostic_v165 as v165,
)
...
class V165ProtocolTests(unittest.TestCase):
    def test_frozen_plan_implementation_and_exact_product(self) -> None:
        self.assertEqual(file_hash(v165.PLAN), v165.PLAN_SHA256)
        self.assertEqual(file_hash(v165.IMPLEMENTATION), v165.IMPLEMENTATION_SHA256)
        self.assertEqual(file_hash(v165.BINARY_PATH), v165.BINARY_SHA256)
        plan = read_json(v165.PLAN)
        implementation = read_json(v165.IMPLEMENTATION)
        self.assertEqual(plan["diagnostic_design"]["seeds"], list(v165.SEEDS))
        self.assertEqual(plan["candidate"]["profile"], v165.PROFILE)
        ...
        self.assertEqual(change["projected_outstanding_speculation_cap"], 1)
        self.assertTrue(change["v164_ready_antihotspot_disabled"])
        manifest = v165._rewrite_candidate(v165._assert_frozen_inputs(), "c" * 40)
        self.assertEqual([run["seed"] for run in manifest["runs"]], list(v165.SEEDS))
        self.assertEqual(len(manifest["reference_build_dependencies"]), 3)
        v165._validate_product(manifest, references_bound=False)
        self.assertTrue(
            all(
                run["metadata"]["v165_lifetime_credit_limit_per_request"] == 2
                and run["metadata"][
                    "v165_second_credit_requires_zero_outstanding_speculation"
                ]
                is True
                and run["metadata"]["v165_ready_antihotspot_enabled"] is False
                and run["metadata"]["v165_operational_score"]
        ...
        events = [
            {
                "kind": "run_config",
                "scheduler": "sche_nash",
                "operational_expert_proxy": v165.PROFILE,
                "reference": {"mode": "offline_required", "offline_load_ok": True},
                "operational_expert_proxy_contract": {
                    "version": "V165",
                    "queue_density_threshold": 8.0,
                    "below_threshold_expert": v165.LOW_EXPERT,
                    "at_or_above_threshold_expert": v165.HIGH_EXPERT,
                    "player_frontier": v165.FRONTIER,
                    "single_change_from_v155": v165.SINGLE_CHANGE,
                    "terminal_pipeline_definition": v165.TERMINAL_DEFINITION,
                    ...
                    "short_work_definition": v165.WORK_DEFINITION,
                    ...
                    "lifetime_short_work_credit_definition": (
                        v165.LIFETIME_CREDIT_CONTRACT_DEFINITION
                    ),
                    ...
                    "decision": {
                        ...
                        "terminal_pipeline_frontier": {
                            ...
                            "lifetime_short_work_credit": {
                                "enabled": True,
                                "definition": v165.LIFETIME_CREDIT_DEFINITION,
                                "credit_limit_per_request_lifetime": 2,
                                "selection_order": v165.SELECTION_ORDER,
                        ...
                        "srpt_hiku2_ocs_queue_router": {
                            ...
                            "queue_density_threshold": 8.0,
                            "selected_expert": (
                                v165.LOW_EXPERT if low else v165.HIGH_EXPERT
                            ),
                            "player_frontier": v165.FRONTIER,
    ...
    def test_blind_log_proves_sequential_credit_and_disabled_antihotspot(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            canonical = Path(tmp)
            self._write_log(canonical, "synthetic-v165")
            evidence = v165._audit_nash_log(
                canonical, {"run_id": "synthetic-v165", "seed": "E09"}
            )
            ...
            gate = v165._mechanism_falsification_gate([evidence])
            ...
            with self.subTest(outstanding=outstanding, projected=projected):
                with tempfile.TemporaryDirectory() as tmp:
                    ...
                    with self.assertRaisesRegex(
                        RuntimeError, "sequential lifetime-credit2 evidence changed"
                    ):
                        v165._audit_nash_log(
                            canonical,
                            {"run_id": "synthetic-v165-bad", "seed": "E09"},
                        )

    def test_reveal_refuses_without_sequential_blind_gate(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            blind = {
                "status": "pass",
                ...
                "sequential_lifetime_credit_invariants_passed": False,
            ...
            write_json_atomic(v165.paths(root)["blind"], blind)
            with self.assertRaisesRegex(RuntimeError, "did not authorize reveal"):
                v165.reveal_v165(root)
