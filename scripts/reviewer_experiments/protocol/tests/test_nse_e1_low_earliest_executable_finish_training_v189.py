...
from scripts.reviewer_experiments.protocol import (
    nse_e1_homogeneous_20node_low_earliest_executable_finish_training_v189 as v189,
)


class EarliestExecutableFinishTrainingV189Tests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.manifest = v189._build_tape_bound_manifest("a" * 40)

    def test_plan_and_v188_failure_are_frozen(self) -> None:
        plan = v189._assert_plan()
        self.assertEqual(plan["plan_hash"], v189.PLAN_HASH)
        self.assertEqual(tuple(plan["cohort"]["seed_order"]), v189.SEEDS)
        ...
        self.assertFalse(plan["mechanistic_hypothesis"]["uses_queue_density_router"])
        self.assertFalse(plan["mechanistic_hypothesis"]["uses_ordinal_borda"])
        self.assertEqual(
            v189.BINARY_SOURCE_COMMIT,
            "549dbe4f7a8484fe430209e12707cd9bd732e0dd",
        )

    def test_manifest_is_exact_candidate_only_tape_reuse_product(self) -> None:
        v189._validate_candidate(self.manifest, references_bound=False)
        self.assertEqual(len(self.manifest["runs"]), 20)
        self.assertEqual(
            [run["seed"] for run in self.manifest["runs"]], list(v189.SEEDS)
        )
        self.assertEqual(
    ...
    def test_profile_has_no_external_expert_and_reuses_exact_v187_tapes(self) -> None:
        control, source = v189._source_manifests()
        ...
        for seed in v189.SEEDS:
            candidate = candidate_by_seed[seed]
            self.assertEqual(
                candidate["environment"]["NASH_OPERATIONAL_EXPERT_PROXY"],
                v189.CANDIDATE_PROFILE,
            )
            self.assertFalse(
                candidate["metadata"]["v189_external_baseline_scores_allowed"]
            )
            self.assertFalse(candidate["metadata"]["v189_uses_queue_density_router"])
            self.assertFalse(candidate["metadata"]["v189_uses_ordinal_borda"])
            self.assertEqual(
                candidate["metadata"]["v189_new_control_or_baseline_runs"], 0
            )
            self.assertEqual(
    ...
    def _evaluate(self, rows: list[dict[str, object]]) -> dict[str, object]:
        with mock.patch.object(
            v189.v187, "bca_interval", return_value={"low": 0.0, "high": 0.0}
        ), mock.patch.object(
            v189.v187.v186,
            "_paired_permutation",
            return_value={"used_as_gate": False},
        ):
            return v189._evaluate_training(rows)
    ...
    def test_prepare_refuses_to_overwrite_existing_root(self) -> None:
        with tempfile.TemporaryDirectory(dir="tmp") as directory:
            with mock.patch.object(v189, "_assert_frozen_inputs"):
                with self.assertRaisesRegex(RuntimeError, "overwrite"):
                    v189.prepare_v189(Path(directory))
