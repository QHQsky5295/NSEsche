...
import scripts.reviewer_experiments.protocol.nse_e3e4_low_density_load_pareto_training_reveal_v97 as reveal_module
from scripts.reviewer_experiments.protocol.nse_e3e4_low_density_load_pareto_training_reveal_v97 import (
    ARMS,
    EXPECTED_SCENARIOS,
    EXPECTED_SEEDS,
    ...
    for arm_id, arm in ARMS.items():
        ...
        for scenario in scenarios:
            for index, seed in enumerate(EXPECTED_SEEDS):
                ...
                elif arm_id in {"v97-e3-load-guard32", "v97-e4-load-guard64"}:
...
class LowDensityLoadParetoTrainingRevealV97Tests(unittest.TestCase):
    ...
    def test_joint_gate_selects_only_direction_consistent_candidates(self) -> None:
        result = evaluate_training_rows(rows_fixture(), THRESHOLDS)
        self.assertTrue(result["joint_training_gate_pass"])
        self.assertEqual(
            result["selected_profiles"]["E3"]["arm_id"], "v97-e3-load-guard32"
        )
        self.assertEqual(
            result["selected_profiles"]["E4"]["arm_id"], "v97-e4-load-guard64"
        )
        self.assertFalse(
            result["dimension_scores"]["E3"]["v97-e3-load-guard64"][
                "all_required_gates_pass"
            ]
        )
        diagnostic = result["arm_aggregates"]["v97-e3-load-guard32"]["E3.spike5x50ms"][
            "formal_absolute_diagnostics"
        ]["qpr_finite_only"]
        self.assertFalse(diagnostic["used_for_V97_selection"])

    def test_one_large_outlier_cannot_satisfy_two_of_three_rule(self) -> None:
        rows = rows_fixture()
        for row in rows:
            if (
                row["arm_id"] == "v97-e3-load-guard32"
                and row["scenario_id"] == "E3.spike5x50ms"
            ):
                value = 10.0 if row["seed"] == "E788" else 0.9
                row["qpr_finite_only"] = value
                row["qpr_zero_completed_as_zero"] = value
        result = evaluate_training_rows(rows, THRESHOLDS)
        gate = result["paired_candidate_results"]["v97-e3-load-guard32"][
            "E3.spike5x50ms"
