...
class InitializerComponentSafetyPreparationV103Tests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.temporary = tempfile.TemporaryDirectory()
        cls.root = Path(cls.temporary.name) / "v103"
        cls.receipt = prepare_v103(cls.root)
    ...
    def test_exact_arms_profiles_density_bounds_and_runtime(self) -> None:
        ...
        for arm_id, manifest in self.arms.items():
            ...
            self.assertEqual(
                {
                    run["metadata"]["v103_upper_queue_density_threshold"]
                    for run in manifest["runs"]
                },
                {upper_density},
            )
            self.assertTrue(
                all(
                    run["metadata"]["v103_nonterminal_queue_density_floor"]
                    == (8.0 if role == "candidate" else None)
                    and run["metadata"]["v103_warm_admissibility"]
                    == ("preserve_anchor_warmness" if role == "candidate" else None)
                    and run["metadata"]["v103_density_band_initializer_guard"]
                    is (role == "candidate")
                    and run["metadata"]["v103_load_least_window_certificate_mode"]
                    == ("disabled" if role == "candidate" else "not_applicable")
                    and run["metadata"]["v103_component_safety_mode"]
                    == component_safety_mode
                    and run["metadata"]["v103_scalar_faasrank_noninferiority"]
                    is (role == "candidate")
                    and run["metadata"]["v103_input_locality_component_noninferiority"]
                    is (role == "candidate")
                    and run["metadata"]["v103_componentwise_faasrank_noninferiority"]
                    is (
                        component_safety_mode
                        == "componentwise_cpu_memory_locality_warm_diversity_noninferiority"
                    )
                    and run["metadata"]["v103_substitution_cap"] is None
                    and run["metadata"]["v103_confirmation_seeds_opened"] is False
                    and run["metadata"]["v103_other_unopened_seeds_opened"] is False
