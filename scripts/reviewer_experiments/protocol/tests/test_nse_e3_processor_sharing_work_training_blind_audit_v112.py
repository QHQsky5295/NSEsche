...
from scripts.reviewer_experiments.protocol.nse_e3_processor_sharing_work_training_blind_audit_v112 import (
    ...
    _validate_processor_sharing_work_diagnostics,
)
...
class ProcessorSharingWorkBlindAuditV112Tests(unittest.TestCase):
    def test_frozen_arm_seed_and_runtime_boundary(self) -> None:
        self.assertEqual(TRAINING_SEEDS, {"E905", "E906", "E907"})
        self.assertEqual(CONFIRMATION_SEEDS, [f"E{i}" for i in range(1066, 1086)])
        self.assertEqual(
    ...
    def test_diagnostics_require_processor_sharing_snapshot_and_ratio_certificate(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
                ...
                windows.append(
                    {
                        ...
                        "cluster": {
                            ...
                            "queue_runnable_remaining_cpu_work_total": 2.0,
                            "queue_runnable_remaining_cpu_work_max": 1.5,
                            "queue_remaining_cpu_work_observation_complete": True,
                            "queue_processor_sharing_work_observation_complete": True,
            ...
            evidence = _validate_processor_sharing_work_diagnostics(
                {"run_id": run_id, "simulation": {"total_frame": 2}},
                canonical,
                expected,
            )
            self.assertEqual(evidence["window_count"], 2)
            self.assertEqual(evidence["remaining_work_complete_window_count"], 2)
            self.assertEqual(evidence["processor_sharing_complete_window_count"], 2)
            ...
            self.assertEqual(evidence["work_source"], PROCESSOR_SHARING_WORK_SOURCE)
            self.assertFalse(evidence["performance_fields_consulted"])

    def test_diagnostics_reject_legacy_or_incomplete_remaining_work(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            run_id = "fixture"
            canonical = Path(temp_dir)
            path = canonical / "reviewer_records" / run_id / "nash_metrics.jsonl.gz"
            ...
            event = {
                ...
                "cluster": {
                    ...
                    "queue_runnable_remaining_cpu_work_total": 0.0,
                    "queue_runnable_remaining_cpu_work_max": 0.0,
                    "queue_remaining_cpu_work_observation_complete": False,
                    "queue_processor_sharing_work_observation_complete": False,
            ...
            with self.assertRaisesRegex(
                RuntimeError, "processor-sharing-work observation changed"
            ):
                _validate_processor_sharing_work_diagnostics(
