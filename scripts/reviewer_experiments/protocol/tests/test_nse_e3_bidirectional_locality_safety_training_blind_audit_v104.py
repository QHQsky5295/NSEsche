...
class InitializerComponentSafetyBlindAuditV104Tests(unittest.TestCase):
    def test_frozen_arm_seed_density_and_runtime_boundary(self) -> None:
        self.assertEqual(TRAINING_SEEDS, {"E881", "E882", "E883"})
        self.assertEqual(CONFIRMATION_SEEDS, [f"E{i}" for i in range(966, 986)])
        self.assertEqual(
            PREVIOUS_CONFIRMATION_SEEDS, [f"E{i}" for i in range(926, 966)]
        )
        self.assertEqual(
            OTHER_UNOPENED_SEEDS,
        ...
        self.assertEqual(CANONICAL_RENAME_RECEIPT, "canonical_rename_receipt_v104.json")
        self.assertEqual(
