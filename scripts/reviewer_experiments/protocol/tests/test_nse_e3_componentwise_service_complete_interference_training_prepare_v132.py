...
class ComponentwiseServiceCompleteInterferencePreparationV132Tests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.temporary = tempfile.TemporaryDirectory()
        cls.root = Path(cls.temporary.name) / "v132"
        cls.receipt = prepare_v132(cls.root)
    ...
    def test_two_arms_lock_exact_profiles_runtime_and_seed_product(self) -> None:
        ...
        for arm_id, role, profile, count in ARMS:
            ...
            for run in manifest["runs"]:
                ...
                self.assertEqual(metadata["v132_arm_id"], arm_id)
                self.assertEqual(metadata["v132_arm_role"], role)
                self.assertEqual(
                    metadata["v132_shock_activation_horizon_frames"],
                    50 if candidate else None,
                )
                self.assertEqual(
                    metadata["v132_critical_service_ratio_numerator"],
                    9 if candidate else None,
                )
                self.assertEqual(
                    metadata["v132_critical_service_ratio_denominator"],
                    10 if candidate else None,
                )
                self.assertEqual(
                    metadata["v132_service_proxy_work_source"],
                    (
                ...
                self.assertIs(
                    metadata["v132_admitted_work_includes_all_blocked_resident"],
                    candidate,
                )
                self.assertIs(
                    metadata["v132_complete_componentwise_service_pareto"],
                    candidate,
                )
                self.assertEqual(
                    metadata["v132_componentwise_service_replay_order"],
                    "native_player_order" if candidate else "not_applicable",
                )
                self.assertIs(
                    metadata["v132_complete_admitted_interference_pareto"], candidate
                )
                self.assertFalse(
                    metadata["v132_componentwise_admitted_interference_pareto"]
                )
                self.assertEqual(
                    metadata["v132_complete_interference_comparison"],
                    (
                ...
                self.assertIs(
                    metadata["v132_complete_interference_inputs_finite_fail_closed"],
                    candidate,
                )
                self.assertFalse(metadata["v132_arrival_phase_guard"])
                self.assertFalse(metadata["v132_outcome_fields_drive_policy"])
    ...
    def test_plan_is_frozen_and_seed_boundaries_remain_sealed(self) -> None:
        ...
        self.assertEqual(
            plan["training_design"]["sealed_confirmation_seeds"], "E1382-E1401"
        )
        self.assertEqual(
            PREVIOUS_CONFIRMATION_SEEDS,
        ...
        sealed = set(CONFIRMATION_SEEDS) | set(PREVIOUS_CONFIRMATION_SEEDS)
        self.assertTrue(TRAINING_SEEDS.issubset(SOURCE_INITIAL_SEEDS))
        self.assertFalse((set(SOURCE_INITIAL_SEEDS) - TRAINING_SEEDS) & sealed)
        self.assertFalse(set(SOURCE_CI_SEEDS) & sealed)
        source = json.loads(self.paths["source"].read_text(encoding="utf-8"))
        self.assertEqual(
            {run["seed"] for run in source["runs"]}, set(SOURCE_INITIAL_SEEDS)
        )

    def test_existing_root_is_never_overwritten(self) -> None:
        with self.assertRaisesRegex(RuntimeError, "refusing to overwrite"):
            prepare_v132(self.root)
