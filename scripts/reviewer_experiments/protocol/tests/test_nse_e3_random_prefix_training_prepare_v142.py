...
class RandomAnchorPreparationV142Tests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.temporary = tempfile.TemporaryDirectory()
        cls.root = Path(cls.temporary.name) / "v142"
        cls.receipt = prepare_v142(cls.root)
    ...
    def test_exact_complete_products_profiles_and_portfolio_metadata(self) -> None:
        ...
        for arm_id, profile, selection_rule in ARMS:
            ...
            for run in manifest["runs"]:
                ...
                self.assertEqual(metadata["v142_arm_id"], arm_id)
                self.assertEqual(metadata["v142_native_selection_rule"], selection_rule)
                self.assertEqual(
                    metadata["v142_native_portfolio_enabled"],
                    selection_rule != "exact_random_prefix",
                )
                self.assertEqual(
                    metadata["v142_native_portfolio_members"],
                    native_members(selection_rule),
                )
                self.assertEqual(
                    metadata["v142_random_shadow_seed_source"], "algorithm_seed"
                )
                self.assertTrue(metadata["v142_random_default_required"])
                self.assertTrue(metadata["v142_native_shadow_exactness_required"])
                self.assertEqual(
                    metadata["v142_service_certificate_scope"],
                    SERVICE_CERTIFICATE_SCOPE,
                )
                self.assertTrue(
                    metadata["v142_nonrandom_native_sources_complete_all_frontier"]
                )
                self.assertTrue(
                    metadata["v142_prefix_service_complete_assignment_required"]
                )
                self.assertEqual(
                    metadata["v142_service_certificate_state_domain"],
                    SERVICE_CERTIFICATE_STATE_DOMAIN,
                )
                self.assertEqual(
                    metadata["v142_paper_welfare_state_domain"],
                    PAPER_WELFARE_STATE_DOMAIN,
                )
                self.assertTrue(
                    metadata["v142_selected_welfare_equals_guard_initializer_required"]
                )
                self.assertFalse(metadata["v142_outcome_fields_drive_policy"])
    ...
    def test_pairing_and_confirmation_information_boundary(self) -> None:
        ...
        self.assertEqual(
            self.receipt["v141_disposition"],
            "technical_reference_build_failure_before_online_execution_no_reveal",
        )
        self.assertEqual(self.receipt["v141_performance_summaries_parsed"], 0)
        self.assertFalse(self.receipt["v141_performance_results_consulted"])
        self.assertEqual(self.receipt["v141_online_runs_started"], 0)
        self.assertEqual(self.receipt["v141_online_runs_canonicalized"], 0)
