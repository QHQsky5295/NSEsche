...
class AllNativePortfolioPreparationV138Tests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.temporary = tempfile.TemporaryDirectory()
        cls.root = Path(cls.temporary.name) / "v138"
        cls.receipt = prepare_v138(cls.root)
    ...
    def test_exact_complete_products_profiles_and_portfolio_metadata(self) -> None:
        ...
        for arm_id, profile, portfolio_rule in ARMS:
            ...
            for run in manifest["runs"]:
                ...
                self.assertEqual(metadata["v138_arm_id"], arm_id)
                self.assertEqual(metadata["v138_native_portfolio_rule"], portfolio_rule)
                self.assertEqual(metadata["v138_native_portfolio_members"], members)
                self.assertTrue(metadata["v138_native_shadow_exactness_required"])
                self.assertTrue(metadata["v138_all_native_members_use_all_frontier"])
                self.assertTrue(
                    metadata["v138_all_player_service_complete_assignment_required"]
                )
                self.assertFalse(metadata["v138_outcome_fields_drive_policy"])
    ...
    def test_pairing_and_confirmation_information_boundary(self) -> None:
        ...
        sealed = set(RETIRED_V137_CONFIRMATION_SEEDS + NEW_CONFIRMATION_SEEDS)
