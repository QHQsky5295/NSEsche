    ...
    return {
        ...
        "service_directed_construction": {
            ...
            "terminal_completion_only": {
                "enabled": invoked,
                "terminal_definition": "function_children_is_empty_in_the_frozen_request_DAG",
                "terminal_player_count": 1 if invoked else 0,
                "nonterminal_player_count": 1 if invoked else 0,
                "terminal_service_choice_target_player_count": 1 if invoked else 0,
                "terminal_service_choice_evaluated_player_count": 1 if invoked else 0,
                "terminal_service_choice_substitution_count": 1 if invoked else 0,
                "nonterminal_exact_anchor_preserved_player_count": 1 if invoked else 0,
                "nonterminal_substitution_count": 0,
                "nonterminal_rule": "exact_anchor_node_is_preserved",
                "uses_completion_outcomes": False,
            },
        },
    }
...
class AdmittedWorkBlindAuditV122Tests(unittest.TestCase):
    def test_frozen_arm_seed_and_runtime_boundary(self) -> None:
        self.assertEqual(TRAINING_SEEDS, {"E1149", "E1150", "E1151"})
        self.assertEqual(CONFIRMATION_SEEDS, [f"E{i}" for i in range(1152, 1172)])
        self.assertEqual(
            PREVIOUS_CONFIRMATION_SEEDS, [f"E{i}" for i in range(932, 1149)]
        )
        self.assertEqual(
            OTHER_UNOPENED_SEEDS,
    ...
    def test_diagnostics_require_admitted_snapshot_and_ratio_certificate(
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual(
                evidence["terminal_service_choice_evaluated_player_count"], 1
            )
            self.assertEqual(evidence["terminal_service_choice_substitution_count"], 1)
            self.assertEqual(
                evidence["nonterminal_exact_anchor_preserved_player_count"], 1
            )
            self.assertEqual(evidence["nonterminal_substitution_count"], 0)
