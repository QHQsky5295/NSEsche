...
class WarmPreserveDensityBandTrainingPreparationV101Tests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.temporary = tempfile.TemporaryDirectory()
        cls.root = Path(cls.temporary.name) / "v101"
        cls.receipt = prepare_v101(cls.root)
    ...
    def test_exact_arms_profiles_density_bounds_and_runtime(self) -> None:
        ...
        for arm_id, manifest in self.arms.items():
            ...
            self.assertEqual(
                {
                    run["metadata"]["v101_upper_queue_density_threshold"]
                    for run in manifest["runs"]
                },
                {upper_density},
            )
            self.assertTrue(
                all(
                    run["metadata"]["v101_nonterminal_queue_density_floor"]
                    == (8.0 if role == "candidate" else None)
                    and run["metadata"]["v101_warm_admissibility"]
                    == ("preserve_anchor_warmness" if role == "candidate" else None)
                    and run["metadata"]["v101_density_band_warm_preserve_guard"]
                    is (role == "candidate")
                    and run["metadata"]["v101_substitution_cap"] is None
                    and run["metadata"]["v101_confirmation_seeds_opened"] is False
                    and run["metadata"]["v101_other_unopened_seeds_opened"] is False
