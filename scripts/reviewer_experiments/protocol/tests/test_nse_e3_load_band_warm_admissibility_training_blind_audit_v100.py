...
class LoadBandWarmAdmissibilityTrainingBlindAuditV100Tests(unittest.TestCase):
    def test_frozen_arm_seed_and_runtime_boundary(self) -> None:
        self.assertEqual(TRAINING_SEEDS, {"E869", "E870", "E871"})
        self.assertEqual(CONFIRMATION_SEEDS, [f"E{i}" for i in range(906, 926)])
        self.assertEqual(
            PREVIOUSLY_RESERVED_SEEDS,
            [f"E{i}" for i in range(766, 786)]
            + [f"E{i}" for i in range(806, 826)]
            + [f"E{i}" for i in range(846, 866)]
            + [f"E{i}" for i in range(872, 906)],
        )
        ...
        self.assertEqual(
            ...
            {None, "preserve_anchor_warmness", "candidate_warm_only"},
        )
