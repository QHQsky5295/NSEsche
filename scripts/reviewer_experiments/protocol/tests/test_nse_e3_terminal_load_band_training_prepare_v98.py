...
class TerminalLoadBandTrainingPreparationV98Tests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.temporary = tempfile.TemporaryDirectory()
        cls.root = Path(cls.temporary.name) / "v98"
        cls.receipt = prepare_v98(cls.root)
    ...
    def test_exact_arms_profiles_thresholds_and_runtime(self) -> None:
        expected = {
            arm_id: (experiment_id, role, profile, floor, count)
            for arm_id, experiment_id, role, profile, floor, count in ARMS
        }
        ...
        for arm_id, manifest in self.arms.items():
            experiment_id, role, profile, floor, count = expected[arm_id]
            ...
            self.assertEqual(
                {
                    run["metadata"]["v98_nonterminal_queue_density_floor"]
                    for run in manifest["runs"]
                },
                {floor},
            )
            self.assertTrue(
                all(
                    run["metadata"]["v98_terminal_load_band_guard"]
                    is (floor is not None)
                    and run["metadata"]["v98_upper_queue_density_threshold"]
                    == (64.0 if role == "candidate" else None)
                    and run["metadata"]["v98_confirmation_seeds_opened"] is False
                    and run["metadata"]["v98_previously_reserved_seeds_opened"] is False
    ...
    def test_arm_references_are_unique_and_tapes_are_paired(self) -> None:
        ...
        e3_sets = [keys for arm, keys in tape_sets.items() if arm.startswith("v98-e3")]
