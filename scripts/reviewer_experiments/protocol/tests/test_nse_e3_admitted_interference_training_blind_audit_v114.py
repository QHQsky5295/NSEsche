...
def _critical(invoked: bool, *, work_source: str) -> dict:
    return {
        ...
        "admitted_interference_pareto": {
            "gate_enabled": invoked,
            "rejected_candidate_count": 1 if invoked else 0,
...
class AdmittedInterferenceBlindAuditV114Tests(unittest.TestCase):
    def test_frozen_arm_seed_and_runtime_boundary(self) -> None:
        self.assertEqual(TRAINING_SEEDS, {"E911", "E912", "E913"})
        self.assertEqual(CONFIRMATION_SEEDS, [f"E{i}" for i in range(1086, 1106)])
        self.assertEqual(
