...
from scripts.reviewer_experiments.protocol.nse_e3_readiness_stratified_work_training_blind_audit_v135 import (
    ...
    READINESS_STRATIFIED_WORK_SOURCE,
    TRAINING_SEEDS,
    _assert_hashed_object,
    _assert_ledger_contract,
    _validate_readiness_stratified_work_diagnostics,
)
...
class ReadinessStratifiedWorkBlindAuditV135Tests(unittest.TestCase):
    def test_frozen_arm_seed_and_runtime_boundary(self) -> None:
        self.assertEqual(TRAINING_SEEDS, {"E1448", "E1449", "E1450"})
        self.assertEqual(CONFIRMATION_SEEDS, [f"E{i}" for i in range(1451, 1471)])
        self.assertEqual(
            PREVIOUS_CONFIRMATION_SEEDS,
            [
                *[f"E{i}" for i in range(1428, 1448)],
    ...
    def test_diagnostics_require_readiness_partition_and_ratio_certificate(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
                ...
                windows.append(
                    {
                        ...
                        "cluster": {
                            ...
                            "queue_runnable_remaining_cpu_work_total": 1.0,
                            "queue_runnable_remaining_cpu_work_max": 1.0,
                            "queue_resident_remaining_cpu_work_total": 5.0,
                            "queue_resident_remaining_cpu_work_max": 4.0,
                            "queue_blocked_resident_remaining_cpu_work_total": 4.0,
                            "queue_blocked_resident_remaining_cpu_work_max": 4.0,
                            "queue_readiness_stratified_cpu_work_total": 11.0,
                            "queue_readiness_stratified_cpu_work_max": 4.0,
                            "queue_cpu_work_observation_complete": True,
                            "queue_admitted_work_observation_complete": True,
                            "queue_remaining_cpu_work_observation_complete": True,
                            "queue_readiness_stratified_work_observation_complete": True,
                            "queue_pending_cpu_value_count": 2,
                            "queue_runnable_remaining_cpu_value_count": 1,
            ...
            evidence = _validate_readiness_stratified_work_diagnostics(
            ...
            self.assertEqual(
                evidence["readiness_stratified_work_complete_window_count"], 2
            )
            self.assertEqual(evidence["pending_cpu_value_count"], 4)
            self.assertEqual(evidence["runnable_remaining_cpu_value_count"], 2)
            self.assertEqual(evidence["resident_remaining_cpu_value_count"], 6)
            self.assertEqual(evidence["pending_immutable_cpu_work_total_sum"], 8.0)
            self.assertEqual(evidence["runnable_immutable_cpu_work_total_sum"], 6.0)
            self.assertEqual(
                evidence["blocked_resident_remaining_cpu_work_total_sum"], 8.0
            )
            self.assertEqual(evidence["readiness_stratified_cpu_work_total_sum"], 22.0)
            self.assertEqual(evidence["threshold_met_window_count"], 1)
            self.assertEqual(evidence["critical_accepted_window_count"], 1)
            self.assertEqual(evidence["work_source"], READINESS_STRATIFIED_WORK_SOURCE)
            self.assertFalse(evidence["performance_fields_consulted"])

            windows[0]["cluster"]["queue_readiness_stratified_cpu_work_total"] = 10.0
            with gzip.open(path, "wt", encoding="utf-8") as handle:
                for event in windows:
                    handle.write(json.dumps(event) + "\n")
            with self.assertRaisesRegex(RuntimeError, "partition arithmetic changed"):
                _validate_readiness_stratified_work_diagnostics(
                    {"run_id": run_id, "simulation": {"total_frame": 2}},
                    canonical,
                    expected,
    ...
    def test_diagnostics_reject_legacy_or_incomplete_readiness_stratified_work(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            run_id = "fixture"
            ...
            event = {
                ...
                "cluster": {
                    ...
                    "queue_runnable_remaining_cpu_work_total": None,
                    "queue_runnable_remaining_cpu_work_max": None,
                    "queue_resident_remaining_cpu_work_total": None,
                    "queue_resident_remaining_cpu_work_max": None,
                    "queue_blocked_resident_remaining_cpu_work_total": None,
                    "queue_blocked_resident_remaining_cpu_work_max": None,
                    "queue_readiness_stratified_cpu_work_total": None,
                    "queue_readiness_stratified_cpu_work_max": None,
                    "queue_admitted_work_observation_complete": False,
                    "queue_remaining_cpu_work_observation_complete": False,
                    "queue_readiness_stratified_work_observation_complete": False,
                    "queue_pending_cpu_value_count": None,
                    "queue_runnable_remaining_cpu_value_count": None,
            ...
            with self.assertRaisesRegex(
                RuntimeError, "readiness-stratified-work observation changed"
            ):
                _validate_readiness_stratified_work_diagnostics(
