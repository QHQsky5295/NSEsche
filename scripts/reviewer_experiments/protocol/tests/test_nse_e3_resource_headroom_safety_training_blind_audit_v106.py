...
class ResourceHeadroomSafetyBlindAuditV106Tests(unittest.TestCase):
    def test_frozen_arm_seed_density_and_runtime_boundary(self) -> None:
        self.assertEqual(TRAINING_SEEDS, {"E887", "E888", "E889"})
        self.assertEqual(CONFIRMATION_SEEDS, [f"E{i}" for i in range(1006, 1026)])
        self.assertEqual(
        ...
        self.assertEqual(CANONICAL_RENAME_RECEIPT, "canonical_rename_receipt_v106.json")
        self.assertEqual(
        ...
        self.assertEqual(
            {item["resource_headroom_safety_mode"] for item in ARMS.values()},
            {
                "not_applicable",
