...
def _critical(invoked: bool, *, work_source: str) -> dict:
    return {
        ...
        "terminal_child_path_pareto": {
            ...
            "accepted_changed_parent_count": 1 if invoked else 0,
            "compared_child_count": 1 if invoked else 0,
            "compared_container_path_count": 2 if invoked else 0,
            "available_container_path_count": 2 if invoked else 0,
            "selected_rank1_path_count": 1 if invoked else 0,
            "selected_rank2_path_count": 1 if invoked else 0,
            "running_available_but_no_running_selected_child_count": 0,
            "input_unavailable_count": 0,
            "running_container_count": 1 if invoked else 0,
            "starting_container_count": 1 if invoked else 0,
            ...
            "comparison": "every_terminal_child_alternative_path_to_each_of_the_first_two_running_first_deterministic_anchor_ranked_hpa_containers_less_than_or_equal_to_anchor",
            ...
            "container_pairing": "same_first_two_running_first_deterministic_anchor_ranked_child_container_nodes",
            "aggregation": "complete_per_child_pareto_on_up_to_two_running_first_deterministic_anchor_ranked_witnesses",
            "protected_anchor_path_witness_count": 2,
            "uses_completion_outcomes": False,
        },
...
class CompletionProximalBlindAuditV131Tests(unittest.TestCase):
    def test_frozen_arm_seed_and_runtime_boundary(self) -> None:
        self.assertEqual(TRAINING_SEEDS, {"E1356", "E1357", "E1358"})
        self.assertEqual(CONFIRMATION_SEEDS, [f"E{i}" for i in range(1359, 1379)])
        self.assertEqual(
            PREVIOUS_CONFIRMATION_SEEDS,
            [
    ...
    def test_diagnostics_require_admitted_snapshot_and_ratio_certificate(
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual(
                evidence["terminal_child_path_accepted_changed_parent_count"], 1
            )
            self.assertEqual(evidence["terminal_child_path_compared_child_count"], 1)
            self.assertEqual(
                evidence["terminal_child_path_compared_container_path_count"], 2
            )
            self.assertEqual(
                evidence["terminal_child_path_available_container_path_count"], 2
            )
            self.assertEqual(
                evidence["terminal_child_path_selected_rank1_path_count"], 1
            )
            self.assertEqual(
                evidence["terminal_child_path_selected_rank2_path_count"], 1
            )
            self.assertEqual(
                evidence[
                    "terminal_child_path_running_available_but_no_running_selected_child_count"
                ],
                0,
            )
            self.assertEqual(evidence["terminal_child_path_input_unavailable_count"], 0)
            self.assertEqual(evidence["terminal_child_path_running_container_count"], 1)
            self.assertEqual(
                evidence["terminal_child_path_starting_container_count"], 1
            )
