...
class RandomAnchorPreparationV141Tests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.temporary = tempfile.TemporaryDirectory()
        cls.root = Path(cls.temporary.name) / "v141"
        cls.receipt = prepare_v141(cls.root)
    ...
    def test_exact_complete_products_profiles_and_portfolio_metadata(self) -> None:
        ...
        for arm_id, profile, selection_rule in ARMS:
            ...
            for run in manifest["runs"]:
                ...
                self.assertEqual(metadata["v141_arm_id"], arm_id)
                self.assertEqual(metadata["v141_native_selection_rule"], selection_rule)
                self.assertEqual(
                    metadata["v141_native_portfolio_enabled"],
                    selection_rule != "exact_random_anchor",
                )
                self.assertEqual(
                    metadata["v141_native_portfolio_members"],
                    native_members(selection_rule),
                )
                self.assertEqual(
                    metadata["v141_random_shadow_seed_source"], "algorithm_seed"
                )
                self.assertTrue(metadata["v141_random_default_required"])
                self.assertTrue(metadata["v141_native_shadow_exactness_required"])
                self.assertTrue(metadata["v141_all_native_members_use_all_frontier"])
                self.assertTrue(
                    metadata["v141_all_player_service_complete_assignment_required"]
                )
                self.assertEqual(
                    metadata["v141_service_certificate_state_domain"],
                    "runtime_existing_aggregates_and_admitted_work",
                )
                self.assertEqual(
                    metadata["v141_paper_welfare_state_domain"],
                    "empty_window_aggregates",
                )
                self.assertTrue(
                    metadata["v141_selected_welfare_equals_guard_initializer_required"]
                )
                self.assertFalse(metadata["v141_outcome_fields_drive_policy"])
    ...
    def test_pairing_and_confirmation_information_boundary(self) -> None:
        tape_sets = [
            {run["workload_tape"]["key"] for run in manifest["runs"]}
        ...
        self.assertEqual(
            self.receipt["v140_disposition"],
            "complete_training_family_falsified_no_candidate_selected",
        )
        self.assertEqual(
            self.receipt["v140_performance_summaries_parsed_before_blind_audit"], 0
        )
        self.assertTrue(self.receipt["v140_performance_results_consulted_after_reveal"])
    ...
    def test_plan_binary_receipt_and_capture_are_frozen(self) -> None:
        ...
        self.assertEqual(self.receipt["binary_source_commit"], BINARY_SOURCE_COMMIT)
