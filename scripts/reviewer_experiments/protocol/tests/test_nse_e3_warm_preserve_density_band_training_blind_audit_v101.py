...
class WarmPreserveDensityBandTrainingBlindAuditV101Tests(unittest.TestCase):
    def test_frozen_arm_seed_density_and_runtime_boundary(self) -> None:
        self.assertEqual(TRAINING_SEEDS, {"E872", "E873", "E874"})
        self.assertEqual(CONFIRMATION_SEEDS, [f"E{i}" for i in range(926, 946)])
        self.assertEqual(
            OTHER_UNOPENED_SEEDS,
            [f"E{i}" for i in range(766, 786)]
            + [f"E{i}" for i in range(806, 826)]
            + [f"E{i}" for i in range(846, 866)]
            + [f"E{i}" for i in range(875, 926)],
        )
