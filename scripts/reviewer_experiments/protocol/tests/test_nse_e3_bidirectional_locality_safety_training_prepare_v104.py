...
class InitializerComponentSafetyPreparationV104Tests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.temporary = tempfile.TemporaryDirectory()
        cls.root = Path(cls.temporary.name) / "v104"
        cls.receipt = prepare_v104(cls.root)
    ...
    def test_exact_arms_profiles_density_bounds_and_runtime(self) -> None:
        expected = {
            ...
            for arm_id, experiment_id, role, profile, upper_density, component_safety_mode, count in ARMS
        }
        ...
        for arm_id, manifest in self.arms.items():
            ...
            self.assertEqual(
                {
                    run["metadata"]["v104_upper_queue_density_threshold"]
                    for run in manifest["runs"]
                },
                {upper_density},
            )
            self.assertTrue(
                all(
                    run["metadata"]["v104_nonterminal_queue_density_floor"]
                    == (8.0 if role == "candidate" else None)
                    and run["metadata"]["v104_warm_admissibility"]
                    == ("preserve_anchor_warmness" if role == "candidate" else None)
                    and run["metadata"]["v104_density_band_initializer_guard"]
                    is (role == "candidate")
                    and run["metadata"]["v104_load_least_window_certificate_mode"]
                    == ("disabled" if role == "candidate" else "not_applicable")
                    and run["metadata"]["v104_component_safety_mode"]
                    == component_safety_mode
                    and run["metadata"]["v104_scalar_faasrank_noninferiority"]
                    is (role == "candidate")
                    and run["metadata"]["v104_input_locality_component_noninferiority"]
                    is (role == "candidate")
                    and run["metadata"]["v104_componentwise_faasrank_noninferiority"]
                    is (component_safety_mode.startswith("componentwise_"))
                    and run["metadata"][
                    ...
                    is False
                    and run["metadata"][
                        "v104_future_child_placement_or_feasibility_used"
                    ]
                    is False
                    and run["metadata"]["v104_substitution_cap"] is None
                    and run["metadata"]["v104_confirmation_seeds_opened"] is False
                    and run["metadata"]["v104_other_unopened_seeds_opened"] is False
    ...
    def test_plan_is_frozen_and_matches_exact_seed_boundary(self) -> None:
        ...
        self.assertEqual(
            plan["training_design"]["previous_confirmation_seeds_remaining_sealed"],
            PREVIOUS_CONFIRMATION_SEEDS,
        )
