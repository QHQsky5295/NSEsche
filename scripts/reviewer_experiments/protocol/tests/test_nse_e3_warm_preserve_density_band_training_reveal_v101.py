...
import scripts.reviewer_experiments.protocol.nse_e3_warm_preserve_density_band_training_reveal_v101 as reveal_module
from scripts.reviewer_experiments.protocol.nse_e3_warm_preserve_density_band_training_reveal_v101 import (
    ARMS,
    EXPECTED_SCENARIOS,
    EXPECTED_SEEDS,
...
class WarmPreserveDensityBandTrainingRevealV101Tests(unittest.TestCase):
    ...
    def test_joint_gate_uses_frozen_narrower_band_tie_break(self) -> None:
        ...
        diagnostic = result["arm_aggregates"]["v101-e3-band8-24-warm-preserve"][
            "E3.spike5x50ms"
        ]["formal_absolute_diagnostics"]["qpr_finite_only"]
        self.assertFalse(diagnostic["used_for_V101_selection"])

    def test_one_large_outlier_cannot_satisfy_two_of_three_rule(self) -> None:
        rows = rows_fixture()
        for row in rows:
            if (
                row["arm_id"] == "v101-e3-band8-24-warm-preserve"
                and row["scenario_id"] == "E3.spike5x50ms"
            ):
                value = 10.0 if row["seed"] == "E874" else 0.9
                row["qpr_finite_only"] = value
                row["qpr_zero_completed_as_zero"] = value
        result = evaluate_training_rows(rows, THRESHOLDS)
        gate = result["paired_candidate_results"]["v101-e3-band8-24-warm-preserve"][
            "E3.spike5x50ms"
