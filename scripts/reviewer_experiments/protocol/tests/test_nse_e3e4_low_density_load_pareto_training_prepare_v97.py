...
class LowDensityLoadParetoTrainingPreparationV97Tests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.temporary = tempfile.TemporaryDirectory()
        cls.root = Path(cls.temporary.name) / "v97"
        cls.receipt = prepare_v97(cls.root)
    ...
    def test_exact_arms_profiles_thresholds_and_runtime(self) -> None:
        expected = {
            arm_id: (experiment_id, role, profile, threshold, count)
            for arm_id, experiment_id, role, profile, threshold, count in ARMS
        }
        self.assertEqual(Counter(item[1] for item in ARMS), {"E3": 3, "E4": 3})
        self.assertEqual(sum(item[-1] for item in ARMS), 36)
        for arm_id, manifest in self.arms.items():
            experiment_id, role, profile, threshold, count = expected[arm_id]
            ...
            self.assertEqual(
                {
                    run["metadata"]["v97_queue_density_threshold"]
                    for run in manifest["runs"]
                },
                {threshold},
            )
            self.assertTrue(
                all(
                    run["metadata"]["v97_low_density_load_least_guard"]
                    is (threshold is not None)
                    and run["metadata"]["v97_confirmation_seeds_opened"] is False
                    and run["metadata"]["v97_previously_reserved_seeds_opened"] is False
    ...
    def test_arm_references_are_unique_and_tapes_are_paired(self) -> None:
        ...
        self.assertEqual(sum(map(len, references)), 36)
        self.assertEqual(len(set().union(*references)), 36)
        tape_sets = {
            arm_id: {
                key
                for run in manifest["runs"]
        ...
        e3_sets = [keys for arm, keys in tape_sets.items() if arm.startswith("v97-e3")]
        e4_sets = [keys for arm, keys in tape_sets.items() if arm.startswith("v97-e4")]
    ...
    def test_capture_and_receipt_close_the_information_boundary(self) -> None:
        ...
        self.assertEqual(self.receipt["arm_online_runs"], 36)
        self.assertEqual(self.receipt["arm_reference_builds"], 36)
    ...
    def test_plan_is_frozen_and_matches_exact_seed_boundary(self) -> None:
        ...
        self.assertEqual(
            plan["training_design"]["previously_reserved_seeds_that_remain_unopened"],
            PREVIOUSLY_RESERVED_SEEDS,
        )
        self.assertEqual(plan["training_design"]["candidate_online_runs"], 36)
        self.assertFalse(
