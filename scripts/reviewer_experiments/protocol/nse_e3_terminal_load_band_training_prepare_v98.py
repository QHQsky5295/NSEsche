...
ROOT = Path("tmp/nse_e3_terminal_load_band_training_20260828_v98")
PLAN = Path(
    "scripts/reviewer_experiments/protocol/"
    "nse_e3_terminal_load_band_training_plan_v98.json"
)
PLAN_SHA256 = "c9dc85faac1d7575859786523d90156c90e1201e243efd3660276f412f38e87b"
V97_RESULT_RECEIPT = Path(
    "scripts/reviewer_experiments/protocol/"
    "nse_e3e4_low_density_load_pareto_training_result_v97.json"
)
V97_RESULT_RECEIPT_SHA256 = (
    "11a9169770d0d2890236c54450fb5b44f2227ee085d3a9918713c9663c7ca809"
)
V97_SOURCE_RESULT = Path(
    "tmp/nse_e3e4_low_density_load_pareto_training_20260828_v97/training-result-v97.json"
)
V97_SOURCE_RESULT_SHA256 = (
    "7e50f26d2a69b01b12ee9b6d74df7c3d20fed3ec99757ef521616264fc019518"
)
FORMAL_RESULT = Path(
...
BINARY_PATH = Path("tmp/nse_v98_build_e3c38c0/release/serverless_sim.exe")
BINARY_SHA256 = "1d77d0626f2d7487caae614b413cb0152c3cd046674c898f4fb71e86b5cc2bc8"
BINARY_SOURCE_COMMIT = "e3c38c09ea70f7e7027cdbb93e8a43e99f18cc78"
...
SOURCE_INITIAL_SEEDS = [f"E{index}" for index in range(826, 836)]
SOURCE_CI_SEEDS = [f"E{index}" for index in range(836, 846)]
TRAINING_SEED_LIST = ["E826", "E827", "E828"]
TRAINING_SEEDS = set(TRAINING_SEED_LIST)
CONFIRMATION_SEEDS = [f"E{index}" for index in range(846, 866)]
...
E3_BAND4 = (
    ...
    "terminal_load_band4_guard64_dual_window_safe_pareto"
)
E3_BAND8 = (
    "faasrank_native_faithful_terminal_ocs_srpt_ready_"
    "terminal_load_band8_guard64_dual_window_safe_pareto"
)
ARMS = (
    ("v98-e3-anchor", "E3", "anchor", E3_ANCHOR, None, 9),
    ("v98-e3-terminal-load-band4", "E3", "candidate", E3_BAND4, 4.0, 9),
    ("v98-e3-terminal-load-band8", "E3", "candidate", E3_BAND8, 8.0, 9),
)
...
def _paths(root: Path) -> dict[str, Path]:
    return {
        "config": root / "v98-e3-training-config.json",
        "source": root / "manifest.v98-e826-e835-full.unbound.json",
        "capture": root / "manifest.v98-tape-capture.unbound.json",
...
def _assert_frozen_inputs() -> None:
    for path, expected in (
        (PLAN, PLAN_SHA256),
        (V97_RESULT_RECEIPT, V97_RESULT_RECEIPT_SHA256),
        (V97_SOURCE_RESULT, V97_SOURCE_RESULT_SHA256),
        ...
        if not path.is_file() or file_hash(path) != expected:
            raise RuntimeError(f"frozen V98 input is missing or changed: {path}")
...
def _rewrite(
    ...
    *,
    ...
    marker.update(
        {
            "purpose": purpose,
            "v98_training_plan_sha256": PLAN_SHA256,
            "v98_training_only": True,
            "v98_binary_source_commit": BINARY_SOURCE_COMMIT,
    ...
    if arm is not None:
        ...
        arm_id, experiment_id, role, profile, nonterminal_floor, expected = arm
        marker.update(
            {
                "v98_arm_id": arm_id,
                "v98_experiment_id": experiment_id,
                "v98_role": role,
                "v98_profile": profile,
                "v98_upper_queue_density_threshold": 64.0
                if role == "candidate"
                else None,
                "v98_nonterminal_queue_density_floor": nonterminal_floor,
                "v98_expected_run_count": expected,
            }
        )
        ...
        metadata.update(
            {
                "v98_training_plan_sha256": PLAN_SHA256,
                "v98_training_only": True,
                "v98_training_seed_metrics_previously_revealed": False,
                "v98_binary_source_commit": BINARY_SOURCE_COMMIT,
                "v98_confirmation_seeds_opened": False,
                "v98_previously_reserved_seeds_opened": False,
                "v98_formal_E01_E20_reexecution": False,
            }
        )
        if arm is not None:
            arm_id, experiment_id, role, profile, nonterminal_floor, _ = arm
            ...
            metadata.update(
                {
                    "v98_arm_id": arm_id,
                    "v98_arm_role": role,
                    "v98_candidate_profile": profile,
                    "v98_candidate_experiment": experiment_id,
                    "v98_upper_queue_density_threshold": 64.0
                    if role == "candidate"
                    else None,
                    "v98_nonterminal_queue_density_floor": nonterminal_floor,
                    "v98_faasrank_model_artifact_sha256": MODEL_SHA256,
                    "v98_native_faithful_initializer": True,
                    "v98_dual_window_safe_pareto": True,
                    "v98_terminal_load_band_guard": nonterminal_floor is not None,
                    "v98_terminal_players_included_without_lower_floor": role
                    == "candidate",
                    "v98_srpt_critical_path_player_order": "srpt_ready" in profile,
                }
            )
...
def prepare_v98(root: Path = ROOT) -> dict:
    _assert_frozen_inputs()
    if root.exists():
        raise RuntimeError(f"refusing to overwrite V98 training root: {root}")
    ...
    capture = derive_integration_smoke_shard(
        paths["source"],
        capture_ids,
        purpose=f"V98 fresh balanced-QoS parent tapes; plan_sha256={PLAN_SHA256}",
    )
    _write(
        paths["capture"],
        _rewrite(
    ...
    for arm in ARMS:
        ...
        template = derive_integration_smoke_shard(
            paths["source"],
            selected,
            purpose=f"V98 {arm_id} terminal-load-band training; plan_sha256={PLAN_SHA256}",
        )
        manifest = _rewrite(
            template,
            purpose=f"V98 {arm_id} terminal-load-band training; plan_sha256={PLAN_SHA256}",
        ...
        if (
            ...
            raise RuntimeError(f"V98 {arm_id} must have {expected} runs/references")
        if {run["seed"] for run in manifest["runs"]} != TRAINING_SEEDS:
            raise RuntimeError(f"V98 {arm_id} escaped E826-E828")
        keys = {item["key"] for item in manifest["reference_build_dependencies"]}
        if all_reference_keys & keys:
            raise RuntimeError("V98 arm-specific reference keys unexpectedly overlap")
        all_reference_keys.update(keys)
        _write(arm_path(root, arm_id), manifest)
    if len(all_reference_keys) != 27:
        raise RuntimeError("V98 must declare exactly 27 arm-specific references")
    ...
    receipt = {
        "schema_version": "NSE_E3_TERMINAL_LOAD_BAND_PREPARED_V98_V1",
        "created_at": utc_now(),
        "formal_results_eligible": False,
        ...
        "V97_result_receipt_path": str(V97_RESULT_RECEIPT),
        "V97_result_receipt_sha256": V97_RESULT_RECEIPT_SHA256,
        "V97_source_result_path": str(V97_SOURCE_RESULT),
        "V97_source_result_sha256": V97_SOURCE_RESULT_SHA256,
        ...
        "arms": [
            {
                ...
                "nonterminal_queue_density_floor": nonterminal_floor,
                "run_count": expected,
                "reference_build_count": expected,
            }
            for arm_id, experiment_id, role, profile, nonterminal_floor, expected in ARMS
        ],
