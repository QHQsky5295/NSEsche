...
OUTPUT = ROOT / "training-result-v105.json"
BLIND_AUDIT_FILE_SHA256 = (
    "c58a5271e30b1def4d93d9f94e06a5fd3b20c4d48e78df88be6adcb1c442ac95"
)
BLIND_AUDIT_HASH = "dbff8a1d2c4ee1502ea27f590c68e195985d311054d14e445db56dbc5f315ae1"
EXPECTED_SEEDS = ("E884", "E885", "E886")
EXPECTED_SCENARIOS = (
    "E3.spike5x50ms",
    "E3.sustained3x200ms",
    "E3.pulse4x4x50ms",
...
ANCHORS = {"E3": "v105-e3-anchor"}
COMPONENT_SAFETY_PREFERENCE = {
    "v105-e3-band8-24-componentwise-bidirectional-locality-noncritical-frontier-pareto-initializer-only": 0,
    "v105-e3-band8-24-bidirectional-locality-noncritical-frontier-pareto-initializer-only": 1,
}
...
def evaluate_training_rows(
    ...
    ranked = sorted(
        ...
        key=lambda item: (
            ...
            COMPONENT_SAFETY_PREFERENCE[item[0]],
            item[0],
        ),
    )
...
def _validate_blind_audit() -> dict[str, Any]:
    if not BLIND_AUDIT.is_file() or file_hash(BLIND_AUDIT) != BLIND_AUDIT_FILE_SHA256:
        raise RuntimeError("V105 blind audit is missing or changed")
    ...
    if (
        ...
        raise RuntimeError("V105 blind audit does not authorize reveal")
    return blind
...
def _load_rows(blind: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    if len(admitted) != 27:
        raise RuntimeError("V105 blind audit run evidence is incomplete")
    ...
    for arm_id, expected in ARMS.items():
        ...
        if not manifest_path.is_file() or file_hash(manifest_path) != frozen.get(
            ...
            raise RuntimeError(f"V105 frozen manifest changed: {arm_id}")
        manifest = load_and_validate_manifest(manifest_path)
        if manifest.get("manifest_hash") != frozen.get("manifest_hash"):
            raise RuntimeError(f"V105 manifest hash changed: {arm_id}")
        for run in manifest["runs"]:
            ...
            if (
                ...
                != expected["profile"]
                or metadata.get("v105_training_plan_sha256") != PLAN_SHA256
                or metadata.get("v105_training_only") is not True
                or metadata.get("v105_training_seed_metrics_previously_revealed")
                is not False
                or metadata.get("v105_confirmation_seeds_opened") is not False
                or metadata.get("v105_other_unopened_seeds_opened") is not False
                or metadata.get("v105_arm_id") != arm_id
                or metadata.get("v105_arm_role") != expected["role"]
                or metadata.get("v105_upper_queue_density_threshold")
                != expected["upper_queue_density_threshold"]
                or metadata.get("v105_nonterminal_queue_density_floor")
                != expected["nonterminal_queue_density_floor"]
                or metadata.get("v105_warm_admissibility")
                != expected["warm_admissibility"]
                or metadata.get("v105_load_least_window_certificate_mode")
                != expected["load_least_window_certificate_mode"]
                or metadata.get("v105_component_safety_mode")
                ...
                is not False
                or metadata.get("v105_future_child_placement_or_feasibility_used")
                is not False
                or metadata.get("v105_critical_frontier_protection")
                is not (expected["role"] == "candidate")
                or metadata.get("v105_critical_frontier_rank_source")
                != (
                    "immutable_srpt_remaining_critical_path_rank"
                    if expected["role"] == "candidate"
                    else "not_applicable"
                )
                or metadata.get("v105_critical_frontier_tie_rule")
                != (
                ...
                is not (expected["role"] == "candidate")
                or metadata.get("v105_outcome_fields_drive_policy") is not False
            ):
                raise RuntimeError(f"V105 arm identity changed: {run['run_id']}")
            run_id = run["run_id"]
            summary_path = (
                ROOT
                / "runs"
            ...
            if not summary_path.is_file() or file_hash(summary_path) != admitted.get(
                ...
                raise RuntimeError(f"V105 summary differs from blind audit: {run_id}")
            ...
            if (
                ...
                raise RuntimeError(
                    f"V105 summary identity/completion mismatch: {run_id}"
                )
            rows.append(
                {
                    "arm_id": arm_id,
                    ...
                    "component_safety_mode": expected["component_safety_mode"],
...
def execute_reveal() -> dict[str, Any]:
    if OUTPUT.exists():
        raise RuntimeError(f"V105 training result already exists: {OUTPUT}")
    if not PLAN.is_file() or file_hash(PLAN) != PLAN_SHA256:
        raise RuntimeError("frozen V105 plan changed before reveal")
    blind = _validate_blind_audit()
    rows = _load_rows(blind)
    if len(rows) != 27:
        raise RuntimeError("V105 revealed row count differs from sealed plan")
    ...
    result = {
        "schema_version": "NSE_E3_CRITICAL_FRONTIER_SAFETY_RESULT_V105_V1",
        "created_at": utc_now(),
        ...
        "selection_rule": {
            ...
            "multiple_passers": (
                ...
                "throughput change, then prefer componentwise noncritical-frontier "
                "safety over scalar noncritical-frontier safety, then arm_id"
            ),
            "frozen_E4": (
                "retain V97 v97-e4-load-guard64 without V105 result-dependent reselection"
            ),
        ...
        **evaluation,
        "revealed_rows": rows,
        "decision": {
            "authorize_preregistration_of_E986_E1005_confirmation": joint_pass,
            ...
            "next_action": (
                ...
                "separately preregister the complete E986-E1005 NSESche confirmation "
                "before generating inputs"
                if joint_pass
                else (
                    "freeze the complete V105 training cohort, keep E926-E1005 and "
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Reveal the frozen V105 training cohort exactly once after blind audit."
    )
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    if args.command != "reveal" or not args.execute:
        parser.error("V105 reveal requires the explicit command: reveal --execute")
    execute_reveal()
