...
from scripts.reviewer_experiments.protocol.nse_e3_causal_raw_persistence_router_training_execute_v145 import (
    EXECUTION_RECEIPT,
    READY_SCHEDULE,
)
from scripts.reviewer_experiments.protocol.nse_e3_causal_raw_persistence_router_training_prepare_v145 import (
    ...
    V142_BASELINE_READY,
    V145_SERVICE_STATE_DOMAIN,
    V145_WELFARE_STATE_DOMAIN,
    pairing_path,
    paths,
    ready_manifest_path,
...
PREPARED = ROOT / "prepared-manifest-v145.json"
OUTPUT = ROOT / "joint-blind-audit-v145-training.json"
RESULT = ROOT / "training-result-v145.json"
EXPECTED_RUNTIME = {
...
def _persistence_comparison(recent: int, baseline: int) -> dict[str, Any]:
    if (
        ...
        raise ValueError("V145 persistence counts must be nonnegative integers")
    recent_scaled = (
...
def _validate_guard(
    ...
    _require(isinstance(reason, str), "invalid V145 guard reason")
    ...
    _require(
        ...
        f"V145 native service boundary changed: {run_id}:{line_number}",
    )
    if players == 0:
        _require(
            selected is None
            ...
            f"V145 empty guard changed: {run_id}:{line_number}",
        )
        return reason, False
    ...
    _require(
        ...
        f"V145 selected initializer changed: {run_id}:{line_number}",
    )
    ...
    if proposal_complete:
        _require(
            ...
            f"V145 proposal service changed: {run_id}:{line_number}",
        )
        ...
        _require(
            ...
            f"V145 proposal deltas changed: {run_id}:{line_number}",
        )
    else:
        _require(
            proposal_players == 0
            ...
            f"V145 unavailable proposal changed: {run_id}:{line_number}",
        )
    ...
    _require(
        ...
        == float(welfare_delta),
        f"V145 welfare certificate changed: {run_id}:{line_number}",
    )
    ...
    _require(
        ...
        f"V145 guard disposition changed: {run_id}:{line_number}",
    )
    return reason, accepted


def _validate_v145_native_diagnostics(
    run: dict[str, Any], canonical: Path
) -> dict[str, Any]:
    path = canonical / "reviewer_records" / run["run_id"] / "nash_metrics.jsonl.gz"
    _require(path.is_file(), f"missing V145 Nash diagnostics: {run['run_id']}")
    counts = Counter()
    reasons = Counter()
        ...
        for line_number, line in enumerate(handle, start=1):
            ...
            if event.get("kind") == "run_config":
                ...
                _require(
                    ...
                    f"V145 run-config changed: {run['run_id']}:{line_number}",
                )
                continue
            ...
            _require(isinstance(decision, dict), "missing V145 decision diagnostics")
            ...
            _require(
                ...
                f"missing V145 persistence diagnostics: {run['run_id']}:{line_number}",
            )
            raw_current = morphology.get("raw_first_seen_current_frame")
            _require(
                ...
                and morphology.get("checked_arithmetic_valid") is True
                ...
                is False,
                f"V145 causal input boundary changed: {run['run_id']}:{line_number}",
            )
            ...
            _require(
                ...
                and morphology.get("rolling_shock_active") is shock_active,
                f"V145 detector changed: {run['run_id']}:{line_number}",
            )
            ...
            if started:
                ...
                if expected_episode_count == 1:
                    ...
                    _require(
                        ...
                        f"V145 first episode lacks preceding baseline: {run['run_id']}:{line_number}",
                    )
                    ...
                    _require(
                        ...
                        f"V145 first episode baseline is ineligible: {run['run_id']}:{line_number}",
                    )
            expected_first_age = (
            ...
            if (
                ...
                _require(
                    ...
                    f"V145 persistence baseline disappeared: {run['run_id']}:{line_number}",
                )
            ...
            if persistence_evaluated:
                _require(
                    ...
                    f"V145 evaluated persistence state is incomplete: {run['run_id']}:{line_number}",
                )
            ...
            _require(
                ...
                and morphology.get("first_episode_age_frames") == expected_first_age
                and morphology.get("baseline_frames") == PERSISTENCE_BASELINE_FRAMES
                and morphology.get("baseline_80_count") == baseline_80_count
                and morphology.get("recent_frames") == PERSISTENCE_RECENT_FRAMES
                and morphology.get("recent_5_count") == recent_5_count
                and morphology.get("evaluation_age_frames")
                == PERSISTENCE_EVALUATION_AGE_FRAMES
                and morphology.get("threshold_numerator")
                == PERSISTENCE_THRESHOLD_NUMERATOR
                and morphology.get("threshold_denominator")
                == PERSISTENCE_THRESHOLD_DENOMINATOR
                and morphology.get("persistence_evaluated_once")
                is persistence_evaluated
                and morphology.get("persistence_evaluation_count")
                == int(persistence_evaluated)
                and morphology.get("persistence_comparison") == expected_comparison
                ...
                f"V145 state transition changed: {run['run_id']}:{line_number}",
            )
            ...
            _require(
                ...
                f"V145 player count changed: {run['run_id']}:{line_number}",
            )
            ...
            if players == 0:
                _require(
                    ...
                    f"V145 empty portfolio changed: {run['run_id']}:{line_number}",
                )
            else:
                active_player_windows += 1
                _require(
                    portfolio.get("enabled") is True
                    and portfolio.get("rule") == "causal_raw_persistence"
                    ...
                    f"V145 portfolio selection changed: {run['run_id']}:{line_number}",
                )
                for candidate in candidates:
                    _require(
                        ...
                        f"V145 candidate invalid: {run['run_id']}:{line_number}",
                    )
            ...
            if selected_candidate is None:
                _require(
                    ...
                    is int,
                    f"V145 empty command hash changed: {run['run_id']}:{line_number}",
                )
                _require(
                    morphology.get("selected_native_assignment_hash") is None
                    or type(morphology.get("selected_native_assignment_hash")) is int,
                    f"V145 empty assignment hash changed: {run['run_id']}:{line_number}",
                )
                _require(
                    ...
                    is False,
                    f"V145 empty dispatch changed: {run['run_id']}:{line_number}",
                )
            else:
                _require(
                    ...
                    f"V145 selected hashes changed: {run['run_id']}:{line_number}",
                )
            ...
            if players > 0:
                _require(
                    ...
                    f"V145 final dispatch changed: {run['run_id']}:{line_number}",
                )
            if accepted:
                counts["accepted_windows"] += 1

    _require(
        counts["run_config"] == 1, f"V145 run-config count changed: {run['run_id']}"
    )
    _require(counts["windows"] == 4000, f"V145 window count changed: {run['run_id']}")
    _require(active_player_windows > 0, f"V145 has no player windows: {run['run_id']}")
    return {
...
def _verify_references(candidate_manifest: Mapping[str, Any]) -> dict[str, Any]:
    ...
    _require(path.is_file(), "missing V145 reference catalog")
    catalog = read_json(path)
    catalog_hash = _assert_hashed_object(catalog, "catalog_hash", "V145 references")
    ...
    _require(
        ...
        "V145 reference product changed",
    )
    stage_root = _stage_root_from_receipts(
        entries, "receipt_path", 9, "V145 references"
    )
    ...
    _require(
        ...
        f"V145 reference ledger changed: {counts}",
    )
    bound = {
    ...
    for key, entry in sorted(entries.items()):
        ...
        _require(
            ...
            f"V145 reference evidence changed: {key}",
        )
        evidence.append(
            {
                "key": key,
...
def _validate_execution_receipt() -> dict[str, Any]:
    ...
    _require(schedule_path.is_file(), "missing V145 ready schedule")
    _require(receipt_path.is_file(), "missing V145 execution receipt")
    schedule = read_json(schedule_path)
    schedule_hash = _assert_hashed_object(
        schedule, "schedule_hash", "V145 ready schedule"
    )
    ...
    _require(
        ...
        "V145 ready schedule binding changed",
    )
    receipt = read_json(receipt_path)
    receipt_hash = _assert_hashed_object(
        receipt, "receipt_hash", "V145 execution receipt"
    )
    _require(
        ...
        "V145 execution boundary changed",
    )
    for scheduled, dispatched in zip(schedule["schedule"], receipt["dispatches"]):
        _require(
            ...
            f"V145 frozen dispatch changed: {scheduled['ordinal']}",
        )
    return {
...
def run_blind_audit(output: Path = OUTPUT) -> dict[str, Any]:
    _require(not output.exists(), f"V145 blind audit already exists: {output}")
    _require(not RESULT.exists(), "V145 reveal exists before blind audit")
    _require(PLAN.is_file() and file_hash(PLAN) == PLAN_SHA256, "V145 plan changed")
    _require(PREPARED.is_file(), "missing V145 prepared receipt")
    prepared = read_json(PREPARED)
    prepared_hash = _assert_hashed_object(
        prepared, "receipt_hash", "V145 prepared receipt"
    )
    ...
    _require(
        ...
        "V145 prepared boundary changed",
    )
    _require(
        ...
        "V145 prepared unbound manifest changed",
    )
    ...
    _require(
        ...
        "V145 ready manifest boundary changed",
    )
    _require(
        ...
        "V145 ready schedule no longer matches the manifest",
    )
    ...
    _require(actual_ids == expected_ids, "V145 canonical set changed")
    _require(
        not list((workspace / "quarantine").glob("**/attempt-*")),
        "V145 online quarantine is nonempty",
    )
    ...
    _require(
        ...
        f"V145 online ledger changed: {ledger_counts}",
    )
    ...
    for run in candidate_manifest["runs"]:
        ...
        _require(
            ...
            == PROFILE
            and metadata.get("v145_native_selection_rule") == SELECTION_RULE
            and metadata.get("v145_native_portfolio_members") == NATIVE_MEMBERS
            and metadata.get("v145_all_three_experts_advanced_every_window") is True
            and metadata.get("v145_shock_detector")
            == "first_seen_arrivals_80_frame_baseline_20_frame_recent_3_over_2_threshold_50_frame_latch_for_episode_onset_only"
            and metadata.get("v145_persistence_test")
            ...
            and metadata.get("v145_quiet_route") == "greedy"
            and metadata.get("v145_first_short_route") == "greedy"
            and metadata.get("v145_first_sustained_route") == "faasrank"
            and metadata.get("v145_recurrent_route") == "load_least"
            and metadata.get("v145_service_certificate_scope")
            == SERVICE_CERTIFICATE_SCOPE
            and metadata.get("v145_service_certificate_state_domain")
            == V145_SERVICE_STATE_DOMAIN
            and metadata.get("v145_paper_welfare_state_domain")
            == V145_WELFARE_STATE_DOMAIN
            and metadata.get("v145_outcome_fields_drive_policy") is False
            and metadata.get("v145_scenario_or_burst_label_used_by_policy") is False
            and metadata.get("v145_future_arrivals_used_by_policy") is False
            and metadata.get("v145_confirmation_inputs_opened") is False
            and isinstance(run.get("reference_dependency"), dict),
            f"V145 candidate manifest boundary changed: {run['run_id']}",
        )
        ...
        _require(
            ...
            f"V145 canonical status changed: {run['run_id']}",
        )
        runtime = _runtime_evidence(audit)
        for field, value in runtime.items():
            runtime_values[field].add(value)
        diagnostics = _validate_v145_native_diagnostics(run, canonical)
        paired = {
            "method_label": ARM_ID,
            "run_id": run["run_id"],
    ...
    for field, expected in EXPECTED_RUNTIME.items():
        _require(
            ...
            f"V145 runtime {field} changed: {runtime_values[field]}",
        )
    ...
    _require(
        ...
        f"V145 runtime git identity changed: {git_commits}",
    )
    _require(len(paired_inputs) == 9, "V145 paired block count changed")
    for (scenario, seed), rows in paired_inputs.items():
        _require(
            len(rows) == 10
            and {row["method_label"] for row in rows} == {*BASELINE_METHODS, ARM_ID},
            f"V145 paired product changed: {scenario}/{seed}",
        )
        for field in (
            ...
            _require(
                ...
                f"V145 paired {field} changed: {scenario}/{seed}",
            )
        _require(
            rows[0]["common_hpa_hash"] == EXPECTED_COMMON_HPA_SHA256,
            f"V145 common HPA changed: {scenario}/{seed}",
        )

    payload = {
        "schema_version": "NSE_E3_CAUSAL_RAW_PERSISTENCE_BLIND_AUDIT_V145_V1",
        "created_at": utc_now(),
        "status": "pass",
