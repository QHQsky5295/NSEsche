...
from scripts.reviewer_experiments.protocol.nse_e3_initializer_component_safety_training_prepare_v103 import (
...
def _verify_tape_catalog() -> tuple[list[dict[str, Any]], dict[str, Any]]:
    _require(TAPES.is_file(), f"missing V103 tape catalog: {TAPES}")
    catalog = read_json(TAPES)
    catalog_hash = _assert_hashed_object(catalog, "catalog_hash", "V103 tapes")
    entries = catalog.get("entries")
    _require(
        isinstance(entries, dict) and len(entries) == 12,
        "V103 tape count changed",
    )
    evidence = []
...
def _verify_reference_stage_ledgers() -> tuple[dict[str, Path], dict[Path, str]]:
    ...
    for root, expected_count in expected_by_root.items():
        ...
        _assert_ledger_contract(
            ...
            f"V103 reference stage {root}",
        )
        _require(
            not list((root / "quarantine").glob("**/attempt-*")),
            f"V103 reference quarantine is nonempty: {root}",
        )
...
def run_blind_audit(output: Path = OUTPUT) -> dict[str, Any]:
    ...
    _require(PLAN.is_file() and file_hash(PLAN) == PLAN_SHA256, "V103 plan changed")
    plan = read_json(PLAN)
    _require(plan.get("formal_results_eligible") is False, "V103 eligibility changed")
    _require(PREPARED.is_file(), f"missing V103 prepared receipt: {PREPARED}")
    prepared = read_json(PREPARED)
    prepared_hash = _assert_hashed_object(
        prepared, "receipt_hash", "V103 prepared receipt"
    )
    _require(
        ...
        "V103 prepared scientific boundary changed",
    )
    ...
    for arm_id, expected in ARMS.items():
        ...
        _require(paths["ready"].is_file(), f"missing V103 ready manifest: {arm_id}")
        manifest = load_and_validate_manifest(paths["ready"])
        _require(
            len(manifest["runs"]) == expected["run_count"],
            f"V103 run count changed: {arm_id}",
        )
        _require(
        ...
        reference_catalog_hash = _assert_hashed_object(
            references, "catalog_hash", f"V103 references {arm_id}"
        )
        ...
        _require(
            ...
            f"V103 reference key set changed: {arm_id}",
        )
        for key, entry in sorted(entries.items()):
        ...
        _require(actual_ids == expected_ids, f"V103 canonical set changed: {arm_id}")
        _require(
            not list((workspace / "quarantine").glob("**/attempt-*")),
            f"V103 online quarantine is nonempty: {arm_id}",
        )
        ...
        for run in manifest["runs"]:
            ...
            _require(
                ...
                and metadata.get("v103_arm_id") == arm_id
                and metadata.get("v103_arm_role") == expected["role"]
                and metadata.get("v103_candidate_profile") == expected["profile"]
                and metadata.get("v103_upper_queue_density_threshold")
                == expected["upper_queue_density_threshold"]
                and metadata.get("v103_nonterminal_queue_density_floor")
                == expected["nonterminal_queue_density_floor"]
                and metadata.get("v103_warm_admissibility")
                == expected["warm_admissibility"]
                and metadata.get("v103_load_least_window_certificate_mode")
                == expected["load_least_window_certificate_mode"]
                and metadata.get("v103_component_safety_mode")
                == expected["component_safety_mode"]
                and metadata.get("v103_scalar_faasrank_noninferiority")
                is (expected["role"] == "candidate")
                and metadata.get("v103_input_locality_component_noninferiority")
                is (expected["role"] == "candidate")
                and metadata.get("v103_componentwise_faasrank_noninferiority")
                is (
                    expected["component_safety_mode"]
                    == "componentwise_cpu_memory_locality_warm_diversity_noninferiority"
                )
                and metadata.get("v103_training_seed_metrics_previously_revealed")
                is False
                and metadata.get("v103_confirmation_seeds_opened") is False
                and metadata.get("v103_other_unopened_seeds_opened") is False,
                f"V103 run boundary changed: {run['run_id']}",
            )
            ...
            _require(
                ...
                f"V103 canonical status changed: {run['run_id']}",
            )
            runtime = {
    ...
    for field, expected in EXPECTED_RUNTIME.items():
        _require(
            ...
            f"V103 runtime {field} changed: {runtime_values[field]}",
        )
    ...
    _require(
        ...
        f"V103 runtime git identity is not singular: {git_commits}",
    )
    for (experiment_id, scenario, seed), rows in paired_inputs.items():
        _require(
            len(rows) == 3,
            f"V103 paired arm count changed: {experiment_id}/{scenario}/{seed}",
        )
        for field in (
            ...
            _require(
                ...
                f"V103 paired {field} changed: {scenario}/{seed}",
            )
        _require(
            rows[0]["common_hpa_hash"] == EXPECTED_COMMON_HPA_SHA256,
            f"V103 common HPA changed: {scenario}/{seed}",
        )

    output_payload = {
        "schema_version": "NSE_E3_INITIALIZER_COMPONENT_SAFETY_BLIND_AUDIT_V103_V1",
        "created_at": utc_now(),
        "status": "pass",
