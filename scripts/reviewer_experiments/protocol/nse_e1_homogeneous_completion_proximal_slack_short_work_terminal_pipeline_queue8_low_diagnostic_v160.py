...
PLAN = Path(
    ...
    "nse_e1_homogeneous_completion_proximal_slack_short_work_terminal_pipeline_queue8_low_diagnostic_plan_v160.json"
)
PLAN_SHA256 = "833128599546122e34c7ff8120f8c76a0af269b989d7528ccc807ee4f32d5448"
IMPLEMENTATION = Path(
    "scripts/reviewer_experiments/protocol/"
    "nse_e1_homogeneous_completion_proximal_slack_short_work_terminal_pipeline_queue8_low_diagnostic_implementation_v160.json"
)
IMPLEMENTATION_SHA256 = (
    "6732173ffd019ad9f94a043c231623e9c20a5c02b005e7e060d10aff25fbe3ce"
)
V159_RESULT = Path(
    "tmp/nse_e1_homogeneous_slack_short_work_terminal_pipeline_queue8_low_diagnostic_20260831_v159/"
    "diagnostic-result-v159.json"
)
V159_RESULT_SHA256 = "8b4630667eb0682d0340c43a62acfa2ff190d7958634917c674f15f122ba9801"
V159_RESULT_HASH = "e0a1463951767d682f59aa2e83415d00aa2769a2e348dd061298a34df24af5c2"

ARM_ID = (
    "v160-low-srpt-slack-completion-proximal-short5p5-"
    "terminal-pipeline-hiku2-ocs-queue8"
)
PROFILE = "srpt_slack_completion_proximal_short5p5_terminal_pipeline_hiku2_ocs_queue8"
FRONTIER = (
    "parents_completed_or_terminal_or_slack_completion_proximal_"
    "short_work_parents_scheduled"
)
SINGLE_CHANGE = (
    "V159_slack_short_work_pipeline_plus_immutable_completion_proximal_"
    "nonterminal_topology_gate"
)
TERMINAL_DEFINITION = (
    "admit_all_parents-completed_and_terminal_parents-scheduled_players_plus_"
    "only_completion-proximal_nonterminal_parents-scheduled_players_with_"
    "request_remaining_work_at_most_5p5_when_current_queue_density_is_"
    "strictly_below_8"
)
COMPLETION_PROXIMAL_DEFINITION = (
    "nonterminal_function_whose_immutable_DAG_children_are_all_terminal_functions"
)
...
PORT = "3211"
BINARY_SOURCE_COMMIT = "67926e684cd383819cf9f5338495812a7d51f1ac"
BINARY_PATH = Path("serverless_sim/target_e1_v160/release/serverless_sim.exe")
BINARY_SHA256 = "9f298bc7f6413a6761ed5daf8963aed9c84e77ce38230d35c9fbda11c7284cd1"


def paths(root: Path = ROOT) -> dict[str, Path]:
    return {
        "manifest": root / f"manifest.{ARM_ID}.unbound.json",
        "prepared": root / "prepared-v160.json",
        "schedule": root / "frozen-run-order-v160.json",
        ...
        "execution": root / "execution-receipt-v160.json",
        "pairing": root / "pairing-audit-v160.json",
        "blind": root / "joint-blind-audit-v160.json",
        "result": root / "diagnostic-result-v160.json",
    }
...
def _assert_frozen_inputs() -> dict[str, Any]:
    ...
    for path, sha256, label in (
        (PLAN, PLAN_SHA256, "V160 plan"),
        (IMPLEMENTATION, IMPLEMENTATION_SHA256, "V160 implementation receipt"),
        (V159_RESULT, V159_RESULT_SHA256, "V159 result"),
        (BINARY_PATH, BINARY_SHA256, "V160 release binary"),
    ...
    if not (
        ...
        and change.get("completion_proximal_definition")
        == COMPLETION_PROXIMAL_DEFINITION
        and change.get("deeper_nonterminal_short_work_rejected_until_ready") is True
        and change.get("changes_paper_formula_welfare_pricing_hpa_metrics_or_reference")
        is False
    ):
        raise RuntimeError("V160 implementation boundary changed")
    result = read_json(V159_RESULT)
    if not (
        _assert_hashed(result, "result_hash", "V159 result") == V159_RESULT_HASH
        ...
        raise RuntimeError("V159 result boundary changed")
    _assert_json_semantic(
...
def _rewrite_candidate(
    ...
    marker.update(
        {
            "purpose": (
                "V160 outcome-disclosed three-seed completion-proximal "
                "slack short-work diagnostic; "
                "never a formal result or paper superiority claim"
            ),
            "v160_role": (
                "result_blind_completion_proximal_slack_short_work_falsification"
            ),
            "v160_plan_sha256": PLAN_SHA256,
            "v160_implementation_sha256": IMPLEMENTATION_SHA256,
            "v160_binary_source_commit": BINARY_SOURCE_COMMIT,
            "v160_protocol_source_commit": protocol_source_commit,
            "v160_binary_sha256": BINARY_SHA256,
            "v160_arm_id": ARM_ID,
            "v160_profile": PROFILE,
            "v160_player_frontier": FRONTIER,
            "v160_single_change_from_v159": SINGLE_CHANGE,
            "v160_short_work_threshold": SHORT_WORK_THRESHOLD,
            "v160_queue_density_threshold": QUEUE_THRESHOLD,
            "v160_queue_boundary": "below_is_strict",
            "v160_completion_proximal_definition": COMPLETION_PROXIMAL_DEFINITION,
            "v160_deeper_nonterminal_short_work_rejected_until_ready": True,
            "v160_environment": COMMON_ENVIRONMENT,
            "v160_expected_run_count": 3,
            "v160_expected_reference_build_count": 3,
            "v160_fixed_order": list(SEEDS),
            "v160_candidate_performance_summaries_parsed": 0,
            "v160_remaining_seventeen_authorized": False,
            "v160_confirmation_inputs_generated": False,
        }
    )
    for run in rewritten["runs"]:
        ...
        run["metadata"] = {
            ...
            "v160_completion_proximal_definition": COMPLETION_PROXIMAL_DEFINITION,
            "v160_deeper_nonterminal_short_work_rejected_until_ready": True,
            "v160_source_e1_run_id": source_run_id,
            "v160_source_e1_run_spec_hash": source_run_spec_hash,
            "v160_candidate_performance_summaries_parsed_before_run": 0,
            "v160_remaining_seventeen_authorized": False,
            "v160_confirmation_inputs_generated": False,
        }
...
def _validate_product(manifest: Mapping[str, Any], *, references_bound: bool) -> None:
    if not (
        ...
        raise RuntimeError("V160 exact E09/E18/E20 product changed")
    ...
    for run in manifest["runs"]:
        ...
        if not (
            ...
            and metadata.get("v160_profile") == PROFILE
            and metadata.get("v160_player_frontier") == FRONTIER
            and metadata.get("v160_short_work_threshold") == SHORT_WORK_THRESHOLD
            and metadata.get("v160_queue_density_threshold") == QUEUE_THRESHOLD
            and metadata.get("v160_queue_boundary") == "below_is_strict"
            and metadata.get("v160_completion_proximal_definition")
            == COMPLETION_PROXIMAL_DEFINITION
            and metadata.get("v160_deeper_nonterminal_short_work_rejected_until_ready")
            is True
        ):
            raise RuntimeError(f"V160 run contract changed: {run.get('run_id')}")


def prepare_v160(root: Path = ROOT) -> dict[str, Any]:
    source = _assert_frozen_inputs()
    if root.exists():
        raise RuntimeError(f"refusing to overwrite V160 root: {root}")
    ...
    schedule = {
        "schema_version": "NSE_E1_HOMOGENEOUS_COMPLETION_PROXIMAL_SLACK_SHORT_WORK_TERMINAL_PIPELINE_QUEUE8_LOW_SCHEDULE_V160_V1",
        "created_at": utc_now(),
    ...
    receipt = {
        "schema_version": "NSE_E1_HOMOGENEOUS_COMPLETION_PROXIMAL_SLACK_SHORT_WORK_TERMINAL_PIPELINE_QUEUE8_LOW_PREPARED_V160_V1",
        "created_at": utc_now(),
        ...
        "v159_result_file_sha256": V159_RESULT_SHA256,
        "v159_result_hash": V159_RESULT_HASH,
        ...
        "completion_proximal_definition": COMPLETION_PROXIMAL_DEFINITION,
        "deeper_nonterminal_short_work_rejected_until_ready": True,
...
def execute_v160(root: Path = ROOT) -> dict[str, Any]:
    output = paths(root)
    if output["execution"].exists():
        raise RuntimeError("V160 execution receipt already exists")
    prepared = read_json(output["prepared"])
    _assert_hashed(prepared, "receipt_hash", "V160 prepared receipt")
    ...
    for ordinal, seed in enumerate(SEEDS, start=1):
        ...
        if completed.returncode != 0:
            raise RuntimeError(f"V160 dispatch {seed} failed: {completed.returncode}")
        ...
        if not (
            ...
            raise RuntimeError(f"V160 canonical is not a QC pass: {run['run_id']}")
        dispatches.append(
            {
                "ordinal": ordinal,
                "seed": seed,
    ...
    receipt = {
        "schema_version": "NSE_E1_HOMOGENEOUS_COMPLETION_PROXIMAL_SLACK_SHORT_WORK_TERMINAL_PIPELINE_QUEUE8_LOW_EXECUTION_V160_V1",
        "created_at": utc_now(),
        "candidate_performance_summaries_parsed": 0,
        "plan_sha256": PLAN_SHA256,
...
def _count(value: Any, label: str) -> int:
    if isinstance(value, bool) or not isinstance(value, int) or value < 0:
        raise RuntimeError(f"V160 {label} is invalid")
    return value
...
def _finite_optional(value: Any, label: str) -> float | None:
    ...
    if not math.isfinite(result):
        raise RuntimeError(f"V160 {label} is nonfinite")
    return result
...
def _audit_nash_log(canonical: Path, run: Mapping[str, Any]) -> dict[str, Any]:
    ...
    terminal = short = completion_proximal = deeper_rejected = 0
    ...
    with gzip.open(log, "rt", encoding="utf-8") as stream:
        for line in stream:
            ...
            if kind not in counts:
                raise RuntimeError(f"unexpected V160 Nash observation kind: {kind}")
            counts[kind] += 1
            if kind == "run_config":
                ...
                if not (
                    ...
                    and contract.get("completion_proximal_short_work_required") is True
                    and contract.get("completion_proximal_definition")
                    == COMPLETION_PROXIMAL_DEFINITION
                    and contract.get("uses_completed_request_outcomes") is False
                    and contract.get("reference_policy_independent") is True
                ):
                    raise RuntimeError("V160 run_config contract changed")
            elif kind == "window":
                if event.get("frame") != counts["window"] - 1:
                    raise RuntimeError("V160 scheduler window sequence changed")
                ...
                if (
                    ...
                    raise RuntimeError("V160 decision frontier/hash changed")
                ...
                topology_gate = frontier.get("completion_proximal_short_work_gate", {})
                completion_proximal_now = _count(
                    topology_gate.get(
                        "admitted_completion_proximal_short_work_players"
                    ),
                    "completion-proximal admission count",
                )
                deeper_rejected_now = _count(
                    topology_gate.get("rejected_deeper_nonterminal_short_work_players"),
                    "deeper nonterminal rejection count",
                )
                if not (
                    ...
                    and topology_gate.get("enabled") is True
                    and topology_gate.get("definition")
                    == COMPLETION_PROXIMAL_DEFINITION
                    and topology_gate.get("uses_completion_or_performance_outcomes")
                    is False
                    and incomplete_now <= terminal_now + short_now
                    and completion_proximal_now == short_now
                    and deeper_rejected_now <= rejected_now
                    ...
                    raise RuntimeError(
                        "V160 completion-proximal slack short-work evidence changed"
                    )
                terminal += terminal_now
                short += short_now
                completion_proximal += completion_proximal_now
                deeper_rejected += deeper_rejected_now
                ...
                if not (
                    ...
                    raise RuntimeError("V160 route telemetry is incomplete")
                expected = (
                    LOW_EXPERT if float(density) < QUEUE_THRESHOLD else HIGH_EXPERT
                )
                if route.get("selected_expert") != expected:
                    raise RuntimeError("V160 route does not match queue density")
                ...
                if key is None:
                    if source != "not_requested":
                        raise RuntimeError("V160 unrequested reference reason changed")
                    reference_not_requested += 1
                elif source in ("offline_table", "offline_table_nonpositive"):
                    reference_available += 1
                else:
                    raise RuntimeError("V160 bound reference source changed")
                ...
                if not (
                    ...
                    raise RuntimeError("V160 Nash terminal marker changed")
    ...
    if (
        ...
        raise RuntimeError("V160 Nash log cardinality changed")
    if reference_available + reference_not_requested != counts["window"]:
        raise RuntimeError("V160 reference replay coverage changed")
    return {
        "run_id": run_id,
        "seed": run["seed"],
        ...
        "admitted_completion_proximal_short_work_players": completion_proximal,
        "rejected_deeper_nonterminal_short_work_players": deeper_rejected,
...
def blind_audit_v160(root: Path = ROOT) -> dict[str, Any]:
    output = paths(root)
    if output["blind"].exists():
        raise RuntimeError("V160 blind audit already exists")
    _assert_frozen_inputs()
    prepared = read_json(output["prepared"])
    prepared_hash = _assert_hashed(prepared, "receipt_hash", "V160 prepared receipt")
    execution = read_json(output["execution"])
    execution_hash = _assert_hashed(execution, "receipt_hash", "V160 execution receipt")
    manifest = load_and_validate_manifest(output["ready"])
    _validate_product(manifest, references_bound=True)
    ...
    if (
        ...
        raise RuntimeError("V160 exact pairing failed")
    ...
    if [item["seed"] for item in execution["dispatches"]] != list(SEEDS):
        raise RuntimeError("V160 execution order changed")
    ...
    if {path.name for path in canonical_root.iterdir() if path.is_dir()} != {
        ...
        raise RuntimeError("V160 canonical product changed")
    quarantine = output["workspace"] / "quarantine"
    if quarantine.exists() and any(quarantine.rglob("attempt-*")):
        raise RuntimeError("V160 has unexplained quarantined attempts")
    ...
    if len(identities) != 1:
        raise RuntimeError("V160 runtime identity is not unanimous")
    ...
    if not (
        ...
        raise RuntimeError("V160 runtime identity changed")
    terminal = sum(
    ...
    completion_proximal = sum(
        x["admitted_completion_proximal_short_work_players"] for x in audits
    )
    deeper_rejected = sum(
        x["rejected_deeper_nonterminal_short_work_players"] for x in audits
    )
    rejected = sum(
    ...
    if min(
        ...
        raise RuntimeError("V160 mechanism falsification breadth is insufficient")
    ...
    if (
        ...
        or completion_proximal != short
    ):
        raise RuntimeError("V160 mechanism falsification breadth is insufficient")
    document = {
        "schema_version": "NSE_E1_HOMOGENEOUS_COMPLETION_PROXIMAL_SLACK_SHORT_WORK_TERMINAL_PIPELINE_QUEUE8_LOW_BLIND_AUDIT_V160_V1",
        "created_at": utc_now(),
        "status": "pass",
        ...
        "below_threshold_route_windows": low_routes,
        "at_or_above_threshold_route_windows": high_routes,
        "terminal_completion_proximal_deeper_rejection_congested_short_and_over_work_paths_exercised": True,
        "completion_proximal_topology_invariants_passed": True,
...
def _load_candidate(
    ...
    if len(rows) != 3 or [row["seed"] for row in rows] != list(SEEDS):
        raise RuntimeError("V160 candidate result product changed")
    return rows


def reveal_v160(root: Path = ROOT) -> dict[str, Any]:
    output = paths(root)
    if output["result"].exists():
        raise RuntimeError("V160 result already exists")
    blind = read_json(output["blind"])
    blind_hash = _assert_hashed(blind, "blind_audit_hash", "V160 blind audit")
    if not (
        ...
        and blind.get(
            "terminal_completion_proximal_deeper_rejection_congested_short_and_over_work_paths_exercised"
        )
        is True
        and blind.get("completion_proximal_topology_invariants_passed") is True
        and blind.get("work_and_queue_threshold_invariants_passed") is True
        and blind.get("both_routes_exercised") is True
    ):
        raise RuntimeError("V160 blind audit did not authorize reveal")
    manifest = load_and_validate_manifest(output["ready"])
    ...
    mechanism = {
        ...
        for key in (
            ...
            "admitted_completion_proximal_short_work_players",
            "rejected_deeper_nonterminal_short_work_players",
            "rejected_nonterminal_players_with_incomplete_parents",
    ...
    mechanism["pass"] = (
        ...
        and blind["completion_proximal_topology_invariants_passed"]
    ...
    document = {
        "schema_version": "NSE_E1_HOMOGENEOUS_COMPLETION_PROXIMAL_SLACK_SHORT_WORK_TERMINAL_PIPELINE_QUEUE8_LOW_DIAGNOSTIC_RESULT_V160_V1",
        "created_at": utc_now(),
        "training_only": True,
        "formal_results_eligible": False,
        ...
        "disposition": (
            ...
            if passed
            else "retain_all_three_valid_diagnostic_runs_and_retire_completion_proximal_candidate"
        ),
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    if action == "prepare":
        document, key = prepare_v160(), "receipt_hash"
    elif action == "execute":
        document, key = execute_v160(), "receipt_hash"
    elif action == "blind-audit":
        document, key = blind_audit_v160(), "blind_audit_hash"
    else:
        document, key = reveal_v160(), "result_hash"
