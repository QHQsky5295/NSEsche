...
from scripts.reviewer_experiments.protocol.nse_e3_warm_preserve_density_band_training_blind_audit_v101 import (
    ARMS,
...
OUTPUT = ROOT / "training-result-v101.json"
BLIND_AUDIT_FILE_SHA256 = "TO_BE_FROZEN_AFTER_V101_BLIND_AUDIT"
BLIND_AUDIT_HASH = "TO_BE_FROZEN_AFTER_V101_BLIND_AUDIT"
EXPECTED_SEEDS = ("E872", "E873", "E874")
EXPECTED_SCENARIOS = (
    "E3.spike5x50ms",
    "E3.sustained3x200ms",
    "E3.pulse4x4x50ms",
...
ANCHORS = {"E3": "v101-e3-anchor"}
DENSITY_PREFERENCE = {24.0: 0, 32.0: 1}
...
def evaluate_training_rows(
    ...
    ranked = sorted(
        ...
        key=lambda item: (
            ...
            DENSITY_PREFERENCE[float(item[1]["upper_queue_density_threshold"])],
            item[0],
        ),
    )
...
def _validate_blind_audit() -> dict[str, Any]:
    if not BLIND_AUDIT.is_file() or file_hash(BLIND_AUDIT) != BLIND_AUDIT_FILE_SHA256:
        raise RuntimeError("V101 blind audit is missing or changed")
    ...
    if (
        ...
        raise RuntimeError("V101 blind audit does not authorize reveal")
    return blind
...
def _load_rows(blind: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    if len(admitted) != 27:
        raise RuntimeError("V101 blind audit run evidence is incomplete")
    ...
    for arm_id, expected in ARMS.items():
        ...
        if not manifest_path.is_file() or file_hash(manifest_path) != frozen.get(
            ...
            raise RuntimeError(f"V101 frozen manifest changed: {arm_id}")
        manifest = load_and_validate_manifest(manifest_path)
        if manifest.get("manifest_hash") != frozen.get("manifest_hash"):
            raise RuntimeError(f"V101 manifest hash changed: {arm_id}")
        for run in manifest["runs"]:
            ...
            if (
                ...
                != expected["profile"]
                or metadata.get("v101_training_plan_sha256") != PLAN_SHA256
                or metadata.get("v101_training_only") is not True
                or metadata.get("v101_training_seed_metrics_previously_revealed")
                is not False
                or metadata.get("v101_confirmation_seeds_opened") is not False
                or metadata.get("v101_other_unopened_seeds_opened") is not False
                or metadata.get("v101_arm_id") != arm_id
                or metadata.get("v101_arm_role") != expected["role"]
                or metadata.get("v101_upper_queue_density_threshold")
                != expected["upper_queue_density_threshold"]
                or metadata.get("v101_nonterminal_queue_density_floor")
                != expected["nonterminal_queue_density_floor"]
                or metadata.get("v101_warm_admissibility")
                != expected["warm_admissibility"]
            ):
                raise RuntimeError(f"V101 arm identity changed: {run['run_id']}")
            run_id = run["run_id"]
            summary_path = (
                ROOT
                / "runs"
            ...
            if not summary_path.is_file() or file_hash(summary_path) != admitted.get(
                ...
                raise RuntimeError(f"V101 summary differs from blind audit: {run_id}")
            ...
            if (
                ...
                raise RuntimeError(
                    f"V101 summary identity/completion mismatch: {run_id}"
                )
            rows.append(
                {
                    "arm_id": arm_id,
...
def execute_reveal() -> dict[str, Any]:
    if OUTPUT.exists():
        raise RuntimeError(f"V101 training result already exists: {OUTPUT}")
    if not PLAN.is_file() or file_hash(PLAN) != PLAN_SHA256:
        raise RuntimeError("frozen V101 plan changed before reveal")
    blind = _validate_blind_audit()
    rows = _load_rows(blind)
    if len(rows) != 27:
        raise RuntimeError("V101 revealed row count differs from sealed plan")
    ...
    result = {
        "schema_version": "NSE_E3_WARM_PRESERVE_DENSITY_BAND_RESULT_V101_V1",
        "created_at": utc_now(),
        ...
        "selection_rule": {
            ...
            "multiple_passers": (
                ...
                "throughput change, then prefer upper density threshold 24, then arm_id"
            ),
            "frozen_E4": (
                "retain V97 v97-e4-load-guard64 without V101 result-dependent reselection"
            ),
        ...
        "decision": {
            ...
            "next_action": (
                ...
                if joint_pass
                    ...
                    "freeze the complete V101 training cohort, keep E926-E945 and "
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Reveal the frozen V101 training cohort exactly once after blind audit."
    )
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    if args.command != "reveal" or not args.execute:
        parser.error("V101 reveal requires the explicit command: reveal --execute")
    execute_reveal()
