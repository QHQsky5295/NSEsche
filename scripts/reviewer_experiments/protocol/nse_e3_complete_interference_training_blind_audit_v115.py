...
from scripts.reviewer_experiments.protocol.nse_e3_complete_interference_training_prepare_v115 import (
...
OUTPUT = ROOT / "joint-blind-audit-v115-training.json"
RESULT = ROOT / "training-result-v115.json"
...
def _validate_admitted_work_diagnostics(
    ...
    """Validate V115 mechanism evidence without opening any performance summary."""

    path = canonical / "reviewer_records" / run["run_id"] / "nash_metrics.jsonl.gz"
    _require(path.is_file(), f"missing V115 Nash diagnostics: {run['run_id']}")
    ...
    with gzip.open(path, "rt", encoding="utf-8") as handle:
        for line_number, line in enumerate(handle, start=1):
            ...
            _require(type(frame) is int and frame >= 0, "invalid V115 window frame")

            cluster = event.get("cluster")
            _require(isinstance(cluster, dict), "missing V115 cluster diagnostics")
            admitted_fields = (
            ...
            if candidate:
                _require(
                    ...
                    f"V115 admitted-work observation changed: {run['run_id']}:{line_number}",
                )
                ...
                _require(
                    ...
                    f"V115 admitted-work snapshot counts changed: "
                ...
                _require(
                    ...
                    f"V115 admitted-work aggregate changed: {run['run_id']}:{line_number}",
                )
            ...
            _require(
                ...
                f"V115 critical-service contract changed: {run['run_id']}:{line_number}",
            )
            for field in (
                ...
                _require(
                    ...
                    f"V115 critical {field} changed: {run['run_id']}:{line_number}",
                )
            _require(
                not critical["accepted"] or critical["evaluated"],
                f"V115 accepted without evaluation: {run['run_id']}:{line_number}",
            )
            ...
            _require(
                ...
                f"V115 admitted-interference contract changed: "
                f"{run['run_id']}:{line_number}",
            )
            ...
            if invoked and all(value is not None for value in interference_values):
                _require(
                    ...
                    f"V115 admitted-interference evidence changed: "
                    f"{run['run_id']}:{line_number}",
                )
                ...
                _require(
                    ...
                    f"V115 partial admitted-interference evidence: "
                    f"{run['run_id']}:{line_number}",
                )
            if critical["accepted"]:
                _require(
                    ...
                    f"V115 accepted worse admitted interference: "
                    f"{run['run_id']}:{line_number}",
                )
            ...
            if not invoked:
                _require(
                    ...
                    f"V115 non-invoked critical state changed: {run['run_id']}:{line_number}",
                )
                _require(
                    interference["rejected_candidate_count"] == 0
                    and interference["input_unavailable_count"] == 0,
                    f"V115 non-invoked interference state changed: "
                    f"{run['run_id']}:{line_number}",
                )
            if critical["accepted"]:
                values = tuple(
                ...
                _require(
                    ...
                    f"V115 accepted evidence is nonfinite: {run['run_id']}:{line_number}",
                )
                ...
                _require(
                    ...
                    f"V115 accepted admitted-work certificate changed: "
                    f"{run['run_id']}:{line_number}",
                )
            ...
            if not candidate:
                _require(
                    ...
                    f"V115 anchor used causal state: {run['run_id']}:{line_number}",
                )
                continue
            if not gate["history_complete"]:
                _require(
                    gate["baseline_count"] == 0 and gate["recent_count"] == 0,
                    f"V115 incomplete history leaked counts: {run['run_id']}:{line_number}",
                )
                continue
            sufficient = (
            ...
            if sufficient and recent_scaled >= baseline_scaled:
                ...
                _require(
                    ...
                    f"V115 exact threshold did not activate: "
                    f"{run['run_id']}:{line_number}",
                )

    _require(
        counts["windows"] == run["simulation"]["total_frame"],
        f"V115 diagnostic window count changed: {run['run_id']}",
    )
    if candidate:
        _require(
            ...
            f"V115 mechanism was not exercised: {run['run_id']}",
        )
    return {
...
def _verify_tape_catalog() -> tuple[list[dict[str, Any]], dict[str, Any]]:
    _require(TAPES.is_file(), f"missing V115 tape catalog: {TAPES}")
    catalog = read_json(TAPES)
    catalog_hash = _assert_hashed_object(catalog, "catalog_hash", "V115 tapes")
    entries = catalog.get("entries")
    _require(
        isinstance(entries, dict) and len(entries) == 12, "V115 tape count changed"
    )
    evidence = []
...
def _verify_reference_stage_ledgers() -> tuple[dict[str, Path], dict[Path, str]]:
    ...
    for arm_id, expected in ARMS.items():
        ...
        root = _stage_root_from_receipts(
            entries, "receipt_path", expected["run_count"], f"V115 reference {arm_id}"
        )
        roots[arm_id] = root
    ...
    for root, expected_count in expected_by_root.items():
        ...
        _assert_ledger_contract(
            ...
            f"V115 reference stage {root}",
        )
        _require(
            not list((root / "quarantine").glob("**/attempt-*")),
            f"V115 reference quarantine is nonempty: {root}",
        )
...
def _validate_run_boundary(
    ...
    required = {
        ...
        "v115_arm_id": arm_id,
        "v115_arm_role": expected["role"],
        "v115_candidate_profile": expected["profile"],
        "v115_candidate_experiment": "E3",
        "v115_shock_rate_ratio": expected["shock_rate_ratio"],
        "v115_shock_threshold_numerator": expected["shock_threshold_numerator"],
        "v115_shock_threshold_denominator": expected["shock_threshold_denominator"],
        ...
        "v115_shock_activation_horizon_frames": expected[
            "shock_activation_horizon_frames"
        ],
        "v115_nonterminal_queue_density_floor": 8.0 if candidate else None,
        "v115_warm_admissibility": "preserve_anchor_warmness" if candidate else None,
        "v115_load_least_window_certificate_mode": (
            "disabled" if candidate else "not_applicable"
        ),
        "v115_critical_service_ratio_numerator": expected[
            "critical_service_threshold_numerator"
        ],
        "v115_critical_service_ratio_denominator": expected[
    ...
    _require(
        ...
        f"V115 run boundary changed: {run['run_id']}",
    )


def run_blind_audit(output: Path = OUTPUT) -> dict[str, Any]:
    _require(not output.exists(), f"V115 blind audit already exists: {output}")
    _require(not RESULT.exists(), "V115 reveal exists before blind audit")
    _require(PLAN.is_file() and file_hash(PLAN) == PLAN_SHA256, "V115 plan changed")
    plan = read_json(PLAN)
    _require(plan.get("formal_results_eligible") is False, "V115 eligibility changed")
    _require(PREPARED.is_file(), f"missing V115 prepared receipt: {PREPARED}")
    prepared = read_json(PREPARED)
    prepared_hash = _assert_hashed_object(
        prepared, "receipt_hash", "V115 prepared receipt"
    )
    _require(
        ...
        "V115 prepared scientific boundary changed",
    )
    ...
    for arm_id, expected in ARMS.items():
        ...
        _require(paths["ready"].is_file(), f"missing V115 ready manifest: {arm_id}")
        ...
        _require(
            ...
            f"V115 ready boundary changed: {arm_id}",
        )
        ...
        reference_catalog_hash = _assert_hashed_object(
            references, "catalog_hash", f"V115 references {arm_id}"
        )
        ...
        _require(
            ...
            f"V115 reference key set changed: {arm_id}",
        )
        for key, entry in sorted(entries.items()):
            ...
            _require(
                ...
                f"V115 reference evidence changed: {key}",
            )
        ...
        _require(actual_ids == expected_ids, f"V115 canonical set changed: {arm_id}")
        _require(
            not list((workspace / "quarantine").glob("**/attempt-*")),
            f"V115 online quarantine is nonempty: {arm_id}",
        )
        ...
        for run in manifest["runs"]:
            ...
            _require(
                ...
                f"V115 canonical status changed: {run['run_id']}",
            )
            runtime = {
    ...
    _require(len(run_evidence) == 18, "V115 run evidence count must be 18")
    _require(len(reference_evidence) == 18, "V115 reference evidence count must be 18")
    _require(len(tape_evidence) == 12, "V115 tape evidence count must be 12")
    for field, expected in EXPECTED_RUNTIME.items():
        _require(
            runtime_values[field] == {expected},
            f"V115 runtime {field} changed: {runtime_values[field]}",
        )
    ...
    _require(
        ...
        f"V115 runtime git identity is not singular: {git_commits}",
    )
    _require(len(paired_inputs) == 9, "V115 paired input product changed")
    for (scenario, seed), rows in paired_inputs.items():
        _require(len(rows) == 2, f"V115 paired arm count changed: {scenario}/{seed}")
        for field in (
            ...
            _require(
                ...
                f"V115 paired {field} changed: {scenario}/{seed}",
            )
        _require(
            rows[0]["common_hpa_hash"] == EXPECTED_COMMON_HPA_SHA256,
            f"V115 common HPA changed: {scenario}/{seed}",
        )

    output_payload = {
        "schema_version": "NSE_E3_COMPLETE_INTERFERENCE_BLIND_AUDIT_V115_V1",
        "created_at": utc_now(),
        "status": "pass",
