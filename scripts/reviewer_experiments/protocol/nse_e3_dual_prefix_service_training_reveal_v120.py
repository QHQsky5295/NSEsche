...
from scripts.reviewer_experiments.protocol.nse_e3_dual_prefix_service_training_blind_audit_v120 import (
    ARMS,
...
OUTPUT = ROOT / "training-result-v120.json"
BLIND_AUDIT_FILE_SHA256 = (
    "a2fd8ea8a3f0413fdeffd69ad84312c9077afb5ad1a81e637c2bffc544dbf94e"
)
BLIND_AUDIT_HASH = "a6308bfffe7be74b7e612081bc95f814d8d3676c75d7b55bf363f1d97008561f"
EXPECTED_SEEDS = ("E929", "E930", "E931")
EXPECTED_SCENARIOS = (
    "E3.spike5x50ms",
    "E3.sustained3x200ms",
    "E3.pulse4x4x50ms",
...
ANCHOR_ID = "v120-e3-anchor"
CANDIDATE_ID = (
    "v120-e3-causal-shock15-horizon50-admitted-work10-"
    "all-player-dual-prefix-service-directed-initializer-only"
)
...
def _validate_blind_audit() -> dict[str, Any]:
    ...
    if any(
        ...
        raise RuntimeError("V120 blind audit has not been frozen into the reveal code")
    if not BLIND_AUDIT.is_file() or file_hash(BLIND_AUDIT) != BLIND_AUDIT_FILE_SHA256:
        raise RuntimeError("V120 blind audit is missing or changed")
    ...
    if (
        ...
        raise RuntimeError("V120 blind audit does not authorize reveal")
    return blind
...
def _load_rows(blind: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    if len(admitted) != 18:
        raise RuntimeError("V120 blind audit run evidence is incomplete")
    ...
    for arm_id, expected in ARMS.items():
        ...
        if not manifest_path.is_file() or file_hash(manifest_path) != frozen.get(
            ...
            raise RuntimeError(f"V120 frozen manifest changed: {arm_id}")
        manifest = load_and_validate_manifest(manifest_path)
        if manifest.get("manifest_hash") != frozen.get("manifest_hash"):
            raise RuntimeError(f"V120 manifest hash changed: {arm_id}")
        for run in manifest["runs"]:
            ...
            if (
                ...
                != expected["profile"]
                or metadata.get("v120_training_plan_sha256") != PLAN_SHA256
                or metadata.get("v120_training_only") is not True
                or metadata.get("v120_training_seed_metrics_previously_revealed")
                is not False
                or metadata.get("v120_confirmation_seeds_opened") is not False
                or metadata.get("v120_other_unopened_seeds_opened") is not False
                or metadata.get("v120_arm_id") != arm_id
                or metadata.get("v120_arm_role") != expected["role"]
                or metadata.get("v120_shock_rate_ratio") != expected["shock_rate_ratio"]
                or metadata.get("v120_shock_threshold_numerator")
                != expected["shock_threshold_numerator"]
                or metadata.get("v120_shock_threshold_denominator")
                != expected["shock_threshold_denominator"]
                or metadata.get("v120_shock_activation_horizon_frames")
                != expected["shock_activation_horizon_frames"]
                or metadata.get("v120_critical_service_ratio_numerator")
                != expected["critical_service_threshold_numerator"]
                or metadata.get("v120_critical_service_ratio_denominator")
                != expected["critical_service_threshold_denominator"]
                or metadata.get("v120_service_proxy_work_source")
                != (
                    "admitted_pending_plus_all_resident_remaining_and_same_window_projected_cpu_v1"
                    if candidate
                    else "not_applicable"
                )
                or metadata.get("v120_admitted_work_includes_all_blocked_resident")
                is not candidate
                or metadata.get("v120_complete_componentwise_service_pareto")
                is not candidate
                or metadata.get("v120_componentwise_service_scope")
                != ("all_current_players" if candidate else "not_applicable")
                or metadata.get("v120_componentwise_service_comparison")
                != (
                    "every_current_player_alternative_proxy_lte_anchor_plus_epsilon"
                    if candidate
                    else "not_applicable"
                )
                or metadata.get("v120_dual_prefix_anchor_enabled") is not candidate
                or metadata.get("v120_anchor_service_baseline_state")
                != (
                    "independent_exact_anchor_native_order_prefix"
                    if candidate
                    else "not_applicable"
                )
                or metadata.get("v120_candidate_service_state")
                != (
                    "evolving_alternative_native_order_prefix"
                    if candidate
                    else "not_applicable"
                )
                or metadata.get("v120_outcome_fields_drive_policy") is not False
                or metadata.get("v120_scenario_or_burst_label_used_by_policy")
                is not False
                or metadata.get("v120_completion_or_performance_fields_used_by_policy")
                is not False
                or metadata.get("v120_future_arrivals_used_by_policy") is not False
            ):
                raise RuntimeError(f"V120 arm identity changed: {run['run_id']}")
            run_id = run["run_id"]
            summary_path = (
                ROOT
                / "runs"
            ...
            if not summary_path.is_file() or file_hash(summary_path) != admitted.get(
                ...
                raise RuntimeError(f"V120 summary differs from blind audit: {run_id}")
            ...
            if (
                ...
                raise RuntimeError(
                    f"V120 summary identity/completion mismatch: {run_id}"
                )
            rows.append(
                {
                    "arm_id": arm_id,
...
def execute_reveal() -> dict[str, Any]:
    if OUTPUT.exists():
        raise RuntimeError(f"V120 training result already exists: {OUTPUT}")
    if not PLAN.is_file() or file_hash(PLAN) != PLAN_SHA256:
        raise RuntimeError("frozen V120 plan changed before reveal")
    blind = _validate_blind_audit()
    rows = _load_rows(blind)
    if len(rows) != 18:
        raise RuntimeError("V120 revealed row count differs from sealed plan")
    ...
    result = {
        "schema_version": "NSE_E3_DUAL_PREFIX_SERVICE_RESULT_V120_V1",
        "created_at": utc_now(),
        ...
        **evaluation,
        ...
        "decision": {
            ...
            "next_action": (
                "freeze V120 with frozen V97 E4 and separately preregister the "
                "complete E1106-E1125 NSESche confirmation before generating inputs"
                if joint_pass
                else (
                    "freeze the complete V120 training cohort, keep E929-E1105 and "
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Reveal the frozen V120 training cohort exactly once after blind audit."
    )
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    if args.command != "reveal" or not args.execute:
        parser.error("V120 reveal requires the explicit command: reveal --execute")
    execute_reveal()
