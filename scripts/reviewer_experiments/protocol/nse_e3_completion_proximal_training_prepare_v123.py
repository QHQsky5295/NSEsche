...
ROOT = Path("tmp/nse_e3_completion_proximal_training_20260829_v123")
PLAN = Path(
    "scripts/reviewer_experiments/protocol/"
    "nse_e3_completion_proximal_training_plan_v123.json"
)
PLAN_SHA256 = "8f4ebb1d8f12f1d5b6d059f40c82998440a239cc75a5ffc8c2b74f1cbe54b3cc"
V122_RESULT_RECEIPT = Path(
    "scripts/reviewer_experiments/protocol/"
    "nse_e3_terminal_completion_training_result_v122.json"
)
V122_RESULT_RECEIPT_SHA256 = (
    "344898c7ae3bcc9783e7524d57562035acbc1788422c6aadee2b64748178bbcb"
)
V122_RESULT_RECEIPT_HASH = (
    "269cb3932fb7be88b44aa4f13c03ac061572c5506d6fbf2e47e3f644cbfa0498"
)
V122_RESULT_RECEIPT_COMMIT = "23bd31de08e13cdf18b1744263e6aefae4d359de"
...
BINARY_PATH = Path("tmp/nse_v123_build_9528f1a/release/serverless_sim.exe")
BINARY_SHA256 = "d48a9a9e4c8cd312d878b9e9c444dc9f16289a4c4a93aca9c2817f6935e9d8ec"
BINARY_SOURCE_COMMIT = "9528f1ae8e4232121ce84b8a4b085b33e291aaf2"

SOURCE_INITIAL_SEEDS = [f"E{index}" for index in range(1172, 1182)]
SOURCE_CI_SEEDS = [f"E{index}" for index in range(1182, 1192)]
TRAINING_SEED_LIST = ["E1172", "E1173", "E1174"]
TRAINING_SEEDS = set(TRAINING_SEED_LIST)
CONFIRMATION_SEEDS = [f"E{index}" for index in range(1175, 1195)]
PREVIOUS_CONFIRMATION_SEEDS = [f"E{index}" for index in range(932, 1172)]
...
E3_ADMITTED_WORK = (
    ...
    "causal_arrival_shock15_horizon50_admitted_work10_completion_proximal_depth1_initializer_only_"
    "guard64_dual_window_safe_pareto"
)
ARMS = (
    ("v123-e3-anchor", "anchor", E3_ANCHOR, 9),
    (
        "v123-e3-causal-shock15-horizon50-admitted-work10-completion-proximal-depth1-initializer-only",
        "candidate",
        E3_ADMITTED_WORK,
        9,
    ),
...
def _paths(root: Path) -> dict[str, Path]:
    return {
        "config": root / "v123-e3-training-config.json",
        "source": root / "manifest.v123-e1172-e1181-full.unbound.json",
        "capture": root / "manifest.v123-tape-capture.unbound.json",
...
def _assert_frozen_inputs() -> None:
    for path, expected in (
        (PLAN, PLAN_SHA256),
        (V122_RESULT_RECEIPT, V122_RESULT_RECEIPT_SHA256),
        ...
        if not path.is_file() or file_hash(path) != expected:
            raise RuntimeError(f"frozen V123 input is missing or changed: {path}")
    v122 = json.loads(V122_RESULT_RECEIPT.read_text(encoding="utf-8"))
    if v122.get("receipt_hash") != V122_RESULT_RECEIPT_HASH:
        raise RuntimeError("frozen V122 receipt object hash changed")
    ...
    *,
    ...
    marker.update(
        {
            ...
            "v123_binary_source_commit": BINARY_SOURCE_COMMIT,
    ...
    if arm is not None:
        if frozen_model_config is None:
            raise RuntimeError("V123 arm rewrite lacks frozen model binding")
        arm_id, role, profile, expected = arm
        candidate = role == "candidate"
        marker.update(
            {
                "v123_arm_id": arm_id,
                "v123_experiment_id": "E3",
                "v123_role": role,
                "v123_profile": profile,
                "v123_shock_threshold_numerator": 3 if candidate else None,
                "v123_shock_threshold_denominator": 2 if candidate else None,
                "v123_shock_activation_horizon_frames": 50 if candidate else None,
                "v123_critical_service_ratio_numerator": 9 if candidate else None,
                "v123_critical_service_ratio_denominator": 10 if candidate else None,
                ...
                "v123_expected_run_count": expected,
            ...
            metadata.update(
                {
                    "v123_arm_id": arm_id,
                    "v123_arm_role": role,
                    "v123_candidate_profile": profile,
                    "v123_candidate_experiment": "E3",
                    "v123_shock_rate_ratio": "3/2" if candidate else None,
                    "v123_shock_threshold_numerator": 3 if candidate else None,
                    "v123_shock_threshold_denominator": 2 if candidate else None,
                    ...
                    "v123_faasrank_model_artifact_sha256": MODEL_SHA256,
                    "v123_critical_service_ratio_numerator": 9 if candidate else None,
                    "v123_critical_service_ratio_denominator": 10
                    if candidate
                    else None,
...
def prepare_v123(root: Path = ROOT) -> dict:
    _assert_frozen_inputs()
    if root.exists():
        raise RuntimeError(f"refusing to overwrite V123 training root: {root}")
    ...
    capture = derive_integration_smoke_shard(
        paths["source"], capture_ids, purpose=f"V123 fresh parent tapes {PLAN_SHA256}"
    )
    _write(
        paths["capture"],
        _rewrite(capture, purpose=f"V123 fresh parent tapes {PLAN_SHA256}"),
    )
    ...
    for arm in ARMS:
        ...
        template = derive_integration_smoke_shard(
            paths["source"], selected, purpose=f"V123 {arm_id} {PLAN_SHA256}"
        )
        manifest = _rewrite(
            template,
            purpose=f"V123 {arm_id} {PLAN_SHA256}",
            arm=arm,
        ...
        if (
            ...
            raise RuntimeError(f"V123 {arm_id} must have 9 runs/references")
        if {run["seed"] for run in manifest["runs"]} != TRAINING_SEEDS:
            raise RuntimeError(f"V123 {arm_id} escaped E1172-E1174")
        keys = {item["key"] for item in manifest["reference_build_dependencies"]}
        if all_reference_keys & keys:
            raise RuntimeError("V123 arm-specific reference keys unexpectedly overlap")
        all_reference_keys.update(keys)
        _write(arm_path(root, arm_id), manifest)
    if len(all_reference_keys) != 18:
        raise RuntimeError("V123 must declare exactly 18 arm-specific references")
    ...
    receipt = {
        "schema_version": "NSE_E3_COMPLETION_PROXIMAL_PREPARED_V123_V1",
        "created_at": utc_now(),
        "formal_results_eligible": False,
        ...
        "V122_result_receipt_path": str(V122_RESULT_RECEIPT),
        "V122_result_receipt_sha256": V122_RESULT_RECEIPT_SHA256,
        "V122_result_receipt_hash": V122_RESULT_RECEIPT_HASH,
        "V122_result_receipt_commit": V122_RESULT_RECEIPT_COMMIT,
