...
from scripts.reviewer_experiments.protocol.nse_e3_all_native_portfolio_training_execute_v138 import (
    EXECUTION_RECEIPT,
    READY_SCHEDULE,
    ready_manifest_path,
    workspace_path,
)
from scripts.reviewer_experiments.protocol.nse_e3_all_native_portfolio_training_prepare_v138 import (
...
OUTPUT = ROOT / "joint-blind-audit-v138-training.json"
RESULT = ROOT / "training-result-v138.json"
...
def _verify_tapes() -> tuple[list[dict[str, Any]], dict[str, Any]]:
    _require(TAPES.is_file(), f"missing V138 tape catalog: {TAPES}")
    catalog = read_json(TAPES)
    catalog_hash = _assert_hashed_object(catalog, "catalog_hash", "V138 tapes")
    entries = catalog.get("entries")
    _require(
        isinstance(entries, dict) and len(entries) == 12, "V138 tape count changed"
    )
    evidence = []
    for key, entry in sorted(entries.items()):
        info = inspect_tape(Path(entry["path"]))
        _require(info.sha256 == entry["sha256"], f"V138 tape hash changed: {key}")
        _require(info.workload_seed in TRAINING_SEEDS, f"V138 tape seed changed: {key}")
        evidence.append(
            {
                "key": key,
                "sha256": info.sha256,
    ...
    _require(
        ...
        f"V138 capture ledger changed: {counts}",
    )
    _require(
        not list((capture_root / "quarantine").glob("**/attempt-*")),
        "V138 tape capture quarantine is nonempty",
    )
    return evidence, {
...
def _verify_references() -> tuple[list[dict[str, Any]], dict[str, str]]:
    ...
    for arm_id in ARM_IDS:
        ...
        _require(path.is_file(), f"missing V138 reference catalog: {arm_id}")
        catalog = read_json(path)
        catalog_hash = _assert_hashed_object(
            catalog, "catalog_hash", f"V138 references {arm_id}"
        )
        entries = catalog.get("entries")
        _require(
            isinstance(entries, dict) and len(entries) == 9,
            f"V138 reference count changed: {arm_id}",
        )
        root = _stage_root_from_receipts(
            entries, "receipt_path", 9, f"V138 references {arm_id}"
        )
        rows, last_hash = _read_ledger(root / "ledger.jsonl")
        counts = Counter(row["event_type"] for row in rows)
        _require(
            counts == Counter({"reference_build_canonicalized": 9}),
            f"V138 reference ledger changed: {arm_id}/{counts}",
        )
        _require(
            not list((root / "quarantine").glob("**/attempt-*")),
            f"V138 reference quarantine is nonempty: {arm_id}",
        )
        ...
        for key, entry in sorted(entries.items()):
            ...
            _require(
                ...
                f"V138 reference evidence changed: {key}",
            )
            evidence.append(
                {
                    "arm_id": arm_id,
    ...
    _require(len(evidence) == 27, "V138 reference evidence count changed")
...
def _expected_portfolio_selection(
    ...
    _require(rule == "service_welfare_borda", f"unknown V138 portfolio rule: {rule}")
...
def _validate_native_diagnostics(
    ...
    _require(path.is_file(), f"missing V138 Nash diagnostics: {run['run_id']}")
    counts = Counter()
    reasons = Counter()
    with gzip.open(path, "rt", encoding="utf-8") as handle:
        for line_number, line in enumerate(handle, start=1):
            ...
            _require(isinstance(decision, dict), "missing V138 decision diagnostics")
            native = decision.get("native_shadow_anchor")
            _require(isinstance(native, dict), "missing V138 native shadow diagnostics")
            portfolio = decision.get("native_portfolio")
            _require(isinstance(portfolio, dict), "missing V138 portfolio diagnostics")
            players = decision.get("request_function_players")
            _require(
                type(players) is int and players >= 0,
                f"invalid V138 player count: {run['run_id']}:{line_number}",
            )
            _require(
                ...
                f"V138 portfolio boundary changed: {run['run_id']}:{line_number}",
            )
            _require(
                ...
                f"V138 portfolio certificate definition changed: {run['run_id']}:{line_number}",
            )
            if players > 0:
                ...
                _require(
                    ...
                    f"V138 portfolio members changed: {run['run_id']}:{line_number}",
                )
                for candidate in candidates:
                    _require(
                        ...
                        f"V138 portfolio candidate invalid: {run['run_id']}:{line_number}",
                    )
                ...
                for candidate, expected_ranks in zip(candidates, ranks):
                    _require(
                        ...
                        f"V138 portfolio ranks changed: {run['run_id']}:{line_number}",
                    )
                ...
                _require(
                    ...
                    f"V138 portfolio selection changed: {run['run_id']}:{line_number}",
                )
                ...
                _require(
                    ...
                    f"V138 selected native mismatch: {run['run_id']}:{line_number}",
                )
            _require(
                native.get("certificate_uses_completion_outcomes") is False,
                f"V138 native certificate consulted outcomes: {run['run_id']}",
            )
            ...
            _require(
                ...
                f"V138 native service cohort count invalid: {run['run_id']}:{line_number}",
            )
            guard = decision.get("window_safe_guard")
            _require(isinstance(guard, dict), "missing V138 native guard diagnostics")
            reason = guard.get("reason")
            _require(isinstance(reason, str), "invalid V138 native guard reason")
            if players == 0:
                _require(
                    ...
                    f"V138 empty-window diagnostics mismatch: {run['run_id']}:{line_number}",
                )
                reasons[reason] += 1
                continue
            ...
            if initializer_complete and proposal_complete:
                _require(
                    ...
                    f"V138 native service cohort mismatch: {run['run_id']}:{line_number}",
                )
                ...
                _require(
                    ...
                    f"V138 native service certificate incomplete: {run['run_id']}:{line_number}",
                )
                ...
                _require(
                    ...
                    f"V138 native service deltas changed: {run['run_id']}:{line_number}",
                )
            else:
                _require(
                    guard.get("accepted") is False,
                    f"V138 unavailable native service certificate was accepted: {run['run_id']}:{line_number}",
                )
                if not initializer_complete:
                    _require(
                        initializer_players == 0
                        and reason == "initializer_readiness_service_unavailable",
                        f"V138 initializer service fallback mismatch: {run['run_id']}:{line_number}",
                    )
                else:
                    _require(
                        ...
                        f"V138 proposal service fallback mismatch: {run['run_id']}:{line_number}",
                    )
            ...
            _require(
                ...
                == float(welfare_delta),
                f"V138 native welfare certificate changed: {run['run_id']}:{line_number}",
            )
            ...
            _require(
                ...
                f"V138 native guard decision changed: {run['run_id']}:{line_number}",
            )
            reasons[reason] += 1
            if guard.get("accepted") is True:
                ...
                _require(
                    ...
                    f"V138 native accepted window violated certificate: {run['run_id']}:{line_number}",
                )
    _require(counts["windows"] == 4000, f"V138 window count changed: {run['run_id']}")
    _require(
        counts["player_windows"] > 0,
        f"V138 has no native player windows: {run['run_id']}",
    )
    return {
...
def _validate_execution_receipt() -> dict[str, Any]:
    ...
    _require(ready_schedule_path.is_file(), "missing V138 ready schedule")
    _require(execution_path.is_file(), "missing V138 execution receipt")
    schedule = read_json(ready_schedule_path)
    schedule_hash = _assert_hashed_object(
        schedule, "schedule_hash", "V138 ready schedule"
    )
    receipt = read_json(execution_path)
    receipt_hash = _assert_hashed_object(
        receipt, "receipt_hash", "V138 execution receipt"
    )
    _require(
        ...
        "V138 execution receipt boundary changed",
    )
    for scheduled, dispatched in zip(schedule["schedule"], receipt["dispatches"]):
        ...
        _require(
            ...
            f"V138 frozen dispatch changed: {scheduled['ordinal']}",
        )
    return {
...
def run_blind_audit(output: Path = OUTPUT) -> dict[str, Any]:
    _require(not output.exists(), f"V138 blind audit already exists: {output}")
    _require(not RESULT.exists(), "V138 reveal exists before blind audit")
    _require(PLAN.is_file() and file_hash(PLAN) == PLAN_SHA256, "V138 plan changed")
    _require(PREPARED.is_file(), "missing V138 prepared receipt")
    prepared = read_json(PREPARED)
    prepared_hash = _assert_hashed_object(
        prepared, "receipt_hash", "V138 prepared receipt"
    )
    _require(
        ...
        "V138 prepared scientific boundary changed",
    )
    _require(
        ...
        "V138 prepared treatment boundary changed",
    )
    ...
    for manifest_id, expected_count, arm in products:
        ...
        _require(
            ...
            f"V138 ready boundary changed: {manifest_id}",
        )
        ...
        _require(
            ...
            f"V138 pairing changed: {manifest_id}",
        )
        ...
        _require(
            actual_ids == expected_ids, f"V138 canonical set changed: {manifest_id}"
        )
        _require(
            not list((workspace / "quarantine").glob("**/attempt-*")),
            f"V138 online quarantine is nonempty: {manifest_id}",
        )
        ...
        _require(
            ...
            f"V138 online ledger changed: {manifest_id}/{counts}",
        )
        for run in manifest["runs"]:
            if candidate:
                _require(
                    ...
                    == arm["profile"]
                    and run.get("metadata", {}).get("v138_native_portfolio_rule")
                    == arm["portfolio_rule"]
                    and run.get("metadata", {}).get("v138_native_portfolio_members")
                    == NATIVE_KINDS
                    and run.get("metadata", {}).get(
                        "v138_all_native_members_use_all_frontier"
                    )
                    is True
                    and run.get("metadata", {}).get("v138_outcome_fields_drive_policy")
                    is False
                    and isinstance(run.get("reference_dependency"), dict),
                    f"V138 candidate manifest boundary changed: {run['run_id']}",
                )
            else:
                _require(
                    run.get("method") in BASELINE_METHODS
                    and run.get("reference_dependency") is None,
                    f"V138 baseline manifest boundary changed: {run['run_id']}",
                )
            ...
            _require(
                ...
                f"V138 canonical status changed: {run['run_id']}",
            )
    ...
    _require(len(run_evidence) == 108, "V138 run evidence count changed")
    for field, expected in EXPECTED_RUNTIME.items():
        _require(
            runtime_values[field] == {expected},
            f"V138 runtime {field} changed: {runtime_values[field]}",
        )
    ...
    _require(
        ...
        f"V138 runtime git identity changed: {git_commits}",
    )
    _require(len(paired_inputs) == 9, "V138 paired block count changed")
    for (scenario, seed), rows in paired_inputs.items():
        _require(
            len(rows) == 12
            and {row["method_label"] for row in rows} == set(METHOD_LABELS),
            f"V138 block product changed: {scenario}/{seed}",
        )
        for field in (
            ...
            _require(
                ...
                f"V138 paired {field} changed: {scenario}/{seed}",
            )
        _require(
            rows[0]["common_hpa_hash"] == EXPECTED_COMMON_HPA_SHA256,
            f"V138 common HPA changed: {scenario}/{seed}",
        )

    payload = {
        "schema_version": "NSE_E3_ALL_NATIVE_PORTFOLIO_BLIND_AUDIT_V138_V1",
        "created_at": utc_now(),
        "status": "pass",
