...
OUTPUT = ROOT / "joint-blind-audit-v97-training.json"
RESULT = ROOT / "training-result-v97.json"
...
ARMS = {
    ...
    for arm_id, experiment_id, role, profile, threshold, count in PREPARED_ARMS
}
...
def _verify_tape_catalog() -> tuple[list[dict[str, Any]], dict[str, Any]]:
    _require(TAPES.is_file(), f"missing V97 tape catalog: {TAPES}")
    catalog = read_json(TAPES)
    catalog_hash = _assert_hashed_object(catalog, "catalog_hash", "V97 tapes")
    entries = catalog.get("entries")
    _require(isinstance(entries, dict) and len(entries) == 12, "V97 tape count changed")
    evidence = []
    ...
    _require(
        ...
        "V97 tape kind boundary changed",
    )

    _require(E4_TAPES.is_file(), f"missing V97 E4 tape projection: {E4_TAPES}")
    projection = read_json(E4_TAPES)
    projection_hash = _assert_hashed_object(
        projection, "catalog_hash", "V97 E4 tape projection"
    )
    projected = projection.get("entries")
    _require(
        isinstance(projected, dict) and len(projected) == 3,
        "V97 E4 tape projection count changed",
    )
    ...
    _assert_ledger_contract(
        rows, Counter({"capture_canonicalized": 3}), "V97 tape capture"
    )
    _require(
        not list(
        ...
        "V97 tape capture has quarantined attempts",
    )
    return evidence, {
        "catalog_path": str(TAPES),
        "catalog_file_sha256": file_hash(TAPES),
        "catalog_hash": catalog_hash,
        "E4_projection_path": str(E4_TAPES),
        "E4_projection_file_sha256": file_hash(E4_TAPES),
        "E4_projection_hash": projection_hash,
        "capture_ledger_last_hash": last_hash,
    }


def run_blind_audit(output: Path = OUTPUT) -> dict[str, Any]:
    _require(not output.exists(), f"V97 blind audit already exists: {output}")
    _require(not RESULT.exists(), "V97 reveal exists before blind audit")
    _require(PLAN.is_file() and file_hash(PLAN) == PLAN_SHA256, "V97 plan changed")
    plan = read_json(PLAN)
    _require(plan.get("formal_results_eligible") is False, "V97 eligibility changed")
    _require(PREPARED.is_file(), f"missing V97 prepared receipt: {PREPARED}")
    prepared = read_json(PREPARED)
    prepared_hash = _assert_hashed_object(
        prepared, "receipt_hash", "V97 prepared receipt"
    )
    _require(
        ...
        == PREVIOUSLY_RESERVED_SEEDS
        and prepared.get("arm_online_runs") == 36
        and prepared.get("arm_reference_builds") == 36,
        "V97 prepared scientific boundary changed",
    )
    ...
    for arm_id, expected in ARMS.items():
        ...
        _require(paths["ready"].is_file(), f"missing V97 ready manifest: {arm_id}")
        manifest = load_and_validate_manifest(paths["ready"])
        _require(
            len(manifest["runs"]) == expected["run_count"],
            f"V97 run count changed: {arm_id}",
        )
        _require(
            ...
            f"V97 ready flags changed: {arm_id}",
        )
        ...
        reference_catalog_hash = _assert_hashed_object(
            references, "catalog_hash", f"V97 references {arm_id}"
        )
        ...
        _require(
            ...
            f"V97 reference key set changed: {arm_id}",
        )
        for key, entry in sorted(entries.items()):
        ...
        _require(
            ...
            f"V97 reference quarantine is nonempty: {arm_id}",
        )
        ...
        _require(actual_ids == expected_ids, f"V97 canonical set changed: {arm_id}")
        _require(
            not list((workspace / "quarantine").glob("**/attempt-*")),
            f"V97 online quarantine is nonempty: {arm_id}",
        )
        ...
        for run in manifest["runs"]:
            ...
            _require(
                ...
                and metadata.get("v97_arm_id") == arm_id
                and metadata.get("v97_arm_role") == expected["role"]
                and metadata.get("v97_candidate_profile") == expected["profile"]
                and metadata.get("v97_queue_density_threshold")
                == expected["queue_density_threshold"]
                and metadata.get("v97_training_seed_metrics_previously_revealed")
                is False
                and metadata.get("v97_confirmation_seeds_opened") is False
                and metadata.get("v97_previously_reserved_seeds_opened") is False,
                f"V97 run boundary changed: {run['run_id']}",
            )
            ...
            _require(
                ...
                f"V97 canonical status changed: {run['run_id']}",
            )
            runtime = {
    ...
    _require(len(run_evidence) == 36, "V97 run evidence count must be 36")
    _require(len(reference_evidence) == 36, "V97 reference evidence count must be 36")
    _require(len(tape_evidence) == 12, "V97 tape evidence count must be 12")
    for field, expected in EXPECTED_RUNTIME.items():
        _require(
            runtime_values[field] == {expected},
            f"V97 runtime {field} changed: {runtime_values[field]}",
        )
    ...
    _require(
        ...
        f"V97 runtime git identity is not singular: {git_commits}",
    )
    for (experiment_id, scenario, seed), rows in paired_inputs.items():
        _require(
            len(rows) == 3,
            f"V97 paired arm count changed: {experiment_id}/{scenario}/{seed}",
        )
        for field in (
            ...
            _require(
                ...
                f"V97 paired {field} changed: {scenario}/{seed}",
            )
        _require(
            rows[0]["common_hpa_hash"] == EXPECTED_COMMON_HPA_SHA256,
            f"V97 common HPA changed: {scenario}/{seed}",
        )
    ...
    output_payload = {
        "schema_version": "NSE_E3E4_LOW_DENSITY_LOAD_PARETO_BLIND_AUDIT_V97_V1",
        "created_at": utc_now(),
        "status": "pass",
