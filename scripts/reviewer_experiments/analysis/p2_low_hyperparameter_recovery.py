"""Result-blind P2 low-load hyperparameter recovery selection and analysis."""
...
from ..protocol.p2_low_hyperparameter_recovery import (
    P2_LOW_CONTROL,
    P2_LOW_MANIFEST_SCHEMA,
...
SELECTION_SCHEMA = "NSE_P2_LOW_HYPERPARAMETER_ONLINE_SELECTION_V1"
REPORT_SCHEMA = "NSE_P2_LOW_HYPERPARAMETER_GATE_REPORT_V1"
...
def _setting(run: Mapping[str, Any]) -> str:
    if run.get("method") != "sche_nash":
        raise ProtocolValidationError("P2 low screen admits only sche_nash")
    metadata = run.get("metadata")
    if not isinstance(metadata, Mapping):
        raise ProtocolValidationError("P2 low run lacks metadata")
    label = str(metadata.get("parameter_setting", ""))
    if label not in P2_LOW_SETTING_LABELS:
        raise ProtocolValidationError(f"unsupported P2 low setting: {label}")
    return label


def _validate_ready_manifest(path: Path) -> dict[str, Any]:
    manifest = load_and_validate_manifest(path)
    marker = manifest.get("p2_low_hyperparameter_recovery_screen")
    if (
        not isinstance(marker, Mapping)
        or marker.get("schema_version") != P2_LOW_MANIFEST_SCHEMA
        or tuple(marker.get("development_seeds", ()))
        != tuple(P2_LOW_HYPERPARAMETER_RECOVERY_SEEDS)
        or tuple(row.get("label") for row in marker.get("settings", ()))
        != P2_LOW_SETTING_LABELS
        or marker.get("control_setting") != P2_LOW_CONTROL
        or marker.get("operational_refinement") != P2_LOW_OPERATIONAL_REFINEMENT
        ...
        raise ProtocolValidationError(
            "P2 low analysis requires the complete bound 25-run manifest"
        )
    ...
    if not isinstance(runtime, Mapping):
        raise ProtocolValidationError("P2 low runtime receipt is missing")
    _verified_artifact(
        path,
        runtime.get("path"),
        runtime.get("sha256"),
        artifact_cache,
        "P2 low runtime binary",
    )
    for run in manifest["runs"]:
        ...
        if not run_id or run_id in run_ids or len(spec_hash) != 64:
            raise ProtocolValidationError(
                "P2 low run identity is missing or duplicated"
            )
        if spec_hash in spec_hashes:
            raise ProtocolValidationError("P2 low run_spec_hash is duplicated")
        run_ids.add(run_id)
        ...
        if not isinstance(tape_hash, str) or len(tape_hash) != 64:
            raise ProtocolValidationError("P2 low tape is not hash-bound")
        _verified_artifact(
            path, tape.get("path"), tape_hash, artifact_cache, "P2 low workload tape"
        )
        ...
        if (
            ...
            raise ProtocolValidationError("P2 low reference is not fully bound")
        _verified_artifact(
            path,
            dependency.get("path"),
            dependency.get("sha256"),
            artifact_cache,
            "P2 low offline reference",
        )
        _verified_artifact(
            path,
            ...
            artifact_cache,
            "P2 low reference receipt",
        )
        reference_hashes.add(str(dependency["sha256"]))
    if observed != expected:
        raise ProtocolValidationError("P2 low manifest order/product differs")
    if len(tape_groups) != 5 or any(len(group) != 1 for group in tape_groups.values()):
        raise ProtocolValidationError("P2 low settings are not tape-paired by seed")
    if len(tape_hashes) != 5 or len(reference_hashes) != EXPECTED_RUN_COUNT:
        raise ProtocolValidationError("P2 low tape/reference identities are not unique")
    return manifest
        ...
        rows.append(
            {
                ...
                "price_feedback_rate": parameters["price_feedback_rate"],
...
def build_online_selection(manifest_path: Path, canonical_root: Path) -> dict[str, Any]:
    ...
    if canonical_root.exists():
        raise ProtocolValidationError(
            "P2 low canonical result root must not exist at selection freeze"
        )
    report: dict[str, Any] = {
        ...
        "analysis_contract": {
            ...
            "gate_condition_count_per_neighbour": 8,
        },
...
def write_online_selection(
    ...
    if output_path.exists():
        raise ProtocolValidationError("refusing to overwrite P2 low selection")
...
def _validate_selection(
    ...
    if not isinstance(selection, dict):
        raise ProtocolValidationError("P2 low selection is not an object")
    ...
    if (
        ...
        or contract.get("gate_condition_count_per_neighbour") != 8
        ...
        raise ProtocolValidationError("P2 low selection no longer matches inputs")
    return selection
...
def _metric_row(
    ...
    row["price_feedback_rate"] = run["metadata"]["nash_parameters"][
        "price_feedback_rate"
    ]
    row["quality_weight"] = run["metadata"]["nash_parameters"]["quality_weight"]
    return row
...
def _nash_runtime(
    ...
    expected = run["metadata"]["nash_parameters"]
    if len(configs) != 1:
        ...
        issues.extend(
            validate_runtime_contract_config(
                config,
                expected_candidate=P2_LOW_OPERATIONAL_REFINEMENT,
                expected_r0=float(expected["price_feedback_rate"]),
            )
        )
        observed_quality = _number(config.get("quality_weight"), nonnegative=True)
        if observed_quality is None or not math.isclose(
            observed_quality,
            float(expected["quality_weight"]),
            rel_tol=1.0e-6,
            abs_tol=1.0e-8,
        ):
            issues.append("run_config quality weight differs from manifest")
    ...
    for index, window in enumerate(windows):
        ...
        if window.get("global_ready_player_admission") is not None:
            issues.append(f"ready_order window {index} has deferral telemetry")
...
def _series_summary(
    ...
    order = {
        seed: index for index, seed in enumerate(P2_LOW_HYPERPARAMETER_RECOVERY_SEEDS)
    }
...
def evaluate_gate(rows: Sequence[Mapping[str, Any]]) -> dict[str, Any]:
    ...
    tolerance = 1.0e-12
    for label in P2_LOW_SETTING_LABELS:
        if label == P2_LOW_CONTROL:
            continue
        paired_rows = []
        for seed in P2_LOW_HYPERPARAMETER_RECOVERY_SEEDS:
            centre = index.get((seed, P2_LOW_CONTROL), {})
            candidate = index.get((seed, label), {})
            throughput_difference = (
                _number(candidate.get("throughput_requests_per_ms"), nonnegative=True)
                - _number(centre.get("throughput_requests_per_ms"), nonnegative=True)
                if _number(
                    candidate.get("throughput_requests_per_ms"), nonnegative=True
                )
                is not None
                and _number(centre.get("throughput_requests_per_ms"), nonnegative=True)
                is not None
                else None
            )
            qpr_difference = (
                _number(candidate.get("qpr"), nonnegative=True)
                - _number(centre.get("qpr"), nonnegative=True)
                if _number(candidate.get("qpr"), nonnegative=True) is not None
                and _number(centre.get("qpr"), nonnegative=True) is not None
                else None
            )
            ...
            paired_rows.append(
                {
                    ...
                    "qpr_ratio": qpr_ratio,
                    "strict_joint_win": strict_joint_win,
                    "joint_nonloss": joint_nonloss,
                }
            )
        throughput_difference_summary = _series_summary(
            paired_rows, "throughput_difference", signed=True
        )
        qpr_difference_summary = _series_summary(
            paired_rows, "qpr_difference", signed=True
        )
        throughput_mean = aggregate_index[(label, "throughput_requests_per_ms")]["mean"]
        centre_throughput_mean = aggregate_index[
            (P2_LOW_CONTROL, "throughput_requests_per_ms")
        ]["mean"]
        qpr_mean = aggregate_index[(label, "qpr")]["mean"]
        centre_qpr_mean = aggregate_index[(P2_LOW_CONTROL, "qpr")]["mean"]
        completion_mean = aggregate_index[(label, "completion_ratio")]["mean"]
        centre_completion_mean = aggregate_index[(P2_LOW_CONTROL, "completion_ratio")][
            "mean"
        ]
        latency_mean = aggregate_index[(label, "latency_mean_ms")]["mean"]
        centre_latency_mean = aggregate_index[(P2_LOW_CONTROL, "latency_mean_ms")][
            "mean"
        ]
        wall_mean = aggregate_index[(label, "placement_policy_wall_mean_ns")]["mean"]
        centre_wall_mean = aggregate_index[
            (P2_LOW_CONTROL, "placement_policy_wall_mean_ns")
        ]["mean"]
        mean_throughput_ratio = _ratio(throughput_mean, centre_throughput_mean)
        mean_qpr_ratio = _ratio(qpr_mean, centre_qpr_mean)
        latency_ratio = _ratio(latency_mean, centre_latency_mean)
        wall_ratio = _ratio(wall_mean, centre_wall_mean)
        ...
        runtime_rows = [
            ...
            for seed in P2_LOW_HYPERPARAMETER_RECOVERY_SEEDS
            for setting in (P2_LOW_CONTROL, label)
        ]
        conditions = {
            "condition_1_population_and_identity": population_pass,
            "condition_2_viable_dual_mean_effect": (
                mean_throughput_ratio is not None
                and mean_throughput_ratio >= 1.015
                and mean_qpr_ratio is not None
                and mean_qpr_ratio >= 1.11
            ),
            "condition_3_paired_robustness": (
                len(paired_rows) == 5 and joint_wins >= 3 and joint_nonlosses >= 4
            ),
            "condition_4_per_seed_safety": per_seed_floor_pass,
            "condition_5_leave_one_out_stability": all(
                row["passed"] for row in loo_rows
            ),
            "condition_6_completion_and_latency": (
                completion_mean is not None
                and centre_completion_mean is not None
                and completion_mean >= centre_completion_mean - tolerance
                and latency_ratio is not None
                and latency_ratio <= 1.05
            ),
            "condition_7_runtime_reference_integrity": (
                len(runtime_rows) == 10
                and all(row.get("nash_runtime_pass") is True for row in runtime_rows)
            ),
            "condition_8_overhead": wall_ratio is not None and wall_ratio <= 1.50,
        }
    ...
    label_order = {label: index for index, label in enumerate(P2_LOW_SETTING_LABELS)}
    eligible.sort(
        key=lambda report: (
            -min(report["mean_throughput_ratio"], report["mean_qpr_ratio"]),
            -math.sqrt(report["mean_throughput_ratio"] * report["mean_qpr_ratio"]),
            label_order[report["setting"]],
        )
    )
    selected = eligible[0]["setting"] if eligible else None
    status = (
        "complete_p2_low_parameter_candidate_selected_formal_preregistration_authorized"
        if selected is not None
        else "complete_p2_low_parameter_recovery_failed_formal_blocked"
    )
    return {
        "status": status,
        ...
        "eligible_settings": [report["setting"] for report in eligible],
        "selected_setting": selected,
        "formal_confirmation_preregistration_authorized": selected is not None,
        "formal_confirmation_sampling_authorized": False,
...
def analyze(
    ...
    marker = manifest["p2_low_hyperparameter_recovery_screen"]
    ...
    for run in manifest["runs"]:
        ...
        if not isinstance(summary, Mapping):
            raise ProtocolValidationError("P2 low canonical summary is not an object")
        ...
        if not isinstance(audit, Mapping):
            raise ProtocolValidationError("P2 low canonical audit is not an object")
    ...
    report: dict[str, Any] = {
        ...
        "definitions": {
            ...
            "paired_difference": "neighbour_minus_centre_on_identical_seed_tape",
        ...
        "formal_confirmation_preregistration_authorized": gate[
            "formal_confirmation_preregistration_authorized"
        ],
        "formal_confirmation_sampling_authorized": False,
        "formal_progression_authorized": False,
    }
...
def write_analysis(
    ...
    if output_path.exists():
        raise ProtocolValidationError("refusing to overwrite P2 low gate report")
