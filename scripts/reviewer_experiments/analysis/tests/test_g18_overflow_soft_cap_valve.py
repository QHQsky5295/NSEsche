...
from scripts.reviewer_experiments.analysis.g18_overflow_soft_cap_valve import (
    OVERFLOW_SOFT_CAP_RELEASE_VALVE_SCHEMA,
    ZERO_VIOLATION_FIELDS,
    _overflow_soft_cap_telemetry,
    evaluate_gate,
)
...
class G18OverflowSoftCapValveGateTests(unittest.TestCase):
    def _passing_rows(self) -> list[dict[str, object]]:
        throughput = {G18_CONTROL: 1.00, G18_CANDIDATE: 1.10}
        qpr = {G18_CONTROL: 0.040, G18_CANDIDATE: 0.050}
        latency = {G18_CONTROL: 10.0, G18_CANDIDATE: 10.4}
        completion = {G18_CONTROL: 0.80, G18_CANDIDATE: 0.85}
        rows = []
        ordinal = 0
        for load in FORMAL_E1_LOADS:
            for seed in G18_OVERFLOW_SOFT_CAP_VALVE_SEEDS:
                for method in G18_EFFECTIVE_METHODS:
                    ordinal += 1
                    candidate = method == G18_CANDIDATE
    ...
    def test_candidate_passes_all_nine_frozen_conditions(self) -> None:
        ...
        self.assertTrue(report["at_or_below_cap_release_summary"]["passed"])
    ...
    def test_one_joint_win_and_four_joint_nonlosses_pass(self) -> None:
        rows = self._passing_rows()
        for seed in G18_OVERFLOW_SOFT_CAP_VALVE_SEEDS[1:]:
    ...
    def test_two_joint_losses_fail_nonloss_gate(self) -> None:
        rows = self._passing_rows()
        for seed in G18_OVERFLOW_SOFT_CAP_VALVE_SEEDS[:2]:
            row = self._candidate(rows, "low", seed)
    ...
    def test_per_seed_floor_accepts_boundary_and_rejects_below(self) -> None:
        rows = self._passing_rows()
        target = self._candidate(rows, "middle", G18_OVERFLOW_SOFT_CAP_VALVE_SEEDS[0])
    ...
    def test_leave_one_out_allows_one_zero_but_not_one_negative(self) -> None:
        ...
        for seed, difference in zip(G18_OVERFLOW_SOFT_CAP_VALVE_SEEDS, differences):
        ...
        self._candidate(rows, "high", G18_OVERFLOW_SOFT_CAP_VALVE_SEEDS[3])[
    ...
    def test_completion_and_latency_accept_exact_1_05_boundary(self) -> None:
        rows = self._passing_rows()
        for row in rows:
            if row["load"] == "low" and row["effective_method"] == G18_CANDIDATE:
    ...
    def test_activation_requires_material_bounded_seed_in_every_load(self) -> None:
        rows = self._passing_rows()
        for row in rows:
            if row["load"] == "middle" and row["effective_method"] == G18_CANDIDATE:
                row["first_overflow_soft_cap_bounded_window_count"] = 0
        report = self._candidate_report(evaluate_gate(rows))
        self.assertFalse(
            report["conditions"][
    ...
    def test_cross_load_release_evidence_requires_three_runs_and_two_loads(
        ...
        rows = self._passing_rows()
        for row in rows:
            if row["effective_method"] == G18_CANDIDATE:
                row["first_overflow_at_or_below_soft_cap_release_window_count"] = 0
        low_candidates = [
            row
            for row in rows
            if row["load"] == "low" and row["effective_method"] == G18_CANDIDATE
        ...
        for row in low_candidates[:3]:
            row["first_overflow_at_or_below_soft_cap_release_window_count"] = 1
        report = self._candidate_report(evaluate_gate(rows))
        self.assertFalse(report["at_or_below_cap_release_summary"]["passed"])
        target = self._candidate(rows, "middle", G18_OVERFLOW_SOFT_CAP_VALVE_SEEDS[0])
        target["first_overflow_at_or_below_soft_cap_release_window_count"] = 1
        report = self._candidate_report(evaluate_gate(rows))
        self.assertTrue(report["at_or_below_cap_release_summary"]["passed"])
    ...
    def test_each_zero_violation_contract_and_long_deferral_fail_closed(self) -> None:
        for field in ZERO_VIOLATION_FIELDS:
            rows = self._passing_rows()
            candidate = next(
                row for row in rows if row["effective_method"] == G18_CANDIDATE
            )
            candidate[field] = 1
            report = self._candidate_report(evaluate_gate(rows))
            self.assertFalse(
        ...
        rows = self._passing_rows()
        next(row for row in rows if row["effective_method"] == G18_CANDIDATE)[
            "longest_positive_deferral_episode_windows"
        ] = 2
        report = self._candidate_report(evaluate_gate(rows))
        self.assertFalse(
    ...
    def test_paired_report_restores_g18_seed_and_candidate_labels(self) -> None:
        result = evaluate_gate(self._passing_rows())
        self.assertEqual(result["candidate_reports"][0]["candidate"], G18_CANDIDATE)
        self.assertEqual(
            {row["seed"] for row in result["paired_rows"]},
            set(G18_OVERFLOW_SOFT_CAP_VALVE_SEEDS),
        )
        self.assertTrue(
            all(
    ...
    def _telemetry(
        *,
        ...
        return {
            ...
            "soft_cap_applicable": applicable,
            "soft_cap_material_pass": material_pass,
            "soft_cap_numerator": 5,
            "soft_cap_denominator": 4,
            "soft_cap_scaled_node_count": 100,
            "soft_cap_rounded_limit": 25,
            "admission_mode": mode,
    ...
    def _valid_telemetry_records(self):
        telemetry = [
            ...
            self._telemetry(
                feasible=24,
                admitted=24,
                mode="first_overflow_at_or_below_soft_cap_release",
                before=False,
                after=True,
                applicable=True,
                material_pass=False,
            ...
            self._telemetry(
                feasible=26,
                admitted=25,
                mode="first_overflow_soft_cap_bounded",
                before=False,
                after=True,
                applicable=True,
                material_pass=True,
    ...
    def test_soft_cap_valve_accepts_exact_state_and_boundary_sequence(self) -> None:
        result = _overflow_soft_cap_telemetry(self._valid_telemetry_records(), 20)
        self.assertTrue(result["g18_activation_pass"])
        self.assertEqual(
            result["first_overflow_at_or_below_soft_cap_release_window_count"], 2
        )
        self.assertEqual(result["first_overflow_soft_cap_bounded_window_count"], 1)
        self.assertEqual(result["persistent_overflow_release_window_count"], 2)
        self.assertEqual(result["soft_cap_applicable_window_count"], 3)
        self.assertEqual(result["soft_cap_material_pass_window_count"], 1)
    ...
    def test_soft_cap_arithmetic_and_state_drift_fail_closed(self) -> None:
        records = self._valid_telemetry_records()
        records[1][1]["soft_cap_scaled_node_count"] = 97
        records[2][1]["valve_open_before"] = False
        result = _overflow_soft_cap_telemetry(records, 20)
        self.assertFalse(result["g18_activation_pass"])
        self.assertTrue(result["g18_activation_issues"])
