"""Result-blind selection freeze and fail-closed G18 development analysis.
...
the preregistered G18 performance differences and soft-cap-valve activation
...
from ..protocol.g18_overflow_soft_cap_valve import (
    G18_CANDIDATE,
    G18_CONTROL,
...
SELECTION_SCHEMA = "NSE_G18_OVERFLOW_SOFT_CAP_VALVE_ONLINE_SELECTION_V1"
REPORT_SCHEMA = "NSE_G18_OVERFLOW_SOFT_CAP_VALVE_GATE_REPORT_V1"
...
OVERFLOW_SOFT_CAP_RELEASE_VALVE_SCHEMA = (
    "global_feasible_ready_material_first_overflow_ceil_5n_over_4_prefix_then_"
...
G18_TO_SHARED_SEEDS = dict(
    zip(
        G18_OVERFLOW_SOFT_CAP_VALVE_SEEDS,
        shared.G12_GLOBAL_READY_ADMISSION_SEEDS,
    )
)
SHARED_TO_G18_SEEDS = {value: key for key, value in G18_TO_SHARED_SEEDS.items()}
...
def _effective_method(run: Mapping[str, Any]) -> str:
    if run.get("method") != "sche_nash":
        raise ProtocolValidationError("G18 initial stage admits only sche_nash arms")
    metadata = run.get("metadata")
    if not isinstance(metadata, Mapping):
        raise ProtocolValidationError("G18 run lacks candidate metadata")
    method = str(metadata.get("m1_operational_candidate", ""))
    if method not in G18_EFFECTIVE_METHODS:
        raise ProtocolValidationError(f"unsupported G18 effective method: {method}")
    return method


def _validate_ready_manifest(path: Path) -> dict[str, Any]:
    manifest = load_and_validate_manifest(path)
    marker = manifest.get("g18_overflow_soft_cap_valve_development")
    if (
        not isinstance(marker, Mapping)
        or marker.get("schema_version") != G18_MANIFEST_SCHEMA
        or marker.get("control") != G18_CONTROL
        or marker.get("candidate") != G18_CANDIDATE
        or tuple(marker.get("loads", ())) != tuple(FORMAL_E1_LOADS)
        or tuple(marker.get("development_seeds", ()))
        != tuple(G18_OVERFLOW_SOFT_CAP_VALVE_SEEDS)
        ...
        raise ProtocolValidationError("G18 requires the complete bound 30-run manifest")

    expected = {
        (load, seed, method)
        for load in FORMAL_E1_LOADS
        for seed in G18_OVERFLOW_SOFT_CAP_VALVE_SEEDS
        for method in G18_EFFECTIVE_METHODS
    }
    ...
    if not isinstance(runtime, Mapping):
        raise ProtocolValidationError("G18 runtime receipt is missing")
    _verified_artifact(
        path,
        runtime.get("path"),
        runtime.get("sha256"),
    ...
    for run in manifest["runs"]:
        ...
        if identity in observed:
            raise ProtocolValidationError(f"duplicate G18 matrix identity: {identity}")
        observed.add(identity)
        run_id = str(run.get("run_id", ""))
        spec_hash = str(run.get("run_spec_hash", ""))
        if not run_id or run_id in run_ids or len(spec_hash) != 64:
            raise ProtocolValidationError("G18 run identity is missing or duplicated")
        if spec_hash in spec_hashes:
            raise ProtocolValidationError("G18 run_spec_hash is duplicated")
        run_ids.add(run_id)
        ...
        if not isinstance(tape_hash, str) or len(tape_hash) != 64:
            raise ProtocolValidationError("G18 workload tape is not hash-bound")
        _verified_artifact(
            path, tape.get("path"), tape_hash, artifact_cache, "G18 workload tape"
        )
        ...
        if (
            ...
            raise ProtocolValidationError("G18 reference is not fully bound")
        _verified_artifact(
            path,
            dependency.get("path"),
            dependency.get("sha256"),
            artifact_cache,
            "G18 offline reference",
        )
        _verified_artifact(
            path,
            ...
            artifact_cache,
            "G18 reference receipt",
        )
        reference_hashes.add(str(dependency["sha256"]))
    if observed != expected:
        raise ProtocolValidationError("G18 manifest does not cover the exact product")
    if len(tape_groups) != 15 or any(len(group) != 1 for group in tape_groups.values()):
        raise ProtocolValidationError(
            "G18 methods are not paired on one tape per load/seed"
        )
    if len(tape_hashes) != 15 or len(reference_hashes) != EXPECTED_RUN_COUNT:
        raise ProtocolValidationError("G18 tape/reference identities are not unique")
    return manifest
...
def build_online_selection(manifest_path: Path, canonical_root: Path) -> dict[str, Any]:
    ...
    if canonical_root.exists() or canonical_root.parent.exists():
        raise ProtocolValidationError(
            "G18 selection must be frozen before the online result workspace exists"
        )
    report: dict[str, Any] = {
...
def write_online_selection(
    ...
    if output_path.exists():
        raise ProtocolValidationError("refusing to overwrite frozen G18 selection")
...
def _validate_selection(
    ...
    if not isinstance(selection, dict):
        raise ProtocolValidationError("G18 selection is not an object")
    ...
    if not isinstance(stored, str) or object_hash(payload) != stored:
        raise ProtocolValidationError("G18 selection document hash mismatch")
    ...
    if (
        ...
        raise ProtocolValidationError("G18 selection no longer matches frozen inputs")
    return selection
...
def _overflow_soft_cap_telemetry(
    ...
    totals = {
        ...
        "soft_cap_applicable_window_count": 0,
        "soft_cap_material_pass_window_count": 0,
        "first_overflow_at_or_below_soft_cap_release_window_count": 0,
        "first_overflow_soft_cap_bounded_window_count": 0,
    ...
    for index, telemetry, assigned in records:
        ...
        if (
            telemetry.get("schema") != OVERFLOW_SOFT_CAP_RELEASE_VALVE_SCHEMA
            or any(value is None for value in values.values())
            or any(not isinstance(value, bool) for value in booleans.values())
        ):
            issues.append(
                f"window {index} has invalid G18 schema/count/state telemetry"
            )
            continue
        ...
        soft_cap_applicable = bool(booleans["soft_cap_applicable"])
        soft_cap_material_pass = bool(booleans["soft_cap_material_pass"])
        expected_overflow = feasible_ready > node_count
        expected_first_overflow = not expected_valve_open and expected_overflow
        expected_scaled_node_count = 5 * node_count
        expected_rounded_limit = (expected_scaled_node_count + 3) // 4
        expected_material_pass = (
            expected_first_overflow and feasible_ready > expected_rounded_limit
        )
        ...
        elif expected_material_pass:
            expected_mode = "first_overflow_soft_cap_bounded"
        else:
            expected_mode = "first_overflow_at_or_below_soft_cap_release"
        expected_admitted = (
            expected_rounded_limit if expected_material_pass else feasible_ready
        )
        ...
        if (
            ...
            or soft_cap_applicable != expected_first_overflow
            or soft_cap_material_pass != expected_material_pass
            or int(values["soft_cap_numerator"]) != 5
            or int(values["soft_cap_denominator"]) != 4
            or int(values["soft_cap_scaled_node_count"]) != expected_scaled_node_count
            or int(values["soft_cap_rounded_limit"]) != expected_rounded_limit
            ...
            issues.append(f"window {index} violates G18 soft-cap-valve accounting")
        ...
        totals["soft_cap_applicable_window_count"] += int(soft_cap_applicable)
        totals["soft_cap_material_pass_window_count"] += int(soft_cap_material_pass)
        mode_counter = {
            "first_overflow_at_or_below_soft_cap_release": (
                "first_overflow_at_or_below_soft_cap_release_window_count"
            ),
            "first_overflow_soft_cap_bounded": (
                "first_overflow_soft_cap_bounded_window_count"
            ),
    ...
    return {
        "g18_activation_pass": not issues,
        "g18_activation_issues": issues,
...
def _empty_telemetry_result(issues: Sequence[str]) -> dict[str, Any]:
    return {
        "g18_activation_pass": not issues,
        "g18_activation_issues": list(issues),
...
def _nash_runtime(
    ...
    effective = _effective_method(run)
    is_candidate = effective == G18_CANDIDATE
        ...
        if not is_candidate:
            if telemetry is not None:
                activation_issues.append(
                    f"control window {index} unexpectedly has G18 telemetry"
                )
        elif not isinstance(telemetry, Mapping):
            activation_issues.append(f"candidate window {index} lacks G18 telemetry")
        else:
    ...
    if is_candidate:
        telemetry_result = _overflow_soft_cap_telemetry(
            telemetry_records, int(marker["node_count"])
        )
        telemetry_result["g18_activation_issues"] = [
            *activation_issues,
            *telemetry_result["g18_activation_issues"],
        ]
        telemetry_result["g18_activation_pass"] = not telemetry_result[
            "g18_activation_issues"
        ]
    activation_pass = telemetry_result["g18_activation_pass"]
    return {
        **identity,
        "nash_runtime_pass": not issues and activation_pass,
        "nash_runtime_issues": [*issues, *telemetry_result["g18_activation_issues"]],
...
def _to_shared_row(row: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if seed in G18_TO_SHARED_SEEDS:
        adapted["seed"] = G18_TO_SHARED_SEEDS[seed]
    if adapted.get("effective_method") == G18_CANDIDATE:
        adapted["effective_method"] = shared.G12_CANDIDATE
    adapted["g12_activation_pass"] = adapted.get("g18_activation_pass")
    adapted["g12_activation_issues"] = copy.deepcopy(
        adapted.get("g18_activation_issues", [])
    )
    return adapted
...
def _restore_shared_labels(value: Any) -> Any:
    ...
    if value == shared.G12_CANDIDATE:
        return G18_CANDIDATE
    if isinstance(value, str) and value in SHARED_TO_G18_SEEDS:
        return SHARED_TO_G18_SEEDS[value]
    if value == "complete_g12_development_strong_baseline_preregistration_authorized":
        return "complete_g18_development_strong_baseline_preregistration_authorized"
    if value == "complete_g12_development_gate_failed":
        return "complete_g18_development_gate_failed"
    return value
...
def _activation_evidence(
    ...
    activation_rows = []
    at_or_below_cap_runs = []
    persistent_runs = []
    for load in FORMAL_E1_LOADS:
        candidates = [
            index.get((load, seed, G18_CANDIDATE), {})
            for seed in G18_OVERFLOW_SOFT_CAP_VALVE_SEEDS
        ]
        material_deferral = sum(
            (_finite_int(row.get("first_overflow_soft_cap_bounded_window_count")) or 0)
            > 0
            and (_finite_int(row.get("deferred_feasible_players_total")) or 0) > 0
            for row in candidates
        )
        for row in candidates:
            if (
                _finite_int(
                    row.get("first_overflow_at_or_below_soft_cap_release_window_count")
                )
                or 0
            ) > 0:
                at_or_below_cap_runs.append(
                    {
                        "load": load,
                        "seed": row.get("seed"),
        ...
        local_pass = (
            len(candidates) == 5
            and material_deferral >= 1
            and all(
                row.get("g18_activation_pass") is True
                and (
        ...
        activation_rows.append(
            {
                "load": load,
                "material_soft_cap_positive_deferral_seeds": material_deferral,
                "run_evidence": [
                    {
                        "seed": row.get("seed"),
                        "run_id": row.get("run_id"),
                        "first_overflow_soft_cap_bounded_window_count": row.get(
                            "first_overflow_soft_cap_bounded_window_count"
                        ),
                        "first_overflow_at_or_below_soft_cap_release_window_count": row.get(
                            "first_overflow_at_or_below_soft_cap_release_window_count"
                        ),
                        ...
                        "issues": row.get("g18_activation_issues", []),
                    }
                    for row in candidates
                ],
    ...
    below_summary = cross_load_summary(at_or_below_cap_runs)
...
def evaluate_gate(rows: Sequence[Mapping[str, Any]]) -> dict[str, Any]:
    """Evaluate all nine immutable G18 development gate conditions."""
    ...
    for load in FORMAL_E1_LOADS:
        candidate_completion = aggregate_index[
            (load, G18_CANDIDATE, "completion_ratio")
        ]["mean"]
        control_completion = aggregate_index[(load, G18_CONTROL, "completion_ratio")][
            "mean"
        ]
        candidate_latency = aggregate_index[(load, G18_CANDIDATE, "latency_mean_ms")][
            "mean"
        ]
        control_latency = aggregate_index[(load, G18_CONTROL, "latency_mean_ms")][
            "mean"
        ]
    ...
    conditions = {
        ...
        "07_soft_cap_valve_activation_and_nine_zero_violation_contracts": (
    ...
    candidate_report["at_or_below_cap_release_summary"] = below_summary
    ...
    result["status"] = (
        "complete_g18_development_strong_baseline_preregistration_authorized"
        if qualified
        else "complete_g18_development_gate_failed"
    )
    result["selected_candidate"] = G18_CANDIDATE if qualified else None
    ...
    result["shared_statistics_core"] = {
        ...
        "g18_conditions_3_5_6_7_evaluated_separately": True,
    }
    return result
...
def analyze(
    ...
    for run in manifest["runs"]:
        ...
        if not isinstance(summary, Mapping):
            raise ProtocolValidationError("G18 canonical summary is not an object")
        ...
        if not isinstance(audit, Mapping):
            raise ProtocolValidationError("G18 canonical audit is not an object")
    ...
    report: dict[str, Any] = {
        ...
        "definitions": {
            ...
            "overflow_soft_cap_valve": "first_overflow_admits_ceil_5N_over_4_only_when_F_exceeds_that_cap_else_releases_all; adjacent_overflow_releases_all",
...
def write_analysis(
    ...
    if output_path.exists():
        raise ProtocolValidationError("refusing to overwrite G18 gate report")
