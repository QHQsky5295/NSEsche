...
pub fn add_routers(mut router: Router) -> Router {
    async fn get_network_topo(Json(req): Json<GetNetworkTopoReq>) -> (StatusCode, Json<Value>) {
        (
            ...
            Json(
                ApiHandlerImpl
                    .handle_get_network_topo(req)
                    .await
                    .serialize(),
            ),
        )
    }
    router = router.route("/get_network_topo", post(get_network_topo));

    async fn get_env_id() -> (StatusCode, Json<Value>) {
        (
            StatusCode::OK,
            Json(ApiHandlerImpl.handle_get_env_id().await.serialize()),
        )
    }
    router = router.route("/get_env_id", post(get_env_id));

    async fn reset(Json(req): Json<ResetReq>) -> (StatusCode, Json<Value>) {
        (
            StatusCode::OK,
            Json(ApiHandlerImpl.handle_reset(req).await.serialize()),
        )
    }
    router = router.route("/reset", post(reset));

    async fn step(Json(req): Json<StepReq>) -> (StatusCode, Json<Value>) {
        (
            StatusCode::OK,
            Json(ApiHandlerImpl.handle_step(req).await.serialize()),
        )
    }
    router = router.route("/step", post(step));

    async fn rl_step(Json(req): Json<RlStepReq>) -> (StatusCode, Json<Value>) {
        (
            StatusCode::OK,
            Json(ApiHandlerImpl.handle_rl_step(req).await.serialize()),
        )
    }
    router = router.route("/rl_step", post(rl_step));
