                    ...
                    match dag.dag_inner.add_edge(
                        ...
                        *task_node,
                        ...
                        Err(e) => {
                            // 处理循环依赖错误，记录警告但不崩溃
                            log::warn!(
                                "Detected cycle when adding edge from task {} to task {}: {:?}. Skipping this dependency to maintain DAG structure.",
                                dependency_id, task.task_id, e
                            );
                            // 跳过这个依赖关系，继续处理其他依赖
                        }
                    }
...
impl SimEnv {
    ...
    fn fn_gen_rand_fn(&self) -> FnId {
        ...
        let (cpu, out_put_size) = if self.help.config().fntype_cpu() {
            (self.env_rand_f(2.0, 400.0), self.env_rand_f(0.1, 80.0))
        } else if self.help.config().fntype_data() {
            (self.env_rand_f(20.0, 3000.0), self.env_rand_f(10.0, 300.0))
        } else {
        ...
        self.core.fns_mut().push(Func {
            fn_id: id,
            cpu,
            mem: self.env_rand_f(10.0, 2000.0),
            out_put_size,
            nodes: HashSet::new(),
            cold_start_container_mem_use: 100.0,
            cold_start_container_cpu_use: self.env_rand_f(0.1, 2.0),
    ...
    pub fn fn_gen_fn_dags(&self, _env: &SimEnv) {
        ...
        self.help.config().dag_type_mix() {
            for i in 0..50 {
                if i >= 5 {
                    // 随机确定每个图中节点的数量
                    let mapcnt = env.env_rand_i(4, 10); //2-4
                    let dag_i = env.core.dags().len();
...
mod tests {
    ...
    fn print_node_info(dag: &FnDAG, node_idx: NodeIndex) {
        // 正确访问节点权重
        let fn_id = *dag.dag_inner.node_weight(node_idx).unwrap();
