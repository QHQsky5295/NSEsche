...
|---|---|---|---|---|
| R1-1 | Eq./code audit; 20 retained NSESche solver streams | active-window convergence distributions | weighted-potential proposition, assumptions, zero-complexity case, finite bound | P1 |
| R1-2 | bounded formula/control-path audit | none beyond R1-1 P1 | separate boundedness from convergence; delete overclaim | writing + P1 |
| R1-3 | fixed-baseline Eq. (19), common multiplier, Eq. (20) log contract | none | prove conditional price-ratio invariance and monotone/saturating `gamma`; state limits | writing |
| R2-1 | inner/outer round and fixed-point fields | same P1 convergence extraction | formal definitions of inner PNE and conditional outer placement fixed point | P1 |
| R2-2 | 120 references, 1,794.156 s total build wall time, 15 negative/0 zero rows, per-window lookup fields | 300 exact-small states and retained-log coverage/overhead | define state/profile key, update trigger, nonpositive fallback, large-state empirical status | P1 |
...
| R3-1 | exact solver fields in every retained window | same P1 extraction | condition/bound theorem and empirical non-convergence table | P1 |
